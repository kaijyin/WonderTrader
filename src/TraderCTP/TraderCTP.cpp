...
TraderCTP::TraderCTP()
    : m_pUserAPI(NULL), m_mapPosition(NULL), m_ayOrders(NULL), m_ayTrades(NULL),
      ...
      m_iRequestID(0), m_bQuickStart(false), m_bInQuery(false),
      m_bStopped(false), m_lastQryTime(0) {}
...
void TraderCTP::connect() {
  ...
  for (std::string front : m_strFront) {
    ...
    m_sink->handleTraderLog(LL_INFO,
                            fmtutil::format("registerFront: {}", front));
  }
...
uint32_t TraderCTP::genRequestID() { return m_iRequestID.fetch_add(1) + 1; }

int TraderCTP::login(const char *user, const char *pass,
...
int TraderCTP::doLogin() {
  ...
  if (iResult != 0) {
    write_log(m_sink, LL_ERROR, "[TraderCTP] Sending login request failed: {}",
              iResult);
  }
...
int TraderCTP::logout() {
  ...
  if (iResult != 0) {
    write_log(m_sink, LL_ERROR, "[TraderCTP] Sending logout request failed: {}",
              iResult);
  }
...
int TraderCTP::orderInsert(WTSEntrust *entrust) {
  ...
  if (strlen(entrust->getUserTag()) > 0) {
    m_eidCache.put(
        ...
        [this](const char *message) { write_log(m_sink, LL_WARN, message); });
  }

  /// 报单价格条件: 限价
  req.OrderPriceType = wrapPriceType(entrust->getPriceType(),
                                     strcmp(entrust->getExchg(), "CFFEX") == 0);
  /// 买卖方向:
  req.Direction =
...
int TraderCTP::orderAction(WTSEntrustAction *action) {
  ...
  if (iResult != 0) {
    write_log(m_sink, LL_ERROR, "[TraderCTP] Sending cancel request failed: {}",
              iResult);
  }
...
void TraderCTP::OnRspAuthenticate(
...
void TraderCTP::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,
  ...
  if (!IsErrorRspInfo(pRspInfo)) {
    ...
    write_log(m_sink, LL_INFO,
              "[TraderCTP][{}-{}] Login succeed, AppID: {}, Sessionid: {}, "
              "login time: {}...",
    ...
    write_log(m_sink, LL_ERROR, "[TraderCTP][{}-{}] Login failed: {}",
              m_strBroker.c_str(), m_strUser.c_str(), pRspInfo->ErrorMsg);
    m_wrapperState = WS_LOGINFAILED;
...
void TraderCTP::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout,
  ...
  if (!IsErrorRspInfo(pRspInfo)) {
    if (pSettlementInfoConfirm != NULL) {
      ...
      if (uConfirmDate >= m_lDate) {
        ...
        write_log(m_sink, LL_INFO,
                  "[TraderCTP][{}-{}] Trading channel initialized...",
                  m_strBroker.c_str(), m_strUser.c_str());
        ...
        write_log(m_sink, LL_INFO,
                  "[TraderCTP][{}-{}] Confirming settlement data...",
                  m_strBroker.c_str(), m_strUser.c_str());
        confirm();
      }
    } else {
...
void TraderCTP::OnRspSettlementInfoConfirm(
    CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm,
    CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
...
void TraderCTP::OnRspOrderAction(
...
void TraderCTP::OnRspQryTradingAccount(
...
void TraderCTP::OnRspQryInvestorPosition(
...
void TraderCTP::OnRspQrySettlementInfo(
  ...
  if (bIsLast && !m_strSettleInfo.empty()) {
    m_sink->onRspSettlementInfo(atoi(pSettlementInfo->TradingDay),
...
void TraderCTP::OnRspError(CThostFtdcRspInfoField *pRspInfo, int nRequestID,
                           bool bIsLast) {
  m_sink->handleTraderLog(
      LL_ERROR, fmtutil::format("{} rsp error: {} : {}", nRequestID,
...
int TraderCTP::wrapDirectionType(WTSDirectionType dirType,
...
WTSDirectionType
TraderCTP::wrapDirectionType(TThostFtdcDirectionType dirType,
...
WTSDirectionType
TraderCTP::wrapPosDirection(TThostFtdcPosiDirectionType dirType) {
...
int TraderCTP::wrapPriceType(WTSPriceType priceType,
                             bool isCFFEX /* = false */) {
...
WTSTimeCondition
TraderCTP::wrapTimeCondition(TThostFtdcTimeConditionType timeCond) {
...
WTSEntrustAction *
TraderCTP::makeAction(CThostFtdcInputOrderActionField *actionField) {
  WTSEntrustAction *pRet = WTSEntrustAction::create(actionField->InstrumentID,
                                                    actionField->ExchangeID);
  pRet->setOrderID(actionField->OrderSysID);

  generateEntrustID(pRet->getEntrustID(), actionField->FrontID,
                    actionField->SessionID, atoi(actionField->OrderRef));
...
WTSError *TraderCTP::makeError(CThostFtdcRspInfoField *rspInfo,
                               WTSErroCode ec /* = WEC_NONE */) {
  WTSError *pRet = WTSError::create(
      ec, fmtutil::format("{}({})", rspInfo->ErrorMsg, rspInfo->ErrorID));
  return pRet;
}
...
WTSTradeInfo *TraderCTP::makeTradeInfo(CThostFtdcTradeField *tradeField) {
  ...
  WTSTradeInfo *pRet =
      WTSTradeInfo::create(tradeField->InstrumentID, contract->getExchg());
  ...
  double amount = contract->getCommInfo()->getVolScale() * tradeField->Volume *
                  pRet->getPrice();
...
void TraderCTP::generateEntrustID(char *buffer, uint32_t frontid,
                                  uint32_t sessionid, uint32_t orderRef) {
  fmtutil::format_to(buffer, "{:06d}#{:010d}#{:06d}", frontid, sessionid,
                     orderRef);
}

bool TraderCTP::extractEntrustID(const char *entrustid, uint32_t &frontid,
                                 uint32_t &sessionid, uint32_t &orderRef) {
...
void TraderCTP::OnRtnInstrumentStatus(
...
bool TraderCTP::isConnected() { return (m_wrapperState == WS_ALLREADY); }
...
int TraderCTP::queryConfirm() {
...

  {
    ...
    m_queQuery.push([this]() {
      ...
      int iResult =
          m_pUserAPI->ReqQrySettlementInfoConfirm(&req, genRequestID());
      if (iResult != 0) {
        write_log(m_sink, LL_ERROR,
                  "[TraderCTP][{}-{}] Sending query of settlement data "
