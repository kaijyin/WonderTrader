...
void on_getbar(CtxHandler ctxid, const char *code, const char *period,
               WTSBarStruct *bar, WtUInt32 count, bool isLast) {
  if (bar)
    fmt::print("on_getbar@{}.{}\n", bar->date, bar->time);
  else
    int x = 1;
...
void on_gettick(CtxHandler ctxid, const char *code, WTSTickStruct *tick,
                bool isLast) {
  if (tick)
...
void on_bar(CtxHandler ctxid, const char *code, const char *period,
            WTSBarStruct *newBar) {
  // fmt::print("on_bar\n");
}
