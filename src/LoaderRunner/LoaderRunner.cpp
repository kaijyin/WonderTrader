...
int main(int argc, char *argv[]) {
  ...
  auto mParam = opt("-m", "--module",
                    "loader module filepath, CTPLoader.dll for win and "
                    "libCTPLoader.so for linux as default",
                    false);
  auto cParam =
      opt("-c", "--config", "configure filepath, config.ini as default", false);
  ...
  if (handle == NULL) {
    ...
    if (runner == NULL) {
      ...
      std::string cfgfile =
          cParam->exists() ? cParam->get<std::string>() : "config.ini";
