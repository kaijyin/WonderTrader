...
bool WtSimpDataMgr::initStore(WTSVariant *cfg) {
  ...
  if (funcCreator == NULL) {
    WTSLogger::error("Data reader {} loading failed: entrance function "
...
IHotMgr *WtSimpDataMgr::get_hot_mgr() { return _runner->get_hot_mgr(); }

uint32_t WtSimpDataMgr::get_date() { return _cur_date; }

uint32_t WtSimpDataMgr::get_min_time() { return _cur_min_time; }

uint32_t WtSimpDataMgr::get_secs() { return _cur_secs; }

void WtSimpDataMgr::reader_log(WTSLogLevel ll, const char *message) {
  WTSLogger::log_raw(ll, message);
}

void WtSimpDataMgr::on_bar(const char *code, WTSKlinePeriod period,
                           WTSBarStruct *newBar) {}

void WtSimpDataMgr::handle_push_quote(const char *stdCode,
                                      WTSTickData *curTick) {
  if (curTick == NULL)
    return;
  ...
  if (_cur_date != 0 &&
      (uDate < _cur_date || (uDate == _cur_date && uTime < _cur_act_time))) {
    return;
  }
...
WTSKlineSlice *WtSimpDataMgr::get_kline_slice(const char *stdCode,
