...
bool WtExecRunner::init(const char *logCfg /* = "logcfgexec.json" */,
                        bool isFile /* = true */) {
#ifdef _MSC_VER
...
WTSSessionInfo *WtExecRunner::get_session_info(const char *sid,
                                               bool isCode /* = true */) {
  if (!isCode)
...
void WtExecRunner::release() { WTSLogger::stop(); }
...
uint64_t WtExecRunner::get_real_time() {
  return TimeUtils::makeTime(_data_mgr.get_date(),
                             _data_mgr.get_raw_time() * 100000 +
                                 _data_mgr.get_secs());
}
...
uint32_t WtExecRunner::get_trading_day() { return _data_mgr.get_trading_day(); }