 ...
 *	顶点A5接口的要点：
 *	1
 *Fix_GetItem需要传入一个buffer长度的参数，这个参数每次调用以后会被改写，必须重置为buffer长度，下次调用才能成功
 *	2
 *回报推送混乱，订单回报有时候不会推送，成交以后、撤单以后都不会推送原订单的最新状态，所以需要手动做以下处理：
 *		a 下单成功以后直接模拟一个订单回报，状态“未成交”
 *		b
 *本地缓存未完成订单，当撤单回报来的时候，读取原订单的数据，然后修改原订单的状态，再模拟一个原订单的回报
 *		c
 *收到成交回报以后，也需要读取原订单的数据，修改原订单的状态和成交数量、剩余数量，再模拟一个原订单的回报
 */
void inst_hlp() {}
...
static bool cb_global(HANDLE_CONN conn, HANDLE_SESSION sess,
                      int nResv) { // 异步run函数回调原型
  int code = -1;
  std::string errmsg;
  char buf[256];
  int len = 256;
...
// 订单回调
bool cb_rtn_order(HANDLE_CONN hconn, HANDLE_SESSION hsess, int64_t subid,
                  void *pData) {
  /**
  ...
  */
  ...
  if (type ==
      FIX_PUB_TYPE_COMM) { // 收到发布,一般用户在此应快速处理，如进队列，不应积压
...
// 成交回调
bool cb_rtn_trade(HANDLE_CONN hconn, HANDLE_SESSION hsess, int64_t subid,
                  void *pData) {
  /**
  调用Fix_GetPublishType先判断事件。
  */
...
TraderDD::TraderDD()
    : m_hConn(NULL), m_wrapperState(WS_NOTLOGIN), m_uLastQryTime(0),
      m_bInQuery(false), m_strandIO(NULL), m_lastQryTime(0), m_orderRef(1),
      m_lDate(0), m_mapLives(NULL) {
...
void TraderDD::reconnect() {
  ...
  m_hConn = Fix_Connect(m_strFront.c_str(), m_strCommUser.c_str(),
                        m_strCommPass.c_str(), 20000);
  if (m_hConn == NULL) {
    if (m_traderSink) {
    ...
    StdThreadPtr thrd(new StdThread([this]() {
      ...
      if (m_traderSink)
        write_log(m_traderSink, LL_WARN, "[TraderDD]账号{}正在重连……",
                  m_strUser.c_str());
      reconnect();
    }));
    return;
  ...
  //	if (!Fix_SubscribeByToken(sess, subid, m_strUser.c_str(),
  //m_strToken.c_str(), (void *)cb_rtn_trade, this))   //
  ...
  //"[TraderDD]订阅成交回报失败：{}({})", buf, code); 		Fix_ReleaseSession(sess);
  //	}
  ...
  //	m_strToken = GetItem(sess, FID_TOKEN);
  //	if (!Fix_SubscribeByToken(sess, subid, m_strUser.c_str(),
  //m_strToken.c_str(), (void *)cb_rtn_order, this))  // Fix_SubscribeByCustomer
  ...
  //		if (m_traderSink)
  //			write_log(m_traderSink, LL_ERROR,
  //"[TraderDD]订阅委托回报失败：{}({})", buf, code); 		Fix_ReleaseSession(sess);
  //		return;
  //	}
  //	Fix_ReleaseSession(sess);
  //}
...
int TraderDD::login(const char *user, const char *pass,
...
void TraderDD::qryGDNo() {
  m_strandIO->post([this]() {
    ...
    if (code < 0) {
      ...
      char buf[256] = {0};
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR, "[TraderDD]股东号查询失败: {}({})", buf,
                code);
    } else {
      ...
      // m_traderSink->handleTraderLog(LL_ERROR,
      // "[TraderDD]账户数据初始化完成..."); m_wrapperState = WS_ALLREADY;
...
void TraderDD::qryZJZH() {
  m_strandIO->post([this]() {
    ...
    if (code < 0) {
      ...
      char buf[256] = {0};
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR, "[TraderDD]资金账户查询失败: {}({})",
                buf, code);
...
void TraderDD::doLogin() {
  m_strandIO->post([this]() {
    ...
    // 订阅订单回报
    {
      ...
      if (!Fix_SubscribeByToken(sess, subid, m_strUser.c_str(),
                                m_strToken.c_str(), (void *)cb_rtn_order,
                                this)) // Fix_SubscribeByCustomer
      { // session 订阅id，客户号，交易密码，回调函数，pData(回调中传回)
        int len = 128;
        code = Fix_GetCode(sess);
        len = 256;
    ...
    // 订阅成交回报
    {
      ...
      // m_strToken = GetItem(sess, FID_TOKEN);
      if (!Fix_SubscribeByToken(sess, subid, m_strUser.c_str(),
                                m_strToken.c_str(), (void *)cb_rtn_trade,
                                this)) //  Fix_SubscribeByCustomer
      {
        int len = 128;
        code = Fix_GetCode(sess);
...
int TraderDD::orderInsert(WTSEntrust *entrust) {
  ...
  m_strandIO->post([this, entrust]() {
    ...
    if (code < 0) {
      ...
      char buf[256] = {0};
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR, "[TraderDD]委托指令发送失败: {}({})",
                buf, code);
...
int TraderDD::orderAction(WTSEntrustAction *action) {
  ...
  m_strandIO->post([this, action]() {
      ...
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR,
                "[TraderDD]撤单指令发送失败，错误信息：{}({})", buf, code);
...
int TraderDD::queryAccount() {
  ...
  m_queQuery.push([this]() {
    ...
    if (code < 0) {
      ...
      char buf[256] = {0};
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR, "[TraderDD]可用资金查询失败: {}({})",
                buf, code);
    } else {
      ...
      for (int i = 0; i < row; i++) {
        ...
        fundInfo->setBalance(Fix_GetDouble(sess, FID_ZQSZ, i) +
                             fundInfo->getAvailable());
...
int TraderDD::queryPositions() {
  ...
  m_queQuery.push([this]() {
    ...
    if (!ret || code < 0) {
      ...
      char buf[256] = {0};
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR, "[TraderDD]用户持仓查询失败: {}({})",
                buf, code);
    } else {
      ...
      for (int i = 0; i < row; i++) {
        ...
        if (strncmp(buf, "0", 1) ==
            0) // 流通类型,1 流通 0非流通，如果是非流通则过掉
          continue;

        WTSContractInfo *contract =
            m_bdMgr->getContract(code.c_str(), exchg.c_str());
        if (contract) {
          WTSCommodityInfo *commInfo = contract->getCommInfo();
          WTSPositionItem *pInfo = WTSPositionItem::create(
              code.c_str(), commInfo->getCurrency(), exchg.c_str());
          pInfo->setDirection(WDT_LONG);

          double prevol =
              Fix_GetInt64(sess, FID_ZQSL, i); // 昨天的持仓，今天是不会变的
          double newvol =
              Fix_GetInt64(sess, FID_JCCL, i); // 今天的持仓，这个是真实持仓
          ...
          double cost = Fix_GetDouble(sess, FID_TBCBJ,
                                      i); // atof(fields[FID_TBCBJ].c_str());
...
void TraderDD::OnRtnOrder(HANDLE_CONN hconn, HANDLE_SESSION hsess) {
  ...
  if (!isCancel) {
    ...
    if (ordInfo == NULL) {
      ...
      ordInfo->setOffsetType(Fix_GetDouble(hsess, FID_JYLB) == 1 ? WOT_OPEN
                                                                 : WOT_CLOSE);
      ...
      std::string usertag =
          m_iniHelper.readString(ENTRUST_SECTION, ordInfo->getEntrustID(), "");
      if (usertag.empty()) {
    ...
    // std::cout << "push order report: " << " code: " << ordInfo->getCode() <<
    // " exchg: " << ordInfo->getExchg() << " vol: " << ordInfo->getVolume() <<
    // " price: " << ordInfo->getPrice() << " msg: " << buf << std::endl;
  } else {
    len = 256;
...
void TraderDD::OnRtnTrade(HANDLE_CONN hconn, HANDLE_SESSION hsess) {
  ...
  trdInfo->setOffsetType(Fix_GetDouble(hsess, FID_JYLB) == 1 ? WOT_OPEN
                                                             : WOT_CLOSE);
  ...
  std::cout << "code: " << trdInfo->getCode()
            << " exchg: " << trdInfo->getExchg()
            << " vol: " << trdInfo->getVolume()
            << " price: " << trdInfo->getPrice() << std::endl;
...
int TraderDD::queryOrders() {
  ...
  m_queQuery.push([this]() {
    ...
    if (code < 0) {
      ...
      char buf[256] = {0};
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR, "[TraderDD]用户订单查询失败: {}({})",
                buf, code);
    } else {
      ...
      for (int i = 0; i < row; i++) {
        ...
        if (contract) {
          ...
          ordInfo->setDirection(WDT_LONG);
          ordInfo->setPriceType(WPT_LIMITPRICE);
          ordInfo->setOffsetType(
              Fix_GetInt64(sess, FID_JYLB, i) == 1 ? WOT_OPEN : WOT_CLOSE);
          ...
          // std::cout << "code: " << ordInfo->getCode() << "  exchg: " <<
          // ordInfo->getExchg() << "  price: " << ordInfo->getPrice() << "
          // volume: " << ordInfo->getVolume() << std::endl;
...
int TraderDD::queryTrades() {
  ...
  m_queQuery.push([this]() {
    ...
    if (code < 0) {
      ...
      char buf[256] = {0};
      Fix_GetErrMsg(sess, buf, len);
      write_log(m_traderSink, LL_ERROR, "[TraderDD]用户成交查询失败: {}({})",
                buf, code);
    } else {
      ...
      for (int i = 0; i < row; i++) {
        ...
        if (contract) {
          ...
          WTSTradeInfo *trdInfo =
              WTSTradeInfo::create(code.c_str(), exchg.c_str());
          ...
          trdInfo->setOffsetType(strncpy(buf, "1", 1) == 0 ? WOT_OPEN
                                                           : WOT_CLOSE);
          nSize = 256;
          Fix_GetItem(sess, FID_WTH, buf, nSize, i);
          ...
          // std::cout << "code: " << trdInfo->getCode() << "  exchg: " <<
          // trdInfo->getExchg() << "  price: " << trdInfo->getPrice() << "
          // volume: " << trdInfo->getVolume() << std::endl;
...
bool TraderDD::isConnected() { return (m_wrapperState == WS_ALLREADY); }
