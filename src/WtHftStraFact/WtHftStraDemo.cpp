...
WtHftStraDemo::WtHftStraDemo(const char *id)
    : HftStrategy(id), _last_tick(NULL), _last_entry_time(UINT64_MAX),
      _channel_ready(false), _last_calc_time(0), _stock(false), _unit(1),
      _cancel_cnt(0), _reserved(0) {}
...
void WtHftStraDemo::do_calc(IHftStraCtx *ctx) {
  ...
  uint64_t now = TimeUtils::makeTime(
      ctx->stra_get_date(),
      ctx->stra_get_time() * 100000 +
          ctx->stra_get_secs()); //(uint64_t)ctx->stra_get_date()*1000000000 +
                                 //(uint64_t)ctx->stra_get_time()*100000 +
                                 //ctx->stra_get_secs();
  ...
  uint32_t curMin = curTick->actiontime() /
  ...
  // 计算部分
  double pxInThry = (curTick->bidprice(0) * curTick->askqty(0) +
...
void WtHftStraDemo::on_tick(IHftStraCtx *ctx, const char *code,
                            WTSTickData *newTick) {
...
void WtHftStraDemo::on_bar(IHftStraCtx *ctx, const char *code,
                           const char *period, uint32_t times,
                           WTSBarStruct *newBar) {}

void WtHftStraDemo::on_trade(IHftStraCtx *ctx, uint32_t localid,
                             const char *stdCode, bool isBuy, double qty,
                             double price, const char *userTag) {
  do_calc(ctx);
}

void WtHftStraDemo::on_position(IHftStraCtx *ctx, const char *stdCode,
                                bool isLong, double prevol, double preavail,
                                double newvol, double newavail) {}

void WtHftStraDemo::on_order(IHftStraCtx *ctx, uint32_t localid,
