...
void TraderAdapter::onLoginResult(bool bSucc, const char *msg,
  ...
  if (!bSucc) {
    ...
    _date = tradingdate;
    WTSLogger::info("[{}]交易账号登录成功, 当前交易日:{}", _id.c_str(),
                    tradingdate);
...
void TraderAdapter::onRspAccount(WTSArray *ayAccounts) {
  if (ayAccounts && ayAccounts->size() > 0) {
    for (std::size_t idx = 0; idx < ayAccounts->size(); idx++) {
      ...
      getDumper().on_account(_id.c_str(), _date, accInfo->getCurrency(),
                             accInfo->getPreBalance(), accInfo->getBalance(),
                             accInfo->getBalance() + accInfo->getDynProfit(),
                             accInfo->getCloseProfit(), accInfo->getDynProfit(),
                             accInfo->getCommission(), accInfo->getMargin(),
                             accInfo->getDeposit(), accInfo->getWithdraw(),
                             idx == ayAccounts->size() - 1);
    }
  }
...
void TraderAdapter::onPushTrade(WTSTradeInfo *tInfo) {
  WTSContractInfo *cInfo =
      _bd_mgr->getContract(tInfo->getCode(), tInfo->getExchg());
  if (cInfo == NULL)
    return;

  getDumper().on_trade(
      _id.c_str(), cInfo->getExchg(), cInfo->getCode(), _date,
      tInfo->getTradeID(), tInfo->getRefOrder(),
      (uint32_t)tInfo->getDirection(), (uint32_t)tInfo->getOffsetType(),
      tInfo->getVolume(), tInfo->getPrice(), tInfo->getAmount(),
      (uint32_t)tInfo->getOrderType(), (uint32_t)tInfo->getTradeType(),
      tInfo->getTradeTime(), true);
}
...
void TraderAdapter::onRspTrades(const WTSArray *ayTrades) {
  if (ayTrades && ayTrades->size() > 0) {
    for (std::size_t idx = 0; idx < ayTrades->size(); idx++) {
      ...
      getDumper().on_trade(
          _id.c_str(), cInfo->getExchg(), cInfo->getCode(), _date,
          pItem->getTradeID(), pItem->getRefOrder(),
          (uint32_t)pItem->getDirection(), (uint32_t)pItem->getOffsetType(),
          pItem->getVolume(), pItem->getPrice(), pItem->getAmount(),
          (uint32_t)pItem->getOrderType(), (uint32_t)pItem->getTradeType(),
          pItem->getTradeTime(), idx == ayTrades->size() - 1);
    }
  }
...
void TraderAdapter::onRspOrders(const WTSArray *ayOrders) {
  if (ayOrders && ayOrders->size() > 0) {
    for (std::size_t idx = 0; idx < ayOrders->size(); idx++) {
      ...
      getDumper().on_order(
          _id.c_str(), cInfo->getExchg(), cInfo->getCode(), _date,
          pItem->getOrderID(), pItem->getDirection(), pItem->getOffsetType(),
          pItem->getVolume(), pItem->getVolLeft(), pItem->getVolTraded(),
          pItem->getPrice(), pItem->getOrderType(), pItem->getPriceType(),
          pItem->getOrderTime(), pItem->getOrderState(), pItem->getStateMsg(),
          idx == ayOrders->size() - 1);
    }
  }
...
void TraderAdapter::onPushOrder(WTSOrderInfo *oInfo) {
  WTSContractInfo *cInfo =
      _bd_mgr->getContract(oInfo->getCode(), oInfo->getExchg());
  if (cInfo == NULL)
    return;
  ...
  getDumper().on_order(_id.c_str(), cInfo->getExchg(), cInfo->getCode(), _date,
                       oInfo->getOrderID(), oInfo->getDirection(),
                       oInfo->getOffsetType(), oInfo->getVolume(),
                       oInfo->getVolLeft(), oInfo->getVolTraded(),
                       oInfo->getPrice(), oInfo->getOrderType(),
                       oInfo->getPriceType(), oInfo->getOrderTime(),
                       oInfo->getOrderState(), oInfo->getStateMsg(), true);
}
...
void TraderAdapter::onRspPosition(const WTSArray *ayPositions) {
  if (ayPositions && ayPositions->size() > 0) {
    for (std::size_t idx = 0; idx < ((WTSArray *)ayPositions)->size(); idx++) {
      WTSPositionItem *pItem =
          (WTSPositionItem *)(((WTSArray *)ayPositions)->at(idx));
      WTSContractInfo *cInfo = _bd_mgr->getContract(pItem->getCode());
      if (cInfo == NULL)
        continue;
      WTSCommodityInfo *commInfo = cInfo->getCommInfo();

      getDumper().on_position(
          _id.c_str(), cInfo->getExchg(), cInfo->getCode(), _date,
          (pItem->getDirection() == WDT_LONG ? 0 : 1),
          pItem->getTotalPosition(), pItem->getPositionCost(),
          pItem->getMargin(), pItem->getAvgPrice(), pItem->getDynProfit(),
...
void TraderAdapter::onTraderError(WTSError *err, void *pData /* = NULL */) {
  if (err)
    WTSLogger::error("[{}]交易通道出现错误: {}", _id.c_str(),
                     err->getMessage());
}
