...
inline void pipe_rdmreader_log(IRdmDtReaderSink *sink, WTSLogLevel ll,
...
bool WtRdmDtReader::loadStkAdjFactorsFromFile(const char *adjfile) {
  if (!StdFile::exists(adjfile)) {
    pipe_rdmreader_log(_sink, LL_ERROR, "Adjusting factors file {} not exists",
                       adjfile);
    return false;
  }
  ...
  if (doc == NULL) {
    pipe_rdmreader_log(_sink, LL_ERROR,
                       "Loading adjusting factors file {} failed", adjfile);
    return false;
  }
  ...
  pipe_rdmreader_log(_sink, LL_INFO,
                     "{} adjusting factors of {} tickers loaded", fct_cnt,
                     stk_cnt);
  doc->release();
  return true;
}

WTSTickSlice *WtRdmDtReader::readTickSliceByDate(const char *stdCode,
                                                 uint32_t uDate) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
  ...
  if (uDate <= curTDate) {
    ...
    if (commInfo->isFuture()) {
      ...
      if (strlen(ruleTag) > 0) {
        ...
        pipe_rdmreader_log(_sink, LL_INFO,
                           "{} contract on {} confirmed with rule {}: {} -> {}",
                           ruleTag, uDate, stdCode, curCode.c_str());
    ...
    if (!bHasHisTick) {
      for (;;) {
        ...
        if (!hotCode.empty()) {
          std::stringstream ss;
          ss << _base_dir << "his/ticks/" << cInfo._exchg << "/" << uDate << "/"
             << hotCode << ".dsb";
          filename = ss.str();
        ...
        if (tBlkPair._buffer.size() < sizeof(HisTickBlock)) {
          pipe_rdmreader_log(_sink, LL_ERROR,
                             "Sizechecking of tick data file {} failed",
                             filename.c_str());
    ...
    while (bHasHisTick) {
      ...
      uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisTickBlock)) /
  ...
  while (isToday) {
    ...
    if (commInfo->isFuture()) {
      ...
      if (strlen(ruleTag) > 0)
        curCode =
            _hot_mgr->getCustomRawCode(ruleTag, cInfo.stdCommID(), curTDate);
      // else if (cInfo.isHot())
      //	curCode = _hot_mgr->getRawCode(cInfo._exchg, cInfo._product,
      //curTDate); else if (cInfo.isSecond()) 	curCode =
      //_hot_mgr->getSecondRawCode(cInfo._exchg, cInfo._product, curTDate);
    }
    ...
    WTSTickSlice *slice =
        WTSTickSlice::create(stdCode, tBlock->_ticks, tBlock->_size);
    return slice;
  }
...
WTSTickSlice *WtRdmDtReader::readTickSliceByRange(const char *stdCode,
                                                  uint64_t stime,
                                                  uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
  ...
  pipe_rdmreader_log(_sink, LL_DEBUG, "Reading ticks of {} between {} and {}",
                     stdCode, stime, etime);
  ...
  uint32_t endTDate =
      _base_data_mgr->calcTradingDate(stdPID, rDate, rTime, false);
  uint32_t beginTDate =
      _base_data_mgr->calcTradingDate(stdPID, lDate, lTime, false);
  uint32_t curTDate = _base_data_mgr->calcTradingDate(stdPID, 0, 0, false);
  ...
  while (nowTDate < curTDate) {
    ...
    if (commInfo->isFuture()) {
      ...
      if (strlen(ruleTag) > 0) {
        ...
        pipe_rdmreader_log(_sink, LL_INFO,
                           "{} contract on {} confirmed: {} -> {}", ruleTag,
                           curTDate, stdCode, curCode.c_str());
        hotCode = cInfo._product;
        hotCode += "_";
        hotCode += ruleTag;
      }
    ...
    if (!bHasHisTick) {
      for (;;) {
        ...
        if (!hotCode.empty()) {
          std::stringstream ss;
          ss << _base_dir << "his/ticks/" << cInfo._exchg << "/" << nowTDate
             << "/" << hotCode << ".dsb";
          filename = ss.str();
          if (StdFile::exists(filename.c_str())) {
            bHitHot = true;
        ...
        if (!bHitHot) {
          ...
          ss << _base_dir << "his/ticks/" << cInfo._exchg << "/" << nowTDate
             << "/" << curCode << ".dsb";
          filename = ss.str();
          pipe_rdmreader_log(_sink, LL_DEBUG, "Reading ticks from {}...",
                             filename);
          if (!StdFile::exists(filename.c_str())) {
            break;
          }
    ...
    while (bHasHisTick) {
      ...
      WTSTickStruct *pTick =
          std::lower_bound(tBlock->_ticks, tBlock->_ticks + (tcnt - 1), eTick,
      ...
      if (pTick->action_date > eTick.action_date ||
          pTick->action_time >= eTick.action_time) {
        pTick--;
        eIdx--;
      }
      ...
      if (beginTDate != nowTDate) {
        ...
        // WTSTickSlice* slice = WTSTickSlice::create(stdCode, tBlock->_ticks,
        // eIdx + 1); ayTicks->append(slice, false);
        slice->appendBlock(tBlock->_ticks, eIdx + 1);
      } else {
        // 如果交易日相同，则查找起始的位置
        pTick = std::lower_bound(
            tBlock->_ticks, tBlock->_ticks + eIdx, sTick,
            [](const WTSTickStruct &a, const WTSTickStruct &b) {
              if (a.action_date != b.action_date)
                return a.action_date < b.action_date;
              else
        ...
        // WTSTickSlice* slice = WTSTickSlice::create(stdCode, tBlock->_ticks +
        // sIdx, eIdx - sIdx + 1); ayTicks->append(slice, false);
  ...
  while (hasToday) {
    ...
    WTSTickStruct *pTick = std::lower_bound(
        tBlock->_ticks, tBlock->_ticks + (tBlock->_size - 1), eTick,
        [](const WTSTickStruct &a, const WTSTickStruct &b) {
          if (a.action_date != b.action_date)
            return a.action_date < b.action_date;
          else
    ...
    if (beginTDate != curTDate) {
      ...
      // 如果交易日相同，则查找起始的位置
      pTick =
          std::lower_bound(tBlock->_ticks, tBlock->_ticks + eIdx, sTick,
                           [](const WTSTickStruct &a, const WTSTickStruct &b) {
                             if (a.action_date != b.action_date)
                               return a.action_date < b.action_date;
                             else
...
WTSOrdQueSlice *
WtRdmDtReader::readOrdQueSliceByRange(const char *stdCode, uint64_t stime,
                                      uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
  ...
  if (isToday) {
    ...
    WTSOrdQueStruct *pItem = std::lower_bound(
        rtBlock->_queues, rtBlock->_queues + (rtBlock->_size - 1), eTick,
    ...
    if (pItem->action_date > eTick.action_date ||
        pItem->action_time > eTick.action_time) {
      pItem--;
      eIdx--;
    }
    ...
    if (beginTDate != endTDate) {
      // 如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
      WTSOrdQueSlice *slice =
          WTSOrdQueSlice::create(stdCode, rtBlock->_queues, eIdx + 1);
      return slice;
    } else {
      // 如果交易日相同，则查找起始的位置
      pItem = std::lower_bound(
          rtBlock->_queues, rtBlock->_queues + eIdx, sTick,
          [](const WTSOrdQueStruct &a, const WTSOrdQueStruct &b) {
            if (a.action_date != b.action_date)
              return a.action_date < b.action_date;
            else
      ...
      WTSOrdQueSlice *slice = WTSOrdQueSlice::create(
          stdCode, rtBlock->_queues + sIdx, eIdx - sIdx + 1);
      return slice;
    }
  } else {
    ...
    if (it == _his_ordque_map.end()) {
      std::stringstream ss;
      ss << _base_dir << "his/queue/" << cInfo._exchg << "/" << endTDate << "/"
         << curCode << ".dsb";
      ...
      if (hisBlkPair._buffer.size() < sizeof(HisOrdQueBlockV2)) {
        pipe_rdmreader_log(_sink, LL_ERROR,
                           "Sizechecking of orderqueue data file {} failed",
                           filename.c_str());
        hisBlkPair._buffer.clear();
        return NULL;
      }

      HisOrdQueBlockV2 *tBlockV2 =
          (HisOrdQueBlockV2 *)hisBlkPair._buffer.c_str();

      if (hisBlkPair._buffer.size() !=
          (sizeof(HisOrdQueBlockV2) + tBlockV2->_size)) {
        pipe_rdmreader_log(_sink, LL_ERROR,
                           "Sizechecking of orderqueue data file {} failed",
                           filename.c_str());
        return NULL;
      ...
      // 需要解压
      std::string buf = WTSCmpHelper::uncompress_data(
          tBlockV2->_data, (std::size_t)tBlockV2->_size);
    ...
    uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisOrdQueBlock)) /
                    sizeof(WTSOrdQueStruct);
    if (tcnt <= 0)
      return NULL;

    WTSOrdQueStruct *pItem = std::lower_bound(
        tBlock->_items, tBlock->_items + (tcnt - 1), eTick,
        [](const WTSOrdQueStruct &a, const WTSOrdQueStruct &b) {
          if (a.action_date != b.action_date)
            return a.action_date < b.action_date;
          else
    ...
    if (beginTDate != endTDate) {
      // 如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
      WTSOrdQueSlice *slice =
          WTSOrdQueSlice::create(stdCode, tBlock->_items, eIdx + 1);
      return slice;
    } else {
      // 如果交易日相同，则查找起始的位置
      pItem = std::lower_bound(
          tBlock->_items, tBlock->_items + eIdx, sTick,
          [](const WTSOrdQueStruct &a, const WTSOrdQueStruct &b) {
            if (a.action_date != b.action_date)
              return a.action_date < b.action_date;
            else
      ...
      WTSOrdQueSlice *slice = WTSOrdQueSlice::create(
          stdCode, tBlock->_items + sIdx, eIdx - sIdx + 1);
      return slice;
    }
  }
}
...
WTSOrdDtlSlice *
WtRdmDtReader::readOrdDtlSliceByRange(const char *stdCode, uint64_t stime,
                                      uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
  ...
  if (isToday) {
    ...
    WTSOrdDtlStruct *pItem = std::lower_bound(
        rtBlock->_details, rtBlock->_details + (rtBlock->_size - 1), eTick,
    ...
    if (beginTDate != endTDate) {
      // 如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
      WTSOrdDtlSlice *slice =
          WTSOrdDtlSlice::create(stdCode, rtBlock->_details, eIdx + 1);
      return slice;
    } else {
      // 如果交易日相同，则查找起始的位置
      pItem = std::lower_bound(
          rtBlock->_details, rtBlock->_details + eIdx, sTick,
          [](const WTSOrdDtlStruct &a, const WTSOrdDtlStruct &b) {
            if (a.action_date != b.action_date)
              return a.action_date < b.action_date;
            else
      ...
      WTSOrdDtlSlice *slice = WTSOrdDtlSlice::create(
          stdCode, rtBlock->_details + sIdx, eIdx - sIdx + 1);
      return slice;
    }
  } else {
    std::string key = fmt::format("{}-{}", stdCode, endTDate);
    ...
    if (it == _his_ordque_map.end()) {
      std::stringstream ss;
      ss << _base_dir << "his/orders/" << cInfo._exchg << "/" << endTDate << "/"
         << curCode << ".dsb";
      std::string filename = ss.str();
      if (!StdFile::exists(filename.c_str()))
        return NULL;
      ...
      if (hisBlkPair._buffer.size() < sizeof(HisOrdDtlBlockV2)) {
        pipe_rdmreader_log(_sink, LL_ERROR,
                           "Sizechecking of orderdetail data file {} failed",
                           filename.c_str());
        hisBlkPair._buffer.clear();
        return NULL;
      }

      HisOrdDtlBlockV2 *tBlockV2 =
          (HisOrdDtlBlockV2 *)hisBlkPair._buffer.c_str();

      if (hisBlkPair._buffer.size() !=
          (sizeof(HisOrdDtlBlockV2) + tBlockV2->_size)) {
        pipe_rdmreader_log(_sink, LL_ERROR,
                           "Sizechecking of orderdetail data file {} failed",
                           filename.c_str());
        return NULL;
    ...
    uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisOrdDtlBlock)) /
    ...
    if (beginTDate != endTDate) {
      // 如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
      WTSOrdDtlSlice *slice =
          WTSOrdDtlSlice::create(stdCode, tBlock->_items, eIdx + 1);
      return slice;
    } else {
      // 如果交易日相同，则查找起始的位置
      pItem = std::lower_bound(
...
WTSTransSlice *WtRdmDtReader::readTransSliceByRange(const char *stdCode,
                                                    uint64_t stime,
                                                    uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
  ...
  if (isToday) {
    ...
    WTSTransStruct *pItem = std::lower_bound(
        rtBlock->_trans, rtBlock->_trans + (rtBlock->_size - 1), eTick,
    ...
    if (beginTDate != endTDate) {
      // 如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
      WTSTransSlice *slice =
          WTSTransSlice::create(stdCode, rtBlock->_trans, eIdx + 1);
      return slice;
    } else {
      // 如果交易日相同，则查找起始的位置
      pItem = std::lower_bound(
          rtBlock->_trans, rtBlock->_trans + eIdx, sTick,
          [](const WTSTransStruct &a, const WTSTransStruct &b) {
            if (a.action_date != b.action_date)
              return a.action_date < b.action_date;
            else
      ...
      WTSTransSlice *slice = WTSTransSlice::create(
          stdCode, rtBlock->_trans + sIdx, eIdx - sIdx + 1);
      return slice;
    }
  } else {
    std::string key = fmt::format("{}-{}", stdCode, endTDate);
    ...
    if (it == _his_ordque_map.end()) {
      std::stringstream ss;
      ss << _base_dir << "his/trans/" << cInfo._exchg << "/" << endTDate << "/"
         << curCode << ".dsb";
      std::string filename = ss.str();
      if (!StdFile::exists(filename.c_str()))
        return NULL;
      ...
      if (hisBlkPair._buffer.size() < sizeof(HisTransBlockV2)) {
        pipe_rdmreader_log(_sink, LL_ERROR,
                           "Sizechecking of transaction data file {} failed",
                           filename.c_str());
        hisBlkPair._buffer.clear();
        return NULL;
      }
      ...
      if (hisBlkPair._buffer.size() !=
          (sizeof(HisTransBlockV2) + tBlockV2->_size)) {
        pipe_rdmreader_log(_sink, LL_ERROR,
                           "Sizechecking of transaction data file {} failed",
                           filename.c_str());
        return NULL;
    ...
    uint32_t tcnt = (tBlkPair._buffer.size() - sizeof(HisTransBlock)) /
                    sizeof(WTSTransStruct);
    if (tcnt <= 0)
      return NULL;

    WTSTransStruct *pItem =
        std::lower_bound(tBlock->_items, tBlock->_items + (tcnt - 1), eTick,
                         [](const WTSTransStruct &a, const WTSTransStruct &b) {
                           if (a.action_date != b.action_date)
                             return a.action_date < b.action_date;
                           else
    ...
    if (beginTDate != endTDate) {
      // 如果开始的交易日和当前的交易日不一致，则返回全部的tick数据
      WTSTransSlice *slice =
          WTSTransSlice::create(stdCode, tBlock->_items, eIdx + 1);
      return slice;
    } else {
      // 如果交易日相同，则查找起始的位置
      pItem = std::lower_bound(
...
bool WtRdmDtReader::cacheHisBarsFromFile(void *codeInfo, const std::string &key,
                                         const char *stdCode,
  ...
  uint32_t endTDate =
      _base_data_mgr->calcTradingDate(stdPID, curDate, curTime, false);
  ...
  if (strlen(ruleTag) > 0) // 如果是读取期货主力连续数据
  {
    ...
    for (;;) {
      ...
      ss << _base_dir << "his/" << pname << "/" << cInfo->_exchg << "/"
         << cInfo->_exchg << "." << cInfo->_product << "_" << ruleTag;
      if (cInfo->isExright())
      ...
      if (content.size() < sizeof(HisKlineBlock)) {
        pipe_rdmreader_log(_sink, LL_ERROR,
                           "Sizechecking of his kline data file {} failed",
                           filename.c_str());
        break;
      }
      ...
      pipe_rdmreader_log(
          _sink, LL_INFO,
          "{} items of back {} data of hot contract {} directly loaded", barcnt,
          pname.c_str(), stdCode);
      break;
    }
    ...
    HotSections secs;
    if (strlen(ruleTag)) {
      if (!_hot_mgr->splitCustomSections(ruleTag, stdPID, 19900102, endTDate,
                                         secs))
        return false;
    }
    ...
    for (auto it = secs.rbegin(); it != secs.rend(); it++) {
      ...
      if (period != KP_DAY) {
        uint64_t sTime =
            _base_data_mgr->getBoundaryTime(stdPID, leftDt, false, true);
        uint64_t eTime =
            _base_data_mgr->getBoundaryTime(stdPID, rightDt, false, false);
  ...
  } else if (cInfo->isExright() &&
             commInfo->isStock()) // 如果是读取股票复权数据
  {
    std::vector<WTSBarStruct> *hotAy = NULL;
    ...
    do {
      ...
      if (kBlock->_version == BLOCK_VERSION_CMP) {
        ...
        buffer = WTSCmpHelper::uncompress_data(kBlockV2->_data,
                                               (std::size_t)kBlockV2->_size);
      } else {
      ...
      pipe_rdmreader_log(_sink, LL_INFO,
                         "{} history exrighted {} data of {} directly cached",
                         barcnt, pname.c_str(), stdCode);
      break;
    } while (false);
    ...
    do {
...

      {
        ...
        if (pBar != NULL) {
          ...
          std::vector<WTSBarStruct> *tempAy = new std::vector<WTSBarStruct>();
          tempAy->resize(curCnt);
          memcpy(tempAy->data(), &firstBar[sIdx],
                 sizeof(WTSBarStruct) * curCnt);
          realCnt += curCnt;
  ...
  if (realCnt > 0) {
    ...
    for (auto it = barsSections.rbegin(); it != barsSections.rend(); it++) {
      ...
      memcpy(barList._bars.data() + curIdx, tempAy->data(),
  ...
  pipe_rdmreader_log(_sink, LL_INFO, "{} history {} data of {} cached", realCnt,
                     pname.c_str(), stdCode);
  return true;
}

WTSBarStruct *
WtRdmDtReader::indexBarFromCacheByRange(const std::string &key, uint64_t stime,
                                        uint64_t etime, uint32_t &count,
                                        bool isDay /* = false */) {
  ...
  std::size_t eIdx, sIdx;
  {
    ...
    auto eit =
        std::lower_bound(barsList._bars.begin(), barsList._bars.end(), eBar,
    ...
    if (eit == barsList._bars.end())
      ...
      if ((isDay && eit->date > eBar.date) ||
          (!isDay && eit->time > eBar.time)) {
        eit--;
      }
    ...
    auto sit =
        std::lower_bound(barsList._bars.begin(), eit, sBar,
                         [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
                           if (isDay)
                             return a.date < b.date;
                           else
...
WTSBarStruct *
WtRdmDtReader::indexBarFromCacheByCount(const std::string &key, uint64_t etime,
                                        uint32_t &count,
                                        bool isDay /* = false */) {
...
uint32_t WtRdmDtReader::readBarsFromCacheByRange(
    const std::string &key, uint64_t stime, uint64_t etime,
    std::vector<WTSBarStruct> &ayBars, bool isDay /* = false */) {
  uint32_t rDate, rTime, lDate, lTime;
  rDate = (uint32_t)(etime / 10000);
  rTime = (uint32_t)(etime % 10000);
  lDate = (uint32_t)(stime / 10000);
...
WTSKlineSlice *WtRdmDtReader::readKlineSliceByRange(const char *stdCode,
                                                    WTSKlinePeriod period,
                                                    uint64_t stime,
                                                    uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  ...
  if (strlen(ruleTag) > 0) {
    ...
    pipe_rdmreader_log(_sink, LL_INFO, "{} contract on {} confirmed: {} -> {}",
                       ruleTag, curTDate, stdCode, raw_code);
  } else {
  ...
  if (bHasToday) {
    ...
    if (cInfo._exright != 2) {
      ...
      if (kPair != NULL) {
        ...
        WTSBarStruct *pBar = std::lower_bound(
            kPair->_block->_bars,
            kPair->_block->_bars + (kPair->_block->_size - 1), eBar,
            [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
              if (isDay)
                return a.date < b.date;
              else
        ...
        if ((isDay && pBar->date > eBar.date) ||
            (!isDay && pBar->time > eBar.time)) {
          pBar--;
          idx--;
        }
        ...
        if ((isDay && pBar->date > sBar.date) ||
            (!isDay && pBar->time > sBar.time)) {
          rtHead = &kPair->_block->_bars[0];
          rtCnt = idx + 1;
        } else {
          pBar = std::lower_bound(
              kPair->_block->_bars, kPair->_block->_bars + idx, sBar,
              [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
                if (isDay)
                  return a.date < b.date;
                else
      ...
      if (kPair != NULL) {
        ...
        if (newSize > oldSize) {
          ...
          memcpy(&barsList._rt_bars[idx], &kPair->_block->_bars[idx],
                 sizeof(WTSBarStruct) * (newSize - oldSize + 1));
        ...
        auto it = std::lower_bound(
            barsList._rt_bars.begin(), barsList._rt_bars.end(), eBar,
            [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
              if (isDay)
                return a.date < b.date;
              else
        ...
        if ((isDay && pBar->date > sBar.date) ||
          ...
          it = std::lower_bound(
              barsList._rt_bars.begin(), barsList._rt_bars.begin() + idx, sBar,
              [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
                if (isDay)
                  return a.date < b.date;
                else
  ...
  if (bNeedHisData) {
    hisHead =
        indexBarFromCacheByRange(key, stime, etime, hisCnt, period == KP_DAY);
  }

  if (hisCnt + rtCnt > 0) {
    WTSKlineSlice *slice =
        WTSKlineSlice::create(stdCode, period, 1, hisHead, hisCnt);
    if (rtCnt > 0)
...
WtRdmDtReader::TickBlockPair *WtRdmDtReader::getRTTickBlock(const char *exchg,
                                                            const char *code) {
  std::string key = fmt::format("{}.{}", exchg, code);

  std::string path =
      fmt::format("{}rt/ticks/{}/{}.dmb", _base_dir.c_str(), exchg, code);
  ...
  if (block._file == NULL || block._block == NULL) {
    ...
    if (!block._file->map(path.c_str(), boost::interprocess::read_only,
...
WtRdmDtReader::OrdDtlBlockPair *
WtRdmDtReader::getRTOrdDtlBlock(const char *exchg, const char *code) {
  std::string key = fmt::format("{}.{}", exchg, code);

  std::string path =
      fmt::format("{}rt/orders/{}/{}.dmb", _base_dir.c_str(), exchg, code);
  if (!StdFile::exists(path.c_str()))
    return NULL;
...
WtRdmDtReader::OrdQueBlockPair *
WtRdmDtReader::getRTOrdQueBlock(const char *exchg, const char *code) {
  std::string key = fmt::format("{}.{}", exchg, code);

  std::string path =
      fmt::format("{}rt/queue/{}/{}.dmb", _base_dir.c_str(), exchg, code);
  if (!StdFile::exists(path.c_str()))
    return NULL;
...
WtRdmDtReader::TransBlockPair *
WtRdmDtReader::getRTTransBlock(const char *exchg, const char *code) {
  std::string key = fmt::format("{}.{}", exchg, code);

  std::string path =
      fmt::format("{}rt/trans/{}/{}.dmb", _base_dir.c_str(), exchg, code);
  if (!StdFile::exists(path.c_str()))
    return NULL;
...
WtRdmDtReader::RTKlineBlockPair *
WtRdmDtReader::getRTKilneBlock(const char *exchg, const char *code,
                               WTSKlinePeriod period) {
  ...
  std::string path = fmtutil::format("{}rt/{}/{}/{}.dmb", _base_dir.c_str(),
                                     subdir.c_str(), exchg, code);
  if (!StdFile::exists(path.c_str()))
    return NULL;

  RTKlineBlockPair &block =
      (period == KP_Minute1 ? _rt_min1_map[key] : _rt_min5_map[key]);
  if (block._file == NULL || block._block == NULL) {
    if (block._file == NULL) {
      block._file.reset(new BoostMappingFile());
    }
...
WTSKlineSlice *WtRdmDtReader::readKlineSliceByCount(const char *stdCode,
                                                    WTSKlinePeriod period,
                                                    uint32_t count,
                                                    uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  pipe_rdmreader_log(_sink, LL_INFO, "CodeInfo of {}: {},{},{}", stdCode,
                     cInfo._exchg, cInfo._product, cInfo._code);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
  const char *stdPID = commInfo->getFullPid();
  ...
  if (strlen(ruleTag) > 0) {
    ...
    pipe_rdmreader_log(_sink, LL_INFO, "{} contract on {} confirmed: {} -> {}",
                       ruleTag, curTDate, stdCode, raw_code.c_str());
  } else {
    raw_code = cInfo._code;
  }
    ...
    if (cInfo._exright != 2) {
      ...
      if (kPair != NULL) {
        ...
        if (newSize > oldSize) {
          ...
          // 因为每次拷贝，最后一条K线都有可能是未闭合的，所以需要把最后一条K线覆盖
          memcpy(&barsList._rt_bars[idx], &kPair->_block->_bars[idx],
                 sizeof(WTSBarStruct) * (newSize - idx));
  ...
  pipe_rdmreader_log(
      _sink, LL_DEBUG,
      "His {} bars of {} loaded, {} from history, {} from realtime",
      PERIOD_NAME[period], stdCode, hisCnt, rtCnt);
...
WTSTickSlice *WtRdmDtReader::readTickSliceByCount(const char *stdCode,
                                                  uint32_t count,
                                                  uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
  ...
  WTSSessionInfo *sInfo = _base_data_mgr->getSession(
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._code)->getSession());
  ...
  while (left > 0) {
    ...
    if (commInfo->isFuture()) {
      ...
      if (strlen(ruleTag) > 0) {
        curCode =
            _hot_mgr->getCustomRawCode(ruleTag, cInfo.stdCommID(), nowTDate);
      ...
      //{
      //	curCode = _hot_mgr->getRawCode(cInfo._exchg, cInfo._product,
      //nowTDate); 	hotCode = cInfo._product; 	hotCode += "_HOT";
      //	pipe_rdmreader_log(_sink, LL_INFO, "Hot contract on {}
      //confirmed: {} -> {}", curTDate, stdCode, curCode.c_str());
      // }
      // else if (cInfo.isSecond())
      //{
      //	curCode = _hot_mgr->getSecondRawCode(cInfo._exchg,
      //cInfo._product, nowTDate); 	hotCode = cInfo._product; 	hotCode += "_2ND";
      //	pipe_rdmreader_log(_sink, LL_INFO, "Second contract on {}
      //confirmed: {} -> {}", curTDate, stdCode, curCode.c_str());
      // }
    }
    ...
    if (!bHasHisTick) {
      for (;;) {
        ...
        if (tBlkPair._buffer.size() < sizeof(HisTickBlock)) {
          pipe_rdmreader_log(_sink, LL_ERROR,
                             "Sizechecking of his tick data file {} failed",
                             filename.c_str());
          tBlkPair._buffer.clear();
          break;
        }
...
double WtRdmDtReader::getAdjFactorByDate(const char *stdCode,
                                         uint32_t date /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
  ...
  auto it = std::lower_bound(
      factList.begin(), factList.end(), factor,
      [](const AdjFactor &a, const AdjFactor &b) { return a._date < b._date; });
