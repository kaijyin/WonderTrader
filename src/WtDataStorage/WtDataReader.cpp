...
inline void pipe_reader_log(IDataReaderSink *sink, WTSLogLevel ll,
 ...
 *	处理块数据
 */
bool proc_block_data(std::string &content, bool isBar,
                     bool bKeepHead /* = true */) {
...
void WtDataReader::init(WTSVariant *cfg, IDataReaderSink *sink,
  ...
  pipe_reader_log(sink, LL_INFO,
                  "WtDataReader initialized, rt dir is {}, hist dir is {}, "
                  "adjust_flag is {}",
  ...
  if (!bLoaded && cfg->has("adjfactor"))
    ...
    pipe_reader_log(sink, LL_INFO,
                    "No adjusting factor file configured, loading skipped");
}
...
bool WtDataReader::loadStkAdjFactorsFromLoader() {
  ...
  bool ret = _loader->loadAllAdjFactors(
      &_adj_factors, [](void *obj, const char *stdCode, uint32_t *dates,
                        double *factors, uint32_t count) {
...
bool WtDataReader::loadStkAdjFactorsFromFile(const char *adjfile) {
  if (!StdFile::exists(adjfile)) {
    pipe_reader_log(_sink, LL_ERROR, "Adjusting factors file {} not exists",
                    adjfile);
    return false;
  }
  ...
  if (doc == NULL) {
    pipe_reader_log(_sink, LL_ERROR, "Loading adjusting factors file {} failed",
                    adjfile);
    return false;
  }
  ...
  pipe_reader_log(_sink, LL_INFO, "{} adjusting factors of {} tickers loaded",
                  fct_cnt, stk_cnt);
  doc->release();
  return true;
}

WTSTickSlice *WtDataReader::readTickSlice(const char *stdCode, uint32_t count,
                                          uint64_t etime /* = 0 */) {
  ...
  if (isToday) {
    ...
    WTSTickSlice *slice =
        WTSTickSlice::create(stdCode, tBlock->_ticks + sIdx, cnt);
    return slice;
  } else {
    ...
    if (it == _his_tick_map.end()) {
      std::stringstream ss;
      ss << _his_dir << "ticks/" << cInfo._exchg << "/" << endTDate << "/"
         << curCode << ".dsb";
      ...
      if (tBlkPair._buffer.size() < sizeof(HisTickBlock)) {
        pipe_reader_log(_sink, LL_ERROR,
...
WTSOrdQueSlice *WtDataReader::readOrdQueSlice(const char *stdCode,
                                              uint32_t count,
                                              uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
  ...
  if (isToday) {
    ...
    WTSOrdQueSlice *slice =
        WTSOrdQueSlice::create(stdCode, rtBlock->_queues + sIdx, cnt);
    return slice;
  } else {
    thread_local static char key[64] = {0};
    fmtutil::format_to(key, "{}-{}", stdCode, endTDate);
    ...
    if (it == _his_ordque_map.end()) {
      std::stringstream ss;
      ss << _his_dir << "queue/" << cInfo._exchg << "/" << endTDate << "/"
         << curCode << ".dsb";
      std::string filename = ss.str();
      if (!StdFile::exists(filename.c_str()))
        return NULL;
      ...
      if (hisBlkPair._buffer.size() < sizeof(HisOrdQueBlockV2)) {
        pipe_reader_log(_sink, LL_ERROR, "历史委托队列数据文件{}大小校验失败",
                        filename);
    ...
    WTSOrdQueStruct *pTick = std::lower_bound(
    ...
    uint32_t sIdx = eIdx + 1 - cnt;
    WTSOrdQueSlice *slice =
        WTSOrdQueSlice::create(stdCode, tBlock->_items + sIdx, cnt);
    return slice;
  }
}

WTSOrdDtlSlice *WtDataReader::readOrdDtlSlice(const char *stdCode,
                                              uint32_t count,
                                              uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
  ...
  if (isToday) {
    ...
    WTSOrdDtlSlice *slice =
        WTSOrdDtlSlice::create(stdCode, rtBlock->_details + sIdx, cnt);
    return slice;
  } else {
    thread_local static char key[64] = {0};
    fmtutil::format_to(key, "{}-{}", stdCode, endTDate);
    ...
    if (it == _his_ordque_map.end()) {
      std::stringstream ss;
      ss << _his_dir << "orders/" << cInfo._exchg << "/" << endTDate << "/"
         << curCode << ".dsb";
      std::string filename = ss.str();
      if (!StdFile::exists(filename.c_str()))
        return NULL;
      ...
      if (hisBlkPair._buffer.size() < sizeof(HisOrdDtlBlockV2)) {
        pipe_reader_log(_sink, LL_ERROR, "历史逐笔委托数据文件{}大小校验失败",
                        filename.c_str());
        hisBlkPair._buffer.clear();
        return NULL;
      }
    ...
    WTSOrdDtlStruct *pTick = std::lower_bound(
        tBlock->_items, tBlock->_items + (tcnt - 1), eTick,
        [](const WTSOrdDtlStruct &a, const WTSOrdDtlStruct &b) {
          if (a.action_date != b.action_date)
            return a.action_date < b.action_date;
    ...
    uint32_t sIdx = eIdx + 1 - cnt;
    WTSOrdDtlSlice *slice =
        WTSOrdDtlSlice::create(stdCode, tBlock->_items + sIdx, cnt);
    return slice;
  }
}

WTSTransSlice *WtDataReader::readTransSlice(const char *stdCode, uint32_t count,
                                            uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
  ...
  if (isToday) {
    ...
    uint32_t sIdx = eIdx + 1 - cnt;
    WTSTransSlice *slice =
        WTSTransSlice::create(stdCode, rtBlock->_trans + sIdx, cnt);
    return slice;
  } else {
    thread_local static char key[64] = {0};
    fmtutil::format_to(key, "{}-{}", stdCode, endTDate);
    ...
    if (it == _his_ordque_map.end()) {
      std::stringstream ss;
      ss << _his_dir << "trans/" << cInfo._exchg << "/" << endTDate << "/"
         << curCode << ".dsb";
      std::string filename = ss.str();
      if (!StdFile::exists(filename.c_str()))
        return NULL;
      ...
      if (hisBlkPair._buffer.size() < sizeof(HisTransBlockV2)) {
        pipe_reader_log(_sink, LL_ERROR, "历史逐笔成交数据文件{}大小校验失败",
                        filename.c_str());
        hisBlkPair._buffer.clear();
        return NULL;
      }
    ...
    uint32_t sIdx = eIdx + 1 - cnt;
    WTSTransSlice *slice =
        WTSTransSlice::create(stdCode, tBlock->_items + sIdx, cnt);
    return slice;
  }
}

bool WtDataReader::cacheFinalBarsFromLoader(void *codeInfo,
  ...
  pipe_reader_log(_sink, LL_INFO,
                  "Reading final bars of {} via extended loader...", stdCode);

  bool ret = _loader->loadFinalHisBars(
      &barList, stdCode, period,
  ...
  if (ret)
    pipe_reader_log(_sink, LL_INFO,
                    "{} items of back {} data of {} loaded via extended loader",
                    barList._bars.size(), pname.c_str(), stdCode);

  return ret;
}

bool WtDataReader::cacheIntegratedBars(void *codeInfo, const std::string &key,
                                       const char *stdCode,
                                       WTSKlinePeriod period) {
  CodeHelper::CodeInfo *cInfo = (CodeHelper::CodeInfo *)codeInfo;
  ...
  uint32_t endTDate = _base_data_mgr->calcTradingDate(cInfo->stdCommID(),
                                                      curDate, curTime, false);
  ...
  do {
    ...
    std::stringstream ss;
    ss << _his_dir << pname << "/" << cInfo->_exchg << "/" << cInfo->_exchg
       << "." << cInfo->_product << "_" << ruleTag;
    if (cInfo->isExright())
    ...
    if (content.size() < sizeof(HisKlineBlock)) {
      pipe_reader_log(_sink, LL_ERROR, "历史K线数据文件{}大小校验失败",
                      filename);
      break;
    }
    ...
    pipe_reader_log(
        _sink, LL_INFO,
        "{} items of back {} data of wrapped contract {} directly loaded",
  ...
  for (auto it = secs.rbegin(); it != secs.rend(); it++) {
    ...
    if (period != KP_DAY) {
      uint64_t sTime = _base_data_mgr->getBoundaryTime(cInfo->stdCommID(),
                                                       leftDt, false, true);
      uint64_t eTime = _base_data_mgr->getBoundaryTime(cInfo->stdCommID(),
                                                       rightDt, false, false);
    ...
    if (NULL != _loader) {
      std::string wCode =
          fmt::format("{}.{}.{}", cInfo->_exchg, cInfo->_product,
                      (char *)curCode + strlen(cInfo->_product));
      bLoaded = _loader->loadRawHisBars(
    ...
    if (!bLoaded) {
      std::stringstream ss;
      ss << _his_dir << pname << "/" << cInfo->_exchg << "/" << curCode
         << ".dsb";
      std::string filename = ss.str();
      if (!StdFile::exists(filename.c_str()))
        continue;
      ...
      if (content.size() < sizeof(HisKlineBlock)) {
        pipe_reader_log(_sink, LL_ERROR,
                        "Sizechecking of his dta file {} failed",
                        filename.c_str());
        return false;
      }
      proc_block_data(content, true, false);
  ...
  pipe_reader_log(_sink, LL_INFO, "{} items of back {} data of {} cached",
                  realCnt, pname.c_str(), stdCode);

  return true;
}

bool WtDataReader::cacheAdjustedStkBars(void *codeInfo, const std::string &key,
                                        const char *stdCode,
                                        WTSKlinePeriod period) {
  CodeHelper::CodeInfo *cInfo = (CodeHelper::CodeInfo *)codeInfo;
  ...
  do {
    ...
    std::stringstream ss;
    ss << _his_dir << pname << "/" << cInfo->_exchg << "/" << cInfo->_code
       << flag << ".dsb";
    std::string filename = ss.str();
    if (!StdFile::exists(filename.c_str()))
      break;
    ...
    pipe_reader_log(
        _sink, LL_INFO,
        "{} items of adjusted back {} data of stock {} directly loaded", barcnt,
        pname.c_str(), stdCode);
  } while (false);
  ...
  do {
    ...
    std::string buffer;
    std::string rawCode =
        fmt::format("{}.{}.{}", cInfo->_exchg, cInfo->_product, curCode);
    if (NULL != _loader) {
      bLoaded = _loader->loadRawHisBars(
          &buffer, rawCode.c_str(), period,
          [](void *obj, WTSBarStruct *bars, uint32_t count) {
            std::string *buff = (std::string *)obj;
            buff->resize(sizeof(WTSBarStruct) * count);
            memcpy((void *)buff->c_str(), bars, sizeof(WTSBarStruct) * count);
...
bool WtDataReader::cacheHisBarsFromFile(void *codeInfo, const std::string &key,
                                        const char *stdCode,
                                        WTSKlinePeriod period) {
  CodeHelper::CodeInfo *cInfo = (CodeHelper::CodeInfo *)codeInfo;
  WTSCommodityInfo *commInfo =
...
WTSKlineSlice *WtDataReader::readKlineSlice(const char *stdCode,
  ...
  if (strlen(ruleTag) > 0) {
    ...
    pipe_reader_log(_sink, LL_INFO, "{} contract on {} confirmed: {} -> {}",
                    ruleTag, curTDate, stdCode, barsList._raw_code.c_str());
  } else {
  ...
  if (commInfo->isFuture())
  {
          ...
          if (strlen(ruleTag) > 0)
          {
                  barsList._raw_code = _hot_mgr->getCustomRawCode(ruleTag,
  cInfo.stdCommID(), curTDate); pipe_reader_log(_sink, LL_INFO, "{} contract on
  {} confirmed with rule {}: {} -> {}", ruleTag, curTDate, stdCode,
  barsList._raw_code.c_str());
          }
          //else if (cInfo.isHot())
          //{
          //	barsList._raw_code = _hot_mgr->getRawCode(cInfo._exchg,
  cInfo._product, curTDate);
          //	pipe_reader_log(_sink, LL_INFO, "Hot contract on {}  confirmed:
  {} -> {}", curTDate, stdCode, barsList._raw_code.c_str());
          //}
          //else if (cInfo.isSecond())
          //{
          //	barsList._raw_code = _hot_mgr->getSecondRawCode(cInfo._exchg,
  cInfo._product, curTDate);
          //	pipe_reader_log(_sink, LL_INFO, "Second contract on {}
  confirmed: {} -> {}", curTDate, stdCode, barsList._raw_code.c_str());
          //}
          else
          {
                  barsList._raw_code = cInfo._code;
  ...
  */
  ...
  if (bHasToday) {
    ...
    if (kPair != NULL && kPair->_block && kPair->_block->_size > 0) {
      ...
      pBar = std::lower_bound(
          kPair->_block->_bars,
          kPair->_block->_bars + (kPair->_block->_size - 1), bar,
          [period](const WTSBarStruct &a, const WTSBarStruct &b) {
            if (period == KP_DAY)
              return a.date < b.date;
            else
      ...
      if ((period == KP_DAY && pBar->date > bar.date) ||
          (period != KP_DAY && pBar->time > bar.time)) {
        pBar--;
        idx--;
      }
      ...
      if (cInfo._exright == 2 /* && commInfo->isStock()*/) {
        ...
        if (barsList._rt_cursor == UINT_MAX || idx > barsList._rt_cursor) {
          ...
          memcpy(&barsList._bars[oldSize], &kPair->_block->_bars[sIdx],
...
WtDataReader::TickBlockPair *WtDataReader::getRTTickBlock(const char *exchg,
                                                          const char *code) {
  thread_local static char key[64] = {0};
  ...
  if (block._file == NULL || block._block == NULL) {
    ...
    if (!block._file->map(path, boost::interprocess::read_only,
...
WtDataReader::OrdDtlBlockPair *
WtDataReader::getRTOrdDtlBlock(const char *exchg, const char *code) {
  thread_local static char key[64] = {0};
  fmtutil::format_to(key, "{}#{}", exchg, code);
...
WtDataReader::OrdQueBlockPair *
WtDataReader::getRTOrdQueBlock(const char *exchg, const char *code) {
  thread_local static char key[64] = {0};
  fmtutil::format_to(key, "{}#{}", exchg, code);
...
WtDataReader::TransBlockPair *WtDataReader::getRTTransBlock(const char *exchg,
                                                            const char *code) {
  thread_local static char key[64] = {0};
  fmtutil::format_to(key, "{}#{}", exchg, code);
...
WtDataReader::RTKlineBlockPair *
WtDataReader::getRTKilneBlock(const char *exchg, const char *code,
                              WTSKlinePeriod period) {
  ...
  if (block._file == NULL || block._block == NULL) {
    ...
    block._last_cap = block._block->_capacity;
    pipe_reader_log(_sink, LL_DEBUG, "RT {} block of {}.{} loaded",
                    subdir.c_str(), exchg, code);
  } else if (block._last_cap != block._block->_capacity) {
    // 说明文件大小已变, 需要重新映射
    pipe_reader_log(_sink, LL_DEBUG,
                    "RT {} block of {}.{} expanded to {}, remapping...",
...
void WtDataReader::onMinuteEnd(uint32_t uDate, uint32_t uTime,
                               uint32_t endTDate /* = 0 */) {
  // 这里应该触发检查
  ...
  for (auto it = _bars_cache.begin(); it != _bars_cache.end(); it++) {
    ...
    if (barsList._period != KP_DAY) {
      if (!barsList._raw_code.empty()) {
        RTKlineBlockPair *kBlk =
            getRTKilneBlock(barsList._exchg.c_str(), barsList._raw_code.c_str(),
                            barsList._period);
        if (kBlk == NULL)
          continue;
        ...
        for (;;) {
          ...
          if (barTime <= nowTime) {
            ...
            if (barsList._factor == DBL_MAX) {
              ...
              _sink->on_bar(barsList._code.c_str(), barsList._period,
    ...
    //{
    //	if (barsList._his_cursor != UINT_MAX && barsList._bars.size() - 1 >
    //barsList._his_cursor)
    //	{
    //		for (;;)
    //		{
    //			WTSBarStruct& nextBar =
    //barsList._bars[barsList._his_cursor + 1];

    //			if (nextBar.date <= endTDate)
    //			{
    //				_sink->on_bar(barsList._code.c_str(),
    //barsList._period, &nextBar);
    //			}
    //			else
    //			{
    //				break;
...
double WtDataReader::getAdjFactorByDate(const char *stdCode,
                                        uint32_t date /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
...
const WtDataReader::AdjFactorList &
WtDataReader::getAdjFactors(const char *code, const char *exchg,
                            const char *pid) {
  ...
  if (it == _adj_factors.end()) {
    ...
    if (_loader) {
      if (_sink)
        pipe_reader_log(_sink, LL_INFO,
                        "No adjusting factors of {} cached, searching via "
                        "extented loader...",
                        key);
      _loader->loadAdjFactors(
          this, key,
          [](void *obj, const char *stdCode, uint32_t *dates, double *factors,
            ...
            pipe_reader_log(self->_sink, LL_INFO,
                            "{} items of adjusting factors of {} loaded via "
                            "extended loader",
                            count, stdCode);
          });
    }
