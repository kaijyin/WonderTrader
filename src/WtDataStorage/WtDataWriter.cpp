...
inline void pipe_writer_log(IDataWriterSink *sink, WTSLogLevel ll,
 ...
 *	处理块数据
 */
extern bool proc_block_data(std::string &content, bool isBar,
                            bool bKeepHead = true);
...
WtDataWriter::_TaskInfo::_TaskInfo(WTSObject *data, uint64_t dtype,
                                   uint32_t flag /* = 0*/)
...
WtDataWriter::_TaskInfo::~_TaskInfo() { _obj->release(); }

WtDataWriter::WtDataWriter()
    : _terminated(false), _save_tick_log(false), _log_group_size(1000),
      _disable_day(false), _disable_min1(false), _disable_min5(false),
      _disable_orddtl(false), _disable_ordque(false), _disable_trans(false),
      _disable_tick(false), _disable_his(false), _skip_notrade_tick(false),
...
bool WtDataWriter::init(WTSVariant *params, IDataWriterSink *sink) {
  ...
  pipe_writer_log(sink, LL_INFO,
                  "WtDataWriter initialized, root dir: {}, save_csv_tick: {}, "
                  "async_mode: {}, log_group_size: {}, disable_history: {}, "
                  "disable_tick: {}, disable_min1: {}, disable_min5: {}, "
                  "disable_day: {}, disable_trans: {}, disable_ordque: {}, "
                  "disable_orders: {}, min_price_mode: {}",
                  _base_dir, _save_tick_log, _async_proc, _log_group_size,
                  _disable_his, _disable_tick, _disable_min1, _disable_min5,
                  _disable_day, _disable_trans, _disable_ordque,
                  _disable_orddtl, _min_price_mode);
  return true;
}
...
void DataManager::preloadRtCaches(const char* exchg)
{
        ...
        if (ayCts != NULL && ayCts->size() > 0)
        {
                for (auto it = ayCts->begin(); it != ayCts->end(); it++)
                {
                        ...
                        releaseBlock(getTickBlock(ct->getCode(),
TimeUtils::getCurDate(), true)); releaseBlock(getKlineBlock(ct->getCode(),
KP_Minute1, true)); releaseBlock(getKlineBlock(ct->getCode(), KP_Minute5,
true)); cnt += 3; if (isStk && strcmp(commInfo->getProduct(), "STK") == 0)
                        {
                                releaseBlock(getOrdQueBlock(ct->getCode(),
TimeUtils::getCurDate(), true)); releaseBlock(getTransBlock(ct->getCode(),
TimeUtils::getCurDate(), true)); cnt += 2; if (strcmp(ct->getExchg(), "SZSE") ==
0)
                                {
                                        releaseBlock(getOrdDtlBlock(ct->getCode(),
TimeUtils::getCurDate(), true)); cnt++;
                                }
                        }
                }
        }
        ...
        if (ayCts != NULL)
                ayCts->release();
        pipe_writer_log(_sink, LL_INFO,
"预加载%个品种的实时数据缓存文件{}个,耗时{}微秒", codecnt, cnt,
WTSLogger::fmtInt64(ticker.micro_seconds())); _preloaded = true;
}
*/
...
void WtDataWriter::loadCache() {
  ...
  _tick_cache_block->_size =
      min(_tick_cache_block->_size, _tick_cache_block->_capacity);
...
void *WtDataWriter::resizeRTBlock(BoostMFPtr &mfPtr, uint32_t nCount) {
  ...
  try {
    ...
    pipe_writer_log(
        _sink, LL_ERROR,
        "Exception occured while expanding RT cache file {} to {}: {}",
  ...
  try {
    ...
    pipe_writer_log(_sink, LL_ERROR,
                    "Exception occured while mapping RT cache file {}: {}",
                    filename, ex.what());
    return NULL;
  }
...
void WtDataWriter::procTick(WTSTickData *curTick, uint32_t procFlag) {
  do {
    ...
    if (cnt % _log_group_size == 0) {
      pipe_writer_log(_sink, LL_INFO, "{} ticks received from exchange {}", cnt,
                      curTick->exchg());
    }
  } while (false);
}
...
void WtDataWriter::procQueue(WTSOrdQueData *curOrdQue) {
  do {
    ...
    if (blk->_size >= blk->_capacity) {
      ...
      pBlockPair->_block =
          (RTOrdQueBlock *)resizeRTBlock<RTDayBlockHeader, WTSOrdQueStruct>(
              pBlockPair->_file, blk->_capacity * 2);
      blk = pBlockPair->_block;
    }

    memcpy(&blk->_queues[blk->_size], &curOrdQue->getOrdQueStruct(),
           sizeof(WTSOrdQueStruct));
    blk->_size += 1;
    ...
    cnt++;
    if (cnt % _log_group_size == 0) {
      pipe_writer_log(_sink, LL_INFO, "{} queues received from exchange {}",
                      cnt, curOrdQue->exchg());
    }
  } while (false);
}
...
void WtDataWriter::procOrder(WTSOrdDtlData *curOrdDtl) {
  do {
    ...
    if (blk->_size >= blk->_capacity) {
      pBlockPair->_file->sync();
      pBlockPair->_block =
          (RTOrdDtlBlock *)resizeRTBlock<RTDayBlockHeader, WTSOrdDtlStruct>(
              pBlockPair->_file, blk->_capacity * 2);
      blk = pBlockPair->_block;
    }

    memcpy(&blk->_details[blk->_size], &curOrdDtl->getOrdDtlStruct(),
           sizeof(WTSOrdDtlStruct));
    blk->_size += 1;
    ...
    cnt++;
    if (cnt % _log_group_size == 0) {
      pipe_writer_log(_sink, LL_INFO, "{} orders received from exchange {}",
                      cnt, curOrdDtl->exchg());
    }
  } while (false);
}
...
void WtDataWriter::procTrans(WTSTransData *curTrans) {
  do {
    ...
    if (blk->_size >= blk->_capacity) {
      pBlockPair->_file->sync();
      pBlockPair->_block =
          (RTTransBlock *)resizeRTBlock<RTDayBlockHeader, WTSTransStruct>(
              pBlockPair->_file, blk->_capacity * 2);
      blk = pBlockPair->_block;
    }

    memcpy(&blk->_trans[blk->_size], &curTrans->getTransStruct(),
           sizeof(WTSTransStruct));
    blk->_size += 1;
    ...
    if (cnt % _log_group_size == 0) {
      pipe_writer_log(_sink, LL_INFO,
                      "{} transactions received from exchange {}", cnt,
                      curTrans->exchg());
    }
  } while (false);
}
...
void WtDataWriter::pipeToTicks(WTSContractInfo *ct, WTSTickData *curTick) {
  ...
  if (blk && blk->_size >= blk->_capacity) {
    pBlockPair->_file->sync();
    pBlockPair->_block =
        (RTTickBlock *)resizeRTBlock<RTDayBlockHeader, WTSTickStruct>(
            pBlockPair->_file, blk->_capacity * 2);
    blk = pBlockPair->_block;
    if (blk)
      pipe_writer_log(_sink, LL_DEBUG, "RT tick block of {} resized to {}",
                      ct->getFullCode(), blk->_capacity);
  }

  if (blk == NULL) {
    pipe_writer_log(_sink, LL_DEBUG, "RT tick block of {} is not valid",
                    ct->getFullCode());
    return;
  }

  memcpy(&blk->_ticks[blk->_size], &curTick->getTickStruct(),
         sizeof(WTSTickStruct));
  blk->_size += 1;

  if (_save_tick_log && pBlockPair->_fstream) {
    *(pBlockPair->_fstream)
        << curTick->code() << "," << curTick->tradingdate() << ","
        << curTick->actiondate() << "," << curTick->actiontime() << ","
        << TimeUtils::getLocalTime(false) << "," << curTick->price() << ","
        << curTick->totalvolume() << "," << curTick->openinterest() << ","
        << (uint64_t)curTick->totalturnover() << "," << curTick->volume() << ","
        << curTick->additional() << "," << (uint64_t)curTick->turnover()
        << std::endl;
  }
}

WtDataWriter::OrdQueBlockPair *
WtDataWriter::getOrdQueBlock(WTSContractInfo *ct, uint32_t curDate,
  ...
  if (pBlock->_block == NULL) {
    std::string path =
        fmt::format("{}rt/queue/{}/", _base_dir.c_str(), ct->getExchg());
    if (bAutoCreate)
    ...
    if (!BoostFile::exists(path.c_str())) {
      ...
      pipe_writer_log(_sink, LL_INFO,
                      "Data file {} not exists, initializing...", path.c_str());

      uint64_t uSize =
          sizeof(RTDayBlockHeader) + sizeof(WTSOrdQueStruct) * HFT_SIZE_STEP;
    ...
    if (!isNew && pBlock->_block->_date != curDate) {
      pipe_writer_log(_sink, LL_INFO,
                      "date[{}] of orderqueue cache block[{}] is different "
                      "from current date[{}], reinitializing...",
                      pBlock->_block->_date, path.c_str(), curDate);
      pBlock->_block->_size = 0;
      pBlock->_block->_date = curDate;

      memset(&pBlock->_block->_queues, 0,
             sizeof(WTSOrdQueStruct) * pBlock->_block->_capacity);
    }
    ...
    if (isNew) {
      ...
      // 检查缓存文件是否有问题,要自动恢复
      do {
        uint64_t uSize = sizeof(RTDayBlockHeader) +
                         sizeof(WTSOrdQueStruct) * pBlock->_block->_capacity;
        uint64_t oldSize = pBlock->_file->size();
        if (oldSize != uSize) {
          uint32_t oldCnt = (uint32_t)((oldSize - sizeof(RTDayBlockHeader)) /
                                       sizeof(WTSOrdQueStruct));
          ...
          pipe_writer_log(_sink, LL_WARN,
                          "Oderqueue cache file of {} on date {} repaired",
                          ct->getCode(), curDate);
        }
...
WtDataWriter::OrdDtlBlockPair *
WtDataWriter::getOrdDtlBlock(WTSContractInfo *ct, uint32_t curDate,
                             bool bAutoCreate /* = true */) {
  if (ct == NULL)
    return NULL;
  ...
  if (pBlock->_block == NULL) {
    std::string path =
        fmt::format("{}rt/orders/{}/", _base_dir.c_str(), ct->getExchg());
    if (bAutoCreate)
      BoostFile::create_directories(path.c_str());
    path += ct->getCode();
    path += ".dmb";
    ...
    if (!BoostFile::exists(path.c_str())) {
      ...
      uint64_t uSize =
          sizeof(RTDayBlockHeader) + sizeof(WTSOrdDtlStruct) * HFT_SIZE_STEP;
    ...
    if (!isNew && pBlock->_block->_date != curDate) {
      pipe_writer_log(_sink, LL_INFO,
                      "date[{}] of orderdetail cache block[{}] is different "
                      "from current date[{}], reinitializing...",
                      pBlock->_block->_date, path.c_str(), curDate);
      pBlock->_block->_size = 0;
      pBlock->_block->_date = curDate;

      memset(&pBlock->_block->_details, 0,
             sizeof(WTSOrdDtlStruct) * pBlock->_block->_capacity);
    }
    ...
    if (isNew) {
      ...
      for (;;) {
        ...
        if (oldSize != uSize) {
          ...
          pipe_writer_log(_sink, LL_WARN,
                          "Orderdetail cache file of {} on date {} repaired",
                          ct->getCode(), curDate);
        }
...
WtDataWriter::TransBlockPair *
WtDataWriter::getTransBlock(WTSContractInfo *ct, uint32_t curDate,
                            bool bAutoCreate /* = true */) {
  if (ct == NULL)
    return NULL;
  ...
  if (pBlock->_block == NULL) {
    std::string path =
        fmt::format("{}rt/trans/{}/", _base_dir.c_str(), ct->getExchg());
    if (bAutoCreate)
      BoostFile::create_directories(path.c_str());
    path += ct->getCode();
    path += ".dmb";
    ...
    if (!BoostFile::exists(path.c_str())) {
      ...
      uint64_t uSize =
          sizeof(RTDayBlockHeader) + sizeof(WTSTransStruct) * HFT_SIZE_STEP;
    ...
    if (!isNew && pBlock->_block->_date != curDate) {
      pipe_writer_log(_sink, LL_INFO,
                      "date[{}] of transaction cache block[{}] is different "
                      "from current date[{}], reinitializing...",
                      pBlock->_block->_date, path.c_str(), curDate);
      pBlock->_block->_size = 0;
      pBlock->_block->_date = curDate;

      memset(&pBlock->_block->_trans, 0,
             sizeof(WTSTransStruct) * pBlock->_block->_capacity);
    }
    ...
    if (isNew) {
      ...
      for (;;) {
        ...
        if (oldSize != uSize) {
          ...
          pipe_writer_log(_sink, LL_WARN,
                          "Transaction cache file of {} on date {} repaired",
                          ct->getCode(), curDate);
        }
...
WtDataWriter::TickBlockPair *
WtDataWriter::getTickBlock(WTSContractInfo *ct, uint32_t curDate,
                           bool bAutoCreate /* = true */) {
  if (ct == NULL)
    return NULL;
  ...
  if (pBlock->_block == NULL) {
    std::string path =
        fmt::format("{}rt/ticks/{}/", _base_dir.c_str(), ct->getExchg());
    if (bAutoCreate)
      BoostFile::create_directories(path.c_str());
    ...
    if (!BoostFile::exists(path.c_str())) {
      ...
      uint64_t uSize =
          sizeof(RTTickBlock) + sizeof(WTSTickStruct) * HFT_SIZE_STEP;
      BoostFile bf;
      bf.create_new_file(path.c_str());
      bf.truncate_file((uint32_t)uSize);
      bf.close_file();
    ...
    if (!isNew && pBlock->_block->_date != curDate) {
      pipe_writer_log(_sink, LL_INFO,
                      "date[{}] of tick cache block[{}] is different from "
                      "current date[{}], reinitializing...",
                      pBlock->_block->_date, path.c_str(), curDate);
      pBlock->_block->_size = 0;
      pBlock->_block->_date = curDate;

      memset(&pBlock->_block->_ticks, 0,
             sizeof(WTSTickStruct) * pBlock->_block->_capacity);
    }
    ...
    if (isNew) {
      ...
      // 检查缓存文件是否有问题,要自动恢复
      do {
        uint64_t uSize = sizeof(RTTickBlock) +
                         sizeof(WTSTickStruct) * pBlock->_block->_capacity;
        uint64_t realSz = pBlock->_file->size();
        if (realSz != uSize) {
          uint32_t realCap = (uint32_t)((realSz - sizeof(RTTickBlock)) /
                                        sizeof(WTSTickStruct));
          uint32_t markedCap = pBlock->_block->_capacity;
          pipe_writer_log(_sink, LL_WARN,
                          "Tick cache file of {} on {} repaired, real "
                          "capiacity:{}, marked capacity:{}",
...
void WtDataWriter::pipeToKlines(WTSContractInfo *ct, WTSTickData *curTick) {
  ...
  if (minutes == INVALID_UINT32) {
    pipe_writer_log(
        _sink, LL_WARN,
        "[pipeToKlines] [{}.{}] {}.{} is invalid timestamp, skip this tick",
        curTick->exchg(), curTick->code(), curTick->actiondate(),
        curTick->actiontime(), curTime);
    return;
  }
  ...
  if (!_disable_min1) {
    ...
    if (pBlockPair && pBlockPair->_block) {
      ...
      if (blk->_size == blk->_capacity) {
        pBlockPair->_file->sync();
        pBlockPair->_block =
            (RTKlineBlock *)resizeRTBlock<RTKlineBlock, WTSBarStruct>(
                pBlockPair->_file, blk->_capacity * 2);
        blk = pBlockPair->_block;
      }
...
template <typename T> void WtDataWriter::releaseBlock(T *block) {
...
WtDataWriter::KBlockPair *
WtDataWriter::getKlineBlock(WTSContractInfo *ct, WTSKlinePeriod period,
                            bool bAutoCreate /* = true */) {
  if (ct == NULL)
    return NULL;
  ...
  if (pBlock->_block == NULL) {
    ...
    char *s =
        fmt::format_to(path, "{}rt/{}/{}/", _base_dir, subdir, ct->getExchg());
    s[0] = '\0';
    if (bAutoCreate)
    ...
    if (!BoostFile::exists(path)) {
      ...
      pipe_writer_log(_sink, LL_INFO,
                      "Data file {} not exists, initializing...", path);

      uint64_t uSize =
          sizeof(RTKlineBlock) +
          sizeof(WTSBarStruct) * totalMins; // 预分配按照K线条数分配
      BoostFile bf;
    ...
    //	memset(&pBlock->_block->_bars, 0,
    //sizeof(WTSBarStruct)*pBlock->_block->_capacity);
    //}
...
bool WtDataWriter::updateCache(WTSContractInfo *ct, WTSTickData *curTick,
                               uint32_t procFlag) {
  ...
  if (it == _tick_cache_idx.end()) {
    ...
    if (_tick_cache_block->_size >= _tick_cache_block->_capacity) {
      _tick_cache_block =
          (RTTickCache *)resizeRTBlock<RTTickCache, TickCacheItem>(
              _tick_cache_file, _tick_cache_block->_capacity + CACHE_SIZE_STEP);
      pipe_writer_log(_sink, LL_INFO, "Tick Cache resized to {} items",
                      _tick_cache_block->_capacity);
    }
  } else {
    idx = it->second;
  }
  ...
  if (curTick->tradingdate() < item._date) {
    pipe_writer_log(_sink, LL_INFO,
                    "Tradingday[{}] of {} is less than cached tradingday[{}]",
                    curTick->tradingdate(), curTick->code(), item._date);
    return false;
  }
  ...
  if (curTick->tradingdate() > item._date) {
    ...
    if (procFlag == 1) {
      ...
      item._tick.diff_interest =
          item._tick.open_interest - item._tick.pre_interest;
    ...
    //	newTick.trading_date, curTick->exchg(), curTick->code(),
    //curTick->volume(), 	curTick->turnover(), curTick->openinterest(),
    //curTick->additional());
    pipe_writer_log(_sink, LL_INFO,
                    "First tick of new tradingday {},{}.{},{},{},{},{},{}",
                    newTick.trading_date, curTick->exchg(), curTick->code(),
                    curTick->price(), curTick->volume(), curTick->turnover(),
                    curTick->openinterest(), curTick->additional());
  } else {
    // 如果缓存里的数据日期大于最新行情的日期
    // 或者缓存里的时间大于等于最新行情的时间,数据就不需要处理
    WTSSessionInfo *sInfo = ct->getCommInfo()->getSessionInfo();
    uint32_t tdate = sInfo->getOffsetDate(curTick->actiondate(),
                                          curTick->actiontime() / 100000);
    if (tdate > curTick->tradingdate()) {
      pipe_writer_log(
          _sink, LL_WARN,
          "Last tick of {}.{} with time {}.{} has an exception, abandoned",
          curTick->exchg(), curTick->code(), curTick->actiondate(),
          curTick->actiontime());
      return false;
    } else if (curTick->totalvolume() < item._tick.total_volume) {
      pipe_writer_log(_sink, LL_WARN,
                      "Last tick of {}.{} with time {}.{}, volume {} is less "
                      "than cached volume {}, abandoned",
                      curTick->exchg(), curTick->code(), curTick->actiondate(),
                      curTick->actiontime(), curTick->totalvolume(),
                      item._tick.total_volume);
      return false;
    }
    ...
    if (newTick.action_date == item._tick.action_date &&
        newTick.action_time <= item._tick.action_time &&
        newTick.total_volume >= item._tick.total_volume) {
    ...
    // pipe_writer_log(_sink, LL_DEBUG, "Tick cache data updated {}[{}.{}]",
    // newTick.code, newTick.action_date, newTick.action_time);
  }
...
void WtDataWriter::transHisData(const char *sid) {
  ...
  if (_proc_thrd == NULL) {
    _proc_thrd.reset(
        new StdThread(boost::bind(&WtDataWriter::proc_loop, this)));
  } else {
    _proc_cond.notify_all();
  }
}
...
void WtDataWriter::check_loop() {
  ...
  while (!_terminated) {
    ...
    for (auto it = _rt_ticks_blocks.begin(); it != _rt_ticks_blocks.end();
         it++) {
      ...
      if (tBlk->_lasttime != 0 && (now - tBlk->_lasttime > expire_secs)) {
        pipe_writer_log(
            _sink, LL_INFO,
            "tick cache of {} mapping expired, automatically closed", key);
        releaseBlock<TickBlockPair>(tBlk);
      }
    }

    for (auto it = _rt_trans_blocks.begin(); it != _rt_trans_blocks.end();
         it++) {
      const char *key = it->first.c_str();
      ...
      if (tBlk->_lasttime != 0 && (now - tBlk->_lasttime > expire_secs)) {
        pipe_writer_log(
            _sink, LL_INFO,
            "trans cache o {} mapping expired, automatically closed", key);
        releaseBlock<TransBlockPair>(tBlk);
      }
    }

    for (auto it = _rt_orddtl_blocks.begin(); it != _rt_orddtl_blocks.end();
         it++) {
      const char *key = it->first.c_str();
      ...
      if (tBlk->_lasttime != 0 && (now - tBlk->_lasttime > expire_secs)) {
        pipe_writer_log(
            _sink, LL_INFO,
            "order cache of {} mapping expired, automatically closed", key);
    ...
    for (auto &v : _rt_ordque_blocks) {
      ...
      if (tBlk->_lasttime != 0 && (now - tBlk->_lasttime > expire_secs)) {
        pipe_writer_log(
            _sink, LL_INFO,
            "queue cache of {} mapping expired, automatically closed", key);
    ...
    for (auto it = _rt_min1_blocks.begin(); it != _rt_min1_blocks.end(); it++) {
      ...
      if (kBlk->_lasttime != 0 && (now - kBlk->_lasttime > expire_secs)) {
        pipe_writer_log(
            _sink, LL_INFO,
            "min1 cache of {} mapping expired, automatically closed", key);
    ...
    for (auto it = _rt_min5_blocks.begin(); it != _rt_min5_blocks.end(); it++) {
      ...
      if (kBlk->_lasttime != 0 && (now - kBlk->_lasttime > expire_secs)) {
        pipe_writer_log(
            _sink, LL_INFO,
            "min5 cache of {} mapping expired, automatically closed", key);
        releaseBlock<KBlockPair>(kBlk);
      }
    }
  }
...
uint32_t WtDataWriter::dump_bars_via_dumper(WTSContractInfo *ct) {
  ...
  if (it != _tick_cache_idx.end()) {
    ...
    for (auto &item : _dumpers) {
      ...
      if (!bSucc) {
        pipe_writer_log(
            _sink, LL_ERROR,
            "Closing Task of day bar of {} failed via extended dumper {}",
            ct->getFullCode(), id);
      }
    }
  ...
  if (kBlkPair != NULL && kBlkPair->_block->_size > 0) {
    ...
    pipe_writer_log(_sink, LL_INFO, "Transfering min1 bars of {}...",
                    ct->getFullCode());
    ...
    for (auto &item : _dumpers) {
      ...
      bool bSucc =
          dumper->dumpHisBars(key.c_str(), "m1", kBlkPair->_block->_bars, size);
      if (!bSucc) {
        pipe_writer_log(
            _sink, LL_ERROR,
            "Closing Task of m1 bar of {} failed via extended dumper {}",
            ct->getFullCode(), id);
      }
    }
  ...
  if (kBlkPair != NULL && kBlkPair->_block->_size > 0) {
    uint32_t size = kBlkPair->_block->_size;
    pipe_writer_log(_sink, LL_INFO, "Transfering min5 bars of {}...",
                    ct->getFullCode());
    SpinLock lock(kBlkPair->_mutex);
    ...
    for (auto &item : _dumpers) {
      ...
      bool bSucc =
          dumper->dumpHisBars(key.c_str(), "m5", kBlkPair->_block->_bars, size);
      if (!bSucc) {
        pipe_writer_log(
            _sink, LL_ERROR,
            "Closing Task of m5 bar of {} failed via extended dumper {}",
            ct->getFullCode(), id);
      }
    }
...
bool WtDataWriter::proc_block_data(const char *tag, std::string &content,
                                   bool isBar, bool bKeepHead /* = true */) {
  ...
  if (bOldVer) {
    if (isBar) {
      ...
      pipe_writer_log(_sink, LL_INFO,
                      "{} bars of {} transferd to new version...", barcnt, tag);
    } else {
      ...
      buffer.swap(bufv2);
      pipe_writer_log(_sink, LL_INFO,
                      "{} ticks of {} transferd to new version...", tick_cnt,
                      tag);
    }
  }
  ...
  if (f.create_or_open_file(filename.c_str())) {
    ...
    if (bNew) {
      ...
      if (barcnt != 0) {
        ...
        if (oldBS.date == newBar->date &&
            memcmp(&oldBS, newBar, sizeof(WTSBarStruct)) != 0) {
          // 日期相同且数据不同,则用最新的替换最后一条
          oldBS = *newBar;
        } else if (oldBS.date <
                   newBar->date) // 老的K线日期小于新的,则直接追加到后面
        {
      ...
      if (bNeedCompress) {
        std::string cmpData =
            WTSCmpHelper::compress_data(content.data(), content.size());
    ...
    pipe_writer_log(
        _sink, LL_ERROR,
        "ClosingTask of day bar failed: openning history data file {} failed",
        filename.c_str());
    return false;
  }
}
...
uint32_t WtDataWriter::dump_bars_to_file(WTSContractInfo *ct) {
  ...
  if (!_disable_min1) {
    ...
    if (kBlkPair != NULL && kBlkPair->_block->_size > 0) {
      ...
      pipe_writer_log(_sink, LL_INFO, "Openning data storage faile: {}",
                      filename.c_str());
      ...
      if (f.create_or_open_file(filename.c_str())) {
        ...
        // 追加新的数据
        buffer.append((const char *)kBlkPair->_block->_bars,
                      sizeof(WTSBarStruct) * size);

        std::string cmpData =
            WTSCmpHelper::compress_data(buffer.data(), buffer.size());
        ...
        // 最后将缓存清空
        // memset(kBlkPair->_block->_bars, 0,
        // sizeof(WTSBarStruct)*kBlkPair->_block->_size);
        kBlkPair->_block->_size = 0;
      } else {
        pipe_writer_log(_sink, LL_ERROR,
                        "ClosingTask of min1 bar failed: openning history data "
                        "file {} failed",
                        filename.c_str());
      }
    }
  ...
  if (!_disable_min5) {
    ...
    if (kBlkPair != NULL && kBlkPair->_block->_size > 0) {
      uint32_t size = kBlkPair->_block->_size;
      pipe_writer_log(_sink, LL_INFO, "Transfering min5 bar of {}...",
                      ct->getFullCode());
      SpinLock lock(kBlkPair->_mutex);
      ...
      BoostFile::create_directories(ss.str().c_str());
      std::string filename =
          fmtutil::format("{}{}.dsb", path.c_str(), ct->getCode());

      bool bNew = false;
      if (!BoostFile::exists(filename.c_str()))
        bNew = true;

      pipe_writer_log(_sink, LL_INFO, "Openning data storage file: {}",
                      filename.c_str());
      ...
      if (f.create_or_open_file(filename.c_str())) {
        ...
        pipe_writer_log(_sink, LL_ERROR,
                        "ClosingTask of min5 bar failed: openning history data "
                        "file {} failed",
                        filename.c_str());
      }
    }
...
void WtDataWriter::proc_loop() {
  while (!_terminated) {
    ...
    if (fullcode.compare(CMD_CLEAR_CACHE) == 0) {
      ...
      ss_snapshot
          << "date,exchg,code,open,high,low,close,settle,volume,turnover,"
             "openinterest,upperlimit,lowerlimit,preclose,presettle,preinterest"
          << std::endl
          << std::fixed;
      for (auto it = _tick_cache_idx.begin(); it != _tick_cache_idx.end();
           it++) {
        const char *key = it->first.c_str();
        const StringVector &ay = StrUtil::split(key, ".");
        WTSContractInfo *ct =
            _bd_mgr->getContract(ay[1].c_str(), ay[0].c_str());
        if (ct != NULL) {
          setCodes.insert(key);
          ...
          const TickCacheItem &tci = _tick_cache_block->_ticks[idx];
          const WTSTickStruct &ts = tci._tick;
          ss_snapshot << ts.trading_date << "," << ts.exchg << "," << ts.code
                      << "," << ts.open << "," << ts.high << "," << ts.low
                      << "," << ts.price << "," << ts.settle_price << ","
                      << ts.total_volume << "," << ts.total_turnover << ","
                      << ts.open_interest << "," << ts.upper_limit << ","
                      << ts.lower_limit << "," << ts.pre_close << ","
                      << ts.pre_settle << "," << ts.pre_interest << std::endl;
        } else {
          pipe_writer_log(_sink, LL_WARN,
                          "{}[{}] expired, cache will be cleared",
                          ay[1].c_str(), ay[0].c_str());

          // 删除已经过期代码的实时tick文件
          std::string path =
              fmtutil::format("{}rt/ticks/{}/{}.dmb", _base_dir, ay[0], ay[1]);
      ...
      if (setCodes.size() != _tick_cache_idx.size()) {
        ...
        uint32_t size = sizeof(RTTickCache) +
                        sizeof(TickCacheItem) * scale * CACHE_SIZE_STEP;
        std::string buffer;
        ...
        for (const std::string &key : setCodes) {
          ...
          memcpy(&newCache->_ticks[newIdx], &_tick_cache_block->_ticks[oldIdx],
                 sizeof(TickCacheItem));
        ...
        pipe_writer_log(_sink, LL_INFO, "{} expired cache cleared totally",
                        diff);
      }
      ...
      do {
        if (try_count >= 5) {
          pipe_writer_log(_sink, LL_ERROR,
                          "Too many trys to clear rt cache files，skip");
          break;
        }
        ...
        try {
          ...
          pipe_writer_log(
              _sink, LL_ERROR,
              "Error occured while clearing rt cache files，retry in 300s");
      ...
      iniHelper.save();
      pipe_writer_log(_sink, LL_INFO,
                      "ClosingTask mark of Trading session [{}] updated: {}",
                      sid.c_str(), curDate);
    }
    ...
    if (!_disable_his) {
      ...
      if (!_disable_tick) {
        ...
        if (tBlkPair != NULL) {
          ...
          if (tBlkPair->_block->_size > 0) {
            pipe_writer_log(_sink, LL_INFO, "Transfering tick data of {}...",
                            fullcode.c_str());
            ...
            for (auto &item : _dumpers) {
              const char *id = item.first.c_str();
              IHisDataDumper *dumper = item.second;
              bool bSucc = dumper->dumpHisTicks(
                  fullcode.c_str(), tBlkPair->_block->_date,
                  tBlkPair->_block->_ticks, tBlkPair->_block->_size);
              if (!bSucc) {
                pipe_writer_log(_sink, LL_ERROR,
                                "ClosingTask of tick of {} on {} via extended "
                                "dumper {} failed",
                                fullcode.c_str(), tBlkPair->_block->_date, id);
              }
            }
            ...
            { //////////////////////////////////////////////////////////////////////////
              ...
              std::stringstream ss;
              ss << _base_dir << "his/ticks/" << ct->getExchg() << "/"
                 << tBlkPair->_block->_date << "/";
              std::string path = ss.str();
              ...
              if (f.create_new_file(filename.c_str())) {
                // 先压缩数据
                std::string cmp_data = WTSCmpHelper::compress_data(
                    tBlkPair->_block->_ticks,
                    sizeof(WTSTickStruct) * tBlkPair->_block->_size);
                ...
                // 最后将缓存清空
                // memset(tBlkPair->_block->_ticks, 0,
                // sizeof(WTSTickStruct)*tBlkPair->_block->_size);
                tBlkPair->_block->_size = 0;
              } else {
                pipe_writer_log(_sink, LL_ERROR,
                                "ClosingTask of tick failed: openning history "
                                "data file {} failed",
                                filename.c_str());
              }
            }
      ...
      if (!_disable_trans) {
        ...
        if (tBlkPair != NULL && tBlkPair->_block->_size > 0) {
          pipe_writer_log(_sink, LL_INFO,
                          "Transfering transaction data of {}...",
                          fullcode.c_str());
          SpinLock lock(tBlkPair->_mutex);
          ...
          for (auto &item : _dumpers) {
            const char *id = item.first.c_str();
            IHisDataDumper *dumper = item.second;
            bool bSucc = dumper->dumpHisTrans(
                fullcode.c_str(), tBlkPair->_block->_date,
                tBlkPair->_block->_trans, tBlkPair->_block->_size);
            if (!bSucc) {
              pipe_writer_log(_sink, LL_ERROR,
                              "ClosingTask of transaction of {} on {} via "
                              "extended dumper {} failed",
                              fullcode.c_str(), tBlkPair->_block->_date, id);
            }
          }
          ...
          {
            std::stringstream ss;
            ss << _base_dir << "his/trans/" << ct->getExchg() << "/"
               << tBlkPair->_block->_date << "/";
            std::string path = ss.str();
            pipe_writer_log(_sink, LL_INFO, path.c_str());
            BoostFile::create_directories(ss.str().c_str());
            ...
            if (f.create_new_file(filename.c_str())) {
              // 先压缩数据
              std::string cmp_data = WTSCmpHelper::compress_data(
                  tBlkPair->_block->_trans,
                  sizeof(WTSTransStruct) * tBlkPair->_block->_size);
              ...
              pipe_writer_log(_sink, LL_ERROR,
                              "ClosingTask of transaction failed: openning "
                              "history data file {} failed",
                              filename.c_str());
            }
          }
        }
      ...
      if (!_disable_orddtl) {
        ...
        if (tBlkPair != NULL && tBlkPair->_block->_size > 0) {
          pipe_writer_log(_sink, LL_INFO,
                          "Transfering order detail data of {}...",
                          fullcode.c_str());
          SpinLock lock(tBlkPair->_mutex);
          ...
          for (auto &item : _dumpers) {
            const char *id = item.first.c_str();
            IHisDataDumper *dumper = item.second;
            bool bSucc = dumper->dumpHisOrdDtl(
                fullcode.c_str(), tBlkPair->_block->_date,
                tBlkPair->_block->_details, tBlkPair->_block->_size);
            if (!bSucc) {
              pipe_writer_log(_sink, LL_ERROR,
                              "ClosingTask of order details of {} on {} via "
                              "extended dumper {} failed",
                              fullcode.c_str(), tBlkPair->_block->_date, id);
            }
          }
          ...
          {
            std::stringstream ss;
            ss << _base_dir << "his/orders/" << ct->getExchg() << "/"
               << tBlkPair->_block->_date << "/";
            std::string path = ss.str();
            pipe_writer_log(_sink, LL_INFO, path.c_str());
            BoostFile::create_directories(ss.str().c_str());
            ...
            if (f.create_new_file(filename.c_str())) {
              // 先压缩数据
              std::string cmp_data = WTSCmpHelper::compress_data(
                  tBlkPair->_block->_details,
                  sizeof(WTSOrdDtlStruct) * tBlkPair->_block->_size);
              ...
              pipe_writer_log(_sink, LL_ERROR,
                              "ClosingTask of order detail failed: openning "
                              "history data file {} failed",
                              filename.c_str());
            }
          }
      ...
      if (!_disable_ordque) {
        ...
        if (tBlkPair != NULL && tBlkPair->_block->_size > 0) {
          pipe_writer_log(_sink, LL_INFO,
                          "Transfering order queue data of {}...",
                          fullcode.c_str());
          SpinLock lock(tBlkPair->_mutex);
          ...
          for (auto &item : _dumpers) {
            const char *id = item.first.c_str();
            IHisDataDumper *dumper = item.second;
            bool bSucc = dumper->dumpHisOrdQue(
                fullcode.c_str(), tBlkPair->_block->_date,
                tBlkPair->_block->_queues, tBlkPair->_block->_size);
            if (!bSucc) {
              pipe_writer_log(_sink, LL_ERROR,
                              "ClosingTask of order queues of {} on {} via "
                              "extended dumper {} failed",
                              fullcode.c_str(), tBlkPair->_block->_date, id);
            }
          }
          ...
          {
            std::stringstream ss;
            ss << _base_dir << "his/queue/" << ct->getExchg() << "/"
               << tBlkPair->_block->_date << "/";
            std::string path = ss.str();
            pipe_writer_log(_sink, LL_INFO, path.c_str());
            BoostFile::create_directories(ss.str().c_str());
            ...
            if (f.create_new_file(filename.c_str())) {
              // 先压缩数据
              std::string cmp_data = WTSCmpHelper::compress_data(
                  tBlkPair->_block->_queues,
                  sizeof(WTSOrdQueStruct) * tBlkPair->_block->_size);
              ...
              pipe_writer_log(_sink, LL_ERROR,
                              "ClosingTask of order queue failed: openning "
                              "history data file {} failed",
                              filename.c_str());
            }
          }
      ...
      pipe_writer_log(_sink, LL_INFO,
                      "ClosingTask of {}[{}] done, {} datas processed totally",
                      ct->getCode(), ct->getExchg(), count);
    } else {
      pipe_writer_log(
          _sink, LL_INFO,
          "ClosingTask of {}[{}] skipped due to history data disabled",
          ct->getCode(), ct->getExchg());
    }
  }
}