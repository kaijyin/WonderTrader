...
std::vector<uint32_t> splitVolume(uint32_t vol, uint32_t minQty = 1,
                                  uint32_t maxQty = 100) {
...
TraderMocker::TraderMocker()
    : _terminated(false), _listener(NULL), _ticks(NULL), _orders(NULL),
      _awaits(NULL), _trades(NULL), _b_socket(NULL), _max_tick_time(0),
      _last_match_time(0) {
  _auto_order_id = (uint32_t)((TimeUtils::getLocalTimeNow() -
                               TimeUtils::makeTime(20200101, 0)) /
                              1000 * 100);
  _auto_trade_id = (uint32_t)((TimeUtils::getLocalTimeNow() -
                               TimeUtils::makeTime(20200101, 0)) /
                              1000 * 300);
  _auto_entrust_id = (uint32_t)((TimeUtils::getLocalTimeNow() -
                                 TimeUtils::makeTime(20200101, 0)) /
                                1000 * 100);
}
...
uint32_t TraderMocker::makeTradeID() { return ++_auto_trade_id; }

uint32_t TraderMocker::makeOrderID() { return ++_auto_order_id; }
...
bool TraderMocker::makeEntrustID(char *buffer, int length) {
  ...
  try {
    fmtutil::format_to(buffer, "me.{}.{}.{}", TimeUtils::getCurDate(),
                       _mocker_id, _auto_entrust_id++);
    return true;
  } catch (...) {
  }
  ...
  _io_service.post([this, entrust]() {
    ...
    do {
      ...
      if (entrust->getPriceType() == WPT_ANYPRICE &&
          commInfo->getPriceMode() == PM_Limit) {
        bPass = false;
        msg = "价格类型不合法";
        break;
      }
      ...
      if ((commInfo->getCategoty() == CC_Stock) &&
          (entrust->getOffsetType() == WOT_OPEN) &&
          !decimal::eq(decimal::mod(entrust->getVolume(), 100), 0)) {
        bPass = false;
      ...
      if (!decimal::eq(entrust->getPrice(), 0)) {
        ...
        if (!decimal::eq(
                decimal::mod(entrust->getPrice(), pricetick),
                0)) // 整除的检查方式,先小数相除得到商,然后商取整以后,再跟原来的商相减,如果等于0,则是整除,否则是
        {
          bPass = false;
          msg = "委托价格不合法";
          break;
      ...
      if (commInfo->getCoverMode() == CM_CoverToday &&
          (entrust->getOffsetType() == WOT_CLOSE ||
           entrust->getOffsetType() == WOT_CLOSEYESTERDAY)) {
        bPass = false;
        msg = "没有足够的可平仓位";
        break;
      }
      ...
      double validQty = isLong ? (pItem._long._volume - pItem._long._frozen)
                               : (pItem._short._volume - pItem._short._frozen);
    ...
    if (bPass) {
      ...
      if (_listener) {
        write_log(_listener, LL_INFO, "共有{}个品种有待撮合订单",
                  _codes.size());
      }
...
int32_t TraderMocker::match_once() {
  ...
  for (const std::string &fullcode : _codes) {
    ...
    if (curTick && strcmp(curTick->code(), ct->getCode()) == 0) {
      ...
      uint64_t tickTime =
          (uint64_t)curTick->actiondate() * 1000000000 + curTick->actiontime();
      if (decimal::gt(curTick->price(),
                      0) /*&& tickTime >= _last_match_time*/) {
        // 开始处理订单
        // 处理记录
        ...
        for (auto it = _awaits->begin(); it != _awaits->end(); it++) {
          ...
          if (ordInfo->getVolLeft() == 0 ||
              strcmp(ordInfo->getCode(), curTick->code()) != 0)
            continue;

          bool isBuy = (ordInfo->getDirection() == WDT_LONG &&
                        ordInfo->getOffsetType() == WOT_OPEN) ||
                       (ordInfo->getDirection() != WDT_LONG &&
                        ordInfo->getOffsetType() != WOT_OPEN);
          ...
          if (ordInfo->getPriceType() == WPT_LIMITPRICE &&
              ((isBuy && decimal::lt(target, uPrice)) ||
               (!isBuy && decimal::gt(target, uPrice))))
            continue;

          count++;

          double maxVolume = min(uVolume, ordInfo->getVolLeft());
          std::vector<uint32_t> ayVol = splitVolume(
              (uint32_t)maxVolume, (uint32_t)_min_qty, (uint32_t)_max_qty);
          for (uint32_t curVol : ayVol) {

            WTSTradeInfo *trade =
                WTSTradeInfo::create(curTick->code(), curTick->exchg());
        ...
        if (count > 0) {
          // write_log(_listener,LL_INFO, "[TraderMocker]触发 %s.%s 开多 %u
          // 条,价格:%u", tick->exchg(), tick->code(), iCount, uPrice);
...
void TraderMocker::load_positions() {
  ...
  if (_listener)
    write_log(_listener, LL_INFO, "[TraderMocker]共加载{}条持仓数据",
              _positions.size());
}
...
void TraderMocker::reconn_udp() {
  ...
  _b_socket->async_receive_from(
      boost::asio::buffer(_b_buffer), _broad_ep,
...
void TraderMocker::connect() {
  ...
  _thrd_worker.reset(
      new StdThread(boost::bind(&boost::asio::io_service::run, &_io_service)));
...
bool TraderMocker::isConnected() { return _thrd_match != NULL; }

int TraderMocker::login(const char *user, const char *pass,
...
int TraderMocker::logout() { return 0; }
...
int TraderMocker::orderAction(WTSEntrustAction *action) {
  ...
  _io_service.post([this, action]() {
    StdUniqueLock lck(
        _mtx_awaits); // 一定要把awaits锁起来,不然可能会导致一边撮合一边撤单
     ...
     */
    if (ordInfo == NULL) {
      write_log(_listener, LL_ERROR, "订单{}不存在或者已完成",
                action->getOrderID());
      WTSError *err =
          WTSError::create(WEC_ORDERCANCEL, "订单不存在或者处于不可撤销状态");
      if (_listener)
    ...
    for (auto &v : _positions) {
      ...
      if (pItem._long._volume > 0) {
        WTSPositionItem *pInfo = WTSPositionItem::create(
            pItem._code, commInfo->getCurrency(), pItem._exchg);
...
void TraderMocker::handle_read(const boost::system::error_code &e,
                               std::size_t bytes_transferred,
                               bool isBroad /* = true */) {
  if (e) {
    if (_listener)
      write_log(_listener, LL_ERROR, "[TraderMocker]UDP行情接收出错:{}({})",
...
// UDPTick数据包
template <typename T> struct UDPDataPacket : UDPPacketHead {
  T _data;
};
#pragma pack(pop)
...
void TraderMocker::extract_buffer(uint32_t length, bool isBroad /* = true */) {
  ...
  if (header->_type == UDP_MSG_PUSHTICK) {
    ...
    fmtutil::format_to(fullcode, "{}.{}", packet->_data.exchg,
                       packet->_data.code);
    ...
    _max_tick_time =
        max(_max_tick_time, (uint64_t)curTick->actiondate() * 1000000000 +
                                curTick->actiontime());
  }
}