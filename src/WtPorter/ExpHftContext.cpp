...
void ExpHftContext::on_bar(const char *code, const char *period, uint32_t times,
                           WTSBarStruct *newBar) {
  if (newBar == NULL)
    return;
...
void ExpHftContext::on_entrust(uint32_t localid, const char *stdCode,
...
void ExpHftContext::on_order(uint32_t localid, const char *stdCode, bool isBuy,
                             double totalQty, double leftQty, double price,
                             bool isCanceled) {
  getRunner().hft_on_order(_context_id, localid, stdCode, isBuy, totalQty,
                           leftQty, price, isCanceled, getOrderTag(localid));

  HftStraBaseCtx::on_order(localid, stdCode, isBuy, totalQty, leftQty, price,
                           isCanceled);
}

void ExpHftContext::on_position(const char *stdCode, bool isLong, double prevol,
                                double preavail, double newvol, double newavail,
                                uint32_t tradingday) {
  getRunner().hft_on_position(_context_id, stdCode, isLong, prevol, preavail,
                              newvol, newavail);
}
