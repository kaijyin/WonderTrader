...
WtRtRunner::WtRtRunner()
    : _data_store(NULL), _cb_cta_init(NULL), _cb_cta_tick(NULL),
      _cb_cta_calc(NULL), _cb_cta_bar(NULL), _cb_cta_cond_trigger(NULL),
      _cb_cta_sessevt(NULL)

      ,
      _cb_sel_init(NULL), _cb_sel_tick(NULL), _cb_sel_calc(NULL),
      _cb_sel_bar(NULL), _cb_sel_sessevt(NULL)

      ,
      _cb_hft_init(NULL), _cb_hft_tick(NULL), _cb_hft_bar(NULL),
      _cb_hft_ord(NULL), _cb_hft_trd(NULL), _cb_hft_entrust(NULL),
      _cb_hft_chnl(NULL)

      ,
      _cb_hft_orddtl(NULL), _cb_hft_ordque(NULL), _cb_hft_trans(NULL),
      _cb_hft_position(NULL), _cb_hft_sessevt(NULL)
      ...
      ,
      _ext_fnl_bar_loader(NULL), _ext_raw_bar_loader(NULL),
...
bool WtRtRunner::init(const char *logCfg /* = "logcfg.prop" */,
                      bool isFile /* = true */, const char *genDir) {
#ifdef _MSC_VER
...
void WtRtRunner::registerParserPorter(FuncParserEvtCallback cbEvt,
...
void WtRtRunner::registerExecuterPorter(FuncExecInitCallback cbInit,
...
void WtRtRunner::registerCtaCallbacks(
...
void WtRtRunner::registerSelCallbacks(FuncStraInitCallback cbInit,
...
void WtRtRunner::registerHftCallbacks(
    ...
    FuncSessionEvtCallback cbSessEvt, FuncHftPosCallback cbPosition) {
  _cb_hft_init = cbInit;
  _cb_hft_tick = cbTick;
  _cb_hft_bar = cbBar;
...
bool WtRtRunner::loadFinalHisBars(void *obj, const char *stdCode,
                                  WTSKlinePeriod period, FuncReadBars cb) {
...
bool WtRtRunner::loadRawHisBars(void *obj, const char *stdCode,
                                WTSKlinePeriod period, FuncReadBars cb) {
  StdUniqueLock lock(_feed_mtx);
...
bool WtRtRunner::loadAdjFactors(void *obj, const char *stdCode,
                                FuncReadFactors cb) {
  StdUniqueLock lock(_feed_mtx);
  if (_ext_adj_fct_loader == NULL)
    return false;
...
void WtRtRunner::feedAdjFactors(const char *stdCode, uint32_t *dates,
...
void WtRtRunner::feedRawBars(WTSBarStruct *bars, uint32_t count) {
  if (_ext_fnl_bar_loader == NULL) {
    WTSLogger::error(
        "Cannot feed bars because of no extented bar loader registered.");
    return;
  }
...
bool WtRtRunner::createExtParser(const char *id) {
  ...
  adapter->initExt(id, parser, _engine, _engine->get_basedata_mgr(),
                   _engine->get_hot_mgr());
...
uint32_t WtRtRunner::createHftContext(const char *name, const char *trader,
                                      bool bAgent, int32_t slippage /* = 0 */) {
  ...
  if (trdPtr) {
    ...
    WTSLogger::error(
        "Trader {} not exists, Binding trader to HFT strategy failed", trader);
  }
  return ctx->id();
}

uint32_t WtRtRunner::createSelContext(const char *name, uint32_t date,
...
void WtRtRunner::ctx_on_bar(uint32_t id, const char *stdCode,
                            const char *period, WTSBarStruct *newBar,
...
void WtRtRunner::ctx_on_calc(uint32_t id, uint32_t curDate, uint32_t curTime,
                             EngineType eType /* = ET_CTA */) {
  switch (eType) {
  case ET_CTA:
    if (_cb_cta_calc)
...
void WtRtRunner::ctx_on_cond_triggered(uint32_t id, const char *stdCode,
                                       double target, double price,
                                       const char *usertag,
                                       EngineType eType /* = ET_CTA */) {
  switch (eType) {
  case ET_CTA:
...
void WtRtRunner::ctx_on_session_event(uint32_t id, uint32_t curTDate,
                                      bool isBegin /* = true */,
                                      EngineType eType /* = ET_CTA */) {
  switch (eType) {
  case ET_CTA:
...
void WtRtRunner::ctx_on_tick(uint32_t id, const char *stdCode,
                             WTSTickData *newTick,
                             EngineType eType /* = ET_CTA */) {
  switch (eType) {
  case ET_CTA:
...
void WtRtRunner::hft_on_entrust(uint32_t cHandle, WtUInt32 localid,
                                const char *stdCode, bool bSuccess,
                                const char *message, const char *userTag) {
  if (_cb_hft_entrust)
    _cb_hft_entrust(cHandle, localid, stdCode, bSuccess, message, userTag);
}

void WtRtRunner::hft_on_order(uint32_t cHandle, WtUInt32 localid,
                              const char *stdCode, bool isBuy, double totalQty,
                              double leftQty, double price, bool isCanceled,
                              const char *userTag) {
  if (_cb_hft_ord)
    _cb_hft_ord(cHandle, localid, stdCode, isBuy, totalQty, leftQty, price,
                isCanceled, userTag);
}

void WtRtRunner::hft_on_trade(uint32_t cHandle, WtUInt32 localid,
                              const char *stdCode, bool isBuy, double vol,
                              double price, const char *userTag) {
  if (_cb_hft_trd)
    _cb_hft_trd(cHandle, localid, stdCode, isBuy, vol, price, userTag);
}

void WtRtRunner::hft_on_position(uint32_t cHandle, const char *stdCode,
                                 bool isLong, double prevol, double preavail,
                                 double newvol, double newavail) {
  if (_cb_hft_position)
    _cb_hft_position(cHandle, stdCode, isLong, prevol, preavail, newvol,
                     newavail);
}

bool WtRtRunner::config(const char *cfgFile, bool isFile /* = true */) {
  _config = isFile ? WTSCfgLoader::load_from_file(cfgFile)
                   : WTSCfgLoader::load_from_content(cfgFile, false);
...
bool WtRtRunner::initHftStrategies() {
  ...
  for (uint32_t idx = 0; idx < cfg->size(); idx++) {
    ...
    stra->self()->init(cfgItem->get("params"));
    HftStraContext *ctx =
        new HftStraContext(&_hft_engine, id, bAgent, slippage);
    ctx->set_strategy(stra->self());
    ...
    if (trader) {
      ...
      WTSLogger::error(
          "Trader {} not exists, Binding trader to HFT strategy failed",
          traderid);
    }
...
bool WtRtRunner::initParsers(WTSVariant *cfgParsers) {
  ...
  for (uint32_t idx = 0; idx < cfgParsers->size(); idx++) {
    ...
    ParserAdapterPtr adapter(new ParserAdapter);
    adapter->init(realid.c_str(), cfgItem, _engine, _engine->get_basedata_mgr(),
                  _engine->get_hot_mgr());
...
bool WtRtRunner::initExecuters(WTSVariant *cfgExecuter) {
  ...
  for (uint32_t idx = 0; idx < cfgExecuter->size(); idx++) {
    ...
    if (name == "local") {
      WtLocalExecuter *executer =
          new WtLocalExecuter(&_exe_factory, id, &_data_mgr);
      ...
      if (strlen(tid) == 0) {
        ...
        if (trader) {
          ...
          WTSLogger::error(
              "Trader {} not exists, cannot configured for executer %s", tid,
              id);
        }
      }
      ...
      WtDiffExecuter *executer =
          new WtDiffExecuter(&_exe_factory, id, &_data_mgr, &_bd_mgr);
      if (!executer->init(cfgItem))
        return false;
      ...
      WtArbiExecuter *executer =
          new WtArbiExecuter(&_exe_factory, id, &_data_mgr);
      if (!executer->init(cfgItem))
        return false;
...
void WtRtRunner::run(bool bAsync /* = false */) {
  try {
    ...
    print_stack_trace([](const char *message) { WTSLogger::error(message); });
  }
}
...
void WtRtRunner::release() { WTSLogger::stop(); }
...
void WtRtRunner::hft_on_order_queue(uint32_t id, const char *stdCode,
                                    WTSOrdQueData *newOrdQue) {
  if (_cb_hft_ordque)
    _cb_hft_ordque(id, stdCode, &newOrdQue->getOrdQueStruct());
}

void WtRtRunner::hft_on_order_detail(uint32_t id, const char *stdCode,
                                     WTSOrdDtlData *newOrdDtl) {
  if (_cb_hft_orddtl)
    _cb_hft_orddtl(id, stdCode, &newOrdDtl->getOrdDtlStruct());
}

void WtRtRunner::hft_on_transaction(uint32_t id, const char *stdCode,
...
void WtRtRunner::on_ext_parser_quote(const char *id, WTSTickStruct *curTick,
                                     uint32_t uProcFlag) {
...
void WtRtRunner::executer_set_position(const char *id, const char *stdCode,
                                       double target) {
  if (_cb_exec_cmd)
