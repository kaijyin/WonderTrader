...
void register_hft_callbacks(
  ...
  getRunner().registerHftCallbacks(cbInit, cbTick, cbBar, cbChnl, cbOrd, cbTrd,
                                   cbEntrust, cbOrdDtl, cbOrdQue, cbTrans,
                                   cbSessEvt, cbPosition);
}
...
void cta_sub_bar_events(CtxHandler cHandle, const char *stdCode,
                        const char *period) {
  CtaContextPtr ctx = getRunner().getCtaContext(cHandle);
  if (ctx == NULL)
    return;
...
CtxHandler create_sel_context(const char *name, uint32_t date, uint32_t time,
                              const char *period,
                              const char *trdtpl /* = "CHINA"*/,
                              const char *session /* = "TRADING"*/,
                              int32_t slippage /* = 0*/) {
  return getRunner().createSelContext(name, date, time, period, slippage,
                                      trdtpl, session);
}
...
WtString sel_load_userdata(CtxHandler cHandle, const char *key,
                           const char *defVal) {
  SelContextPtr ctx = getRunner().getSelContext(cHandle);
  if (ctx == NULL)
    return defVal;
...
WtUInt32 sel_get_date() { return getRunner().getEngine()->get_date(); }

WtUInt32 sel_get_time() { return getRunner().getEngine()->get_min_time(); }
...
double sel_get_position(CtxHandler cHandle, const char *stdCode,
                        bool bOnlyValid, const char *openTag) {
  SelContextPtr ctx = getRunner().getSelContext(cHandle);
  if (ctx == NULL)
    return 0;
...
WtUInt32 sel_get_bars(CtxHandler cHandle, const char *stdCode,
  ...
  try {
    ...
    if (kData) {
      ...
      for (uint32_t i = 0; i < kData->get_block_counts(); i++)
        cb(cHandle, stdCode, period, kData->get_block_addr(i),
           kData->get_block_size(i), i == kData->get_block_counts() - 1);
...
void sel_set_position(CtxHandler cHandle, const char *stdCode, double qty,
                      const char *userTag) {
  SelContextPtr ctx = getRunner().getSelContext(cHandle);
  if (ctx == NULL)
    return;
...
WtUInt32 sel_get_ticks(CtxHandler cHandle, const char *stdCode,
                       WtUInt32 tickCnt, FuncGetTicksCallback cb) {
  SelContextPtr ctx = getRunner().getSelContext(cHandle);
  if (ctx == NULL)
    return 0;
...
WtUInt32 sel_get_tdate() { return getRunner().getEngine()->get_trading_date(); }
...
WtUInt64 sel_get_detail_entertime(CtxHandler cHandle, const char *stdCode,
                                  const char *openTag) {
  SelContextPtr ctx = getRunner().getSelContext(cHandle);
  if (ctx == NULL)
    return 0;
...
double sel_get_detail_cost(CtxHandler cHandle, const char *stdCode,
                           const char *openTag) {
  SelContextPtr ctx = getRunner().getSelContext(cHandle);
  if (ctx == NULL)
    return 0;
...
double sel_get_detail_profit(CtxHandler cHandle, const char *stdCode,
                             const char *openTag, int flag) {
  SelContextPtr ctx = getRunner().getSelContext(cHandle);
  if (ctx == NULL)
    return 0;
...
WtUInt32 hft_get_date() { return getRunner().getEngine()->get_date(); }

WtUInt32 hft_get_time() { return getRunner().getEngine()->get_raw_time(); }

WtUInt32 hft_get_secs() { return getRunner().getEngine()->get_secs(); }

WtUInt32 hft_get_bars(CtxHandler cHandle, const char *stdCode,
                      const char *period, WtUInt32 barCnt,
                      FuncGetBarsCallback cb) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
...
WtUInt32 hft_get_ticks(CtxHandler cHandle, const char *stdCode,
                       WtUInt32 tickCnt, FuncGetTicksCallback cb) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
    return 0;
...
WtUInt32 hft_get_ordque(CtxHandler cHandle, const char *stdCode,
                        WtUInt32 itemCnt, FuncGetOrdQueCallback cb) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
    return 0;
  try {
...
WtUInt32 hft_get_orddtl(CtxHandler cHandle, const char *stdCode,
                        WtUInt32 itemCnt, FuncGetOrdDtlCallback cb) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
    return 0;
  try {
...
WtUInt32 hft_get_trans(CtxHandler cHandle, const char *stdCode,
                       WtUInt32 itemCnt, FuncGetTransCallback cb) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
    return 0;
  try {
...
WtString hft_buy(CtxHandler cHandle, const char *stdCode, double price,
                 double qty, const char *userTag, int flag) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
    return "";
...
WtString hft_sell(CtxHandler cHandle, const char *stdCode, double price,
                  double qty, const char *userTag, int flag) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
    return "";
...
WtString hft_load_userdata(CtxHandler cHandle, const char *key,
                           const char *defVal) {
  HftContextPtr ctx = getRunner().getHftContext(cHandle);
  if (ctx == NULL)
    return defVal;
