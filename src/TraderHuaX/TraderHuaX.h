...
private:
  ...
  makePositionItem(CTORATstpPositionField *positionField);
  inline WTSAccountInfo *
  makeAccountInfo(CTORATstpTradingAccountField *accountField);
...
private:
  ...
  std::string _flowdir;
  bool
      _encrypt; // 为网络数据是否加密传输，考虑数据安全性，建议以互联网方式接入的终端设置为加密传输
