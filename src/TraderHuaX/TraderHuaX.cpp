...
TraderHuaX::TraderHuaX()
    : _api(NULL), _sink(NULL), _ordref(makeRefID()), _reqid(1), _accounts(NULL),
      _orders(NULL), _trades(NULL), _positions(NULL), _bd_mgr(NULL),
      _tradingday(0), _inited(false), _terminal("PC"), _encrypt(false),
      _flowdir("HuaXTDFlow") {}
...
WTSEntrust *TraderHuaX::makeEntrust(CTORATstpInputOrderField *entrust_info) {
  ...
  if (entrust_info->ExchangeID == TORA_TSTP_EXD_SSE)
    ...
    write_log(_sink, LL_WARN, "[TraderHuaX] unsupport exchange {}",
              entrust_info->ExchangeID);
    return nullptr;
  }
  ...
  WTSEntrust *pRet = WTSEntrust::create(
      code.c_str(), (uint32_t)entrust_info->VolumeTotalOriginal,
      entrust_info->OrderPriceType, ct->getExchg());
...
WTSOrderInfo *TraderHuaX::makeOrderInfo(CTORATstpOrderField *order_info) {
  ...
  pRet->setVolLeft((uint32_t)order_info->VolumeTotalOriginal -
                   order_info->VolumeTraded);
  ...
  if (strlen(usertag) == 0) {
    ...
    if (strlen(pRet->getOrderID()) > 0) {
      _oidCache.put(
          StrUtil::trim(order_info->OrderSysID).c_str(), usertag, 0,
...
WTSTradeInfo *TraderHuaX::makeTradeInfo(CTORATstpTradeField *trade_info) {
  ...
  const char *usertag =
      _oidCache.get(StrUtil::trim(trade_info->OrderSysID).c_str());
  if (strlen(usertag))
    pRet->setUserTag(usertag);
...
inline WTSPositionItem *
TraderHuaX::makePositionItem(CTORATstpPositionField *position_info) {
  std::string code, exchg;
  ...
  if (contract) {
    ...
    std::string key = code;
    WTSPositionItem *pos = WTSPositionItem::create(
        code.c_str(), commInfo->getCurrency(), commInfo->getExchg());
...
inline WTSAccountInfo *
TraderHuaX::makeAccountInfo(CTORATstpTradingAccountField *accountField) {
...
void TraderHuaX::OnFrontDisconnected(int nReason) {
  ...
  _state = TS_NOTLOGIN;
  //_asyncio.post([this](){
  //	write_log(_sink, LL_WARN, "[TraderHuaX] Connection lost, relogin in 2
  //seconds..."); 	std::this_thread::sleep_for(std::chrono::seconds(2));
  //	reconnect();
  //});
}

void TraderHuaX::OnRspError(CTORATstpRspInfoField *pRspInfoField,
                            int nRequestID, bool bIsLast) {
  if (_sink && pRspInfoField)
    write_log(_sink, LL_ERROR, "[TraderHuaX] ErrorID:{} ErrorMsg:{}",
              pRspInfoField->ErrorID, pRspInfoField->ErrorMsg);
}

void TraderHuaX::OnRspOrderInsert(CTORATstpInputOrderField *pInputOrderField,
...
void TraderHuaX::OnErrRtnOrderInsert(CTORATstpInputOrderField *pInputOrderField,
                                     CTORATstpRspInfoField *pRspInfoField,
                                     int nRequestID) {
...
void TraderHuaX::OnErrRtnOrderAction(
    CTORATstpInputOrderActionField *pInputOrderActionField,
    CTORATstpRspInfoField *pRspInfoField, int nRequestID) {
  if (IsErrorInfo(pRspInfoField)) {
    WTSError *error =
        WTSError::create(WEC_ORDERCANCEL, pRspInfoField->ErrorMsg);
    _sink->onTraderError(error);
    error->release();
  }
}

void TraderHuaX::OnRspQryOrder(CTORATstpOrderField *pOrderField,
                               CTORATstpRspInfoField *pRspInfoField,
                               int nRequestID, bool bIsLast) {
...
void TraderHuaX::OnRspQryTrade(CTORATstpTradeField *pTradeField,
                               CTORATstpRspInfoField *pRspInfoField,
                               int nRequestID, bool bIsLast) {
...
void TraderHuaX::OnRspQryPosition(CTORATstpPositionField *pPositionField,
                                  CTORATstpRspInfoField *pRspInfoField,
                                  int nRequestID, bool bIsLast) {
...
void TraderHuaX::OnRspQryTradingAccount(
    CTORATstpTradingAccountField *pTradingAccountField,
...
void TraderHuaX::OnRspUserLogin(CTORATstpRspUserLoginField *pRspUserLoginField,
                                CTORATstpRspInfoField *pRspInfoField,
                                int nRequestID) {
  if (pRspInfoField->ErrorID != 0) {
    write_log(_sink, LL_INFO, "[TraderHuaX] [{}] Login faild, erroMsg: {}...",
              _user.c_str(), pRspInfoField->ErrorMsg);
    if (_sink)
    ...
    if (ret != 0) {
      write_log(_sink, LL_ERROR,
                "[TraderHuaX] ReqGetConnectionInfo fail, ret{}", ret);
    }
  }
}

void TraderHuaX::OnRspGetConnectionInfo(
  ...
  if (pRspInfo->ErrorID == 0) {
    write_log(
        ...
        "outer_port{}\n"
        "mac_address{}\n",
        pConnectionInfoField->InnerIPAddress, pConnectionInfoField->InnerPort,
        pConnectionInfoField->OuterIPAddress, pConnectionInfoField->OuterPort,
        pConnectionInfoField->MacAddress);
    if (_sink)
      _sink->handleEvent(WTE_Connect, 0);
  } else {
    write_log(_sink, LL_ERROR,
              "get connection info fail! error_id[{}] error_msg[{}]\n",
              pRspInfo->ErrorID, pRspInfo->ErrorMsg);
  }
}

void TraderHuaX::OnRspQryShareholderAccount(
  ...
  if (bIsLast) {
    if (!_inited) {
      _tradingday = TimeUtils::getCurDate();
      {
        ...
        ss << _user << "_eid.sc";
        _eidCache.init(ss.str().c_str(), _tradingday,
...

      {
        ...
        if (!StdFile::exists(path.c_str()))
          boost::filesystem::create_directories(path.c_str());
        ss << _user << "_oid.sc";
        _oidCache.init(ss.str().c_str(), _tradingday,
                       [this](const char *message) {
                         write_log(_sink, LL_WARN, message);
                       });
      }
      ...
      write_log(_sink, LL_INFO,
                "[TraderHuaX] [{}] Login succeed, trading date: {}...",
      ...
      write_log(_sink, LL_INFO, "[TraderHuaX] [{}] Connection recovered",
                _user.c_str());
      _state = TS_ALLREADY;
    }
  }
...
void TraderHuaX::OnFrontConnected() {
  ...
  if (ret != 0) {
    write_log(_sink, LL_ERROR, "[TraderHuaX] ReqGetConnectionInfo fail, ret{}",
              ret);
  }
}
...
bool TraderHuaX::init(WTSVariant *params) {
  ...
  const char *creatorName =
      "?CreateTstpTraderApi@CTORATstpTraderApi@TORASTOCKAPI@@SAPEAV12@PEBD_N@Z";
#else
  const char *creatorName =
      "?CreateTstpTraderApi@CTORATstpTraderApi@TORASTOCKAPI@@SAPAV12@PBD_N@Z";
#endif
#else
  const char *creatorName =
      "_ZN12TORASTOCKAPI18CTORATstpTraderApi19CreateTstpTraderApiEPKcb";
#endif
...
void TraderHuaX::reconnect() {
  ...
  if (_api == NULL) {
    ...
    StdThreadPtr thrd(new StdThread([this]() {
      std::this_thread::sleep_for(std::chrono::seconds(2));
      write_log(_sink, LL_WARN, "[TraderHuaX] {} reconnecting...",
                _user.c_str());
      reconnect();
    }));
    return;
...
void TraderHuaX::disconnect() { release(); }

bool TraderHuaX::isConnected() { return (_state == TS_ALLREADY); }
...
void TraderHuaX::doLogin() {
  ...
  // 按照监管要求填写终端信息
  const char *terminalInfo =
      fmtutil::format("{};IIP={};IPORT={};LIP={};MAC={};HD={}", _terminal,
                      _pub_ip, _pub_port, _trade_ip, _mac, _hard_disk);
  ...
  if (ret != 0) {
    ...
    write_log(_sink, LL_ERROR, "[TraderHuaX] Login failed: error code {}",
              erro_code);
...
int TraderHuaX::login(const char *user, const char *pass,
...
int TraderHuaX::orderAction(WTSEntrustAction *action) {
  ...
  if (ret != 0) {
    write_log(_sink, LL_ERROR, "[TraderHuaX] ReqOrderAction fail, ret[{}]",
              ret);
  }
  return 0;
}

uint32_t TraderHuaX::genRequestID() { return _reqid.fetch_add(1); }
...
int TraderHuaX::queryAccount() {
  ...
  if (ret != 0) {
    write_log(_sink, LL_ERROR,
              "[TraderHuaX] ReqQryTradingAccount fail, ret[{}]\n", ret);
  }
  return 0;
}
...
inline WTSDirectionType
TraderHuaX::wrapDirectionType(TTORATstpDirectionType dirType) {
  switch (dirType) {
  ...
  case TORA_TSTP_D_CreditSell:
    return WDT_SHORT;
  default:
    write_log(_sink, LL_WARN, "[TraderHuaX] unsupport DirectionType {}",
              dirType);
    return WDT_NET;
  }
}
...
inline WTSPriceType
TraderHuaX::wrapPriceType(TTORATstpDirectionType priceType) {
...
inline WTSOrderState
TraderHuaX::wrapOrderState(TTORATstpOrderStatusType orderState) {
  switch (orderState) {
