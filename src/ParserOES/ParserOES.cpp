...
inline void write_log(IParserSpi *sink, WTSLogLevel ll, const char *format,
                      const Args &...args) {
  if (sink == NULL)
    return;
...
static int on_connect(MdsAsyncApiChannelT *pAsyncChannel,
...
static int on_disconnect(MdsAsyncApiChannelT *pAsyncChannel,
                         void *pCallbackParams) {
  ParserOES *parser = (ParserOES *)pCallbackParams;
  if (parser == NULL)
    return 0;
...
static int on_message(MdsApiSessionInfoT *pSessionInfo, SMsgHeadT *pMsgHead,
...
bool ParserOES::connect() {
  ...
  if (_udp) {
    MdsAsyncApiChannelT *channel = MdsAsyncApi_AddChannelFromFile(
        _context, "udp_snap1_SH", _config.c_str(), MDSAPI_CFG_DEFAULT_SECTION,
        MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_SNAP1, on_message, this, on_connect,
        this, on_disconnect, this);
    if (!channel) {
      write_log(_sink, LL_WARN,
                "[ParserOES] Loading section {} from config failed",
                MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_SNAP1);
    }

    channel = MdsAsyncApi_AddChannelFromFile(
        _context, "udp_snap1_SZ", _config.c_str(), MDSAPI_CFG_DEFAULT_SECTION,
        MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_SNAP2, on_message, this, on_connect,
        this, on_disconnect, this);
    if (!channel) {
      write_log(_sink, LL_WARN,
                "[ParserOES] Loading section {} from config failed",
    ...
    channel = MdsAsyncApi_AddChannelFromFile(
        _context, "udp_tick1_SH", _config.c_str(), MDSAPI_CFG_DEFAULT_SECTION,
        MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_TICK1, on_message, this, on_connect,
        this, on_disconnect, this);
    if (!channel) {
      write_log(_sink, LL_WARN,
                "[ParserOES] Loading section {} from config failed",
    ...
    channel = MdsAsyncApi_AddChannelFromFile(
        _context, "udp_tick1_SZ", _config.c_str(), MDSAPI_CFG_DEFAULT_SECTION,
        MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_TICK2, on_message, this, on_connect,
        this, on_disconnect, this);
    if (!channel) {
      write_log(_sink, LL_WARN,
                "[ParserOES] Loading section {} from config failed",
    ...
    MdsAsyncApiChannelT *channel = MdsAsyncApi_AddChannelFromFile(
        _context, "tcp_channel", _config.c_str(), MDSAPI_CFG_DEFAULT_SECTION,
        MDSAPI_CFG_DEFAULT_KEY_TCP_ADDR, on_message, this, on_connect, this,
        on_disconnect, this);
    if (!channel) {
      write_log(_sink, LL_WARN,
                "[ParserOES] Loading section {} from config failed",
...
bool ParserOES::isConnected() { return MdsAsyncApi_IsRunning(_context); }
...
void ParserOES::doOnConnected(MdsAsyncApiChannelT *pAsyncChannel) {
  if (_udp) {
    write_log(_sink, LL_ERROR, "[ParserOES] channel {} connected",
  ...
  /* 订阅的数据类型 (dataTypes) 会以最后一次订阅为准,
   * 所以每次都需要指定为所有待订阅的数据类型 */
  int32 dataTypes =
      MDS_SUB_DATA_TYPE_INDEX_SNAPSHOT | MDS_SUB_DATA_TYPE_OPTION_SNAPSHOT |
      MDS_SUB_DATA_TYPE_L2_SNAPSHOT | MDS_SUB_DATA_TYPE_L2_BEST_ORDERS |
      MDS_SUB_DATA_TYPE_L2_ORDER | MDS_SUB_DATA_TYPE_L2_SSE_ORDER |
  ...
  if (!MdsAsyncApi_SubscribeByString(pAsyncChannel, (char *)NULL, (char *)NULL,
                                     MDS_EXCH_SSE, MDS_MD_PRODUCT_TYPE_STOCK,
                                     MDS_SUB_MODE_SET, dataTypes)) {
    write_log(_sink, LL_WARN,
              "[ParserOES] Subscribe stock quotes of SSE failed");
  }

  /* 追加订阅所有上海指数行情 */
  if (!MdsAsyncApi_SubscribeByString(pAsyncChannel, (char *)NULL, (char *)NULL,
                                     MDS_EXCH_SSE, MDS_MD_PRODUCT_TYPE_INDEX,
                                     MDS_SUB_MODE_APPEND, dataTypes)) {
    write_log(_sink, LL_WARN,
              "[ParserOES] Subscribe index quotes of SSE failed");
  }

  /* 追加订阅所有上海期权行情 */
  if (!MdsAsyncApi_SubscribeByString(pAsyncChannel, (char *)NULL, (char *)NULL,
                                     MDS_EXCH_SSE, MDS_MD_PRODUCT_TYPE_OPTION,
                                     MDS_SUB_MODE_APPEND, dataTypes)) {
    write_log(_sink, LL_WARN,
              "[ParserOES] Subscribe option quotes of SSE failed");
  }

  /* 追加订阅所有深圳股票/债券/基金的 Level-2 行情 */
  if (!MdsAsyncApi_SubscribeByString(pAsyncChannel, (char *)NULL, (char *)NULL,
                                     MDS_EXCH_SZSE, MDS_MD_PRODUCT_TYPE_STOCK,
                                     MDS_SUB_MODE_APPEND, dataTypes)) {
    write_log(_sink, LL_WARN,
              "[ParserOES] Subscribe stock quotes of SZSE failed");
  }

  /* 追加订阅所有深圳指数行情 */
  if (!MdsAsyncApi_SubscribeByString(pAsyncChannel, (char *)NULL, (char *)NULL,
                                     MDS_EXCH_SZSE, MDS_MD_PRODUCT_TYPE_INDEX,
                                     MDS_SUB_MODE_APPEND, dataTypes)) {
    write_log(_sink, LL_WARN,
              "[ParserOES] Subscribe index quotes of SZSE failed");
  }

  /* 追加订阅所有深圳期权行情 */
  if (!MdsAsyncApi_SubscribeByString(pAsyncChannel, (char *)NULL, (char *)NULL,
                                     MDS_EXCH_SZSE, MDS_MD_PRODUCT_TYPE_OPTION,
                                     MDS_SUB_MODE_APPEND, dataTypes)) {
    write_log(_sink, LL_WARN,
              "[ParserOES] Subscribe option quotes of SZSE failed");
  }
}

void ParserOES::doOnDisconnected(MdsAsyncApiChannelT *pAsyncChannel) {
  if (_udp) {
    write_log(_sink, LL_ERROR, "[ParserOES] channel {} disconnected",
              pAsyncChannel->pChannelCfg->channelTag);
    return;
  }
...
void ParserOES::doOnMessage(SMsgHeadT *pMsgHead, void *pMsgItem) {
  ...
  case MDS_MSGTYPE_L2_TRADE:
    /* 处理Level2逐笔成交消息 @see MdsL2TradeT */
    {
      ...
      if (recv_cnt % _gpsize == 0)
        write_log(_sink, LL_DEBUG,
                  "[ParserOES] {} transactions received in total", recv_cnt);
    }
    break;
  ...
  case MDS_MSGTYPE_L2_SSE_ORDER:
    /* 处理Level2逐笔委托消息 @see MdsL2OrderT */
    {
      ...
      recv_cnt++;
      if (recv_cnt % _gpsize == 0)
        write_log(_sink, LL_DEBUG, "[ParserOES] {} orders received in total",
                  recv_cnt);
    }
    break;
  ...
  case MDS_MSGTYPE_L2_MARKET_DATA_SNAPSHOT:
    /* 处理Level2快照行情消息 @see MdsL2StockSnapshotBodyT */
    {
      ...
      if (ct == NULL) {
        // if (_sink)
        //	write_log(_sink, LL_ERROR, "[ParserXTP] Instrument {}.{} not
        //exists...", exchg.c_str(), code);
        return;
      }
      WTSCommodityInfo *commInfo = ct->getCommInfo();
      ...
      quote.total_volume =
          (double)pRspMsg->mktDataSnapshot.l2Stock.TotalVolumeTraded;
      quote.total_turnover =
          pRspMsg->mktDataSnapshot.l2Stock.TotalValueTraded / 10000.0;
      ...
      for (int i = 0; i < 10; i++) {
        quote.ask_prices[i] =
            wrapPrice(pRspMsg->mktDataSnapshot.l2Stock.OfferLevels[i].Price);
        quote.ask_qty[i] =
            (double)pRspMsg->mktDataSnapshot.l2Stock.OfferLevels[i].OrderQty;

        quote.bid_prices[i] =
            wrapPrice(pRspMsg->mktDataSnapshot.l2Stock.BidLevels[i].Price);
        quote.bid_qty[i] =
            (double)pRspMsg->mktDataSnapshot.l2Stock.BidLevels[i].OrderQty;
      }
      ...
      recv_cnt++;
      if (recv_cnt % _gpsize == 0)
        write_log(_sink, LL_DEBUG, "[ParserOES] {} L2 ticks received in total",
                  recv_cnt);
    }
    break;

  case MDS_MSGTYPE_L2_BEST_ORDERS_SNAPSHOT:
    /* 处理Level2委托队列消息(买一／卖一前五十笔委托明细) @see
     * MdsL2BestOrdersSnapshotBodyT */
    {
      ...
      buyOS.side = BDT_Buy;
      buyOS.price =
          wrapPrice(pRspMsg->mktDataSnapshot.l2BestOrders.BestBidPrice);
      buyOS.order_items = pRspMsg->mktDataSnapshot.l2BestOrders.NoBidOrders;

      sellOS.side = BDT_Sell;
      sellOS.price =
          wrapPrice(pRspMsg->mktDataSnapshot.l2BestOrders.BestOfferPrice);
      ...
      for (int i = 0; i < 50; i++) {
        if (pRspMsg->mktDataSnapshot.l2BestOrders.BidOrderQty[i] != 0) {
          buyOS.volumes[i] =
              pRspMsg->mktDataSnapshot.l2BestOrders.BidOrderQty[i];
          buyOS.qsize++;
        }

        if (pRspMsg->mktDataSnapshot.l2BestOrders.OfferOrderQty[i] != 0) {
          sellOS.volumes[i] =
              pRspMsg->mktDataSnapshot.l2BestOrders.OfferOrderQty[i];
          sellOS.qsize++;
        }
      }
      ...
      recv_cnt += 2;
      if (recv_cnt % _gpsize == 0)
        write_log(_sink, LL_DEBUG, "[ParserOES] {} queues received in total",
                  recv_cnt);
    }
    break;
  ...
  case MDS_MSGTYPE_MARKET_DATA_SNAPSHOT_FULL_REFRESH:
    /* 处理Level1快照行情消息 @see MdsStockSnapshotBodyT */
    {
      ...
      quote.total_volume =
          (double)pRspMsg->mktDataSnapshot.stock.TotalVolumeTraded;
      quote.total_turnover =
          pRspMsg->mktDataSnapshot.stock.TotalValueTraded / 10000.0;
      ...
      for (int i = 0; i < 5; i++) {
        quote.ask_prices[i] =
            wrapPrice(pRspMsg->mktDataSnapshot.stock.OfferLevels[i].Price);
        quote.ask_qty[i] =
            (double)pRspMsg->mktDataSnapshot.stock.OfferLevels[i].OrderQty;

        quote.bid_prices[i] =
            wrapPrice(pRspMsg->mktDataSnapshot.stock.BidLevels[i].Price);
        quote.bid_qty[i] =
            (double)pRspMsg->mktDataSnapshot.stock.BidLevels[i].OrderQty;
      }
      ...
      recv_cnt++;
      if (recv_cnt % _gpsize == 0)
        write_log(_sink, LL_DEBUG, "[ParserOES] {} L1 ticks received in total",
                  recv_cnt);
    }
    break;
  ...
  case MDS_MSGTYPE_OPTION_SNAPSHOT_FULL_REFRESH:
    /* 处理期权快照行情消息 @see MdsStockSnapshotBodyT */
    {
      ...
      quote.total_volume =
          (double)pRspMsg->mktDataSnapshot.option.TotalVolumeTraded;
      quote.total_turnover =
          pRspMsg->mktDataSnapshot.option.TotalValueTraded / 10000.0;

      quote.pre_close = wrapPrice(pRspMsg->mktDataSnapshot.option.PrevClosePx);
      quote.open_interest =
          (double)pRspMsg->mktDataSnapshot.option.TotalLongPosition;

      // 委卖价格
      for (int i = 0; i < 5; i++) {
        quote.ask_prices[i] =
            wrapPrice(pRspMsg->mktDataSnapshot.option.OfferLevels[i].Price);
        quote.ask_qty[i] =
            (double)pRspMsg->mktDataSnapshot.option.OfferLevels[i].OrderQty;

        quote.bid_prices[i] =
            wrapPrice(pRspMsg->mktDataSnapshot.option.BidLevels[i].Price);
        quote.bid_qty[i] =
            (double)pRspMsg->mktDataSnapshot.option.BidLevels[i].OrderQty;
      }
      ...
      if (recv_cnt % _gpsize == 0)
        write_log(_sink, LL_DEBUG,
                  "[ParserOES] {} Option ticks received in total", recv_cnt);
    }
    break;
  ...
  case MDS_MSGTYPE_INDEX_SNAPSHOT_FULL_REFRESH:
    /* 处理指数行情消息 @see MdsIndexSnapshotBodyT */
    {
      ...
      quote.total_volume =
          (double)pRspMsg->mktDataSnapshot.index.TotalVolumeTraded;
      quote.total_turnover =
          pRspMsg->mktDataSnapshot.index.TotalValueTraded / 10000.0;
      ...
      if (recv_cnt % _gpsize == 0)
        write_log(_sink, LL_DEBUG,
                  "[ParserOES] {} index ticks received in total", recv_cnt);
    }
    break;
  ...
  case MDS_MSGTYPE_MARKET_DATA_REQUEST:
    ...
    if (pMsgHead->status != 0) {
      write_log(_sink, LL_ERROR,
                "recv subscribe-request response, subscription failed! "
                "errCode[{} - {}]",
