 ...
 * Copyright (C)2002-2006 Merkur ( strEmail.Format("%s@%s", "devteam",
...
void CMiniDumper::Enable(LPCTSTR pszAppName, bool bShowErrors,
                         LPCTSTR pszDumpPath /* = ""*/) {
  // if this assert fires then you have two instances of CMiniDumper which is
  // not allowed
  ...
  HMODULE hDbgHelpDll =
      GetDebugHelperDll((FARPROC *)&pfnMiniDumpWriteDump, bShowErrors);
  if (hDbgHelpDll) {
...
HMODULE CMiniDumper::GetDebugHelperDll(FARPROC *ppfnMiniDumpWriteDump,
                                       bool bShowErrors) {
  *ppfnMiniDumpWriteDump = NULL;
  ...
  if (hDll == NULL) {
    if (bShowErrors) {
      ...
      MessageBox(NULL,
                 _T("DBGHELP.DLL not found. Please install a DBGHELP.DLL."),
    ...
    *ppfnMiniDumpWriteDump = GetProcAddress(hDll, "MiniDumpWriteDump");
    if (*ppfnMiniDumpWriteDump == NULL) {
      if (bShowErrors) {
        // Do *NOT* localize that string (in fact, do not use MFC to load it)!
        MessageBox(NULL,
                   _T("DBGHELP.DLL found is too old. Please upgrade to a ")
                   _T("newer version of DBGHELP.DLL."),
                   m_szAppName, MB_ICONSTOP | MB_OK);
      }
    }
  }
...
LONG CMiniDumper::TopLevelFilter(struct _EXCEPTION_POINTERS *pExceptionInfo) {
  ...
  HINSTANCE hInstCrashReporter =
      NULL; // ADDED by fengwen on 2006/11/15 : 使用新的发送错误报告机制。
  ...
  if (hDll) {
    if (pfnMiniDumpWriteDump) {
      ...
      //  Do *NOT* localize that string (in fact, do not use MFC to load it)!
      // COMMENTED by fengwen on 2006/11/15	<begin> :
      // 使用新的发送错误报告机制。 if (MessageBox(NULL, _T("eMule crashed
      // :-(\r\n\r\nA diagnostic file can be created which will help the author
      // to resolve this problem. This file will be saved on your Disk (and not
      // sent).\r\n\r\nDo you want to create this file now?"), m_szAppName,
      // MB_ICONSTOP | MB_YESNO) == IDYES) COMMENTED by fengwen on 2006/11/15
      // <end> : 使用新的发送错误报告机制。
      {
        ...
        char buf[64];
        sprintf(buf, "%4.4d%2.2d%2.2d%2.2d%2.2d%2.2d", curTime.wYear,
        ...
        if (hFile != INVALID_HANDLE_VALUE) {
          ...
          BOOL bOK = (*pfnMiniDumpWriteDump)(
              GetCurrentProcess(), GetCurrentProcessId(), hFile, MiniDumpNormal,
              &ExInfo, NULL, NULL);
          if (bOK) {
            // Do *NOT* localize that string (in fact, do not use MFC to load
            // it)!
            _sntprintf(szResult, ARRSIZE(szResult),
                       _T("Saved dump file to \"%s\".\r\n\r\nPlease send this ")
                       _T("file together with a detailed bug report to ")
                       _T("bastet.wang@gmail.com !\r\n\r\nThank you for ")
                       _T("helping to improve Tsts."),
                       szDumpPath);
            lRetValue = EXCEPTION_EXECUTE_HANDLER;

            // ADDED by fengwen on 2006/11/15	<begin> :
            // 使用新的发送错误报告机制。
            hInstCrashReporter =
                ShellExecute(NULL, _T("open"), _T("CrashReporter.exe"),
                             szDumpPath, NULL, SW_SHOW);
            if (hInstCrashReporter <= (HINSTANCE)32)
              lRetValue = EXCEPTION_CONTINUE_SEARCH;
            // ADDED by fengwen on 2006/11/15	<end> :
            // 使用新的发送错误报告机制。
          } else {
            // Do *NOT* localize that string (in fact, do not use MFC to load
            // it)!
            _sntprintf(
                szResult, ARRSIZE(szResult),
                _T("Failed to save dump file to \"%s\".\r\n\r\nError: %u"),
          ...
          // Do *NOT* localize that string (in fact, do not use MFC to load it)!
          _sntprintf(szResult, ARRSIZE(szResult),
                     _T("Failed to create dump file \"%s\".\r\n\r\nError: %u"),
                     szDumpPath, GetLastError());
        }
      }
    }
...
#ifndef _DEBUG
  if (EXCEPTION_EXECUTE_HANDLER ==
      lRetValue) // ADDED by fengwen on 2006/11/15 :
                 // 由此filter处理了异常,才去中止进程。
  {
    // Exit the process only in release builds, so that in debug builds the
    // exceptio is passed to a possible installed debugger
    ExitProcess(0);
  } else
    return lRetValue;
