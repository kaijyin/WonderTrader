...
StateMonitor::StateMonitor() : _stopped(false), _bd_mgr(NULL), _dt_mgr(NULL) {}

StateMonitor::~StateMonitor() {}

bool StateMonitor::initialize(const char *filename, WTSBaseDataMgr *bdMgr,
                              DataManager *dtMgr) {
  _bd_mgr = bdMgr;
  _dt_mgr = dtMgr;
  ...
  for (const std::string &sid : keys) {
    ...
    if (ssInfo == NULL) {
      WTSLogger::error(
          "Trading session template [{}] not exists,state control rule skipped",
          sid);
      continue;
    }
    ...
    stateInfo->_init_time =
        jItem->getUInt32("inittime"); // 初始化时间,初始化以后数据才开始接收
    stateInfo->_close_time =
        jItem->getUInt32("closetime"); // 收盘时间,收盘后数据不再接收了
    stateInfo->_proc_time =
        jItem->getUInt32("proctime"); // 盘后处理时间,主要把实时数据转到历史去

    strcpy(stateInfo->_session, sid.c_str());

    const auto &auctions =
        ssInfo->getAuctionSections(); // 这里面是偏移过的时间,要注意了!!!
    for (const auto &secInfo : auctions) {
      ...
      etime = etime / 60 * 100 +
              etime % 60; // 先不考虑半夜12点的情况,目前看来,几乎没有
      stateInfo->_sections.emplace_back(StateInfo::Section({stime, etime}));
    }

    const auto &sections =
        ssInfo->getTradingSections(); // 这里面是偏移过的时间,要注意了!!!
    ...
    if (pCommSet) {
      ...
      for (auto it = pCommSet->begin(); it != pCommSet->end(); it++) {
        ...
        _bd_mgr->setTradingDate(
            pid, _bd_mgr->getTradingDate(pid, offDate, offMin, false), false);
        uint32_t prevDate = TimeUtils::getNextDate(curDate, -1);
        if ((ssInfo->getOffsetMins() > 0 &&
             (!_bd_mgr->isTradingDate(pid, curDate) &&
              !(ssInfo->isInTradingTime(curMin) &&
                _bd_mgr->isTradingDate(pid, prevDate)))) ||
            (ssInfo->getOffsetMins() <= 0 &&
             !_bd_mgr->isTradingDate(pid, offDate))) {
      ...
      while (!_stopped) {
        ...
        for (; it != _map.end(); it++) {
          ...
          case SS_ORIGINAL: {
            ...
            uint32_t offInitTime =
                sInfo->offsetTime(stateInfo->_init_time, true);
            uint32_t offCloseTime =
                sInfo->offsetTime(stateInfo->_close_time, false);
            ...
            if (pCommSet) {
              for (auto it = pCommSet->begin(); it != pCommSet->end(); it++) {
                ...
                if ((sInfo->getOffsetMins() > 0 &&
                     (!_bd_mgr->isTradingDate(pid,
                                              curDate) && // 当前日志不是交易日
                      !(sInfo->isInTradingTime(curMin) &&
                        _bd_mgr->isTradingDate(
                            pid,
                            prevDate)))) || // 当前不在交易时间,且昨天是交易日
                    (sInfo->getOffsetMins() <= 0 &&
                     !_bd_mgr->isTradingDate(pid, offDate))) {
                  ss_a << pid << ",";
                  WTSLogger::info("Instrument {} is in holiday", pid);
                } else {
              ...
              WTSLogger::info("No corresponding instrument of trading session "
                              "{}[{}], changed into holiday state",
                              sInfo->name(), stateInfo->_session);
              stateInfo->_state = SS_Holiday;
            }

            if (isAllHoliday) {
              WTSLogger::info("All instruments of trading session {}[{}] are "
                              "in holiday, changed into holiday state",
                              sInfo->name(), stateInfo->_session);
              stateInfo->_state = SS_Holiday;
            } else if (offTime >= offCloseTime) {
              stateInfo->_state = SS_CLOSED;
              WTSLogger::info("Trading session {}[{}] stopped receiving data",
                              sInfo->name(), stateInfo->_session);
              ...
              if (stateInfo->isInSections(offTime)) {
                ...
                WTSLogger::info("Trading session {}[{}] started receiving data",
                                sInfo->name(), stateInfo->_session);
              } else {
                ...
                if (offTime < sInfo->getCloseTime(true)) {
                  ...
                  WTSLogger::info(
                      "Trading session {}[{}] paused receiving data",
                      sInfo->name(), stateInfo->_session);
                } else { // 大于市场收盘时间,但是没有大于接收收盘时间,则还要继续接收,主要是要收结算价
                  stateInfo->_state = SS_RECEIVING;
                  WTSLogger::info(
                      "Trading session {}[{}] started receiving data",
                      sInfo->name(), stateInfo->_session);
                }
              }
            } else if (offTime >= offInitTime) {
              stateInfo->_state = SS_INITIALIZED;
              WTSLogger::info("Trading session {}[{}] initialized",
                              sInfo->name(), stateInfo->_session);
            }
          ...
          case SS_INITIALIZED: {
            ...
            if (offAucSTime == -1 ||
                offTime >= sInfo->getAuctionStartTime(true)) {
              if (!stateInfo->isInSections(offTime) &&
                  offTime < sInfo->getCloseTime(true)) {
                // if (sInfo->_schedule)
                //{
                //	_dt_mgr->preloadRtCaches();
                // }
                ...
                WTSLogger::info("Trading session {}[{}] paused receiving data",
                                sInfo->name(), stateInfo->_session);
              } else {
                // if (sInfo->_schedule)
                //{
          ...
          case SS_PAUSED: {
            ...
            if (!isAllHoliday) {
              ...
              if (stateInfo->isInSections(offTime)) {
                stateInfo->_state = SS_RECEIVING;
                WTSLogger::info(
                    "Trading session {}[{}] continued to receive data",
                    sInfo->name(), stateInfo->_session);
              }
            } else {
              WTSLogger::info("All instruments of trading session {}[{}] are "
          ...
          case SS_CLOSED: {
            uint32_t offTime = sInfo->offsetTime(curMin, true);
            uint32_t offProcTime =
                sInfo->offsetTime(stateInfo->_proc_time, true);
            if (offTime >= offProcTime) {
              if (!_dt_mgr->isSessionProceeded(stateInfo->_session)) {
                stateInfo->_state = SS_PROCING;

                WTSLogger::info(
                    "Trading session {}[{}] started processing closing task",
                    sInfo->name(), stateInfo->_session);
          ...
          case SS_Holiday: {
            ...
            if (offTime >= 0 && offTime < offInitTime) {
              ...
              if (!isAllHoliday) {
                ...
                WTSLogger::info("Trading session {}[{}] state reset",
                                sInfo->name(), stateInfo->_session);
              }
            }
          } break;
