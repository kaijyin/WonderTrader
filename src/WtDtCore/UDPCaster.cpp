...
// UDPTick数据包
template <typename T> struct UDPDataPacket {
  uint32_t _type;
  T _data;
};
#pragma pack(pop)
...
UDPCaster::UDPCaster() : m_bTerminated(false), m_bdMgr(NULL), m_dtMgr(NULL) {}

UDPCaster::~UDPCaster() {}

bool UDPCaster::init(WTSVariant *cfg, WTSBaseDataMgr *bdMgr,
                     DataManager *dtMgr) {
  m_bdMgr = bdMgr;
  m_dtMgr = dtMgr;
  ...
  if (cfgMC) {
    for (uint32_t idx = 0; idx < cfgMC->size(); idx++) {
      ...
      addMRecver(cfgItem->getCString("host"), cfgItem->getInt32("port"),
                 cfgItem->getInt32("sendport"), cfgItem->getUInt32("type"));
    }
  }
...
void UDPCaster::start(int sport) {
  if (!m_listFlatRecver.empty() || !m_listJsonRecver.empty() ||
      !m_listRawRecver.empty()) {
  ...
  try {
    ...
    WTSLogger::error(
        "Exception raised while start subscribing service @ port {}", sport);
  }
...
void UDPCaster::do_receive() {
  m_sktSubscribe->async_receive_from(
      boost::asio::buffer(m_data, max_length), m_senderEP,
        ...
        if (bytes_recvd == sizeof(UDPReqPacket)) {
          ...
          if (req->_type == UDP_MSG_SUBSCRIBE) {
            ...
            for (const std::string &fullcode : ay) {
              ...
              WTSTickData *curTick =
                  m_dtMgr->getCurTick(code.c_str(), exchg.c_str());
              if (curTick == NULL)
                continue;
              ...
              pkt->_type = req->_type;
              memcpy(&pkt->_data, &curTick->getTickStruct(),
              ...
              m_sktSubscribe->async_send_to(
                  ...
                  [this, data](const boost::system::error_code &ec,
                    ...
                    delete data;
                    if (ec) {
                      WTSLogger::error("Sending data on UDP failed: {}",
                                       ec.message().c_str());
                    }
                  });
            }
...
bool UDPCaster::addBRecver(const char *remote, int port, int type /* = 0 */) {
  try {
    boost::asio::ip::address_v4 addr =
        boost::asio::ip::address_v4::from_string(remote);
...
bool UDPCaster::addMRecver(const char *remote, int port, int sendport,
                           int type /* = 0 */) {
  try {
    boost::asio::ip::address_v4 addr =
        boost::asio::ip::address_v4::from_string(remote);
        ...
        while (!tmpQue.empty()) {
          ...
          if (!m_listRawGroup.empty() || !m_listRawRecver.empty()) {
            ...
            if (castData._datatype == UDP_MSG_PUSHTICK) {
              ...
              memcpy(&pack->_data, &curObj->getTickStruct(),
                     sizeof(WTSTickStruct));
              ...
              memcpy(&pack->_data, &curObj->getOrdDtlStruct(),
              ...
              memcpy(&pack->_data, &curObj->getOrdQueStruct(),
              ...
              memcpy(&pack->_data, &curObj->getTransStruct(),
            ...
            for (auto it = m_listRawRecver.begin(); it != m_listRawRecver.end();
                 it++) {
              const UDPReceiverPtr &receiver = (*it);
              m_sktBroadcast->send_to(boost::asio::buffer(buf_raw),
                                      receiver->_ep, 0, ec);
              if (ec) {
                WTSLogger::error(
                    "Error occured while sending to ({}:{}): {}({})",
            ...
            // 组播
            for (auto it = m_listRawGroup.begin(); it != m_listRawGroup.end();
                 it++) {
              ...
              if (ec) {
                WTSLogger::error(
                    "Error occured while sending to ({}:{}): {}({})",
                    item.second->_ep.address().to_string(),
                    item.second->_ep.port(), ec.value(), ec.message());
              }
            }
          }
...
void UDPCaster::handle_send_broad(const EndPoint &ep,
                                  const boost::system::error_code &error,
                                  std::size_t bytes_transferred) {
  if (error) {
    WTSLogger::error("Broadcasting of market data failed, remote addr: {}, "
                     "error message: {}",
                     ep.address().to_string().c_str(), error.message().c_str());
  }
}

void UDPCaster::handle_send_multi(const EndPoint &ep,
                                  const boost::system::error_code &error,
                                  std::size_t bytes_transferred) {
  if (error) {
    WTSLogger::error("Multicasting of market data failed, remote addr: {}, "
                     "error message: {}",
                     ep.address().to_string().c_str(), error.message().c_str());
  }
}
