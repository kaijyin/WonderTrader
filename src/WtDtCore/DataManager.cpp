...
DataManager::DataManager() : _writer(NULL), _bd_mgr(NULL), _state_mon(NULL) {}
...
bool DataManager::init(WTSVariant *params, WTSBaseDataMgr *bdMgr,
                       StateMonitor *stMonitor) {
  _bd_mgr = bdMgr;
  ...
  if (module.empty())
    module =
        WtHelper::get_module_dir() + DLLHelper::wrap_module("WtDataStorage");
  else
    module =
        WtHelper::get_module_dir() + DLLHelper::wrap_module(module.c_str());

  DllHandle libWriter = DLLHelper::load_library(module.c_str());
  if (libWriter) {
    FuncCreateWriter pFuncCreateWriter =
        (FuncCreateWriter)DLLHelper::get_symbol(libWriter, "createWriter");
    if (pFuncCreateWriter == NULL) {
      WTSLogger::error("Initializing of data writer failed: function "
                       "createWriter not found...");
    }

    FuncDeleteWriter pFuncDeleteWriter =
        (FuncDeleteWriter)DLLHelper::get_symbol(libWriter, "deleteWriter");
    if (pFuncDeleteWriter == NULL) {
      WTSLogger::error("Initializing of data writer failed: function "
                       "deleteWriter not found...");
    }
    ...
    WTSLogger::error(
        "Initializing of data writer failed: loading module {} failed...",
        module.c_str());
    return false;
  }
...
IBaseDataMgr *DataManager::getBDMgr() { return _bd_mgr; }
