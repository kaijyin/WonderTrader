...
const char *WEIGHT_ALGS[] = {"Fixed", "DynamicInterest", "DynamicVolume"};
...
bool IndexWorker::init(WTSVariant *config) {
  ...
  _stand_scale = config->getDouble(
      "stand_scale"); // 标准化系数，如计算值为3000，标准化以后为1000，则标准化系数为0.333333
  ...
  if (_trigger != "time") {
    CodeHelper::CodeInfo cInfo =
        CodeHelper::extractStdCode(_trigger.c_str(), hotMgr);
    if (strlen(cInfo._ruletag) > 0)
      _trigger = fmt::format(
          "{}.{}", cInfo._exchg,
          hotMgr->getCustomRawCode(cInfo._ruletag, cInfo.stdCommID()));
  }
  ...
  if (cfgComms != NULL && cfgComms->size() > 0) {
    ...
    for (std::size_t i = 0; i < cnt; i++) {
      ...
      for (const auto &c : codes) {
        std::string fullCode =
            fmt::format("{}.{}", commInfo->getExchg(), c.c_str());
        ...
        if (lastTick) {
          memcpy(&wFactor._tick, &lastTick->getTickStruct(),
                 sizeof(WTSTickStruct));
          lastTick->release();
        }

        WTSLogger::info("Consist {} of block index {}.{} subscribed", fullCode,
                        _exchg, _code);
      }
    }
    ...
    for (std::size_t i = 0; i < cnt; i++) {
      ...
      if (ay.size() == 2) {
        ...
        CodeHelper::CodeInfo cInfo =
            CodeHelper::extractStdCode(fullCode.c_str(), hotMgr);
        if (strlen(cInfo._ruletag) > 0) {
          code = hotMgr->getCustomRawCode(cInfo._ruletag, cInfo.stdCommID());
          WTSLogger::info("{} contract confirmed: {} -> {}.{}", cInfo._ruletag,
                          fullCode, cInfo._exchg, code);
        } else {
          code = cInfo._code;
        }
      ...
      WTSContractInfo *cInfo =
          _factor->get_bd_mgr()->getContract(code.c_str(), exchg.c_str());
      if (cInfo == NULL) {
        WTSLogger::error("Consist {} of block index {}.{} not exists", fullCode,
                         _exchg, _code);
        continue;
      }
  ...
  WTSLogger::info("Block index {}.{} initialized，weight algorithm: {}, "
                  "trigger: {}, timeout: {}",
...
void IndexWorker::generate_tick() {
...

  {
    ...
    for (const auto &v : _weight_scales) {
      ...
      uint64_t curTime = TimeUtils::makeTime(wFactor._tick.action_date,
                                             wFactor._tick.action_time);
      ...
      case 1:                                      // 动态总持
        ...
        total_value +=
            wFactor._tick.open_interest * wFactor._tick.price * wFactor._weight;
        break;
      case 2:                                     // 动态成交量
        total_base += wFactor._tick.total_volume; // 动态成交量
        total_value +=
            wFactor._tick.total_volume * wFactor._tick.price * wFactor._weight;
        break;
      default:
        break;
      }
  ...
  WTSLogger::debug("{}.{} - {}.{} - {}", _cache.exchg, _cache.code,
                   _cache.action_date, _cache.action_time, _cache.price);
  newTick->release();
}