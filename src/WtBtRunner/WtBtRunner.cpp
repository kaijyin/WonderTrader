...
int main(int argc, char *argv[]) {
  ...
  auto cParam =
      opt("-c", "--config", "configure filepath, dtcfg.yaml as default", false);
  auto lParam =
      opt("-l", "--logcfg",
          "logging configure filepath, logcfgbt.yaml as default", false);
  ...
  if (strcmp(mode, "cta") == 0) {
    ...
    // 加载增量回测的基础历史回测数据
    const char *incremental_backtest_base =
        cfg->get("env")->getCString("incremental_backtest_base");
    ...
    replayer.register_task(mocker->id(),
                           cfg->get("sel")->get("task")->getUInt32("date"),
                           cfg->get("sel")->get("task")->getUInt32("time"),
                           cfg->get("sel")->get("task")->getCString("period"));
