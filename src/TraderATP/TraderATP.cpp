...
inline uint32_t makeRefID() {
  ...
  if (auto_refid == 0)
    auto_refid = (uint32_t)((TimeUtils::getLocalTimeNow() -
                             TimeUtils::makeTime(20220101, 0)) /
                            1000 * 100);
  return auto_refid.fetch_add(1);
}

TraderATP::TraderATP()
    : _api(NULL), _sink(NULL), _ordref(makeRefID()), _reqid(1), _orders(NULL),
      _trades(NULL), _positions(NULL), _bd_mgr(NULL), _tradingday(0),
      _close_flag(false), ayOrders(NULL), ayTrades(NULL), _return_nums(0) {}
...
inline WTSDirectionType wrapDirectionType(ATPSideType side,
                                          ATPPositionEffectType pe) {
...
inline ATPSideType wrapDirectionType(WTSDirectionType dirType,
...
inline WTSPriceType wrapOrdType(ATPOrdTypeType ordType) {
  if (ordType == ATPOrdTypeConst::kFixedNew ||
      ordType == ATPOrdTypeConst::kSzBiddingFixed ||
      ordType == ATPOrdTypeConst::kShBiddingFixed ||
      ordType == ATPOrdTypeConst::kFixedFullDealOrCancel ||
      ordType == ATPOrdTypeConst::kFixed)
    return WPT_LIMITPRICE;
  if (ordType == ATPOrdTypeConst::kImmediateDealTransferCancel ||
      ordType == ATPOrdTypeConst::kFullDealOrCancel ||
      ordType == ATPOrdTypeConst::kMarket ||
      ordType == ATPOrdTypeConst::kMarketTransferFixed)
    return WPT_ANYPRICE;
  else if (ordType == ATPOrdTypeConst::kLocalOptimalNew ||
           ordType ==
               ATPOrdTypeConst::kOptimalFiveLevelFullDealTransferCancel ||
           ordType ==
               ATPOrdTypeConst::kOptimalFiveLevelImmediateDealTransferFixed ||
           ordType == ATPOrdTypeConst::kCountPartyOptimalTransferFixed)
...
WTSEntrust *TraderATP::makeEntrust(const ATPRspOrderStatusAckMsg *order_info) {
  ...
  const std::string exchg =
      (order_info->market_id == ATPMarketIDConst::kShangHai) ? "SSE" : "SZSE";
  ...
  WTSEntrust *pRet =
      WTSEntrust::create(code.c_str(), (uint32_t)order_info->order_qty,
                         order_info->price, ct->getExchg());
  pRet->setContractInfo(ct);
  pRet->setDirection(
      wrapDirectionType(order_info->side, order_info->position_effect));
...
WTSOrderInfo *TraderATP::makeOrderInfo(const APIOrderUnit *order_info) {
  ...
  genEntrustID(pRet->getEntrustID(), order_info->client_seq_id);
  // pRet->setEntrustID(genEntrustID(pRet->getEntrustID(),
  // order_info->client_seq_id).c_str());
  ...
  if (strlen(usertag) == 0) {
    ...
    if (strlen(pRet->getOrderID()) > 0) {
      m_oidCache.put(
          StrUtil::trim(pRet->getOrderID()).c_str(), usertag, 0,
          [this](const char *message) { write_log(_sink, LL_ERROR, message); });
    }
  }
...
WTSOrderInfo *
TraderATP::makeOrderInfo(const ATPRspOrderStatusAckMsg *order_status_ack) {
  const std::string code(order_status_ack->security_id);
  const std::string exchg =
      (order_status_ack->market_id == ATPMarketIDConst::kShangHai) ? "SSE"
                                                                   : "SZSE";
  ...
  pRet->setVolTraded((uint32_t)(order_status_ack->order_qty / 100 -
                                order_status_ack->leaves_qty / 100));
...
WTSTradeInfo *TraderATP::makeTradeInfo(const APITradeOrderUnit *trade_info) {
  ...
  const std::string exchg =
      (trade_info->market_id == ATPMarketIDConst::kShangHai) ? "SSE" : "SZSE";
...
WTSTradeInfo *TraderATP::makeTradeRecord(
  ...
  uint32_t uTime =
      (uint32_t)(cash_auction_trade_er->transact_time % 1000000000);
  uint32_t uDate =
      (uint32_t)(cash_auction_trade_er->transact_time / 1000000000);
  ...
  fmtutil::format_to(pRet->getRefOrder(), "{}",
                     cash_auction_trade_er->cl_ord_id);
  pRet->setTradeType(WTT_Common);

  double amount =
      cash_auction_trade_er->total_value_traded / 10000.0; // 成交金额
...
void TraderATP::OnLogin(const std::string &reason) {
  ...
  write_log(_sink, LL_WARN, "[TraderATP] {} login success: {}...", _user,
            reason);
}
...
void TraderATP::OnRspCustLoginResp(
  ...
  for (auto it = cust_login_resp.fund_account_array.begin();
    ...
    for (auto it_1 = it->account_array.begin(); it_1 != it->account_array.end();
         it_1++) {
      write_log(_sink, LL_INFO,
                "[TraderATP][{}] OnRspCustLoginRsp: fund_account_id: {}, "
                "branch_id: {}, market_id: {}",
                it_1->account_id, it->fund_account_id, it->branch_id,
                it_1->market_id);
  ...
  if (cust_login_resp.permisson_error_code == 0) {
...

    {
      ...
      ss << _user << "_eid.sc";
      m_eidCache.init(
          ss.str().c_str(), _tradingday,
          [this](const char *message) { write_log(_sink, LL_WARN, message); });
    }
    ...
    if (_sink)
      _sink->onLoginResult(
          false,
          fmt::format("{}", cust_login_resp.permisson_error_code).c_str(),
          _tradingday);
    _state = TS_ALLREADY;

    write_log(_sink, LL_ERROR, "[TraderATP] CustLogin Fail, code : {}",
              static_cast<uint32_t>(cust_login_resp.permisson_error_code));
  }
}

void TraderATP::OnRspCustLogoutResp(
  ...
  if (cust_logout_resp.permisson_error_code == 0) {
    ...
    write_log(_sink, LL_INFO, "[TraderATP][{}] CustLogout Success!",
              cust_logout_resp.account_id);
  } else {
    write_log(_sink, LL_ERROR, "[TraderATP][{}] Logout failed: {}",
              cust_logout_resp.account_id,
              static_cast<uint32_t>(cust_logout_resp.permisson_error_code));
  }
}

// 订单下达内部响应
void TraderATP::OnRspOrderStatusInternalAck(
    const ATPRspOrderStatusAckMsg &order_status_ack) {
  write_log(_sink, LL_INFO,
            "[TraderATP][{}] cl_ord_no: {}, security_id: {}, ord_status: {}, "
            "price: {}, order_qty: {}, leaves_qty: {}, cum_qty: {}, cl_ord_id: "
            "{}, order_type: {}",
            order_status_ack.account_id, order_status_ack.cl_ord_no,
            order_status_ack.security_id, (int32_t)order_status_ack.ord_status,
            order_status_ack.price, order_status_ack.order_qty,
            order_status_ack.leaves_qty, order_status_ack.cum_qty,
            order_status_ack.cl_ord_id, order_status_ack.order_type);

  if (order_status_ack.reject_reason_code !=
      ATPRejectReasonCodeConst::kNormal) {
    WTSEntrust *entrust = makeEntrust(&order_status_ack);

    if (order_status_ack.orig_cl_ord_no !=
        0) // 可以根据orig_cl_ord_no是否为0来判断是否为撤单委托，需要注意撤单传入的orig_cl_ord_no不能为0
    {
      // 对应撤单回调
      write_log(_sink, LL_ERROR,
                "[TraderATP][{}] Cancelling order error, cl_ord_no: {}, "
                "orig_cl_ord_no: {}, reject code: {}, reject reason: {}",
                order_status_ack.account_id, order_status_ack.cl_ord_no,
                order_status_ack.orig_cl_ord_no,
                order_status_ack.reject_reason_code,
                order_status_ack.ord_rej_reason);
      WTSError *error =
          WTSError::create(WEC_ORDERCANCEL, order_status_ack.ord_rej_reason);
      ...
      write_log(_sink, LL_INFO,
                "[TraderATP][{}] Inserting order error, cl_ord_no: {}, "
                "orig_cl_ord_no: {}, reject code: {}, reject reason: {}",
                order_status_ack.account_id, order_status_ack.cl_ord_no,
                order_status_ack.orig_cl_ord_no,
                order_status_ack.reject_reason_code,
                order_status_ack.ord_rej_reason);
      WTSError *error =
          WTSError::create(WEC_ORDERINSERT, order_status_ack.ord_rej_reason);
      _sink->onRspEntrust(entrust, error);
      error->release();
    ...
    write_log(_sink, LL_INFO,
              "[TraderATP][{}] Order success, cl_ord_no: {}, orig_cl_ord_no: "
...
// 订单下达交易所确认
void TraderATP::OnRspOrderStatusAck(
    const ATPRspOrderStatusAckMsg &order_status_ack) {
  write_log(_sink, LL_INFO,
            "[OnRspOrderStatusAck][{}] account_id: {}, cl_ord_no: {}, code: "
            "{}, price: {}, order_qty: {}, ord_status: {}, side: {}",
            order_status_ack.account_id, order_status_ack.account_id,
            order_status_ack.cl_ord_no, order_status_ack.security_id,
            order_status_ack.price, order_status_ack.order_qty,
            (int32_t)order_status_ack.ord_status, order_status_ack.side);
  ...
  if (order_status_ack.reject_reason_code !=
        ...
        0) // 可以根据orig_cl_ord_no是否为0来判断是否为撤单委托，需要注意撤单传入的orig_cl_ord_no不能为0
    {
      // 对应撤单回调
      write_log(_sink, LL_ERROR,
                "[OnRspOrderStatusAck][{}] Cancelling order error, cl_ord_no: "
                "{}, orig_cl_ord_no: {}, reject code: {}, reject reason: {}",
                order_status_ack.account_id, order_status_ack.cl_ord_no,
                order_status_ack.orig_cl_ord_no,
                order_status_ack.reject_reason_code,
      ...
      write_log(_sink, LL_ERROR,
                "[OnRspOrderStatusAck][{}] Inserting order error, cl_ord_no: "
                "{}, orig_cl_ord_no: {}, reject code: {}, reject reason: {}",
                order_status_ack.account_id, order_status_ack.cl_ord_no,
                order_status_ack.orig_cl_ord_no,
                order_status_ack.reject_reason_code,
    ...
    write_log(_sink, LL_INFO,
              "[OnRspOrderStatusAck][{}] Order success, cl_ord_no: {}, "
              "orig_cl_ord_no: {}, reject code: {}, reject reason: {}",
              order_status_ack.account_id, order_status_ack.cl_ord_no,
              order_status_ack.orig_cl_ord_no,
              order_status_ack.reject_reason_code,
...
// 成交回报
void TraderATP::OnRspCashAuctionTradeER(
    const ATPRspCashAuctionTradeERMsg &cash_auction_trade_er) {
  write_log(_sink, LL_INFO,
            "[OnRspCashAuctionTradeER] code: {}, price: {}, qty: {}, cum_qty: "
            "{}, left_qty: {}, order_status: {}, cl_ord_no: {}, order_id: {}, "
            "cl_ord_id: {}",
            cash_auction_trade_er.security_id, cash_auction_trade_er.price,
            cash_auction_trade_er.order_qty, cash_auction_trade_er.cum_qty,
            cash_auction_trade_er.leaves_qty, cash_auction_trade_er.ord_status,
            cash_auction_trade_er.cl_ord_no, cash_auction_trade_er.order_id,
            cash_auction_trade_er.cl_ord_id);
...
// 订单下达内部拒绝
void TraderATP::OnRspBizRejection(
    const ATPRspBizRejectionOtherMsg &biz_rejection) {
  if (_sink)
    write_log(_sink, LL_ERROR,
              "[OnRspBizRejection] cliend_seq_id: {}, reject_code: {}, "
              "reject_msg: {}",
              biz_rejection.client_seq_id, biz_rejection.reject_reason_code,
              biz_rejection.business_reject_text);
}

void TraderATP::OnRspFundQueryResult(const ATPRspFundQueryResultMsg &msg) {
  write_log(_sink, LL_INFO, "[OnRspFundQueryResult][{}] OnRspFundQueryResult.",
            msg.account_id);
...
void TraderATP::OnRspOrderQueryResult(const ATPRspOrderQueryResultMsg &msg) {
  write_log(_sink, LL_INFO,
            "[OnRspOrderQueryResult][{}] last index: {}, total nums: {}",
            msg.account_id, msg.last_index, msg.total_num);
  ...
  for (const auto &unit : msg.order_array) {
    ...
    _asyncio.post([this, ordInfo] { ayOrders->append(ordInfo, false); });
  }
...
void TraderATP::OnRspTradeOrderQueryResult(
    const ATPRspTradeOrderQueryResultMsg &msg) {
  _return_nums = msg.total_num;

  if (ayTrades == NULL)
    ayTrades = WTSArray::create();

  // write_log(_sink, LL_INFO, "[TraderATP][{}] OnRspTradeOrderQueryResult, last
  // index: {}, total nums: {}", msg.account_id, msg.last_index, msg.total_num);
  ...
  for (const auto &unit : msg.order_array) {
    ...
    _asyncio.post([this, trdInfo] { ayTrades->append(trdInfo, false); });
  }

  // if ((msg.last_index + 1) == msg.total_num)  // 查询完毕
  //{
  //	if (!_sz_acctid.empty() && (strcmp(msg.account_id, _sz_acctid.c_str())
  //== 0))
  //	{
  //		// 此时返回了所有结果
  //		if (_sink)
...
void TraderATP::OnRspShareQueryResult(const ATPRspShareQueryResultMsg &msg) {
  ...
  for (auto &unit : msg.order_array) {
    const std::string exchg =
        (unit.market_id == ATPMarketIDConst::kShangHai) ? "SSE" : "SZSE";

    WTSContractInfo *contract =
        _bd_mgr->getContract(unit.security_id, exchg.c_str());
    if (contract) {
      WTSCommodityInfo *commInfo = contract->getCommInfo();
      std::string key = fmt::format("{}-{}", unit.security_id, exchg.c_str());
      WTSPositionItem *pos = (WTSPositionItem *)_positions->get(key);
      if (pos == NULL) {
        pos = WTSPositionItem::create(unit.security_id, commInfo->getCurrency(),
                                      commInfo->getExchg());
      ...
      pos->setNewPosition(
          (tmp >= 0)
              ? tmp
              : 0); // 剩余股份数量N15(2)，包含当日买入部分，拆分合并，申购赎回
      ...
      write_log(_sink, LL_INFO, "[OnRspShareQueryResult][{}] {}.{}, {}[{}]",
                msg.account_id, exchg, pos->getCode(), pos->getNewPosition(),
                pos->getPrePosition());
    }
  }
...
bool TraderATP::init(WTSVariant *params) {
  ...
  if (!inited) {
    ...
    // std::tr1::unordered_map<std::string, std::string> encrypt_cfg;  //
    // 加密库配置 bool connection_retention_flag = false;   // 是否启用会话保持

    //// encrypt_cfg参数填写：
    // encrypt_cfg["ENCRYPT_SCHEMA"] = "0";              // 字符 0 表示
    // 不对消息中的所有 password 加密 encrypt_cfg["ATP_ENCRYPT_PASSWORD"] = "";
    ...
    ATPRetCodeType ec = ATPTradeAPI::Init(station_name, cfg_path, log_dir_path,
                                          record_all_flag);
...
void TraderATP::reconnect() {
  ...
  prop.report_sync =
      report_sync; // 回报同步数据分区号+序号，首次是空，断线重连时填入的是接受到的最新分区号+序号
...
void TraderATP::disconnect() { release(); }

bool TraderATP::isConnected() { return (_state == TS_ALLREADY); }
...
void TraderATP::doLogin(const char *productInfo) {
  ...
  login_msg.login_mode = _loginmode; //  wrapLoginMode(_loginmode);  //
                                     //  ATPCustLoginModeType::kFundAccountIDMode;
                                     //  // 登录模式，资金账号登录
  login_msg.client_seq_id = genRequestID(); // 客户系统消息号
  login_msg.order_way =
      std::to_string(_order_way).at(0);     //'0';
                                            //// 委托方式，自助委托
...
int TraderATP::login(const char *user, const char *pass,
...
int TraderATP::logout() {
  ...
  // logout_msg.client_feature_code = _product;				//
  // 终端识别码
...
int TraderATP::orderInsert(WTSEntrust *entrust) {
  ...
  p.market_id = _is_sh ? ATPMarketIDConst::kShangHai
                       : ATPMarketIDConst::kShenZhen;    // 市场ID，上海
  ...
  // p.side = (entrust->getDirection() == WOT_OPEN) ? ATPSideConst::kBuy :
  // ATPSideConst::kSell;      // 买卖方向，买
  p.side = wrapDirectionType(entrust->getDirection(), entrust->getOffsetType());
  // p.order_type = ATPOrdTypeConst::kFixedNew;				//
  // 订单类型，限价
  p.order_type = wrapOrdType(entrust->getPriceType(), entrust->getOrderFlag());
  p.price =
      (int32_t)(entrust->getPrice() * 10000); // 委托价格 N13(4)，21.0000元
  p.order_qty =
      (int32_t)(entrust->getVolume() *
                100); // 申报数量N15(2)；股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张；1000.00股
  p.client_seq_id = genRequestID();               // 用户系统消息序号
  p.order_way = std::to_string(_order_way).at(0); // 0;
                                                  // // 委托方式，自助委托
  ...
  // for (int i = 0; i < totals; i++)
  {
    write_log(
        _sink, LL_INFO,
        "[orderInsert] code: {}, acct_id: {}, side: {}, price: {}, qty: {}",
        p.security_id, p.account_id, p.side, p.price, p.order_qty);

    if (strlen(entrust->getUserTag()) > 0) {
      m_eidCache.put(
          entrust->getEntrustID(), entrust->getUserTag(), 0,
          [this](const char *message) { write_log(_sink, LL_WARN, message); });
    }
...
int TraderATP::orderAction(WTSEntrustAction *action) {
  ...
  p.market_id =
      (_is_sh) ? ATPMarketIDConst::kShangHai : ATPMarketIDConst::kShenZhen;
  wt_strcpy(p.cust_id, _cust_id.c_str());                // 客户号ID
  wt_strcpy(p.fund_account_id, _fund_accountid.c_str()); // 资金账户ID
  ...
  // write_log(_sink, LL_ERROR, "[TraderATP][{}]账户超出撤单阈值！",
  // p.account_id); return -1;
...
uint32_t TraderATP::genRequestID() { return _reqid.fetch_add(1) + 1; }
...
int TraderATP::queryAccount() {
  ...
  if (ec != ErrorCode::kSuccess) {
    write_log(_sink, LL_ERROR, "[TraderATP][{}] Query account failed: {}",
              _acctid, ec);
  }
  ...
  _asyncio.post([this]() {
    {
      ...
      if (ec != ErrorCode::kSuccess) {
        write_log(_sink, LL_ERROR, "[TraderATP][{}] Query positions failed: {}",
                  _acctid, ec);
      }
      ...
      if (_return_nums > 100) // 最大单页容量为100条
      {
        ...
        for (int i = 0; i < times; i++) {
          ...
          if (ec != ErrorCode::kSuccess) {
            write_log(_sink, LL_ERROR,
                      "[TraderATP][{}] Query positions failed: {}", _acctid,
                      ec);
          }
  ...
  _asyncio.post([this]() {
    {
      ...
      if (ec != ErrorCode::kSuccess) {
        write_log(_sink, LL_ERROR, "[TraderATP][{}] Query orders failed: {}",
                  _acctid, ec);
      }
      ...
      if (_return_nums > 100) {
        ...
        for (int i = 0; i < times; i++) {
          ...
          if (ec != ErrorCode::kSuccess) {
            write_log(_sink, LL_ERROR,
                      "[TraderATP][{}] Query orders failed: {}", _acctid, ec);
          }
          std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }
      }
  ...
  _asyncio.post([this]() {
    {
      ...
      if (ec != ErrorCode::kSuccess) {
        write_log(_sink, LL_ERROR, "[TraderATP][{}] Query trades failed: {}",
                  _acctid, ec);
      }
      write_log(_sink, LL_INFO, "return num: {}", _return_nums);
      std::this_thread::sleep_for(std::chrono::milliseconds(100));
      ...
      if (_return_nums > 100) {
        ...
        for (int i = 0; i < times; i++) {
          ...
          if (ec != ErrorCode::kSuccess) {
            write_log(_sink, LL_ERROR,
                      "[TraderATP][{}] Query trades failed: {}", _acctid, ec);
          }
          // std::this_thread::sleep_for(std::chrono::milliseconds(50));
        }
      }
