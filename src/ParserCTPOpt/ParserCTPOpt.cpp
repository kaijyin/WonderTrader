...
ParserCTPOpt::~ParserCTPOpt() { m_pUserAPI = NULL; }
...
bool ParserCTPOpt::init(WTSVariant *config) {
  ...
  std::string dllpath =
      getBinDir() + DLLHelper::wrap_module(module.c_str(), "");
  m_hInstCTP = DLLHelper::load_library(dllpath.c_str());
  std::string path = StrUtil::printf("%s/%s/%s/", m_strFlowDir.c_str(),
                                     m_strBroker.c_str(), m_strUserID.c_str());
  ...
  const char *creatorName =
      "?CreateFtdcMdApi@CThostFtdcMdApi@ctp_sopt@@SAPEAV12@PEBD_N1@Z";
#else
  const char *creatorName =
      "?CreateFtdcMdApi@CThostFtdcMdApi@ctp_sopt@@SAPAV12@PBD_N1@Z";
#endif
#else
  const char *creatorName =
      "_ZN8ctp_sopt15CThostFtdcMdApi15CreateFtdcMdApiEPKcbb";
#endif
...
void ParserCTPOpt::release() { disconnect(); }
...
void ParserCTPOpt::OnRspError(CThostFtdcRspInfoField *pRspInfo, int nRequestID,
                              bool bIsLast) {
...
void ParserCTPOpt::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,
...
void ParserCTPOpt::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout,
                                   CThostFtdcRspInfoField *pRspInfo,
                                   int nRequestID, bool bIsLast) {
  if (m_sink) {
...
void ParserCTPOpt::OnFrontDisconnected(int nReason) {
  if (m_sink) {
    write_log(m_sink, LL_ERROR,
              "[ParserCTPOpt] Market data server disconnected: {}", nReason);
    m_sink->handleEvent(WPE_Close, 0);
  }
}

void ParserCTPOpt::OnRspUnSubMarketData(
    CThostFtdcSpecificInstrumentField *pSpecificInstrument,
    CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {}

void ParserCTPOpt::OnRtnDepthMarketData(
  ...
  if (m_bLocalTime) {
    ...
    actTime = strToTime(pDepthMarketData->UpdateTime) * 1000 +
  ...
  WTSContractInfo *cInfo = m_pBaseDataMgr->getContract(
  ...
  if (strcmp(quote.exchg, "CZCE") == 0) {
    quote.total_turnover =
        pDepthMarketData->Turnover * pCommInfo->getVolScale();
  } else {
...
void ParserCTPOpt::OnRspSubMarketData(
    CThostFtdcSpecificInstrumentField *pSpecificInstrument,
    CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
...
void ParserCTPOpt::OnHeartBeatWarning(int nTimeLapse) {
  if (m_sink)
    write_log(m_sink, LL_INFO, "[ParserCTPOpt] Heartbeating, elapse: {}",
              nTimeLapse);
}
...
void ParserCTPOpt::ReqUserLogin() {
  ...
  if (iResult != 0) {
    if (m_sink)
      write_log(m_sink, LL_ERROR,
                "[ParserCTPOpt] Sending login request failed: {}", iResult);
  }
}
...
void ParserCTPOpt::SubscribeMarketData() {
  ...
  if (m_pUserAPI && nCount > 0) {
    ...
    if (iResult != 0) {
      if (m_sink)
        write_log(m_sink, LL_ERROR,
                  "[ParserCTPOpt] Sending md subscribe request failed: {}",
                  iResult);
    } else {
      if (m_sink)
        write_log(
            m_sink, LL_INFO,
            "[ParserCTPOpt] Market data of {} contracts subscribed in total",
            nCount);
    }
  }
  codeFilter.clear();
...
bool ParserCTPOpt::isConnected() { return m_pUserAPI != NULL; }
