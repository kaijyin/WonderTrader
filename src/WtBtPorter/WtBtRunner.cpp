...
WtBtRunner::WtBtRunner()
    : _cta_mocker(NULL), _sel_mocker(NULL)
      ...
      ,
      _cb_cta_init(NULL), _cb_cta_tick(NULL), _cb_cta_calc(NULL),
      _cb_cta_calc_done(NULL), _cb_cta_bar(NULL), _cb_cta_sessevt(NULL),
      _cb_cta_cond_trigger(NULL)

      ,
      _cb_sel_init(NULL), _cb_sel_tick(NULL), _cb_sel_calc(NULL),
      _cb_sel_calc_done(NULL), _cb_sel_bar(NULL), _cb_sel_sessevt(NULL)
      ...
      _ext_adj_fct_loader(NULL), _ext_tick_loader(NULL)

      ,
      _inited(false), _running(false), _async(false) {
  install_signal_hooks([](const char *message) { WTSLogger::error(message); });
}

WtBtRunner::~WtBtRunner() {}

bool WtBtRunner::loadRawHisBars(void *obj, const char *stdCode,
...
bool WtBtRunner::loadFinalHisBars(void *obj, const char *stdCode,
                                  WTSKlinePeriod period, FuncReadBars cb) {
  StdUniqueLock lock(_feed_mtx);
...
bool WtBtRunner::loadAdjFactors(void *obj, const char *stdCode,
                                FuncReadFactors cb) {
  StdUniqueLock lock(_feed_mtx);
  if (_ext_adj_fct_loader == NULL)
    return false;
...
bool WtBtRunner::loadRawHisTicks(void *obj, const char *stdCode, uint32_t uDate,
                                 FuncReadTicks cb) {
  StdUniqueLock lock(_feed_mtx);
...
void WtBtRunner::feedAdjFactors(const char *stdCode, uint32_t *dates,
                                double *factors, uint32_t count) {
  if (_ext_adj_fct_loader == NULL) {
    WTSLogger::error("Cannot feed adjusting factors because of no extented "
                     "adjusting factor loader registered.");
    return;
  }
...
void WtBtRunner::feedRawTicks(WTSTickStruct *ticks, uint32_t count) {
  if (_ext_tick_loader == NULL) {
    WTSLogger::error(
        "Cannot feed ticks because of no extented tick loader registered.");
    return;
  }
...
void WtBtRunner::registerCtaCallbacks(
...
void WtBtRunner::registerSelCallbacks(
    FuncStraInitCallback cbInit, FuncStraTickCallback cbTick,
    FuncStraCalcCallback cbCalc, FuncStraBarCallback cbBar,
    FuncSessionEvtCallback cbSessEvt,
    FuncStraCalcCallback cbCalcDone /* = NULL*/) {
...
void WtBtRunner::registerHftCallbacks(
    FuncStraInitCallback cbInit, FuncStraTickCallback cbTick,
...
uint32_t WtBtRunner::initCtaMocker(const char *name, int32_t slippage /* = 0 */,
  ...
  _cta_mocker = new ExpCtaMocker(&_replayer, name, slippage, persistData,
                                 &_notifier, isRatioSlp);
  if (bIncremental) {
...
uint32_t WtBtRunner::initSelMocker(const char *name, uint32_t date,
  ...
  _replayer.register_task(_sel_mocker->id(), date, time, period, trdtpl,
                          session);
  return _sel_mocker->id();
}

void WtBtRunner::ctx_on_bar(uint32_t id, const char *stdCode,
...
void WtBtRunner::ctx_on_calc(uint32_t id, uint32_t curDate, uint32_t curTime,
                             EngineType eType /* = ET_CTA */) {
  switch (eType) {
  case ET_CTA:
    if (_cb_cta_calc)
...
void WtBtRunner::ctx_on_calc_done(uint32_t id, uint32_t curDate,
                                  uint32_t curTime,
                                  EngineType eType /* = ET_CTA */) {
  switch (eType) {
  case ET_CTA:
...
void WtBtRunner::ctx_on_cond_triggered(uint32_t id, const char *stdCode,
...
void WtBtRunner::ctx_on_session_event(uint32_t id, uint32_t curTDate,
                                      bool isBegin /* = true */,
                                      EngineType eType /* = ET_CTA */) {
  switch (eType) {
  case ET_CTA:
...
void WtBtRunner::ctx_on_tick(uint32_t id, const char *stdCode,
                             WTSTickData *newTick,
                             EngineType eType /*= ET_CTA*/) {
  switch (eType) {
  case ET_CTA:
...
void WtBtRunner::hft_on_order_queue(uint32_t id, const char *stdCode,
                                    WTSOrdQueData *newOrdQue) {
  if (_cb_hft_ordque)
    _cb_hft_ordque(id, stdCode, &newOrdQue->getOrdQueStruct());
}

void WtBtRunner::hft_on_order_detail(uint32_t id, const char *stdCode,
                                     WTSOrdDtlData *newOrdDtl) {
  if (_cb_hft_orddtl)
    _cb_hft_orddtl(id, stdCode, &newOrdDtl->getOrdDtlStruct());
}

void WtBtRunner::hft_on_transaction(uint32_t id, const char *stdCode,
...
void WtBtRunner::hft_on_entrust(uint32_t cHandle, WtUInt32 localid,
...
void WtBtRunner::hft_on_order(uint32_t cHandle, WtUInt32 localid,
...
void WtBtRunner::hft_on_trade(uint32_t cHandle, WtUInt32 localid,
...
void WtBtRunner::config(const char *cfgFile, bool isFile /* = true */) {
  ...
  _cfg = isFile ? WTSCfgLoader::load_from_file(cfgFile)
  ...
  _replayer.init(_cfg->get("replayer"), &_notifier,
                 _ext_fnl_bar_loader != NULL ? this : NULL);
  ...
  if (strcmp(mode, "cta") == 0 && cfgMode) {
    ...
    if (cfgTask)
      _replayer.register_task(
          _sel_mocker->id(), cfgTask->getUInt32("date"),
          cfgTask->getUInt32("time"), cfgTask->getCString("period"),
          cfgTask->getCString("trdtpl"), cfgTask->getCString("session"));
...
void WtBtRunner::release() { WTSLogger::stop(); }

void WtBtRunner::set_time_range(WtUInt64 stime, WtUInt64 etime) {
  _replayer.set_time_range(stime, etime);

  WTSLogger::info("Backtest time range is set to be [{},{}] mannually", stime,
                  etime);
}
...
void WtBtRunner::clear_cache() { _replayer.clear_cache(); }
