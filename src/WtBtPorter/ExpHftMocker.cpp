...
void ExpHftMocker::on_bar(const char *stdCode, const char *period,
  ...
  getRunner().ctx_on_bar(_context_id, stdCode, realPeriod.c_str(), newBar,
                         ET_HFT);
}
...
void ExpHftMocker::on_entrust(uint32_t localid, const char *stdCode,
...
void ExpHftMocker::on_order(uint32_t localid, const char *stdCode, bool isBuy,
                            double totalQty, double leftQty, double price,
                            bool isCanceled, const char *userTag) {
  HftMocker::on_order(localid, stdCode, isBuy, totalQty, leftQty, price,
                      isCanceled, userTag);

  getRunner().hft_on_order(_context_id, localid, stdCode, isBuy, totalQty,
                           leftQty, price, isCanceled, userTag);
}
...
void ExpHftMocker::on_bactest_end() { getRunner().on_backtest_end(); }