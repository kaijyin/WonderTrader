...
int run(const char *cfgfile, bool bAsync = false, bool isFile = true) {
  ...
  if (!isFile) {
    ...
    CLASSMASK =
        ini.readUInt("config", "mask", 1 | 2 | 4); // 1-期货,2-期权,4-股票
    ...
    ONLYINCFG =
        wt_stricmp(ini.readString("config", "onlyincfg", "false").c_str(),
                   "true") == 0;

#ifdef _WIN32
    MODULE_NAME =
        ini.readString("config", "module", "./soptthosttraderapi_se.dll");
#else
    MODULE_NAME =
        ini.readString("config", "module", "./soptthosttraderapi_se.so");
#endif
  } else {
  ...
  if (FRONT_ADDR.empty() || BROKER_ID.empty() || INVESTOR_ID.empty() ||
      PASSWORD.empty() || SAVEPATH.empty()) {
    return 0;
  }
  ...
  if (!map_files.empty()) {
    ...
    for (const std::string &fName : ayFiles) {
      ...
      for (int i = 0; i < cout; i++) {
        ...
        bool isUTF8 = EncodingHelper::isUtf8((unsigned char *)pName.c_str(),
                                             pName.size());
        if (!isUTF8)
        ...
        printf("Commodity name mapping: %s - %s\r\n", ayKeys[i].c_str(),
               isUTF8 ? UTF8toChar(ayVals[i]).c_str() : ayVals[i].c_str());
#else
        printf("Commodity name mapping: %s - %s\r\n", ayKeys[i].c_str(),
               isUTF8 ? ayVals[i].c_str() : ChartoUTF8(ayVals[i]).c_str());
#endif
      }
...
#ifdef _WIN32
#ifdef _WIN64
  g_ctpCreator = (CTPCreator)DLLHelper::get_symbol(
      dllInst,
...
#else
  g_ctpCreator = (CTPCreator)DLLHelper::get_symbol(
      dllInst, "_ZN8ctp_sopt19CThostFtdcTraderApi19CreateFtdcTraderApiEPKc");
#endif
  if (g_ctpCreator == NULL)
    printf("Loading CreateFtdcTraderApi failed\r\n");

  std::string flowPath =
      fmtutil::format("./CTPFlow/{}/{}/", BROKER_ID, INVESTOR_ID);
