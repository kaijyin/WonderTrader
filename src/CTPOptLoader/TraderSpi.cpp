...
inline bool isOption(TThostFtdcProductClassType pClass) {
  if (pClass == THOST_FTDC_PC_Options || pClass == THOST_FTDC_PC_SpotOption ||
      pClass == THOST_FTDC_PC_ETFOption)
    return true;
    ...
    if (pInstrument) {
      ...
      do {
        if (isGranted) {
          ...
          std::cerr << "--->>> OnRspQryInstrument: " << pInstrument->ExchangeID
          ...
          std::string key =
              StrUtil::printf("%s.%s", pInstrument->ExchangeID, pid.c_str());
          ...
          if (it == _commodities.end()) {
            ...
            if (bStock)
              commInfo.m_tradeMode =
                  (pInstrument->PositionDateType == THOST_FTDC_PDT_NoUseHistory)
                      ? TM_LongT1
                      : TM_Long;
          ...
          key = StrUtil::printf("%s.%s", pInstrument->ExchangeID,
