...
public:
  ...
  static inline std::string getLocalTime(bool bIncludeMilliSec = true) {
    ...
    char str[64] = {0};
    if (bIncludeMilliSec)
      sprintf(str, "%02d:%02d:%02d,%03d", tNow->tm_hour, tNow->tm_min,
              tNow->tm_sec, millitm);
    else
  ...
  static inline uint64_t getYYYYMMDDhhmmss() {
    ...
    uint64_t date = (tNow->tm_year + 1900) * 10000 + (tNow->tm_mon + 1) * 100 +
                    tNow->tm_mday;
  ...
  static inline void getDateTime(uint32_t &date, uint32_t &time) {
    ...
    date = (tNow->tm_year + 1900) * 10000 + (tNow->tm_mon + 1) * 100 +
           tNow->tm_mday;
  ...
  static inline uint32_t getCurDate() {
    ...
    time_t now = ltime / 1000;
    tm *tNow = localtime(&now);
    uint32_t date = (tNow->tm_year + 1900) * 10000 + (tNow->tm_mon + 1) * 100 +
                    tNow->tm_mday;
   ...
   *	@isToUTC		是否转成UTC时间
   */
  static inline int64_t makeTime(long lDate, long lTimeWithMs,
                                 bool isToUTC = false) {
    tm t;
    memset(&t, 0, sizeof(tm));
  ...
  static std::string timeToString(int64_t mytime) {
    ...
    char tm_buf[64] = {'\0'};
    if (msec > 0) // 是否有毫秒
      sprintf(tm_buf, "%4d%02d%02d%02d%02d%02d.%03d", t.tm_year + 1900,
              t.tm_mon + 1, t.tm_mday, t.tm_hour, t.tm_min, t.tm_sec, msec);
    else
      sprintf(tm_buf, "%4d%02d%02d%02d%02d%02d", t.tm_year + 1900, t.tm_mon + 1,
  ...
  static uint32_t getNextDate(uint32_t curDate, int days = 1) {
    ...
    return (newT->tm_year + 1900) * 10000 + (newT->tm_mon + 1) * 100 +
           newT->tm_mday;
  }
...
public:
  ...
  public:
    ...
    uint32_t time() { return t.tm_hour * 10000 + t.tm_min * 100 + t.tm_sec; }

    uint32_t time_ms() {
      return t.tm_hour * 10000000 + t.tm_min * 100000 + t.tm_sec * 1000 + _msec;
    }

    const char *fmt(const char *sfmt = "%Y.%m.%d %H:%M:%S",
                    bool hasMilliSec = false) const {
  ...
  class Ticker {
  public:
    Ticker() { _tick = std::chrono::high_resolution_clock::now(); }

    void reset() { _tick = std::chrono::high_resolution_clock::now(); }
