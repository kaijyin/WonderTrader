...
public:
  ...
  template <class T> T readValue(const char *szPath, T defVal) {
    try {
  ...
  std::string readString(const char *szSec, const char *szKey,
                         const char *defVal = "") {
  ...
  int readSecKeyValArray(const char *szSec, FieldArray &ayKey,
                         FieldArray &ayVal) {
    try {
      const boost::property_tree::ptree &_sec = _root.get_child(szSec);
      for (auto it = _sec.begin(); it != _sec.end(); it++) {
  ...
  template <class T> void writeValue(const char *szPath, T val) {
