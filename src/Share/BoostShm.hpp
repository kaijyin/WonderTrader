...
public:
  ...
  bool open(const char *name) {
    try {
      _obj = new boost::interprocess::shared_memory_object(
          boost::interprocess::open_only, name,
          boost::interprocess::read_write);
      _region = new boost::interprocess::mapped_region(
          *_obj, boost::interprocess::read_write);
  ...
  bool create(const char *name, std::size_t size) {
    try {
      ...
      _obj = new boost::interprocess::shared_memory_object(
          boost::interprocess::create_only, name,
          boost::interprocess::read_write);
      _obj->truncate(size);
      _region = new boost::interprocess::mapped_region(
          *_obj, boost::interprocess::read_write);
  ...
  inline bool valid() const { return _obj != nullptr; }
};