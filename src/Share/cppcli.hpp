...
#ifdef CPPCLI_DEBUG
template <class... Args> void __cppcli_debug_print(const Args &...args) {
...
private:
  static int replace_all(std::string &str, const std::string &pattern,
...
public:
  ...
  static void InitCommandMap(int length, char *strArr[],
...
int cppcli::detail::pathUtil::replace_all(std::string &str,
...
std::string cppcli::detail::pathUtil::getFilename(const std::string &filePath) {
  ...
  std::string::size_type pos =
      filePathCopy.find_last_of(CPPCLI_SEPARATOR_TYPE) + 1;
  return std::move(filePathCopy.substr(pos, filePathCopy.length() - pos));
}

std::string cppcli::detail::pathUtil::getFilenameWithOutSuffix(
...
std::string
cppcli::detail::pathUtil::getFileSuffix(const std::string &filePath) {
  std::string filename = getFilename(filePath);
...
void cppcli::detail::algoUtil::InitCommandMap(
    int length, char *strArr[], std::map<std::string, std::string> &stringMap) {
  ...
  for (int currentIndex = 1; currentIndex < length; currentIndex++) {
    ...
    if (keyIndex != -1 && theStr.size() > 0 && currentIndex == keyIndex + 1) {
      ...
      if (theStr.find_first_of('-') == 0 && theStr.size() > 1 &&
          !isdigit(theStr.at(1))) {
        valueTmp = "";
      } else {
        valueTmp = theStr;
      }
      // valueTmp = theStr.find_first_of('-') == 0 && theStr.size() > 1 ? "" :
      // theStr;
    ...
    if (theStr.find_first_of('-') == 0 &&
        int(std::count(theStr.begin(), theStr.end(), '-')) < theStr.size() &&
        !isdigit(theStr.at(1))) {
...
bool cppcli::detail::algoUtil::isDouble(const std::string &value) {
  ...
  if (value.size() < 3)
    return false;
  std::string tmpValue =
      value.at(0) == '-' ? value.substr(0, value.npos) : value;
  int numCount = 0;
  ...
  if (numCount == tmpValue.size() - 1 && tmpValue.rfind('.') > 0 &&
      tmpValue.rfind('.') < tmpValue.size() - 1) {
    return true;
  }
  return false;
}
...
private:
  ...
  cppcli::detail::ValueTypeEnum _valueType =
      cppcli::detail::ValueTypeEnum::STRING;
...
public:
  ...
  Rule(const std::string &shortParam, const std::string &longParam,
  ...
  template <class T,
            class = typename std::enable_if<std::is_same<T, int>::value>::type>
  int get() {
  ...
  template <class... Args> Rule *limitOneOf(Args... args) {
  ...
  template <class T> Rule *setDefault(const T &defaultValue) {
    std::ostringstream oss;
    oss << defaultValue;
    _default = oss.str();
    return this;
...
cppcli::HelpDocEnum cppcli::Rule::detail::HelpDocStruct::_helpDocType =
    cppcli::HelpDocEnum::USE_DEFAULT_HELPDOC;
...
const std::string
cppcli::Rule::getError(cppcli::detail::ErrorEventType errorEventType) {
  std::ostringstream oss;
  ...
  case cppcli::detail::ErrorEventType::NUMRANGE_ERROR: {
    oss << _limitNumRange.first << std::move("(MIN), ") << _limitNumRange.second
        << std::move("(MAX)");
    break;
  }
  }
...
std::string cppcli::Rule::buildHelpInfoLine() {
  ...
  oss << std::setw(commandsDis) << std::left
      << (_longParam.empty() ? _shortParam : _shortParam + " | " + _longParam);

  int writeTime = _helpInfo.size() % helpInfoDis - theDis == 0
                      ? int(_helpInfo.size() / (helpInfoDis - theDis))
                      : int((_helpInfo.size() / (helpInfoDis - theDis))) + 1;
  std::string necessaryOutStr = _necessary ? "true" : "false";
  std::string defaultValueOutStr =
      _default == "[EMPTY]" ? _default : "=" + _default;

  if (writeTime == 1) {
    oss << std::setw(helpInfoDis) << std::left << _helpInfo;
    oss << std::setw(necessaryDis) << std::left
        << "MUST-ENTER[" + necessaryOutStr + "]";
  ...
  for (int i = 0; i < writeTime; i++) {
    if (i == 0) {
      oss << std::setw(helpInfoDis) << std::setw(helpInfoDis)
          << _helpInfo.substr(pos, helpInfoDis - theDis);
      oss << std::setw(necessaryDis) << std::left
          << "MUST-ENTER[" + necessaryOutStr + "]";
      oss << std::setw(defaultStrDis) << std::left << "DEFAULT->" + _default;
      oss << std::endl;
...
std::string cppcli::Rule::debugInfo() const {
  ...
  if (_longParam.empty()) {
    ...
    oss << "command params --> " << _shortParam << "|" << _longParam
        << std::endl;
  }
  ...
  oss << "[CPPCLI_DEBUG]     limitNumRange = (" << _limitNumRange.first << " "
      << _limitNumRange.second << ")";
...
public:
  ...
  cppcli::Rule *operator()(const std::string &shortParam,
                           const std::string &longParam,
                           const std::string helpInfo);
  cppcli::Rule *operator()(const std::string &shortParam,
                           const std::string &longParam,
                           const std::string helpInfo, bool necessary);
...
private:
  ...
  void errorExitFunc(const std::string errorInfo, int index,
                     cppcli::ErrorExitEnum exitType,
  ...
  std::string get(const std::string shortParam) {
    ...
    std::cout << "error: don't set where short-param = " << shortParam
              << std::endl;
    std::exit(-1);
  }
...
int cppcli::Option::Option::detail::valueTypeVerify(Option &opt) {
  ...
  for (int index = 0; index < opt._ruleVec.size(); index++) {
    rule = opt._ruleVec.at(index);
    if (rule->_valueType == cppcli::detail::ValueTypeEnum::STRING ||
        !opt.mapExists(rule)) {
      continue;
    }
...
int cppcli::Option::Option::detail::oneOfVerify(Option &opt) {
  ...
  for (int index = 0; index < opt._ruleVec.size(); index++) {
    ...
    if (std::find(rule->_limitOneVec.begin(), rule->_limitOneVec.end(),
                  rule->_inputValue) == rule->_limitOneVec.end()) {
...
void cppcli::Option::errorExitFunc(const std::string errorInfo, int index,
                                   cppcli::ErrorExitEnum exitType,
                                   cppcli::detail::ErrorEventType eventType) {
  ...
  if (cppcli::Rule::detail::HelpDocStruct::rule != nullptr) {
    oss << std::endl
        << "Use [" << cppcli::Rule::detail::HelpDocStruct::rule->_shortParam
        << "] gain help doc";
  }
...
cppcli::Rule *cppcli::Option::operator()(const std::string &shortParam,
                                         const std::string &longParam,
                                         const std::string helpInfo) {
...
cppcli::Rule *cppcli::Option::operator()(const std::string &shortParam,
  ...
  _ruleVec.push_back(
      new cppcli::Rule(shortParam, longParam, helpInfo, necessary));
  return _ruleVec.back();
}
...
void cppcli::Option::parse() {
  ...
  CPPCLI_DEBUG_PRINT("necessaryResult=", necessaryResult,
                     ", valueTypeResult=", valueTypeResult,
                     ",oneOfResult=", oneOfResult, ", numRangeResult",
                     numRangeResult);
