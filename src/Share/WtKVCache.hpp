...
private:
  bool resize(uint32_t newCap, CacheLogger logger = nullptr) {
    ...
    uint64_t uOldSize =
        sizeof(CacheBlock) + sizeof(CacheItem) * cBlock->_capacity;
    ...
    try {
      ...
      if (logger)
        logger("Got an exception while resizing cache file");
      return false;
    }
    ...
    try {
      if (!pNewMf->map(filename.c_str())) {
        delete pNewMf;
        if (logger)
          logger("Mapping cache file failed");
        return false;
      }
    } catch (std::exception &) {
      if (logger)
        logger("Got an exception while mapping cache file");
      return false;
    }
...
public:
  bool init(const char *filename, uint32_t uDate,
            CacheLogger logger = nullptr) {
    bool isNew = false;
    ...
    if (!isNew && _cache._block->_date != uDate) {
      ...
      memset(&_cache._block->_items, 0,
             sizeof(CacheItem) * _cache._block->_capacity);

      if (logger)
        logger("Cache file reset due to a different date");
    }
    ...
    if (isNew) {
      ...
      do {
        uint64_t uSize =
            sizeof(CacheBlock) + sizeof(CacheItem) * _cache._block->_capacity;
        uint64_t realSz = _cache._file->size();
        if (realSz != uSize) {
          uint32_t realCap =
              (uint32_t)((realSz - sizeof(CacheBlock)) / sizeof(CacheItem));
  ...
  void put(const char *key, const char *val, std::size_t len = 0,
           CacheLogger logger = nullptr) {
    auto it = _indice.find(key);
    if (it != _indice.end()) {
