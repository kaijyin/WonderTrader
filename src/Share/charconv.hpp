...
class UTF8toChar {
public:
  UTF8toChar(const char *utf8_string) { init(utf8_string); }

  UTF8toChar(const std::string &utf8_string) { init(utf8_string.c_str()); }
  ...
  void init(const char *utf8_string) {
    if (0 == utf8_string)
      ...
      // Either TCHAR = Unicode (2 bytes), or utf8_string contains non-ASCII
      // characters. Needs conversion
      needFree = true;
      ...
      MultiByteToWideChar(CP_UTF8, 0, utf8_string, -1, buffer,
                          (int)string_len + 1);
      buffer[string_len] = 0;
  ...
  operator const char *() { return t_string; }

  const char *c_str() { return t_string; }
...
class ChartoUTF8 {
public:
  ChartoUTF8(const std::string &str) { init(str.c_str()); }

  ChartoUTF8(const char *t_string) { init(t_string); }
  ...
  void init(const char *t_string) {
    if (0 == t_string)
      ...
      MultiByteToWideChar(CP_ACP, 0, t_string, -1, w_string,
                          (int)string_len + 1);
      w_string[string_len] = 0;

      // Convert from Unicode (2 bytes) to UTF8
      utf8_string = new char[dst_len];
      WideCharToMultiByte(CP_UTF8, 0, w_string, -1, utf8_string, (int)dst_len,
                          0, 0);
  ...
  operator const char *() { return utf8_string; }

  const char *c_str() const { return utf8_string; }
      ...
      case '%':
        if (isxdigit(src[i + 1]) && isxdigit(src[i + 2])) {
          ...
          if (!((hex >= 48 && hex <= 57) ||  // 0-9
                ...
                (hex >= 65 && hex <= 90) ||  // A-Z
                // 一些特殊符号及保留字[$-_.+!*'(),]  [$&+,/:;=?@]
                hex == 0x21 || hex == 0x24 || hex == 0x26 || hex == 0x27 ||
                hex == 0x28 || hex == 0x29 || hex == 0x2a || hex == 0x2b ||
                hex == 0x2c || hex == 0x2d || hex == 0x2e || hex == 0x2f ||
                hex == 0x3A || hex == 0x3B || hex == 0x3D || hex == 0x3f ||
                hex == 0x40 || hex == 0x5f)) {
  ...
  static bool isUtf8(unsigned char *data, std::size_t len) {
    ...
    while (i < len) {
      if ((data[i] & 0x80) == 0x00) {
        ...
        // preNUm()
        // 返回首个字节8个bits中首bit前面1bit的个数，该数量也是该字符所使用的字节数
        i++;
