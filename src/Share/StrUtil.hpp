...
public:
  ...
  to trim both).
  */
  static inline void trim(std::string &str, const char *delims = " \t\r",
  ...
  static inline std::string trim(const char *str, const char *delims = " \t\r",
                                 bool left = true, bool right = true) {
    std::string ret = str;
    if (right)
  ...
  static inline void trimAllSpace(std::string &str) {
    std::string::iterator destEnd = std::remove_if(
        str.begin(), str.end(), [](const char &c) { return c == ' '; });
  ...
  //{
  //	std::string::iterator
  //destEnd=std::remove_if(str.begin(),str.end(),std::bind1st(std::equal_to<char>(),ch));
  //	str.resize(destEnd-str.begin());
  // }
  ...
  /** Returns a std::stringVector that contains all the substd::strings
  delimited by the characters in the passed <code>delims</code> argument.
  @param
  delims A list of delimiter characters to split by
  @param
  maxSplits The maximum number of splits to perform (0 for unlimited splits). If
  this parameters is > 0, the splitting process will stop after this many
  splits, left to right.
  */
  static inline StringVector split(const std::string &str,
  ...
  splits, left to right.
  */
  static inline void split(const std::string &str, StringVector &ret,
                           const std::string &delims = "\t\n ",
                           unsigned int maxSplits = 0) {
    unsigned int numSplits = 0;
   ...
   *	@ignroreCase是否忽略大小写
   */
  static inline bool startsWith(const char *str, const char *pattern,
   ...
   *	@ignroreCase是否忽略大小写
   */
  static inline bool endsWith(const char *str, const char *pattern,
                              bool ignoreCase = true) {
    size_t thisLen = strlen(str);
    size_t patternLen = strlen(pattern);
    if (thisLen < patternLen || patternLen == 0)
   ...
   */
  static inline std::string standardisePath(const std::string &init,
                                            bool bIsDir = true) {
  ...
  */
  static inline void splitFilename(const std::string &qualifiedName,
  ...
  */
  ...
  /// Constant blank std::string, useful for returning by ref where local does
  /// not exist
  ...
  static inline void replace(std::string &str, const char *src,
                             const char *des) {
    std::string ret = "";
