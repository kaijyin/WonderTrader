 ...
 * It has reference semantics; all copies of the same pool are equivalent and
 * interchangeable. All operations on a pool except assignment are strongly
 * thread safe or sequentially consistent; that is, the behavior of concurrent
 * calls is as if the calls have been issued sequentially in an unspecified
 * order.
 *
 ...
 * \param SchedulingPolicy A task container which determines how tasks are
 * scheduled. It is guaranteed that this container is accessed only by one
 * thread at a time. The scheduler shall not throw exceptions.
 *
...
class thread_pool {
  ...
  shared_ptr<void>
      m_shutdown_controller; // If the last pool holding a pointer to the core
                             // is deleted the controller shuts the pool down.
  ...
  typedef SchedulingPolicy<task_type>
      scheduler_type; //!< Indicates the scheduler's type.
                      /*   typedef thread_pool<Task,
                                             SchedulingPolicy,
                                             SizePolicy,
                                             ShutdownPolicy > pool_type;          //!< Indicates the
                         thread pool's type.
                      */
...
public:
  /*! Constructor.
   * \param initial_threads The pool is immediately resized to set the specified
   * number of threads. The pool's actual number threads depends on the
   * SizePolicy.
   */
