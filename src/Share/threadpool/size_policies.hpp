 ...
 */
template <typename Pool> struct empty_controller {
 ...
 * \param Pool The pool's core type.
 */
template <typename Pool> class resize_controller {
  typedef typename Pool::size_policy_type size_policy_type;
  reference_wrapper<size_policy_type> m_policy;
  shared_ptr<Pool>
      m_pool; //!< to make sure that the pool is alive (the policy pointer is
              //!< valid) as long as the controller exists
 ...
 * \param Pool The pool's core type.
 */
template <typename Pool> class static_size {
