 ...
 * prio_task_funcs can be compared using the operator < which realises a partial
 * ordering. The wrapped task function is invoked by calling the operator ().
 *
 * \see prio_scheduler
 *
 */
...
public:
   ...
   * \return true if the priority of *this is less than right hand side's
 ...
 * The wrapped task function is invoked by calling the operator () and it is
 * executed in regular time intervals until false is returned. The interval
 * length may be zero. Please note that a pool's thread is engaged as long as
 * the task is looped.
 *
 */
...
public:
  /*! Constructor.
   * \param function The task's function object which is looped until false is
   * returned.
   * \param interval The minimum break time in milli seconds before the first
   * execution of the task function and between the following ones.
   */
  ...
  void operator()(void) const {
    if (m_function) {
      if (m_break_s > 0 ||
          m_break_ns > 0) { // Sleep some time before first execution
        xtime xt;
