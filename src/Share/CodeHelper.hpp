...
public:
  typedef struct _CodeInfo {
    ...
    //	uint8_t	_exright;	//是否是复权代码,如SH600000Q: 0-不复权,
    //1-前复权, 2-后复权
    // };
     ...
     *	因为后面要对主力合约做复权处理了
     */
    uint8_t _exright; // 是否是复权代码,如SH600000Q: 0-不复权, 1-前复权,
                      // 2-后复权
    ...
    inline void clear() { memset(this, 0, sizeof(_CodeInfo)); }

    inline bool hasRule() const { return strlen(_ruletag) > 0; }
  } CodeInfo;

private:
  static inline std::size_t find(const char *src, char symbol = '.',
                                 bool bReverse = false) {
...
public:
  ...
  static bool isStdChnFutOptCode(const char *code) {
    /* 定义正则表达式 */
    // static cregex reg_stk =
    // cregex::compile("^[A-Z]+.[A-z]+\\d{4}.(C|P).\\d+$");
    // //CFFEX.IO2007.C.4000 return 	regex_match(code, reg_stk);
    char state = 0;
    std::size_t i = 0;
    for (;; i++) {
      char ch = code[i];
  ...
  static inline bool isStdMonthlyFutCode(const char *code) {
    ...
    /* 定义正则表达式 */
    static cregex reg_stk =
        cregex::compile("^[A-Z]+.[A-z]+.\\d{4}$"); // CFFEX.IO.2007
   ...
   *	这个不会有永续合约的代码传到这里来，如果有的话就是调用的地方有Bug!
   */
  static inline std::string rawMonthCodeToStdCode(const char *code,
                                                  const char *exchg,
                                                  bool isComm = false) {
  ...
  rawFlatCodeToStdCode(const char *code, const char *exchg, const char *pid) {
    thread_local static char buffer[64] = {0};
  ...
  static inline bool isMonthlyCode(const char *code) {
    // using namespace boost::xpressive;
    // 最后3-6位都是数字，才是分月合约
    // static cregex reg_stk = cregex::compile("^.*[A-z|-]\\d{3,6}$");
    // //CFFEX.IO.2007 return 	regex_match(code, reg_stk);
   ...
   *	-- 暂时没有地方调用 --
   */
  static inline std::string rawFutOptCodeToStdCode(const char *code,
                                                   const char *exchg) {
    using namespace boost::xpressive;
    /* 定义正则表达式 */
    static cregex reg_stk = cregex::compile(
        "^[A-z]+\\d{4}-(C|P)-\\d+$"); // 中金所、大商所格式IO2013-C-4000
  ...
  static CodeInfo extractStdChnFutOptCode(const char *stdCode) {
    ...
    if (strcmp(codeInfo._exchg, "SHFE") == 0 ||
        strcmp(codeInfo._exchg, "INE") == 0) {
      fmt::format_to(codeInfo._code, "{}{}{}", ay[1], ay[2], ay[3]);
    } else if (strcmp(codeInfo._exchg, "CZCE") == 0) {
      std::string &s = ay[1];
      fmt::format_to(codeInfo._code, "{}{}{}{}", s.substr(0, s.size() - 4),
                     s.substr(s.size() - 3), ay[2], ay[3]);
    } else {
      fmt::format_to(codeInfo._code, "{}-{}-{}", ay[1], ay[2], ay[3]);
    }
    ...
    if (isStdChnFutOptCode(stdCode)) {
      ...
      if (idx2 == std::string::npos) {
        ...
        if (extlen == 4 && '0' <= lastCh && lastCh <= '9') {
          // 如果最后一段是4位数字，说明是分月合约
          // TODO:
          // 这样的判断存在一个假设，最后一位是数字的一定是期货分月合约，以后可能会有问题，先注释一下
          // 那么code得加上品种id
