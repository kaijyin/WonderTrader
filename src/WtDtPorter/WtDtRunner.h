...
public:
  void initialize(const char *cfgFile, const char *logCfg,
                  const char *modDir = "", bool bCfgFile = true,
...
public:
  ...
  void registerExtHftDataDumper(FuncDumpOrdQue ordQueDumper,
                                FuncDumpOrdDtl ordDtlDumper,
                                FuncDumpTrans transDumper);

  bool dumpHisBars(const char *id, const char *stdCode, const char *period,
                   WTSBarStruct *bars, uint32_t count);

  bool dumpHisTicks(const char *id, const char *stdCode, uint32_t uDate,
                    WTSTickStruct *ticks, uint32_t count);

  bool dumpHisOrdQue(const char *id, const char *stdCode, uint32_t uDate,
                     WTSOrdQueStruct *item, uint32_t count);

  bool dumpHisOrdDtl(const char *id, const char *stdCode, uint32_t uDate,
                     WTSOrdDtlStruct *items, uint32_t count);

  bool dumpHisTrans(const char *id, const char *stdCode, uint32_t uDate,
                    WTSTransStruct *items, uint32_t count);
