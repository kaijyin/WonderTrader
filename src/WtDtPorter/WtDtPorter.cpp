...
void initialize(WtString cfgFile, WtString logCfg, bool bCfgFile,
                bool bLogCfgFile) {
#ifdef _MSC_VER
...
void start(bool bAsync /* = false*/) { getRunner().start(bAsync); }
...
void register_extended_dumper(FuncDumpBars barDumper,
                              FuncDumpTicks tickDumper) {
  getRunner().registerExtDumper(barDumper, tickDumper);
}

void register_extended_hftdata_dumper(FuncDumpOrdQue ordQueDumper,
