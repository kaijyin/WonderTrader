...
void dump_bars(WtString binFolder, WtString csvFolder,
               WtString strFilter /* = "" */,
  ...
  for (boost::filesystem::directory_iterator iter(myPath); iter != endIter;
    ...
    if (bHeader->_type < BT_HIS_Minute1 || bHeader->_type > BT_HIS_Day) {
      if (cbLogger)
        cbLogger(
            StrUtil::printf("文件%s不是K线数据，跳过转换", binFolder).c_str());
      continue;
    }
    ...
    std::stringstream ss;
    ss << "date,time,open,high,low,close,settle,volume,turnover,open_interest,"
          "diff_interest"
       << std::endl;
    ...
    for (uint32_t i = 0; i < kcnt; i++) {
      ...
      ss << curBar.open << "," << curBar.high << "," << curBar.low << ","
         << curBar.close << "," << curBar.settle << "," << curBar.vol << ","
         << curBar.money << "," << curBar.hold << "," << curBar.add
         << std::endl;
    }

    BoostFile::write_file_contents(filename.c_str(), ss.str().c_str(),
                                   (uint32_t)ss.str().size());

    if (cbLogger)
      cbLogger(StrUtil::printf("%s写入完成,共%u条bar", filename.c_str(), kcnt)
                   .c_str());
  }
...
void dump_ticks(WtString binFolder, WtString csvFolder,
                WtString strFilter /* = "" */,
                FuncLogCallback cbLogger /* = NULL */) {
  std::string srcFolder = StrUtil::standardisePath(binFolder);
  if (!BoostFile::exists(srcFolder.c_str())) {
  ...
  for (boost::filesystem::directory_iterator iter(myPath); iter != endIter;
    ...
    ss.precision(6);
    ss << "exchg,code,tradingdate,actiondate,actiontime,price,open,high,low,"
          "settle,preclose,"
       << "presettle,preinterest,total_volume,total_turnover,open_interest,"
          "volume,turnover,additional,";
    for (int i = 0; i < 10; i++) {
      bool hasTail = (i != 9);
      ss << "bidprice" << i + 1 << "," << "bidqty" << i + 1 << "," << "askprice"
         << i + 1 << "," << "askqty" << i + 1 << (hasTail ? "," : "");
    }
    ss << std::endl;

    for (uint32_t i = 0; i < tcnt; i++) {
      const WTSTickStruct &curTick = ticks[i];
      ss << curTick.exchg << "," << curTick.code << "," << curTick.trading_date
         << "," << curTick.action_date << "," << curTick.action_time << ","
         << curTick.price << "," << curTick.open << "," << curTick.high << ","
         << curTick.low << "," << curTick.settle_price << ","
         << curTick.pre_close << "," << curTick.pre_settle << ","
         << curTick.pre_interest << "," << curTick.total_volume << ","
         << curTick.total_turnover << "," << curTick.open_interest << ","
         << curTick.volume << "," << curTick.turn_over << ","
         << curTick.diff_interest << ",";

      for (int j = 0; j < 10; j++) {
        bool hasTail = (j != 9);
        ss << curTick.bid_prices[j] << "," << curTick.bid_qty[j] << ","
           << curTick.ask_prices[j] << "," << curTick.ask_qty[j]
           << (hasTail ? "," : "");
      }
      ss << std::endl;
    }
    ...
    if (cbLogger)
      cbLogger(
          StrUtil::printf("%s写入完成,共%u条tick数据", filename.c_str(), tcnt)
              .c_str());
  }
...
void trans_csv_bars(WtString csvFolder, WtString binFolder, WtString period,
                    FuncLogCallback cbLogger /* = NULL */) {
  if (!BoostFile::exists(csvFolder))
    return;
  ...
  for (boost::filesystem::directory_iterator iter(myPath); iter != endIter;
    ...
    if (!reader.load_from_file(path.c_str())) {
      if (cbLogger)
        cbLogger(
            StrUtil::printf("读取数据文件%s失败...", path.c_str()).c_str());
      continue;
    }
    ...
    if (cbLogger)
      cbLogger(StrUtil::printf("数据文件%s全部读取完成,共%u条", path.c_str(),
                               bars.size())
                   .c_str());
    ...
    std::string cmprsData = WTSCmpHelper::compress_data(
        bars.data(), sizeof(WTSBarStruct) * bars.size());
...
// bool trans_bars(WtString barFile, FuncGetBarItem getter, int count, WtString
// period, FuncLogCallback cbLogger /* = NULL */)
//{
//	if (count == 0)
//	{
//		if (cbLogger)
...
// }
//
// bool trans_ticks(WtString tickFile, FuncGetTickItem getter, int count,
// FuncLogCallback cbLogger/* = NULL*/)
//{
//	if (count == 0)
//	{
...
//	std::string cmp_data = WTSCmpHelper::compress_data(ticks,
//buffer.size()); 	block->_size = cmp_data.size(); 	content.append(cmp_data);
//
//	BoostFile bf;
...
WtUInt32 read_dsb_ticks(WtString tickFile, FuncGetTicksCallback cb,
                        FuncCountDataCallback cbCnt,
                        FuncLogCallback cbLogger /* = NULL */) {
  ...
  if (cbLogger)
    cbLogger(
        StrUtil::printf("%s读取完成,共%u条tick数据", tickFile, tcnt).c_str());

  return (WtUInt32)tcnt;
}

WtUInt32 read_dsb_order_details(WtString dataFile, FuncGetOrdDtlCallback cb,
                                FuncCountDataCallback cbCnt,
                                FuncLogCallback cbLogger /* = NULL*/) {
  ...
  if (cbLogger)
    cbLogger(
        StrUtil::printf("%s读取完成,共%u条order detail数据", dataFile, tcnt)
            .c_str());

  return (WtUInt32)tcnt;
}

WtUInt32 read_dsb_order_queues(WtString dataFile, FuncGetOrdQueCallback cb,
                               FuncCountDataCallback cbCnt,
                               FuncLogCallback cbLogger /* = NULL*/) {
  std::string path = dataFile;
  ...
  if (cbLogger)
    cbLogger(StrUtil::printf("%s读取完成,共%u条order queue数据", dataFile, tcnt)
                 .c_str());

  return (WtUInt32)tcnt;
}

WtUInt32 read_dsb_transactions(WtString dataFile, FuncGetTransCallback cb,
                               FuncCountDataCallback cbCnt,
                               FuncLogCallback cbLogger /* = NULL*/) {
  std::string path = dataFile;
  ...
  if (cbLogger)
    cbLogger(StrUtil::printf("%s读取完成,共%u条transaction数据", dataFile, tcnt)
                 .c_str());

  return (WtUInt32)tcnt;
}

WtUInt32 read_dsb_bars(WtString barFile, FuncGetBarsCallback cb,
                       FuncCountDataCallback cbCnt, FuncLogCallback cbLogger) {
...
WtUInt32 read_dmb_bars(WtString barFile, FuncGetBarsCallback cb,
                       FuncCountDataCallback cbCnt, FuncLogCallback cbLogger) {
  std::string path = barFile;
...
WtUInt32 read_dmb_ticks(WtString tickFile, FuncGetTicksCallback cb,
                        FuncCountDataCallback cbCnt,
                        FuncLogCallback cbLogger /* = NULL */) {
  std::string path = tickFile;
...
WtUInt32 resample_bars(WtString barFile, FuncGetBarsCallback cb,
                       FuncCountDataCallback cbCnt, WtUInt64 fromTime,
                       WtUInt64 endTime, WtString period, WtUInt32 times,
                       WtString sessInfo, FuncLogCallback cbLogger /* = NULL */,
  ...
  if (isDay) {
    ...
    if (fromTime < 100000000 || endTime < 100000000) {
      if (cbLogger)
        cbLogger(
            "分钟线基础数据的开始时间结束时间应为时间，格式如yyyymmddHHMM");
      return 0;
    }
  }
  ...
  if (isDay)
    ...
    bar.time =
        fromTime % 100000000 + ((fromTime / 100000000) - 1990) * 100000000;
  }

  WTSBarStruct *pBar =
      std::lower_bound(bars, bars + (kcnt - 1), bar,
  ...
  if ((isDay && pBar->date < bar.date) || (!isDay && pBar->time < bar.time)) {
    ...
    return 0;
  } else if (sIdx != 0 && ((isDay && pBar->date > bar.date) ||
                           (!isDay && pBar->time > bar.time))) {
    pBar--;
    sIdx--;
  }
  ...
  if (eIdx != 0 &&
      ((isDay && pBar->date > bar.date) || (!isDay && pBar->time > bar.time))) {
    pBar--;
    eIdx--;
  }
  ...
  WTSDataFactory fact;
  WTSKlineData *kline =
      fact.extractKlineData(slice, kp, times, sInfo, true, bAlignSec);
  if (kline == NULL) {
    if (cbLogger)
      cbLogger("K线重采样失败");
    return 0;
  ...
  if (cbLogger)
    cbLogger(StrUtil::printf("%s重采样完成,共将%u条bar重采样为%u条新bar",
                             barFile, hitCnt, newCnt)
                 .c_str());
...
bool store_bars(WtString barFile, WTSBarStruct *firstBar, int count,
                WtString period, FuncLogCallback cbLogger /* = NULL */) {
  if (count == 0) {
    if (cbLogger)
      cbLogger("K线数据条数为0");
    return false;
...
bool store_ticks(WtString tickFile, WTSTickStruct *firstTick, int count,
                 FuncLogCallback cbLogger /* = NULL*/) {
  if (count == 0) {
    if (cbLogger)
      cbLogger("Tick数据条数为0");
...
bool store_order_details(WtString tickFile, WTSOrdDtlStruct *firstItem,
                         int count, FuncLogCallback cbLogger /* = NULL*/) {
  if (count == 0) {
    if (cbLogger)
...
bool store_order_queues(WtString tickFile, WTSOrdQueStruct *firstItem,
                        int count, FuncLogCallback cbLogger /* = NULL*/) {
  if (count == 0) {
    if (cbLogger)
...
bool store_transactions(WtString tickFile, WTSTransStruct *firstItem, int count,
                        FuncLogCallback cbLogger /* = NULL*/) {
  if (count == 0) {
    if (cbLogger)
