...
ParserXeleSkt::ParserXeleSkt()
    : _udp_socket(NULL), _strand(_io_service), _stopped(false), _sink(NULL),
      _prepared(false), _tick_cache(NULL) {}
...
bool ParserXeleSkt::reconnect() {
  ...
  { // 建立广播通道
    ...
    _udp_socket->set_option(
        ip::multicast::join_group(ip::make_address_v4(_mcast_host.c_str()),
                                  ip::make_address_v4(_local_host.c_str())));

    _udp_socket->async_receive_from(
        buffer(_udp_buffer), _udp_ep,
        boost::bind(&ParserXeleSkt::handle_udp_read, this,
                    boost::asio::placeholders::error,
                    boost::asio::placeholders::bytes_transferred));
  }
  write_log(_sink, LL_INFO,
            "[ParserXeleSkt] Ready to receive from multicast tunnel {}:{}...",
            _mcast_host, _mcast_port);
  return true;
}
...
bool ParserXeleSkt::prepare() {
  ...
  if (ec) {
    write_log(_sink, LL_ERROR,
              "[ParserXeleSkt] Preparing snapshots failed, connecting tcp "
              "tunnel {}:{} error: {}",
              _tcp_host, _tcp_port, ec.message());
    return false;
  }
  ...
  write_log(_sink, LL_INFO,
            "[ParserXeleSkt] Preparing snapshots via tcp tunnel {}:{}...",
            _tcp_host, _tcp_port);
  ...
  for (;;) {
    std::size_t reply_length =
        boost::asio::read(s, boost::asio::buffer(buffer), ec);
    if (ec) {
      // 接收错误
      break;
    }
  ...
  while (content.size() > snap_size) {
    ...
    CXeleShfeSnapShot *p =
        (CXeleShfeSnapShot *)(content.data() + sizeof(CXeleShfeMarketHead));
...
bool ParserXeleSkt::connect() {
  if (reconnect()) {
    _thrd_parser.reset(
        new StdThread(boost::bind(&io_service::run, &_io_service)));
  } else {
    return false;
  }
...
bool ParserXeleSkt::isConnected() { return _udp_socket != NULL; }
...
void ParserXeleSkt::handle_udp_read(const boost::system::error_code &e,
  ...
  if (e) {
    ...
    write_log(_sink, LL_ERROR,
              "[ParserXeleSkt] Error occured while receiving: {}({})",
...
void ParserXeleSkt::extract_buffer(uint32_t length) {
  ...
  while (length > len) {
    ...
    if (type == MESSAGE_MARKET_DATA) {
      CXeleShfeMarketData *p = reinterpret_cast<CXeleShfeMarketData *>(
          data + sizeof(CXeleShfeMarketHead));
      static std::size_t packLen =
          sizeof(CXeleShfeMarketHead) + sizeof(CXeleShfeMarketData);
      len += packLen;

      // write_log(_sink, LL_DEBUG, "[ParserXeleSkt] Receiving market data,
      // length: {}", packLen);
      ...
      if (tick != NULL) {
        ...
        uint32_t SnapMillisec =
            (p->SnapDateTime & XELE_MD_MS_MASK) ? FIVE_HUNDRED_MS : ZERO_MS;
        uint32_t actDate = (tNow->tm_year + 1900) * 10000 +
                           (tNow->tm_mon + 1) * 100 + tNow->tm_mday;
        uint32_t actTime =
            tNow->tm_hour * 10000 + tNow->tm_min * 100 + tNow->tm_sec;
        ...
        if (quote.trading_date == 0) {
          quote.trading_date = _bd_mgr->calcTradingDate(
              tick->getContractInfo()->getFullPid(), actDate, actTime / 100000);
        }
        ...
        if (recv_cnt % _gpsize == 0)
          write_log(_sink, LL_DEBUG,
                    "[ParserXeleSkt] {} ticks received in total", recv_cnt);
      }
    } else if (type == MESSAGE_DEPTH) {
      CXeleShfeDepthMarketData *p =
          (CXeleShfeDepthMarketData *)(data + sizeof(CXeleShfeMarketHead));
      static std::size_t packLen =
          sizeof(CXeleShfeMarketHead) + sizeof(CXeleShfeDepthMarketData);
      len += packLen;

      // write_log(_sink, LL_DEBUG, "[ParserXeleSkt] Receiving depth market data
      // of {}, length: {}", p->InstrumentNo, packLen);
      ...
      if (tick != NULL) {
        ...
        if (recv_cnt % _gpsize == 0)
          write_log(_sink, LL_DEBUG,
                    "[ParserXeleSkt] {} depth ticks received in total",
                    recv_cnt);
      }
    } else if (type == MESSAGE_TYPE_HEART_BEAT) {
      CXeleShfeHeartBeat *p = reinterpret_cast<CXeleShfeHeartBeat *>(
          data + sizeof(CXeleShfeMarketHead));
      static std::size_t packLen =
          sizeof(CXeleShfeMarketHead) + sizeof(CXeleShfeHeartBeat);
      len += packLen;
      write_log(_sink, LL_DEBUG,
                "[ParserXeleSkt] Receiving heartbeat packet, length: %d\n",
                packLen);
    }
  }
}
