...
ParserXTP::ParserXTP() : m_pUserAPI(NULL), m_iRequestID(0), m_uTradingDate(0) {}

ParserXTP::~ParserXTP() { m_pUserAPI = NULL; }
...
bool ParserXTP::init(WTSVariant *config) {
  ...
  std::string path =
      StrUtil::printf("%s/%s/", m_strFlowDir.c_str(), m_strUser.c_str());
  ...
  const char *creatorName =
      "?CreateQuoteApi@QuoteApi@API@XTP@@SAPEAV123@EPEBDW4XTP_LOG_LEVEL@@@Z";
#else
  const char *creatorName =
      "?CreateQuoteApi@QuoteApi@API@XTP@@SAPAV123@EPBDW4XTP_LOG_LEVEL@@@Z";
#endif
#else
  const char *creatorName =
      "_ZN3XTP3API8QuoteApi14CreateQuoteApiEhPKc13XTP_LOG_LEVEL";
#endif
...
void ParserXTP::release() { disconnect(); }
...
void ParserXTP::OnError(XTPRI *error_info) {
  if (IsErrorRspInfo(error_info)) {
    write_log(m_sink, LL_ERROR, "[ParserXTP] Error occured: ({}){}",
...
void ParserXTP::OnDisconnected(int nReason) {
  if (m_sink) {
    write_log(m_sink, LL_ERROR,
              "[ParserXTP] Market data server disconnected: {}...", nReason);
    m_sink->handleEvent(WPE_Close, 0);
  }

  _asyncio.post([this]() {
    write_log(m_sink, LL_WARN,
              "[ParserXTP] Connection lost, relogin in 2 seconds...");
    std::this_thread::sleep_for(std::chrono::seconds(2));
    DoLogin();
  });
}

void ParserXTP::OnUnSubMarketData(XTPST *ticker, XTPRI *error_info,
                                  bool is_last) {}
...
void ParserXTP::OnTickByTick(XTPTBT *tbt_data) {
  ...
  WTSContractInfo *ct =
      m_pBaseDataMgr->getContract(tbt_data->ticker, exchg.c_str());
  if (ct == NULL) {
    if (m_sink)
      write_log(m_sink, LL_ERROR, "[ParserXTP] Instrument {}.{} not exists...",
                exchg.c_str(), tbt_data->ticker);
    return;
  }
...
void ParserXTP::OnDepthMarketData(XTPMD *market_data, int64_t bid1_qty[],
  ...
  WTSContractInfo *ct =
      m_pBaseDataMgr->getContract(code.c_str(), exchg.c_str());
  if (ct == NULL) {
    if (m_sink)
      write_log(m_sink, LL_ERROR, "[ParserXTP] Instrument {}.{} not exists...",
                exchg.c_str(), market_data->ticker);
    return;
  }
  WTSCommodityInfo *commInfo = ct->getCommInfo();
...
void ParserXTP::OnSubMarketData(XTPST *ticker, XTPRI *error_info,
                                bool is_last) {
  if (!IsErrorRspInfo(error_info)) {
    ...
    if (m_sink)
      write_log(m_sink, LL_ERROR,
                "[ParserXTP] Market data subscribe failed, code: {}.{}, err "
                "code: {}, err msg: {}",
                ticker->exchange_id == XTP_EXCHANGE_SH ? "SSE" : "SZSE",
                ticker->ticker, error_info->error_id, error_info->error_msg);
  }
}

void ParserXTP::OnSubTickByTick(XTPST *ticker, XTPRI *error_info,
                                bool is_last) {
  if (!IsErrorRspInfo(error_info)) {
    ...
    if (m_sink)
      write_log(m_sink, LL_ERROR,
                "[ParserXTP] Tick-by-tick data subscribe failed, code: {}.{}, "
                "err code: {}, err msg: {}",
                ticker->exchange_id == XTP_EXCHANGE_SH ? "SSE" : "SZSE",
                ticker->ticker, error_info->error_id, error_info->error_msg);
  }
...
void ParserXTP::DoLogin() {
  ...
  int iResult =
      m_pUserAPI->Login(m_strHost.c_str(), m_iPort, m_strUser.c_str(),
                        m_strPass.c_str(), m_iProtocol, m_strLocalIP.c_str());
  if (iResult != 0) {
    if (m_sink) {
      auto error_info = m_pUserAPI->GetApiLastError();
      if (iResult == -1) {
        _asyncio.post(
            [this, iResult] { m_sink->handleEvent(WPE_Connect, iResult); });

        write_log(m_sink, LL_ERROR, "[ParserXTP] Connecting server failed: {}",
                  error_info->error_msg);
      } else {
        m_sink->handleEvent(WPE_Connect, 0);
        _asyncio.post([this, iResult] { m_sink->handleEvent(WPE_Connect, 0); });

        write_log(m_sink, LL_ERROR,
                  "[ParserXTP] Sending login request failed: {}",
                  error_info->error_msg);
      }
    }
  } else {
    ...
    write_log(m_sink, LL_INFO,
              "[ParserXTP] Connecting server successed: {}, begin to subscibe "
              "data ...",
              iResult);
...
void ParserXTP::DoSubscribeMD() {
  ...
  if (!codeFilter.empty()) {
    ...
    if (m_pUserAPI && nCount > 0) {
      int iResult =
          m_pUserAPI->SubscribeMarketData(subscribe, nCount, XTP_EXCHANGE_SH);
      if (iResult != 0) {
        if (m_sink)
          write_log(
              m_sink, LL_ERROR,
              "[ParserXTP] Sending md subscribe request of SSE failed: {}",
              iResult);
      } else {
        if (m_sink)
          write_log(
              m_sink, LL_INFO,
              "[ParserXTP] Market data of {} instruments of SSE subscribed",
              nCount);
      }

      iResult =
          m_pUserAPI->SubscribeTickByTick(subscribe, nCount, XTP_EXCHANGE_SH);
      if (iResult != 0) {
        if (m_sink)
          write_log(m_sink, LL_ERROR,
                    "[ParserXTP] Sending tick_by_tick subscribe request of SSE "
                    "failed: {}",
                    iResult);
      } else {
        if (m_sink)
          write_log(m_sink, LL_INFO,
                    "[ParserXTP] Tick_by_tick data of {} instruments of SSE "
                    "subscribed",
                    nCount);
      }
    }
  ...
  if (!codeFilter.empty()) {
    ...
    if (m_pUserAPI && nCount > 0) {
      int iResult =
          m_pUserAPI->SubscribeMarketData(subscribe, nCount, XTP_EXCHANGE_SZ);
      if (iResult != 0) {
        if (m_sink)
          write_log(
              m_sink, LL_ERROR,
              "[ParserXTP] Sending md subscribe request of SZSE failed: {}",
              iResult);
      } else {
        if (m_sink)
          write_log(
              m_sink, LL_INFO,
              "[ParserXTP] Market data of {} instruments of SZSE subscribed",
              nCount);
      }

      iResult =
          m_pUserAPI->SubscribeTickByTick(subscribe, nCount, XTP_EXCHANGE_SZ);
      if (iResult != 0) {
        if (m_sink)
          write_log(m_sink, LL_ERROR,
                    "[ParserXTP] Sending tick_by_tick subscribe request of "
                    "SZSE failed: {}",
                    iResult);
      } else {
        if (m_sink)
          write_log(m_sink, LL_INFO,
                    "[ParserXTP] Tick_by_tick data of {} instruments of SZSE "
                    "subscribed",
                    nCount);
      }
    }
...
bool ParserXTP::isConnected() { return m_pUserAPI != NULL; }
