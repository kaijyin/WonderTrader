...
inline int wrapDirectionType(WTSDirectionType dirType,
...
inline WTSDirectionType wrapDirectionType(TThostFtdcDirectionType dirType,
...
inline WTSTimeCondition
wrapTimeCondition(TThostFtdcTimeConditionType timeCond) {
...
TraderCTPOpt::TraderCTPOpt()
    : m_pUserAPI(NULL), m_mapPosition(NULL), m_ayOrders(NULL), m_ayTrades(NULL),
      m_ayPosDetail(NULL), m_wrapperState(WS_NOTLOGIN), m_uLastQryTime(0),
      m_iRequestID(0), m_bQuickStart(false), m_optSink(NULL), m_bscSink(NULL),
      m_bInQuery(false), m_bStopped(NULL), m_lastQryTime(0) {}
...
bool TraderCTPOpt::init(WTSVariant *params) {
  ...
  const char *creatorName =
      "?CreateFtdcTraderApi@CThostFtdcTraderApi@ctp_sopt@@SAPEAV12@PEBD@Z";
#else
  const char *creatorName =
      "?CreateFtdcTraderApi@CThostFtdcTraderApi@ctp_sopt@@SAPAV12@PBD@Z";
#endif
#else
  const char *creatorName =
      "_ZN8ctp_sopt19CThostFtdcTraderApi19CreateFtdcTraderApiEPKc";
#endif
...
bool TraderCTPOpt::makeEntrustID(char *buffer, int length) {
  ...
  try {
    ...
    fmtutil::format_to(buffer, "{:06d}#{:010d}#{:06d}", m_frontID,
...
uint32_t TraderCTPOpt::genRequestID() { return m_iRequestID.fetch_add(1) + 1; }

int TraderCTPOpt::login(const char *user, const char *pass,
...
int TraderCTPOpt::doLogin() {
  ...
  if (iResult != 0) {
    write_log(m_bscSink, LL_ERROR,
              "[TraderCTPOpt] Sending login request failed: {}", iResult);
  }
...
int TraderCTPOpt::logout() {
  ...
  if (iResult != 0) {
    write_log(m_bscSink, LL_ERROR,
              "[TraderCTPOpt] Sending logout request failed: {}", iResult);
  }
...
int TraderCTPOpt::orderInsertOpt(WTSEntrust *entrust) {
  ...
  if (entrust->getBusinessType() != BT_EXECUTE) {
    if (m_bscSink)
      write_log(m_bscSink, LL_ERROR,
                "[TraderCTPOpt] Bz type not supported except option execution");
    return -1;
  }
  ...
  if (iResult != 0) {
    write_log(m_bscSink, LL_ERROR,
              "[TraderCTPOpt] Option execution order inserting failed: {}",
              iResult);
  }
...
int TraderCTPOpt::orderActionOpt(WTSEntrustAction *action) {
  ...
  if (iResult != 0) {
    write_log(m_bscSink, LL_ERROR,
              "[TraderCTPOpt] Sending cancel request of option execution order "
              "failed: {}",
              iResult);
  }
...
int TraderCTPOpt::orderInsert(WTSEntrust *entrust) {
  ...
  if (iResult != 0) {
    write_log(m_bscSink, LL_ERROR, "[TraderCTPOpt] Order inserting failed: {}",
              iResult);
  }
...
int TraderCTPOpt::orderAction(WTSEntrustAction *action) {
  ...
  if (iResult != 0) {
    write_log(m_bscSink, LL_ERROR,
              "[TraderCTPOpt] Sending cancel request failed: {}", iResult);
  }
...
void TraderCTPOpt::OnHeartBeatWarning(int nTimeLapse) {
  write_log(m_bscSink, LL_DEBUG, "[TraderCTPOpt][{}-{}] Heartbeating...",
            m_strBroker.c_str(), m_strUser.c_str());
}

void TraderCTPOpt::OnRspAuthenticate(
  ...
  if (!IsErrorRspInfo(pRspInfo)) {
    ...
    write_log(m_bscSink, LL_ERROR,
              "[TraderCTPOpt][{}-{}] Authentiation failed: {}",
...
void TraderCTPOpt::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,
  ...
  if (!IsErrorRspInfo(pRspInfo)) {
    ...
    write_log(m_bscSink, LL_INFO,
              "[TraderCTPOpt][{}-{}] Login succeed, AppID: {}, Sessionid: {}, "
              "login time: {}...",
    ...
    if (lastDate != m_lDate) {
      ...
      write_log(m_bscSink, LL_INFO,
                "[TraderCTPOpt][{}-{}] Trading date changed [{} -> {}], local "
                "cache cleared...",
                m_strBroker.c_str(), m_strUser.c_str(), lastDate, m_lDate);
    }

    write_log(m_bscSink, LL_INFO,
              "[TraderCTPOpt][{}-{}] Login succeed, trading date: {}...",
              m_strBroker.c_str(), m_strUser.c_str(), m_lDate);

    write_log(
        m_bscSink, LL_INFO,
        "[TraderCTPOpt][{}-{}] Querying confirming state of settlement data...",
        m_strBroker.c_str(), m_strUser.c_str());
    queryConfirm();
  } else {
    write_log(m_bscSink, LL_ERROR, "[TraderCTPOpt][{}-{}] Login failed: {}",
              m_strBroker.c_str(), m_strUser.c_str(), pRspInfo->ErrorMsg);
    m_wrapperState = WS_LOGINFAILED;
...
void TraderCTPOpt::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout,
                                   CThostFtdcRspInfoField *pRspInfo,
                                   int nRequestID, bool bIsLast) {
  m_wrapperState = WS_NOTLOGIN;
  if (m_bscSink)
...
void TraderCTPOpt::OnRspQrySettlementInfoConfirm(
  ...
  if (!IsErrorRspInfo(pRspInfo)) {
    if (pSettlementInfoConfirm != NULL) {
      uint32_t uConfirmDate =
          strtoul(pSettlementInfoConfirm->ConfirmDate, NULL, 10);
      if (uConfirmDate >= m_lDate) {
        m_wrapperState = WS_CONFIRMED;

        write_log(m_bscSink, LL_INFO,
                  "[TraderCTPOpt][{}-{}] Trading channel initialized...",
                  m_strBroker.c_str(), m_strUser.c_str());
        ...
        write_log(m_bscSink, LL_INFO,
                  "[TraderCTPOpt][{}-{}] Confirming settlement data...",
                  m_strBroker.c_str(), m_strUser.c_str());
        confirm();
      }
    } else {
...
void TraderCTPOpt::OnRspSettlementInfoConfirm(
    CThostFtdcSettlementInfoConfirmField *pSettlementInfoConfirm,
    CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
...
void TraderCTPOpt::OnRspOrderInsert(CThostFtdcInputOrderField *pInputOrder,
                                    CThostFtdcRspInfoField *pRspInfo,
                                    int nRequestID, bool bIsLast) {
...
void TraderCTPOpt::OnRspExecOrderInsert(
...
void TraderCTPOpt::OnRspExecOrderAction(
...
void TraderCTPOpt::OnErrRtnExecOrderInsert(
    CThostFtdcInputExecOrderField *pInputExecOrder,
    CThostFtdcRspInfoField *pRspInfo) {
  WTSEntrust *entrust = makeEntrust(pInputExecOrder);
  if (entrust) {
...
void TraderCTPOpt::OnRspQryExecOrder(CThostFtdcExecOrderField *pExecOrder,
                                     CThostFtdcRspInfoField *pRspInfo,
                                     int nRequestID, bool bIsLast) {
  if (bIsLast) {
    m_bInQuery = false;
...
void TraderCTPOpt::OnRspOrderAction(
...
void TraderCTPOpt::OnRspQryTradingAccount(
...
void TraderCTPOpt::OnRspQrySettlementInfo(
  ...
  if (bIsLast && !m_strSettleInfo.empty()) {
    m_bscSink->onRspSettlementInfo(atoi(pSettlementInfo->TradingDay),
                                   m_strSettleInfo.c_str());
  }
}

void TraderCTPOpt::OnRspQryInvestorPosition(
  ...
  if (!IsErrorRspInfo(pRspInfo) && pInvestorPosition) {
    ...
    WTSContractInfo *contract = m_bdMgr->getContract(
    ...
    if (contract) {
      std::string key = fmt::format("{}-{}", pInvestorPosition->InstrumentID,
      ...
      if (pos->getTotalPosition() != 0) {
        pos->setAvgPrice(pos->getPositionCost() / pos->getTotalPosition() /
...
void TraderCTPOpt::OnRspQryTrade(CThostFtdcTradeField *pTrade,
                                 CThostFtdcRspInfoField *pRspInfo,
                                 int nRequestID, bool bIsLast) {
  if (bIsLast) {
    m_bInQuery = false;
...
void TraderCTPOpt::OnRspQryOrder(CThostFtdcOrderField *pOrder,
                                 CThostFtdcRspInfoField *pRspInfo,
                                 int nRequestID, bool bIsLast) {
  if (bIsLast) {
    m_bInQuery = false;
...
void TraderCTPOpt::OnRspError(CThostFtdcRspInfoField *pRspInfo, int nRequestID,
                              bool bIsLast) {
  int x = 0;
}
...
WTSOrderInfo *TraderCTPOpt::makeOrderInfo(CThostFtdcOrderField *orderField) {
  WTSContractInfo *contract =
      m_bdMgr->getContract(orderField->InstrumentID, orderField->ExchangeID);
  if (contract == NULL)
    return NULL;
  ...
  pRet->setDirection(
      wrapDirectionType(orderField->Direction, orderField->CombOffsetFlag[0]));
  ...
  // pRet->setEntrustID(generateEntrustID(orderField->FrontID,
  // orderField->SessionID, atoi(orderField->OrderRef)).c_str());
  ...
  std::string usertag =
      m_iniHelper.readString(ENTRUST_SECTION, pRet->getEntrustID(), "");
  if (usertag.empty()) {
...
WTSOrderInfo *
TraderCTPOpt::makeOrderInfo(CThostFtdcExecOrderField *orderField) {
  WTSContractInfo *contract =
      m_bdMgr->getContract(orderField->InstrumentID, orderField->ExchangeID);
  if (contract == NULL)
    return NULL;
  ...
  // pRet->setEntrustID(generateEntrustID(orderField->FrontID,
  // orderField->SessionID, atoi(orderField->ExecOrderRef)).c_str());
  generateEntrustID(pRet->getEntrustID(), orderField->FrontID,
                    orderField->SessionID, atoi(orderField->ExecOrderRef));
...
WTSEntrust *
TraderCTPOpt::makeEntrust(CThostFtdcInputExecOrderField *entrustField) {
  WTSContractInfo *ct = m_bdMgr->getContract(entrustField->InstrumentID,
                                             entrustField->ExchangeID);
  if (ct == NULL)
    return NULL;
  ...
  WTSEntrust *pRet =
      WTSEntrust::create(entrustField->InstrumentID, entrustField->Volume, 0,
                         ct->getExchg(), BT_EXECUTE);
  pRet->setContractInfo(ct);
  ...
  // pRet->setEntrustID(generateEntrustID(m_frontID, m_sessionID,
  // atoi(entrustField->ExecOrderRef)).c_str());
  generateEntrustID(pRet->getEntrustID(), m_frontID, m_sessionID,
                    atoi(entrustField->ExecOrderRef));
...
WTSTradeInfo *TraderCTPOpt::makeTradeRecord(CThostFtdcTradeField *tradeField) {
  ...
  //	if (uTime / 10000 >= 21)
  //	{
  //		uDate = m_pMarketMgr->getPrevTDate(commInfo->getExchg(), uDate,
  //1);
  //	}
  //	else if(uTime <= 3)
  //	{
  ...
  //		uDate = m_pMarketMgr->getPrevTDate(commInfo->getExchg(), uDate,
  //1); 		uDate = TimeUtils::getNextDate(uDate);
  //	}
  // }
  ...
  std::string usertag = m_iniHelper.readString(
      ORDER_SECTION, StrUtil::trim(pRet->getRefOrder()).c_str());
  if (!usertag.empty())
    pRet->setUserTag(usertag.c_str());
...
void TraderCTPOpt::generateEntrustID(char *buffer, uint32_t frontid,
                                     uint32_t sessionid, uint32_t orderRef) {
  fmtutil::format_to(buffer, "{:06d}#{:010d}#{:06d}", frontid, sessionid,
                     orderRef);
}

bool TraderCTPOpt::extractEntrustID(const char *entrustid, uint32_t &frontid,
                                    uint32_t &sessionid, uint32_t &orderRef) {
...
void TraderCTPOpt::OnErrRtnOrderInsert(CThostFtdcInputOrderField *pInputOrder,
                                       CThostFtdcRspInfoField *pRspInfo) {
  WTSEntrust *entrust = makeEntrust(pInputOrder);
  if (entrust) {
    WTSError *err = makeError(pRspInfo);
...
int TraderCTPOpt::queryConfirm() {
  ...
  m_queQuery.push([this]() {
    ...
    if (iResult != 0) {
      write_log(m_bscSink, LL_ERROR,
                "[TraderCTPOpt][{}-{}] Sending query of settlement data "
                "confirming state failed: {}",
...
int TraderCTPOpt::confirm() {
  ...
  if (iResult != 0) {
    write_log(m_bscSink, LL_ERROR,
              "[TraderCTPOpt][{}-{}] Sending confirming of settlement data "
              "failed: {}",
              m_strBroker.c_str(), m_strUser.c_str(), iResult);
    return -1;
  }
