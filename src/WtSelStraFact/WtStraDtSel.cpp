...
WtStraDtSel::WtStraDtSel(const char *id) : SelStrategy(id) {}

WtStraDtSel::~WtStraDtSel() {}

const char *WtStraDtSel::getName() { return "DualThrustSelection"; }

const char *WtStraDtSel::getFactName() { return FACT_NAME; }
...
void WtStraDtSel::on_schedule(ISelStraCtx *ctx, uint32_t uDate,
                              uint32_t uTime) {
  for (auto &curCode : _codes) {
    ...
    if (_isstk)
      code += "-";
    WTSKlineSlice *kline =
        ctx->stra_get_bars(code.c_str(), _period.c_str(), _count);
    if (kline == NULL) {
      // 这里可以输出一些日志
      return;
    }
    ...
    if (decimal::eq(curPos, 0)) {
      if (highPx >= upper_bound) {
        ...
        // 向上突破
        ctx->stra_log_info(fmt::format("{} 向上突破{}>={},多仓进场",
                                       curCode.c_str(), highPx, upper_bound)
                               .c_str());
      } else if (lowPx <= lower_bound && !_isstk) {
        ctx->stra_set_position(curCode.c_str(), -1 * trdUnit, "DT_EnterShort");
        // 向下突破
        ctx->stra_log_info(fmt::format("{} 向下突破{}<={},空仓进场",
                                       curCode.c_str(), lowPx, lower_bound)
                               .c_str());
      }
    }
    // else if(curPos > 0)
      ...
      if (lowPx <= lower_bound) {
        ...
        ctx->stra_log_info(fmt::format("{} 向下突破{}<={},多仓出场",
                                       curCode.c_str(), lowPx, lower_bound)
                               .c_str());
      }
    }
    // else if(curPos < 0)
      ...
      if (highPx >= upper_bound && !_isstk) {
        ...
        ctx->stra_log_info(fmt::format("{} 向上突破{}>={},空仓出场",
                                       curCode.c_str(), highPx, upper_bound)
                               .c_str());
      }
    }
...
void WtStraDtSel::on_tick(ISelStraCtx *ctx, const char *stdCode,
                          WTSTickData *newTick) {}

void WtStraDtSel::on_bar(ISelStraCtx *ctx, const char *stdCode,
