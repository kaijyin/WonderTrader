...
bool UftStrategyMgr::loadFactories(const char *path) {
  ...
  for (boost::filesystem::directory_iterator iter(myPath); iter != endIter;
    ...
    FuncCreateUftStraFact creator =
        (FuncCreateUftStraFact)DLLHelper::get_symbol(hInst,
                                                     "createStrategyFact");
    if (creator == NULL) {
    ...
    if (pFact != NULL) {
      ...
      fInfo._remover = (FuncDeleteUftStraFact)DLLHelper::get_symbol(
  ...
  WTSLogger::info("{} UFT strategy factories in directory[{}] loaded", count,
                  path);

  return true;
}

UftStrategyPtr UftStrategyMgr::createStrategy(const char *factname,
                                              const char *unitname,
                                              const char *id) {
  ...
  UftStrategyPtr ret(new UftStraWrapper(
      fInfo._fact->createStrategy(unitname, id), fInfo._fact));
  _strategies[id] = ret;
  return ret;
}

UftStrategyPtr UftStrategyMgr::createStrategy(const char *name,
                                              const char *id) {
