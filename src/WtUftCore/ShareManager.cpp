...
bool ShareManager::initialize(const char *module) {
  ...
  _get_section_updatetime = (func_get_section_updatetime)DLLHelper::get_symbol(
      _inst, "get_section_updatetime");
  _commit_section =
      (func_commit_section)DLLHelper::get_symbol(_inst, "commit_section");
  ...
  _allocate_double =
      (func_allocate_double)DLLHelper::get_symbol(_inst, "allocate_double");
  _allocate_int32 =
      (func_allocate_int32)DLLHelper::get_symbol(_inst, "allocate_int32");
  _allocate_int64 =
      (func_allocate_int64)DLLHelper::get_symbol(_inst, "allocate_int64");
  _allocate_uint32 =
      (func_allocate_uint32)DLLHelper::get_symbol(_inst, "allocate_uint32");
  _allocate_uint64 =
      (func_allocate_uint64)DLLHelper::get_symbol(_inst, "allocate_uint64");
  _allocate_string =
      (func_allocate_string)DLLHelper::get_symbol(_inst, "allocate_string");
    ...
    _worker.reset(new StdThread([this, microsecs]() {
      while (!_stopped) {
        for (auto &v : _secnames) {
          ...
          uint64_t lastUdtTime =
              _get_section_updatetime(_exchg.c_str(), section);
...
bool ShareManager::init_domain(const char *id) {
  ...
  _exchg = id;
  WTSLogger::info("Share domain [{}] initialing {}", id,
                  ret ? "succeed" : "failed");

  // 初始化同步区
  ret = _init_master("sync", ".sync");
  WTSLogger::info("Sync domain [sync] initialing {}",
                  ret ? "succeed" : "failed");
...
bool ShareManager::set_value(const char *section, const char *key,
                             uint64_t val) {
  if (!_inited)
    return false;
...
const char *ShareManager::get_value(const char *section, const char *key,
                                    const char *defVal /* = "" */) {
  if (!_inited)
    return defVal;
...
int32_t ShareManager::get_value(const char *section, const char *key,
                                int32_t defVal /* = 0 */) {
  if (!_inited)
    return defVal;
...
int64_t ShareManager::get_value(const char *section, const char *key,
                                int64_t defVal /* = 0 */) {
  if (!_inited)
    return defVal;
...
uint32_t ShareManager::get_value(const char *section, const char *key,
                                 uint32_t defVal /* = 0 */) {
  if (!_inited)
    return defVal;
...
uint64_t ShareManager::get_value(const char *section, const char *key,
                                 uint64_t defVal /* = 0 */) {
  if (!_inited)
    return defVal;
...
double ShareManager::get_value(const char *section, const char *key,
                               double defVal /* = 0 */) {
  if (!_inited)
    return defVal;
...
const char *ShareManager::allocate_value(const char *section, const char *key,
                                         const char *initVal /* = ""*/,
  ...
  return _allocate_string(isExchg ? _exchg.c_str() : _sync.c_str(), section,
                          key, initVal, bForceWrite);
}

int32_t *ShareManager::allocate_value(const char *section, const char *key,
                                      int32_t initVal /* = 0*/,
                                      bool bForceWrite /* = false*/,
                                      bool isExchg /* = false*/) {
  if (!_inited)
  ...
  return _allocate_int32(isExchg ? _exchg.c_str() : _sync.c_str(), section, key,
                         initVal, bForceWrite);
}

int64_t *ShareManager::allocate_value(const char *section, const char *key,
                                      int64_t initVal /* = 0*/,
                                      bool bForceWrite /* = false*/,
                                      bool isExchg /* = false*/) {
  if (!_inited)
  ...
  return _allocate_int64(isExchg ? _exchg.c_str() : _sync.c_str(), section, key,
                         initVal, bForceWrite);
}

uint32_t *ShareManager::allocate_value(const char *section, const char *key,
                                       uint32_t initVal /* = 0*/,
                                       bool bForceWrite /* = false*/,
                                       bool isExchg /* = false*/) {
  if (!_inited)
  ...
  return _allocate_uint32(isExchg ? _exchg.c_str() : _sync.c_str(), section,
                          key, initVal, bForceWrite);
}

uint64_t *ShareManager::allocate_value(const char *section, const char *key,
                                       uint64_t initVal /* = 0*/,
                                       bool bForceWrite /* = false*/,
                                       bool isExchg /* = false*/) {
  if (!_inited)
  ...
  return _allocate_uint64(isExchg ? _exchg.c_str() : _sync.c_str(), section,
                          key, initVal, bForceWrite);
}

double *ShareManager::allocate_value(const char *section, const char *key,
                                     double initVal /* = 0*/,
                                     bool bForceWrite /* = false*/,
                                     bool isExchg /* = false*/) {
  if (!_inited)
  ...
  return _allocate_double(isExchg ? _exchg.c_str() : _sync.c_str(), section,
                          key, initVal, bForceWrite);
}