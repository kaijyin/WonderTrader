...
typedef const char *(*func_allocate_string)(const char *, const char *,
                                            const char *, const char *, bool);
typedef int32_t *(*func_allocate_int32)(const char *, const char *,
                                        const char *, int32_t, bool);
typedef int64_t *(*func_allocate_int64)(const char *, const char *,
                                        const char *, int64_t, bool);
typedef uint32_t *(*func_allocate_uint32)(const char *, const char *,
                                          const char *, uint32_t, bool);
typedef uint64_t *(*func_allocate_uint64)(const char *, const char *,
                                          const char *, uint64_t, bool);
typedef double *(*func_allocate_double)(const char *, const char *,
                                        const char *, double, bool);
typedef bool (*func_set_string)(const char *, const char *, const char *,
                                const char *);
typedef bool (*func_set_int32)(const char *, const char *, const char *,
                               int32_t);
typedef bool (*func_set_int64)(const char *, const char *, const char *,
                               int64_t);
typedef bool (*func_set_uint32)(const char *, const char *, const char *,
                                uint32_t);
typedef bool (*func_set_uint64)(const char *, const char *, const char *,
                                uint64_t);
typedef bool (*func_set_double)(const char *, const char *, const char *,
                                double);
typedef const char *(*func_get_string)(const char *, const char *, const char *,
                                       const char *);
typedef int32_t (*func_get_int32)(const char *, const char *, const char *,
                                  int32_t);
typedef int64_t (*func_get_int64)(const char *, const char *, const char *,
                                  int64_t);
typedef uint32_t (*func_get_uint32)(const char *, const char *, const char *,
                                    uint32_t);
typedef uint64_t (*func_get_uint64)(const char *, const char *, const char *,
                                    uint64_t);
typedef double (*func_get_double)(const char *, const char *, const char *,
                                  double);

class ShareManager {
private:
  ShareManager()
      : _inited(false), _stopped(false), _engine(nullptr), _sync("sync") {}
  ~ShareManager() {
    _stopped = true;
    if (_worker)
      _worker->join();
...
public:
   ...
   *	在单向同步区分配字段
   */
  const char *allocate_value(const char *section, const char *key,
                             const char *initVal = "", bool bForceWrite = false,
                             bool isExchg = false);
  int32_t *allocate_value(const char *section, const char *key,
                          int32_t initVal = 0, bool bForceWrite = false,
                          bool isExchg = false);
  int64_t *allocate_value(const char *section, const char *key,
                          int64_t initVal = 0, bool bForceWrite = false,
                          bool isExchg = false);
  uint32_t *allocate_value(const char *section, const char *key,
                           uint32_t initVal = 0, bool bForceWrite = false,
                           bool isExchg = false);
  uint64_t *allocate_value(const char *section, const char *key,
                           uint64_t initVal = 0, bool bForceWrite = false,
                           bool isExchg = false);
  double *allocate_value(const char *section, const char *key,
                         double initVal = 0, bool bForceWrite = false,
                         bool isExchg = false);
