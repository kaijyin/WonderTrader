...
TraderAdapter::TraderAdapter()
    : _id(""), _cfg(NULL), _state(AS_NOTLOGIN), _trader_api(NULL),
      _orders(NULL), _risk_mon_enabled(false), _stat_map(NULL) {}
...
bool TraderAdapter::init(const char *id, WTSVariant *params,
  ...
  if (hInst == NULL) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Loading trading module %s failed", _id.c_str(),
                       dllpath.c_str());
    return false;
  } else {
...
uint32_t TraderAdapter::doEntrust(WTSEntrust *entrust) {
  ...
  //entrust->getExchg()); 	entrust->setContractInfo(cInfo);
  // }
...
OrderIDs TraderAdapter::buy(const char *stdCode, double price, double qty,
  ...
  //{
  //	WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN, "{} is forbidden to
  //trade", stdCode); 	return ret;
  // }
    ...
    if (curRule._atype == AT_Open && !bForceClose) {
        ...
        for (;;) {
          ...
          uint32_t localid =
              closeShort(stdCode, price, curQty,
                         (commInfo->getCoverMode() == CM_CoverToday),
                         flag); // 如果不支持平今, 则直接下平仓标记即可
          ret.emplace_back(localid);
      ...
      if (decimal::gt(maxQty, 0)) {
        ...
        for (;;) {
          double curQty = min(leftQty, unitQty);
          uint32_t localid =
              closeShort(stdCode, price, curQty, false,
                         flag); // 如果不支持平今, 则直接下平仓标记即可
          ret.emplace_back(localid);
    ...
    if (curRule._atype == AT_Open && !bForceClose) {
      ...
      if (decimal::gt(maxQty, 0)) {
        ...
        for (;;) {
          double curQty = min(leftQty, unitQty);
          uint32_t localid =
              closeLong(stdCode, price, curQty, false,
                        flag); // 如果不支持平今, 则直接下平仓标记即可
          ret.emplace_back(localid);
...
void TraderAdapter::onRspEntrust(WTSEntrust *entrust, WTSError *err) {
  if (err && err->getErrorCode() != WEC_NONE) {
    ...
    WTSContractInfo *cInfo =
        _bd_mgr->getContract(entrust->getCode(), entrust->getExchg());
    if (cInfo == NULL)
      return;
...
void TraderAdapter::onRspPosition(const WTSArray *ayPositions) {
  if (ayPositions && ayPositions->size() > 0) {
    for (auto it = ayPositions->begin(); it != ayPositions->end(); it++) {
      ...
      WTSContractInfo *cInfo =
          _bd_mgr->getContract(pItem->getCode(), pItem->getExchg());
      if (cInfo == NULL)
        continue;
...
void TraderAdapter::onRspOrders(const WTSArray *ayOrders) {
  if (ayOrders) {
    ...
    for (auto it = ayOrders->begin(); it != ayOrders->end(); it++) {
      ...
      WTSContractInfo *cInfo =
          _bd_mgr->getContract(orderInfo->getCode(), orderInfo->getExchg());
      if (cInfo == NULL)
        continue;
...
void TraderAdapter::onRspTrades(const WTSArray *ayTrades) {
  if (ayTrades) {
    for (auto it = ayTrades->begin(); it != ayTrades->end(); it++) {
      ...
      WTSTradeStateInfo *statInfo =
          (WTSTradeStateInfo *)_stat_map->get(stdCode);
      if (statInfo == NULL) {
        statInfo = WTSTradeStateInfo::create(stdCode);
        _stat_map->add(stdCode, statInfo, false);
      }
...
inline const char *stateToName(WTSOrderState woState) {
  if (woState == WOS_AllTraded)
    ...
    return "PrtTrd";
  else if (woState == WOS_NotTraded_NotQueuing ||
           woState == WOS_NotTraded_Queuing)
    return "UnTrd";
...
void TraderAdapter::onPushOrder(WTSOrderInfo *orderInfo) {
  ...
  if (orderInfo->getOrderState() == WOS_Canceled &&
      StrUtil::startsWith(orderInfo->getUserTag(), _order_pattern.c_str(),
                          true)) {
    // 撤单的时候, 要更新未完成
    ...
    // WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO, "[{}] {} qty of undone
    // order updated, {} -> {}", _id.c_str(), stdCode.c_str(), oldQty, newQty);
    updateUndone(stdCode.c_str(), -qty);

    WTSLogger::log_dyn(
        "trader", _id.c_str(), LL_DEBUG,
        "[{}] Order {} of {} canceled:{}, action: {}, leftqty: {}", _id.c_str(),
        orderInfo->getUserTag(), stdCode.c_str(), orderInfo->getStateMsg(),
  ...
  if (localid != 0) {
    ...
    for (auto sink : _sinks)
      sink->on_order(
          localid, stdCode.c_str(), orderInfo->getDirection() == WDT_LONG,
          offset, orderInfo->getVolume(), orderInfo->getVolLeft(),
          orderInfo->getPrice(), orderInfo->getOrderState() == WOS_Canceled);
  }
}
...
void TraderAdapter::onPushTrade(WTSTradeInfo *tradeRecord) {
  ...
  if (StrUtil::startsWith(tradeRecord->getUserTag(), _order_pattern.c_str(),
    ...
    //_undone_qty[stdCode] = newQty;
    // WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
    //	"[{}] {} qty of undone orders updated, {} -> {}", _id.c_str(),
    //stdCode.c_str(), oldQty, newQty);
  ...
  if (tradeRecord->getOffsetType() == WOT_OPEN)
    ...
    offset = 2;
  for (auto sink : _sinks)
    sink->on_trade(localid, stdCode.c_str(), isLong, offset, vol,
...
TraderAdapter::getRiskParams(const char *stdCode) {
  ...
  auto it =
      _risk_params_map.find(std::string(stdCode + idx + 1, eIdx - idx + 1));
  if (it != _risk_params_map.end())
    return &it->second;
