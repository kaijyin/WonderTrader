...
void UftStraContext::setTrader(TraderAdapter *trader) { _trader = trader; }
  ...
  if (it != _positions.end()) {
    ...
    for (auto i = pInfo._valid_idx; i < pInfo._details.size(); i++) {
      ...
      if (ds->_volume == 0)
        ...
        ds->_position_profit = (newTick->price() - ds->_open_price) *
                               ds->_volume * volscale *
                               (ds->_direct == 0 ? 1 : -1);
    }
    ...
    if (decimal::gt(pInfo._volume, 0.0))
      pInfo._dynprofit =
          newTick->price() * pInfo._volume * volscale - pInfo._opencost;
...
void UftStraContext::on_order_queue(const char *stdCode,
                                    WTSOrdQueData *newOrdQue) {
  if (_strategy)
    _strategy->on_order_queue(this, stdCode, newOrdQue);
}

void UftStraContext::on_order_detail(const char *stdCode,
                                     WTSOrdDtlData *newOrdDtl) {
  if (_strategy)
    _strategy->on_order_detail(this, stdCode, newOrdDtl);
}

void UftStraContext::on_transaction(const char *stdCode,
                                    WTSTransData *newTrans) {
  if (_strategy)
    _strategy->on_transaction(this, stdCode, newTrans);
}

void UftStraContext::on_bar(const char *code, const char *period,
                            uint32_t times, WTSBarStruct *newBar) {
  if (_strategy)
    _strategy->on_bar(this, code, period, times, newBar);
}

void UftStraContext::on_trade(uint32_t localid, const char *stdCode,
                              bool isLong, uint32_t offset, double vol,
                              double price) {
  ...
  if (isBuy) {
    ...
    if (decimal::lt(pItem._volume, 0)) {
      ...
      double left = thisQty;
      for (uint32_t idx = pItem._valid_idx; idx < pItem._details.size();
           idx++) {
  ...
  if (isLong)
  {
          if (isOpen)
          {
...

                  {
                          ...
                          _pos_blk._block->_size++;
                          uft::DetailStruct& ds =
  _pos_blk._block->_details[idx]; wt_strcpy(ds._code, cInfo->getCode());
                          wt_strcpy(ds._exchg, cInfo->getExchg());
                          ds._direct = 0;
                          ds._open_price = price;
                          ds._open_time = now;
          ...
          else
          {
                  ...
                  double left = vol;
                  for(uint32_t idx = pItem._valid_idx; idx <
  pItem._details.size(); idx++)
                  {
                          uft::DetailStruct* pDS = pItem._details[idx];
                          //只有索引递增，才递进，不递增就不递进
                          if(decimal::eq(pDS->_volume, 0.0) && (idx ==
  pItem._valid_idx+1))
                          {
                                  pItem._valid_idx++;
                                  continue;
                          ...
                          double maxQty = std::min(left, pDS->_volume);
                          {
                                  ...
                                  _rnd_blk._block->_size++;
                                  uft::RoundStruct& rs =
  _rnd_blk._block->_rounds[ridx]; wt_strcpy(rs._code, cInfo->getCode());
                                  wt_strcpy(rs._exchg, cInfo->getExchg());
                                  ...
                                  rs._direct = 0;
                                  rs._volume = maxQty;
                                  rs._profit = (rs._close_price -
  rs._open_price)*maxQty*cInfo->getCommInfo()->getVolScale(); pItem.total_profit
  += rs._profit;
                          }
...
void UftStraContext::on_order(uint32_t localid, const char *stdCode,
                              bool isLong, uint32_t offset, double totalQty,
                              double leftQty, double price,
  ...
  if (_strategy) {
    for (const auto &v : _positions) {
      ...
      if (decimal::gt(pInfo._volume, 0)) {
        _strategy->on_position(this, stdCode, true, pInfo._volume,
                               pInfo._volume, 0, 0);
      } else if (decimal::lt(pInfo._volume, 0)) {
        _strategy->on_position(this, stdCode, false, pInfo._volume,
                               pInfo._volume, 0, 0);
      }
    }
...
void UftStraContext::on_entrust(uint32_t localid, const char *stdCode,
                                bool bSuccess, const char *message) {
  if (!is_my_order(localid))
    return;
...
void UftStraContext::on_position(const char *stdCode, bool isLong,
                                 double prevol, double preavail, double newvol,
                                 double newavail, uint32_t tradingday) {
  // 账户的持仓通知不给策略了
  // if (_strategy)
  //	_strategy->on_position(this, stdCode, isLong, prevol, preavail, newvol,
  //newavail);
}
...
const char *UftStraContext::watch_param(const char *name, const char *val) {
  return ShareManager::self().allocate_value(_name.c_str(), name, val, false,
                                             true);
}
...
const char *UftStraContext::read_param(const char *name,
                                       const char *defVal /* = "" */) {
...
uint32_t UftStraContext::read_param(const char *name,
                                    uint32_t defVal /* = 0 */) {
  return ShareManager::self().get_value(_name.c_str(), name, defVal);
}
...
uint64_t UftStraContext::read_param(const char *name,
                                    uint64_t defVal /* = 0 */) {
  return ShareManager::self().get_value(_name.c_str(), name, defVal);
}
...
uint32_t *UftStraContext::sync_param(const char *name,
                                     uint32_t initVal /* = 0 */,
                                     bool bForceWrite /* = false*/) {
  return ShareManager::self().allocate_value(_name.c_str(), name, initVal,
                                             bForceWrite, false);
...
uint64_t *UftStraContext::sync_param(const char *name,
                                     uint64_t initVal /* = 0 */,
                                     bool bForceWrite /* = false*/) {
  return ShareManager::self().allocate_value(_name.c_str(), name, initVal,
                                             bForceWrite, false);
...
const char *UftStraContext::sync_param(const char *name,
                                       const char *initVal /* = "" */,
                                       bool bForceWrite /* = false*/) {
  return ShareManager::self().allocate_value(_name.c_str(), name, initVal,
                                             bForceWrite);
...
double UftStraContext::stra_get_position(const char *stdCode,
...
uint32_t UftStraContext::stra_get_date() { return _engine->get_date(); }

uint32_t UftStraContext::stra_get_time() { return _engine->get_raw_time(); }

uint32_t UftStraContext::stra_get_secs() { return _engine->get_secs(); }
...
OrderIDs UftStraContext::stra_buy(const char *stdCode, double price, double qty,
                                  int flag /* = 0 */) {
...
OrderIDs UftStraContext::stra_sell(const char *stdCode, double price,
...
uint32_t UftStraContext::stra_enter_long(const char *stdCode, double price,
                                         double qty, int flag /* = 0 */) {
...
uint32_t UftStraContext::stra_exit_long(const char *stdCode, double price,
...
uint32_t UftStraContext::stra_enter_short(const char *stdCode, double price,
                                          double qty, int flag /* = 0 */) {
...
uint32_t UftStraContext::stra_exit_short(const char *stdCode, double price,
                                         double qty, bool isToday /* = false */,
                                         int flag /* = 0 */) {
...
WTSKlineSlice *UftStraContext::stra_get_bars(const char *stdCode,
                                             const char *period,
                                             uint32_t count) {
...
WTSTickSlice *UftStraContext::stra_get_ticks(const char *stdCode,
                                             uint32_t count) {
...
WTSOrdDtlSlice *UftStraContext::stra_get_order_detail(const char *stdCode,
                                                      uint32_t count) {
  WTSOrdDtlSlice *ret =
      _engine->get_order_detail_slice(_context_id, stdCode, count);
...
WTSOrdQueSlice *UftStraContext::stra_get_order_queue(const char *stdCode,
                                                     uint32_t count) {
  WTSOrdQueSlice *ret =
      _engine->get_order_queue_slice(_context_id, stdCode, count);
...
WTSTransSlice *UftStraContext::stra_get_transaction(const char *stdCode,
                                                    uint32_t count) {
  WTSTransSlice *ret =
      _engine->get_transaction_slice(_context_id, stdCode, count);
...
void UftStraContext::load_local_data() {
  ...
  std::string folder =
      fmtutil::format("{}{}/", WtHelper::getOutputDir(), _name);
  ...
  do {
    ...
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_WARN,
                       "{} detected, positions will be overwrited",
                       mannualfile);

    WTSVariant *manual = WTSCfgLoader::load_from_file(mannualfile);
    if (manual == NULL) {
      WTSLogger::log_dyn("strategy", _name.c_str(), LL_ERROR,
                         "parsing mannual file {} failed", mannualfile);
      break;
    }
    ...
    // 强制新建
    {
      std::size_t uSize = sizeof(uft::PositionBlock) +
                          sizeof(uft::DetailStruct) * DATA_SIZE_STEP;
      BoostFile bf;
    ...
    if (_pos_blk._file->map(filename.c_str())) {
      ...
      for (uint32_t i = 0; i < ayDetails->size(); i++) {
        ...
        WTSContractInfo *cInfo =
            _engine->get_basedata_mgr()->getContract(code, exchg);
        if (cInfo == NULL) {
          WTSLogger::log_dyn("strategy", _name.c_str(), LL_ERROR,
                             "{}.{} not exist, skip this details", exchg, code);
          continue;
        }
    ...
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_WARN,
                       "loading mannual file {} done, {} details imported",
                       mannualfile, _pos_blk._block->_size);
  ...
  if (StdFile::exists(mannualfile.c_str()))
    boost::filesystem::rename(
        boost::filesystem::path(mannualfile),
        boost::filesystem::path(fmtutil::format(
            "{}.{}", mannualfile, TimeUtils::getYYYYMMDDhhmmss())));

  if (_pos_blk._block == NULL || _pos_blk._block->_date != _tradingday) {
    SpinLock lock(_pos_blk._mutex);
    std::string filename = folder + "position.membin";
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG,
                       "loading local positions from {}", filename);
    bool isNew = false;
    ...
    if (_pos_blk._file->map(filename.c_str())) {
      ...
      if (_pos_blk._block->_date != 0 &&
          _pos_blk._block->_date != _tradingday) {
        WTSLogger::log_dyn("strategy", _name.c_str(), LL_INFO,
                           "Clearing local position of {}",
                           _pos_blk._block->_date);
        ...
        for (uint32_t i = 0; i < _pos_blk._block->_size; i++) {
          ...
          WTSContractInfo *cInfo =
              _engine->get_basedata_mgr()->getContract(ds._code, ds._exchg);
          if (cInfo == NULL)
            continue;
        ...
        memset(_pos_blk._block->_details, 0,
               sizeof(uft::DetailStruct) * _pos_blk._block->_size);

        if (!details.empty())
          memcpy(_pos_blk._block->_details, details.data(),
                 sizeof(uft::DetailStruct) * details.size());
...

      {
        ...
        for (uint32_t i = 0; i < _pos_blk._block->_size; i++) {
          ...
          posInfo._opencost +=
              ds._volume * ds._open_price * cInfo->getCommInfo()->getVolScale();
  ...
  if (_ord_blk._block == NULL || _ord_blk._block->_date != _tradingday) {
    ...
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG,
                       "loading local orders from {}", filename);
    bool isNew = false;
    if (!StdFile::exists(filename.c_str())) {
      std::size_t uSize =
          sizeof(uft::OrderBlock) + sizeof(uft::OrderStruct) * DATA_SIZE_STEP;
      BoostFile bf;
      bf.create_new_file(filename.c_str());
      bf.truncate_file(uSize);
      bf.close_file();
    ...
    if (_ord_blk._file->map(filename.c_str())) {
      ...
      // 交易日不一致就把数据清掉
      // 复用原文件的好处就是，mmap文件大小会满足历史出现过的单日最高数据量，以后再扩的概率就很低了
      if (_ord_blk._block->_date != 0 &&
          _ord_blk._block->_date != _tradingday) {
        memset(_ord_blk._block->_orders, 0,
               sizeof(uft::OrderStruct) * _ord_blk._block->_size);
  ...
  if (_trd_blk._block == NULL || _trd_blk._block->_date != _tradingday) {
    ...
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG,
                       "loading local trades from {}", filename);
    bool isNew = false;
    if (!StdFile::exists(filename.c_str())) {
      std::size_t uSize =
          sizeof(uft::TradeBlock) + sizeof(uft::TradeStruct) * DATA_SIZE_STEP;
      BoostFile bf;
      bf.create_new_file(filename.c_str());
      bf.truncate_file(uSize);
      bf.close_file();
    ...
    if (_trd_blk._file->map(filename.c_str())) {
      ...
      // 交易日不一致就把数据清掉
      // 复用原文件的好处就是，mmap文件大小会满足历史出现过的单日最高数据量，以后再扩的概率就很低了
      if (_trd_blk._block->_date != 0 &&
          _trd_blk._block->_date != _tradingday) {
        memset(_trd_blk._block->_trades, 0,
               sizeof(uft::TradeStruct) * _trd_blk._block->_size);
  ...
  if (_rnd_blk._block == NULL || _rnd_blk._block->_date != _tradingday) {
    ...
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG,
                       "loading local rouds from {}", filename);
    bool isNew = false;
    if (!StdFile::exists(filename.c_str())) {
      std::size_t uSize =
          sizeof(uft::RoundBlock) + sizeof(uft::RoundStruct) * DATA_SIZE_STEP;
      BoostFile bf;
      bf.create_new_file(filename.c_str());
      bf.truncate_file(uSize);
      bf.close_file();
    ...
    if (_rnd_blk._file->map(filename.c_str())) {
      ...
      // 交易日不一致就把数据清掉
      // 复用原文件的好处就是，mmap文件大小会满足历史出现过的单日最高数据量，以后再扩的概率就很低了
      if (_rnd_blk._block->_date != 0 &&
          _rnd_blk._block->_date != _tradingday) {
        memset(_rnd_blk._block->_rounds, 0,
               sizeof(uft::RoundStruct) * _rnd_blk._block->_size);
