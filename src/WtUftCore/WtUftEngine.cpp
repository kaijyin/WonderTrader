...
WtUftEngine::WtUftEngine() : _cfg(NULL), _tm_ticker(NULL), _notifier(NULL) {
...
void WtUftEngine::set_date_time(uint32_t curDate, uint32_t curTime,
                                uint32_t curSecs /* = 0 */,
                                uint32_t rawTime /* = 0 */) {
  _cur_date = curDate;
  _cur_time = curTime;
...
WTSCommodityInfo *WtUftEngine::get_commodity_info(const char *stdCode) {
  ...
  WTSContractInfo *cInfo =
      _base_data_mgr->getContract(ay[1].c_str(), ay[0].c_str());
  if (cInfo == NULL)
    return NULL;
...
WTSSessionInfo *WtUftEngine::get_session_info(const char *sid,
                                              bool isCode /* = false */) {
  if (!isCode)
...
WTSTickSlice *WtUftEngine::get_tick_slice(uint32_t sid, const char *code,
                                          uint32_t count) {
  return NULL;
...
WTSKlineSlice *WtUftEngine::get_kline_slice(uint32_t sid, const char *stdCode,
...
void WtUftEngine::init(WTSVariant *cfg, IBaseDataMgr *bdMgr,
                       WtUftDtMgr *dataMgr, EventNotifier *notifier) {
...
void WtUftEngine::on_bar(const char *stdCode, const char *period,
...
WTSOrdQueSlice *WtUftEngine::get_order_queue_slice(uint32_t sid,
                                                   const char *code,
                                                   uint32_t count) {
  return _data_mgr->get_order_queue_slice(code, count);
}

WTSOrdDtlSlice *WtUftEngine::get_order_detail_slice(uint32_t sid,
                                                    const char *code,
                                                    uint32_t count) {
  return _data_mgr->get_order_detail_slice(code, count);
}

WTSTransSlice *WtUftEngine::get_transaction_slice(uint32_t sid,
                                                  const char *code,
                                                  uint32_t count) {
