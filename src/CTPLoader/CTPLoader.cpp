...
int run(const char *cfgfile, bool bAsync = false, bool isFile = true) {
  ...
  if (!StdFile::exists(MODULE_NAME.c_str())) {
    MODULE_NAME =
        StrUtil::printf("%straders/%s", getBinDir(), MODULE_NAME.c_str());
  }
  ...
  g_ctpCreator = (CTPCreator)DLLHelper::get_symbol(
      dllInst, "?CreateFtdcTraderApi@CThostFtdcTraderApi@@SAPEAV1@PEBD@Z");
#else
  g_ctpCreator = (CTPCreator)DLLHelper::get_symbol(
      dllInst, "?CreateFtdcTraderApi@CThostFtdcTraderApi@@SAPAV1@PBD@Z");
#endif
#else
  g_ctpCreator = (CTPCreator)DLLHelper::get_symbol(
      dllInst, "_ZN19CThostFtdcTraderApi19CreateFtdcTraderApiEPKc");
#endif
