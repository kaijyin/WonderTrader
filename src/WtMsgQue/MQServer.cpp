...
MQServer::MQServer(MQManager *mgr)
    : _sock(-1), _ready(false), _mgr(mgr), _confirm(false),
      m_bTerminated(false), m_bTimeout(false) {
  _id = makeMQSvrId();
}
...
bool MQServer::init(const char *url, bool confirm /* = false */) {
  ...
  if (_sock < 0) {
    _mgr->log_server(
        _id, fmt::format("MQServer {} has an error {} while initializing", _id,
                         _sock)
                 .c_str());
    return false;
  }
  ...
  if (nn_bind(_sock, url) < 0) {
    _mgr->log_server(
        _id,
        fmt::format("MQServer {} has an error while binding url {}", _id, url)
            .c_str());
    return false;
  } else {
    _mgr->log_server(
        _id, fmt::format("MQServer {} has binded to {} ", _id, url).c_str());
  }
...
void MQServer::publish(const char *topic, const void *data, uint32_t dataLen) {
  if (_sock < 0) {
    _mgr->log_server(
        _id,
        fmt::format("MQServer {} has not been initialized yet", _id).c_str());
    return;
  }
      ...
      while (!m_bTerminated) {
        ...
        while (!tmpQue.empty()) {
          ...
          if (!pubData._data.empty()) {
            ...
            int bytes_snd = 0;
            for (;;) {
              int bytes = nn_send(_sock, m_sendBuf.data() + bytes_snd,
                                  len - bytes_snd, 0);
              if (bytes >= 0) {
                bytes_snd += bytes;
                if (bytes_snd == len)
