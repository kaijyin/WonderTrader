...
MQClient::MQClient(MQManager *mgr)
    : _sock(-1), m_bReady(false), _mgr(mgr), m_bTerminated(false),
      _cb_message(NULL), m_iCheckTime(0), m_bNeedCheck(false) {
  _id = makeMQCientId();
}
...
bool MQClient::init(const char *url, FuncMQCallback cb) {
  ...
  if (_sock < 0) {
    _mgr->log_client(
        _id, fmtutil::format("MQClient {} has an error {} while initializing",
                             _id, _sock));
    return false;
  }
  ...
  if (nn_connect(_sock, url) < 0) {
    _mgr->log_client(
        _id, fmtutil::format("MQClient {} has an error while connecting url {}",
                             _id, url));
    return false;
  } else {
    _mgr->log_client(
        _id, fmtutil::format("MQClient {} has connected to {} ", _id, url));
  }
...
void MQClient::start() {
  ...
  if (_sock < 0) {
    _mgr->log_client(
        _id, fmtutil::format("MQClient {} has not been initialized yet", _id));
    return;
  }
  ...
  if (m_thrdRecv == NULL) {
    ...
    _mgr->log_client(
        _id, fmtutil::format("MQClient {} has started successfully", _id));
  } else {
    _mgr->log_client(_id,
                     fmtutil::format("MQClient {} has already started", _id));
  }
}
