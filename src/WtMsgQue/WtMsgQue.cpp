...
void destroy_server(WtUInt32 id) { getMgr().destroy_server(id); }

void publish_message(WtUInt32 id, const char *topic, const char *data,
                     WtUInt32 dataLen) {
...
void destroy_client(WtUInt32 id) { getMgr().destroy_client(id); }

void subscribe_topic(WtUInt32 id, const char *topic) {
  return getMgr().sub_topic(id, topic);
}

void start_client(WtUInt32 id) { getMgr().start_client(id); }
