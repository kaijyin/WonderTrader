...
public:
  ...
  virtual void notifyLogin(int errorNo, int maxOrderRef,
  ...
  virtual void notifyOrder(const YDOrder *pOrder,
                           ...
                           const YDAccount *pAccount) override;
...
protected:
  WTSOrderInfo *makeOrderInfo(const YDOrder *orderField,
                              const YDInstrument *instInfo);
  WTSEntrust *makeEntrust(const YDInputOrder *entrustField,
                          const YDInstrument *instInfo);
  WTSTradeInfo *makeTradeRecord(const YDTrade *tradeField,
                                const YDInstrument *instInfo);
  ...
  inline uint32_t genRequestID() { return m_iRequestID.fetch_add(1) + 1; }
