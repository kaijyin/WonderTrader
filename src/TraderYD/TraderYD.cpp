...
TraderYD::TraderYD()
    : m_pUserAPI(NULL), m_mapPosition(NULL), m_mapOrders(NULL),
      m_mapTrades(NULL), m_wrapperState(WS_NOTLOGIN), m_ayFunds(NULL),
      m_uLastQryTime(0), m_iRequestID(0), m_bCatchup(false), m_bStopped(false),
      m_bApiInited(false) {}
...
void TraderYD::notifyLogin(int errorNo, int maxOrderRef, bool isMonitor) {
  if (errorNo == 0) {
    ...
    write_log(m_sink, LL_ERROR, "[TraderYD] {} Login failed: {}",
              m_strUser.c_str(), errorNo);
...
void TraderYD::notifyFailedOrder(const YDInputOrder *pFailedOrder,
...
void TraderYD::notifyFailedCancelOrder(
    const YDFailedCancelOrder *pFailedCancelOrder, const YDExchange *pExchange,
    const YDAccount *pAccount) {
...
void TraderYD::notifyFinishInit() {
  ...
  // 先查资金
  {
    ...
    accountInfo->setBalance(accountInfo->getPreBalance() +
                            accountInfo->getDeposit() -
  ...
  // 再查持仓
  {
    ...
    for (int i = 0; i < cnt; i++) {
      ...
      WTSContractInfo *contract = m_bdMgr->getContract(
          instInfo->InstrumentID, instInfo->m_pExchange->ExchangeID);
      if (contract) {
        WTSCommodityInfo *commInfo = contract->getCommInfo();
        std::string key =
            fmt::format("{}-{}", contract->getCode(),
                        wrapPosDirection(pInfo->PositionDirection));
        WTSPositionItem *pos = (WTSPositionItem *)m_mapPosition->get(key);
        if (pos == NULL) {
          pos = WTSPositionItem::create(contract->getCode(),
        ...
        pos->setMargin(0);
        pos->setDynProfit(0);
        pos->setPositionCost(pInfo->PrePosition * commInfo->getVolScale() *
                             pInfo->PreSettlementPrice);
        ...
        write_log(m_sink, LL_INFO, "{} PrePosition of {} updated:{}[{}]",
                  pos->getDirection() == WDT_LONG ? "Long" : "Short",
                  contract->getFullCode(), pos->getTotalPosition(),
                  pos->getAvailPosition());
      }
    }
  }
}

void TraderYD::notifyOrder(const YDOrder *pOrder,
                           const YDInstrument *pInstrument,
                           const YDAccount *pAccount) {
  ...
  if (orderInfo) {
    ...
    if (it == m_mapOrders->end()) {
      ...
      if (orderInfo->getOffsetType() != WOT_OPEN) {
        const char *key = fmtutil::format("{}-{}", orderInfo->getCode(),
                                          orderInfo->getDirection());
        WTSPositionItem *pos = (WTSPositionItem *)m_mapPosition->get(key);
        ...
        WTSCommodityInfo *commInfo =
            orderInfo->getContractInfo()->getCommInfo();
      ...
      if (preOrd->isAlive() && orderInfo->getOrderState() == WOS_Canceled &&
          orderInfo->getOffsetType() != WOT_OPEN) {
        std::string key = fmt::format("{}-{}", orderInfo->getCode(),
                                      orderInfo->getDirection());
        WTSPositionItem *pos = (WTSPositionItem *)m_mapPosition->get(key);
        double preQty = pos->getPrePosition();
        double newQty = pos->getNewPosition();
...
void TraderYD::notifyTrade(const YDTrade *pTrade,
                           const YDInstrument *pInstrument,
                           const YDAccount *pAccount) {
  ...
  if (trdInfo) {
    ...
    if (it == m_mapTrades->end()) {
      ...
      // 成交回报，主要更新持仓
      std::string key =
          fmt::format("{}-{}", trdInfo->getCode(), trdInfo->getDirection());
      WTSPositionItem *pos = (WTSPositionItem *)m_mapPosition->get(key);
      if (pos == NULL) {
        pos = WTSPositionItem::create(
            contract->getCode(), commInfo->getCurrency(), commInfo->getExchg());
        pos->setContractInfo(contract);
...
int TraderYD::login(const char *user, const char *pass,
...
int TraderYD::orderInsert(WTSEntrust *entrust) {
  ...
  // 这个例子使用限价单
  req.OrderType =
      wrapPriceType(entrust->getPriceType(), entrust->getOrderFlag());
  // 说明是普通报单
...
WTSOrderInfo *TraderYD::makeOrderInfo(const YDOrder *orderField,
                                      const YDInstrument *instInfo) {
  const YDExchange *exchgInfo = instInfo->m_pExchange;

  WTSContractInfo *contract =
      m_bdMgr->getContract(instInfo->InstrumentID, exchgInfo->ExchangeID);
  if (contract == NULL)
    return NULL;
  ...
  pRet->setPriceType(decimal::eq(orderField->Price, 0.0) ? WPT_ANYPRICE
                                                         : WPT_LIMITPRICE);
...
WTSEntrust *TraderYD::makeEntrust(const YDInputOrder *entrustField,
                                  const YDInstrument *instInfo) {
  WTSContractInfo *ct = m_bdMgr->getContract(instInfo->InstrumentID,
                                             instInfo->m_pExchange->ExchangeID);
  if (ct == NULL)
    return NULL;

  WTSEntrust *pRet =
      WTSEntrust::create(ct->getCode(), entrustField->OrderVolume,
                         entrustField->Price, ct->getExchg());
  ...
  pRet->setPriceType(decimal::eq(entrustField->Price, 0.0) ? WPT_ANYPRICE
                                                           : WPT_LIMITPRICE);
...
WTSError *TraderYD::makeError(int errorno, WTSErroCode ec) {
  WTSError *pRet =
      WTSError::create(ec, fmt::format("ErrorNo: {}", errorno).c_str());
  return pRet;
}

WTSTradeInfo *TraderYD::makeTradeRecord(const YDTrade *tradeField,
                                        const YDInstrument *instInfo) {
  WTSContractInfo *contract = m_bdMgr->getContract(
      instInfo->InstrumentID, instInfo->m_pExchange->ExchangeID);
  if (contract == NULL)
  ...
  WTSTradeInfo *pRet =
      WTSTradeInfo::create(contract->getCode(), commInfo->getExchg());
  pRet->setContractInfo(contract);
...
bool TraderYD::isConnected() { return (m_wrapperState == WS_ALLREADY); }
