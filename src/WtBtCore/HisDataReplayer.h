...
public:
  virtual void handle_tick(const char *stdCode, WTSTickData *curTick,
                           uint32_t pxType) = 0;
  virtual void handle_order_queue(const char *stdCode,
                                  WTSOrdQueData *curOrdQue) {};
  virtual void handle_order_detail(const char *stdCode,
  ...
  virtual void handle_bar_close(const char *stdCode, const char *period,
                                uint32_t times, WTSBarStruct *newBar) = 0;
 ...
 *	@dates
 */
typedef void (*FuncReadFactors)(void *obj, const char *stdCode, uint32_t *dates,
 ...
 *	@count		条数
 */
typedef void (*FuncReadTicks)(void *obj, WTSTickStruct *firstItem,
                              uint32_t count);
 ...
 *	@count		条数
 */
typedef void (*FuncReadOrdDtl)(void *obj, WTSOrdDtlStruct *firstItem,
                               uint32_t count);
 ...
 *	@count		条数
 */
typedef void (*FuncReadOrdQue)(void *obj, WTSOrdQueStruct *firstItem,
                               uint32_t count);
 ...
 *	@count		条数
 */
typedef void (*FuncReadTrans)(void *obj, WTSTransStruct *firstItem,
                              uint32_t count);
...
public:
  ...
  virtual bool loadFinalHisBars(void *obj, const char *stdCode,
                                WTSKlinePeriod period, FuncReadBars cb) = 0;
  ...
  virtual bool loadAdjFactors(void *obj, const char *stdCode,
                              FuncReadFactors cb) = 0;

  /*
   *	加载历史Tick数据
   */
  virtual bool loadRawHisTicks(void *obj, const char *stdCode, uint32_t uDate,
                               FuncReadTicks cb) = 0;
...
private:
  template <typename T> class HftDataList {
  public:
    std::string _code;
    uint32_t _date;
    /*
  ...
  typedef struct _BarsList {
    ...
    inline void mark() { _untouch_days = 0; }

    inline std::size_t size() { return sizeof(WTSBarStruct) * _bars.size(); }

    _BarsList()
        : _cursor(UINT_MAX), _count(0), _times(1), _factor(1),
          _untouch_days(0) {}
  } BarsList;
  ...
  typedef struct _TaskInfo {
    ...
    uint32_t
        _day; // 日期,根据周期变化,每日为0,每周为0~6,对应周日到周六,每月为1~31,每年为0101~1231
    uint32_t _time; // 时间,精确到分钟
    bool
        _strict_time; // 是否是严格时间,严格时间即只有时间相等才会执行,不是严格时间,则大于等于触发时间都会执行
...
private:
   ...
   *	从自定义数据文件缓存历史数据
   */
  bool cacheRawBarsFromBin(const std::string &key, const char *stdCode,
                           WTSKlinePeriod period, bool bForBars = true);

  /*
   *	从csv文件缓存历史数据
   */
  bool cacheRawBarsFromCSV(const std::string &key, const char *stdCode,
                           WTSKlinePeriod period, bool bSubbed = true);

  /*
   *	从自定义数据文件缓存历史tick数据
   */
  bool cacheRawTicksFromBin(const std::string &key, const char *stdCode,
                            uint32_t uDate);

  /*
   *	从自定义数据文件缓存历史委托明细数据
   */
  bool cacheRawOrdDtlFromBin(const std::string &key, const char *stdCode,
                             uint32_t uDate);

  /*
   *	从自定义数据文件缓存历史委托队列
   */
  bool cacheRawOrdQueFromBin(const std::string &key, const char *stdCode,
                             uint32_t uDate);

  /*
   *	从自定义数据文件缓存历史成交明细数据
   */
  bool cacheRawTransFromBin(const std::string &key, const char *stdCode,
                            uint32_t uDate);

  /*
   *	从csv文件缓存历史tick数据
   */
  bool cacheRawTicksFromCSV(const std::string &key, const char *stdCode,
                            uint32_t uDate);

  /*
   *	从外部加载器缓存历史数据
   */
  bool cacheFinalBarsFromLoader(const std::string &key, const char *stdCode,
                                WTSKlinePeriod period, bool bSubbed = true);

  /*
   *	从外部加载器缓存历史tick数据
   */
  bool cacheRawTicksFromLoader(const std::string &key, const char *stdCode,
                               uint32_t uDate);

  /*
   *	缓存整合的期货合约历史K线（针对.HOT//2ND）
   */
  bool cacheIntegratedFutBarsFromBin(void *codeInfo, const std::string &key,
                                     const char *stdCode, WTSKlinePeriod period,
                                     bool bSubbed = true);

  /*
   *	缓存复权股票K线数据
   */
  bool cacheAdjustedStkBarsFromBin(void *codeInfo, const std::string &key,
                                   const char *stdCode, WTSKlinePeriod period,
                                   bool bSubbed = true);
  ...
  void simTickWithUnsubBars(uint64_t stime, uint64_t etime,
                            uint32_t endTDate = 0, int pxType = 0);

  void simTicks(uint32_t uDate, uint32_t uTime, uint32_t endTDate = 0,
                int pxType = 0);
  ...
  inline uint64_t getNextTickTime(uint32_t curTDate,
                                  uint64_t stime = UINT64_MAX);
  inline uint64_t getNextOrdQueTime(uint32_t curTDate,
                                    uint64_t stime = UINT64_MAX);
  inline uint64_t getNextOrdDtlTime(uint32_t curTDate,
                                    uint64_t stime = UINT64_MAX);
  inline uint64_t getNextTransTime(uint32_t curTDate,
                                   uint64_t stime = UINT64_MAX);

  void reset();

  void dump_btstate(const char *stdCode, WTSKlinePeriod period, uint32_t times,
                    uint64_t stime, uint64_t etime, double progress,
                    int64_t elapse);
  void notify_state(const char *stdCode, WTSKlinePeriod period, uint32_t times,
                    uint64_t stime, uint64_t etime, double progress);

  uint32_t locate_barindex(const std::string &key, uint64_t curTime,
...
public:
  ...
  inline void enable_tick(bool bEnabled = true) { _tick_enabled = bEnabled; }
   ...
   *	注册任务
   *	@date
   *日期,根据周期变化,每日为0,每周为0~6,对应周日到周六,每月为1~31,每年为0101~1231
   *	@time 时间,精确到分钟
   *	@period	时间周期，可以是分钟、天、周、月、年
   */
  void register_task(uint32_t taskid, uint32_t date, uint32_t time,
                     const char *period, const char *trdtpl = "CHINA",
                     const char *session = "TRADING");

  WTSKlineSlice *get_kline_slice(const char *stdCode, const char *period,
                                 uint32_t count, uint32_t times = 1,
                                 bool isMain = false);

  WTSTickSlice *get_tick_slice(const char *stdCode, uint32_t count,
                               uint64_t etime = 0);

  WTSOrdDtlSlice *get_order_detail_slice(const char *stdCode, uint32_t count,
                                         uint64_t etime = 0);

  WTSOrdQueSlice *get_order_queue_slice(const char *stdCode, uint32_t count,
                                        uint64_t etime = 0);

  WTSTransSlice *get_transaction_slice(const char *stdCode, uint32_t count,
                                       uint64_t etime = 0);
  ...
  double calc_fee(const char *stdCode, double price, double qty,
                  uint32_t offset);
...
private:
  ...
  std::map<std::string, WTSTickStruct>
      _day_cache; // 每日Tick缓存,当tick回放未开放时,会用到该缓存
  ...
  typedef struct _FeeItem {
    ...
    _FeeItem() { memset(this, 0, sizeof(_FeeItem)); }
  } FeeItem;
