 ...
 *	处理块数据
 */
bool proc_block_data(const char *tag, std::string &content, bool isBar,
                     bool bKeepHead = true) {
  ...
  if (bCmped) {
    ...
    if (content.size() != (sizeof(BlockHeaderV2) + blkV2->_size)) {
      WTSLogger::error("Size check failed while processing {} data of {}",
                       isBar ? "bar" : "tick", tag);
      return false;
    }

    // 将文件头后面的数据进行解压
    buffer = WTSCmpHelper::uncompress_data(content.data() + BLOCK_HEADERV2_SIZE,
                                           blkV2->_size);
  } else {
    if (!bOldVer) {
      // 如果不是老版本，直接返回
      ...
      buffer.append(content.data() + BLOCK_HEADER_SIZE,
                    content.size() - BLOCK_HEADER_SIZE);
    }
  }
  ...
  if (bOldVer) {
    if (isBar) {
      ...
      WTSLogger::debug("{} bars of {} transferd to new version...", barcnt,
                       tag);
    } else {
      ...
      WTSLogger::debug("{} ticks of {} transferd to new version...", tick_cnt,
                       tag);
    }
  }
...
HisDataReplayer::HisDataReplayer()
    : _listener(NULL), _cur_date(0), _cur_time(0), _cur_secs(0), _cur_tdate(0),
      _tick_enabled(true), _opened_tdate(0), _closed_tdate(0),
      _tick_simulated(true), _running(false), _begin_time(0), _end_time(0),
      _bt_loader(NULL), _min_period("d"), _cache_clear_days(0),
...
bool HisDataReplayer::init(WTSVariant *cfg,
  ...
  bool isRangeCfg =
      (_begin_time == 0 || _end_time == 0); // 是否从配置文件读取回测区间
  if (_begin_time == 0)
    _begin_time = cfg->getUInt64("stime");

  if (_end_time == 0)
    _end_time = cfg->getUInt64("etime");
  WTSLogger::info("Backtest time range is set to be [{},{}] via config",
                  _begin_time, _end_time);

  _cache_clear_days = cfg->getUInt32("cache_clear_days");
  WTSLogger::info("Unused cache data will be cleard in {} days",
                  _cache_clear_days);

  _tick_enabled = cfg->getBoolean("tick");
  WTSLogger::info("Tick data replaying is {}",
                  _tick_enabled ? "enabled" : "disabled");
  ...
  WTSLogger::info("Resampled bars will be aligned by section: {}",
                  _align_by_section ? "yes" : " no");
  ...
  if (cfgBF->has("rules")) {
    ...
    for (const std::string &ruleTag : tags) {
      ...
      WTSLogger::info("{} rules loaded from {}", ruleTag,
                      cfgRules->getCString(ruleTag.c_str()));
    }
  }
...
bool HisDataReplayer::loadStkAdjFactorsFromLoader() {
  ...
  bool ret = _bt_loader->loadAllAdjFactors(
      this, [](void *obj, const char *stdCode, uint32_t *dates, double *factors,
               uint32_t count) {
        ...
        std::sort(fctrLst.begin(), fctrLst.end(),
                  [](const AdjFactor &left, const AdjFactor &right) {
  ...
  if (ret)
    WTSLogger::info(
        "Adjusting factors of {} contracts loaded via extended loader",
        _adj_factors.size());
  return ret;
}
...
bool HisDataReplayer::loadStkAdjFactorsFromFile(const char *adjfile) {
  ...
  WTSLogger::info("{} items of adjust factors for {} tickers loaded from {}",
                  fct_cnt, stk_cnt, adjfile);
  doc->release();
  return true;
}

void HisDataReplayer::register_task(uint32_t taskid, uint32_t date,
                                    uint32_t time, const char *period,
...
void HisDataReplayer::reset() {
  ...
  for (auto &m : _ticks_cache) {
    HftDataList<WTSTickStruct> &cacheItem =
        (HftDataList<WTSTickStruct> &)m.second;
    cacheItem._cursor = UINT_MAX;
  }

  for (auto &m : _orddtl_cache) {
    HftDataList<WTSOrdDtlStruct> &cacheItem =
        (HftDataList<WTSOrdDtlStruct> &)m.second;
    cacheItem._cursor = UINT_MAX;
  }

  for (auto &m : _ordque_cache) {
    HftDataList<WTSOrdQueStruct> &cacheItem =
        (HftDataList<WTSOrdQueStruct> &)m.second;
    cacheItem._cursor = UINT_MAX;
  }

  for (auto &m : _trans_cache) {
    HftDataList<WTSTransStruct> &cacheItem =
        (HftDataList<WTSTransStruct> &)m.second;
    cacheItem._cursor = UINT_MAX;
  }
...
void HisDataReplayer::dump_btstate(const char *stdCode, WTSKlinePeriod period,
                                   uint32_t times, uint64_t stime,
                                   uint64_t etime, double progress,
                                   int64_t elapse) {
  std::string output;
  {
...
void HisDataReplayer::notify_state(const char *stdCode, WTSKlinePeriod period,
                                   uint32_t times, uint64_t stime,
                                   uint64_t etime, double progress) {
  if (!_notifier)
    return;
...
uint32_t HisDataReplayer::locate_barindex(const std::string &key, uint64_t now,
  ...
  auto it =
      std::lower_bound(barsList->_bars.begin(), barsList->_bars.end(), bar,
                       [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
                         if (isDay)
...
void HisDataReplayer::stop() {
  ...
  WTSLogger::log_raw(
      LL_WARN,
      "Terminating flag reset to true, backtesting will quit at next round");
}

bool HisDataReplayer::prepare() {
  if (_running) {
    WTSLogger::log_raw(
        LL_ERROR, "Cannot run more than one backtesting task at the same time");
    return false;
  }
  ...
  _cur_secs = 0;
  _cur_tdate =
      _bd_mgr.calcTradingDate(DEFAULT_SESSIONID, _cur_date, _cur_time, true);
...
void HisDataReplayer::run(bool bNeedDump /* = false*/) {
  if (_task == NULL) {
    ...
    if (_main_key.empty() && !_bars_cache.empty()) {
      ...
      WTSLogger::info("Main K bars automatic determined: {}",
                      _main_key.c_str());
    }
    ...
    if (!_main_key.empty()) {
      ...
      WTSLogger::log_raw(LL_INFO,
                         "Main K bars not subscribed and backtesting of tick "
                         "data not available , replaying done");
...
void HisDataReplayer::run_by_ticks(bool bNeedDump /* = false */) {
  ...
  uint64_t end_tdate =
      _bd_mgr.calcTradingDate(DEFAULT_SESSIONID, edt, etime, true);
...
void HisDataReplayer::run_by_bars(bool bNeedDump /* = false */) {
  ...
  CodeHelper::CodeInfo codeInfo =
      CodeHelper::extractStdCode(barsList->_code.c_str(), NULL);
  ...
  notify_state(barsList->_code.c_str(), barsList->_period, barsList->_times,
               _begin_time, _end_time, 0);

  if (bNeedDump)
    dump_btstate(barsList->_code.c_str(), barsList->_period, barsList->_times,
                 _begin_time, _end_time, 100.0, ticker.nano_seconds());
  ...
  for (; !_terminated;) {
    ...
    if (barsList->_cursor != UINT_MAX) {
      ...
      if (isDay)
        nextBarTime =
            (uint64_t)barsList->_bars[barsList->_cursor].date * 10000 +
            sInfo->getCloseTime();
      else {
      ...
      if (nextBarTime > _end_time) {
        WTSLogger::info("{} is beyond ending time {},replaying done",
                        nextBarTime, _end_time);
        break;
      }
      ...
      if (isDay || (!isDay && sInfo->offsetTime(nextTime, false) !=
                                  sInfo->getCloseTime(true))) {
        nextTDate =
            _bd_mgr.calcTradingDate(commId.c_str(), nextDate, nextTime, false);
        if (_opened_tdate != nextTDate) {
           ...
           */
          uint64_t beginTimeofDay =
              _bd_mgr.getBoundaryTime(sInfo->id(), nextTDate, true, true);
      ...
      bool isEndTDate =
          (sInfo->offsetTime(_cur_time, false) >= sInfo->getCloseTime(true));
      ...
      for (int i = 0; i < 4; i++) {
        if (_tick_simulated)
          simTicks(nextDate, nextTime, (isDay || isEndTDate) ? nextTDate : 0,
                   i);

        if (!_tick_enabled)
          simTickWithUnsubBars(curBarTime, nextBarTime,
                               (isDay || isEndTDate) ? nextTDate : 0, i);
      }
      ...
      notify_state(barsList->_code.c_str(), barsList->_period, barsList->_times,
                   _begin_time, _end_time,
                   replayed_barcnt * 100.0 / total_barcnt);
      ...
      WTSLogger::log_raw(LL_ERROR,
                         "No back data initialized, replaying canceled");
      break;
    }
  }
...
void HisDataReplayer::run_by_tasks(bool bNeedDump /* = false */) {
  ...
  const char *DEF_SESS =
      (strlen(_task->_session) == 0) ? DEFAULT_SESSIONID : _task->_session;
  sInfo = _bd_mgr.getSession(DEF_SESS);
  WTSLogger::info("Start to backtest with task frequency from {}...",
                  _begin_time);
  ...
  if (_task->_period != TPT_Minute) {
    ...
    for (; !_terminated;) {
      ...
      if (_cur_time == endtime) {
        if (!_bd_mgr.isHoliday(_task->_trdtpl, _cur_date, true)) {
          ...
          case TPT_Minute:
            break;
          case TPT_Monthly:
            // if (preTDate % 1000000 < _task->_day && _cur_date % 1000000 >=
            // _task->_day) 	fired = true;
            if (_cur_date % 1000000 == _task->_day)
              fired = true;
            else if (bHasHoliday) {
              // 上一个交易日在上个月,且当前日期大于触发日期
              // 说明这个月的开始日期在节假日内,顺延到今天
              if ((preTDate % 10000 / 100 < _cur_date % 10000 / 100) &&
                  _cur_date % 1000000 > _task->_day) {
                fired = true;
              } else if (preTDate % 1000000 < _task->_day &&
                         _cur_date % 1000000 > _task->_day) {
          ...
          case TPT_Weekly:
            ...
            break;
          case TPT_Yearly:
            if (preTDate % 10000 < _task->_day &&
                _cur_date % 10000 >= _task->_day)
              fired = true;
            break;
          }
        }
      ...
      if (!fired) {
        ...
        uint32_t newTDate =
            _bd_mgr.calcTradingDate(DEF_SESS, _cur_date, _cur_time, true);
        ...
        bool bEndSession =
            sInfo->offsetTime(curTime, true) >= sInfo->getCloseTime(true);
        if (_listener)
          _listener->handle_session_begin(_cur_tdate);
        check_cache_days();
    ...
    for (; !_terminated;) {
      ...
      if (bNewTDate) {
        ...
        mins = _task->_time;
        uint32_t nextTDate =
            _bd_mgr.getNextTDate(_task->_trdtpl, _cur_tdate, 1, true);
...
void HisDataReplayer::simTicks(uint32_t uDate, uint32_t uTime,
                               uint32_t endTDate /* = 0 */,
                               int pxType /* = 0 */) {
  // 这里应该触发检查
      ...
      if (barsList->_bars.size() > barsList->_cursor) {
        for (;;) {
          ...
          if (nextBar.date == endTDate) {
            const std::string &ticker = _ticker_keys[barsList->_code];
            if (ticker == it->first) {
              CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(
                  barsList->_code.c_str(), &_hot_mgr);
              WTSCommodityInfo *commInfo =
                  _bd_mgr.getCommodity(cInfo._exchg, cInfo._product);
...
void HisDataReplayer::simTickWithUnsubBars(uint64_t stime, uint64_t nowTime,
                                           uint32_t endTDate /* = 0 */,
                                           int pxType /* = 0 */) {
  // uint64_t nowTime = (uint64_t)uDate * 10000 + uTime;
        ...
        for (;;) {
          ...
          if (nextBar.date == endTDate) {
            CodeHelper::CodeInfo cInfo =
                CodeHelper::extractStdCode(barsList->_code.c_str(), &_hot_mgr);
            WTSCommodityInfo *commInfo =
                _bd_mgr.getCommodity(cInfo._exchg, cInfo._product);
...
uint64_t HisDataReplayer::getNextTickTime(uint32_t curTDate,
                                          uint64_t stime /* = UINT64_MAX */) {
  ...
  for (auto &v : _tick_sub_map) {
    ...
    if (tickList._cursor == UINT_MAX) {
      if (stime == UINT64_MAX) {
        ...
        for (tickList._cursor = 1;; tickList._cursor++) {
          ...
          uint32_t tickMin =
              tickList._items[tickList._cursor - 1].action_time / 100000;
        ...
        auto tit = std::lower_bound(
            tickList._items.begin(), tickList._items.end(), curTick,
            [](const WTSTickStruct &a, const WTSTickStruct &b) {
    ...
    while (!sInfo->isInTradingTime(nextMinTime) &&
    ...
    uint64_t lastTime =
        (uint64_t)nextTick.action_date * 1000000000 + nextTick.action_time;
...
uint64_t HisDataReplayer::getNextTransTime(uint32_t curTDate,
                                           uint64_t stime /* = UINT64_MAX */) {
  uint64_t nextTime = UINT64_MAX;
  for (auto v : _trans_sub_map) {
    ...
    if (itemList._cursor == UINT_MAX) {
      if (stime == UINT64_MAX)
        ...
        auto tit = std::lower_bound(
            itemList._items.begin(), itemList._items.end(), curItem,
            [](const WTSTransStruct &a, const WTSTransStruct &b) {
              if (a.action_date != b.action_date)
                return a.action_date < b.action_date;
              else
                return a.action_time < b.action_time;
    ...
    uint64_t lastTime =
        (uint64_t)nextItem.action_date * 1000000000 + nextItem.action_time;
...
uint64_t HisDataReplayer::getNextOrdDtlTime(uint32_t curTDate,
                                            uint64_t stime /* = UINT64_MAX */) {
  uint64_t nextTime = UINT64_MAX;
  for (auto v : _orddtl_sub_map) {
    ...
    if (itemList._cursor == UINT_MAX) {
      if (stime == UINT64_MAX)
        ...
        auto tit = std::lower_bound(
            itemList._items.begin(), itemList._items.end(), curItem,
            [](const WTSOrdDtlStruct &a, const WTSOrdDtlStruct &b) {
              if (a.action_date != b.action_date)
                return a.action_date < b.action_date;
              else
                return a.action_time < b.action_time;
...
uint64_t HisDataReplayer::getNextOrdQueTime(uint32_t curTDate,
                                            uint64_t stime /* = UINT64_MAX */) {
  uint64_t nextTime = UINT64_MAX;
  for (auto v : _ordque_sub_map) {
    ...
    if (itemList._cursor == UINT_MAX) {
      if (stime == UINT64_MAX)
        ...
        auto tit = std::lower_bound(
            itemList._items.begin(), itemList._items.end(), curItem,
            [](const WTSOrdQueStruct &a, const WTSOrdQueStruct &b) {
              if (a.action_date != b.action_date)
                return a.action_date < b.action_date;
              else
                return a.action_time < b.action_time;
...
void HisDataReplayer::onMinuteEnd(uint32_t uDate, uint32_t uTime,
                                  uint32_t endTDate /* = 0 */,
                                  bool tickSimulated /* = true */) {
  // 这里应该触发检查
  uint64_t nowTime = (uint64_t)uDate * 10000 + uTime;
    ...
    if (barsList->_period != KP_DAY) {
        ...
        for (;;) {
          ...
          if (barTime <= nowTime) {
            ...
            if (barsList->_period == KP_Minute5)
              times *= 5;
            _listener->handle_bar_close(barsList->_code.c_str(), "m", times,
                                        &nextBar);
          } else {
            break;
          }
      ...
      if (barsList->_bars.size() > barsList->_cursor) {
        for (;;) {
          ...
          if (nextBar.date <= endTDate) {
            _listener->handle_bar_close(barsList->_code.c_str(), "d",
                                        barsList->_times, &nextBar);
          } else {
            break;
          }
...
WTSKlineSlice *HisDataReplayer::get_kline_slice(const char *stdCode,
                                                const char *period,
                                                uint32_t count,
                                                uint32_t times /* = 1 */,
                                                bool isMain /* = false */) {
  ...
  // 不做判断,主要为了防止没有tick数据,而采用第二方案
  {
    if (_ticker_keys.find(stdCode) == _ticker_keys.end())
      ...
      if (strcmp(period, "m") == 0 && oldKey.at(0) == 'd') {
        _ticker_keys[stdCode] = key;
        _min_period = period;
      } else if (oldKey.at(0) == period[0] &&
                 times < strtoul(oldKey.substr(2).c_str(), NULL, 10)) {
        _ticker_keys[stdCode] = key;
        _min_period = period;
      }
    }
  ...
  if (!bHasCache) {
    if (realTimes != 1) {
      std::string rawKey =
          StrUtil::printf("%s#%s#%u", stdCode, period, baseTimes);
  ...
  bool isClosed =
      (sInfo->offsetTime(_cur_time, true) >= sInfo->getCloseTime(true));
  if (realTimes != 1 && !bHasCache) {
    std::string rawKey =
        StrUtil::printf("%s#%s#%u", stdCode, period, baseTimes);
    BarsListPtr &rawBars = _bars_cache[rawKey];
    WTSKlineSlice *rawKline = WTSKlineSlice::create(
        stdCode, kp, realTimes, &rawBars->_bars[0], rawBars->_bars.size());
    rawKline->setCode(stdCode);

    static WTSDataFactory dataFact;
    WTSKlineData *kData = dataFact.extractKlineData(
        rawKline, kp, realTimes, sInfo, true, _align_by_section);
    rawKline->release();
    ...
    if (kData) {
      ...
      kData->release();
      WTSLogger::info("{} resampled {}{} back kline of {} ready",
                      barsList->_bars.size(), period, times, stdCode);
    } else {
      WTSLogger::error("Resampling {}{} back kline of {} failed", period, times,
                       stdCode);
      return NULL;
    }
  }
  ...
  if (kBlkPair->_cursor == UINT_MAX) {
    ...
    auto it = std::lower_bound(
        kBlkPair->_bars.begin(), kBlkPair->_bars.end(), bar,
        [isDay, isClosed](const WTSBarStruct &a, const WTSBarStruct &b) {
          if (isDay)
            if (!isClosed)
              return a.date < b.date;
            else
    ...
    if (isDay) {
      if (kBlkPair->_cursor <= kBlkPair->_count) {
        if (!isClosed) {
          while (kBlkPair->_bars[kBlkPair->_cursor - 1].date < _cur_tdate &&
                 kBlkPair->_cursor < kBlkPair->_count &&
                 kBlkPair->_bars[kBlkPair->_cursor].date < _cur_tdate) {
            kBlkPair->_cursor++;
          }
        } else {
          while (kBlkPair->_bars[kBlkPair->_cursor - 1].date <= _cur_tdate &&
      ...
      if (kBlkPair->_cursor <= kBlkPair->_count) {
        while (kBlkPair->_bars[kBlkPair->_cursor - 1].time < curMin &&
               kBlkPair->_cursor < kBlkPair->_count) {
          kBlkPair->_cursor++;
        }
  ...
  WTSKlineSlice *kline = WTSKlineSlice::create(
      stdCode, kp, 1, kBlkPair->_bars.data() + sIdx, realCnt);
  return kline;
}

WTSTickSlice *HisDataReplayer::get_tick_slice(const char *stdCode,
                                              uint32_t count, uint64_t etime) {
  if (!_tick_enabled)
    return NULL;
  ...
  if (tickList._cursor == UINT_MAX) {
    ...
    if (tit == tickList._items.end()) {
      ...
      if (thisTick.action_date > uDate ||
          (thisTick.action_date == uDate && thisTick.action_time > uTime)) {
        if (idx > 0) {
          tit--;
          idx--;
        } else {
  ...
  WTSTickSlice *ticks =
      WTSTickSlice::create(stdCode, tickList._items.data() + sIdx, realCnt);
  return ticks;
}

WTSOrdDtlSlice *
HisDataReplayer::get_order_detail_slice(const char *stdCode, uint32_t count,
                                        uint64_t etime /* = 0 */) {
  ...
  if (dataList._cursor == UINT_MAX) {
    ...
    auto tit = std::lower_bound(
        dataList._items.begin(), dataList._items.end(), curItem,
        [](const WTSOrdDtlStruct &a, const WTSOrdDtlStruct &b) {
          if (a.action_date != b.action_date)
            return a.action_date < b.action_date;
          else
  ...
  WTSOrdDtlSlice *ticks =
      WTSOrdDtlSlice::create(stdCode, dataList._items.data() + sIdx, realCnt);
  return ticks;
}

WTSOrdQueSlice *
HisDataReplayer::get_order_queue_slice(const char *stdCode, uint32_t count,
                                       uint64_t etime /* = 0 */) {
  ...
  WTSOrdQueSlice *ticks =
      WTSOrdQueSlice::create(stdCode, dataList._items.data() + sIdx, realCnt);
  return ticks;
}

WTSTransSlice *
HisDataReplayer::get_transaction_slice(const char *stdCode, uint32_t count,
                                       uint64_t etime /* = 0 */) {
  ...
  WTSTransSlice *ticks =
      WTSTransSlice::create(stdCode, dataList._items.data() + sIdx, realCnt);
  return ticks;
}
...
bool HisDataReplayer::checkTicks(const char *stdCode, uint32_t uDate) {
  ...
  if (it == _ticks_cache.end())
    ...
    HftDataList<WTSTickStruct> &tickList =
        (HftDataList<WTSTickStruct> &)it->second;
...
std::string HisDataReplayer::get_rawcode(const char *stdCode) {
  ...
  if (cInfo.hasRule()) {
    std::string code = _hot_mgr.getCustomRawCode(cInfo._ruletag,
                                                 cInfo.stdCommID(), _cur_tdate);
...
WTSSessionInfo *HisDataReplayer::get_session_info(const char *sid,
                                                  bool isCode /* = false */) {
  if (!isCode)
  ...
  WTSCommodityInfo *cInfo =
      _bd_mgr.getCommodity(codeInfo._exchg, codeInfo._product);
  if (cInfo == NULL)
    return NULL;
...
double HisDataReplayer::calc_fee(const char *stdCode, double price, double qty,
                                 uint32_t offset) {
  CodeHelper::CodeInfo codeInfo =
      CodeHelper::extractStdCode(stdCode, &_hot_mgr);
...
void HisDataReplayer::checkUnbars() {
  for (const std::string &stdCode : _unsubbed_in_need) {
    // 先检查是否已经在未订阅K线中
    bool bHasBars =
        _codes_in_unsubbed.find(stdCode) != _codes_in_unsubbed.end();
    if (bHasBars)
      continue;
    ...
    auto it =
        std::lower_bound(kBlkPair->_bars.begin(), kBlkPair->_bars.end(), bar,
                         [](const WTSBarStruct &a, const WTSBarStruct &b) {
                           return a.time < b.time;
                         });
...
bool HisDataReplayer::cacheRawTicksFromBin(const std::string &key,
                                           const char *stdCode,
                                           uint32_t uDate) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);
  ...
  if (strlen(cInfo._ruletag) > 0) {
    rawCode =
        _hot_mgr.getCustomRawCode(cInfo._ruletag, cInfo.stdCommID(), uDate);
  }
  ...
  if (strlen(ruleTag) > 0) {
    ...
    bHit = _his_dt_mgr.load_raw_ticks(
        cInfo._exchg, wrappCode.c_str(), uDate,
        [&content](std::string &data) { content.swap(data); });
  }
  ...
  if (!bHit) {
     ...
     */
    bHit = _his_dt_mgr.load_raw_ticks(
        cInfo._exchg, rawCode.c_str(), uDate,
        [&content](std::string &data) { content.swap(data); });
  }
...
bool HisDataReplayer::cacheRawOrdDtlFromBin(const std::string &key,
                                            const char *stdCode,
                                            uint32_t uDate) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);

  std::string content;
  bool bHit = _his_dt_mgr.load_raw_orddtl(
      cInfo._exchg, cInfo._code, uDate,
      [&content](std::string &data) { content.swap(data); });
...
bool HisDataReplayer::cacheRawOrdQueFromBin(const std::string &key,
                                            const char *stdCode,
                                            uint32_t uDate) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);

  std::string content;
  bool bHit = _his_dt_mgr.load_raw_ordque(
      cInfo._exchg, cInfo._code, uDate,
      [&content](std::string &data) { content.swap(data); });
...
bool HisDataReplayer::cacheRawTransFromBin(const std::string &key,
                                           const char *stdCode,
                                           uint32_t uDate) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);

  std::string content;
  bool bHit = _his_dt_mgr.load_raw_trans(
      cInfo._exchg, cInfo._code, uDate,
      [&content](std::string &data) { content.swap(data); });
...
bool HisDataReplayer::cacheRawTicksFromLoader(const std::string &key,
                                              const char *stdCode,
                                              uint32_t uDate) {
  if (NULL == _bt_loader)
    return false;
  ...
  bool bSucc = _bt_loader->loadRawHisTicks(
      &dataList, stdCode, uDate,
      [](void *obj, WTSTickStruct *firstItem, uint32_t count) {
  ...
  if (dataList._count > 0)
    WTSLogger::info(
        "{} items of back tick data of {} on {} loaded via extended loader",
        dataList._count, stdCode, uDate);

  return true;
}

bool HisDataReplayer::cacheRawTicksFromCSV(const std::string &key,
                                           const char *stdCode,
                                           uint32_t uDate) {
  if (strlen(stdCode) == 0)
    return false;
  ...
  if (StdFile::exists(filename.c_str())) {
    ...
    if (content.size() < sizeof(HisTickBlockV2)) {
      WTSLogger::error("Sizechecking of back tick data file {} failed",
                       filename);
      return false;
    }
    ...
    WTSLogger::warn("If you want to use tick data in csv mode, you can use "
                    "wtpy.WtDataHelper.store_ticks to generate dsb file",
                    filename.c_str());
    return false;
    ...
    // std::stringstream ss;
    // ss << _base_dir << "csv/ticks/" << stdCode << "_tick_" << uDate <<
    // ".csv"; std::string csvfile = ss.str();
    ...
    //	{
    //		WTSLogger::info("{} items of data loaded",
    //tickList._items.size());
    //	}
    //}
    // tickList._count = tickList._items.size();
    // ifs.close();
    // WTSLogger::info("Data file {} all loaded, totally {} items",
    // csvfile.c_str(), tickList._items.size());
    ...
    // std::string cmpData = WTSCmpHelper::compress_data(tickList._items.data(),
    // sizeof(WTSTickStruct)*tickList._count); pBlk->_size = cmpData.size();
    // content.append(cmpData);

    // StdFile::write_file_content(filename.c_str(), content.c_str(),
    // content.size()); WTSLogger::info("Ticks transfered to file {}",
    // filename.c_str());
  }
...
bool HisDataReplayer::cacheFinalBarsFromLoader(const std::string &key,
                                               const char *stdCode,
  ...
  if (strlen(ruleTag) > 0) {
    ...
    ss << cInfo._exchg << "." << cInfo._code
       << (cInfo._exright == 1 ? SUFFIX_QFQ : SUFFIX_HFQ) << ".dsb";
  } else
    ss << cInfo._code << ".dsb";
  std::string filename = ss.str();
  ...
  if (_bt_loader->isAutoTrans() && StdFile::exists(filename.c_str())) {
    ...
    if (content.size() < sizeof(HisKlineBlockV2)) {
      WTSLogger::error("Sizechecking of back kbar data file {} failed",
                       filename.c_str());
    } else {
      HisKlineBlockV2 *kBlock = (HisKlineBlockV2 *)content.c_str();
      std::string rawData =
          WTSCmpHelper::uncompress_data(kBlock->_data, kBlock->_size);
      ...
      uint64_t stime =
          isDay ? barsList->_bars[0].date : barsList->_bars[0].time;
      uint64_t etime = isDay ? barsList->_bars[barcnt - 1].date
                             : barsList->_bars[barcnt - 1].time;

      WTSLogger::info("{} items of back {} data of {} directly loaded from dsb "
                      "file, from {} to {}",
                      barcnt, pname.c_str(), stdCode, stime, etime);
      bHit = true;
    }
  }
  ...
  if (!bHit) {
    ...
    std::string buffer;
    bool bSucc = _bt_loader->loadFinalHisBars(
        barsList.get(), stdCode, period,
        [](void *obj, WTSBarStruct *firstBar, uint32_t count) {
          BarsList *bars = (BarsList *)obj;
          bars->_count = count;
          bars->_bars.resize(count);
          memcpy((void *)bars->_bars.data(), firstBar,
                 sizeof(WTSBarStruct) * count);
        });
    ...
    uint64_t etime = isDay ? barsList->_bars[barsList->_count - 1].date
                           : barsList->_bars[barsList->_count - 1].time;

    WTSLogger::info("{} items of back {} data of {} loaded via extended "
                    "loader, from {} to {}",
                    barsList->_count, pname.c_str(), stdCode, stime, etime);
    ...
    if (_bt_loader->isAutoTrans()) {
      ...
      std::string cmpData = WTSCmpHelper::compress_data(
          barsList->_bars.data(), sizeof(WTSBarStruct) * barsList->_count);
...
bool HisDataReplayer::cacheRawBarsFromCSV(const std::string &key,
                                          const char *stdCode,
                                          WTSKlinePeriod period,
                                          bool bSubbed /* = true*/) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);
  ...
  if (StdFile::exists(filename.c_str())) {
    ...
    WTSLogger::info("{} items of back {} data of {} directly loaded from dsb "
                    "file, from {} to {}",
                    barcnt, p_suffix.c_str(), stdCode, stime, etime);
  } else {
    // 如果没有格式化的历史数据文件, 则从csv加载
    std::stringstream ss;
    ...
    WTSLogger::info("Reading data from {}, with fields: {}...", csvfile,
                    reader.fields());
    ...
    while (reader.next_row()) {
      ...
      if (period != KP_DAY)
        bs.time = TimeUtils::timeToMinBar(bs.date,
                                          strToTime(reader.get_string("time")));
    ...
    WTSLogger::info("Data file {} all loaded, totally {} items, from {} to {}",
                    csvfile.c_str(), barsList->_bars.size(), stime, etime);
...
bool HisDataReplayer::cacheIntegratedFutBarsFromBin(void *codeInfo,
  ...
  do {
     ...
     */
    std::string content;
    std::string wrappCode =
        StrUtil::printf("%s.%s_%s", cInfo->_exchg, cInfo->_product, ruleTag);
    if (cInfo->isExright())
      wrappCode += cInfo->_exright == 1 ? SUFFIX_QFQ : SUFFIX_HFQ;
    bool bSucc = _his_dt_mgr.load_raw_bars(
        cInfo->_exchg, wrappCode.c_str(), period,
        [&content](std::string &data) { content.swap(data); });

    if (!bSucc) {
      WTSLogger::warn("Loading {} bars of {} via HisDtMgr failed",
                      PERIOD_NAME[period], stdCode);
      break;
    }
    ...
    uint64_t etime =
        isDay ? hotAy->at(barcnt - 1).date : hotAy->at(barcnt - 1).time;

    WTSLogger::info("{} items of back {} data of hot contract {} directly "
                    "loaded, from {} to {}",
                    barcnt, pname.c_str(), stdCode, stime, etime);
  ...
  // if (cInfo.isHot())
  //{
  //	if (!_hot_mgr.splitHotSecions(cInfo._exchg, cInfo._product, 19900102,
  //endTDate, secs)) 		return false;
  // }
  // else if (cInfo.isSecond())
  //{
  //	if (!_hot_mgr.splitSecondSecions(cInfo._exchg, cInfo._product, 19900102,
  //endTDate, secs)) 		return false;
  // }
  if (strlen(ruleTag) > 0) {
    if (!_hot_mgr.splitCustomSections(ruleTag, cInfo->stdCommID(), 19900102,
                                      endTDate, secs))
      return false;
  }
  ...
  for (auto it = secs.rbegin(); it != secs.rend(); it++) {
    ...
    if (period != KP_DAY) {
      ...
      sBar.date = leftDt;
      sBar.time = ((uint32_t)(sTime / 10000) - 19900000) * 10000 +
                  (uint32_t)(sTime % 10000);

      if (sBar.time < lastHotTime) // 如果边界时间小于主力的最后一根Bar的时间,
                                   // 说明已经有交叉了, 则不需要再处理了
      {
        bAllCovered = true;
      ...
      eBar.date = rightDt;
      eBar.time = ((uint32_t)(eTime / 10000) - 19900000) * 10000 +
                  (uint32_t)(eTime % 10000);

      if (eBar.time <= lastHotTime) // 右边界时间小于最后一条Hot时间,
                                    // 说明全部交叉了, 没有再找的必要了
        break;
    } else {
      sBar.date = leftDt;
      if (sBar.date < lastHotTime) // 如果边界时间小于主力的最后一根Bar的时间,
                                   // 说明已经有交叉了, 则不需要再处理了
      {
        bAllCovered = true;
    ...
    if (NULL != _bt_loader) {
      // 分月合约代码
      std::string wCode =
          StrUtil::printf("%s.%s.%s", cInfo->_exchg, cInfo->_product,
                          (char *)curCode + strlen(cInfo->_product));
      bLoaded = _bt_loader->loadRawHisBars(
          &buffer, wCode.c_str(), period,
          [](void *obj, WTSBarStruct *bars, uint32_t count) {
    ...
    if (!bLoaded) {
      bLoaded = _his_dt_mgr.load_raw_bars(
          cInfo->_exchg, curCode, period,
          [&buffer](std::string &data) { buffer.swap(data); });

      if (!bLoaded) {
        WTSLogger::warn("Loading {} bars of {} via HisDtMgr failed",
                        PERIOD_NAME[period], curCode);
        break;
      }
    }
    ...
    WTSBarStruct *pBar = std::lower_bound(
        firstBar, firstBar + (barcnt - 1), sBar,
        [period](const WTSBarStruct &a, const WTSBarStruct &b) {
          if (period == KP_DAY) {
            return a.date < b.date;
          } else {
            return a.time < b.time;
    ...
    if ((period == KP_DAY && pBar->date < sBar.date) ||
        (period != KP_DAY && pBar->time < sBar.time)) // 早于边界时间
    {
      // 早于边界时间, 说明没有数据了,
      // 因为lower_bound会返回大于等于目标位置的数据
      continue;
    }

    pBar = std::lower_bound(
        firstBar + sIdx, firstBar + (barcnt - 1), eBar,
        [period](const WTSBarStruct &a, const WTSBarStruct &b) {
          if (period == KP_DAY) {
            return a.date < b.date;
          } else {
    ...
    if ((period == KP_DAY && pBar->date > eBar.date) ||
        (period != KP_DAY && pBar->time > eBar.time)) {
      pBar--;
      eIdx--;
    }
  ...
  if (realCnt > 0) {
    ...
    for (auto it = barsSections.rbegin(); it != barsSections.rend(); it++) {
      ...
      memcpy(barsList->_bars.data() + curIdx, tempAy->data(),
             tempAy->size() * sizeof(WTSBarStruct));
      curIdx += tempAy->size();
      delete tempAy;
    }
    barsSections.clear();
  ...
  WTSLogger::info("{} items of back {} data of {} cached", realCnt, pname,
                  stdCode);

  return true;
}

const HisDataReplayer::AdjFactorList &
HisDataReplayer::getAdjFactors(const char *code, const char *exchg,
                               const char *pid /* = "" */) {
  ...
  if (it == _adj_factors.end()) {
    ...
    if (_bt_loader) {
      WTSLogger::info(
          "No adjusting factors of {} cached, searching via extented loader...",
          key);
      _bt_loader->loadAdjFactors(
          this, key,
          [](void *obj, const char *stdCode, uint32_t *dates, double *factors,
             uint32_t count) {
            ...
            WTSLogger::info("{} items of adjusting factors of {} loaded via "
                            "extended loader",
                            count, stdCode);
          });
    }
...
bool HisDataReplayer::cacheAdjustedStkBarsFromBin(void *codeInfo,
                                                  const std::string &key,
                                                  const char *stdCode,
                                                  WTSKlinePeriod period,
                                                  bool bSubbed /* = true */) {
  ...
  do {
     ...
     */
    std::string wrappCode = fmt::format(
        "{}{}", cInfo->_code, (cInfo->_exright == 1 ? SUFFIX_QFQ : SUFFIX_HFQ));
    std::string content;
    bool bSucc = _his_dt_mgr.load_raw_bars(
        cInfo->_exchg, wrappCode.c_str(), period,
        [&content](std::string &data) { content.swap(data); });
    ...
    WTSLogger::info("{} items of adjusted back {} data of {} directly loaded",
                    barcnt, PERIOD_NAME[period], stdCode);
  } while (false);
  ...
  do {
    ...
    if (NULL != _bt_loader) {
      std::string wCode =
          StrUtil::printf("%s.%s.%s", cInfo->_exchg, cInfo->_product, curCode);
      bLoaded = _bt_loader->loadRawHisBars(
          &buffer, wCode.c_str(), period,
          [](void *obj, WTSBarStruct *bars, uint32_t count) {
            std::string *buff = (std::string *)obj;
    ...
    if (pBar != NULL) {
      ...
      auto &ayFactors =
          getAdjFactors(cInfo->_code, cInfo->_exchg, cInfo->_product);
      if (!ayFactors.empty()) {
        WTSLogger::info("Adjusting bars of {} with adjusting factors...",
                        stdCode);
        // 做复权处理
        ...
        for (auto it = ayFactors.rbegin(); it != ayFactors.rend(); it++) {
          ...
          pBar = std::lower_bound(
              firstBar, firstBar + lastIdx - 1, bar,
              [period](const WTSBarStruct &a, const WTSBarStruct &b) {
                return a.date < b.date;
              });
        ...
        WTSLogger::info(
            "No adjusting factors of {} found, ajusting task skipped...",
            stdCode);
      }
  ...
  WTSLogger::info("{} items of back {} data of {} cached", realCnt,
                  PERIOD_NAME[period], stdCode);

  return true;
}

bool HisDataReplayer::cacheRawBarsFromBin(const std::string &key,
                                          const char *stdCode,
                                          WTSKlinePeriod period,
                                          bool bSubbed /* = true*/) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, &_hot_mgr);
  ...
  if (!bLoaded) {
    // 读取历史的
    // std::stringstream ss;
    // ss << _base_dir << "his/" << pname << "/" << cInfo._exchg << "/" <<
    // cInfo._code << ".dsb"; std::string filename = ss.str(); if
    // (StdFile::exists(filename.c_str()))
    //{
    //	//如果有格式化的历史数据文件, 则直接读取
    //	std::string content;
    //	StdFile::read_file_content(filename.c_str(), content);
    ...
    //	{
    //		WTSLogger::error("Sizechecking of back kbar data file {}
    //failed", filename.c_str()); 		return false;
    //	}
    ...
    //}
    bLoaded = _his_dt_mgr.load_raw_bars(
        cInfo._exchg, cInfo._code, period,
        [&buffer](std::string &data) { buffer.swap(data); });
