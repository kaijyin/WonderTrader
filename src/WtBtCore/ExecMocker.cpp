...
#define PRICE_DOUBLE_TO_INT(x)                                                 \
  (((x) == DBL_MAX)                                                            \
       ? 0                                                                     \
       : ((x) > 0 ? PRICE_DOUBLE_TO_INT_P(x) : PRICE_DOUBLE_TO_INT_N(x)))

extern uint32_t makeLocalOrderID();

ExecMocker::ExecMocker(HisDataReplayer *replayer)
    : _replayer(replayer), _position(0), _undone(0), _ord_qty(0), _ord_cnt(0),
      _cacl_cnt(0), _cacl_qty(0), _sig_cnt(0), _sig_px(DBL_MAX),
      _last_tick(NULL) {}
...
bool ExecMocker::init(WTSVariant *cfg) {
  ...
  _volmode =
      cfg->getInt32("volmode"); // 数量模式：0-反复正负，-1-一直卖，+1-一直买
  ...
  FuncCreateExeFact creator =
      (FuncCreateExeFact)DLLHelper::get_symbol(hInst, "createExecFact");
  if (creator == NULL) {
  ...
  _factory._remover =
      (FuncDeleteExeFact)DLLHelper::get_symbol(hInst, "deleteExecFact");
...
uint64_t ExecMocker::getCurTime() {
  return TimeUtils::makeTime(_replayer->get_date(),
                             _replayer->get_raw_time() * 100000 +
                                 _replayer->get_secs());
}

void ExecMocker::handle_bar_close(const char *stdCode, const char *period,
...
void ExecMocker::handle_session_end(uint32_t curTDate) {
  ...
  WTSLogger::info("Total entrust:{}, total quantity:{}, total cancels:{}, "
                  "total cancel quantity:{}, total signals:{}",
...
void ExecMocker::handle_init() {
  ...
  WTSKlineSlice *kline =
      _replayer->get_kline_slice(_code.c_str(), basePeriod, 10, times, true);
  if (kline)
    kline->release();

  _replayer->sub_tick(0, _code.c_str());

  _trade_logs << "localid,signaltime,ordertime,bs,sigprice,ordprice,lmtprice,"
                 "tradetime,trdprice,qty,sigtimespan,exectime,cancel"
              << std::endl;

  _exec_unit->on_channel_ready();

  _sig_time =
      (uint64_t)_replayer->get_date() * 10000 + _replayer->get_raw_time();
...
WTSTickSlice *ExecMocker::getTicks(const char *stdCode, uint32_t count,
                                   uint64_t etime /*= 0*/) {
...
double ExecMocker::getPosition(const char *stdCode, bool validOnly /* = true */,
                               int32_t flag /* = 3 */) {
  return _position;
}

OrderMap *ExecMocker::getOrders(const char *stdCode) { return NULL; }

double ExecMocker::getUndoneQty(const char *stdCode) { return _undone; }

OrderIDs ExecMocker::buy(const char *stdCode, double price, double qty,
                         bool bForceClose /* = false */) {
  uint64_t curTime = (uint64_t)_replayer->get_date() * 1000000000 +
...
OrderIDs ExecMocker::cancel(const char *stdCode, bool isBuy,
                            double qty /*= 0*/) {
...
void ExecMocker::handle_entrust(uint32_t localid, const char *stdCode,
                                bool bSuccess, const char *message,
                                uint64_t ordTime) {
  _exec_unit->on_entrust(localid, stdCode, bSuccess, message);
}

void ExecMocker::handle_order(uint32_t localid, const char *stdCode, bool isBuy,
                              double leftover, double price, bool isCanceled,
                              uint64_t ordTime) {
  uint64_t curTime = (uint64_t)_last_tick->actiondate() * 1000000000 +
                     _last_tick->actiontime();
  uint64_t curUnixTime =
      TimeUtils::makeTime(_last_tick->actiondate(), _last_tick->actiontime());

  uint64_t sigUnixTime = TimeUtils::makeTime((uint32_t)(_sig_time / 10000),
                                             _sig_time % 10000 * 100000);
  uint64_t ordUnixTime = TimeUtils::makeTime((uint32_t)(ordTime / 1000000000),
                                             ordTime % 1000000000);
  ...
  if (isCanceled) {
    ...
    _trade_logs << localid << "," << _sig_time << "," << ordTime << ","
                << (isBuy ? "B" : "S") << "," << _sig_px << "," << 0 << ","
                << price << "," << curTime << "," << price << "," << 0 << ","
...
void ExecMocker::handle_trade(uint32_t localid, const char *stdCode, bool isBuy,
                              double vol, double fireprice, double price,
                              uint64_t ordTime) {
  uint64_t curTime = (uint64_t)_last_tick->actiondate() * 1000000000 +
                     _last_tick->actiontime();
  uint64_t curUnixTime =
  ...
  _trade_logs << localid << "," << _sig_time << "," << ordTime << ","
              << (isBuy ? "B" : "S") << "," << _sig_px << "," << fireprice
              << "," << price << "," << curTime << "," << price << "," << vol
