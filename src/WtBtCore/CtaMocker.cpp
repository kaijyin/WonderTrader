...
const char *ACTION_NAMES[] = {"OL", "CL", "OS", "CS", "SYN"};
...
CtaMocker::CtaMocker(HisDataReplayer *replayer, const char *name,
                     int32_t slippage /* = 0 */, bool persistData /* = true */,
                     EventNotifier *notifier /* = NULL */,
                     bool isRatioSlp /* = false */)
    : ICtaStraCtx(name), _replayer(replayer), _total_calc_time(0),
      _emit_times(0), _is_in_schedule(false), _ud_modified(false),
      _strategy(NULL), _slippage(slippage), _ratio_slippage(isRatioSlp),
      _schedule_times(0), _total_closeprofit(0), _notifier(notifier),
      _has_hook(false), _hook_valid(true), _cur_step(0), _wait_calc(false),
      _in_backtest(false), _persist_data(persistData) {
...
void CtaMocker::dump_stradata() {
  ...
  { // 持仓数据保存
    ...
    for (auto it = _pos_map.begin(); it != _pos_map.end(); it++) {
      ...
      for (auto dit = pInfo._details.begin(); dit != pInfo._details.end();
           dit++) {
        ...
        dItem.AddMember("opentag", rj::Value(dInfo._opentag, allocator),
                        allocator);
  ...
  { // 信号保存
    ...
    for (auto &m : _sig_map) {
      ...
      jItem.AddMember("usertag", rj::Value(sInfo._usertag.c_str(), allocator),
                      allocator);
  ...
  { // 条件单保存
    ...
    for (auto it = _condtions.begin(); it != _condtions.end(); it++) {
      ...
      for (auto &condInfo : condList) {
        ...
        cItem.AddMember("usertag", rj::Value(condInfo._usertag, allocator),
                        allocator);
...
void CtaMocker::dump_chartdata() {
  ...
  if (_chart_code.empty()) {
    // 如果没有设置主K线，就用主K线落地
    klineItem.AddMember("code", rj::Value(_main_code.c_str(), allocator),
                        allocator);
    klineItem.AddMember("period", rj::Value(_main_period.c_str(), allocator),
                        allocator);
  } else {
    klineItem.AddMember("code", rj::Value(_chart_code.c_str(), allocator),
                        allocator);
    klineItem.AddMember("period", rj::Value(_chart_period.c_str(), allocator),
                        allocator);
  }
  ...
  if (!_chart_indice.empty()) {
    ...
    for (const auto &v : _chart_indice) {
      ...
      jIndex.AddMember("name", rj::Value(cIndex._name.c_str(), allocator),
                       allocator);
      ...
      for (const auto &v2 : cIndex._lines) {
        ...
        jLine.AddMember("name", rj::Value(cLine._name.c_str(), allocator),
                        allocator);
      ...
      for (const auto &v3 : cIndex._base_lines) {
        jBaseLines.AddMember(rj::Value(v3.first.c_str(), allocator),
                             rj::Value(v3.second), allocator);
      }
  ...
  //		jMark.AddMember("icon", rj::Value(mark._icon.c_str(),
  //allocator), allocator); 		jMark.AddMember("tag", rj::Value(mark._tag.c_str(),
  //allocator), allocator);
  ...
  if (_persist_data) {
    ...
    StdFile::write_file_content(filename.c_str(), (void *)content.c_str(),
                                content.size());
...
void CtaMocker::dump_outputs() {
...

  {
    ...
    for (auto it = _user_datas.begin(); it != _user_datas.end(); it++) {
      root.AddMember(rj::Value(it->first.c_str(), allocator),
                     rj::Value(it->second.c_str(), allocator), allocator);
    }
...
void CtaMocker::log_signal(const char *stdCode, double target, double price,
                           uint64_t gentime, const char *usertag /* = "" */) {
  _sig_logs << stdCode << "," << target << "," << price << "," << gentime << ","
            << usertag << "\n";
}

void CtaMocker::log_trade(const char *stdCode, bool isLong, bool isOpen,
                          uint64_t curTime, double price, double qty,
                          const char *userTag, double fee, uint32_t barNo) {
  _trade_logs << stdCode << "," << curTime << "," << (isLong ? "LONG" : "SHORT")
              << "," << (isOpen ? "OPEN" : "CLOSE") << "," << price << ","
              << qty << "," << userTag << "," << fee << "," << barNo << "\n";
}

void CtaMocker::log_close(const char *stdCode, bool isLong, uint64_t openTime,
                          double openpx, uint64_t closeTime, double closepx,
                          double qty, double profit, double maxprofit,
                          double maxloss, double totalprofit /* = 0 */,
                          const char *enterTag /* = "" */,
                          const char *exitTag /* = "" */,
                          uint32_t openBarNo /* = 0 */,
                          uint32_t closeBarNo /* = 0 */) {
  _close_logs << stdCode << "," << (isLong ? "LONG" : "SHORT") << ","
              << openTime << "," << openpx << "," << closeTime << "," << closepx
              << "," << qty << "," << profit << "," << maxprofit << ","
              << maxloss << "," << totalprofit << "," << enterTag << ","
              << exitTag << "," << openBarNo << "," << closeBarNo << "\n";
}
...
bool CtaMocker::init_cta_factory(WTSVariant *cfg) {
  ...
  FuncCreateStraFact creator =
      (FuncCreateStraFact)DLLHelper::get_symbol(hInst, "createStrategyFact");
  if (creator == NULL) {
  ...
  _factory._remover =
      (FuncDeleteStraFact)DLLHelper::get_symbol(hInst, "deleteStrategyFact");
  _factory._fact = _factory._creator();

  WTSVariant *cfgStra = cfg->get("strategy");
  if (cfgStra) {
    _strategy = _factory._fact->createStrategy(cfgStra->getCString("name"),
                                               cfgStra->getCString("id"));
    if (_strategy) {
      WTSLogger::info("Strategy {}.{} is created,strategy ID: {}",
                      _factory._fact->getName(), _strategy->getName(),
                      _strategy->id());
    }
...
void CtaMocker::load_incremental_data(const char *incremental_backtest_base) {
  ...
  std::string strategyDumpFilename =
      folder + fmtutil::format("{}.json", incremental_backtest_base);
  if (boost::filesystem::exists(strategyDumpFilename)) {
      ...
      for (rj::SizeType i = 0; i < positions.Size(); i++) {
        ...
        if (positionEntry.HasMember("details")) {
          ...
          for (rj::SizeType j = 0; j < details.Size(); j++) {
            ...
            curPosDetail._max_price =
                positionDetailEntry["maxprice"].GetDouble();
            curPosDetail._min_price =
                positionDetailEntry["minprice"].GetDouble();
            curPosDetail._volume = positionDetailEntry["volume"].GetDouble();
            curPosDetail._opentime =
                positionDetailEntry["opentime"].GetUint64();
            curPosDetail._opentdate = positionDetailEntry["opentdate"].GetInt();
            curPosDetail._profit = positionDetailEntry["profit"].GetDouble();
            curPosDetail._max_profit =
                positionDetailEntry["maxprofit"].GetDouble();
            curPosDetail._max_loss = positionDetailEntry["maxloss"].GetDouble();
            strcpy(curPosDetail._opentag,
                   positionDetailEntry["opentag"].GetString());
    ...
    if (d.HasMember("signals")) {
      for (rj::Value::ConstMemberIterator itr = d["signals"].MemberBegin();
           itr != d["signals"].MemberEnd(); ++itr) {
    ...
    if (d.HasMember("conditions") && d["conditions"].HasMember("items")) {
      ...
      for (rj::Value::ConstMemberIterator itr =
               conditionItemsEntry.MemberBegin();
           itr != conditionItemsEntry.MemberEnd(); ++itr) {
        std::string stkCode = itr->name.GetString();
        for (rj::SizeType i = 0; i < itr->value.Size(); i++) {
          const rj::Value &conditionItemStkCondEntry = itr->value[i];
          CondEntrust condEntrust;
          strcpy(condEntrust._usertag,
                 conditionItemStkCondEntry["usertag"].GetString());
          condEntrust._field =
              (WTSCompareField)conditionItemStkCondEntry["field"].GetInt();
          condEntrust._alg =
              (WTSCompareType)conditionItemStkCondEntry["alg"].GetInt();
          condEntrust._target = conditionItemStkCondEntry["target"].GetDouble();
          condEntrust._qty = conditionItemStkCondEntry["qty"].GetDouble();
          condEntrust._action =
              (char)conditionItemStkCondEntry["action"].GetUint();
    ...
    WTSLogger::warn("fail load incremental data json: {}",
                    strategyDumpFilename);
  }
}
...
void CtaMocker::handle_bar_close(const char *stdCode, const char *period,
...
void CtaMocker::handle_replay_done() {
  ...
  if (_emit_times > 0) {
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_INFO,
                       "Strategy has been scheduled {} times, totally taking "
                       "{} us, {:.3f} us each time",
                       _emit_times, _total_calc_time,
                       _total_calc_time * 1.0 / _emit_times);
  } else {
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_INFO,
  ...
  if (_has_hook && _hook_valid) {
    WTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_DEBUG,
                           "Replay done, notify control thread");
    while (_wait_calc)
      _cond_calc.notify_all();
    WTSLogger::log_dyn_raw("strategy", _name.c_str(), LL_DEBUG,
                           "Notify control thread the end done");
  }

  WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG,
                     "Notify strategy the end of backtest");
  this->on_bactest_end();
}
...
void CtaMocker::proc_tick(const char *stdCode, double last_px, double cur_px) {
  {
    ...
    if (it != _sig_map.end()) {
      // if (sInfo->isInTradingTime(_replayer->get_raw_time(), true))
      {
        ...
        if (sInfo._sigtype == 2)
          on_condition_triggered(stdCode, sInfo._volume, cur_px,
  ...
  if (!_condtions.empty()) {
      ...
      if (!_replayer->is_tick_simulated()) {
        ...
        case WCT_Equal:
          isMatched = decimal::le(left_px, entrust._target) &&
                      decimal::ge(right_px, entrust._target);
          break;
        case WCT_Larger:
        ...
        if (isMatched) {
           ...
           * 2
           * alg一样的调价单，如果是WCT_Larger与WCT_LargerOrEqual，取触发价较小的，WCT_Smaller与WCT_SmallerOrEqual，取触发价较大的
           */
          if (matchedEntrust == NULL) {
            matchedEntrust = &entrust;
            if (entrust._alg == WCT_Larger || entrust._alg == WCT_LargerOrEqual)
              curPrice = max(left_px, entrust._target);
            else if (entrust._alg == WCT_Smaller ||
    ...
    if (matchedEntrust != NULL) {
      ...
      //_replayer->is_tick_enabled() ? newTick->price() : entrust._target;
      ////如果开启了tick回测,则用tick数据的价格,如果没有开启,则只能用条件单价格
      WTSLogger::log_dyn(
          "strategy", _name.c_str(), LL_INFO,
          "Condition order triggered[newprice: {}{}{}], instrument: {}, {} {}",
          curPrice, CMP_ALG_NAMES[entrust._alg], entrust._target, stdCode,
          ACTION_NAMES[entrust._action], entrust._qty);
...
void CtaMocker::handle_tick(const char *stdCode, WTSTickData *newTick,
                            uint32_t pxType /* = 0 */) {
...
//////////////////////////////////////////////////////////////////////////
// 回调函数
void CtaMocker::on_bar(const char *stdCode, const char *period, uint32_t times,
                       WTSBarStruct *newBar) {
  if (newBar == NULL)
    return;
...
void CtaMocker::on_init() {
  ...
  WTSLogger::info("CTA Strategy initialized with {} slippage: {}",
                  _ratio_slippage ? "ratio" : "absolute", _slippage);
}
...
void CtaMocker::update_dyn_profit(const char *stdCode, double price) {
  ...
  if (it != _pos_map.end()) {
    ...
    if (pInfo._volume == 0) {
      ...
      double dynprofit = 0;
      for (auto pit = pInfo._details.begin(); pit != pInfo._details.end();
           pit++) {
        DetailInfo &dInfo = *pit;
        dInfo._profit = dInfo._volume * (price - dInfo._price) *
                        commInfo->getVolScale() * (dInfo._long ? 1 : -1);
        if (dInfo._profit > 0)
...
void CtaMocker::on_tick(const char *stdCode, WTSTickData *newTick,
                        bool bEmitStrategy /* = true */) {
  // 这个逻辑全部迁移到handle_tick里去了
}

void CtaMocker::on_bar_close(const char *code, const char *period,
                             WTSBarStruct *newBar) {
  if (_strategy)
...
void CtaMocker::enable_hook(bool bEnabled /* = true */) {
  ...
  WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG, "Calculating hook {}",
...
bool CtaMocker::step_calc() {
  ...
  if (bNotify)
    WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG,
                       "Notify calc thread, wait for calc done");
  ...
  for (auto it = _kline_tags.begin(); it != _kline_tags.end(); it++) {
    ...
    if (isMainUdt || _kline_tags.empty()) {
      ...
      if (offTime <= sInfo->getCloseTime(true)) {
        ...
        if (_has_hook && _hook_valid) {
          WTSLogger::log_dyn("strategy", _name.c_str(), LL_DEBUG,
                             "Calc done, notify control thread");
          while (_cur_step == 1)
            _cond_calc.notify_all();
        ...
        WTSLogger::log_dyn(
            "strategy", _name.c_str(), LL_INFO,
            "{} is not trading time,strategy will not be scheduled", curTime);
      }
      break;
    }
  }
...
void CtaMocker::on_session_begin(uint32_t curTDate) {
  ...
  for (auto &it : _pos_map) {
    ...
    if (!decimal::eq(pInfo._frozen, 0)) {
      log_debug("{} of {} frozen released on {}", pInfo._frozen, stdCode,
                curTDate);
      pInfo._frozen = 0;
    }
  }
...
void CtaMocker::on_session_end(uint32_t curTDate) {
  ...
  for (auto it = _pos_map.begin(); it != _pos_map.end(); it++) {
    ...
    _pos_logs << fmt::format("{},{},{},{:.2f},{:.2f}\n", curDate, stdCode,
                             pInfo._volume, pInfo._closeprofit,
                             pInfo._dynprofit);
  }

  _fund_logs << fmt::format(
      "{},{:.2f},{:.2f},{:.2f},{:.2f}\n", curDate, _fund_info._total_profit,
      _fund_info._total_dynprofit,
      _fund_info._total_profit + _fund_info._total_dynprofit -
          _fund_info._total_fees,
      _fund_info._total_fees);

  if (_notifier)
    _notifier->notifyFund("BT_FUND", curDate, _fund_info._total_profit,
                          _fund_info._total_dynprofit,
                          _fund_info._total_profit +
                              _fund_info._total_dynprofit -
                              _fund_info._total_fees,
                          _fund_info._total_fees);
}
...
//////////////////////////////////////////////////////////////////////////
// 策略接口
void CtaMocker::stra_enter_long(const char *stdCode, double qty,
                                const char *userTag /* = "" */,
  ...
  if (decimal::eq(limitprice, 0.0) &&
      decimal::eq(stopprice, 0.0)) // 如果不是动态下单模式,则直接触发
  {
    double curQty = stra_get_position(stdCode);
    if (decimal::lt(curQty, 0))
      append_signal(stdCode, qty, userTag, 0.0, _is_in_schedule ? 0 : 1);
    else
      append_signal(stdCode, curQty + qty, userTag, 0.0,
                    _is_in_schedule ? 0 : 1);
  } else {
...
void CtaMocker::stra_enter_short(const char *stdCode, double qty,
                                 const char *userTag /* = "" */,
                                 double limitprice, double stopprice) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
...
void CtaMocker::stra_exit_long(const char *stdCode, double qty,
                               const char *userTag /* = "" */,
                               double limitprice, double stopprice) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
      ...
      decimal::eq(stopprice, 0.0)) // 如果不是动态下单模式,则直接触发
  {
    ...
    append_signal(stdCode, totalQty - maxQty, userTag, 0.0,
                  _is_in_schedule ? 0 : 1);
  } else {
    CondList &condList = get_cond_entrusts(stdCode);
...
void CtaMocker::stra_exit_short(const char *stdCode, double qty,
                                const char *userTag /* = "" */,
                                double limitprice, double stopprice) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
      ...
      decimal::eq(stopprice, 0.0)) // 如果不是动态下单模式,则直接触发
  {
    ...
    append_signal(stdCode, curQty + maxQty, userTag, 0.0,
                  _is_in_schedule ? 0 : 1);
  } else {
    CondList &condList = get_cond_entrusts(stdCode);
...
void CtaMocker::stra_set_position(const char *stdCode, double qty,
                                  const char *userTag /* = "" */,
  ...
  if (commInfo->isT1()) {
    ...
    if (decimal::lt(qty, frozen)) {
      WTSLogger::log_dyn(
          "strategy", _name.c_str(), LL_ERROR,
          "New position of {} cannot be set to {} due to {} being frozen",
          stdCode, qty, frozen);
      return;
    }
  }
  ...
  _replayer->sub_tick(_context_id, stdCode);
  if (decimal::eq(limitprice, 0.0) &&
      decimal::eq(stopprice, 0.0)) // 没有设置触发条件，则直接添加信号
  {
    append_signal(stdCode, qty, userTag, 0.0, _is_in_schedule ? 0 : 1);
  } else {
    CondList &condList = get_cond_entrusts(stdCode);
...
void CtaMocker::append_signal(const char *stdCode, double qty,
                              const char *userTag /* = "" */,
                              double price /* = 0.0 */,
                              uint32_t sigType /* = 0 */) {
  ...
  sInfo._gentime = (uint64_t)_replayer->get_date() * 1000000000 +
                   (uint64_t)_replayer->get_raw_time() * 100000 +
                   _replayer->get_secs();
...
void CtaMocker::do_set_position(const char *stdCode, double qty,
                                double price /* = 0.0 */,
                                const char *userTag /* = "" */) {
  ...
  uint64_t curTm =
      (uint64_t)_replayer->get_date() * 10000 + _replayer->get_min_time();
  ...
  if (decimal::gt(pInfo._volume * diff,
                  0)) // 当前持仓和仓位变化方向一致, 增加一条明细, 增加数量即可
  {
    pInfo._volume = qty;
    ...
    for (auto it = pInfo._details.begin(); it != pInfo._details.end(); it++) {
      ...
      pInfo._dynprofit = pInfo._dynprofit * dInfo._volume /
                         (dInfo._volume + maxQty); // 浮盈也要做等比缩放
      pInfo._last_exittime = curTm;
      _fund_info._total_profit += profit;

      double fee = _replayer->calc_fee(stdCode, trdPx, maxQty,
                                       dInfo._opentdate == curTDate ? 2 : 1);
      _fund_info._total_fees += fee;
      // 这里写成交记录
      ...
      log_close(stdCode, dInfo._long, dInfo._opentime, dInfo._price, curTm,
                trdPx, maxQty, profit, maxProf, maxLoss,
                _total_closeprofit - _fund_info._total_fees, dInfo._opentag,
                userTag, dInfo._open_barno, _schedule_times);
    ...
    if (left > 0) {
      ...
      _fund_info._total_fees += fee;
      log_trade(stdCode, dInfo._long, true, curTm, trdPx, abs(left), userTag,
                fee, _schedule_times);
...
WTSKlineSlice *CtaMocker::stra_get_bars(const char *stdCode, const char *period,
                                        uint32_t count,
                                        bool isMain /* = false */) {
  thread_local static char key[64] = {0};
  ...
  WTSKlineSlice *kline =
      _replayer->get_kline_slice(stdCode, basePeriod, count, times, isMain);
  ...
  if (kline) {
    ...
    CodeHelper::CodeInfo cInfo =
        CodeHelper::extractStdCode(stdCode, _replayer->get_hot_mgr());
    WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
...
uint32_t CtaMocker::stra_get_tdate() { return _replayer->get_trading_date(); }

uint32_t CtaMocker::stra_get_date() { return _replayer->get_date(); }

uint32_t CtaMocker::stra_get_time() { return _replayer->get_min_time(); }

double CtaMocker::stra_get_fund_data(int flag) {
  switch (flag) {
  case 0:
    return _fund_info._total_profit - _fund_info._total_fees +
           _fund_info._total_dynprofit;
  case 1:
...
const char *CtaMocker::stra_load_user_data(const char *key,
                                           const char *defVal /*= ""*/) {
...
double CtaMocker::stra_get_position(const char *stdCode,
                                    bool bOnlyValid /* = false */,
                                    const char *userTag /* = "" */) {
...
uint64_t CtaMocker::stra_get_detail_entertime(const char *stdCode,
                                              const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double CtaMocker::stra_get_detail_cost(const char *stdCode,
                                       const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double CtaMocker::stra_get_detail_profit(const char *stdCode,
                                         const char *userTag,
                                         int flag /* = 0 */) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
...
void CtaMocker::add_chart_mark(double price, const char *icon,
                               const char *tag) {
  if (!_is_in_schedule) {
  ...
  _mark_logs << curTime << "," << price << "," << icon << "," << tag
             << std::endl;
}
...
bool CtaMocker::register_index_line(const char *idxName, const char *lineName,
                                    uint32_t lineType) {
...
bool CtaMocker::add_index_baseline(const char *idxName, const char *lineName,
                                   double val) {
  auto it = _chart_indice.find(idxName);
  if (it == _chart_indice.end()) {
    WTSLogger::error("Index {} not registered", idxName);
...
bool CtaMocker::set_index_value(const char *idxName, const char *lineName,
                                double val) {
  if (!_is_in_schedule) {
    WTSLogger::error("Marks can be added only during schedule");
    return false;
  ...
  uint64_t curTime = _replayer->get_date();
  curTime = curTime * 10000 + _replayer->get_min_time();
  _index_logs << curTime << "," << idxName << "," << lineName << "," << val
              << std::endl;
  return true;
}
