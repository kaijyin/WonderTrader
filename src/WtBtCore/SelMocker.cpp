...
SelMocker::SelMocker(HisDataReplayer *replayer, const char *name,
                     int32_t slippage /* = 0 */, bool isRatioSlp /* = false */)
    : ISelStraCtx(name), _replayer(replayer), _total_calc_time(0),
...
void SelMocker::log_signal(const char *stdCode, double target, double price,
                           uint64_t gentime, const char *usertag /* = "" */) {
  _sig_logs << stdCode << "," << target << "," << price << "," << gentime << ","
            << usertag << "\n";
}

void SelMocker::log_trade(const char *stdCode, bool isLong, bool isOpen,
                          uint64_t curTime, double price, double qty,
                          const char *userTag, double fee) {
  _trade_logs << stdCode << "," << curTime << "," << (isLong ? "LONG" : "SHORT")
              << "," << (isOpen ? "OPEN" : "CLOSE") << "," << price << ","
              << qty << "," << userTag << "," << fee << "\n";
}

void SelMocker::log_close(const char *stdCode, bool isLong, uint64_t openTime,
...
bool SelMocker::init_sel_factory(WTSVariant *cfg) {
  ...
  FuncCreateSelStraFact creator = (FuncCreateSelStraFact)DLLHelper::get_symbol(
      hInst, "createSelStrategyFact");
  if (creator == NULL) {
  ...
  _factory._remover = (FuncDeleteSelStraFact)DLLHelper::get_symbol(
      hInst, "deleteSelStrategyFact");
  ...
  if (cfgStra) {
    ...
    if (_strategy) {
      WTSLogger::info("Strategy {}.{} created,strategy ID: {}",
...
//////////////////////////////////////////////////////////////////////////
// IDataSink
void SelMocker::handle_init() { this->on_init(); }

void SelMocker::handle_bar_close(const char *stdCode, const char *period,
...
void SelMocker::handle_replay_done() {
  WTSLogger::log_dyn("strategy", _name.c_str(), LL_INFO,
                     "Strategy has been scheduled for {} times,totally taking "
                     "{} microsecs,average of {} microsecs",
                     _emit_times, _total_calc_time,
                     _total_calc_time / _emit_times);
...
void SelMocker::handle_tick(const char *stdCode, WTSTickData *newTick,
                            uint32_t pxType) {
  ...
  _price_map[stdCode].second =
      (uint64_t)newTick->actiondate() * 1000000000 + newTick->actiontime();
...
//////////////////////////////////////////////////////////////////////////
// 回调函数
void SelMocker::on_bar(const char *stdCode, const char *period, uint32_t times,
                       WTSBarStruct *newBar) {
  if (newBar == NULL)
    return;
...
void SelMocker::on_init() {
  ...
  WTSLogger::info("SEL Strategy initialized with {} slippage: {}",
...
void SelMocker::on_tick(const char *stdCode, WTSTickData *newTick,
                        bool bEmitStrategy /* = true */) {
  // By Wesley @ 2022.04.19
  // 这个逻辑迁移到handle_tick去了
}

void SelMocker::on_bar_close(const char *code, const char *period,
                             WTSBarStruct *newBar) {
  if (_strategy)
...
bool SelMocker::on_schedule(uint32_t curDate, uint32_t curTime,
                            uint32_t fireTime) {
  ...
  for (auto &v : _pos_map) {
    ...
    if (_sig_map.find(code) == _sig_map.end() &&
        !decimal::eq(pInfo._volume, 0.0)) {
      // 新的信号中没有该持仓,则要清空
...
void SelMocker::stra_set_position(const char *stdCode, double qty,
                                  const char *userTag /* = "" */) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
...
void SelMocker::append_signal(const char *stdCode, double qty,
                              const char *userTag /* = "" */,
                              double price /* = 0.0*/) {
...
void SelMocker::do_set_position(const char *stdCode, double qty,
                                double price /* = 0.0 */,
                                const char *userTag /* = "" */,
                  ...
                  0)) // 当前持仓和仓位变动方向一致,增加一条明细,增加数量即可
  {
    ...
    log_trade(stdCode, dInfo._long, true, curTm, trdPx, abs(diff), userTag,
              fee);
    ...
    for (auto it = pInfo._details.begin(); it != pInfo._details.end(); it++) {
      ...
      _fund_info._total_fees += fee;
      // 这里写成交记录
      log_trade(stdCode, dInfo._long, false, curTm, trdPx, maxQty, userTag,
                fee);
      // 这里写平仓记录
      log_close(stdCode, dInfo._long, dInfo._opentime, dInfo._price, curTm,
                trdPx, maxQty, profit, dInfo._max_profit, dInfo._max_loss,
                pInfo._closeprofit, dInfo._opentag, userTag, dInfo._open_barno,
                _schedule_times);
...
WTSKlineSlice *SelMocker::stra_get_bars(const char *stdCode, const char *period,
                                        uint32_t count) {
  ...
  WTSKlineSlice *kline =
      _replayer->get_kline_slice(stdCode, basePeriod, count, times, false);
...
uint32_t SelMocker::stra_get_tdate() { return _replayer->get_trading_date(); }

uint32_t SelMocker::stra_get_date() { return _replayer->get_date(); }

uint32_t SelMocker::stra_get_time() { return _replayer->get_min_time(); }
...
const char *SelMocker::stra_load_user_data(const char *key,
                                           const char *defVal /*= ""*/) {
...
double SelMocker::stra_get_position(const char *stdCode,
...
uint64_t SelMocker::stra_get_detail_entertime(const char *stdCode,
                                              const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double SelMocker::stra_get_detail_cost(const char *stdCode,
                                       const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double SelMocker::stra_get_detail_profit(const char *stdCode,
                                         const char *userTag,
                                         int flag /* = 0 */) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
