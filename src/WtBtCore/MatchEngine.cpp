...
void MatchEngine::clear() { _orders.clear(); }
...
void MatchEngine::fire_orders(const char *stdCode, OrderIDs &to_erase) {
  for (auto &v : _orders) {
    ...
    if (ordInfo._state == 0) // 需要激活
    {
      ...
      _sink->handle_order(localid, stdCode, ordInfo._buy, ordInfo._left,
                          ordInfo._limit, false, ordInfo._time);
      ordInfo._state = 1;
    }
  }
}
...
void MatchEngine::match_orders(WTSTickData *curTick, OrderIDs &to_erase) {
  ...
  uint64_t curUnixTime =
      TimeUtils::makeTime(curTick->actiondate(), curTick->actiontime());
  ...
  for (auto &v : _orders) {
    ...
    {
      _sink->handle_order(localid, ordInfo._code, ordInfo._buy, 0,
                          ordInfo._limit, true, ordInfo._time);
      ordInfo._state = 99;

      to_erase.emplace_back(localid);

      WTSLogger::info("订单{}已撤销, 剩余数量: {}", localid,
                      ordInfo._left * (ordInfo._buy ? 1 : -1));
      ordInfo._left = 0;
      continue;
    }
    ...
    if (ordInfo._buy) {
      ...
      if (decimal::le(price, ordInfo._limit)) {
        ...
        _sink->handle_trade(localid, ordInfo._code, ordInfo._buy, qty,
                            ordInfo._price, price, ordInfo._time);

        ordInfo._traded += qty;
        ordInfo._left -= qty;

        _sink->handle_order(localid, ordInfo._code, ordInfo._buy, ordInfo._left,
                            price, false, ordInfo._time);
...
void MatchEngine::update_lob(WTSTickData *curTick) {
  ...
  for (uint32_t i = 0; i < 10; i++) {
    if (PRICE_DOUBLE_TO_INT(curTick->askprice(i)) == 0 &&
        PRICE_DOUBLE_TO_INT(curTick->bidprice(i)) == 0)
      break;
...
OrderIDs MatchEngine::buy(const char *stdCode, double price, double qty,
                          uint64_t curTime) {
  ...
  if (decimal::eq(price, lastTick->price()))
    ordInfo._queue =
        (uint32_t)round((lastTick->askqty(0) * lastTick->askprice(0) +
                         lastTick->bidqty(0) * lastTick->bidprice(0)) /
                        (lastTick->askprice(0) + lastTick->bidprice(0)));
...
OrderIDs MatchEngine::sell(const char *stdCode, double price, double qty,
                           uint64_t curTime) {
  WTSTickData *lastTick = grab_last_tick(stdCode);
  if (lastTick == NULL)
    return OrderIDs();
...
OrderIDs MatchEngine::cancel(const char *stdCode, bool isBuy, double qty,
                             FuncCancelCallback cb) {
  OrderIDs ret;
  for (auto &v : _orders) {
    OrderInfo &ordInfo = (OrderInfo &)v.second;
    if (ordInfo._state != 1)
