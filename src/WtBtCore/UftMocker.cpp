...
const char *OFFSET_NAMES[] = {"OPEN", "CLOSE", "CLOSET"};

extern std::vector<uint32_t> splitVolume(uint32_t vol);
extern std::vector<double> splitVolume(double vol, double minQty = 1.0,
                                       double maxQty = 100.0,
                                       double qtyTick = 1.0);
...
UftMocker::UftMocker(HisDataReplayer *replayer, const char *name)
    : IUftStraCtx(name), _replayer(replayer), _strategy(NULL),
      _use_newpx(false), _error_rate(0), _match_this_tick(false) {
...
bool UftMocker::init_uft_factory(WTSVariant *cfg) {
  ...
  log_info("UFT match params: use_newpx-{}, error_rate-{}, match_this_tick-{}",
           _use_newpx, _error_rate, _match_this_tick);

  DllHandle hInst = DLLHelper::load_library(module);
  if (hInst == NULL)
    return false;

  FuncCreateUftStraFact creator =
      (FuncCreateUftStraFact)DLLHelper::get_symbol(hInst, "createStrategyFact");
  if (creator == NULL) {
  ...
  _factory._remover =
      (FuncDeleteUftStraFact)DLLHelper::get_symbol(hInst, "deleteStrategyFact");
  _factory._fact = _factory._creator();

  WTSVariant *cfgStra = cfg->get("strategy");
  if (cfgStra) {
    _strategy =
        _factory._fact->createStrategy(cfgStra->getCString("name"), "uft");
...
void UftMocker::handle_order_detail(const char *stdCode,
                                    WTSOrdDtlData *curOrdDtl) {
  on_order_detail(stdCode, curOrdDtl);
}

void UftMocker::handle_order_queue(const char *stdCode,
                                   WTSOrdQueData *curOrdQue) {
  on_order_queue(stdCode, curOrdQue);
}

void UftMocker::handle_transaction(const char *stdCode,
                                   WTSTransData *curTrans) {
  on_transaction(stdCode, curTrans);
}

void UftMocker::handle_bar_close(const char *stdCode, const char *period,
...
void UftMocker::on_bar(const char *stdCode, const char *period, uint32_t times,
                       WTSBarStruct *newBar) {
  if (_strategy)
...
void UftMocker::on_ordque_updated(const char *stdCode,
                                  WTSOrdQueData *newOrdQue) {
  if (_strategy)
...
void UftMocker::on_orddtl_updated(const char *stdCode,
                                  WTSOrdDtlData *newOrdDtl) {
  if (_strategy)
...
uint32_t UftMocker::id() { return _context_id; }
...
void UftMocker::on_session_begin(uint32_t curTDate) {
  ...
  for (auto &it : _pos_map) {
    ...
    if (!decimal::eq(pInfo.frozen(), 0)) {
      log_debug("{} frozen of {} released on {}", pInfo.frozen(), stdCode,
                curTDate);
  ...
  for (auto it = _pos_map.begin(); it != _pos_map.end(); it++) {
    ...
    if (!decimal::eq(pInfo._long.volume(), 0.0))
      _pos_logs << fmt::format("{},{},LONG,{},{:.2f},{:.2f}\n", curTDate,
                               stdCode, pInfo._long.volume(),
                               pInfo._long._closeprofit,
                               pInfo._long._dynprofit);

    if (!decimal::eq(pInfo._short.volume(), 0.0))
      _pos_logs << fmt::format("{},{},LONG,{},{:.2f},{:.2f}\n", curTDate,
                               stdCode, pInfo._short.volume(),
                               pInfo._short._closeprofit,
                               pInfo._short._dynprofit);
  }
  ...
  postTask([this, localid]() {
    ...
    if (ordInfo._offset != 0) {
      ...
      if (commInfo->getCoverMode() == CM_CoverToday) {
        ...
        // 如果不分平昨平今，则先释放今仓
        double maxQty =
            std::min(ordInfo._left, pItem._newvol - pItem._newavail);
        pItem._newavail += maxQty;
        pItem._preavail += ordInfo._left - maxQty;
      }
    }

    log_debug("Order {} canceled, action: {} {} @ {}({})", ordInfo._localid,
              OFFSET_NAMES[ordInfo._offset], ordInfo._isLong ? "long" : "short",
              ordInfo._total, ordInfo._left);
    ordInfo._left = 0;
    on_order(localid, ordInfo._code, ordInfo._isLong, ordInfo._offset,
             ordInfo._total, ordInfo._left, ordInfo._price, true);
    _orders.erase(it);
  });
...
OrderIDs UftMocker::stra_buy(const char *stdCode, double price, double qty,
                             int flag /* = 0 */) {
...
OrderIDs UftMocker::stra_sell(const char *stdCode, double price, double qty,
                              int flag /* = 0 */) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
    log_error("Cannot find corresponding commodity info of {}", stdCode);
...
uint32_t UftMocker::stra_enter_long(const char *stdCode, double price,
                                    double qty, int flag /* = 0 */) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
    log_error("Cannot find corresponding commodity info of {}", stdCode);
    return 0;
  ...
  postTask([this, localid]() {
    ...
    log_debug("order placed: open long of {} @ {} by {}", ordInfo._code,
              ordInfo._price, ordInfo._total);
...
uint32_t UftMocker::stra_enter_short(const char *stdCode, double price,
                                     double qty, int flag /* = 0 */) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
    log_error("Cannot find corresponding commodity info of {}", stdCode);
  ...
  postTask([this, localid]() {
    const OrderInfo &ordInfo = _orders[localid];
    log_debug("order placed: open short of {} @ {} by {}", ordInfo._code,
              ordInfo._price, ordInfo._total);
    on_entrust(localid, ordInfo._code, true, "entrust success");
  });
...
uint32_t UftMocker::stra_exit_long(const char *stdCode, double price,
  ...
  postTask([this, localid]() {
    const OrderInfo &ordInfo = _orders[localid];
    log_debug("order placed: {} long of {} @ {} by {}",
              OFFSET_NAMES[ordInfo._offset], ordInfo._code, ordInfo._price,
              ordInfo._total);
    on_entrust(localid, ordInfo._code, true, "entrust success");
  });
...
uint32_t UftMocker::stra_exit_short(const char *stdCode, double price,
                                    double qty, bool isToday /* = false */,
                                    int flag /* = 0 */) {
  PosInfo &pInfo = _pos_map[stdCode];
  ...
  postTask([this, localid]() {
    const OrderInfo &ordInfo = _orders[localid];
    log_debug("order placed: {} short of {} @ {} by {}",
              OFFSET_NAMES[ordInfo._offset], ordInfo._code, ordInfo._price,
              ordInfo._total);
    on_entrust(localid, ordInfo._code, true, "entrust success");
  });
...
void UftMocker::on_order(uint32_t localid, const char *stdCode, bool isLong,
                         uint32_t offset, double totalQty, double leftQty,
                         double price, bool isCanceled) {
  if (_strategy)
    _strategy->on_order(this, localid, stdCode, isLong, offset, totalQty,
                        leftQty, price, isCanceled);
}

void UftMocker::on_trade(uint32_t localid, const char *stdCode, bool isLong,
                         uint32_t offset, double vol, double price) {
...
void UftMocker::on_entrust(uint32_t localid, const char *stdCode, bool bSuccess,
                           const char *message) {
  if (_strategy)
  ...
  if (it != _pos_map.end()) {
    ...
    PosInfo &pInfo = (PosInfo &)it->second;
    {
      ...
      if (pItem.volume() == 0)
        ...
        double dynprofit = 0;
        for (auto pit = pItem._details.begin(); pit != pItem._details.end();
             pit++) {

          DetailInfo &dInfo = *pit;
          dInfo._profit =
              dInfo._volume * (price - dInfo._price) * commInfo->getVolScale();
          if (dInfo._profit > 0)
...

    {
      ...
      if (pItem.volume() == 0)
        ...
        for (auto pit = pItem._details.begin(); pit != pItem._details.end();
          ...
          DetailInfo &dInfo = *pit;
          dInfo._profit =
              dInfo._volume * (dInfo._price - price) * commInfo->getVolScale();
          if (dInfo._profit > 0)
            dInfo._max_profit = max(dInfo._profit, dInfo._max_profit);
          else if (dInfo._profit < 0)
            dInfo._max_loss = min(dInfo._profit, dInfo._max_loss);
...
bool UftMocker::procOrder(uint32_t localid) {
  ...
  if (_error_rate > 0 && genRand(10000) <= _error_rate) {
    ...
    on_order(localid, ordInfo._code, ordInfo._isLong, ordInfo._offset,
             ordInfo._total, ordInfo._left, ordInfo._price, false);
  }
  ...
  double orderQty = ordInfo._isLong ? curTick->askqty(0)
                                    : curTick->bidqty(0); // 看对手盘的数量
  ...
  for (uint32_t curQty : vols) {
    on_trade(ordInfo._localid, ordInfo._code, ordInfo._isLong, ordInfo._offset,
             curQty, curPx);
...
WTSKlineSlice *UftMocker::stra_get_bars(const char *stdCode, const char *period,
                                        uint32_t count) {
...
WTSOrdQueSlice *UftMocker::stra_get_order_queue(const char *stdCode,
                                                uint32_t count) {
  return _replayer->get_order_queue_slice(stdCode, count);
}

WTSOrdDtlSlice *UftMocker::stra_get_order_detail(const char *stdCode,
                                                 uint32_t count) {
  return _replayer->get_order_detail_slice(stdCode, count);
}

WTSTransSlice *UftMocker::stra_get_transaction(const char *stdCode,
                                               uint32_t count) {
...
double UftMocker::stra_get_position(const char *stdCode,
  ...
  if (iFlag == 1)
    ...
    return bOnlyValid ? (posInfo._long.valid() - posInfo._short.valid())
                      : (posInfo._long.volume() - posInfo._short.volume());
}
...
double UftMocker::stra_enum_position(const char *stdCode) {
  ...
  for (auto it = _pos_map.begin(); it != _pos_map.end(); it++) {
    ...
    const PosInfo &pInfo = it->second;
    _strategy->on_position(this, stdCode, true, pInfo._long._prevol,
                           pInfo._long._preavail, pInfo._long._newvol,
                           pInfo._long._newavail);
    _strategy->on_position(this, stdCode, false, pInfo._short._prevol,
                           pInfo._short._preavail, pInfo._short._newvol,
                           pInfo._short._newavail);
...
uint32_t UftMocker::stra_get_date() { return _replayer->get_date(); }

uint32_t UftMocker::stra_get_time() { return _replayer->get_raw_time(); }

uint32_t UftMocker::stra_get_secs() { return _replayer->get_secs(); }
...
void UftMocker::dump_outputs() {
  ...
  content = "code,direct,opentime,openprice,closetime,closeprice,qty,profit,"
            "maxprofit,maxloss,totalprofit,entertag,exittag\n";
...
void UftMocker::log_trade(const char *stdCode, bool isLong, uint32_t offset,
                          uint64_t curTime, double price, double qty,
                          double fee) {
  _trade_logs << stdCode << "," << curTime << "," << (isLong ? "LONG" : "SHORT")
              << "," << OFFSET_NAMES[offset] << "," << price << "," << qty
              << "," << fee << "\n";
}

void UftMocker::log_close(const char *stdCode, bool isLong, uint64_t openTime,
...
void UftMocker::update_position(const char *stdCode, bool isLong,
                                uint32_t offset, double qty,
                                double price /* = 0.0 */) {
  ...
  // 获取时间
  uint64_t curTm = (uint64_t)_replayer->get_date() * 1000000000 +
                   (uint64_t)_replayer->get_min_time() * 100000 +
                   _replayer->get_secs();
  ...
  if (offset == 0) {
    ...
    if (commInfo->isT1()) {
      // ASSERT(diff>0);
      log_debug("{} position of {} frozen up to {}", pos_dir, stdCode,
                pItem.frozen());
    } else {
      pItem._newavail += qty;
    }
    ...
    for (auto it = pItem._details.begin(); it != pItem._details.end(); it++) {
      ...
      dInfo._max_profit =
          dInfo._max_profit * dInfo._volume / (dInfo._volume + maxQty);
      dInfo._max_loss =
          dInfo._max_loss * dInfo._volume / (dInfo._volume + maxQty);
      ...
      // 这里写平仓记录
      log_close(stdCode, isLong, dInfo._opentime, dInfo._price, curTm, trdPx,
                maxQty, profit, maxProf, maxLoss, pItem._closeprofit);
    ...
    for (auto it = pItem._details.begin(); it != pItem._details.end(); it++) {
      ...
      if (!isLong)
        profit *= -1;
      pItem._closeprofit += profit;
      pItem._dynprofit = pItem._dynprofit * dInfo._volume /
  ...
  log_info("[{:04d}.{:05d}] {} position of {} updated: {} {} to {}",
           _replayer->get_min_time(), _replayer->get_secs(), pos_dir, stdCode,
           OFFSET_NAMES[offset], qty, pItem.volume());
