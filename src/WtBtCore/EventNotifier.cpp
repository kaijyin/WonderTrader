...
EventNotifier::EventNotifier() : _mq_sid(0) {}
...
bool EventNotifier::init(WTSVariant *cfg) {
  ...
  WTSLogger::info("EventNotifier initialized with channel {}",
                  m_strURL.c_str());
...
void EventNotifier::notifyData(const char *topic, void *data,
                               uint32_t dataLen) {
  if (_publisher)
    _publisher(_mq_sid, topic, (const char *)data, dataLen);
}

void EventNotifier::notifyFund(const char *topic, uint32_t uDate,
                               double total_profit, double dynprofit,
                               double dynbalance, double total_fee) {
  std::string output;
  {
  ...
  if (_publisher)
    _publisher(_mq_sid, topic, (const char *)output.c_str(),
