...
std::vector<double> splitVolume(double vol, double minQty = 1.0,
                                double maxQty = 100.0, double qtyTick = 1.0) {
...
HftMocker::HftMocker(HisDataReplayer *replayer, const char *name)
    : IHftStraCtx(name), _replayer(replayer), _strategy(NULL),
      _use_newpx(false), _error_rate(0), _match_this_tick(false),
      _has_hook(false), _hook_valid(true), _resumed(false) {
...
bool HftMocker::init_hft_factory(WTSVariant *cfg) {
  ...
  log_info("HFT match params: use_newpx-{}, error_rate-{}, match_this_tick-{}",
  ...
  FuncCreateHftStraFact creator =
      (FuncCreateHftStraFact)DLLHelper::get_symbol(hInst, "createStrategyFact");
  if (creator == NULL) {
  ...
  _factory._remover =
      (FuncDeleteHftStraFact)DLLHelper::get_symbol(hInst, "deleteStrategyFact");
...
void HftMocker::handle_order_detail(const char *stdCode,
                                    WTSOrdDtlData *curOrdDtl) {
  on_order_detail(stdCode, curOrdDtl);
}

void HftMocker::handle_order_queue(const char *stdCode,
                                   WTSOrdQueData *curOrdQue) {
  on_order_queue(stdCode, curOrdQue);
}

void HftMocker::handle_transaction(const char *stdCode,
                                   WTSTransData *curTrans) {
  on_transaction(stdCode, curTrans);
}

void HftMocker::handle_bar_close(const char *stdCode, const char *period,
...
void HftMocker::on_bar(const char *stdCode, const char *period, uint32_t times,
                       WTSBarStruct *newBar) {
  if (_strategy)
...
void HftMocker::on_ordque_updated(const char *stdCode,
...
void HftMocker::on_orddtl_updated(const char *stdCode,
...
uint32_t HftMocker::id() { return _context_id; }
...
void HftMocker::on_session_end(uint32_t curTDate) {
  ...
  _fund_logs << fmt::format(
      "{},{:.2f},{:.2f},{:.2f},{:.2f}\n", curTDate, _fund_info._total_profit,
...
bool HftMocker::stra_cancel(uint32_t localid) {
  postTask([this, localid]() {
    ...
    on_order(localid, ordInfo->_code, ordInfo->_isBuy, ordInfo->_total,
             ordInfo->_left, ordInfo->_price, true, ordInfo->_usertag);
...
OrderIDs HftMocker::stra_cancel(const char *stdCode, bool isBuy,
                                double qty /* = 0 */) {
  OrderIDs ret;
  uint32_t cnt = 0;
  for (auto it = _orders.begin(); it != _orders.end(); it++) {
...
OrderIDs HftMocker::stra_buy(const char *stdCode, double price, double qty,
                             const char *userTag, int flag /* = 0 */,
...
void HftMocker::on_order(uint32_t localid, const char *stdCode, bool isBuy,
                         double totalQty, double leftQty, double price,
                         bool isCanceled /* = false */,
                         const char *userTag /* = "" */) {
  if (_strategy)
    _strategy->on_order(this, localid, stdCode, isBuy, totalQty, leftQty, price,
                        isCanceled, userTag);
}

void HftMocker::on_trade(uint32_t localid, const char *stdCode, bool isBuy,
...
void HftMocker::on_entrust(uint32_t localid, const char *stdCode, bool bSuccess,
...
bool HftMocker::procOrder(uint32_t localid) {
  ...
  if (_error_rate > 0 && genRand(10000) <= _error_rate) {
    ...
    on_order(localid, ordInfo->_code, ordInfo->_isBuy, ordInfo->_total,
             ordInfo->_left, ordInfo->_price, false, ordInfo->_usertag);
  ...
  double orderQty = ordInfo->_isBuy ? curTick->askqty(0)
  ...
  for (uint32_t curQty : vols) {
    ...
    _sig_logs << _replayer->get_date() << "." << _replayer->get_raw_time()
              << "." << _replayer->get_secs() << ","
              << (ordInfo->_isBuy ? "+" : "-") << curQty << "," << curPos << ","
              << curPx << std::endl;
  }
...
OrderIDs HftMocker::stra_sell(const char *stdCode, double price, double qty,
                              const char *userTag, int flag /* = 0 */,
                              bool bForceClose /* = false */) {
  WTSCommodityInfo *commInfo = _replayer->get_commodity_info(stdCode);
  if (commInfo == NULL) {
...
WTSKlineSlice *HftMocker::stra_get_bars(const char *stdCode, const char *period,
                                        uint32_t count) {
...
WTSOrdQueSlice *HftMocker::stra_get_order_queue(const char *stdCode,
                                                uint32_t count) {
  return _replayer->get_order_queue_slice(stdCode, count);
}

WTSOrdDtlSlice *HftMocker::stra_get_order_detail(const char *stdCode,
                                                 uint32_t count) {
  return _replayer->get_order_detail_slice(stdCode, count);
}

WTSTransSlice *HftMocker::stra_get_transaction(const char *stdCode,
                                               uint32_t count) {
...
uint32_t HftMocker::stra_get_date() { return _replayer->get_date(); }

uint32_t HftMocker::stra_get_time() { return _replayer->get_raw_time(); }

uint32_t HftMocker::stra_get_secs() { return _replayer->get_secs(); }
...
const char *HftMocker::stra_load_user_data(const char *key,
                                           const char *defVal /*= ""*/) {
...
void HftMocker::log_trade(const char *stdCode, bool isLong, bool isOpen,
                          uint64_t curTime, double price, double qty,
                          double fee, const char *userTag /* = ""*/) {
  _trade_logs << stdCode << "," << curTime << "," << (isLong ? "LONG" : "SHORT")
              << "," << (isOpen ? "OPEN" : "CLOSE") << "," << price << ","
              << qty << "," << fee << "," << userTag << "\n";
}

void HftMocker::log_close(const char *stdCode, bool isLong, uint64_t openTime,
...
void HftMocker::do_set_position(const char *stdCode, double qty,
                                double price /* = 0.0 */,
                                const char *userTag /*= ""*/) {
  ...
  log_debug("[{:04d}.{:05d}] {} position updated: {} -> {}",
            _replayer->get_min_time(), _replayer->get_secs(), stdCode,
            pInfo._volume, qty);
                  ...
                  0)) // 当前持仓和仓位变化方向一致, 增加一条明细, 增加数量即可
  {
    ...
    log_trade(stdCode, dInfo._long, true, curTm, trdPx, abs(diff), fee,
              userTag);
    ...
    for (auto it = pInfo._details.begin(); it != pInfo._details.end(); it++) {
      ...
      _fund_info._total_fees += fee;
      // 这里写成交记录
      log_trade(stdCode, dInfo._long, false, curTm, trdPx, maxQty, fee,
                userTag);
      // 这里写平仓记录
      log_close(stdCode, dInfo._long, dInfo._opentime, dInfo._price, curTm,
                trdPx, maxQty, profit, maxProf, maxLoss, pInfo._closeprofit,
                dInfo._usertag, userTag);
    ...
    if (left > 0) {
      ...
      _fund_info._total_fees += fee;
      log_trade(stdCode, dInfo._long, true, curTm, trdPx, abs(left), fee,
                userTag);
    }
  }
}
