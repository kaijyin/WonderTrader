...
public:
  ...
  void addParam(int param) { m_vecParams.emplace_back(param); }
...
public:
  static WTSExpressLine *
  create(uint32_t size, WTSExpressLineType lineType = WELT_Polyline,
         uint32_t decimal = 0,
         uint32_t uStyle = ELS_LINE_VISIBLE | ELS_TITLE_VISIBLE) {
  ...
  virtual void retain() { WTSObject::retain(); }
  ...
  WTSLineInfo *getLineInfo(uint32_t idx = 0) {
    if (m_ayLineInfo == NULL || m_ayLineInfo->size() == 0 ||
        idx >= m_ayLineInfo->size())
      return NULL;
...
public:
  static WTSExpressData *create(const char *title,
                                WTSExpressType eType = WET_Unique) {
...
protected:
  WTSExpressData()
      : m_ayExpLines(NULL), m_dDevide(1.0), m_dBaseLine(0.0),
        m_bBaseLine(false) {}
...
public:
  ...
  uint32_t getLineCount() {
    return (NULL == m_ayExpLines) ? 0 : m_ayExpLines->size();
  }
  ...
  bool crossOver(uint32_t idx0, uint32_t idx1) {
    ...
    WTSExpressLine *line0 =
        STATIC_CONVERT(m_ayExpLines->at(idx0), WTSExpressLine *);
    WTSExpressLine *line1 =
        STATIC_CONVERT(m_ayExpLines->at(idx1), WTSExpressLine *);
  ...
  double maxvalue(int32_t head, int32_t tail) const {
    ...
    for (; it != m_ayExpLines->end(); it++) {
      ...
      bool bAbs = (line->getLineType() == WELT_VolStick ||
                   line->getLineType() == WELT_AStickLine);
