...
public:
   ...
   *	返回值	历史数据封装类指针
   */
  virtual WTSTickSlice *getTicks(const char *stdCode, uint32_t count,
                                 uint64_t etime = 0) = 0;
   ...
   */
  virtual OrderIDs buy(const char *stdCode, double price, double qty,
   ...
   *	返回值	本地订单号数组: 一个买入操作可能会拆成最多3个订单发出
   */
  virtual OrderIDs sell(const char *stdCode, double price, double qty,
                        bool bForceClose = false) = 0;
   ...
   *	根据指定的方向和数量撤单
   *	code 合约代码
   *	qty	最少撤单数量, 如果有多个委托, 那么按照时间顺序一个一个撤单,
   *直到撤销的数量大于等于qty 这个一般用不到, 所以传0的时候,
   *就是把全部买单或者卖单撤销 isBuy 买单or卖单
   *
   *	返回值 返回实际发送了撤单指令的数量
   */
   ...
   *	返回值		是否注册成功
   */
  virtual bool registerTimer(const char *stdCode, uint32_t elapse) {
    return false;
  }
};
...
public:
  ...
  virtual void on_order(uint32_t localid, const char *stdCode, bool isBuy,
                        double leftover, double price, bool isCanceled) = 0;
  ...
  virtual void on_account(const char *currency, double prebalance,
                          ...
                          double fee, double deposit, double withdraw) {}
...
//////////////////////////////////////////////////////////////////////////
// 执行单元工厂接口
typedef void (*FuncEnumUnitCallback)(const char *factName, const char *unitName,
                                     bool isLast);
