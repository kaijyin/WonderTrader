...
public:
   ...
   *	grab接口读取数据以后,增加引用计数
   */
  inline WTSObject *at(uint32_t idx) {
  ...
  inline uint32_t idxOf(WTSObject *obj) {
    if (obj == NULL)
      return -1;
  ...
  template <typename T> inline T *at(uint32_t idx) {
    if (idx < 0 || idx >= _vec.size())
      return NULL;
   ...
   *	用法同at函数
   */
  inline WTSObject *operator[](uint32_t idx) {
    if (idx < 0 || idx >= _vec.size())
      return NULL;
   ...
   *	增加引用计数
   */
  inline WTSObject *grab(uint32_t idx) {
    if (idx < 0 || idx >= _vec.size())
      return NULL;
   ...
   *	数据自动增加引用计数
   */
  inline void append(WTSObject *obj, bool bAutoRetain = true) {
    if (bAutoRetain && obj)
      obj->retain();
   ...
   *	新数据引用计数增加
   */
  inline void set(uint32_t idx, WTSObject *obj, bool bAutoRetain = true) {
  ...
  inline void append(WTSArray *ay) {
    if (ay == NULL)
      return;
  ...
  inline ConstIterator begin() const { return _vec.begin(); }

  inline ReverseIterator rbegin() { return _vec.rbegin(); }

  inline ConstReverseIterator rbegin() const { return _vec.rbegin(); }
  ...
  inline ConstIterator end() const { return _vec.end(); }

  inline ReverseIterator rend() { return _vec.rend(); }

  inline ConstReverseIterator rend() const { return _vec.rend(); }

  inline void sort(SortFunc func) { std::sort(_vec.begin(), _vec.end(), func); }
 ...
 *	所有WTSObject的派生类都适用
 */
template <class T> class WTSMap : public WTSObject {
public:
  /*
   *	容器迭代器的定义
   */
   ...
   *	用法同get函数
   */
  inline WTSObject *operator[](const T &_key) {
    Iterator it = _map.find(_key);
    if (it == _map.end())
      return NULL;
   ...
   *	获取容器起始位置的迭代器
   */
  ReverseIterator rbegin() { return _map.rbegin(); }

  ConstReverseIterator rbegin() const { return _map.rbegin(); }

  /*
   *	获取容易末尾位置的迭代器
   */
  ReverseIterator rend() { return _map.rend(); }

  ConstReverseIterator rend() const { return _map.rend(); }

  inline Iterator find(const T &key) { return _map.find(key); }

  inline ConstIterator find(const T &key) const { return _map.find(key); }

  inline void erase(ConstIterator it) { _map.erase(it); }

  inline void erase(Iterator it) { _map.erase(it); }

  inline void erase(T key) { _map.erase(key); }

  Iterator lower_bound(const T &key) { return _map.lower_bound(key); }

  ConstIterator lower_bound(const T &key) const {
    return _map.lower_bound(key);
  }

  Iterator upper_bound(const T &key) { return _map.upper_bound(key); }
...
public:
   ...
   *	如果key存在,则将原有数据释放
   */
  inline void add(const T &_key, WTSObject *obj,
                  bool bAutoRetain = true) noexcept {
    if (bAutoRetain && obj)
      obj->retain();
   ...
   *	获取容器起始位置的迭代器
   */
  inline ConstIterator begin() const noexcept { return _map.begin(); }

  /*
   *	获取容易末尾位置的迭代器
   */
  inline ConstIterator end() const noexcept { return _map.end(); }
...
public:
  ...
  void swap(WTSQueue *right) { _queue.swap(right->_queue); }
