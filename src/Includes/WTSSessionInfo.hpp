...
public:
  ...
  typedef struct _TradingSection {
    ...
    _TradingSection(uint32_t stime, uint32_t etime, uint32_t stime_raw,
                    uint32_t etime_raw)
        : first(stime), second(etime), first_raw(stime_raw),
          second_raw(etime_raw) {}
  } TradingSection;
...
public:
  ...
  static WTSSessionInfo *create(const char *sid, const char *name,
                                int32_t offset = 0) {
...
public:
   ...
   *	@uTime	当前时间,格式如0910
   *	@autoAdjust
   *是否自动调整,如果开启,非交易时间内的行情,会自动对齐到下一个交易时间,如8点59分的行情,会自动算作9点00的行情
  ...
  uint32_t timeToSeconds(uint32_t uTime) {
    ...
    for (; it != m_tradingTimes.end(); it++) {
      TradingSection &section = *it;
      uint32_t startSecs =
          (section.first / 100 * 60 + section.first % 100) * 60;
      uint32_t stopSecs =
          (section.second / 100 * 60 + section.second % 100) * 60;
    ...
    for (; it != m_tradingTimes.end(); it++) {
      ...
      if (startSecs + offset >= stopSecs) {
        ...
        if (offset == 0) {
          ...
          return originalTime((desMin / 60 * 100 + desMin % 60)) * 100 +
                 stopSecs % 60;
        }
      } else {
        // 干好位于该区间
  ...
  inline uint32_t getCloseTime(bool bOffseted = false) const {
    ...
    uint32_t ret = bOffseted
                       ? m_tradingTimes[m_tradingTimes.size() - 1].second
                       : m_tradingTimes[m_tradingTimes.size() - 1].second_raw;
