 ...
 *
 * \brief
 * Wt行情数据定义文件,包括tick、bar、orderqueue、orderdetail、transaction等数据
 */
#pragma once
#include <chrono>
#include <deque>
...
public:
  ...
  double maxvalue(int32_t head, int32_t tail, bool isAbs = false) const {
    ...
    if (begin < 0 || begin >= m_vecData.size() || end < 0 ||
        end > m_vecData.size())
      return INVALID_DOUBLE;
   ...
   *	在数组末尾添加数据
   */
  inline void append(double val) { m_vecData.emplace_back(val); }
   ...
   *	用法同getValue接口
   */
  inline double &operator[](uint32_t idx) { return m_vecData[idx]; }

  inline double operator[](uint32_t idx) const { return m_vecData[idx]; }

  inline std::vector<double> &getDataRef() { return m_vecData; }
};
...
protected:
  WTSKlineSlice() : _period(KP_Minute1), _times(1), _count(0) {}
...
public:
  static WTSKlineSlice *create(const char *code, WTSKlinePeriod period,
                               uint32_t times, WTSBarStruct *bars = NULL,
                               int32_t count = 0) {
  ...
  inline std::size_t get_block_counts() const { return _blocks.size(); }
   ...
   */
  WTSValueArray *extractData(WTSKlineFieldType type, int32_t head = 0,
                             int32_t tail = -1) const {
    if (_count == 0)
      return NULL;
...
protected:
  WTSKlineData()
      : m_kpPeriod(KP_Minute1), m_uTimes(1), m_bUnixTime(false),
        m_bClosed(true) {}
...
public:
   ...
   *	@param nSize 预先分配的大小
   */
  static inline WTSHisTickData *create(const char *stdCode,
                                       unsigned int nSize = 0,
                                       bool bValidOnly = false,
                                       double factor = 1.0) {
   ...
   */
  static inline WTSHisTickData *
  create(const char *stdCode, bool bValidOnly = false, double factor = 1.0) {
    WTSHisTickData *pRet = new WTSHisTickData;
    wt_strcpy(pRet->m_strCode, stdCode);
    pRet->m_bValidOnly = bValidOnly;
    pRet->m_dFactor = factor;
...
public:
  static inline WTSTickSlice *
  create(const char *code, WTSTickStruct *ticks = NULL, uint32_t count = 0) {
  ...
  inline bool insertBlock(std::size_t idx, WTSTickStruct *ticks,
                          uint32_t count) {
    if (ticks == NULL || count == 0)
      return false;
...
public:
  static inline WTSOrdDtlSlice *
  create(const char *code, WTSOrdDtlStruct *firstItem, uint32_t count) {
...
public:
  static inline WTSOrdQueSlice *
  create(const char *code, WTSOrdQueStruct *firstItem, uint32_t count) {
    if (count == 0 || firstItem == NULL)
      return NULL;
...
public:
  static inline WTSTransSlice *
  create(const char *code, WTSTransStruct *firstItem, uint32_t count) {
    if (count == 0 || firstItem == NULL)
      return NULL;
