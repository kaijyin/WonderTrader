...
public:
   ...
   */
  virtual void on_bar(const char *stdCode, WTSKlinePeriod period,
   ...
   *	@brief	获取当前1分钟线的时间
   *	@details
   *这里的分钟线时间是处理过的1分钟线时间,如现在是9:00:32秒,真实事件为0900,但是对应的1分钟线时间为0901
   */
 ...
 *	@count	K线条数
 *	@factor
 *复权因子，最新的复权因子，如果是后复权，则factor不为1.0，如果是前复权，则factor必须为1.0
 */
...
public:
   ...
   *	@param sink	模块回调接口
   */
  virtual void init(WTSVariant *cfg, IDataReaderSink *sink,
   ...
   *	@param endTDate
   *如果闭合的分钟线是交易日最后一条分钟线,则endTDate为当前交易日,如20200410,其他情况为0
   */
  virtual void onMinuteEnd(uint32_t uDate, uint32_t uTime,
                           uint32_t endTDate = 0) = 0;
   ...
   *	@param etime
   *结束时间,精确到毫秒,格式如yyyyMMddhhmmssmmm,如果要读取到最后一条,etime为0,默认为0
   */
  virtual WTSTickSlice *readTickSlice(const char *stdCode, uint32_t count,
                                      uint64_t etime = 0) = 0;
   ...
   *结束时间,精确到毫秒,格式如yyyyMMddhhmmssmmm,如果要读取到最后一条,etime为0,默认为0
   */
  virtual WTSOrdDtlSlice *readOrdDtlSlice(const char *stdCode, uint32_t count,
                                          uint64_t etime = 0) {
    return NULL;
  }
  /*
   ...
   *结束时间,精确到毫秒,格式如yyyyMMddhhmmssmmm,如果要读取到最后一条,etime为0,默认为0
   */
  virtual WTSOrdQueSlice *readOrdQueSlice(const char *stdCode, uint32_t count,
                                          uint64_t etime = 0) {
    return NULL;
  }
   ...
   *结束时间,精确到毫秒,格式如yyyyMMddhhmmssmmm,如果要读取到最后一条,etime为0,默认为0
   */
  virtual WTSTransSlice *readTransSlice(const char *stdCode, uint32_t count,
                                        uint64_t etime = 0) {
    return NULL;
  }
   ...
   *	@details
   *切片不会复制数据,只把缓存中的数据指针传递出来,所以叫做切片
   *
   *	@param	stdCode	标准品种代码,如SSE.600000,SHFE.au.2005
   *	@param	period	K线周期
   ...
   */
  virtual WTSKlineSlice *readKlineSlice(const char *stdCode,
   ...
   */
  virtual double getAdjFactorByDate(const char *stdCode, uint32_t date = 0) {
    return 1.0;
  }
