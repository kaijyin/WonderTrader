...
public:
  ...
  inline void setFeeRates(double open, double close, double closeToday,
                          bool byVolume) {
    m_dOpenFee = open;
    m_dCloseFee = close;
...
public:
  ...
  template <typename T> inline T *getExtData() noexcept {
    return static_cast<T *>(m_pExtData);
  }
...
private:
  ...
  void *
      m_pExtData; // 扩展数据，主要是绑定一些和合约相关的数据，这样可以避免在很多地方建map，导致多次查找
};
