...
public:
  WTSEntrust()
      : m_iPrice(0), m_dVolume(0), m_businessType(BT_CASH),
        m_direction(WDT_LONG), m_priceType(WPT_ANYPRICE), m_orderFlag(WOF_NOR),
        m_offsetType(WOT_OPEN), m_bIsNet(false), m_bIsBuy(true),
        m_pContract(NULL) {}

  virtual ~WTSEntrust() {}

public:
  static inline WTSEntrust *create(const char *code, double vol, double price,
                                   const char *exchg = "",
                                   WTSBusinessType bType = BT_CASH) noexcept {
...
public:
  ...
  inline void setDirection(WTSDirectionType dType) noexcept {
    m_direction = dType;
  }
  ...
  inline void setOffsetType(WTSOffsetType oType) noexcept {
    m_offsetType = oType;
  }
  ...
  inline void setBusinessType(WTSBusinessType bType) noexcept {
    m_businessType = bType;
  }
  inline WTSBusinessType getBusinessType() const noexcept {
  ...
  inline void setContractInfo(WTSContractInfo *cInfo) noexcept {
    m_pContract = cInfo;
  }
  inline WTSContractInfo *getContractInfo() const noexcept {
    return m_pContract;
  }
...
public:
  WTSEntrustAction() : m_actionFlag(WAF_CANCEL), m_businessType(BT_CASH) {}

  virtual ~WTSEntrustAction() {}

public:
  static inline WTSEntrustAction *
  create(const char *code, const char *exchg = "",
         WTSBusinessType bType = BT_CASH) noexcept {
  ...
  static inline WTSEntrustAction *createCancelAction(const char *eid,
                                                     const char *oid) noexcept {
...
public:
  WTSOrderInfo()
      : m_orderState(WOS_Submitting), m_orderType(WORT_Normal),
        m_uInsertDate(0), m_uInsertTime(0), m_dVolTraded(0), m_dVolLeft(0),
        m_bIsError(false) {}
...
public:
  ...
  inline const char *getStateMsg() const noexcept {
...
public:
  WTSTradeInfo()
      : m_orderType(WORT_Normal), m_tradeType(WTT_Common), m_uAmount(0),
        m_dPrice(0), m_businessType(BT_CASH), m_pContract(NULL) {}
...
public:
  ...
  inline void setTradeID(const char *tradeid) noexcept {
    wt_strcpy(m_strTradeID, tradeid);
  }
  inline void setRefOrder(const char *oid) noexcept {
    wt_strcpy(m_strRefOrder, oid);
  }
...
public:
  static inline WTSPositionItem *create(const char *code,
  ...
  inline double getTotalPosition() const {
    return m_dPrePosition + m_dNewPosition;
  }
  inline double getAvailPosition() const {
    return m_dAvailPrePos + m_dAvailNewPos;
  }

  inline double getFrozenPosition() const {
    return getTotalPosition() - getAvailPosition();
  }
  inline double getFrozenNewPos() const {
    return m_dNewPosition - m_dAvailNewPos;
  }
  inline double getFrozenPrePos() const {
    return m_dPrePosition - m_dAvailPrePos;
  }
...
public:
  WTSPositionItem()
      : m_direction(WDT_LONG), m_dPrePosition(0), m_dNewPosition(0),
        m_dAvailPrePos(0), m_dAvailNewPos(0), m_dMargin(0), m_dAvgPrice(0),
        m_dDynProfit(0), m_dTotalPosCost(0), m_businessType(BT_CASH),
        m_pContract(NULL) {}
...
public:
  WTSAccountInfo()
      : m_strCurrency("CNY"), m_dBalance(0), m_dPreBalance(0), m_dCommission(0),
        m_dFrozenMargin(0), m_dFrozenCommission(0), m_dCloseProfit(0),
        m_dDynProfit(0), m_dDeposit(0), m_dWithdraw(0), m_dAvailable(0) {}
...
public:
  ...
  inline void setFrozenMargin(double frozenmargin) {
    m_dFrozenMargin = frozenmargin;
  }
  inline void setCloseProfit(double closeprofit) {
    m_dCloseProfit = closeprofit;
  }
  ...
  inline void setFrozenCommission(double frozencommission) {
    m_dFrozenCommission = frozencommission;
  }
