...
WtDtRunner::WtDtRunner() : _data_store(NULL), _is_inited(false) {
...
void WtDtRunner::initialize(const char *cfgFile, bool isFile /* = true */,
                            const char *modDir /* = "" */,
                            const char *logCfg /* = "logcfg.yaml" */,
                            FuncOnTickCallback cbTick /* = NULL */,
                            FuncOnBarCallback cbBar /* = NULL */) {
  if (_is_inited) {
  ...
  WTSVariant *config = isFile ? WTSCfgLoader::load_from_file(cfgFile)
...
WTSKlineSlice *WtDtRunner::get_bars_by_range(const char *stdCode,
                                             const char *period,
                                             uint64_t beginTime,
                                             uint64_t endTime /* = 0 */) {
  if (!_is_inited) {
  ...
  return _data_mgr.get_kline_slice_by_range(stdCode, kp, realTimes, beginTime,
                                            endTime);
}

WTSKlineSlice *WtDtRunner::get_bars_by_date(const char *stdCode,
                                            const char *period,
                                            uint32_t uDate /* = 0 */) {
  if (!_is_inited) {
    WTSLogger::error("WtDtServo not initialized");
  ...
  if (basePeriod[0] == 'm') {
    ...
    WTSLogger::log_raw(LL_ERROR,
                       "get_bars_by_date only supports minute period");
    return NULL;
  }
...
WTSTickSlice *WtDtRunner::get_ticks_by_range(const char *stdCode,
                                             uint64_t beginTime,
                                             uint64_t endTime /* = 0 */) {
  if (!_is_inited) {
    WTSLogger::error("WtDtServo not initialized");
...
WTSTickSlice *WtDtRunner::get_ticks_by_date(const char *stdCode,
                                            uint32_t uDate /* = 0 */) {
  if (!_is_inited) {
    WTSLogger::error("WtDtServo not initialized");
    return NULL;
...
WTSKlineSlice *WtDtRunner::get_bars_by_count(const char *stdCode,
  ...
  return _data_mgr.get_kline_slice_by_count(stdCode, kp, realTimes, count,
                                            endTime);
}

WTSTickSlice *WtDtRunner::get_ticks_by_count(const char *stdCode,
                                             uint32_t count,
                                             uint64_t endTime /* = 0 */) {
  if (!_is_inited) {
    WTSLogger::error("WtDtServo not initialized");
...
WTSKlineSlice *WtDtRunner::get_sbars_by_date(const char *stdCode, uint32_t secs,
                                             uint32_t uDate /* = 0 */) {
  if (!_is_inited) {
    WTSLogger::error("WtDtServo not initialized");
    return NULL;
...
void WtDtRunner::start() { _parsers.run(); }
    ...
    if (sit != _tick_sub_map.end()) {
      ...
      for (uint32_t flag : flags) {
        if (flag == 0) {
          ...
          std::string wCode = fmtutil::format(
              "{}{}", stdCode, (flag == 1) ? SUFFIX_QFQ : SUFFIX_HFQ);
          if (flag == 1) {
          ...
          } else //(flag == 2)
          {
            ...
            // 这里做一个复权因子的处理
            double factor = _data_mgr.get_exright_factor(
                stdCode, curTick->getContractInfo()->getCommInfo());
            newTS.open *= factor;
            newTS.high *= factor;
            newTS.low *= factor;
            newTS.price *= factor;
...
void WtDtRunner::sub_tick(const char *codes, bool bReplace,
  ...
  if (bInner) {
    ...
    flags.insert(flag);
    WTSLogger::info("Tick dada of {} subscribed with flag {} for inner use",
                    stdCode, flag);
  } else {
    StdUniqueLock lock(_mtx_subs);
    if (bReplace)
...
void WtDtRunner::trigger_bar(const char *stdCode, const char *period,
                             WTSBarStruct *lastBar) {
  if (_cb_bar == NULL)
    return;
...
void WtDtRunner::clear_cache() { _data_mgr.clear_cache(); }