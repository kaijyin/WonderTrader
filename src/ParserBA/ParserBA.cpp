...
void ParserBA::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,
  ...
  if (bIsLast && !IsErrorRspInfo(pRspInfo)) {
    ...
    write_log(m_sink, LL_INFO, "[ParserCTP] Market data server logined, {}",
              m_uTradingDate);
...
void ParserBA::ReqUserLogin() {
  ...
  if (iResult != 0) {
    if (m_sink)
      write_log(m_sink, LL_ERROR,
                "[ParserCTP] Sending login request failed: {}", iResult);
  }
}
...
void ParserBA::DoSubscribeMD() {
  ...
  if (m_pUserAPI && nCount > 0) {
    ...
    if (iResult != 0) {
      if (m_sink)
        write_log(m_sink, LL_ERROR,
                  "[ParserCTP] Sending md subscribe request failed: {}",
                  iResult);
    } else {
      if (m_sink)
        write_log(m_sink, LL_INFO,
                  "[ParserCTP] Market data of {} contracts subscribed totally",
                  nCount);
    }
  }
  codeFilter.clear();
