...
WtStockVWapExeUnit::WtStockVWapExeUnit()
    : _last_tick(NULL), _comm_info(NULL) // 品种信息
      ,
      _ord_sticky(0), _cancel_cnt(0), _channel_ready(false), _last_fire_time(0),
      _fired_times(0), _total_times(0), _total_secs(0), _price_mode(0),
      _price_offset(0), _target_pos(0), _cancel_times(0), _begin_time(0),
      _end_time(0), _is_clear{false}, _is_KC{false}, isCanCancel{true} {}
...
inline uint32_t calTmSecs(uint32_t begintime,
                          uint32_t endtime) // 计算执行时间：s
{
  return ((endtime / 100) * 3600 + (endtime % 100) * 60) -
         ((begintime / 100) * 3600 + (begintime % 100) * 60);
}
...
const char *WtStockVWapExeUnit::getFactName() { return FACT_NAME; }

const char *WtStockVWapExeUnit::getName() { return "WtStockVWapExeUnit"; }

void WtStockVWapExeUnit::init(ExecuteContext *ctx, const char *stdCode,
                              WTSVariant *cfg) {
  ...
  _fire_span =
      (_total_secs - _tail_secs) /
      _total_times; // 单次发单时间间隔,要去掉尾部时间计算,这样的话,最后剩余的数量就有一个兜底发单的机制了

  ctx->writeLog(
      fmt::format("执行单元WtStockVWapExeUnit[{}] 初始化完成,订单超时 {} "
                  "秒,执行时限 {} 秒,收尾时间 {} 秒",
                  stdCode, _ord_sticky, _total_secs, _tail_secs)
          .c_str());
  ...
  if (!StdFile::exists(filename.c_str())) {
    _ctx->writeLog(fmtutil::format("Vwap file {} not exists ,check and return.",
                                   filename.c_str()));
    return;
  }
...
void WtStockVWapExeUnit::on_order(uint32_t localid, const char *stdCode,
  ...
  if (isCanceled && _cancel_cnt == 0) {
    ...
    if (!decimal::eq(realPos, _this_target)) {
      _ctx->writeLog(fmtutil::format(
          "Order {} of {} canceled, re_fire will be done", localid, stdCode));
      _cancel_times++;
  ...
  if (!isCanceled && _cancel_cnt != 0) { // 一般出现问题，需要返回检查  触发撤单
                                         // cnt++,onorder响应处理才会--
    _ctx->writeLog(fmtutil::format(
        "Order {} of {}  hasn't canceled, error will be return ", localid,
        stdCode));
    return;
  }
}
...
void WtStockVWapExeUnit::on_channel_ready() {
  ...
  if (!decimal::eq(undone, 0) &&
      !_orders_mon.has_order()) { // 未完成单不在监控中，撤单
    /*
     *	如果未完成单不为0，而OMS没有订单
     ...
     */
    _ctx->writeLog(
        fmt::format("{} unmanaged orders of {},cancel all", undone, _code)
            .c_str());
...
void WtStockVWapExeUnit::on_tick(WTSTickData *newTick) {
  ...
  if (isFirstTick) // 如果是第一笔tick,则检查目标仓位,不符合则下单
  {
    ...
    if (!decimal::eq(
            newVol,
            undone +
                realPos)) { // 如果是第一笔TICK，且目标量==未完成+仓位，退出
      do_calc();
    }
  } else {
    ...
    if (_ord_sticky != 0 && _orders_mon.has_order()) {
      _orders_mon.check_orders(
          _ord_sticky, now, [this, &hasCancel](uint32_t localid) {
            if (_ctx->cancel(localid)) {
              _cancel_cnt++;
              _ctx->writeLog(
                  fmt::format("Order expired, cancelcnt updated to {}",
                              _cancel_cnt)
                      .c_str()); // 订单过期，撤单量更新
              hasCancel = true;
            }
...
void WtStockVWapExeUnit::on_trade(uint32_t localid, const char *stdCode,
                                  bool isBuy, double vol,
                                  double price) { // 在ontick中触发
}
/*
下单结果回报
*/
void WtStockVWapExeUnit::on_entrust(uint32_t localid, const char *stdCode,
...
void WtStockVWapExeUnit::do_calc() {
  ...
  if (!decimal::eq(target_pos, _target_pos)) {
    _ctx->writeLog(
        fmtutil::format("{} can sell hold pos not enough, target adjust {}->{}",
                        code, _target_pos, target_pos));
    _target_pos = target_pos;
  }
  // 有正在撤销的订单,则不能进行下一轮计算
  if (_cancel_cnt != 0) {
    _ctx->writeLog(
        fmt::format("{}尚有未完成的撤单指令，暂时退出本轮执行", _code).c_str());
    return;
  }
  ...
  if (decimal::eq(round_hands(target_pos, _min_hands),
                  round_hands(realPos, _min_hands)) &&
      !(target_pos == 0 && realPos < _min_hands && realPos > target_pos)) {
  ...
  if (decimal::lt(diffQty * undone, 0)) {
    ...
    if (!ids.empty()) {
      ...
      _ctx->writeLog(fmtutil::format(
          ...
          __FILE__, __LINE__, _code.c_str(), _cancel_cnt)); // 相反的订单取消
    }
    return;
  }
  if (!decimal::eq(undone, 0)) {
    _ctx->writeLog(
        fmt::format("{}上一轮有挂单未完成,暂时退出本轮执行", _code).c_str());
    return;
  }
  if (_last_tick == NULL) {
    _ctx->writeLog(
        fmt::format("{}没有最新的tick数据，退出执行逻辑", _code).c_str());
    return;
  }
  ...
  double InminsTm = calTmStamp(
      _last_tick->actiontime()); // 当前tick属于vwap240分钟内的第几(-1)分钟
  ...
  if (leftTimes == 0 && !decimal::eq(diffQty, 0)) {
    ...
    curQty = max(_Vwap_vol, _min_open_lots) * abs(diffQty) /
             diffQty; // curqty=单位预测量sum
  }
  ...
  if (decimal::eq(targetPx, 0.0))
    targetPx = decimal::eq(_last_tick->price(), 0.0) ? _last_tick->preclose()
                                                     : _last_tick->price();

  // 检查涨跌停价
  isCanCancel = true;
  if (isBuy && !decimal::eq(_last_tick->upperlimit(), 0) &&
      decimal::gt(targetPx, _last_tick->upperlimit())) {
    _ctx->writeLog(
        fmt::format("Buy price {} of {} modified to upper limit price",
                    targetPx, _code.c_str(), _last_tick->upperlimit())
            .c_str());
    targetPx = _last_tick->upperlimit();
    isCanCancel = false; // 如果价格被修正为涨跌停价，订单不可撤销
  }
  if (isBuy != 1 && !decimal::eq(_last_tick->lowerlimit(), 0) &&
      decimal::lt(targetPx, _last_tick->lowerlimit())) {
    _ctx->writeLog(
        fmt::format("Sell price {} of {} modified to lower limit price",
                    targetPx, _code.c_str(), _last_tick->lowerlimit())
            .c_str());
...
void WtStockVWapExeUnit::fire_at_once(double qty) {
  ...
  {
    targetPx = isBuy ? curTick->askprice(0)
                     : curTick->bidprice(0); // 买入方向：卖价，卖出方向：买价
  }

  targetPx += _comm_info->getPriceTick() * _cancel_times *
              (isBuy ? 1 : -1); // 增加价格偏移
  // 检查涨跌停价
  isCanCancel = true;
  if (isBuy && !decimal::eq(_last_tick->upperlimit(), 0) &&
