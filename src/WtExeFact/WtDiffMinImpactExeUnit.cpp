...
WtDiffMinImpactExeUnit::WtDiffMinImpactExeUnit()
    : _last_tick(NULL), _comm_info(NULL), _price_mode(0), _price_offset(0),
      _expire_secs(0), _cancel_cnt(0), _left_diff(0), _cancel_times(0),
...
const char *WtDiffMinImpactExeUnit::getFactName() { return FACT_NAME; }

const char *WtDiffMinImpactExeUnit::getName() {
  return "WtDiffMinImpactExeUnit";
}

void WtDiffMinImpactExeUnit::init(ExecuteContext *ctx, const char *stdCode,
                                  WTSVariant *cfg) {
  ...
  ctx->writeLog(fmtutil::format(
      "DiffMiniImpactExecUnit {} inited, order price: {} ± {} ticks, order "
      "expired: {} secs, order timespan:{} millisec, order qty: {} @ {:.2f}",
      stdCode, PriceModeNames[_price_mode + 1], _price_offset, _expire_secs,
      _entrust_span, _by_rate ? "byrate" : "byvol",
      _by_rate ? _qty_rate : _order_lots));
}

void WtDiffMinImpactExeUnit::on_order(uint32_t localid, const char *stdCode,
...
void WtDiffMinImpactExeUnit::on_trade(uint32_t localid, const char *stdCode,
  ...
  _ctx->writeLog(fmtutil::format("Left diff of {} updated to {}", _code.c_str(),
                                 _left_diff));
}

void WtDiffMinImpactExeUnit::on_entrust(uint32_t localid, const char *stdCode,
...
void WtDiffMinImpactExeUnit::do_calc() {
  ...
  if (flag) {
    _ctx->writeLog(fmtutil::format(
        "Duplicated calculating, DiffMinImpactExeUnit of {}", _code));
    return;
  }

  if (_cancel_cnt != 0) {
    _ctx->writeLog(
        fmtutil::format("In Cancelling, DiffMinImpactExeUnit of {}", _code));
    return;
  }
  ...
  if (!decimal::eq(undone, 0)) {
    _ctx->writeLog(fmtutil::format(
        "Live orders exist, DiffMinImpactExeUnit of {}", _code));
    return;
  }
  ...
  if (!decimal::eq(_last_tick->upperlimit(), 0) &&
    ...
    _ctx->writeLog(
        fmtutil::format("Buy price {} of {} modified to upper limit price",
                        buyPx, _code.c_str(), _last_tick->upperlimit()));
...
void WtDiffMinImpactExeUnit::set_position(const char *stdCode, double newDiff) {
  ...
  if (_left_diff != newDiff) {
    ...
    _ctx->writeLog(
        fmtutil::format("Diff of {} updated to {}", stdCode, _left_diff));
