...
const char *WtVWapExeUnit::getFactName() { return FACT_NAME; }

const char *WtVWapExeUnit::getName() { return "WtVWapExeUnit"; }

void WtVWapExeUnit::init(ExecuteContext *ctx, const char *stdCode,
                         WTSVariant *cfg) {
  ...
  ctx->writeLog(fmt::format("执行单元WtVWapExeUnit[{}] 初始化完成,订单超时 {} "
                            "秒,执行时限 {} 秒,收尾时间 {} 秒",
...
void WtVWapExeUnit::on_order(uint32_t localid, const char *stdCode, bool isBuy,
                             double leftover, double price, bool isCanceled) {
...
void WtVWapExeUnit::on_trade(uint32_t localid, const char *stdCode, bool isBuy,
                             double vol, double price) { // 在ontick中触发
}
/*
下单结果回报
*/
void WtVWapExeUnit::on_entrust(uint32_t localid, const char *stdCode,
