...
WtStockMinImpactExeUnit::WtStockMinImpactExeUnit()
    : _last_tick(NULL), _comm_info(NULL), _price_mode(0), _price_offset(0),
      _expire_secs(0)
      //, _cancel_cnt(0)
      ,
      _target_pos(0), _cancel_times(0), _last_place_time(0), _last_tick_time(0),
      _is_clear{false}, _min_order{0}, _is_KC{false},
      _is_cancel_unmanaged_order{true}, _is_finish{true}, _is_first_tick{true},
      _is_ready{false}, _is_total_money_ready{false} {}
...
const char *WtStockMinImpactExeUnit::getFactName() { return FACT_NAME; }

const char *WtStockMinImpactExeUnit::getName() {
  return "WtStockMinImpactExeUnit";
}

void WtStockMinImpactExeUnit::init(ExecuteContext *ctx, const char *stdCode,
                                   WTSVariant *cfg) {
  ...
  _price_mode = cfg->getInt32(
      "pricemode"); // 价格类型,0-最新价,-1-最优价,1-对手价,2-自动,默认为0
  _entrust_span = cfg->getUInt32("span"); // 发单时间间隔，单位毫秒
  _by_rate = cfg->getBoolean(
      "byrate"); // 是否按照对手的挂单数的比例下单，如果是true，则rate字段生效，如果是false则lots字段生效
  _order_lots = cfg->getDouble("lots"); // 单次发单手数
  _qty_rate = cfg->getDouble("rate");   // 下单手数比例
  if (cfg->has("total_money")) {
    _is_total_money_ready = true;
    _total_money = cfg->getDouble(
        "total_money"); // 执行器使用的金额，配合目标比例使用，该字段留空或值小于0，则以账户当前余额为准
  }
  ...
  ctx->writeLog(
      fmt::format(
          "MiniImpactExecUnit {} inited, order price: {} ± {} ticks, order "
          "expired: {} secs, order timespan:{} millisec, order qty: {} @ "
          "{:.2f} min_order: {:.2f} is_cancel_unmanaged_order: {}",
          stdCode, PriceModeNames[_price_mode + 1], _price_offset, _expire_secs,
          _entrust_span, _by_rate ? "byrate" : "byvol",
          _by_rate ? _qty_rate : _order_lots, _min_order,
          _is_cancel_unmanaged_order ? "true" : "false")
          .c_str());
}

void WtStockMinImpactExeUnit::on_order(uint32_t localid, const char *stdCode,
                                       bool isBuy, double leftover,
                                       double price, bool isCanceled) {
  {
    StdUniqueLock lock(_mtx_calc);
    _ctx->writeLog(fmtutil::format("on_order localid:{} stdCode:{} isBuy:{} "
                                   "leftover:{} price:{} isCanceled:{}",
                                   localid, stdCode, isBuy, leftover, price,
                                   isCanceled));
    ...
    if (isCanceled || leftover == 0) {
      ...
      if (isCanceled)
        _ctx->writeLog(fmtutil::format("{} {} canceled, earse from mon",
                                       stdCode, localid));
      else
        _ctx->writeLog(
            fmtutil::format("{} {} done, earse from mon", stdCode, localid));
    }
  ...
  // 如果有撤单,也触发重新计算
  if (isCanceled) {
    _ctx->writeLog(fmt::format("Order {} of {} canceled, recalc will be done",
                               localid, stdCode)
                       .c_str());
    _cancel_times++;
    do_calc();
...
void WtStockMinImpactExeUnit::on_account(const char *currency,
                                         double prebalance, double balance,
                                         double dynbalance, double avaliable,
                                         double closeprofit, double dynprofit,
                                         double margin, double fee,
                                         double deposit, double withdraw) {
  if (strcmp(currency, "CNY") == 0) {
    _ctx->writeLog(
        fmtutil::format("avaliable update {}->:{}", _avaliable, avaliable));
...
void WtStockMinImpactExeUnit::check_unmanager_order() {
  ...
  if (!decimal::eq(undone, 0) && _is_cancel_unmanaged_order) {
    _ctx->writeLog(
        fmt::format(
            "{} Unmanaged live orders with qty {} of {} found, cancel all",
            _code, undone, _code.c_str())
            .c_str());
    bool isBuy = (undone > 0);
    OrderIDs ids = _ctx->cancel(_code.c_str(), isBuy);
    _orders_mon.push_order(ids.data(), ids.size(), _now);
    for (auto id : ids)
      _ctx->writeLog(fmt::format("{} mon push unmanager order {} enter time:{}",
                                 _code.c_str(), id, _now)
                         .c_str());
  }
}
...
void WtStockMinImpactExeUnit::on_tick(WTSTickData *newTick) {
  ...
  if (_last_tick) {
    ...
    if (_sess_info != NULL &&
        !_sess_info->isInTradingTime(newTick->actiontime() / 100000))
      return;
  }
  ...
  uint64_t curTickTime =
      TimeUtils::makeTime(_last_tick->actiondate(), _last_tick->actiontime());
  ...
  _orders_mon.enumOrder([this](uint32_t localid, uint64_t entertime,
                               bool cancancel) {
    _ctx->writeLog(fmtutil::format("[{}]{} entertime:{} cancancel:{} now:{} "
                                   "last_tick_time:{} live_time:{}",
                                   _code, localid, entertime, cancancel, _now,
                                   _last_tick_time, _now - entertime));
  });
  ...
  if (_expire_secs != 0 && _orders_mon.has_order()) {
    _orders_mon.check_orders(_expire_secs, _now, [this](uint32_t localid) {
      if (_ctx->cancel(localid)) {
        _ctx->writeLog(fmt::format("[{}] Expired order of {} canceled", localid,
                                   _code.c_str())
                           .c_str());
  ...
  if (!_cancel_map.empty()) {
    ...
    for (uint32_t localid : erro_cancel_orders) {
      ...
      _orders_mon.erase_order(localid);
      _ctx->writeLog(
          fmtutil::format("error order:{} canceled by {} times,erase forcely",
                          localid, _max_cancel_time));
    }
  }
...
void WtStockMinImpactExeUnit::on_trade(uint32_t localid, const char *stdCode,
                                       bool isBuy, double vol, double price) {}

void WtStockMinImpactExeUnit::on_entrust(uint32_t localid, const char *stdCode,
  ...
  if (!bSuccess) {
    ...
    _orders_mon.erase_order(localid);
    _ctx->writeLog(fmtutil::format("{} {} entrust failed erase from mon",
                                   _code.c_str(), localid));
  }
  // 委托不成功，重新处理
  do_calc();
...
void WtStockMinImpactExeUnit::set_position(const char *stdCode, double newVol) {
  ...
  if (is_clear() && decimal::eq(newVol, 0)) {
    _ctx->writeLog(
        fmt::format(
            "{} is in clearing processing, position can not be set to 0",
            stdCode)
            .c_str());
    return;
  }
  ...
  if (decimal::lt(newVol, 0)) {
    _ctx->writeLog(
        fmt::format("{} is an error stock target position", newVol).c_str());
    return;
  }
  ...
  if (is_clear())
    _ctx->writeLog(
        fmt::format("{} is set to be in clearing processing", stdCode).c_str());
  else
    _ctx->writeLog(fmt::format("Target position of {} is set tb be {}", stdCode,
                               _target_pos)
                       .c_str());
...
inline bool WtStockMinImpactExeUnit::is_clear() { return _is_clear; }
...
void WtStockMinImpactExeUnit::do_calc() {
  ...
  if (!decimal::eq(target_pos, _target_pos)) {
    _ctx->writeLog(
        fmtutil::format("{} can sell hold pos not enough, target adjust {}->{}",
                        stdCode, _target_pos, target_pos));
  ...
  _ctx->writeLog(
      fmtutil::format("{}: target: {} hold:{} left {} wait to execute",
                      _code.c_str(), target_pos, curPos, diffPos));
  // 在判断的时候，要两边四舍五入，防止一些碎股导致一直无法完成执行
  if (decimal::eq(round_hands(target_pos, _min_hands),
                  round_hands(curPos, _min_hands)) &&
      !(target_pos == 0 && curPos < _min_hands && curPos > target_pos)) {
    _ctx->writeLog(fmtutil::format("{}: target position {} set finish",
                                   _code.c_str(), _target_pos));
    _is_finish = true;
    return;
  }
  ...
  if (decimal::lt(diffPos * undone, 0)) {
    _ctx->writeLog(
        fmt::format("{} undone:{} diff:{} cancel", stdCode, undone, diffPos)
            .c_str());
    ...
    if (!ids.empty()) {
      ...
      for (auto localid : ids) {
        _ctx->writeLog(
            fmt::format("{} mon push wait cancel order {} enter time:{}",
                        _code.c_str(), localid, _now)
                .c_str());
        _ctx->writeLog(fmt::format("[{}] live opposite order of {} canceled",
                                   localid, _code.c_str())
                           .c_str());
      }
    }
  ...
  if (_last_tick == NULL) {
    _ctx->writeLog(
        fmt::format("No lastest tick data of {}, execute later", _code.c_str())
            .c_str());
    return;
  }
  ...
  if (_now - _last_place_time < _entrust_span) {
    _ctx->writeLog(fmtutil::format("entrust span {} last_place_time {} _now {}",
                                   _entrust_span, _last_place_time, _now));
    return;
  }
  ...
  if (isBuy) {
    ...
    if (_avaliable) {
      ...
      max_can_buy =
          (int)(max_can_buy / _min_order) * _min_order; // 2023.6.5-zhaoyk
  ...
  if (_price_mode == AUTOPX) {
    double mp = (_last_tick->bidqty(0) - _last_tick->askqty(0)) * 1.0 /
                (_last_tick->bidqty(0) + _last_tick->askqty(0));
    bool isUp = (mp > 0);
    if (isUp) {
    ...
    if (_price_mode == BESTPX) {
      ...
      buyPx =
          _last_tick->askprice(0) + _comm_info->getPriceTick() * _price_offset;
      sellPx =
          _last_tick->bidprice(0) - _comm_info->getPriceTick() * _price_offset;
    }
  }

  // 如果最后价格为0，再做一个修正
  if (decimal::eq(buyPx, 0.0))
    buyPx = decimal::eq(_last_tick->price(), 0.0) ? _last_tick->preclose()
                                                  : _last_tick->price();

  if (decimal::eq(sellPx, 0.0))
    sellPx = decimal::eq(_last_tick->price(), 0.0) ? _last_tick->preclose()
                                                   : _last_tick->price();
  ...
  if (!decimal::eq(_last_tick->upperlimit(), 0) &&
      decimal::gt(buyPx, _last_tick->upperlimit())) {
    _ctx->writeLog(
        fmt::format("Buy price {} of {} modified to upper limit price", buyPx,
                    _code.c_str(), _last_tick->upperlimit())
            .c_str());
    buyPx = _last_tick->upperlimit();
    isCanCancel = false; // 如果价格被修正为涨跌停价，订单不可撤销
  }

  if (!decimal::eq(_last_tick->lowerlimit(), 0) &&
      decimal::lt(sellPx, _last_tick->lowerlimit())) {
    _ctx->writeLog(
        fmt::format("Sell price {} of {} modified to lower limit price", sellPx,
                    _code.c_str(), _last_tick->lowerlimit())
            .c_str());
  ...
  if (isBuy) {
    ...
    for (auto id : ids) {
      _ctx->writeLog(fmt::format("{} mon push buy order {} enter time:{}",
                                 _code.c_str(), id, _now)
                         .c_str());
    }
  } else {
    ...
    _orders_mon.push_order(ids.data(), ids.size(), _now, isCanCancel);
    for (auto id : ids) {
      _ctx->writeLog(fmt::format("{} mon push sell order {} enter time:{}",
                                 _code.c_str(), id, _now)
                         .c_str());
    }
  }
