...
inline bool is_clear(double target) { return (target == DBL_MAX); }

WtMinImpactExeUnit::WtMinImpactExeUnit()
    : _last_tick(NULL), _comm_info(NULL), _price_mode(0), _price_offset(0),
      _expire_secs(0), _cancel_cnt(0), _target_pos(0), _cancel_times(0),
      _last_place_time(0), _last_tick_time(0), _in_calc(false),
      _min_open_lots(1) {}
...
const char *WtMinImpactExeUnit::getFactName() { return FACT_NAME; }

const char *WtMinImpactExeUnit::getName() { return "WtMinImpactExeUnit"; }

void WtMinImpactExeUnit::init(ExecuteContext *ctx, const char *stdCode,
                              WTSVariant *cfg) {
  ...
  ctx->writeLog(fmtutil::format(
      "MiniImpactExecUnit of {} inited, order price @ {}±{} ticks, expired "
      "after {} secs, reorder after {} millisec, lots policy: {} @ {:.2f}, min "
      "open lots: {}",
      stdCode, PriceModeNames[_price_mode + 1], _price_offset, _expire_secs,
      _entrust_span, _by_rate ? "byrate" : "byvol",
      _by_rate ? _qty_rate : _order_lots, _min_open_lots));
}
/*
 *	订单回报
 *	localid	本地单号
 ...
 *	isCanceled	是否已撤销
 */
void WtMinImpactExeUnit::on_order(uint32_t localid, const char *stdCode,
                                  bool isBuy, double leftover, double price,
                                  bool isCanceled) {
  {
    ...
    if (isCanceled || leftover == 0) // 已撤销或剩余订单为0
    {
      ...
      if (_cancel_cnt > 0) {
        _cancel_cnt--;
        _ctx->writeLog(fmtutil::format(
            "[{}@{}] Order of {} cancelling done, cancelcnt -> {}", __FILE__,
            __LINE__, _code.c_str(), _cancel_cnt));
      }
    }
  ...
  if (isCanceled) {
    _ctx->writeLog(fmtutil::format(
        "Order {} of {} canceled, recalc will be done", localid, stdCode));
    _cancel_times++;
    do_calc();
  }
}
...
void WtMinImpactExeUnit::on_channel_ready() {
  ...
  if (!decimal::eq(undone, 0) && !_orders_mon.has_order()) {
     ...
     */
    _ctx->writeLog(fmtutil::format(
        "Unmanaged live orders with qty {} of {} found, cancel all", undone,
        _code.c_str()));

    bool isBuy = (undone > 0);
    OrderIDs ids = _ctx->cancel(_code.c_str(), isBuy); // 根据本地订单号撤单
    _orders_mon.push_order(ids.data(), ids.size(),
                           _ctx->getCurTime()); // push into orderpair
    _cancel_cnt += ids.size();

    _ctx->writeLog(fmtutil::format("[{}@{}]cancelcnt -> {}", __FILE__, __LINE__,
                                   _cancel_cnt));
     ...
     */
    _ctx->writeLog(fmtutil::format(
        "Local orders of {} not confirmed in trading channel, clear all",
        _code.c_str()));
    _orders_mon.clear_orders();
  } else {
    _ctx->writeLog(fmtutil::format(
        "Unrecognized condition while channle ready, {:.2f} live orders of {} "
        "exists, local orders {}exist",
...
void WtMinImpactExeUnit::on_tick(WTSTickData *newTick) {
  ...
  if (_expire_secs != 0 && _orders_mon.has_order() &&
      _cancel_cnt == 0) // 订单超时秒数！=0&&hasorder && 撤单量==0
  {
    ...
    _orders_mon.check_orders(_expire_secs, now, [this](uint32_t localid) {
      if (_ctx->cancel(localid)) {
        _cancel_cnt++;
        _ctx->writeLog(fmtutil::format(
            "[{}@{}] Expired order of {} canceled, cancelcnt -> {}", __FILE__,
            __LINE__, _code.c_str(), _cancel_cnt));
      }
    });
  }
 ...
 *	price	成交价格
 */
void WtMinImpactExeUnit::on_trade(uint32_t localid, const char *stdCode,
 ...
 *	下单结果回报
 */
void WtMinImpactExeUnit::on_entrust(uint32_t localid, const char *stdCode,
...
void WtMinImpactExeUnit::do_calc() {
  ...
  if (decimal::lt(
          diffPos * undone,
          0)) // diff*undone 剩余需要完成*未完成订单 <0   true   -> cancel
  {
    ...
    if (!ids.empty()) {
      ...
      _cancel_cnt += ids.size();
      _ctx->writeLog(fmtutil::format(
          "[{}@{}] live opposite order of {} canceled, cancelcnt -> {}",
          __FILE__, __LINE__, _code.c_str(), _cancel_cnt)); // 相反的订单已取消
    }
    return;
  }

  // 因为是逐笔发单，所以如果有不需要撤销的未完成单，则暂不发单
  //****此处逐笔指 拆单的回合中上一笔订单量未完结，则不发单，
  //逐笔发单指每个回合成交的单
  ...
  if (_last_tick == NULL) {
    _ctx->writeLog(fmtutil::format("No lastest tick data of {}, execute later",
                                   _code.c_str()));
    return;
  }
  ...
  if (now - _last_place_time <
      _entrust_span) // 当前时间-上ticktime <发单时间间隔
    return;
  ...
  if (decimal::eq(curPos, newVol)) {
    ...
    double lPos = _ctx->getPosition(
        stdCode, true, 1); // 获取（多头）持仓  可用持仓   多头 //返回值
                           // 轧平后的仓位: 多仓>0, 空仓<0
    ...
    _ctx->writeLog(fmtutil::format(
        "Clearing process triggered, target position of {} has been set to {}",
        _code.c_str(), newVol));
  }

  bool bForceClose = is_clear(_target_pos); // target==del_max  return 1

  bool isBuy = decimal::gt(
      newVol, curPos); // 如何判断买卖方向    真实目标仓位-获取持仓   > 0 多头

  // 如果相比上次没有更新的tick进来，则先不下单，防止开盘前集中下单导致通道被封
  uint64_t curTickTime = (uint64_t)_last_tick->actiondate() * 1000000000 +
                         _last_tick->actiontime();
  if (curTickTime <= _last_tick_time) {
    _ctx->writeLog(
        fmtutil::format("No tick of {} updated, {} <= {}, execute later", _code,
                        curTickTime, _last_tick_time));
    return;
  }
  ...
  {
    this_qty = isBuy ? _last_tick->askqty(0)
                     : _last_tick->bidqty(0); // isbuy ture   askqty 买价量
  ...
  bool isOpen =
      (isBuy && decimal::ge(curPos, 0)) || (!isBuy && decimal::le(curPos, 0));
  ...
  if (isOpen &&
      decimal::lt(
          this_qty,
          _min_open_lots)) // if 开仓&&this_qty<min_open_lots（最小开仓数量）
  {
    this_qty = _min_open_lots;
    _ctx->writeLog(fmtutil::format(
        "Lots of {} changed from {} to {} due to minimum open lots", _code,
        this_qty, _min_open_lots));
  }
  ...
  if (_price_mode == 2) // 价格类型,0-最新价,-1-最优价,1-对手价,2-自动,默认为0
  {
    ...
    if (decimal::eq(buyPx, 0.0)) // buypx==0，return ture
      buyPx = decimal::eq(_last_tick->price(), 0.0)
                  ? _last_tick->preclose()
                  : _last_tick->price(); // 如果上一tick ==0  取lasttick 收盘价

    if (decimal::eq(sellPx, 0.0))
      sellPx = decimal::eq(_last_tick->price(), 0.0) ? _last_tick->preclose()
                                                     : _last_tick->price();

    buyPx += _comm_info->getPriceTick() *
             _cancel_times; // // * 撤单次数    扩大追单步幅
  ...
  if (!decimal::eq(_last_tick->upperlimit(), 0) &&
      decimal::gt(buyPx, _last_tick->upperlimit())) // upperlimit涨停价
  {
    _ctx->writeLog(fmtutil::format(
        "Buy price {} of {} modified to upper limit price", buyPx,
        _code.c_str(), _last_tick->upperlimit())); // 买入价，，，改为上限价
  ...
  if (!decimal::eq(_last_tick->lowerlimit(), 0) &&
    ...
    _ctx->writeLog(
        fmtutil::format("Sell price {} of {} modified to lower limit price",
                        sellPx, _code.c_str(), _last_tick->lowerlimit()));
...
void WtMinImpactExeUnit::set_position(const char *stdCode, double newVol) {
  ...
  if (is_clear(_target_pos) && decimal::eq(newVol, 0)) {
    _ctx->writeLog(fmtutil::format(
        "{} is in clearing processing, position can not be set to 0", stdCode));
    return;
  }
  ...
  if (is_clear(_target_pos))
    _ctx->writeLog(
        fmtutil::format("{} is set to be in clearing processing", stdCode));
  else
    _ctx->writeLog(fmtutil::format("Target position of {} is set tb be {}",
                                   stdCode, _target_pos));
