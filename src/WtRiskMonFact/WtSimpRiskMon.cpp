...
const char *WtSimpleRiskMon::getName() { return "WtSimpleRiskMon"; }

const char *WtSimpleRiskMon::getFactName() { return FACT_NAME; }
...
void WtSimpleRiskMon::init(WtPortContext *ctx, WTSVariant *cfg) {
  ...
  ctx->writeRiskLog(
      fmt::format("Params inited, Checking frequency: {} s, MaxIDD: "
                  "{}({:.2f}%), MaxMDD: {}({:.2f}%), Capital: {:.1f}, Profit "
                  "Boudary: {:.2f}%, Calc Span: {} mins, Risk Scale: {:.2f}",
                  _calc_span, _inner_day_active ? "ON" : "OFF", _inner_day_fd,
                  _multi_day_active ? "ON" : "OFF", _multi_day_fd, _base_amount,
                  _basic_ratio, _risk_span, _risk_scale)
          .c_str());
}
  ...
  _thrd.reset(new std::thread([this]() {
    while (!_stopped) {
      if (_ctx && _ctx->isInTrading()) {
         ...
         *
         * 动作:
         * 方式A:  所有品种减仓（减少到 30% 仓位）,
         *下一交易日重新按策略新仓位补齐 方式B:  所有盈利品种都 平仓,
         *下一交易日重新按策略新仓位补齐
         */

        if (_inner_day_active && fs._max_dyn_bal != DBL_MAX) {
          double predynbal =
              fundInfo->predynbalance() + _base_amount; // 上日动态权益
          double maxBal = fs._max_dyn_bal + _base_amount; // 当日最大动态权益
          double curBal =
              fs._balance + fs._dynprofit + _base_amount; // 当前动态权益

          double rate = 0.0;
          if (!decimal::eq(maxBal, predynbal))
            rate = (maxBal - curBal) * 100 /
                   (maxBal - predynbal); // 当日盈利回撤比例
          ...
          if (maxBal > (_basic_ratio * predynbal / 100.0)) {
             ...
             */
            uint32_t maxTime =
                _ctx->transTimeToMin(fundInfo->max_dynbal_time());
            uint32_t curTime =
                _ctx->transTimeToMin(_ctx->getCurTime()); // 转成日内分钟数

            if (rate >= _inner_day_fd && curTime - maxTime <= _risk_span &&
                !_limited) {
              _ctx->writeRiskLog(
                  fmt::format("Current IDD {:.2f}%, ≥MaxIDD {:.2f}%, Position "
                              "down to {:.1f}%",
                              rate, _inner_day_fd, _risk_scale)
                      .c_str());
              ...
              _ctx->writeRiskLog(
                  fmt::format(
                      "Current Balance Ratio: {:.2f}%, Current IDD: {:.2f}%",
                      curBal * 100.0 / predynbal, rate)
                      .c_str());
              //_limited = false;
            }
          } else {
            // 如果当日最大权益没有超过盈利边界条件
            _ctx->writeRiskLog(fmt::format("Current Balance Ratio: {:.2f}%",
                                           curBal * 100.0 / predynbal)
                                   .c_str());
            //_limited = false;
          }
        }
        ...
        if (_multi_day_active && fs._max_md_dyn_bal._date != 0) {
          ...
          if (curBal < maxBal) {
            ...
            if (rate >= _multi_day_fd) {
              _ctx->writeRiskLog(fmt::format("Current MDD {:.2f}%, >= MaxMDD "
                                             "{:.2f}%, Position down to 0.0%",
                                             rate, _multi_day_fd)
                                     .c_str());
