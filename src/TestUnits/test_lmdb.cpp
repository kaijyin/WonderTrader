...
TEST(test_lmdb, test_query) {
...

  {
    ...
    query.get_lowers(std::string("key-000"), std::string("key-030"), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 10);
                     });

    query.get_lowers(std::string("key-000"), std::string("key-005"), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 5);
                     });

    query.get_uppers(std::string("key-030"), std::string("key-999"), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 10);
                     });

    query.get_uppers(std::string("key-095"), std::string("key-999"), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 6);
                     });

    query.get_range(std::string("key-030"), std::string("key-050"),
                    [](const ValueArray &ayKeys, const ValueArray &ayVals) {
...
TEST(test_lmdb, test_endian) {
...

  {
    ...
    query.get_lowers(makeData(0, true), makeData(20220120, true), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 10);
    ...
    query.get_lowers(makeData(0, true), makeData(20220105, true), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 5);
                       for (const std::string &item : ayVals) {
                         uint32_t iv = *((uint32_t *)item.data());
    ...
    query.get_uppers(makeData(20220150, true), makeData(20229999, true), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 10);
                       for (const std::string &item : ayVals) {
                         uint32_t iv = *((uint32_t *)item.data());
    ...
    query.get_uppers(makeData(20220196, true), makeData(20229999, true), 10,
                     [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                       EXPECT_EQ(ayKeys.size(), 5);
                       for (const std::string &item : ayVals) {
                         uint32_t iv = *((uint32_t *)item.data());
    ...
    query.get_range(makeData(20220130, true), makeData(20220150, true),
                    [](const ValueArray &ayKeys, const ValueArray &ayVals) {
                      EXPECT_EQ(ayKeys.size(), 21);
                      for (const std::string &item : ayVals) {
                        uint32_t iv = *((uint32_t *)item.data());
