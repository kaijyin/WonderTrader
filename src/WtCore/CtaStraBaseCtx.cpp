...
CtaStraBaseCtx::CtaStraBaseCtx(WtCtaEngine *engine, const char *name,
                               int32_t slippage)
    : ICtaStraCtx(name), _engine(engine), _total_calc_time(0), _emit_times(0),
      _last_cond_min(0), _is_in_schedule(false), _ud_modified(false),
      _last_barno(0), _slippage(slippage) {
...
void CtaStraBaseCtx::init_outputs() {
  ...
  _trade_logs.reset(new BoostFile());
  {
    ...
    if (isNewFile) {
      _trade_logs->write_file(
          "code,time,direct,action,price,qty,tag,fee,barno\n");
    } else {
...
void CtaStraBaseCtx::log_signal(const char *stdCode, double target,
                                double price, uint64_t gentime,
                                const char *usertag /* = "" */) {
  if (_sig_logs) {
    std::stringstream ss;
    ss << stdCode << "," << target << "," << price << "," << gentime << ","
       << usertag << "\n";
    _sig_logs->write_file(ss.str());
  }
}

void CtaStraBaseCtx::log_trade(const char *stdCode, bool isLong, bool isOpen,
  ...
  if (_trade_logs) {
    std::stringstream ss;
    ss << stdCode << "," << curTime << "," << (isLong ? "LONG" : "SHORT") << ","
       << (isOpen ? "OPEN" : "CLOSE") << "," << price << "," << qty << ","
       << userTag << "," << fee << "," << barNo << "\n";
    _trade_logs->write_file(ss.str());
  }

  _engine->notify_trade(this->name(), stdCode, isLong, isOpen, curTime, price,
                        userTag);
}

void CtaStraBaseCtx::log_close(const char *stdCode, bool isLong,
                               uint64_t openTime, double openpx,
                               uint64_t closeTime, double closepx, double qty,
  ...
  if (_close_logs) {
    std::stringstream ss;
    ss << stdCode << "," << (isLong ? "LONG" : "SHORT") << "," << openTime
       << "," << openpx << "," << closeTime << "," << closepx << "," << qty
       << "," << profit << "," << totalprofit << "," << enterTag << ","
  ...
  { // 读取仓位
    ...
    if (!jPos.IsNull() && jPos.IsArray()) {
      for (const rj::Value &pItem : jPos.GetArray()) {
        ...
        bool isExpired = (strlen(ruleTag) == 0 &&
                          _engine->get_contract_info(stdCode) == NULL);
      ...
      for (auto &m : jItems.GetObject()) {
        ...
        for (auto &cItem : cListItem.GetArray()) {
          ...
          log_info("{} condition recovered, {} {}, condition: newprice {} {}",
                   stdCode, ACTION_NAMES[condInfo._action], condInfo._qty,
                   CMP_ALG_NAMES[condInfo._alg], condInfo._target);
          count++;
        }
      }

      log_info("{} conditions recovered, setup time: {}", count,
               _last_cond_min);
    }
  }
  ...
  if (root.HasMember("signals")) {
    ...
    if (!jSignals.IsNull() && jSignals.IsObject()) {
      for (auto &m : jSignals.GetObject()) {
        ...
        log_info("{} untouched signal recovered, target pos: {}", stdCode,
                 sInfo._volume);
        stra_sub_ticks(stdCode);
      }
    }
...
//////////////////////////////////////////////////////////////////////////
// 回调函数
void CtaStraBaseCtx::on_bar(const char *stdCode, const char *period,
...
void CtaStraBaseCtx::on_tick(const char *stdCode, WTSTickData *newTick,
  ...
  // 先检查是否要信号要触发
  {
    ...
    if (it != _sig_map.end()) {
      ...
      if (sInfo->isInTradingTime(_engine->get_raw_time(), true)) {
        ...
        do_set_position(stdCode, sInfo._volume, sInfo._usertag.c_str(),
                        (sInfo._sigtype != 0 && !sInfo._triggered));
        _sig_map.erase(it);

        // 如果是条件单触发，则回调on_condition_triggered
        if (sInfo._sigtype == 2)
          on_condition_triggered(stdCode, sInfo._volume, newTick->price(),
  ...
  if (!_condtions.empty()) {
    ...
    for (const CondEntrust &entrust : condList) {
      ...
      if (isMatched) {
        log_info("Condition triggered[newprice {}{} targetprice {}], "
                 "instrument: {}, {} {}",
                 curPrice, CMP_ALG_NAMES[entrust._alg], entrust._target,
                 stdCode, ACTION_NAMES[entrust._action], entrust._qty);
  ...
  for (auto it = _kline_tags.begin(); it != _kline_tags.end(); it++) {
    ...
    if (isMainUdt || _kline_tags.empty()) {
      ...
      if (offTime <= sInfo->getCloseTime(true)) {
        ...
        if (_emit_times % 20 == 0) {
          log_info("Strategy has been scheduled {} times, totally taking {} "
                   "us, {:.3f} us each time",
...
void CtaStraBaseCtx::on_session_begin(uint32_t uTDate) {
  ...
  for (auto &it : _pos_map) {
    const char *stdCode = it.first.c_str();
    PosInfo &pInfo = (PosInfo &)it.second;
    if (pInfo._frozen_date != 0 && pInfo._frozen_date < uTDate &&
        !decimal::eq(pInfo._frozen, 0)) {
      log_debug("{} of %s frozen on {} released on {}", pInfo._frozen, stdCode,
                pInfo._frozen_date, uTDate);
...
//////////////////////////////////////////////////////////////////////////
// 策略接口
void CtaStraBaseCtx::stra_enter_long(const char *stdCode, double qty,
                                     const char *userTag /* = "" */,
...
void CtaStraBaseCtx::stra_enter_short(const char *stdCode, double qty,
                                      const char *userTag /* = "" */,
                                      double limitprice, double stopprice) {
  WTSCommodityInfo *commInfo = _engine->get_commodity_info(stdCode);
  if (commInfo == NULL) {
...
void CtaStraBaseCtx::stra_exit_long(const char *stdCode, double qty,
                                    const char *userTag /* = "" */,
                                    double limitprice, double stopprice) {
  WTSCommodityInfo *commInfo = _engine->get_commodity_info(stdCode);
  if (commInfo == NULL) {
...
void CtaStraBaseCtx::stra_exit_short(const char *stdCode, double qty,
                                     const char *userTag /* = "" */,
                                     double limitprice, double stopprice) {
  WTSCommodityInfo *commInfo = _engine->get_commodity_info(stdCode);
  if (commInfo == NULL) {
...
double CtaStraBaseCtx::stra_get_day_price(const char *stdCode,
                                          int flag /* = 0 */) {
  if (_engine)
...
void CtaStraBaseCtx::stra_set_position(const char *stdCode, double qty,
                                       const char *userTag /* = "" */,
...
void CtaStraBaseCtx::append_signal(const char *stdCode, double qty,
                                   const char *userTag /* = "" */,
                                   uint32_t sigType) {
  ...
  sInfo._gentime = (uint64_t)_engine->get_date() * 1000000000 +
                   (uint64_t)_engine->get_raw_time() * 100000 +
                   _engine->get_secs();
...
void CtaStraBaseCtx::do_set_position(const char *stdCode, double qty,
                                     const char *userTag /* = "" */,
                                     bool bFireAtOnce /* = false */) {
  PosInfo &pInfo = _pos_map[stdCode];
  double curPx = _price_map[stdCode];
  uint64_t curTm =
      (uint64_t)_engine->get_date() * 10000 + _engine->get_min_time();
  ...
  if (decimal::gt(
    ...
    for (auto it = pInfo._details.begin(); it != pInfo._details.end(); it++) {
      ...
      // 浮盈也要做等比缩放
      pInfo._dynprofit =
          pInfo._dynprofit * dInfo._volume / (dInfo._volume + maxQty);
      pInfo._last_exittime = curTm;
      _fund_info._total_profit += profit;

      // 计算手续费
      // double fee = _engine->calc_fee(stdCode, trdPx, maxQty, dInfo._opentdate
      // == curTDate ? 2 : 1);
      double fee = commInfo->calcFee(trdPx, maxQty,
                                     dInfo._opentdate == curTDate ? 2 : 1);
      _fund_info._total_fees += fee;

      // 这里写平仓记录
      log_close(stdCode, dInfo._long, dInfo._opentime, dInfo._price, curTm,
                trdPx, maxQty, profit, pInfo._closeprofit, dInfo._opentag,
                userTag, dInfo._open_barno, _last_barno);
...
WTSKlineSlice *CtaStraBaseCtx::stra_get_bars(const char *stdCode,
                                             const char *period, uint32_t count,
                                             bool isMain /* = false */) {
  thread_local static char key[64] = {0};
  ...
  WTSKlineSlice *kline =
      _engine->get_kline_slice(_context_id, stdCode, basePeriod, count, times);
  if (kline) {
    // 如果K线获取不到,说明也不会有闭合事件发生,所以不更新本地标记
    bool isFirst =
        (_kline_tags.find(key) ==
         _kline_tags.end()); // 如果没有保存标记,说明是第一次拉取该K线
    KlineTag &tag = _kline_tags[key];
    tag._closed = false;
    ...
    if (isMain && isFirst && !_condtions.empty()) {
      ...
      if (lastBartime > _last_cond_min) {
        log_info("Conditions expired, setup time: {}, time of last bar of main "
                 "kbars: {}, all cleared",
                 _last_cond_min, lastBartime);
        _condtions.clear();
      }
    }
...
WTSTickSlice *CtaStraBaseCtx::stra_get_ticks(const char *stdCode,
                                             uint32_t count) {
...
void CtaStraBaseCtx::stra_sub_bar_events(const char *stdCode,
                                         const char *period) {
  thread_local static char key[64] = {0};
  fmtutil::format_to(key, "{}#{}", stdCode, period);
...
uint32_t CtaStraBaseCtx::stra_get_date() { return _engine->get_date(); }

uint32_t CtaStraBaseCtx::stra_get_time() { return _engine->get_min_time(); }
...
const char *CtaStraBaseCtx::stra_load_user_data(const char *key,
                                                const char *defVal /*= ""*/) {
...
double CtaStraBaseCtx::stra_get_position(const char *stdCode,
                                         bool bOnlyValid /* = false */,
                                         const char *userTag /* = "" */) {
  double totalPos = 0;
  auto sit = _sig_map.find(stdCode);
  if (sit != _sig_map.end()) {
    WTSLogger::warn("{} has untouched signal, [userTag] will be ignored",
                    stdCode);
...
uint64_t CtaStraBaseCtx::stra_get_detail_entertime(const char *stdCode,
                                                   const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double CtaStraBaseCtx::stra_get_detail_cost(const char *stdCode,
                                            const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double CtaStraBaseCtx::stra_get_detail_profit(const char *stdCode,
                                              const char *userTag,
                                              int flag /* = 0 */) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
...
void CtaStraBaseCtx::add_chart_mark(double price, const char *icon,
                                    const char *tag) {
  if (!_is_in_schedule) {
...
bool CtaStraBaseCtx::register_index_line(const char *idxName,
                                         const char *lineName,
                                         uint32_t lineType) {
...
bool CtaStraBaseCtx::add_index_baseline(const char *idxName,
...
bool CtaStraBaseCtx::set_index_value(const char *idxName, const char *lineName,
                                     double val) {
  if (!_is_in_schedule) {
    WTSLogger::error("Marks can be added only during schedule");
    return false;
  ...
  if (_idx_logs) {
    std::stringstream ss;
    ss << curTime << "," << idxName << "," << lineName << "," << val
       << std::endl;
    ;
