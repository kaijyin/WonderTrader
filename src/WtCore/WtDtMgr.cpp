...
WtDtMgr::WtDtMgr()
    : _reader(NULL), _engine(NULL), _loader(NULL), _bars_cache(NULL),
      _ticks_adjusted(NULL), _rt_tick_map(NULL), _force_cache(false) {}
...
bool WtDtMgr::initStore(WTSVariant *cfg) {
  ...
  FuncCreateDataReader funcCreator =
      (FuncCreateDataReader)DLLHelper::get_symbol(hInst, "createDataReader");
  if (funcCreator == NULL) {
    WTSLogger::error("Loading data reader module {} failed, entrance function "
                     "createDataReader not found",
                     module.c_str());
  ...
  _reader = funcCreator();
  if (_reader == NULL) {
    WTSLogger::error("Creating instance of data reader module {} failed",
                     module.c_str());
    DLLHelper::free_library(hInst);
    return false;
  }
...
IHotMgr *WtDtMgr::get_hot_mgr() { return _engine->get_hot_mgr(); }

uint32_t WtDtMgr::get_date() { return _engine->get_date(); }

uint32_t WtDtMgr::get_min_time() { return _engine->get_min_time(); }

uint32_t WtDtMgr::get_secs() { return _engine->get_secs(); }

void WtDtMgr::reader_log(WTSLogLevel ll, const char *message) {
  WTSLogger::log_raw(ll, message);
}

void WtDtMgr::on_bar(const char *code, WTSKlinePeriod period,
                     WTSBarStruct *newBar) {
  ...
  for (auto it = _bars_cache->begin(); it != _bars_cache->end(); it++) {
    ...
    if (kData->times() != 1) {
      ...
      if (kData->isClosed()) {
        ...
        // 更新完K线以后, 统一通知交易引擎
        _bar_notifies.emplace_back(
            NotifyItem(code, speriod, times * kData->times(), lastBar));
      }
    } else {
...
WTSTickSlice *WtDtMgr::get_tick_slice(const char *stdCode, uint32_t count,
                                      uint64_t etime /* = 0 */) {
  if (_reader == NULL)
    return NULL;
  ...
  if (it == _ticks_adjusted->end()) {
    ...
    WTSTickSlice *slice =
        _reader->readTickSlice(pureStdCode.c_str(), 999999, etime);
    std::vector<WTSTickStruct> ayTicks;
    ayTicks.resize(slice->size());
    std::size_t offset = 0;
    for (std::size_t bIdx = 0; bIdx < slice->get_block_counts(); bIdx++) {
      memcpy(&ayTicks[0] + offset, slice->get_block_addr(bIdx),
             slice->get_block_size(bIdx) * sizeof(WTSTickStruct));
  ...
  WTSTickStruct *pTick =
      std::lower_bound(&ticks.front(), &ticks.back(), eTick,
  ...
  WTSTickSlice *slice =
      WTSTickSlice::create(stdCode, &ticks.front() + sIdx, cnt);
  return slice;
}

WTSOrdQueSlice *WtDtMgr::get_order_queue_slice(const char *stdCode,
                                               uint32_t count,
                                               uint64_t etime /* = 0 */) {
  if (_reader == NULL)
    return NULL;
...
WTSOrdDtlSlice *WtDtMgr::get_order_detail_slice(const char *stdCode,
                                                uint32_t count,
                                                uint64_t etime /* = 0 */) {
  if (_reader == NULL)
    return NULL;
...
WTSTransSlice *WtDtMgr::get_transaction_slice(const char *stdCode,
                                              uint32_t count,
                                              uint64_t etime /* = 0 */) {
  if (_reader == NULL)
    return NULL;
...
WTSKlineSlice *WtDtMgr::get_kline_slice(const char *stdCode,
  ...
  if (kData == NULL || kData->size() < count) {
    ...
    WTSKlineSlice *rawData =
        _reader->readKlineSlice(stdCode, period, realCount, etime);
    if (rawData != NULL && rawData->size() > 0) {
      if (times != 1) {
        ...
        for (uint32_t bIdx = 0; bIdx < rawData->get_block_counts(); bIdx++) {
          memcpy(pBar, rawData->get_block_addr(bIdx),
                 sizeof(WTSBarStruct) * rawData->get_block_size(bIdx));
    ...
    if (kData) {
      ...
      if (times != 1)
        WTSLogger::debug("{} bars of {} resampled every {} bars: {} -> {}",
                         PERIOD_NAME[period], stdCode, times, realCount,
                         kData->size());
    }
  }
