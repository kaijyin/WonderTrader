...
typedef std::function<void(const char *, bool, double, double, double, double)>
    FuncEnumChnlPosCallBack;
...
public:
  ...
  typedef struct _PosItem {
    ...
    _PosItem() { memset(this, 0, sizeof(_PosItem)); }
  ...
  typedef struct _RiskParams {
    ...
    _RiskParams() { memset(this, 0, sizeof(_RiskParams)); }
  } RiskParams;
...
public:
  ...
  void addSink(ITrdNotifySink *sink) { _sinks.insert(sink); }
...
private:
  ...
  inline void logTrade(uint32_t localid, const char *stdCode,
                       WTSTradeInfo *trdInfo);
  inline void logOrder(uint32_t localid, const char *stdCode,
...
public:
  ...
  uint32_t openShort(const char *stdCode, double price, double qty, int flag,
                     WTSContractInfo *cInfo = NULL);
  uint32_t closeLong(const char *stdCode, double price, double qty,
                     bool isToday, int flag, WTSContractInfo *cInfo = NULL);
  uint32_t closeShort(const char *stdCode, double price, double qty,
                      bool isToday, int flag, WTSContractInfo *cInfo = NULL);

  OrderIDs buy(const char *stdCode, double price, double qty, int flag,
               bool bForceClose, WTSContractInfo *cInfo = NULL);
  OrderIDs sell(const char *stdCode, double price, double qty, int flag,
                bool bForceClose, WTSContractInfo *cInfo = NULL);
...
private:
  ...
  wt_hashmap<std::string, std::string>
      _trade_refs;                       // 用于记录成交单和订单的匹配
