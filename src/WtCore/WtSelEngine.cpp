...
WtSelEngine::WtSelEngine() : _terminated(false), _cfg(NULL) {}
...
void WtSelEngine::on_bar(const char *stdCode, const char *period,
      ...
      for (auto it = sids.begin(); it != sids.end(); it++) {
        ...
        if (cit != _ctx_map.end()) {
          ...
          if (opt == 0) {
            ...
            wCode = fmt::format("{}{}", stdCode,
                                opt == 1 ? SUFFIX_QFQ : SUFFIX_HFQ);
            if (opt == 1) {
...
void WtSelEngine::on_minute_end(uint32_t curDate, uint32_t curTime) {
  ...
  for (auto &v : _tasks) {
    ...
    case TPT_Minute: {
      ...
      if (minutes != 0 && (minutes % tInfo->_time ==
                           0)) // 如果分钟数能被整除,且不为0,则可以触发
      {
        bIgnore = false;
      }
    } break;
    case TPT_Monthly:
      ...
      if (_cur_date % 1000000 == tInfo->_day)
        ...
        if ((preTDate % 10000 / 100 < _cur_date % 10000 / 100) &&
            _cur_date % 1000000 > tInfo->_day) {
          bIgnore = false;
        } else if (preTDate % 1000000 < tInfo->_day &&
                   _cur_date % 1000000 > tInfo->_day) {
...
void WtSelEngine::init(WTSVariant *cfg, IBaseDataMgr *bdMgr, WtDtMgr *dataMgr,
...
void WtSelEngine::addContext(SelContextPtr ctx, uint32_t date, uint32_t time,
                             TaskPeriodType period, bool bStrict /* = true */,
                             const char *trdtpl /* = "CHINA" */,
                             const char *sessionID /* ="TRADING" */) {
  if (ctx == NULL)
    return;

  auto it = _tasks.find(ctx->id());
  if (it != _tasks.end()) {
    WTSLogger::error("Task registration failed: task id {} already registered",
                     ctx->id());
    return;
  }
...
void WtSelEngine::handle_pos_change(const char *straName, const char *stdCode,
                                    double diffQty) {
  ...
  //	std::string code = _hot_mgr->getRawCode(cInfo._exchg, cInfo._product,
  //_cur_tdate); 	realCode = CodeHelper::rawMonthCodeToStdCode(code.c_str(),
  //cInfo._exchg);
  // }
  // else if (CodeHelper::isStdFut2ndCode(stdCode))
  //{
  //	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode);
  //	std::string code = _hot_mgr->getSecondRawCode(cInfo._exchg,
  //cInfo._product, _cur_tdate); 	realCode =
  ...
  for (auto &execid : exec_ids)
    _exec_mgr.handle_pos_change(realCode.c_str(), targetPos, diffQty,
                                execid.c_str());
}
