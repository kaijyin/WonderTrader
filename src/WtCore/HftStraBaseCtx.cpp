...
HftStraBaseCtx::HftStraBaseCtx(WtHftEngine *engine, const char *name,
                               bool bAgent, int32_t slippage)
    : IHftStraCtx(name), _engine(engine), _data_agent(bAgent),
      _slippage(slippage) {
...
uint32_t HftStraBaseCtx::id() { return _context_id; }

void HftStraBaseCtx::setTrader(TraderAdapter *trader) { _trader = trader; }
...
void HftStraBaseCtx::on_order_queue(const char *stdCode,
...
void HftStraBaseCtx::on_order_detail(const char *stdCode,
...
void HftStraBaseCtx::on_transaction(const char *stdCode,
...
void HftStraBaseCtx::on_bar(const char *stdCode, const char *period,
...
OrderIDs HftStraBaseCtx::stra_cancel(const char *stdCode, bool isBuy,
                                     double qty) {
  // 撤单频率检查
...
OrderIDs HftStraBaseCtx::stra_buy(const char *stdCode, double price, double qty,
  ...
  CodeHelper::CodeInfo cInfo =
      CodeHelper::extractStdCode(stdCode, _engine->get_hot_mgr());
  if (strlen(cInfo._ruletag) > 0) {
    std::string code = _engine->get_hot_mgr()->getCustomRawCode(
        cInfo._ruletag, cInfo.stdCommID(), _engine->get_trading_date());
    std::string realCode =
        CodeHelper::rawMonthCodeToStdCode(code.c_str(), cInfo._exchg);

    WTSContractInfo *ct =
        _engine->get_basedata_mgr()->getContract(code.c_str(), cInfo._exchg);
    ...
    auto ids =
        _trader->buy(realCode.c_str(), price, qty, flag, bForceClose, ct);
    ...
    WTSContractInfo *ct =
        _engine->get_basedata_mgr()->getContract(cInfo._code, cInfo._exchg);
    if (ct == NULL) {
...
OrderIDs HftStraBaseCtx::stra_sell(const char *stdCode, double price,
  ...
  WTSCommodityInfo *commInfo =
      _engine->get_basedata_mgr()->getCommodity(cInfo._exchg, cInfo._product);
  ...
  if (strlen(cInfo._ruletag) > 0) {
    ...
    auto ids =
        _trader->sell(realCode.c_str(), price, qty, flag, bForceClose, ct);
    for (auto localid : ids)
      setUserTag(localid, userTag);
    return ids;
  } else {
...
uint32_t HftStraBaseCtx::stra_enter_long(const char *stdCode, double price,
                                         double qty, const char *userTag,
                                         int flag /* = 0*/) {
...
uint32_t HftStraBaseCtx::stra_exit_long(const char *stdCode, double price,
                                        double qty, const char *userTag,
                                        bool isToday /* = false*/,
                                        int flag /* = 0*/) {
  std::string realCode = stdCode;
...
uint32_t HftStraBaseCtx::stra_enter_short(const char *stdCode, double price,
                                          double qty, const char *userTag,
                                          int flag /* = 0*/) {
  std::string realCode = stdCode;
  CodeHelper::CodeInfo cInfo =
  ...
  //	std::string code = _engine->get_hot_mgr()->getRawCode(cInfo._exchg,
  //cInfo._product, _engine->get_trading_date()); 	realCode =
  //CodeHelper::rawMonthCodeToStdCode(code.c_str(), cInfo._exchg);
  ...
  //	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode);
  //	std::string code =
  //_engine->get_hot_mgr()->getSecondRawCode(cInfo._exchg, cInfo._product,
  //_engine->get_trading_date()); 	realCode =
  //CodeHelper::rawMonthCodeToStdCode(code.c_str(), cInfo._exchg);
...
uint32_t HftStraBaseCtx::stra_exit_short(const char *stdCode, double price,
                                         double qty, const char *userTag,
                                         bool isToday /* = false*/,
                                         int flag /* = 0*/) {
  std::string realCode = stdCode;
...
WTSKlineSlice *HftStraBaseCtx::stra_get_bars(const char *stdCode,
                                             const char *period,
                                             uint32_t count) {
...
WTSTickSlice *HftStraBaseCtx::stra_get_ticks(const char *stdCode,
                                             uint32_t count) {
...
WTSOrdDtlSlice *HftStraBaseCtx::stra_get_order_detail(const char *stdCode,
                                                      uint32_t count) {
  WTSOrdDtlSlice *ret =
...
WTSOrdQueSlice *HftStraBaseCtx::stra_get_order_queue(const char *stdCode,
                                                     uint32_t count) {
  WTSOrdQueSlice *ret =
...
WTSTransSlice *HftStraBaseCtx::stra_get_transaction(const char *stdCode,
                                                    uint32_t count) {
  WTSTransSlice *ret =
...
void HftStraBaseCtx::on_trade(uint32_t localid, const char *stdCode, bool isBuy,
  ...
  if (_sig_logs && _data_agent) {
    ...
    _sig_logs->write_file(fmt::format("{}.{}.{},{}{},{},{}\n", stra_get_date(),
                                      stra_get_time(), stra_get_secs(),
                                      isBuy ? "+" : "-", vol, curPos, price));
  }
...
void HftStraBaseCtx::on_order(uint32_t localid, const char *stdCode, bool isBuy,
...
void HftStraBaseCtx::on_entrust(uint32_t localid, const char *stdCode,
...
void HftStraBaseCtx::on_position(const char *stdCode, bool isLong,
...
double HftStraBaseCtx::stra_get_undone(const char *stdCode) {
  ...
  //	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode);
  //	std::string code = _engine->get_hot_mgr()->getRawCode(cInfo._exchg,
  //cInfo._product, _engine->get_trading_date()); 	std::string realCode =
  //CodeHelper::rawMonthCodeToStdCode(code.c_str(), cInfo._exchg);
  ...
  //	std::string code =
  //_engine->get_hot_mgr()->getSecondRawCode(cInfo._exchg, cInfo._product,
  //_engine->get_trading_date()); 	std::string realCode =
  //CodeHelper::rawMonthCodeToStdCode(code.c_str(), cInfo._exchg);
...
uint32_t HftStraBaseCtx::stra_get_date() { return _engine->get_date(); }

uint32_t HftStraBaseCtx::stra_get_time() { return _engine->get_raw_time(); }

uint32_t HftStraBaseCtx::stra_get_secs() { return _engine->get_secs(); }

const char *HftStraBaseCtx::stra_load_user_data(const char *key,
                                                const char *defVal /*= ""*/) {
...
void HftStraBaseCtx::do_set_position(const char *stdCode, double qty,
                                     double price /* = 0.0 */,
                                     const char *userTag /*= ""*/) {
  ...
  uint64_t curTm = (uint64_t)_engine->get_date() * 1000000000 +
...
void HftStraBaseCtx::update_dyn_profit(const char *stdCode,
                                       WTSTickData *newTick) {
  auto it = _pos_map.find(stdCode);
  if (it != _pos_map.end()) {
...
void HftStraBaseCtx::log_trade(const char *stdCode, bool isLong, bool isOpen,
...
void HftStraBaseCtx::log_close(const char *stdCode, bool isLong,
                               uint64_t openTime, double openpx,
                               uint64_t closeTime, double closepx, double qty,
                               double profit, double maxprofit, double maxloss,
