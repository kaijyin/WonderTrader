...
WtEngine::WtEngine()
    : _port_fund(NULL), _risk_volscale(1.0), _risk_date(0), _terminated(false),
      _evt_listener(NULL), _adapter_mgr(NULL), _notifier(NULL),
      _fund_udt_span(0), _ready(false) {
...
void WtEngine::set_date_time(uint32_t curDate, uint32_t curTime,
                             uint32_t curSecs /* = 0 */,
                             uint32_t rawTime /* = 0 */) {
  _cur_date = curDate;
  _cur_time = curTime;
...
WTSSessionInfo *WtEngine::get_session_info(const char *sid,
                                           bool isCode /* = false */) {
  if (!isCode)
...
void WtEngine::on_tick(const char *stdCode, WTSTickData *curTick) {
  ...
  push_task([this]() { update_fund_dynprofit(); });
}
...
void WtEngine::update_fund_dynprofit() {
  ...
  if (fundInfo._max_md_dyn_bal._date == 0 ||
      decimal::gt(dynbalance, fundInfo._max_md_dyn_bal._dyn_balance)) {
    fundInfo._max_md_dyn_bal._dyn_balance = dynbalance;
    fundInfo._max_md_dyn_bal._date = _cur_tdate;
  }

  if (fundInfo._min_md_dyn_bal._date == 0 ||
      decimal::lt(dynbalance, fundInfo._min_md_dyn_bal._dyn_balance)) {
...
uint32_t WtEngine::getCurDate() { return _cur_date; }

uint32_t WtEngine::getCurTime() { return _cur_time; }

uint32_t WtEngine::getTradingDate() { return _cur_tdate; }

bool WtEngine::isInTrading() { return false; }
...
void WtEngine::setVolScale(double scale) {
  ...
  WTSLogger::log_by_cat("risk", LL_INFO,
                        "Position risk scale updated: {} - > {}", oldScale,
                        scale);
  save_datas();
}
...
void WtEngine::init(WTSVariant *cfg, IBaseDataMgr *bdMgr, WtDtMgr *dataMgr,
  ...
  if (cfgRisk) {
    ...
    WTSLogger::log_raw(
        LL_WARN,
        "RiskMon is not configured, portfilio fund will be updated every 5s");
  }
}
...
void WtEngine::on_session_end() {
  ...
  if (fundInfo._last_date < _cur_tdate) {
    ...
    BoostFilePtr fund_log(new BoostFile());
    {
      ...
      if (isNewFile) {
        fund_log->write_file(
            "date,predynbalance,prebalance,balance,closeprofit,dynprofit,fee,"
            "maxdynbalance,maxtime,mindynbalance,mintime,mdmaxbalance,"
            "mdmaxdate,mdminbalance,mdmindate\n");
      } else {
    ...
    fund_log->write_file(fmt::format(
        "{},{},{},{},{},{},{},{},{},{},{},{},{},{},{}\n", _cur_tdate,
        fundInfo._predynbal, fundInfo._prebalance, fundInfo._balance,
        fundInfo._profit, fundInfo._dynprofit, fundInfo._fees,
        fundInfo._max_dyn_bal, fundInfo._max_time, fundInfo._min_dyn_bal,
        fundInfo._min_time, fundInfo._max_md_dyn_bal._dyn_balance,
        fundInfo._max_md_dyn_bal._date, fundInfo._min_md_dyn_bal._dyn_balance,
        fundInfo._min_md_dyn_bal._date));
...
void WtEngine::save_datas() {
  ...
  if (_port_fund != NULL) { // 保存资金数据
    ...
    jMaxMD.AddMember("dyn_balance", fundInfo._max_md_dyn_bal._dyn_balance,
                     allocator);

    rj::Value jMinMD(rj::kObjectType);
    jMinMD.AddMember("date", fundInfo._min_md_dyn_bal._date, allocator);
    jMinMD.AddMember("dyn_balance", fundInfo._min_md_dyn_bal._dyn_balance,
                     allocator);
  ...
  { // 读取仓位
    ...
    if (!jPos.IsNull() && jPos.IsArray()) {
      for (const rj::Value &pItem : jPos.GetArray()) {
        ...
        WTSLogger::debug("Porfolio position confirmed,{} -> {}", stdCode,
                         pInfo->_volume);
      }
    }
...
WTSTickSlice *WtEngine::get_tick_slice(uint32_t sid, const char *code,
                                       uint32_t count) {
...
WTSKlineSlice *WtEngine::get_kline_slice(uint32_t sid, const char *stdCode,
...
double WtEngine::get_cur_price(const char *stdCode) {
  ...
  std::string sCode =
      (lastChar == SUFFIX_QFQ) ? std::string(stdCode, len - 1) : stdCode;
...
double WtEngine::get_exright_factor(const char *stdCode,
  ...
  if (commInfo->isStock())
    ...
    if (strlen(ruleTag) > 0)
      return _hot_mgr->getRuleFactor(ruleTag, commInfo->getFullPid(),
                                     get_trading_date());
  }
...
void WtEngine::sub_tick(uint32_t sid, const char *stdCode) {
  ...
  if (strlen(ruleTag) > 0) {
    ...
    uint32_t flag = 0;
    if (stdCode[length - 1] == SUFFIX_QFQ ||
        stdCode[length - 1] == SUFFIX_HFQ) {
      length--;
    ...
    CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
    std::string rawCode =
        _hot_mgr->getCustomRawCode(ruleTag, cInfo.stdCommID(), _cur_tdate);
    std::string stdRawCode =
        CodeHelper::rawMonthCodeToStdCode(rawCode.c_str(), cInfo._exchg);
  }
  ...
  //	std::string rawCode = _hot_mgr->getRawCode(cInfo._exchg, cInfo._product,
  //_cur_tdate); 	std::string stdRawCode =
  //CodeHelper::rawMonthCodeToStdCode(rawCode.c_str(), cInfo._exchg);
  ...
  //	CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode);
  //	std::string rawCode = _hot_mgr->getSecondRawCode(cInfo._exchg,
  //cInfo._product, _cur_tdate); 	std::string stdRawCode =
  //CodeHelper::rawMonthCodeToStdCode(rawCode.c_str(), cInfo._exchg);
  //	//_ticksubed_raw_codes.insert(stdRawCode);
  //}
  else {
...
void WtEngine::load_fees(const char *filename) {
  ...
  for (const std::string &fullPid : keys) {
    ...
    WTSCommodityInfo *commInfo =
        _base_data_mgr->getCommodity(ay[0].c_str(), ay[1].c_str());
    if (commInfo == NULL)
      continue;

    commInfo->setFeeRates(
        cfgItem->getDouble("open"), cfgItem->getDouble("close"),
        cfgItem->getDouble("closetoday"), cfgItem->getBoolean("byvolume"));
...
double WtEngine::calc_fee(const char *stdCode, double price, double qty,
                          uint32_t offset) {
  CodeHelper::CodeInfo codeInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  const char *stdPID = codeInfo.stdCommID();
  auto it = _fee_map.find(stdPID);
  if (it == _fee_map.end()) {
    WTSLogger::warn("Fee template of {} not found, return 0.0 as default",
                    stdPID);
    return 0.0;
  }
...
void WtEngine::append_signal(const char *stdCode, double qty,
  ...
  if (bStandBy || decimal::eq(curPx, 0.0)) {
    ...
    sInfo._volume = qty;
    sInfo._gentime = (uint64_t)_cur_date * 1000000000 +
                     (uint64_t)_cur_raw_time * 100000 + _cur_secs;
  } else {
...
void WtEngine::do_set_position(const char *stdCode, double qty,
                  ...
                  0)) // 当前持仓和目标仓位方向一致, 增加一条明细, 增加数量即可
  {
    ...
    for (auto it = pInfo->_details.begin(); it != pInfo->_details.end(); it++) {
      ...
      double fee = commInfo->calcFee(curPx, maxQty,
      ...
      log_close(stdCode, dInfo._long, dInfo._opentime, dInfo._price, curTm,
                curPx, maxQty, profit, pInfo->_closeprofit);
...
void WtEngine::push_task(TaskItem task) {
  ...
  if (_thrd_task == NULL) {
    _thrd_task.reset(new StdThread([this] { task_loop(); }));
  }
...
bool WtEngine::init_riskmon(WTSVariant *cfg) {
  ...
  if (hInst == NULL) {
    WTSLogger::log_by_cat("risk", LL_ERROR, "Riskmon module {} loading failed",
                          dllpath.c_str());
    return false;
  }

  FuncCreateRiskMonFact creator =
      (FuncCreateRiskMonFact)DLLHelper::get_symbol(hInst, "createRiskMonFact");
  if (creator == NULL) {
    DLLHelper::free_library(hInst);
    WTSLogger::log_by_cat("risk", LL_ERROR,
                          "Riskmon module {} is not compatible",
                          module.c_str());
    return false;
  }
  ...
  _risk_fact._remover =
      (FuncDeleteRiskMonFact)DLLHelper::get_symbol(hInst, "deleteRiskMonFact");
  _risk_fact._fact = _risk_fact._creator();

  const char *name = cfg->getCString("name");

  _risk_mon.reset(new WtRiskMonWrapper(
      _risk_fact._fact->createRiskMonotor(name), _risk_fact._fact));
...
void WtEngine::init_outputs() {
  ...
  _close_logs.reset(new BoostFile());
  {
    ...
    if (isNewFile) {
      _close_logs->write_file("code,direct,opentime,openprice,closetime,"
                              "closeprice,qty,profit,totalprofit\n");
    } else {
...
void WtEngine::log_trade(const char *stdCode, bool isLong, bool isOpen,
...
void WtEngine::log_close(const char *stdCode, bool isLong, uint64_t openTime,
                         double openpx, uint64_t closeTime, double closepx,
                         double qty, double profit,
