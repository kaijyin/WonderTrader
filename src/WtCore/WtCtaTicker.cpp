...
void WtCtaRtTicker::init(IDataReader *store, const char *sessionID) {
  ...
  if (_s_info == NULL)
    WTSLogger::fatal("Session {} is invalid, CtaTicker cannot run correctly",
                     sessionID);
  else
    ...
    if (_last_emit_pos < _cur_pos) {
      ...
      if (_store)
        _store->onMinuteEnd(_date, thisMin,
                            bEndingTDate ? _engine->getTradingDate() : 0);
...
void WtCtaRtTicker::run() {
   ...
   */
  uint32_t curTDate = _engine->get_basedata_mgr()->calcTradingDate(
      _s_info->id(), _engine->get_date(), _engine->get_min_time(), true);
  ...
  _thrd.reset(new StdThread([this]() {
    while (!_stopped) {
      ...
      if (_time != UINT_MAX && _s_info->isInTradingTime(_time / 100000, true)) {
        ...
        if (now >= _next_check_time && _last_emit_pos < _cur_pos) {
          ...
          if (thisMin == 0) {
            ...
            _time = 0;
            WTSLogger::info(
                "Data automatically changed at time 00:00: {} -> {}", lastDate,
                _date);
          }
          ...
          WTSLogger::info("Minute bar {}.{:04d} closed automatically", _date,
                          thisMin);
          if (_store)
            _store->onMinuteEnd(_date, thisMin,
                                bEndingTDate ? _engine->getTradingDate() : 0);
      ...
      } else // if(offTime >= _s_info->getOpenTime(true) && offTime <=
             // _s_info->getCloseTime(true))
      {
        // 收盘以后，如果发现上次触发的位置不等于总的分钟数，说明少了最后一分钟的闭合逻辑
        uint32_t total_mins = _s_info->getTradingMins();
        if (_time != UINT_MAX && _last_emit_pos != 0 &&
            _last_emit_pos < total_mins &&
            offTime >= _s_info->getCloseTime(true)) {
          WTSLogger::warn("Tradingday {} will be ended forcely, last_emit_pos: "
                          "{}, time: {}",
                          _engine->getTradingDate(),
                          _last_emit_pos.fetch_add(0), _time);
