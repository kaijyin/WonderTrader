...
bool CtaStrategyMgr::loadFactories(const char *path) {
  ...
  for (boost::filesystem::directory_iterator iter(myPath); iter != endIter;
    ...
    if (fact != NULL) {
      ...
      fInfo._remover = (FuncDeleteStraFact)DLLHelper::get_symbol(
          hInst, "deleteStrategyFact");
      fInfo._fact = fact;
  ...
  WTSLogger::info("{} CTA strategy factories in directory[{}] loaded", count,
                  path);

  return true;
}

CtaStrategyPtr CtaStrategyMgr::createStrategy(const char *factname,
                                              const char *unitname,
                                              const char *id) {
  ...
  CtaStrategyPtr ret(new CtaStraWrapper(
      fInfo._fact->createStrategy(unitname, id), fInfo._fact));
  _strategies[id] = ret;
  return ret;
}

CtaStrategyPtr CtaStrategyMgr::createStrategy(const char *name,
                                              const char *id) {
