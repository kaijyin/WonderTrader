...
SelStraBaseCtx::SelStraBaseCtx(WtSelEngine *engine, const char *name,
                               int32_t slippage)
    : ISelStraCtx(name), _engine(engine), _total_calc_time(0), _emit_times(0),
      _is_in_schedule(false), _ud_modified(false), _schedule_date(0),
      _schedule_time(0), _slippage(slippage) {
...
void SelStraBaseCtx::log_signal(const char *stdCode, double target,
...
void SelStraBaseCtx::log_trade(const char *stdCode, bool isLong, bool isOpen,
...
void SelStraBaseCtx::log_close(const char *stdCode, bool isLong,
...
//////////////////////////////////////////////////////////////////////////
// 回调函数
void SelStraBaseCtx::on_bar(const char *stdCode, const char *period,
...
void SelStraBaseCtx::on_tick(const char *stdCode, WTSTickData *newTick,
...
bool SelStraBaseCtx::on_schedule(uint32_t curDate, uint32_t curTime,
                                 uint32_t fireTime) {
  ...
  if (_emit_times % 20 == 0)
    log_info("Strategy has been scheduled {} times, totally taking {} us, "
             "{:.3f} us each time",
...
void SelStraBaseCtx::on_session_begin(uint32_t uTDate) {
  ...
  for (auto &it : _pos_map) {
    ...
    if (pInfo._frozen_date != 0 && pInfo._frozen_date < uTDate &&
      ...
      log_debug("{} of {} frozen on {} released on {}", pInfo._frozen, stdCode,
...
void SelStraBaseCtx::stra_set_position(const char *stdCode, double qty,
                                       const char *userTag /* = "" */) {
  WTSCommodityInfo *commInfo = _engine->get_commodity_info(stdCode);
  if (commInfo == NULL) {
  ...
  if (commInfo->isT1()) {
    ...
    if (decimal::lt(qty, frozen)) {
      log_error("New position of {} cannot be set to {} due to {} being frozen",
                stdCode, qty, frozen);
      return;
    }
  }
...
void SelStraBaseCtx::append_signal(const char *stdCode, double qty,
                                   const char *userTag /* = "" */) {
...
void SelStraBaseCtx::do_set_position(const char *stdCode, double qty,
                                     const char *userTag /* = "" */,
...
WTSKlineSlice *SelStraBaseCtx::stra_get_bars(const char *stdCode,
                                             const char *period,
                                             uint32_t count) {
  thread_local static char key[64] = {0};
  ...
  WTSKlineSlice *kline = _engine->get_kline_slice(
      _context_id, stdCode, basePeriod, count, times, etime);
...
WTSTickSlice *SelStraBaseCtx::stra_get_ticks(const char *stdCode,
                                             uint32_t count) {
...
double SelStraBaseCtx::stra_get_day_price(const char *stdCode,
                                          int flag /* = 0 */) {
  if (_engine)
...
const char *SelStraBaseCtx::stra_load_user_data(const char *key,
                                                const char *defVal /*= ""*/) {
...
double SelStraBaseCtx::stra_get_position(const char *stdCode,
...
uint64_t SelStraBaseCtx::stra_get_detail_entertime(const char *stdCode,
                                                   const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double SelStraBaseCtx::stra_get_detail_cost(const char *stdCode,
                                            const char *userTag) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
    return 0;
...
double SelStraBaseCtx::stra_get_detail_profit(const char *stdCode,
                                              const char *userTag,
                                              int flag /* = 0 */) {
  auto it = _pos_map.find(stdCode);
  if (it == _pos_map.end())
