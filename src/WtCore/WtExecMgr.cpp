...
void WtExecuterMgr::set_positions(wt_hashmap<std::string, double> target_pos) {
    ...
    for (auto &m : target_pos) {
      ...
      if (!isFltd) {
        if (!decimal::eq(desVol, oldVol)) {
          // 输出日志
          WTSLogger::info(
              "[Filters] {} target position reset by code filter: {} -> {}",
              stdCode.c_str(), oldVol, desVol);
        }

        des_port[stdCode] = desVol;
      } else {
        // 输出日志
        WTSLogger::info("[Filters] {} target position ignored by filter",
                        stdCode.c_str());
      }
    }
  ...
  for (auto &v : _executers) {
    ...
    if (_filter_mgr && _filter_mgr->is_filtered_by_executer(executer->name())) {
      WTSLogger::info(
          "[Filters] Executer {} is filtered, all signals will be ignored",
          executer->name());
      continue;
    }
...
void WtExecuterMgr::handle_pos_change(const char *stdCode, double targetPos,
  ...
  if (_filter_mgr != NULL) {
    ...
    if (!isFltd) {
      if (!decimal::eq(targetPos, oldVol)) {
        // 输出日志
        WTSLogger::info(
            "[Filters] {} target position reset by filter: {} -> {}", stdCode,
            oldVol, targetPos);
        // 差量也要重算
  ...
  for (auto &v : _executers) {
    ...
    if (_filter_mgr && _filter_mgr->is_filtered_by_executer(executer->name())) {
      WTSLogger::info(
          "[Filters] All signals to executer {} are ignored by executer filter",
          executer->name());
      continue;
    }
...
void WtExecuterMgr::add_target_to_cache(const char *stdCode, double targetPos,
                                        const char *execid /* = "ALL" */) {
  ...
  for (uint32_t i = 0; i < config->size(); i++) {
    ...
    if (itemExec->isArray()) {
      ...
      for (uint32_t k = 0; k < cnt; k++) {
        ...
        WTSLogger::info("Signal of strategy {} will be routed to executer {}",
                        straName, execId);
