...
void HftStraContext::on_order_queue(const char *stdCode,
...
void HftStraContext::on_order_detail(const char *stdCode,
...
void HftStraContext::on_transaction(const char *stdCode,
...
void HftStraContext::on_bar(const char *code, const char *period,
...
void HftStraContext::on_trade(uint32_t localid, const char *stdCode, bool isBuy,
                              double vol, double price) {
  const char *innerCode = get_inner_code(stdCode);
  if (_strategy)
    _strategy->on_trade(this, localid, innerCode, isBuy, vol, price,
                        getOrderTag(localid));

  HftStraBaseCtx::on_trade(localid, innerCode, isBuy, vol, price);
}

void HftStraContext::on_order(uint32_t localid, const char *stdCode, bool isBuy,
                              double totalQty, double leftQty, double price,
                              bool isCanceled /* = false */) {
  const char *innerCode = get_inner_code(stdCode);
  if (_strategy)
    _strategy->on_order(this, localid, innerCode, isBuy, totalQty, leftQty,
                        price, isCanceled, getOrderTag(localid));

  HftStraBaseCtx::on_order(localid, innerCode, isBuy, totalQty, leftQty, price,
                           isCanceled);
}

void HftStraContext::on_position(const char *stdCode, bool isLong,
...
void HftStraContext::on_entrust(uint32_t localid, const char *stdCode,
                                bool bSuccess, const char *message) {
  if (_strategy)
    _strategy->on_entrust(localid, bSuccess, message, getOrderTag(localid));

  HftStraBaseCtx::on_entrust(localid, get_inner_code(stdCode), bSuccess,
                             message);
}