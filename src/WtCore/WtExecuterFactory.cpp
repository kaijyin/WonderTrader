...
ExecuteUnitPtr WtExecuterFactory::createExeUnit(const char *factname,
                                                const char *unitname) {
  ...
  if (unit == NULL) {
    WTSLogger::error("Createing execution unit failed: {}.{}", factname,
                     unitname);
    return ExecuteUnitPtr();
  }
...
ExecuteUnitPtr WtExecuterFactory::createDiffExeUnit(const char *factname,
                                                    const char *unitname) {
  auto it = _factories.find(factname);
  if (it == _factories.end())
    return ExecuteUnitPtr();
  ...
  if (unit == NULL) {
    WTSLogger::error("Createing diff execution unit failed: {}.{}", factname,
                     unitname);
    return ExecuteUnitPtr();
  }
  return ExecuteUnitPtr(new ExeUnitWrapper(unit, fInfo._fact));
...
ExecuteUnitPtr WtExecuterFactory::createArbiExeUnit(const char *factname,
                                                    const char *unitname) {
  auto it = _factories.find(factname);
  if (it == _factories.end())
    return ExecuteUnitPtr();
  ...
  if (unit == NULL) {
    WTSLogger::error("Createing arbi execution unit failed: {}.{}", factname,
                     unitname);
    return ExecuteUnitPtr();
  }
  return ExecuteUnitPtr(new ExeUnitWrapper(unit, fInfo._fact));
