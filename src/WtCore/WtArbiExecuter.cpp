...
WtArbiExecuter::WtArbiExecuter(WtExecuterFactory *factory, const char *name,
                               IDataManager *dataMgr)
    : IExecCommand(name), _factory(factory), _data_mgr(dataMgr),
      _channel_ready(false), _scale(1.0), _auto_clear(true), _trader(NULL) {}
...
bool WtArbiExecuter::init(WTSVariant *params) {
  ...
  WTSLogger::log_dyn("executer", _name.c_str(), LL_INFO,
                     "Local executer inited, scale: {}, auto_clear: {}, "
                     "strict_sync: {}, thread poolsize: {}, code_groups: {}",
                     _scale, _auto_clear, _strict_sync, poolsize,
                     _groups.size());

  return true;
}

ExecuteUnitPtr WtArbiExecuter::getUnit(const char *stdCode,
...
WTSTickSlice *WtArbiExecuter::getTicks(const char *stdCode, uint32_t count,
                                       uint64_t etime /* = 0 */) {
  if (_data_mgr == NULL)
    return NULL;
...
double WtArbiExecuter::getPosition(const char *stdCode,
                                   bool validOnly /* = true */,
                                   int32_t flag /* = 3 */) {
  if (NULL == _trader)
    return 0.0;
...
OrderIDs WtArbiExecuter::buy(const char *stdCode, double price, double qty,
...
OrderIDs WtArbiExecuter::sell(const char *stdCode, double price, double qty,
                              bool bForceClose /* = false*/) {
  if (!_channel_ready)
    return OrderIDs();
...
uint64_t WtArbiExecuter::getCurTime() {
  ...
  // return TimeUtils::makeTime(_stub->get_date(), _stub->get_raw_time() *
  // 100000 + _stub->get_secs());
}
...
void WtArbiExecuter::on_position_changed(const char *stdCode, double diffPos) {
  ...
  if (!decimal::eq(diffPos, 0)) {
    WTSLogger::log_dyn(
        "executer", _name.c_str(), LL_INFO,
        "Target position of {} changed: {} -> {} : {} with scale:{}", stdCode,
        oldVol, newVol, traderTarget, _scale);
  }

  if (_trader && !_trader->checkOrderLimits(stdCode)) {
    WTSLogger::log_dyn("executer", _name.c_str(), LL_INFO, "{} is disabled",
                       stdCode);
    return;
  }
...
void WtArbiExecuter::set_position(
    const wt_hashmap<std::string, double> &targets) {
  /*
   *	先要把目标头寸进行组合匹配
   */
  ...
  for (auto it = targets.begin(); it != targets.end(); it++) {
    ...
    if (_trader && !_trader->checkOrderLimits(stdCode)) {
      WTSLogger::log_dyn("executer", _name.c_str(), LL_WARN,
                         "{} is disabled due to entrust limit control ",
                         stdCode);
      continue;
    }
  ...
  for (auto it = _target_pos.begin(); it != _target_pos.end(); it++) {
    ...
    WTSLogger::log_dyn("executer", _name.c_str(), LL_INFO,
                       "{} is not in target, set to 0 automatically", code);
    ...
    if (_pool) {
      _pool->schedule([unit, code]() { unit->self()->set_position(code, 0); });
    } else {
      unit->self()->set_position(code, 0);
    }
  ...
  if (_strict_sync) {
    for (const std::string &stdCode : _channel_holds) {
      ...
      WTSLogger::log_dyn(
          "executer", _name.c_str(), LL_INFO,
          "{} is not in management, set to 0 due to strict sync mode",
          stdCode.c_str());
      ...
      if (_pool) {
        ...
        _pool->schedule(
            [unit, code]() { unit->self()->set_position(code.c_str(), 0); });
      } else {
...
void WtArbiExecuter::on_trade(uint32_t localid, const char *stdCode, bool isBuy,
...
void WtArbiExecuter::on_order(uint32_t localid, const char *stdCode, bool isBuy,
  ...
  // unit->self()->on_order(localid, stdCode, isBuy, leftQty, price,
  // isCanceled);
  if (_pool) {
    std::string code = stdCode;
    _pool->schedule([localid, unit, code, isBuy, leftQty, price, isCanceled]() {
      unit->self()->on_order(localid, code.c_str(), isBuy, leftQty, price,
                             isCanceled);
    });
  } else {
...
void WtArbiExecuter::on_entrust(uint32_t localid, const char *stdCode,
...
void WtArbiExecuter::on_channel_ready() {
  ...
  for (auto it = _unit_map.begin(); it != _unit_map.end(); it++) {
    ...
    if (unitPtr) {
      ...
      if (_pool) {
        _pool->schedule([unitPtr]() { unitPtr->self()->on_channel_ready(); });
      } else {
        unitPtr->self()->on_channel_ready();
      }
    }
...
void WtArbiExecuter::on_channel_lost() {
  ...
  for (auto it = _unit_map.begin(); it != _unit_map.end(); it++) {
    ...
    if (unitPtr) {
      if (_pool) {
        _pool->schedule([unitPtr]() { unitPtr->self()->on_channel_lost(); });
      } else {
...
void WtArbiExecuter::on_account(const char *currency, double prebalance,
                                double balance, double dynbalance,
                                double avaliable, double closeprofit,
                                double dynprofit, double margin, double fee,
  ...
  for (auto it = _unit_map.begin(); it != _unit_map.end(); it++) {
    ...
    if (unitPtr) {
      if (_pool) {
        ...
        _pool->schedule([unitPtr, strCur, prebalance, balance, dynbalance,
                         avaliable, closeprofit, dynprofit, margin, fee,
                         deposit, withdraw]() {
          unitPtr->self()->on_account(
              strCur.c_str(), prebalance, balance, dynbalance, avaliable,
              closeprofit, dynprofit, margin, fee, deposit, withdraw);
        });
      } else {
        unitPtr->self()->on_account(currency, prebalance, balance, dynbalance,
                                    avaliable, closeprofit, dynprofit, margin,
...
void WtArbiExecuter::on_position(const char *stdCode, bool isLong,
  ...
  // 获取上一期的主力合约
  std::string prevCode =
      hotMgr->getPrevRawCode(cInfo._exchg, cInfo._product, tradingday);
  ...
  WTSLogger::log_dyn("executer", _name.c_str(), LL_INFO,
                     "Prev hot contract of {}.{} on {} is {}", cInfo._exchg,
                     cInfo._product, tradingday, prevCode);
  ...
  if (it != _clear_excludes.end()) {
    WTSLogger::log_dyn("executer", _name.c_str(), LL_INFO,
                       "Position of {}, as prev hot contract, won't be cleared "
                       "for it's in exclude list",
                       stdCode);
    return;
  }
  ...
  if (!_clear_includes.empty()) {
    ...
    if (it == _clear_includes.end()) {
      WTSLogger::log_dyn("executer", _name.c_str(), LL_INFO,
                         "Position of {}, as prev hot contract, won't be "
                         "cleared for it's not in include list",
                         stdCode);
      return;
    }
  }
  ...
  // 最后再进行自动清理
  WTSLogger::log_dyn("executer", _name.c_str(), LL_INFO,
                     "Position of {}, as prev hot contract, will be cleared",
                     stdCode);
  ExecuteUnitPtr unit = getUnit(stdCode);
  if (unit) {
    if (_pool) {
      std::string code = stdCode;
      _pool->schedule(
          [unit, code]() { unit->self()->clear_all_position(code.c_str()); });
    } else {
