...
public:
  ...
  /*
   *	是否因为策略被过滤掉了
   *	如果过滤器是忽略的话, 就会返回true, 如果是重定向仓位, 就会返回false,
   *而目标仓位是一个被过滤器改过的值
   *
   *	@sname		策略名称
   ...
   *	@isDiff		是否是增量
   *
   *	return		是否过滤掉了, 如果过滤掉了,
   *该持仓就不加入最终组合目标仓位
   */
  bool is_filtered_by_strategy(const char *straName, double &targetPos,
                               bool isDiff = false);
...
private:
  ...
  FilterMap _code_filters; // 代码过滤器, 包括合约代码和品种代码,
                           // 同一时间只有一个生效, 合约代码优先级高于品种代码
