...
public:
  WtRiskMonWrapper(WtRiskMonitor *mon, IRiskMonitorFact *fact)
      : _mon(mon), _fact(fact) {}
  ~WtRiskMonWrapper() {
    if (_mon) {
...
public:
  ...
  void set_date_time(uint32_t curDate, uint32_t curTime, uint32_t curSecs = 0,
                     uint32_t rawTime = 0);
  ...
  WTSTickSlice *get_tick_slice(uint32_t sid, const char *stdCode,
                               uint32_t count);
  WTSKlineSlice *get_kline_slice(uint32_t sid, const char *stdCode,
                                 const char *period, uint32_t count,
                                 uint32_t times = 1, uint64_t etime = 0);
   ...
   *	@commInfo	品种信息
   */
  double get_exright_factor(const char *stdCode,
  ...
  inline void setRiskMonitor(WtRiskMonPtr &monitor) { _risk_mon = monitor; }
...
public:
  virtual void init(WTSVariant *cfg, IBaseDataMgr *bdMgr, WtDtMgr *dataMgr,
...
protected:
  ...
  uint32_t
      _cur_time; // 当前时间, 是1分钟线时间, 比如0900, 这个时候的1分钟线是0901,
                 // _cur_time也就是0901, 这个是为了CTA里面方便
  ...
  // By Wesley @ 2022.02.07
  // 这个好像没有用到，不需要了
  // wt_hashset<std::string>		_ticksubed_raw_codes;
  // //tick订阅表（真实代码模式）
