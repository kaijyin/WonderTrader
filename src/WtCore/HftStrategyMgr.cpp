...
bool HftStrategyMgr::loadFactories(const char *path) {
  ...
  for (boost::filesystem::directory_iterator iter(myPath); iter != endIter;
    ...
    FuncCreateHftStraFact creator =
        (FuncCreateHftStraFact)DLLHelper::get_symbol(hInst,
                                                     "createStrategyFact");
    if (creator == NULL) {
    ...
    if (pFact != NULL) {
      ...
      fInfo._remover = (FuncDeleteHftStraFact)DLLHelper::get_symbol(
  ...
  WTSLogger::info("{} HFT strategy factories in directory[{}] loaded", count,
                  path);

  return true;
}

HftStrategyPtr HftStrategyMgr::createStrategy(const char *factname,
                                              const char *unitname,
                                              const char *id) {
  ...
  HftStrategyPtr ret(new HftStraWrapper(
      fInfo._fact->createStrategy(unitname, id), fInfo._fact));
  _strategies[id] = ret;
  return ret;
}

HftStrategyPtr HftStrategyMgr::createStrategy(const char *name,
                                              const char *id) {
