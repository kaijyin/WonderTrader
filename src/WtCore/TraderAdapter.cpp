...
uint32_t makeLocalOrderID() {
  ...
  if (_auto_order_id == 0) {
    ...
    _auto_order_id = (uint32_t)((TimeUtils::getLocalTimeNow() -
                                 TimeUtils::makeTime(curYear, 0)) /
                                1000 * 50);
  }
...
TraderAdapter::TraderAdapter(EventNotifier *caster /* = NULL */)
    : _id(""), _cfg(NULL), _state(AS_NOTLOGIN), _trader_api(NULL),
      _orders(NULL), _stat_map(NULL), _risk_mon_enabled(false),
      _save_data(false), _notifier(caster), _ignore_sefmatch(false) {}
...
bool TraderAdapter::init(const char *id, WTSVariant *params,
                         IBaseDataMgr *bdMgr, ActionPolicyMgr *policyMgr) {
  if (params == NULL)
    return false;
  ...
  if (cfgRisk) {
    if (cfgRisk->getBoolean("active")) {
      ...
      for (auto it = keys.begin(); it != keys.end(); it++) {
        ...
        rParam._cancel_total_limits =
            vProdItem->getUInt32("cancel_total_limits");
        rParam._cancel_times_boundary =
            vProdItem->getUInt32("cancel_times_boundary");
        rParam._cancel_stat_timespan =
            vProdItem->getUInt32("cancel_stat_timespan");

        rParam._order_total_limits = vProdItem->getUInt32("order_total_limits");
        rParam._order_times_boundary =
            vProdItem->getUInt32("order_times_boundary");
        rParam._order_stat_timespan =
            vProdItem->getUInt32("order_stat_timespan");

        WTSLogger::log_dyn(
            "trader", _id.c_str(), LL_INFO,
            "[{}] Risk control rule {} of trading channel loaded", _id.c_str(),
            product);
      }
      ...
      if (it == _risk_params_map.end()) {
        WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
                           "[{}] Some instruments may not be monitored due to "
                           "no default risk control rule of trading channel",
                           _id.c_str());
      }
    } else {
      WTSLogger::log_dyn(
          "trader", _id.c_str(), LL_WARN,
          "[{}] Risk control rule of trading channel not activated",
          _id.c_str());
    }
  } else {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
                       "[{}] No risk control rule setup of trading channel",
                       _id.c_str());
  }
  ...
  std::string module =
      DLLHelper::wrap_module(params->getCString("module"), "lib");
  ;

  // 先看工作目录下是否有交易模块
  std::string dllpath =
      WtHelper::getModulePath(module.c_str(), "traders", true);
  ...
  if (hInst == NULL) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Loading trading module {} failed", _id.c_str(),
                       dllpath.c_str());
    return false;
  } else {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                       "[{}] Trader module {} loaded", _id.c_str(),
                       dllpath.c_str());
  }

  FuncCreateTrader pFunCreateTrader =
      (FuncCreateTrader)DLLHelper::get_symbol(hInst, "createTrader");
  if (NULL == pFunCreateTrader) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_FATAL,
                       "[{}] Entrance function createTrader not found",
                       _id.c_str());
    return false;
  }
  ...
  if (NULL == _trader_api) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_FATAL,
                       "[{}] Creating trading api failed", _id.c_str());
    return false;
  }
  ...
  if (!_trader_api->init(params)) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Entrance function deleteTrader not found", id);
    return false;
  }
...
bool TraderAdapter::initExt(const char *id, ITraderApi *api,
                            IBaseDataMgr *bdMgr, ActionPolicyMgr *policyMgr) {
  if (api == NULL)
    return false;
...
void TraderAdapter::initSaveData() {
  ...
  _trades_log.reset(new BoostFile());
  {
    ...
    if (isNewFile) {
      _trades_log->write_file(
          "localid,date,time,code,action,volume,price,tradeid,orderid\n");
    } else {
  ...
  _orders_log.reset(new BoostFile());
  {
    ...
    if (isNewFile) {
      _orders_log->write_file("localid,date,inserttime,code,action,volume,"
                              "traded,price,orderid,canceled,remark\n");
    } else {
...
void TraderAdapter::logTrade(uint32_t localid, const char *stdCode,
                             WTSTradeInfo *trdInfo) {
  if (_trades_log == NULL || trdInfo == NULL)
    return;

  _trades_log->write_file(fmt::format(
      "{},{},{},{},{},{},{},{},{}\n", localid, trdInfo->getTradeDate(),
      trdInfo->getTradeTime(), stdCode,
      formatAction(trdInfo->getDirection(), trdInfo->getOffsetType()),
      trdInfo->getVolume(), trdInfo->getPrice(), trdInfo->getTradeID(),
      trdInfo->getRefOrder()));
}

void TraderAdapter::logOrder(uint32_t localid, const char *stdCode,
                             WTSOrderInfo *ordInfo) {
  if (_orders_log == NULL || ordInfo == NULL)
    return;

  _orders_log->write_file(fmt::format(
      "{},{},{},{},{},{},{},{},{},{},{}\n", localid, ordInfo->getOrderDate(),
      ordInfo->getOrderTime(), stdCode,
  ...
  { // 资金保存
    ...
    if (ayFunds && ayFunds->size() > 0) {
      for (auto it = ayFunds->begin(); it != ayFunds->end(); it++) {
        ...
        jFunds.AddMember(rj::Value(fundInfo->getCurrency(), allocator), fItem,
                         allocator);
      }
    }
...
const TraderAdapter::RiskParams *
TraderAdapter::getRiskParams(const char *stdCode) {
...
double TraderAdapter::getPosition(const char *stdCode, bool bValidOnly,
                                  int32_t flag /* = 3 */) {
...
void TraderAdapter::enumPosition(FuncEnumChnlPosCallBack cb) {
  for (auto &v : _positions) {
    ...
    if (decimal::gt(pItem.l_prevol + pItem.l_newvol, 0))
      cb(stdCode, true, pItem.l_prevol, pItem.l_preavail, pItem.l_newvol,
         pItem.l_newavail);
    if (decimal::gt(pItem.s_prevol + pItem.s_newvol, 0))
      cb(stdCode, false, pItem.s_prevol, pItem.s_preavail, pItem.s_newvol,
         pItem.s_newavail);
  }
}
...
uint32_t TraderAdapter::doEntrust(WTSEntrust *entrust) {
  ...
  if (ret < 0) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Order placing failed: {}", _id.c_str(), ret);
    return UINT_MAX;
  } else {
...
void TraderAdapter::updateUndone(const char *stdCode, double qty,
  ...
  if (bOuput)
    WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                       "[{}] {} qty of undone order updated, {} -> {}",
                       _id.c_str(), stdCode, oldQty, undone);
}
...
bool TraderAdapter::checkCancelLimits(const char *stdCode) {
  ...
  if (statInfo && riskPara->_cancel_total_limits != 0 &&
    ...
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] {} cancel {} times totaly, beyond boundary {} "
                       "times, adding to excluding list",
                       _id.c_str(), stdCode, statInfo->total_cancels(),
                       riskPara->_cancel_total_limits);
  ...
  if (it != _cancel_time_cache.end()) {
    ...
    if (cnt >= riskPara->_cancel_times_boundary) {
      ...
      if (times > riskPara->_cancel_times_boundary) {
        WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                           "[{}] {} cancel {} times within {} seconds, beyond "
                           "boundary {} times, adding to excluding list",
                           _id.c_str(), stdCode, times,
                           riskPara->_cancel_stat_timespan,
...
bool TraderAdapter::checkOrderLimits(const char *stdCode) {
  ...
  if (statInfo && riskPara->_order_total_limits != 0 &&
    ...
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] {} entrust {} times totally, beyond boundary {} "
                       "times, adding to excluding list",
                       _id.c_str(), stdCode, statInfo->total_orders(),
                       riskPara->_order_total_limits);
    _exclude_codes.insert(stdCode);
    return false;
  }
  ...
  if (it != _order_time_cache.end()) {
    ...
    if (cnt >= riskPara->_order_times_boundary) {
      ...
      if (times > riskPara->_order_times_boundary) {
        WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                           "[{}] {} entrust {} times within {} seconds, beyond "
                           "boundary {} times, adding to excluding list",
                           _id.c_str(), stdCode, times,
                           riskPara->_order_stat_timespan,
                           riskPara->_order_times_boundary);
        _exclude_codes.insert(stdCode);
        return false;
      }
...
OrderIDs TraderAdapter::buy(const char *stdCode, double price, double qty,
  ...
  if (isSelfMatched(stdCode)) {
    WTSLogger::log_dyn(
        "trader", _id.c_str(), LL_ERROR,
        "[{0}] Instructions on {1} are forbidden: {1} is in self matches",
        _id.c_str(), stdCode);
    return ret;
  }
  ...
  WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                     "[{}] Buying {} of quantity {}", _id.c_str(), stdCode,
                     qty);
  ...
  const ActionRuleGroup &ruleGP =
      _policy_mgr->getActionRules(commInfo->getFullPid());

  double left = qty;

  double unitQty =
      (price == 0.0) ? cInfo->getMaxMktVol() : cInfo->getMaxLmtVol();
    ...
    if (curRule._atype == AT_Open && !bForceClose) {
      ...
      if (curRule._limit != 0) {
        if (statItem.l_openvol + statItem.s_openvol >= curRule._limit) {
          ...
          maxQty = min(maxQty, curRule._limit - statItem.l_openvol -
                                   statItem.s_openvol);
        }
      }
      ...
      WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                         "[{}] Signal of buying {} of quantity {} triggered: "
                         "Opening long of quantity {}",
      ...
      if (!bForceClose && curRule._pure && !decimal::eq(pItem.s_prevol, 0.0)) {
        WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
                           "[{}] Closing new short position of {} skipped "
                           "because of non-zero pre short position",
                           _id.c_str(), stdCode);
        continue;
      }
      ...
      if (decimal::gt(maxQty, 0)) {
        ...
        for (;;) {
          double curQty = min(leftQty, unitQty);
          uint32_t localid =
              closeShort(stdCode, price, curQty,
                         (commInfo->getCoverMode() == CM_CoverToday), flag,
                         cInfo); // 如果不支持平今, 则直接下平仓标记即可
          ret.emplace_back(localid);
        ...
        if (commInfo->getCoverMode() == CM_CoverToday) {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of buying {} of quantity {} "
                             "triggered: Closing new short of quantity {}",
                             _id.c_str(), stdCode, qty, maxQty);
        } else {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of buying {} of quantity {} "
                             "triggered: Closing short of quantity {}",
                             _id.c_str(), stdCode, qty, maxQty);
        }
      }
      ...
      if (!bForceClose && curRule._pure && !decimal::eq(pItem.s_newvol, 0.0)) {
        WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
                           "[{}] Closing pre short position of {} skipped "
                           "because of non-zero new short position",
                           _id.c_str(), stdCode);
        continue;
      }
      ...
      if (decimal::gt(maxQty, 0)) {
        ...
        for (;;) {
          double curQty = min(leftQty, unitQty);
          uint32_t localid =
              closeShort(stdCode, price, curQty, false, flag,
                         cInfo); // 如果不支持平今, 则直接下平仓标记即可
          ret.emplace_back(localid);
        ...
        if (commInfo->getCoverMode() == CM_CoverToday) {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of buying {} of quantity {} "
                             "triggered: Closing old short of quantity {}",
                             _id.c_str(), stdCode, qty, maxQty);
        } else {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of buying {} of quantity {} "
      ...
      if (commInfo->getCoverMode() != CM_CoverToday) {
        ...
        if (decimal::gt(maxQty, 0)) {
          ...
          for (;;) {
            double curQty = min(leftQty, unitQty);
            uint32_t localid =
                closeShort(stdCode, price, curQty, false, flag, cInfo);
            ret.emplace_back(localid);
        ...
        if (decimal::gt(left, 0) && decimal::gt(pItem.s_newavail, 0)) {
          ...
          for (;;) {
            double curQty = min(leftQty, unitQty);
            uint32_t localid =
                closeShort(stdCode, price, curQty, true, flag, cInfo);
            ret.emplace_back(localid);
  ...
  if (left > 0) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Signal of buying {} of quantity {} left quantity "
                       "of {} not triggered",
...
OrderIDs TraderAdapter::sell(const char *stdCode, double price, double qty,
  ...
  WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                     "[{}] Selling {} of quantity {}", _id.c_str(), stdCode,
                     qty);
  ...
  for (auto it = ruleGP.begin(); it != ruleGP.end(); it++) {
    ...
    if (curRule._atype == AT_Open && !bForceClose) {
      ...
      WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                         "[{}] Signal of selling {} of quantity {} triggered: "
                         "Opening short of quantity {}",
                         _id.c_str(), stdCode, qty, maxQty);
    } else if (curRule._atype == AT_CloseToday) {
      double maxQty = 0;
      // 如果要区分平昨平今的品种, 则只读取可平今仓即可
      ...
      if (!bForceClose && curRule._pure && !decimal::eq(pItem.l_prevol, 0.0)) {
        WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
                           "[{}] Closing new long position of {} skipped "
                           "because of non-zero pre long position",
                           _id.c_str(), stdCode);
        continue;
      }
      ...
      if (decimal::gt(maxQty, 0)) {
        ...
        for (;;) {
          double curQty = min(leftQty, unitQty);
          uint32_t localid =
              closeLong(stdCode, price, curQty,
                        (commInfo->getCoverMode() == CM_CoverToday), flag,
                        cInfo); // 如果不支持平今, 则直接下平仓标记即可
          ret.emplace_back(localid);
        ...
        if (commInfo->getCoverMode() == CM_CoverToday) {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of selling {} of quantity {} "
                             "triggered: Closing new long of quantity {}",
                             _id.c_str(), stdCode, qty, maxQty);
        } else {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of selling {} of quantity {} "
                             "triggered: Closing long of quantity {}",
                             _id.c_str(), stdCode, qty, maxQty);
        }
      }
      ...
      if (!bForceClose && curRule._pure && !decimal::eq(pItem.l_newvol, 0.0)) {
        WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
                           "[{}] Closing pre long position of {} skipped "
                           "because of non-zero new long position",
                           _id.c_str(), stdCode);
        continue;
      }
      ...
      if (decimal::gt(maxQty, 0)) {
        ...
        for (;;) {
          double curQty = min(leftQty, unitQty);
          uint32_t localid =
              closeLong(stdCode, price, curQty, false, flag,
                        cInfo); // 如果不支持平今, 则直接下平仓标记即可
          ret.emplace_back(localid);
        ...
        if (commInfo->getCoverMode() == CM_CoverToday) {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of selling {} of quantity {} "
                             "triggered: Closing old long of quantity {}",
                             _id.c_str(), stdCode, qty, maxQty);
        } else {
          WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                             "[{}] Signal of selling {} of quantity {} "
      ...
      // 如果不区分平昨平今, 则统一平仓
      if (commInfo->getCoverMode() != CM_CoverToday) {
        double maxQty = min(pItem.avail_pos(true),
                            left); // 不区分平昨平今, 则读取全部可平量
        if (decimal::gt(maxQty, 0)) {
          double leftQty = maxQty;
          for (;;) {
            double curQty = min(leftQty, unitQty);
            uint32_t localid =
                closeLong(stdCode, price, curQty, false, flag, cInfo);
            ret.emplace_back(localid);
        ...
        if (decimal::gt(left, 0) && decimal::gt(pItem.l_newavail, 0)) {
          ...
          if (decimal::gt(maxQty, 0)) {
            ...
            for (;;) {
              double curQty = min(leftQty, unitQty);
              uint32_t localid =
                  closeLong(stdCode, price, curQty, true, flag, cInfo);
              ret.emplace_back(localid);
...
bool TraderAdapter::doCancel(WTSOrderInfo *ordInfo) {
  ...
  if (commInfo->getCategoty() == CC_FutOption ||
      commInfo->getCategoty() == CC_SpotOption)
    stdCode =
        CodeHelper::rawFutOptCodeToStdCode(cInfo->getCode(), cInfo->getExchg());
  else if (CodeHelper::isMonthlyCode(cInfo->getCode())) // 如果是分月合约
    stdCode =
        CodeHelper::rawMonthCodeToStdCode(cInfo->getCode(), cInfo->getExchg());
  else
    stdCode = CodeHelper::rawFlatCodeToStdCode(
        cInfo->getCode(), cInfo->getExchg(), cInfo->getProduct());
  // 撤单频率检查
  if (!checkCancelLimits(stdCode.c_str()))
    return false;

  WTSEntrustAction *action =
      WTSEntrustAction::create(ordInfo->getCode(), cInfo->getExchg());
...
bool TraderAdapter::cancel(uint32_t localid) {
  ...
  _cancel_time_cache[ordInfo->getCode()].emplace_back(
...
OrderIDs TraderAdapter::cancel(const char *stdCode, bool isBuy,
                               double qty /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, NULL);
  ...
  if (_orders != NULL && _orders->size() > 0) {
    for (auto it = _orders->begin(); it != _orders->end(); it++) {
      ...
      bool bBuy = (orderInfo->getDirection() == WDT_LONG &&
                   orderInfo->getOffsetType() == WOT_OPEN) ||
                  (orderInfo->getDirection() == WDT_SHORT &&
                   orderInfo->getOffsetType() != WOT_OPEN);
      if (bBuy != isBuy)
        continue;
...
uint32_t TraderAdapter::closeLong(const char *stdCode, double price, double qty,
                                  bool isToday, int flag,
                                  WTSContractInfo *cInfo /* = NULL */) {
  WTSEntrust *entrust = WTSEntrust::create(stdCode, qty, price);
  entrust->setContractInfo(cInfo == NULL ? getContract(stdCode) : cInfo);
  if (decimal::eq(price, 0.0))
...
uint32_t TraderAdapter::closeShort(const char *stdCode, double price,
                                   double qty, bool isToday, int flag,
                                   WTSContractInfo *cInfo /* = NULL */) {
  WTSEntrust *entrust = WTSEntrust::create(stdCode, qty, price);
  entrust->setContractInfo(cInfo == NULL ? getContract(stdCode) : cInfo);
  if (decimal::eq(price, 0.0))
...
void TraderAdapter::handleEvent(WTSTraderEvent e, int32_t ec) {
  if (e == WTE_Connect) {
    if (ec == 0) {
      _trader_api->login(_cfg->getCString("user"), _cfg->getCString("pass"),
                         WT_PRODUCT);
    } else {
      WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                         "[{}] Trading channel connecting failed: {}",
                         _id.c_str(), ec);
    }
  } else if (e == WTE_Close) {
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Trading channel disconnected: {}", _id.c_str(),
                       ec);
...
void TraderAdapter::onLoginResult(bool bSucc, const char *msg,
                                  uint32_t tradingdate) {
  if (!bSucc) {
    _state = AS_LOGINFAILED;
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Trader login failed: {}", _id.c_str(), msg);
    ...
    _state = AS_LOGINED;
    WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                       "[{}] Trader login succeed, trading date: {}",
                       _id.c_str(), tradingdate);
...
void TraderAdapter::onRspEntrust(WTSEntrust *entrust, WTSError *err) {
  if (err && err->getErrorCode() != WEC_NONE) {
    ...
    if (commInfo->getCategoty() == CC_FutOption ||
        commInfo->getCategoty() == CC_SpotOption)
      stdCode = CodeHelper::rawFutOptCodeToStdCode(cInfo->getCode(),
                                                   cInfo->getExchg());
    else if (CodeHelper::isMonthlyCode(cInfo->getCode())) // 如果是分月合约
      stdCode = CodeHelper::rawMonthCodeToStdCode(cInfo->getCode(),
                                                  cInfo->getExchg());
    else
      stdCode = CodeHelper::rawFlatCodeToStdCode(
          cInfo->getCode(), cInfo->getExchg(), cInfo->getProduct());
    ...
    WTSLogger::log_dyn(
        "trader", _id.c_str(), LL_ERROR,
        "[{}] Order placing failed: {}, instrument: {}, action: {}, qty: {}",
        _id.c_str(), err->getMessage(), entrust->getCode(), action.c_str(),
        qty);
    ...
    if (strlen(entrust->getUserTag()) > 0) {
      ...
      if (_notifier)
        _notifier->notify(
            id(), fmt::format(" Order placing failed: {}", err->getMessage())
                      .c_str());
    } else {
      WTSLogger::log_dyn("trader", _id.c_str(), LL_WARN,
                         "[{}] Outter Order placing failed: {}, instrument: "
                         "{}, action: {}, qty: {}",
                         _id.c_str(), err->getMessage(), entrust->getCode(),
                         action.c_str(), qty);
    }
  }
}
...
void TraderAdapter::onRspAccount(WTSArray *ayAccounts) {
  ...
  if (ayAccounts) {
    ...
    for (auto sink : _sinks) {
      for (uint32_t idx = 0; idx < ayAccounts->size(); idx++) {
        ...
        sink->on_account(fundInfo->getCurrency(), fundInfo->getPreBalance(),
                         fundInfo->getBalance(),
                         fundInfo->getBalance() + fundInfo->getDynProfit(),
                         fundInfo->getAvailable(), fundInfo->getCloseProfit(),
                         fundInfo->getDynProfit(), fundInfo->getMargin(),
                         fundInfo->getCommission(), fundInfo->getDeposit(),
                         fundInfo->getWithdraw());
      }
    }
  }
  ...
  if (_state == AS_TRADES_QRYED) {
    ...
    WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                       "[{}] Trading channel ready", _id.c_str());
    for (auto &sink : _sinks)
...
void TraderAdapter::onRspPosition(const WTSArray *ayPositions) {
  if (ayPositions && ayPositions->size() > 0) {
    ...
    for (auto it = _positions.begin(); it != _positions.end(); it++) {
      ...
      for (auto sink : _sinks) {
        sink->on_position(stdCode, true, pItem.l_prevol, pItem.l_preavail,
                          pItem.l_newvol, pItem.l_newavail, _trading_day);
        sink->on_position(stdCode, false, pItem.s_prevol, pItem.s_preavail,
                          pItem.s_newvol, pItem.s_newavail, _trading_day);
      }
    }
  }

  WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                     "[{}] Position data updated", _id.c_str());
...
void TraderAdapter::onRspOrders(const WTSArray *ayOrders) {
  if (ayOrders) {
    ...
    for (auto it = ayOrders->begin(); it != ayOrders->end(); it++) {
      ...
      bool isBuy = (orderInfo->getDirection() == WDT_LONG &&
                    orderInfo->getOffsetType() == WOT_OPEN) ||
                   (orderInfo->getDirection() == WDT_SHORT &&
                    orderInfo->getOffsetType() != WOT_OPEN);
      ...
      WTSTradeStateInfo *statInfo =
          (WTSTradeStateInfo *)_stat_map->get(stdCode.c_str());
      if (statInfo == NULL) {
      ...
      if (isBuy) {
        ...
        if (orderInfo->isError()) {
          statItem.b_wrongs++;
          statItem.b_wrongqty +=
              orderInfo->getVolume() - orderInfo->getVolTraded();
      ...
      if (!StrUtil::startsWith(orderInfo->getUserTag(), _order_pattern.c_str(),
                               true))
        continue;
      ;
    ...
    for (auto it = _undone_qty.begin(); it != _undone_qty.end(); it++) {
      ...
      WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                         "[{}]{} undone quantity {}", _id.c_str(), stdCode,
                         curQty);
    }
  }
...
void TraderAdapter::printPosition(const char *code, const PosItem &pItem) {
  WTSLogger::log_dyn(
      "trader", _id.c_str(), LL_INFO,
      "[{}] {} position updated, long:{}[{}]|{}[{}], short:{}[{}]|{}[{}]",
      _id.c_str(), code, pItem.l_prevol, pItem.l_preavail, pItem.l_newvol,
      pItem.l_newavail, pItem.s_prevol, pItem.s_preavail, pItem.s_newvol,
      pItem.s_newavail);
}
  ...
  if (ayTrades) {
    for (auto it = ayTrades->begin(); it != ayTrades->end(); it++) {
      ...
      if (commInfo->getCategoty() == CC_Future)
        ...
        stdCode = CodeHelper::rawFlatCodeToStdCode(
            cInfo->getCode(), cInfo->getExchg(), commInfo->getProduct());
    ...
    for (auto it = _stat_map->begin(); it != _stat_map->end(); it++) {
      ...
      WTSLogger::log_dyn(
          "trader", _id.c_str(), LL_INFO,
          "[{}] {} action stats updated, long opened: {}, long closed: {}, new "
          "long closed: {}, short opened: {}, short closed: {}, new short "
          "closed: {}",
          _id.c_str(), stdCode, pItem->open_volume_long(),
          pItem->close_volume_long(), pItem->closet_volume_long(),
          pItem->open_volume_short(), pItem->close_volume_short(),
          pItem->closet_volume_short());
    }
  }
...
bool TraderAdapter::checkSelfMatch(const char *stdCode, WTSTradeInfo *tInfo) {
  ...
  if (it != _trade_refs.end()) {
    ...
    if (oid.compare(refid) != 0) {
      ...
      WTSLogger::log_dyn("trader", _id.c_str(), LL_FATAL,
                         "[{0}] Self matching detected on {1}!!! Instructions "
                         "on {1} will be forbidden!!!",
                         _id.c_str(), stdCode);
...
inline const char *stateToName(WTSOrderState woState) {
  if (woState == WOS_AllTraded)
    return "AllTrd";
  else if (woState == WOS_PartTraded_NotQueuing ||
           woState == WOS_PartTraded_Queuing)
    return "PrtTrd";
...
void TraderAdapter::onPushOrder(WTSOrderInfo *orderInfo) {
  ...
  WTSLogger::log_dyn(
      "trader", _id.c_str(), LL_DEBUG,
      "[{}] Order notified, instrument: {}, usertag: {}, state: {}",
      _id.c_str(), stdCode.c_str(), orderInfo->getUserTag(),
      stateToName(orderInfo->getOrderState()));
    ...
    if (it == _orderids.end()) {
      ...
      if (orderInfo->getOffsetType() != WOT_OPEN) {
        ...
        if (isLong) // 平多
        {
          if (isToday) {
            pItem.l_newavail -=
                min(pItem.l_newavail, qty); // 如果是平今, 则只需要更新可平今仓
          } else {
            double left = qty;
  ...
  if (StrUtil::startsWith(orderInfo->getUserTag(), _order_pattern.c_str(),
    ...
    if (orderInfo->getOrderState() == WOS_Canceled) {
      ...
      WTSLogger::log_dyn(
          "trader", _id.c_str(), LL_INFO,
          "[{}] Order {} of {} canceled:{}, action: {}, leftqty: {}",
          _id.c_str(), orderInfo->getUserTag(), stdCode.c_str(),
          orderInfo->getStateMsg(),
          formatAction(orderInfo->getDirection(), orderInfo->getOffsetType()),
          qty);
    }
  }
  ...
  if (localid != 0) {
    ...
    // 通知所有监听接口
    for (auto sink : _sinks)
      sink->on_order(localid, stdCode.c_str(), isBuy, orderInfo->getVolume(),
                     orderInfo->getVolLeft(), orderInfo->getPrice(),
                     orderInfo->getOrderState() == WOS_Canceled);
  }
...
void TraderAdapter::onPushTrade(WTSTradeInfo *tradeRecord) {
  ...
  bool isBuy = (tradeRecord->getDirection() == WDT_LONG &&
                tradeRecord->getOffsetType() == WOT_OPEN) ||
               (tradeRecord->getDirection() == WDT_SHORT &&
                tradeRecord->getOffsetType() != WOT_OPEN);
  ...
  WTSLogger::log_dyn("trader", _id.c_str(), LL_INFO,
                     "[{}] Trade notified, instrument: {}, usertag: {}, "
                     "trdqty: {}, trdprice: {}",
                     _id.c_str(), stdCode.c_str(), tradeRecord->getUserTag(),
                     tradeRecord->getVolume(), tradeRecord->getPrice());
  ...
  // 如果是自己的订单，则更新未完成单
  uint32_t localid = 0;
  if (StrUtil::startsWith(tradeRecord->getUserTag(), _order_pattern.c_str(),
                          true)) {
  ...
  for (auto sink : _sinks)
    sink->on_trade(localid, stdCode.c_str(), isBuy, vol,
...
void TraderAdapter::onTraderError(WTSError *err, void *pData /* = NULL */) {
  if (err)
    WTSLogger::log_dyn("trader", _id.c_str(), LL_ERROR,
                       "[{}] Error of trading channel occured: {}", _id.c_str(),
                       err->getMessage());

  if (_notifier)
    _notifier->notify(
        id(),
        fmt::format("Trading channel error: {}", err->getMessage()).c_str());
}

IBaseDataMgr *TraderAdapter::getBaseDataMgr() { return _bd_mgr; }
...
//////////////////////////////////////////////////////////////////////////
// CTPWrapperMgr
bool TraderAdapterMgr::addAdapter(const char *tname,
                                  TraderAdapterPtr &adapter) {
