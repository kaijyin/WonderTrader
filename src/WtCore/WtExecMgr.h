...
public:
  ...
  void handle_pos_change(const char *stdCode, double targetPos, double diffPos,
   ...
   *	清除缓存的目标仓位
   */
  inline void clear_cached_targets() { _all_cached_targets.clear(); }
   ...
   *	@execid		执行器ID
   */
  void add_target_to_cache(const char *stdCode, double targetPos,
                           const char *execid = "ALL");
