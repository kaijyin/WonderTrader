...
bool SelStrategyMgr::loadFactories(const char *path) {
  ...
  for (boost::filesystem::directory_iterator iter(myPath); iter != endIter;
    ...
    FuncCreateSelStraFact creator =
        (FuncCreateSelStraFact)DLLHelper::get_symbol(hInst,
                                                     "createMfStrategyFact");
    if (creator == NULL) {
    ...
    if (fact != NULL) {
      ...
      fInfo._remover = (FuncDeleteSelStraFact)DLLHelper::get_symbol(
          hInst, "deleteMfStrategyFact");
      fInfo._fact = fact;
  ...
  WTSLogger::info("{} SEL strategy factories in directory[{}] loaded", count,
                  path);

  return true;
}

SelStrategyPtr SelStrategyMgr::createStrategy(const char *factname,
                                              const char *unitname,
                                              const char *id) {
  ...
  SelStrategyPtr ret(new SelStraWrapper(
      fInfo._fact->createStrategy(unitname, id), fInfo._fact));
  _strategies[id] = ret;
  return ret;
}

SelStrategyPtr SelStrategyMgr::createStrategy(const char *name,
                                              const char *id) {
