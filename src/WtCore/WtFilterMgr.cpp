...
void WtFilterMgr::load_filters(const char *fileName) {
  ...
  uint64_t lastModTime =
      boost::filesystem::last_write_time(boost::filesystem::path(_filter_file));
  if (lastModTime <= _filter_timestamp)
    return;

  if (_filter_timestamp != 0) {
    WTSLogger::info("Filters configuration file {} modified, will be reloaded",
                    _filter_file);
    if (_notifier)
  ...
  if (filterStra) {
    ...
    for (const std::string &key : keys) {
      ...
      if (fAct == FA_None) {
        WTSLogger::error("Action {} of strategy filter {} not recognized",
                         action, key);
        continue;
      }
  ...
  if (filterCodes) {
    ...
    for (const std::string &stdCode : codes) {
      ...
      if (fAct == FA_None) {
        WTSLogger::error("Action {} of code filter {} not recognized", action,
                         stdCode);
        continue;
      }
  ...
  if (filterExecuters) {
    ...
    for (const std::string &execid : executer_ids) {
      ...
      WTSLogger::info("Executer {} is %s", execid,
...
const char *FLTACT_NAMEs[] = {"Ignore", "Redirect"};

bool WtFilterMgr::is_filtered_by_strategy(const char *straName,
                                          double &targetPos,
                                          bool isDiff /* = false */) {
  ...
  if (it != _stra_filters.end()) {
    ...
    if (isDiff) {
      ...
      WTSLogger::info("[Filters] Strategy filter {} triggered, the change of "
                      "position ignored directly",
                      straName);
      return true;
    }

    WTSLogger::info(
        "[Filters] Strategy filter {} triggered, action: {}", straName,
        fItem._action <= FA_Redirect ? FLTACT_NAMEs[fItem._action] : "Unknown");
...
bool WtFilterMgr::is_filtered_by_code(const char *stdCode, double &targetPos) {
  ...
  if (cit != _code_filters.end()) {
    ...
    WTSLogger::info("[Filters] Code filter {} triggered, action: {}", stdCode,
                    fItem._action <= FA_Redirect ? FLTACT_NAMEs[fItem._action]
                                                 : "Unknown");
    if (fItem._action == FA_Ignore) {
      return true;
    } else if (fItem._action == FA_Redirect) {
  ...
  if (cit != _code_filters.end()) {
    const FilterItem &fItem = cit->second;
    WTSLogger::info(
        "[Filters] CommID filter {} triggered, action: {}", cInfo.stdCommID(),
        fItem._action <= FA_Redirect ? FLTACT_NAMEs[fItem._action] : "Unknown");
    if (fItem._action == FA_Ignore) {
      return true;
    } else if (fItem._action == FA_Redirect) {
