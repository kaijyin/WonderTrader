...
public:
  ...
  virtual void init(WTSVariant *cfg, IBaseDataMgr *bdMgr, WtDtMgr *dataMgr,
                    IHotMgr *hotMgr, EventNotifier *notifier) override;
...
public:
  // uint32_t	register_task(const char* name, uint32_t date, uint32_t time,
  // TaskPeriodType period, bool bStrict = true, const char* trdtpl = "CHINA");
  void addContext(SelContextPtr ctx, uint32_t date, uint32_t time,
  ...
  void handle_pos_change(const char *straName, const char *stdCode,
                         double diffQty);
