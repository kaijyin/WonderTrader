...
struct MATRADEAPI CReqSysField {
  ...
  int iFlashModule;            // 软加速模块类型 0:默认连接 1:普通连接
                    // 2:TCP_DIRECT连接[Lnx64支持] 4:RDMA连接[Lnx64支持]
  char chFlashPub; // 软加速推送模式 '0':按账户普通  '1':按账户精简
                   // '2':按连接普通 '3':按连接精简
};
...
struct MATRADEAPI CReqSubTopicField {
  ...
  char chDataSet;         // 成交数据集 '0':仅含本连接发送委托的成交信息
                          // '1':含所有连接发送委托的成交信息
};
...
struct MATRADEAPI CRspHeartBeatField {
  ...
  char szServerNodeUse[4 + 1];      // 节点用途 ‘n’:待机节点 ‘s’:备机节点
                                    // ‘a’:主用节点
...
struct MATRADEAPI CReqCosLoginField {
  ...
  char szThirdParty
      [1024 +
       1]; // 第三方
           // 使用第三方客户端的用户代码必输,字段内容由密钥dll指定接口获取;
  char chChannel;           // 操作渠道
  char szSerChannel[8 + 1]; // 服务渠道
};
...
struct MATRADEAPI CRspCosLoginField {
  ...
  char szSessionId
      [128 +
       1]; // 会话凭证
           // 客户端需保存，后续请求填写到固定入参会话凭证(8814)中，否则后续请求将被拒绝。
};
...
virtual int OnFileDownload(const char *p_pszFileName, int p_nResult) {
  return 0;
}

// 客户端上传目录时，每个文件上传时该方法被调用 p_nResult:进度百分比
virtual int OnFolderUpload(const char *p_pszFileName, int p_nResult) {
  return 0;
}

// 客户端下载目录时，每个文件下载时该方法被调用 p_nResult:进度百分比
virtual int OnFolderDownload(const char *p_pszFileName, int p_nResult) {
  return 0;
}

// 连接保活响应
virtual int OnRspHeartBeat(CFirstSetField *p_pFirstSetField,
                           CRspHeartBeatField *p_pRspField,
...
// 安全检查响应
virtual int OnRspSafeCheck(CFirstSetField *p_pFirstSetField,
                           CRspSafeCheckField *p_pRspField,
                           LONGLONG p_llRequestId, int p_iFieldNum,
                           int p_iFieldIndex) {
  return 0;
}
...
// 文件信息查询响应
virtual int OnRspQryFileInfo(CFirstSetField *p_pFirstSetField,
                             CRspQryFileInfoField *p_pRspField,
                             LONGLONG p_llRequestId, int p_iFieldNum,
                             int p_iFieldIndex) {
  return 0;
}
...
// 策略启动响应
virtual int OnRspStgyStart(CFirstSetField *p_pFirstSetField,
                           CRspStgyStartField *p_pRspField,
                           LONGLONG p_llRequestId, int p_iFieldNum,
                           int p_iFieldIndex) {
  return 0;
}
...
// 策略停止响应
virtual int OnRspStgyStop(CFirstSetField *p_pFirstSetField,
                          CRspStgyStopField *p_pRspField,
                          LONGLONG p_llRequestId, int p_iFieldNum,
                          int p_iFieldIndex) {
  return 0;
}
...
// 策略状态查询响应
virtual int OnRspQryStgyStat(CFirstSetField *p_pFirstSetField,
                             CRspQryStgyStatField *p_pRspField,
                             LONGLONG p_llRequestId, int p_iFieldNum,
                             int p_iFieldIndex) {
  return 0;
}
...
// COS系统登录功能响应
virtual int OnRspCosLogin(CFirstSetField *p_pFirstSet,
                          CRspCosLoginField *p_pRspField,
                          LONGLONG p_llRequestId, int p_iFieldNum,
                          int p_iFieldIndex) {
  return 0;
}
...
public:
  ...
  virtual int RegisterServer(const char *p_pszIp, int p_iPort,
                             unsigned int p_uiTimeout = 1,
                             const char *p_pszPktVer = "01",
                             bool p_bCos = false, int p_iSsl = -1);
  ...
  // 注册账号信息
  virtual int RegisterAcct(char p_chChannel, const char *p_pszOpSite = 0,
                           int p_iSiteLen = 0);
  ...
  // 连接保活请求
  virtual int ReqHeartBeat(CReqHeartBeatField *p_pReqField,
                           LONGLONG p_llRequestId);

  // 安全检查请求
  virtual int ReqSafeCheck(CReqSafeCheckField *p_pReqField,
                           LONGLONG p_llRequestId);

  // 上传文件请求
  virtual int ReqFileUpload(CReqFileUploadField *p_pReqField,
                            LONGLONG p_llRequestId);

  // 下载文件请求
  virtual int ReqFileDownload(CReqFileDownloadField *p_pReqField,
                              LONGLONG p_llRequestId);

  // 上传目录请求
  virtual int ReqFolderUpload(CReqFolderUploadField *p_pReqField,
                              LONGLONG p_llRequestId);

  // 下载目录请求
  virtual int ReqFolderDownload(CReqFolderDownloadField *p_pReqField,
                                LONGLONG p_llRequestId);

  // 文件信息查询请求
  virtual int ReqQryFileInfo(CReqQryFileInfoField *p_pReqField,
                             LONGLONG p_llRequestId);

  // 策略启动请求
  virtual int ReqStgyStart(CReqStgyStartField *p_pReqField,
                           LONGLONG p_llRequestId);

  // 策略停止请求
  virtual int ReqStgyStop(CReqStgyStopField *p_pReqField,
                          LONGLONG p_llRequestId);

  // 策略状态查询请求
  virtual int ReqQryStgyStat(CReqQryStgyStatField *p_pReqField,
                             LONGLONG p_llRequestId);

  // COS系统登录功能请求
  virtual int ReqCosLogin(CReqCosLoginField *p_pReqField,
                          LONGLONG p_llRequestId);

  // 设置业务包头信息
  int SetBizPackHead(const char *p_pszFuncId, LONGLONG p_llRequestId,
                     char *p_pszMsgId, char p_chPkgType, char p_chPkgVer = '2',
                     char p_chFuncType = 0);

  // 订阅主题
  int SubTopic(const char *p_pszTopic, const char *p_pszFilter,
               char p_chDataSet = '0');
  ...
  // 订阅请求响应
  void OnRspSubTopic(CFirstSetField *p_pFirstSetField, LONGLONG p_llRequestId,
                     int p_iFieldNum);

  // 退订请求响应
  void OnRspUnsubTopic(CFirstSetField *p_pFirstSetField, LONGLONG p_llRequestId,
                       int p_iFieldNum);

  // 连接保活请求响应
  void OnRspHeartBeat(CFirstSetField *p_pFirstSetField, LONGLONG p_llRequestId,
                      int p_iFieldNum);

  // 安全检查请求响应
  void OnRspSafeCheck(CFirstSetField *p_pFirstSetField, LONGLONG p_llRequestId,
                      int p_iFieldNum);

  // 文件信息查询请求响应
  void OnRspQryFileInfo(CFirstSetField *p_pFirstSetField,
                        LONGLONG p_llRequestId, int p_iFieldNum);

  // 策略启动请求响应
  void OnRspStgyStart(CFirstSetField *p_pFirstSetField, LONGLONG p_llRequestId,
                      int p_iFieldNum);

  // 策略停止请求响应
  void OnRspStgyStop(CFirstSetField *p_pFirstSetField, LONGLONG p_llRequestId,
                     int p_iFieldNum);

  // 策略状态查询请求响应
  void OnRspQryStgyStat(CFirstSetField *p_pFirstSetField,
                        LONGLONG p_llRequestId, int p_iFieldNum);

  // 量化系统注册功能响应
  void OnRspCosLogin(CFirstSetField *p_pFirstSetField, LONGLONG p_llRequestId,
                     int p_iFieldNum);
...
public:
  // 异步回调函数
  virtual void OnArCallback(const char *p_pszMsgId,
                            const unsigned char *p_pszDataBuff, int p_iDataLen);

  // 发布回调函数
  virtual void OnPsCallback(const char *p_pszAcceptSn,
                            const unsigned char *p_pszDataBuff, int p_iDataLen);

  // 连接回调函数
  virtual void OnNetCallback(const char *p_pszNetState,
                             const unsigned char *p_pszDataBuff,
                             int p_iDataLen);

  // 文件上传回调
  virtual void OnFileUpCallback(const char *p_pszFileName,
                                const unsigned char *p_pszDataBuff,
                                int p_iDataLen);

  // 文件下载回调
  virtual void OnFileDownCallback(const char *p_pszFileName,
                                  const unsigned char *p_pszDataBuff,
                                  int p_iDataLen);

  // 目录上传回调
  virtual void OnFolderUpCallback(const char *p_pszFileName,
                                  const unsigned char *p_pszDataBuff,
                                  int p_iDataLen);

  // 目录下载回调
  virtual void OnFolderDownCallback(const char *p_pszFileName,
                                    const unsigned char *p_pszDataBuff,
                                    int p_iDataLen);
