...
struct MATRADEAPI CReqOptUserLoginField {
  ...
  char
      chEncryptType; // 加密方式 ‘0’:金证KBSS系统加密
                     // ‘1’:金证Win版集中交易加密 ‘2’:金证Unix版集中交易加密
                     // ‘3’:外部重加密 ‘4’ : 外部加密
...
struct MATRADEAPI CRspOptUserLoginField {
  ...
  char chSzFlashMode;     // 软加速深圳市场加速模式 ‘0’-不加速 ‘1’-软件加速
                          // ‘2’-FPGA加速
  char chShFlashMode;     // 软加速上海市场加速模式 ‘0’-不加速 ‘1’-软件加速
                          // ‘2’-FPGA加速
...
struct MATRADEAPI CReqOptBaseInfoField {
  ...
  char chOptType;            // 合约类型
  char chIsRemind;           // 附加提醒标志
                   // 1-返回提醒信息标签；非1-不返回（用于委托时进行提醒）
...
struct MATRADEAPI CRspOptBaseInfoField {
  ...
  char szQryPos[32 + 1];        // 定位串
  char szRemindFlags[16 + 1];   // 提醒信息标签
                              // 根据入参IS_REMIND进行返回，若IS_REMIND送’1’，该出参返回。格式为’0,1,2,3’，
                              // ’0’表示严重价外期权开仓委托时提醒
                              // ’1’表示临近到期期权开仓委托时提醒
                              // ’2’表示合约调整期权开仓委托时提醒
                              // ‘3’表示虚值合约提醒
...
struct MATRADEAPI CReqOptOrderField {
  ...
  short
      iStkBizAction; // 证券业务行为
                     // 字典[STK_BIZ_ACTION]，认购行权、认沽行权、普通询价默认送100
  char szStkpbu[8 + 1]; // 交易单元
  int iOrderBsn;        // 委托批号
  ...
  char szClientInfo
      [256 +
       1]; // 终端信息
           // 1，通过mid接入时，值必须和固定入参F_OP_SITE(操作站点)的值保持一致；
           // 2，通过其它方式接入时，值可以为空，如果送值，必须和固定入参F_OP_SITE(操作站点)的值保持一致
  LONGLONG llCustCode;          // 客户代码
  int iIntOrg;                  // 内部机构
  char chSecurityLevel;         // 安全手段 0:无安全 1:校验票据 2:校验密码
  char szSecurityInfo[256 + 1]; // 安全信息 安全手段为校验票据则需填写票据
                                // 安全手段为密码则需填写交易密码密文
  char szOrderIdEx[64 +
                   1]; // 外部合同序号 此序号为外部传入，期权系统不做任何处理
  char
      chEncryptType; // 加密方式 当安全手段为2：校验密码时，为必填入参。
                     // 加密方式: ‘0’:金证KBSS系统加密方式
                     // ‘1’:金证Win版集中交易加密方式
                     // ‘2’:金证Unix版集中交易加密方式 ‘3’:外部重加密方式
                     // ‘4’:外部加密方式
                     // 说明：该入参根据接入券商要求填写，默认值为0。
                     // 外部重加密方式’3’：使用非金证加密函数将密码明文加密为密文后，然后使用金证公司提供的通信加密函数kbss_comencrypt对密文再一次加密。
                     // 外部加密方式’4’：使用非金证加密函数将密码明文加密为密文后作为入参。
  char szEncryptKey[32 + 1]; // 加密因子 当安全手段为2：校验密码时，为必填入参。
                             // 加密因子可以填写为资产账户：CUACCT_CODE参数值
...
struct MATRADEAPI CReqOptUndlStkLockField {
  ...
  char szSecurityInfo
      [256 +
       1]; // 安全手段为校验票据则需填写票据,安全手段为密码则需填写交易密码密文
...
struct MATRADEAPI CReqOptMaxTradeQtyField {
  ...
  char szOptNum
      [16 +
       1]; // 合约编码
           // 若为开平仓、行权，合约编码不能为空；若为证券锁定解锁，证券代码不能为空；若为构建与拆分组合均不送；若为行权指令合并申报均不送
  char szStkCode
      [8 +
       1]; // 证券代码
           // 若为开平仓、行权，合约编码不能为空；若为证券锁定解锁，证券代码不能为空；若为构建与拆分组合均不送；若为行权指令合并申报均不送
  char szOrderPrice
      [21 +
       1]; // 委托价格
           // 开平仓必送，行权、证券锁定解锁、构建与拆分组合、行权指令合并申报非必送
  int iStkBiz;       // 证券业务 字典[STK_BIZ]
  int iStkBizAction; // 业务行为
                     // 字典[STK_BIZ_ACTION]，其中行权、证券锁定解锁、构建与拆分组合、行权指令合并申报送100
  char szCombNum[16 +
                 1]; // 组合编码 构建组合策略时不需送入，组合策略单边平仓时送值
  char szCombStraCode[16 + 1]; // 组合策略代码 构建与拆分组合必送，其他不送
  char
      szLeg1Num[16 + 1]; // 成分一合约编码 1.认购普通义务仓转备兑仓成分一合约编码必送；
                         // 2.构建组合时，除认购普通义务仓转备兑仓之外，其他组合策略有几条腿就必须送入几个成分合约编码，并需按序送入；
                         // 3.解除组合时，若送入组合编码COMB_NUM，则成分合约不需送入。
                         // 4.行权指令合并申报成分一合约编码和成分二合约编码必送
  char
      szLeg2Num[16 + 1]; // 成分二合约编码 1.认购普通义务仓转备兑仓成分一合约编码必送；
                         // 2.构建组合时，除认购普通义务仓转备兑仓之外，其他组合策略有几条腿就必须送入几个成分合约编码，并需按序送入；
                         // 3.解除组合时，若送入组合编码COMB_NUM，则成分合约不需送入。
                         // 4.行权指令合并申报成分一合约编码和成分二合约编码必送
  char
      szLeg3Num[16 + 1]; // 成分三合约编码 1.认购普通义务仓转备兑仓成分一合约编码必送；
                         // 2.构建组合时，除认购普通义务仓转备兑仓之外，其他组合策略有几条腿就必须送入几个成分合约编码，并需按序送入；
                         // 3.解除组合时，若送入组合编码COMB_NUM，则成分合约不需送入。
                         // 4.行权指令合并申报成分一合约编码和成分二合约编码必送
  char
      szLeg4Num[16 + 1]; // 成分四合约编码 1.认购普通义务仓转备兑仓成分一合约编码必送；
                         // 2.构建组合时，除认购普通义务仓转备兑仓之外，其他组合策略有几条腿就必须送入几个成分合约编码，并需按序送入；
                         // 3.解除组合时，若送入组合编码COMB_NUM，则成分合约不需送入。
                         // 4.行权指令合并申报成分一合约编码和成分二合约编码必送
};
...
struct MATRADEAPI CReqOptExpendableFundField {
  ...
  int iValueFlag;        // 取值标志 字段用于标识，可复合取值
                         // MARKET_VALUE、FUND_VALUE、 STK_VALUE、FUND_LOAN
                         // 四个字段是否取值 0：均不计算 1：取MARKET_VALUE值
                         // 2：取FUND_VALUE值 4：取STK_VALUE值 8：取FUND_LOAN值
                         // 可复合取值
};
...
struct MATRADEAPI CRspOptExpendableFundField {
  ...
  char szMarginInclRlt[21 +
                       1]; // 已占用保证金(含未成交)
                           // 包含卖出开仓委托未成交冻结的保证金(按前结算价计算)
  ...
  char szMarginInclDyn[21 +
                       1]; // 动态占用保证金(含未成交)
                           // 包含卖出开仓委托未成交冻结的保证金(按实时价格计算)
  char szDailyInAmt[21 + 1];  // 当日入金
  char szDailyOutAmt[21 + 1]; // 当日出金
  char szFundRealAvl
      [21 +
       1]; // 资金实际可用
           // 当金证股票期权柜台系统启用动态可用功能时，资金实际可用=min（资金可用金额，资金动态可用）；当不启用动态可用时，资金实际可用=资金可用金额
};
...
struct MATRADEAPI CRspOptExpendableCuField {
  ...
  char szOptUndlCode[8 + 1];     // 标的证券代码
  char szExerciseVal[21 +
                     1]; // 行权价值 认购权利仓的行权价值：MAX((标的-行权价), 0)
                         // * 合约单位 * 合约张数
                         // 认沽权利仓的行权价值：MAX((行权价-标的价), 0) *
                         // 合约单位 * 合约张数
...
struct MATRADEAPI CRspOptCurrDayOrderField {
  ...
  char szMarginPreFrz[21 +
                      1]; // 预占用保证金
                          // 卖开委托时填写预冻结的保证金金额，其他情况填“0”
  char szMarginFrz[21 +
                   1]; // 占用保证金
                       // 卖开成交时填写实际冻结的保证金金额，其他情况填“0”
  char szMarginPreUfz[21 +
                      1]; // 预解冻保证金
                          // 买平委托时填写预解冻的保证金金额，其他情况填“0”
  char szMarginUfz[21 +
                   1]; // 解冻保证金
                       // 买平成交时填写实际解冻的保证金金额，其他情况填“0”
};
...
struct MATRADEAPI CRtnOptOrderFillField {
  ...
  int iCuacctSn;             // 账户序号
  char szStkbd[2 + 1];       // 交易板块 字典[STKBD]
  char chMatchedType; // 成交类型 ‘0’:内部撤单成交 ‘1’:非法委托撤单成交
                      // ‘2’:交易所成交（含主动撤单成交与撮合委托成交）
  ...
  char szCombStraCode[16 +
                      1]; // 组合策略代码 构建组合、解除组合时填写，其他情况填空
  char szLeg1Num[16 +
                 1]; // 成分一合约编码
                     // 构建组合、解除组合、行权指令合并申报时填写，其他情况填空
  char szLeg2Num[16 +
                 1]; // 成分二合约编码
                     // 构建组合、解除组合、行权指令合并申报时填写，其他情况填空
  char szLeg3Num[16 +
                 1]; // 成分三合约编码  构建组合、解除组合时填写，其他情况填空
  char szLeg4Num[16 +
                 1]; // 成分四合约编码  构建组合、解除组合时填写，其他情况填空
...
struct MATRADEAPI CReqOptCombStraPosField {
  ...
  char szCombStraCode[16 +
                      1];  // 组合策略代码  COMB_STRA_CODE  VARCHAR(16)  X  8928
...
struct MATRADEAPI CRspOptSettListField {
  ...
  char szAddMarginNotice[2048 +
                         1];  // 追保通知内容  ADD_MARGIN_NOTICE  VARCHAR(2048)
...
struct MATRADEAPI CRspOptRiskLvlField {
  ...
  char szStkexFundAvlRlt[21 + 1]; // 实时可用金额(交易所)  STKEX_FUND_AVL_RLT
                                  // CMONEY  8864
  ...
  char szStkexRiskValueRlt[21 + 1]; // 实时风险率(交易所)  STKEX_RISK_VALUE_RLT
                                    // CRATE  9089
  char szMarginUsed[21 + 1];    // 占用保证金  MARGIN_USED  CMONEY  9041
  char szMarginUsedRlt[21 + 1]; // 实时占用保证金  MARGIN_USED_RLT  CMONEY  9042
  char
      szStkexMagUsed[21 + 1]; // 占用保证金(交易所)  STKEX_MAG_USED  CMONEY 9043
  char szStkexMagUsedRlt[21 + 1]; // 实时占用保证金(交易所)  STKEX_MAG_USED_RLT
                                  // CMONEY  9044
  char szHedgedMargin[21 + 1]; // 对冲后保证金  HEDGED_MARGIN  CMONEY  9045
  char szHedgedMarginRlt[21 +
                         1]; // 对冲后实时保证金  HEDGED_MARGIN_RLT  CMONEY 9046
  char szStkexHedgedMag[21 + 1]; // 对冲后保证金(交易所)  STKEX_HEDGED_MAG
                                 // CMONEY  9047
  char szStkexHedgedMagRlt[21 + 1]; // 对冲后实时保证金(交易所)
                                    // STKEX_HEDGED_MAG_RLT  CMONEY  9048
};
...
struct MATRADEAPI CReqOptCombExeField {
  ...
  char szOrderIdEx[64 +
                   1]; // 外部合同序号 此序号为外部传入，期权系统不做任何处理
  char chEncryptType;  // 加密方式 当安全手段为2：校验密码时，为必填入参。
                       // 加密方式: ‘0’:金证KBSS系统加密方式
                       // ‘1’:金证Win版集中交易加密方式
                       // ‘2’:金证Unix版集中交易加密方式 ‘3’:外部重加密方式
                       // ‘4’:外部加密方式
...
struct MATRADEAPI CReqOptQryCurrDayOrderExField {
  LONGLONG llCuacctCode; // 资产账户
  char chQueryFlag;      // 查询方向 0:向后取数据 1:向前取数据 其他全部返回
  char szQryPos[32 + 1]; // 定位串 首页查询，该字段不送值；
                         // 翻页查询送上次查询结果中最后一条记录的QRY_POS
  int iQryNum;           // 查询行数
};
...
struct MATRADEAPI CReqOptQrySettListFileField {
  int iTrdDate; // 交易日期
                // 当交易日期入参为非必输入参时，默认为前一交易日结算单，入参格式YYYYMMDD，例如20160203表示2016年2月3日的结算单
  int iTrdMonth; // 交易月份 入参格式YYYYMM，例如201602表示2016年2月的结算单
  char
      chSettListType; // 结算单类型
                      // 字典[SETT_LIST_TYPE]，结算单类型为’D’日结单时，交易日TRD_DATE入参有效。当结算单类型为’M’月结单时，交易月份TRD_MONTH入参有效。
  int iIntOrg;        // 内部机构
  LONGLONG llCustCode;   // 客户代码
  char szQryPos[32 + 1]; // 定位串
  int iQryNum;           // 查询行数
...
struct MATRADEAPI CReqOptRpcUserLoginField {
  char szAcctType[2 + 1];   // 账户类型 U:客户代码 Z:资产账户 05:深A证券账户
                            // 15:沪A证券账户
  char szAcctId[32 + 1];    // 账户标识
  char chUseScope;          // 使用范围 ‘4’:期权交易密码 ‘5’:期权资金密码
  char chAuthType;          // 认证类型 ‘0’--密码
  char szAuthData[256 + 1]; // 认证数据 明文
  char chEncryptType;       // 加密方式 加密方式: ‘0’:金证KBSS系统加密方式
                            // ‘1’:金证Win版集中交易加密方式
                      // ‘2’:金证Unix版集中交易加密方式 ‘3’:外部重加密方式
                      // ‘4’:外部加密方式
                      // 说明：该入参根据接入券商要求填写，默认值为0。
                      ...
                      // 外部加密方式’4’：
                      // 使用非金证加密函数将密码明文加密为密文后作为入参。
  char szEncryptKey[32 +
                    1]; // 加密因子 加密因子可以填写为账户标识：ACCT_ID参数值
  char chChannel;       // 操作渠道
  char szSerChannel[8 + 1]; // 服务渠道
};
...
struct MATRADEAPI CRspOptRpcUserLoginField {
  ...
  char chTrdacctStatus;      // 账户状态
  char szStkpbu[8 + 1];      // 交易单元
  char szAcctType[2 + 1]; // 登陆账户类型 U:客户代码 Z:资产账户 05:深A证券账户
                          // 15:沪A证券账户
  ...
  char chPwdMdfFlag; // 密码修改标识 0-不修改 1-修改
                     // 开户初始化密码后，首次登陆返回1；
                     // 重置密码后，首次登陆返回1； 密码修改后，登陆返回0。
...
struct MATRADEAPI CReqOptQryRpcCurrDayRiskNotifyField {
  ...
  LONGLONG
      llQryPos; // 定位串
                // 1表示首页；从第二页开始（包括第二页），此值为上一页的最后一条记录中的“定位串”字段的值加1。
  int iQryNum;  // 查询行数
};
