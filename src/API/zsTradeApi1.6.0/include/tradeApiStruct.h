...
struct STReqQryHoldingCredit {
  ...
  char
      chBizFlag; // 业务标志 '0':当前账户股份 '1':当前信用账户对应的普通账户股份
};
...
struct STReqQryContract {
  ...
  char szTrdacct[10 + 1];  // 交易账户
  char szStkCode[8 + 1];   // 证券代码
  char chContractStatus; // 合约状态 '0':开仓未归还 '1':部分归还 '2':到期未平仓
                         // '3':主动了结 '4':手工了结 '5':实时合约 '6':展期中
...
struct STRspQryContract {
  ...
  char chContractStatus; // 合约状态 '0':开仓未归还 '1':部分归还 '2':合约已过期
                         // '3':主动了结 '4':手工了结 '5':实时合约 '6':展期中
...
struct STRspQryCustDebts {
  ...
  char chCreditStatus;       // 信用状态 '0':正常 '1':未激活 '2':黑名单 '3':冻结
                             // '9':过期
...
struct STReqFundTransfer {
  ...
  char chDirect;          // 划拨方向 '0':普通节点划入、VIP 系统划出
                          // '1':普通节点划出、VIP 系统划入
};
