...
typedef enum XTP_PRICE_TYPE {
  XTP_PRICE_LIMIT = 1, ///< 限价单-沪 / 深 / 沪期权 / 深期权
                       ///< （除普通股票业务外，其余未特指的业务均使用此种类型）
...
typedef enum XTP_FUND_TRANSFER_TYPE {
  ...
  XTP_FUND_INTER_TRANSFER_OUT, ///< 跨节点转出
                               ///< 从本XTP节点1，转出到对端XTP节点2，XTP服务器之间划拨，只能“一账号两中心”跨节点用户使用
  XTP_FUND_INTER_TRANSFER_IN,  ///< 跨节点转入
                              ///< 从对端XTP节点2，转入到本XTP节点1，XTP服务器之间划拨，只能“一账号两中心”跨节点用户使用
...
typedef enum XTP_TBT_TYPE {
  ...
  XTP_TBT_STATE =
      3, ///< 逐笔状态订单，2.2.32版本新增字段，为上海新债券Level2行情中独有
} XTP_TBT_TYPE;
...
/// 行权价顺序类型，
/// 从1开始，1表示行权价最高，2次之。如果行权价相同，则填写相同数字，用A表示行权价大于等于B，B大于等于C依次类推（C、D）
