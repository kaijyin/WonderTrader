...
public:
  ...
  // 取某一商品基础数据
  virtual int GetOneStaticData(tagAClt_Instrument *pInstrument,
                               tagAClt_CommBaseData *pData) = 0;
  // 取交易所商品基础数据
  virtual int GetStaticData(tagAClt_CommBaseData *pArr, int nCount) = 0;

  // 取某一商品的补充数据(1.14+)
  virtual int GetOneSupplementData(tagAClt_Instrument *pInstrument,
                                   tagAClt_SupplementData *pData) = 0;
  // 取交易所商品补充数据(1.14+)
