...
struct stStructMsg {
  ...
  stStructMsg() { memset(this, 0, sizeof(stStructMsg)); }
};
typedef void (*pStructMessageCallbackMethod)(const char *pTime,
                                             stStructMsg &stMsg, int nType);
// 证券行情
typedef struct {
  char Market[4];        // 交易所
...
typedef struct {
  ...
  int64 SubsAndRedeState;  // 申购赎回状态，-1=无申赎资格 0=禁止申赎 1=允许申赎
                          // 2=允许申购，禁止赎回 3=禁止申购，允许赎回
...
typedef struct {
  ...
  char sBZ[4];    // 币种
  char sZZYMMXX
      [2]; // 证转银密码选项(0：都不输，1：只输资金密码，2：只输银行密码，3：两个密码都要输)
  char sYZZMMXX
      [2]; // 银转证密码选项(0：都不输，1：只输资金密码，2：只输银行密码，3：两个密码都要输)
  char sZHH[11];    // 主/辅账户号
...
struct ITPDK_CusReqInfo {
  ...
  char sKZLSH[9];   // 扩展流水号
  ITPDK_CusReqInfo()
      : AccountId{0}, SecuAccount{0}, Password{0}, TradeNodeID(-1),
        EntrustWay{0}, NodeInfo{0}, DevelCode{0}, Token(-1), RetCode(0),
        ErrMsg{0}, sKZLSH{0} {}
};
// 客户节点信息
struct ITPDK_KHJD {
