...
inline unsigned string2TimeID(const char *timeBuffer) {
  ...
  int tmp =
      (timeBuffer[0] * 36000 + timeBuffer[1] * 3600 + timeBuffer[3] * 600 +
       timeBuffer[4] * 60 + timeBuffer[6] * 10 + timeBuffer[7]);
...
inline const char *timeID2String(unsigned timeID, char *buffer) {
  ...
  sprintf(buffer, "%02d:%02d:%02d", (timeID / 3600 + START_HOUR) % 24,
          timeID / 60 % 60, timeID % 60);
  return buffer;
}
