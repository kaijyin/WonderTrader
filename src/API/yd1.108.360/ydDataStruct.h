...
class YDExchange {
public:
  YDExchangeID ExchangeID; // Short string identify the exchange, e.g. "CFFEX",
                           // "DCE", "SHFE", ...
  int ExchangeRef;         // An integer uniquely identify the exchange
  int ConnectionCount;     // Number of connections(seats) to the exchange
  int ProductRefStart;
  int ProductRefEnd;
  bool UseTodayPosition;     // Whether today and previous positions are traded
                             // separately. True for SHFE and INE
  bool UseArbitragePosition; // Besides speculation and hedge, whether the
                             // exchange maintains separate position for
                             // arbitration. True for CFFEX
  bool CloseTodayFirst;      // When "UseTodayPosition" is false, whether today
                        // opened positions are closed first. The commission may
                        // be different. True for CFFEX
  bool SingleSideMargin; // Whether margin is calculated using larger side of
                         // the product or product group. True for SHFE, INE,
                         // and CFFEX
  int OptionExecutionSupport; // 0 for not support, 1 for support without risk
                              // control, 2 for support with risk constrol
  int OptionAbandonExecutionSupport; // 0 for not support, 1 for support without
                                     // risk control, 2 for support with risk
                                     // constrol
  int QuoteVolumeRestriction; // 0 for allow single side, 1 for allow different
                              // volume, 2 for require same volume
  short FutureMarginType; // 0(default) buy and sell side margin are calculatd
                          // separately; 1 for max(buy, sell) side margin (ZCE);
  bool CombineHedgeSpecPosition; // When true, hedge position are combined into
                                 // speculation position(CZCE)
  bool TradeStockOptions;        // whether this exchange is trading stock/etf
                                 // options(SSE/SZSE)
...
class YDProduct {
public:
  YDProductID ProductID; // Short string identify the product, e.g. "cu", "IC",
                         // "cu_o", ...
  int ProductRef;
  int ExchangeRef;
  ...
  double UnderlyingMultiply; // Volume of corresponding underlying instrument.
                             // Used for options, normally equals to 1
  ...
  int SystemUse7[32];
  const YDProduct
      *m_pMarginProduct; // Points to the product that represents the product
                         // group, when single side margin is used
...
class YDInstrument {
public:
  YDInstrumentID InstrumentID; // Short string identify the instrument, e.g.
                               // "cu2101", "IC2212", "c2103-C-2620", ...
  ...
  int MarginCalcMethod; // Option margin calculation method, 1 for CFFEX, 2 for
                        // SSE & SZSE, 0 for others
  YDInstrumentID InstrumentHint; // Hint of this instrument, useful for options
                                 // of SSE/SZSE, e.g. "510050C2009M02350"
  const YDInstrument
      *m_pUnderlyingInstrument; // Points to underlying instrument, only valid
                                // for options
  const YDExchange *m_pExchange;
...
public:
  ...
  double MaxMoneyUsage; // Rate of fund that can be used for trading, should be
                        // less than 1
  double Deposit;
  double Withdraw;
...
public:
  ...
  char ConnectionSelectionType; // Refer to "Connection Selection Type" section
                                // of ydDataType.h
  union {
    double Price; // Price for limit orders
    struct {
      int CombPositionDetailID; // Set only when YDOrderFlag is
                                // YD_YOF_CombPosition, and Direction is
                                // YD_D_Split for SSE or SZSE
      int SystemUse6;
    };
  };
  int OrderVolume;
  int OrderRef;   // User defined reference of the order. Will be passed back in
                  // later order and trade notifications
  char OrderType; // Refer to "Order Type" section of ydDataType.h
  char YDOrderFlag;  // Refer to "YD OrderFlag" section of ydDataType.h
  char ConnectionID; // Target connection ID if ConnectionSelectionType is not
                     // YD_CS_Any
...
public:
  ...
  int Position;
  int PositionByOrder;
  // Yesterday position used for commission calculation only, based on each
  // exchange's commission rules.
  ...
  // DEPRECATED: This function is deprecated and will be remove in future
  // release. Use getOpenPrice() instead
  double OpenPrice(void) const { return getOpenPrice(); }
  ...
  // Return yesterday position calculated according to "first open, first close"
  // rule
...
public:
  ...
  const YDCombPositionDef *pCombPositionDef;
  const YDAccount *pAccount;
  /// if following 2 fields are specified, it should match one leg of this
