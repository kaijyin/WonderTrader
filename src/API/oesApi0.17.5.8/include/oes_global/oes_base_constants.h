 ...
 *          - 调整 ‘买卖类型’
 * 中期权标的锁定(OES_BS_TYPE_UF)、期权标的解锁(OES_BS_TYPE_UU)、
 ...
 *          - 重命名 ‘出入金方向’ 的宏定义 eOesCashDirectT =>
 * eOesFundTrsfDirectT
 ...
 *              - 调整 OES_CASH_TRSF_STS_INVALID_OES =>
 * OES_FUND_TRSF_STS_INVALID_OES
 *              - 调整 OES_CASH_TRSF_STS_INVALID_COUNTER =>
 * OES_FUND_TRSF_STS_INVALID_COUNTER
 ...
 *              - 融券卖出, 质押式逆回购 OES_BS_TYPE_CS =>
 * OES_BS_TYPE_CREDIT_SELL
 ...
 *              - 调整 OES_FUND_TRSF_STS_DCLR_ROLLBACK =>
 * OES_FUND_TRSF_STS_UNDECLARED_ROLLBACK
 *              - 调整 OES_FUND_TRSF_STS_DONE_ROLLBACK =>
 * OES_FUND_TRSF_STS_DECLARED_ROLLBACK
 ...
 *          - 调整产品风险等级 eOesSecurityRiskLevelT 中 OES_RISK_LEVEL_VERY_LOW
 * 字段含义:
 *              - 重定义 "极低风险" => "最低风险"
 ...
 * 存托凭证(OES_SUB_SECURITY_TYPE_STOCK_CDR) 子类别定义
 ...
 *              - 增加 '对手方最优价格申报 (OES_ORD_TYPE_SH_MTL_BEST)' 类型
 * (仅适用于科创板)
 *              - 增加 '本方最优价格申报 (OES_ORD_TYPE_SH_MTL_SAMEPATY_BEST)'
 * 类型 (仅适用于科创板)
 *          - 股东账户交易权限枚举(eOesTradingPermissionT)中新增
 ...
 *              - 调整 '货币基金申赎 (OES_PERMIS_CURRENCY_ETF)' 权限描述为
 * '货币ETF申赎'
 *              - 重命名 '创业板交易 (OES_PERMIS_GEM)' -> '核准制创业板交易
 * (OES_PERMIS_GEM_UNREGISTRATION)'
 ...
 *              - 重命名 质押式逆回购 (OES_BS_TYPE_CREDIT_SELL =>
 * OES_BS_TYPE_REVERSE_REPO)
 *              - 废弃 OES_BS_TYPE_CREDIT_SELL (融资买入或质押式逆回购)
 *              - 废弃 OES_BS_TYPE_CREDIT_BUY (融资买入)
 * @version 0.15.11.3   2020/06/29
 *          - 证券类别枚举(eOesSecurityTypeT) 中新增
 * 管理类(OES_SECURITY_TYPE_MGR) 类别定义
 *          - 证券子类别枚举(eOesSubSecurityTypeT) 中新增
 *              - 指定登记(OES_SUB_SECURITY_TYPE_MGR_SSE_DESIGNATION) 子类别定义
 *              - 指定撤消(OES_SUB_SECURITY_TYPE_MGR_SSE_RECALL_DESIGNATION)
 * 子类别定义
 *              - 托管注册(OES_SUB_SECURITY_TYPE_MGR_SZSE_DESIGNATION)
 * 子类别定义
 *              - 托管撤消(OES_SUB_SECURITY_TYPE_MGR_SZSE_CANCEL_DESIGNATION)
 * 子类别定义
 ...
 *              - 字段重命名 基础设施基金交易权限(OES_PERMIS_INFRASTRUCTURE_FUND
 * => OES_PERMIS_REITS)
 ...
 *          - 新增 通知消息的枚举值定义(eOesNotifySourceT, eOesNotifyTypeT,
 * eOesNotifyLevelT, eOesNotifyScopeT)
 ...
 *          - 删除交易限制枚举类型(eOesTradingLimitT), 改为使用宏定义,
 * 并新增如下交易限制
 ...
 *              - 新增 国际市场互联平台 (暂未对接)
 * (OES_PLATFORM_INTERNATIONAL_MARKET)
...
/** 批量委托接口建议的每批次最大委托数量 (@note 超过该数量将自动拆分为多次提交,
 * 可能会因为超出流量限制而导致部分委托被服务器端拒绝) */
...
/** 现货开仓相关的交易限制集合 */
#define __OES_LIMIT_OPEN_POSITION_STK                                          \
  OES_LIMIT_BUY                                                                \
  | OES_LIMIT_REPO | OES_LIMIT_REVERSE_REPO

/** 现货平仓相关的交易限制集合 */
#define __OES_LIMIT_CLOSE_POSITION_STK OES_LIMIT_SELL

/** 期权开仓相关的交易限制集合 */
#define __OES_LIMIT_OPEN_POSITION_OPT                                          \
  OES_LIMIT_BUY_OPEN                                                           \
  | OES_LIMIT_SELL_OPEN | OES_LIMIT_COVERED_OPEN | OES_LIMIT_UNDERLYING_FREEZE

/** 期权平仓相关的交易限制集合 */
#define __OES_LIMIT_CLOSE_POSITION_OPT                                         \
  OES_LIMIT_SELL_CLOSE                                                         \
  | OES_LIMIT_BUY_CLOSE | OES_LIMIT_COVERED_CLOSE |                            \
      OES_LIMIT_UNDERLYING_UNFREEZE

/** 信用开仓相关的交易限制集合 */
#define __OES_LIMIT_OPEN_POSITION_CRD                                          \
  OES_LIMIT_BUY                                                                \
  | OES_LIMIT_COLLATERAL_TRANSFER_OUT | OES_LIMIT_MARGIN_BUY |                 \
      OES_LIMIT_SHORT_SELL | OES_LIMIT_REPAY_STOCK_BY_BUY

/** 信用平仓相关的交易限制集合 */
#define __OES_LIMIT_CLOSE_POSITION_CRD                                         \
  OES_LIMIT_SELL                                                               \
  | OES_LIMIT_COLLATERAL_TRANSFER_IN | OES_LIMIT_REPAY_MARGIN_BY_SELL

/** 信用担保品划转交易限制集合 */
#define __OES_LIMIT_COLLATERAL_TRANSFER_CRD                                    \
  OES_LIMIT_COLLATERAL_TRANSFER_IN                                             \
  | OES_LIMIT_COLLATERAL_TRANSFER_OUT

/** 开仓相关的所有交易限制集合 */
#define __OES_LIMIT_OPEN_POSITION_ALL                                          \
  __OES_LIMIT_OPEN_POSITION_STK                                                \
  | __OES_LIMIT_OPEN_POSITION_OPT | __OES_LIMIT_OPEN_POSITION_CRD

/** 平仓相关的所有交易限制集合 */
#define __OES_LIMIT_CLOSE_POSITION_ALL                                         \
  __OES_LIMIT_CLOSE_POSITION_STK                                               \
  | __OES_LIMIT_CLOSE_POSITION_OPT | __OES_LIMIT_CLOSE_POSITION_CRD
/* -------------------------           */
...
typedef enum _eOesSubSecurityType {
  ...
  OES_SUB_SECURITY_TYPE_STOCK_SME =
      12, /**< 中小板股票, Small & Medium Enterprise (SME) Board */
  OES_SUB_SECURITY_TYPE_STOCK_GEM =
      13, /**< 创业板股票, Growth Enterprise Market (GEM) */
  OES_SUB_SECURITY_TYPE_STOCK_KSH = 14,  /**< 科创板股票 */
  OES_SUB_SECURITY_TYPE_STOCK_KCDR = 15, /**< 科创板存托凭证 */
  OES_SUB_SECURITY_TYPE_STOCK_CDR =
      16, /**< 存托凭证, Chinese Depository Receipt (CDR) */
  ...
  OES_SUB_SECURITY_TYPE_BOND_GBF =
      21, /**< 国债 (国债/地方债/政策性金融债/上交所政府支持债) */
  OES_SUB_SECURITY_TYPE_BOND_CBF = 22, /**< 企业债 */
  OES_SUB_SECURITY_TYPE_BOND_CPF = 23, /**< 公司债 */
  OES_SUB_SECURITY_TYPE_BOND_CCF = 24, /**< 可转换债券 */
  OES_SUB_SECURITY_TYPE_BOND_FBF =
      25, /**< 金融机构发行债券 (仅适用于深交所, 上交所已废弃) */
...
typedef enum _eOesSecurityIssueType {
  ...
  OES_ISSUE_TYPE_MKT_QUOTA =
      1,                    /**< 按市值限额申购 (检查认购限额, 不预冻结资金) */
...
typedef enum _eOesOrdStatus {
  ...
  OES_ORD_STATUS_CANCEL_DONE =
      5, /**< 撤单指令已执行 (适用于撤单请求, 并做为撤单请求的终结状态) */
  ...
  /* 深证前台废单 @deprecated 已废弃 (深交所不会返回该错误, 之前版本的取值为:
     15) */
...
typedef enum _eOesBuySellType {
  ...
  __OES_BS_TYPE_MAX_TRADING_ALIGNED8 =
      40, /**< 对外开放的交易类业务的买卖类型最大值 (按8字节对齐的大小) */
  /* -------------------------           */
...
typedef enum _eOesEtfSubFlag {
  ...
  OES_ETF_SUBFLAG_ALLOW_SUB =
      1, /**< 可以进行现金替代(先用证券, 如证券不足可用现金替代) */
  ...
  OES_ETF_SUBFLAG_HK_REFUND_SUB =
      7, /**< 港市退补现金替代 (仅适用于跨沪深港ETF产品) */
  OES_ETF_SUBFLAG_HK_MUST_SUB =
...
typedef enum _eOesExecType {
  ...
  OES_EXECTYPE_AUTO_CANCELLED =
      4,                   /**< 自动撤单 (市价委托发生自动撤单后的委托回报) */
...
typedef enum _eOesFundTrsfStatus {
  ...
  OES_FUND_TRSF_STS_SUSPENDED =
      13 /**< 挂起状态 (主柜的出入金执行状态未知, 待人工干预处理) */
} eOesFundTrsfStatusT;
...
typedef enum _eOesBusinessType {
  ...
  _OES_BUSINESS_TYPE_MAX_ALIGNED8 =
      0x08, /**< 业务范围最大值 (单一业务, 按8字节对齐的大小) */
 ...
 * 投资者分类
 *
 * A类专业投资者: 满足《证券期货投资者适当性管理办法》第八条 (一)、 (二)、 (三)
 * 点,
 *      比如证券公司、期货公司、基金管理公司、商业银行、保险公司、发行的理财产品等
 * B类专业投资者: 满足《证券期货投资者适当性管理办法》第八条 (四)、 (五) 点,
 *      可以是法人或者其他组织、自然人, 满足一定的净资产和金融资产的要求,
 * 具有相关的投资经验 C类专业投资者:
 * 满足《证券期货投资者适当性管理办法》第十一条 (一)、 (二) 点,
 *      由普通投资者主动申请转化而来, 满足一定的净资产和金融资产的要求,
 * 具有相关的投资经验
 */
...
typedef enum _eOesOptLimitOpenFlag {
  ...
  OES_OPT_LIMIT_OPEN_FLAG_LIMITED =
      1 /**< 限制卖出开仓(不包括备兑开仓)和买入开仓 */
...
typedef enum _eOesCrdDebtRepayMode {
  ...
  __OES_CRD_DEBT_REPAY_MODE_MAX_COMPACT,       /**<
                                                  融资融券合同约定的负债归还模式的最大值
                                                */

  __OES_CRD_DEBT_REPAY_MODE_INTEREST_ONLY =
      10, /**< 仅归还息费 (仅适用于API接口 @see
