 ...
 *          - 新增 '期权限仓额度查询' 相关报文定义
 *              - 新增结构体
 * 查询期权限仓额度消息过滤条件(OesQryOptPositionLimitFilterT)
 *              - 新增结构体 期权限仓额度信息内容(OesOptPositionLimitItemT)
 *              - 新增结构体
 * 查询期权限仓额度消息请求(OesQryOptPositionLimitReqT)
 *              - 新增结构体
 * 查询期权限仓额度消息应答(OesQryOptPositionLimitRspT)
 *          - 新增 '期权限购额度信息查询' 相关报文定义
 *              - 新增结构体
 * 查询期权限购额度信息过滤条件(OesQryOptPurchaseLimitFilterT)
 *              - 新增结构体 期权限购额度信息内容(OesOptPurchaseLimitItemT)
 *              - 新增结构体
 * 查询期权限购额度信息请求(OesQryOptPurchaseLimitReqT)
 *              - 新增结构体
 * 查询期权限购额度信息应答(OesQryOptPurchaseLimitRspT)
 *          - 新增 '期权行权指派查询' 相关报文定义
 *              - 新增结构体
 * 查询期权行权指派消息过滤条件(OesQryOptExerciseAssignFilterT)
 *              - 新增结构体 期权行权指派信息内容(OesOptxerciseAssignItemT)
 *              - 新增结构体
 * 查询期权行权指派消息请求(OesQryOptExerciseAssignReqT)
 *              - 新增结构体
 * 查询期权行权指派消息应答(OesQryOptExerciseAssignRspT)
...
/** 每条查询应答报文中期权产品的最大数量 @deprecated 已过时, 请使用
 * OES_MAX_OPT_OPTION_ITEM_CNT_PER_PACK */
#define OES_MAX_OPTION_ITEM_CNT_PER_PACK OES_MAX_OPT_OPTION_ITEM_CNT_PER_PACK

/** 每条查询应答报文中期权行权指派信息的最大数量 @deprecated 已过时, 请使用
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_OPTION_FILTER {0}, 0, {0}, 0
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_OPTION_ITEM NULLOBJ_OES_OPTION_BASE_INFO
/* -------------------------           */
...
#define NULLOBJ_OES_QRY_OPTION_REQ                                             \
  {NULLOBJ_OES_QRY_REQ_HEAD}, { NULLOBJ_OES_QRY_OPTION_FILTER }
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_OPT_HOLDING_FILTER {0}, {0}, {0}, 0, 0, {0}, 0
/* -------------------------           */
...
#define NULLOBJ_OES_OPT_HOLDING_ITEM                                           \
  __NULLOBJ_OES_OPT_HOLDING_BASE_INFO_PKT,                                     \
      __NULLOBJ_OES_OPT_HOLDING_RPT_INFO_PKT, {0}, {0}, 0
/* -------------------------           */
...
#define NULLOBJ_OES_QRY_OPT_HOLDING_REQ                                        \
  {NULLOBJ_OES_QRY_REQ_HEAD}, { NULLOBJ_OES_QRY_OPT_HOLDING_FILTER }
/* -------------------------           */
...
#define NULLOBJ_OES_QRY_OPT_UNDERLYING_HOLDING_REQ                             \
  {NULLOBJ_OES_QRY_REQ_HEAD}, { NULLOBJ_OES_QRY_OPT_UNDERLYING_HOLDING_FILTER }
/* -------------------------           */
...
typedef struct _OesQryOptUnderlyingHoldingRsp {
  ...
  OesQryRspHeadT rspHead;
  /** 期权标的持仓信息数组 */
  OesOptUnderlyingHoldingItemT qryItems[OES_MAX_OPT_HOLDING_ITEM_CNT_PER_PACK];
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_OPT_POSITION_LIMIT_FILTER {0}, {0}, {0}, 0, 0, {0}, 0
/* -------------------------           */
...
#define NULLOBJ_OES_QRY_OPT_POSITION_LIMIT_REQ                                 \
  {NULLOBJ_OES_QRY_REQ_HEAD}, { NULLOBJ_OES_QRY_OPT_POSITION_LIMIT_FILTER }
/* -------------------------           */
...
typedef struct _OesQryOptPositionLimitRsp {
  ...
  OesQryRspHeadT rspHead;
  /** 期权标的持仓信息数组 */
  OesOptPositionLimitItemT qryItems[OES_MAX_OPT_UNDERLYING_ITEM_CNT_PER_PACK];
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_OPT_PURCHASE_LIMIT_FILTER {0}, {0}, 0, {0}, 0
/* -------------------------           */
...
#define NULLOBJ_OES_QRY_OPT_PURCHASE_LIMIT_REQ                                 \
  {NULLOBJ_OES_QRY_REQ_HEAD}, { NULLOBJ_OES_QRY_OPT_PURCHASE_LIMIT_FILTER }
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OES_QRY_OPT_EXERCISE_ASSIGN_FILTER {0}, {0}, {0}, 0, 0, {0}, 0
/* -------------------------           */
...
#define NULLOBJ_OES_QRY_OPT_EXERCISE_ASSIGN_REQ                                \
  {NULLOBJ_OES_QRY_REQ_HEAD}, { NULLOBJ_OES_QRY_OPT_EXERCISE_ASSIGN_FILTER }
/* -------------------------           */
