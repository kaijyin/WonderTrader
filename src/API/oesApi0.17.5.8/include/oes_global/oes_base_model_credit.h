 ...
 *          - 调整 '融资融券合约负债流水的基础信息 (OesCrdDebtJournalBaseInfoT)'
 * 结构体定义
 ...
 *              - 新增 公共资金头寸可用余额 (publicCashPositionAvailableBal)
 * 字段
 * @version 0.17.1      2021/06/21
 *          - 调整 '信用资产基础信息 (OesCrdCreditAssetBaseInfoT)' 结构体定义
 *              - 重命名 证券持仓市值 (holdingMarketCap) =>
 * collateralHoldingMarketCap,
 *                @note
 * 字段取值含义也调整为包含担保品转出冻结和直接还券冻结在内的持仓总市值
 *              - 新增 融资融券其他费用 (otherCreditFee) 字段
 *          - 调整 '客户单证券融资融券负债统计基础信息
 * (OesCrdSecurityDebtStatsBaseInfoT)' 结构体定义
 *              - 调整字段定义顺序
 *              - 删除 '证券持仓市值 (holdingMarketCap)' 等市值字段
 *              - 删除 '证券名称 (securityName)' 字段
 *              - 新增市值计算使用的证券价格、证券头寸可用数量等字段
 *              - 重命名 合计待归还负债数量 (securityRepayableQty) =>
 * shortSellRepayableDebtQty
 *              - 重命名 转出冻结的持仓数量 (collateralTotalTrsfOutQty) =>
 * collateralTrsfOutQty
 *              - 重命名 直接还券冻结的持仓数量 (collateralTotalRepayDirectQty)
 * => collateralRepayDirectQty
 *          - 调整 '融资融券合约回报信息 (OesCrdDebtContractReportT)' 结构体定义
 *              - 新增 资金账户代码(cashAcctId) 字段
 *              - 重命名 合计待归还负债数量 (securityRepayableQty) =>
 * shortSellRepayableDebtQty
 *              - 重命名 合约当前待归还负债数量 (contractRepayableQty) =>
 * contractRepayableDebtQty
...
typedef struct _OesCrdCreditAssetBaseInfo {
   ...
   * - 公式: 总资产 = 现金余额(包含冻结资金在内的资金余额) +
   * 担保证券市值(不包含买入在途,包含卖出在途和转出在途) + 其它担保资产价值
   */
  int64 totalAssetValue;
  /**
   * 总负债 (不包括在途负债; 单位精确到元后四位, 即1元=10000)
   * - 公式: 总负债 = 融资已买入金额 + 融券已卖出证券市值 +
   * 利息及费用(不包含未成交部分的利息及费用) + 其它负债金额
   */
  ...
  /** 买券还券可用资金 (买券还券/买高流通性证券可用资金,
   * 即:包含融券卖出所得资金在内的可用余额; 单位精确到元后四位, 即1元=10000) */
  ...
  /** 日中累计已用于归还负债的资金总额 (卖券还款或现金归还金额;
   * 单位精确到元后四位, 即1元=10000) */
  int64 totalRepaidAmt;
  /** 日中为归还负债而在途冻结的资金总额 (卖券还款或现金归还冻结金额;
   * 单位精确到元后四位, 即1元=10000) */
  int64 repayFrzAmt;
   ...
   * - 公式: 融资买入已用授信额度 = 融资负债金额 + 融资负债交易费用 +
   * 在途融资金额 + 在途融资交易费用 + 其他负债金额
   */
  ...
  /**
   * 证券持仓总市值 (日初持仓市值+累计买入持仓-累计卖出持仓; 单位精确到元后四位,
   * 即1元=10000)
   * - 包括自有持仓和融资买入持仓
   * - 包含在途卖出冻结的持仓市值
   * - 包含转出冻结的证券持仓市值
  ...
  /** 融资融券专项头寸总费用
   * (包含融资专项头寸成本费、融券专项头寸成本费和转融通成本费;
   * 单位精确到元后四位, 即1元=10000) */
  int64 creditTotalSpecialFee;
  /** 融资专项头寸成本费 (已包含在 '融资融券专项头寸总费用' 中;
   * 单位精确到元后四位, 即1元=10000) */
  int64 marginBuySpecialFee;
  /** 融券专项头寸成本费 (已包含在 '融资融券专项头寸总费用' 中;
   * 单位精确到元后四位, 即1元=10000) */
...
#define __OES_CRD_DEBT_CONTRACT_BASE_INFO_PKT                                                   \
  ...
  int64 trdAmt;                                                                                 \
  /** 成交费用 (仅用于展示, 负债部分参见合约手续费(currentDebtFee)字段.   \
   * 单位精确到元后四位, 即1元=10000) */                                             \
  int64 trdFee;                                                                                 \
                                                                                                \
...
typedef struct _OesCrdDebtContractReport {
  ...
  union {
     ...
     * - 公式: 同一证券合计可归还负债数量 = 日初融券负债数量 -
     * 当日已归还融券数量 - 在途归还融券数量
     */
    int64 securityRepayableDebtQty;
    /** 同一证券所有融券合约的合计可归还负债数量 @deprecated 已废弃,
     * 为了兼容旧版本而暂时保留 */
  ...
  union {
    /**
     * 该融券合约的当前可归还负债数量
     * - 公式: 合约当前可归还负债数量 = 实时合约数量 -
     * 归还指定合约的在途归还数量
     * - @note 实际允许归还的负债数量,
     * 为该融券合约可归还负债数量与对应证券可归还负债数量的较小者
     */
    int32 contractRepayableDebtQty;
    /** 该融券合约的当前可归还负债数量 @deprecated 已废弃,
     * 为了兼容旧版本而暂时保留 */
...
#define NULLOBJ_OES_CRD_DEBT_CONTRACT_REPORT                                   \
  __NULLOBJ_OES_CRD_DEBT_CONTRACT_BASE_INFO_PKT, {0}, {0}, 0, {0}
/* -------------------------           */
...
typedef struct _OesCrdSecurityDebtStatsBaseInfo {
  ...
  /** 融券卖出保证金比例 (单位:万分比) */
  int32 shortSellRatio;
  /** 市值计算使用的证券价格 (等价于最新价或公允价, 取决于是否开启公允价格;
   * 价格单位精确到元后四位, 即1元=10000) */
  ...
  union {
     ...
     * - 公式: 同一证券合计待归还负债数量 = 日初融券负债数量 -
     * 当日已归还融券数量 - 在途归还融券数量
     */
    int64 shortSellRepayableDebtQty;
    /** 同一证券所有融券合约的合计待归还负债数量 @deprecated 已废弃,
     * 为了兼容旧版本而暂时保留 */
    int64 securityRepayableQty;
  };
  ...
  /** 当日已归还融券金额 (按开仓价格计算的理论上已归还融券金额;
   * 单位精确到元后四位, 即1元=10000) */
  int64 shortSellRepaidAmt;
  /** 当日实际归还融券金额 (按成交价计算的当日实际归还融券金额;
   * 单位精确到元后四位, 即1元=10000) */
