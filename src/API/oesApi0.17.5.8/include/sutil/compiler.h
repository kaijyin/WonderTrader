 ...
 */
#ifndef offsetof
#define offsetof(TYPE, MEMBER) __spk_offsetof(TYPE, MEMBER)
#endif

#ifndef container_of
#define container_of(PTR, TYPE, MEMBER) __spk_container_of(PTR, TYPE, MEMBER)
#endif

#define __spk_offsetof(TYPE, MEMBER) ((int32) & ((TYPE *)0)->MEMBER)

#define __spk_container_of(PTR, TYPE, MEMBER)                                  \
  ((TYPE *)((char *)(PTR) - offsetof(TYPE, MEMBER)))

#define __spk_sizeof_submember(TYPE, MEMBER) (sizeof(((TYPE *)0)->MEMBER))
/* -------------------------           */
...
#define SPK_CEIL_TIMES(SIZE, BASESIZE) (((SIZE) + (BASESIZE) - 1) / (BASESIZE))
...
#define SPK_FLOOR_TIMES(SIZE, BASESIZE) ((SIZE) / (BASESIZE))
...
#define SPK_SIGNED_ROUND_TIMES(SIZE, BASESIZE)                                 \
  (((SIZE) + (((SIZE) >= 0 ? (BASESIZE) : -(BASESIZE)) >> 1)) / (BASESIZE))
/* -------------------------           */
...
#define __SPK_SWAP_INT16(x)                                                    \
  (((((uint16)x) >> 8) & 0x00FF) | ((((uint16)x) << 8) & 0xFF00))

/* Swap 4 byte, 32 bit values */
#define __SPK_SWAP_INT32(x)                                                    \
  (((((uint32)x) >> 24) & 0x000000FF) | ((((uint32)x) >> 8) & 0x0000FF00) |    \
   ((((uint32)x) << 8) & 0x00FF0000) | ((((uint32)x) << 24) & 0xFF000000))
...
#define SPK_SLEEP_YIELD() Sleep(0)
...
#elif defined(__WINDOWS__) || defined(__MINGW__)
#define SPK_SLEEP_MS(MS) Sleep((MS))
...
#define SPK_SLEEP_MS(MS)                                                       \
  do {                                                                         \
    ...
    select(0, (fd_set *)NULL, (fd_set *)NULL, (fd_set *)NULL, &__SLEEP_MS_tv); \
  } while (0)
...
#define SPK_SLEEP_US(US)                                                       \
  do {                                                                         \
    ...
    select(0, (fd_set *)NULL, (fd_set *)NULL, (fd_set *)NULL, &__SLEEP_US_tv); \
  } while (0)
...
#define SPK_SLEEP_NS(NS)                                                       \
  do {                                                                         \
    ...
    select(0, (fd_set *)NULL, (fd_set *)NULL, (fd_set *)NULL, &__SLEEP_NS_tv); \
  } while (0)
...
#define SPK_TIMED_SLEEP(MS)                                                    \
  do {                                                                         \
    ...
    if (__TIMED_SLEEP_msec > 999) {                                            \
      ...
      __TIMED_SLEEP_tsReq.tv_nsec = (__TIMED_SLEEP_msec % 1000) * 1000000;     \
    } else {                                                                   \
...
#define SPK_TIMED_SLEEP_SINCE(MS, P_SINCE_TV)                                  \
  do {                                                                         \
    ...
    if (__TIMED_SINCE_msec > 999) {                                            \
      __TIMED_SINCE_abstime.tv_sec =                                           \
          (P_SINCE_TV)->tv_sec + (__TIMED_SINCE_msec / 1000);                  \
      ...
      __TIMED_SINCE_abstime.tv_nsec =                                          \
          (P_SINCE_TV)->tv_usec * 1000 + __TIMED_SINCE_msec * 1000000;         \
    }                                                                          \
    ...
    while (clock_nanosleep(CLOCK_REALTIME, TIMER_ABSTIME,                      \
                           &__TIMED_SINCE_abstime,                             \
                           (struct timespec *)NULL) == EINTR)                  \
      ;                                                                        \
  } while (0)
