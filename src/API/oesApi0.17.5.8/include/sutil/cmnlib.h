...
#define SPK_UNSIGNED_DOUBLE_TO_INT64_PRECISELY(d, realPrecision,               \
                                               finalPrecision)                 \
...
#define SPK_SIGNED_FLOAT_TO_INT32(f, multiplier)                               \
  ((int32)((f) * (multiplier) + ((f) >= 0.0F ? SPK_FLOAT_ROUNDING_WINDAGE      \
                                             : -SPK_FLOAT_ROUNDING_WINDAGE)))

#define SPK_SIGNED_FLOAT_TO_INT64(f, multiplier)                               \
  ((int64)((f) * (multiplier) + ((f) >= 0.0F ? SPK_FLOAT_ROUNDING_WINDAGE      \
                                             : -SPK_FLOAT_ROUNDING_WINDAGE)))

#define SPK_SIGNED_DOUBLE_TO_INT32(d, multiplier)                              \
  ((int32)((d) * (multiplier) + ((d) >= 0.0 ? SPK_DOUBLE_ROUNDING_WINDAGE      \
                                            : -SPK_DOUBLE_ROUNDING_WINDAGE)))

/* @note 仅适用于取值确定不会超过浮点数精度范围的场景 */
#define SPK_SIGNED_DOUBLE_TO_INT64(d, multiplier)                              \
  ((int64)((d) * (multiplier) + ((d) >= 0.0 ? SPK_DOUBLE_ROUNDING_WINDAGE      \
                                            : -SPK_DOUBLE_ROUNDING_WINDAGE)))

#define SPK_SIGNED_DOUBLE_TO_INT64_PRECISELY(d, realPrecision, finalPrecision) \
  (((int64)((d) * (realPrecision) + ((d) >= 0.0                                \
                                         ? SPK_DOUBLE_ROUNDING_WINDAGE         \
                                         : -SPK_DOUBLE_ROUNDING_WINDAGE))) *   \
   (finalPrecision / realPrecision))
...
#define SPK_SIGNED_TO_DOUBLE(i, multiplier)                                    \
  (((double)(i) / (multiplier)) +                                              \
   ((i) >= 0 ? SPK_DOUBLE_WINDAGE : -SPK_DOUBLE_WINDAGE))
/* -------------------------           */
...
#else
#define SPK_TOUPPER(c) (SPK_ISLOWER(c) ? 'A' + ((c) - 'a') : (c))
#endif
...
#else
#define SPK_TOLOWER(c) (SPK_ISUPPER(c) ? 'a' + ((c) - 'A') : (c))
#endif
/* -------------------------           */
...
#define SPK_SWITCH_NULL_STR(s) ((void *)(s) == (void *)0 ? "NULL" : (s))
