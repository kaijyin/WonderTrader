 ...
 *          - 添加新的消息标志:
 *              - SMSG_MSGFLAG_NESTED       嵌套的组合消息
 * (消息体由一到多条包含消息头的完整消息组成)
...
typedef enum _eSMsgFlag {
  ...
  SMSG_MSGFLAG_NESTED = 0x20, /**< 消息标志-嵌套的组合消息
                                 (消息体由一到多条包含消息头的完整消息组成) */
...
typedef struct _SMsgHead {
  ...
  uint8 msgId;   /**< 消息代码 */
  uint8 status;  /**< 状态码 */
  uint8 detailStatus; /**< 明细状态代码 (@note 当消息为嵌套的组合消息时,
                         复用该字段记录消息体中的消息条数) */
  int32 msgSize; /**< 消息大小 */
} SMsgHeadT;
...
/* 结构体初始化值定义 */
#define NULLOBJ_SPK_MSG_HEAD 0, 0, 0, 0, 0
/* -------------------------           */
