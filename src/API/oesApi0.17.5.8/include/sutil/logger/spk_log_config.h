...
typedef struct _SLogCfgItem {
   ...
   * - 如果配置值小于 2048 则以兆为单位计算, 否则以字节为单位计算,
   * 最大文件长度为2GB
   */
  int32 maxFileLength;
  /** 日志文件最大备份数 */
...
/* 结构体初始化值定义 */
#define NULLOBJ_LOG_CFG_ITEM {0}, {0}, {0}, {0}, {0}, 0, 0, 0, 0
/* -------------------------           */
 ...
 * 尝试加载所有的日志配置区段
 */
int32 SLogCfg_LoadAllConfig(const char *pConfigFile,
...
int32 SLogCfg_LoadAllConfigAndCategoryList(
    const char *pConfigFile, const char *pRootSection,
    const char *pRootCategoryField, SLogCfgItemT *pOutLogConfigList,
    int32 *pLogConfigCount, SLogCfgSectionInfoT *pOutLogSectionList,
    int32 *pLogSectionCount, const SLogLevelT **ppOutAllowableMinLogLevel);
/* -------------------------           */
