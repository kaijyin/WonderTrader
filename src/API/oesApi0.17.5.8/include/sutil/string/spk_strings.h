 ...
 * 构造被前、后缀包裹的字符串
 */
char *SStr_BuildWrappedString(char *pBuf, const char *pTarget, int32 targetLen,
                              const char *pPrefix, const char *pPostfix);
 ...
 */
static __inline BOOL SStr_IsAnyChar(const char ch, const char *accept) {
  SLOG_ASSERT(accept);
  while (*accept) {
    if (*accept++ == ch) {
      return TRUE;
 ...
 * check a char is any of a set of characters
 */
static __inline BOOL SStr_IsAnyChar2(const char ch, const char *accept,
                                     int len) {
  SLOG_ASSERT(accept);
  while (*accept && len--) {
    if (*accept++ == ch) {
 ...
 * @see     strpbrk()
 */
static __inline char *SStr_Strpbrk(const char *s, const char *accept) {
  if (__spk_unlikely(!s)) {
    return (char *)NULL;
  }
 ...
 *          characters in accept, or NULL if no such character is found.
 */
static __inline char *SStr_StrpbrkReverse(const char *s, const char *accept) {
  int32 endPos = 0;
 ...
 */
static __inline char *SStr_Strsep2(char **ppString, char **ppTokenEnd,
                                   const char *pDelim) {
 ...
 * @see     strsep()
 */
static __inline char *SStr_Strsep(char **stringp, const char *delim) {
 ...
 */
static __inline BOOL SStr_IsEmpty(const char *pStr) {
  return (!(pStr && *pStr));
}
 ...
 * @return  TRUE, 为空; FALSE, 不为空
 */
static __inline BOOL SStr_IsBlank(const char *pStr) {
  if (pStr) {
    while (*pStr) {
      if (!SPK_ISSPACE(*pStr)) {
        return FALSE;
 ...
 */
static __inline BOOL SStr_IsVisibleAsciiString(const char *pStr) {
 ...
 * @return  TRUE, 有效; FALSE, 无效
 */
static __inline BOOL SStr_IsValidMd5String(const char *pStr) {
 ...
 * @return  TRUE, 有效; FALSE, 无效
 */
static __inline BOOL SStr_IsValidPasswdString(const char *pStr) {
  if (__spk_unlikely(SStr_IsEmpty(pStr))) {
    return FALSE;
  }

  while (*pStr) {
    if (__spk_unlikely(!SPK_ISALNUM(*pStr) && !SStr_IsAnyChar(*pStr, "@._-"))) {
      return FALSE;
    }
    pStr++;
  }
 ...
 */
static __inline int32 SStr_CheckPasswdStrength(const char *pStr, int32 strength,
                                               int32 minLength) {
  ...
  strengthLevel = (uppercaseCnt > 0 ? 1 : 0) + (lowercaseCnt > 0 ? 1 : 0) +
                  (digitCnt > 0 ? 1 : 0) + (symbolCnt > 0 ? 1 : 0);
  if (__spk_unlikely((minLength > 0 && totalLength < minLength) ||
                     strengthLevel <
                         SPK_MIN(strength, _STRONGEST_PASSWD_STRENGTH))) {
    return EINVAL;
  }
 ...
 * 若字符串为空则返回指定的值
 */
static __inline const char *SStr_SwitchBlankStr(const char *s, const char *v) {
 ...
 */
static __inline const char *SStr_Ltrim(const char *pStr) {
 ...
 */
static __inline const char *SStr_LtrimZero(const char *pStr) {
  char ch = 0;
  char prev = 0;
 ...
 * @return  去除空格后的字符串
 */
static __inline char *SStr_RtrimRude(char *pStr) {
  int32 endPos = 0;
 ...
 * @return  去除空格后的字符串
 */
static __inline char *SStr_TrimRude(char *pStr) {
 ...
 * 返回转换为大写后的字符串
 */
static __inline char *SStr_ToUpper(char *pStr) {
  char *pPtr = pStr;
 ...
 * 返回转换为大写后的字符串
 */
static __inline char *SStr_ToUpperR(char *pBuf, const char *pStr) {
  int32 j = 0;
 ...
 * 返回转换为小写后的字符串
 */
static __inline char *SStr_ToLower(char *pStr) {
  char *pPtr = pStr;
 ...
 * 返回转换为小写后的字符串
 */
static __inline char *SStr_ToLowerR(char *pBuf, const char *pStr) {
  char *pPtr = pBuf;
  int32 j = 0;
 ...
 * @return      返回拷贝后字符串
 */
static __inline char *SStr_StrCopy2(char *pBuf, const char *pStr,
                                    int32 maxStrLen, int32 *pOutCopyLen) {
  char *pPtr = pBuf;
 ...
 * @return      返回拷贝后字符串
 */
static __inline char *SStr_StrCopy(char *pBuf, const char *pStr,
                                   int32 maxStrLen) {
 ...
 * <p>需保证缓存有足够的空间(maxStrlen + 1)</p>
 *
 * @param   strLen      有效字符长度(不包括结尾的'\0')，即buf的长度需至少为
 * strLen + 1
 */
static __inline char *SStr_StrFixedLengthCopy(char *pBuf, const char *pStr,
                                              int32 strLen) {
  SLOG_ASSERT(pBuf);
 ...
 * @param[out]  pBuf        用于输出数据的缓存区指针
 * @param       pStr        待拷贝的字符串
 * @param       from        子字符串的开始位置 (从0开始计数,
 * 小于等于0则从头开始复制)
 * @param       subLen      子字符串的长度
 * (小于等于0表示从末尾开始计数的子字符串结束位置)
 *                          - 大于0, 子字符串的长度
 *                          - 等于0, 复制从 from 至末尾的所有字符
 *                          - 小于0, 末尾开始计数的子字符串结束位置
 * @return      返回复制到字符串
 */
static __inline char *SStr_SubStrCopy(char *pBuf, const char *pStr, int32 from,
                                      int32 subLen) {
  int32 totalLen = 0;
 ...
 *                      为 maxStrlen + 1
 */
static __inline char *SStr_StrCat(char *pBuf, const char *s1, const char *s2,
                                  int32 maxStrlen) {
  char *pPtr = pBuf;
 ...
 * @return 原字符串地址
 */
static __inline char *SStr_StrCatP(char **ppString, const char *s,
                                   int32 *pMaxStrlen) {
  char *pBegin = (char *)NULL;
  char *pEnd = (char *)NULL;
  int32 maxStrlen = -1;
 ...
 *                      为 maxStrlen + 1
 */
static __inline char *SStr_StrCatChStr(char *pBuf, char ch, const char *pStr,
                                       int32 maxStrlen) {
  char *pPtr = pBuf;
 ...
 *                      为 maxStrlen + 1
 */
static __inline char *SStr_StrTrimCopy(char *pBuf, const char *pStr,
                                       int32 maxStrlen) {
  char *pPtr = pBuf;
 ...
 *                      为 maxStrlen + 1
 */
static __inline char *SStr_StrFixedLengthTrimCopy(char *pBuf, const char *pStr,
                                                  int32 maxStrlen) {
  char *pPtr = pBuf;
 ...
 *                      为 alignedLen + 1
 */
static __inline char *SStr_StrRightAlignmentCopy(char *pBuf, const char *pStr,
                                                 int32 alignedLen,
                                                 char leftFiller) {
 ...
 * 跳过空字符
 */
static __inline char *SStr_SkipSpaces(char **ppStr) {
  SLOG_ASSERT(ppStr);
 ...
 * 从字符串末端反向跳过空字符
 */
static __inline void SStr_SkipSpacesReverse(char **ppEnd, char *pBegin) {
  char *pOriginEnd;
 ...
 */
static __inline int32 SStr_SkipString(char **ppStr, const char *pSkipKey) {
 ...
 * @return  已替换的字符数量
 */
static __inline int32 SStr_ReplaceInvisibleChar(char *pStr,
 ...
 * @return  已替换的字符数量
 */
static __inline int32 SStr_ReplaceChar(char *pStr, const char c1, const char c2,
                                       int32 n) {
  int32 count = 0;
 ...
 * @return  已替换的字符数量
 */
static __inline int32 SStr_ReplaceCharReverse(char *pStr, const char c1,
                                              const char c2, int32 n) {
  int32 endPos = 0;
  int32 count = 0;
 ...
 */
static __inline const char *SStr_GetParentSection(const char *pStr,
                                                  const char *pDelimChars,
 ...
 */
static __inline const char *SStr_GetLastSection(const char *pStr,
                                                const char *pDelimChars) {
  const char *pPtr = pStr;
  const char *pBase = pStr;
