 ...
 * - ${var:=string} Use var if set, otherwise use string and assign string to
 * var
...
/* 通过变量名称返回字符串变量定义 */
const SStrVarT *SStr_GetVarItemByName(const SStrVarT *pVarItems,
                                      int32 varItemsNumber,
                                      const char *pVarName);

/* 通过变量名称返回字符串变量的值 */
const char *SStr_GetVarValueByName(const SStrVarT *pVarItems,
                                   int32 varItemsNumber, const char *pVarName,
                                   BOOL isSystemEnvAble);

/* 设置字符串变量的值 */
void SStr_SetVarValueByName(const SStrVarT *pVarItems, int32 varItemsNumber,
                            const char *pVarName, const char *pVarValue,
                            BOOL isSystemEnvAble);

/* 替换所有的字符串变量 */
char *SStr_ReplaceAllVar(char *pBuf, int32 maxStrlen, const char *pTarget);

/* 替换所有的字符串变量 */
char *SStr_ReplaceAllVar2(char *pBuf, int32 maxStrlen, const char *pTarget,
                          const SStrVarT *pVarItems, int32 varItemsNumber);

/* 替换所有的字符串变量 */
char *SStr_ReplaceAllVar3(char *pBuf, int32 maxStrlen, const char *pTarget,
                          const SStrVarT *pVarItems, int32 varItemsNumber,
                          BOOL isSystemEnvAble);
/* -------------------------           */
