...
#if defined(__LINUX__) || defined(__CYGWIN__) || defined(__HPUX__) ||          \
    defined(__AIX__) || defined(__SOLARIS__)
#include <features.h>
#endif
/* -------------------------           */
...
/* 因为 MinGW 下原生的 timespec 结构不是完全按照64位对齐的,
 * 所以也需要使用自定义的结构 */
#define STimespecT _SPK_STRUCT_TIMESPEC

#if defined(__MINGW__)
/* 为了兼容 MinGW 下的 pthread 库而定义的, 原生 struct timespec
 * 结构的跨平台兼容性定义 */
...
/* CPU缓存行填充字段的初始值定义 */
#define __NULLOBJ_SPK_CACHE_LINE_PADDING 0, 0, 0, 0, 0, 0, 0,

#define __NULLOBJ_SPK_CACHE_LINE_PADDING2 , 0, 0, 0, 0, 0, 0, 0
