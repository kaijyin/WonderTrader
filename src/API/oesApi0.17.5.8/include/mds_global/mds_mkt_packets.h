 ...
 *              - 重定义 MDS_TICK_TYPE_LATEST_ONLY ->
 * MDS_TICK_TYPE_LATEST_SIMPLIFIED
 *                  - 只订阅最新的行情快照数据, 并忽略和跳过已经过时的数据
 * (推送的数据量最小, 不会重复发送最新快照)
 *              - 重定义 MDS_TICK_TYPE_ALL_TICK -> MDS_TICK_TYPE_ALL_INCREMENTS
 *                  - 订阅所有时点的行情快照数据 (需要注意的是,
 * 在该模式下将会额外发送Level2行情快照的增量更新消息)
 *              - 新增 MDS_TICK_TYPE_LATEST_TIMELY
 *                  - 只订阅最新的行情快照数据, 并立即发送最新数据
 * (会更及时的获取到交易活跃的最新行情, 但会重复发送最新快照)
 ...
 *              - 开始时间 (-1: 从头开始获取, 0: 从最新位置开始获取实时行情, >0:
 * 从指定的起始时间开始获取)
 *              - 在推送实时行情数据之前, 是否需要推送已订阅产品的初始的行情快照
 *          - 调整了行情订阅的应答消息, 将返回各市场的实际订阅结果,
 * 即实际已订阅的产品数量
 * @version 0.15.5.1    2017/11/15
 *          - 增加数值类型的协议版本号 MDS_APPL_VER_VALUE, 以方便比对版本
 * @version 0.15.5.9    2018/05/03
 *          - 增加仅内部使用的消息类型 MDS_MSGTYPE_COMPRESSED_PACKETS
 * (压缩的数据包)
 * @version 0.15.5.10   2018/05/24
 *          - 行情订阅条件和订阅配置中增加 '待订阅的内部频道号',
 * 该字段仅内部使用, 前端不需要关心和处理 (兼容之前版本)
 *          - 行情订阅条件和订阅配置中增加 '逐笔数据的过期时间类型
 * tickExpireType' (兼容之前版本)
 *          - 协议版本号升级 => 0.15.5.10 (兼容之前版本)
 * @version 0.15.5.11   2018/06/01
 *          - 协议版本号升级 => 0.15.5.11。协议本身与之前的版本不兼容,
 * 但将通过如下方式兼容之前的API:
 *            - 如果使用的是旧版本的API, 那么服务器端将不再推送增量更新消息
 * (只推送全量快照), 以此来保持兼容
 *            - 如果需要使用增量更新消息的话, 就需要更新到最新版本的API,
 * 否则不需要更新API
 * @version 0.15.5.16   2018/08/23
 *          - 增加可订阅的数据种类 (DataType), 以支持单独订阅指数行情和期权行情
 *              - MDS_SUB_DATA_TYPE_INDEX_SNAPSHOT, 与L1_SNAPSHOT的区别在于,
 * INDEX_SNAPSHOT可以单独订阅指数行情
 *              - MDS_SUB_DATA_TYPE_OPTION_SNAPSHOT, 与L1_SNAPSHOT的区别在于,
 * OPTION_SNAPSHOT可以单独订阅期权行情
 ...
 *          - 行情订阅条件和订阅配置中增加 '逐笔数据的数据重建标识
 * tickRebuildFlag', 用于 标识是否订阅重建到的逐笔数据 (@see
 * eMdsSubscribedTickRebuildFlagT)
 *          - 扩展行情订阅条件中的订阅模式, 增加批量订阅模式,
 * 以支持更灵活可控的行情订阅 (@see eMdsSubscribeModeT)
 * @version 0.15.10.6   2020/04/19
 *          - 将延迟统计相关的时间戳字段升级为纳秒级时间戳 (内部使用的字段,
 * 协议保持兼容, STimeval32T => STimespec32T)
 * @version 0.15.12     2021/03/19
 *          - 增加行情消息类型: Level2 上交所逐笔委托行情
 * (MDS_MSGTYPE_L2_SSE_ORDER, 仅适用于上交所)
 *          - 增加可订阅的数据种类: 上交所逐笔委托
 * (MDS_SUB_DATA_TYPE_L2_SSE_ORDER, 仅适用于上交所)
 *          - 协议版本号升级 => 0.15.12, 协议发生了变化, 不过兼容之前的API
 *          - @note 兼容性相关的注意事项如下:
 *              - 如果使用的是旧版本的API,
 * 那么服务器端将不再推送上交所Level2快照的增量更新 消息,
 * 将只推送全量快照。如果需要使用增量更新消息, 就需要升级到最新版本
 *              - 逐笔成交和逐笔委托结构体中有两个内部使用的字段发生变化:
 *                  - 调整内部字段 '内部频道号 (__channelNo)' 的定义顺序,
 * 如果使用了该字 段就需要升级到最新版本
 *                  - 删除内部字段 '对应的原始行情的序列号 (__origTickSeq)',
 * 该字段没有 业务含义通常不会被使用到, 如果使用了该字段, 需要修改或删除相关代码
 *
 * @since   2016/01/03
 */
...
/** 当前采用的协议版本号 */
#define MDS_APPL_VER_ID "0.17.5.8"
 ...
 */
#define MDS_APPL_VER_VALUE 1001705081
...
typedef enum _eMdsMsgType {
  ...
  /** 查询证券(股票/债券/基金)静态信息 (88/0x58)  (0x55的更新版本, @since
     0.15.11) */
 ...
 * - 10: (BatchBegin)   批量订阅-开始订阅, 开始一轮新的批量订阅
 * (之前的订阅参数将被清空, 行情推送也将暂停直到批量订阅结束)
 * - 11: (BatchAppend)  批量订阅-追加订阅, 增加订阅列表中的股票
 * - 12: (BatchDelete)  批量订阅-删除订阅, 删除订阅列表中的股票
 * - 13: (BatchEnd)     批量订阅-结束订阅, 结束本轮的批量订阅,
 * 提交和启用本轮的订阅参数
 *
 * @note 当订阅模式为 Append/Delete/BatchDelete 时将忽略 isRequireInitialMktData
 * 和 beginTime 这两个订阅参数
 */
 ...
 * -  1: (All) 订阅该市场和证券类型下的所有产品行情 (为兼容之前的版本,
 * 也可以赋值为 -1)
 ...
 * 用于标识订阅最新的行情快照还是所有时点的行情快照)
 *
 * 取值说明:
 * -  0: (LatestSimplified) 只订阅最新的行情快照数据, 并忽略和跳过已经过时的数据
 *       - 该模式推送的数据量最小, 服务器端会做去重处理,
 * 不会再推送重复的和已经过时的快照数据
 *       - 优点: 该模式在时延和带宽方面都更加优秀,
 * 该模式优先关注快照行情的时效性, 并避免推送 没有实质变化的重复快照
 *       - 缺点: 当没有行情变化时 (例如没有交易或盘中休市等),
 * 就不会推送任何快照行情了, 这一 点可能会带来困扰,
 * 不好确定是发生丢包了还是没有行情导致的
 *       - 注意: 过时和重复的快照都会被过滤掉
 * -  1: (LatestTimely) 只订阅最新的行情快照数据, 并立即发送最新数据
 *       - 只要有行情更新事件, 便立即推送该产品的最新行情,
 * 但行情数据本身可能是重复的, 即只有 行情时间有变化, 行情数据本身没有变化
 *       - 优点: 可以获取到时间点更完整的快照行情,
 * 不会因为行情数据没有变化而跳过 (如果是因为 接收慢等原因导致快照已经过时了,
 * 该快照还是会被忽略掉)
 *       - 缺点: 会收到仅更新时间有变化, 但行情数据本身并没有更新的重复数据,
 * 带宽和数据量上会 有一定的浪费
 *       - 注意: 重复的快照不会被过滤掉, 但过时的快照还是会被过滤掉
 * -  2: (AllIncrements) 订阅所有时点的行情快照数据 (包括Level2增量更新消息)
 *       - 该模式会推送所有时点的行情数据, 包括Level2行情快照的增量更新消息
 *       - 如果需要获取全量的行情明细, 或者需要直接使用Level2的增量更新消息,
 * 可以使用该模式
 *       - 如果没有特别需求的话, 不需要订阅增量更新消息,
 * 增量消息处理起来比较麻烦
 *          - 增量更新消息 (字段级别的增量更新) 只有上交所Level-2快照有,
 * 深交所行情里面没有
 *          - 在对下游系统进行推送时,
 * 增量快照和完整快照在推送时间上是没有区别的, 增量更新并 不会比完整快照更快,
 * 只是信息角度不一样
 *
 * 补充说明:
 * - 当以 tickType=0 的模式订阅行情时, 服务器端会对重复的快照行情做去重处理,
 * 不会再推送重复 的和已经过时的快照数据。
 * - 如果需要获取到"所有时点"的快照, 可以使用 tickType=1
 * 的模式订阅行情。此模式下只要行情时 间有变化,
 * 即使数据重复也会对下游推送。但过时的快照还是会被忽略掉。
 * - 只有通过 tickType=2 的模式才能接收到完整的所有时间点的行情数据。
 * - 快照行情 "过时" 表示: 不是当前最新的快照即为"过时",
 * 即存在时间点比该快照更新的快照 (同一 只股票)。
 * - @note  上交所行情存在更新时间相同但数据不同的Level-2快照。(频率不高,
 * 但会存在这样的数据)
 */
typedef enum _eMdsSubscribedTickType {
  /** 只订阅最新的行情快照数据, 并忽略和跳过已经过时的数据 (推送的数据量最小,
     服务器端会做去重处理, 不会再推送重复的和已经过时的快照数据) */
  MDS_TICK_TYPE_LATEST_SIMPLIFIED = 0,

  /** 只订阅最新的行情快照数据, 并立即发送最新数据
     (可以获取到时间点更完整的快照行情, 只要行情时间有变化,
     即使数据重复也会对下游推送) */
...
typedef enum _eMdsSubscribedTickExpireType {
  ...
  /** 立即过期 (1秒, 如果逐笔数据时间落后于最新快照时间1秒,
     则视为过期并跳过该逐笔数据) */
  MDS_TICK_EXPIRE_TYPE_IMMEDIATE = 1,

  /** 及时过期 (3秒, 如果逐笔数据时间落后于最新快照时间3秒,
     则视为过期并跳过该逐笔数据) */
  MDS_TICK_EXPIRE_TYPE_TIMELY = 2,

  /** 超时过期 (30秒, 如果逐笔数据时间落后于最新快照时间30秒,
     则视为过期并跳过该逐笔数据) */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_MKT_DATA_REQUEST_ENTRY 0, 0, {0}, 0
/* -------------------------           */
 ...
 *
 * - 对于可同时订阅产品数量有如下限制:
 *   - 每个订阅请求中最多能同时指定 4000 只产品,
 * 可以通过追加订阅的方式订阅更多数量的产品
...
typedef struct _MdsMktDataRequestReq {
   ...
   * -  0: (LatestSimplified) 只订阅最新的行情快照数据,
   * 并忽略和跳过已经过时的数据
   *       - 该模式推送的数据量最小, 没有重复数据, 也不会重复发送最新快照
   *       - 该模式在时延和带宽方面都相对优秀, 如果没有特殊需求,
   * 推荐使用该模式即可
   * -  1: (LatestTimely) 只订阅最新的行情快照数据, 并立即发送最新数据
   *       - 只要有行情更新事件, 便立即推送该产品的最新行情,
   * 但也会因此重复发送多次相同的最新行情
   *       - 如果某些产品的交易很活跃, 而客户端处理又比较耗时的话,
   * 那么该模式可能会更及时的获取到 这些产品的最新行情
   *       - 此外, 因为与 AllIncrements 模式下的数据一一对应,
   * 可以方便与增量更新消息进行比对测试
   ...
   * -  0: 不需要推送初始的行情快照
   * -  1: 需要推送初始的行情快照,
   * 即确保客户端可以至少收到一幅已订阅产品的快照行情 (如果有的话)
   *
   * @note    从 0.15.9.1 开始, 允许在会话过程中任意时间指定
   * isRequireInitialMktData 标志来订阅初始快照。不过频繁订阅初始行情快照,
   * 会对当前客户端的行情获取速度产 生不利影响。应谨慎使用, 避免频繁订阅
   ...
   * -  2: 只订阅重建到的逐笔数据 (仅重建数据 @note
   * 需要通过压缩行情端口进行订阅, 非压缩行情端口不支持该选项)
   *
   ...
   *
   * @note    从 0.15.9.1 开始, 允许在会话过程中任意时间指定 beginTime
   * 订阅参数。不过 频繁指定起始时间,
   * 会对当前客户端的行情获取速度产生不利影响。应谨慎使用, 避免 频繁订阅
   * @note    当订阅模式为 Append/Delete/BatchDelete 时将忽略
   *          isRequireInitialMktData、beginTime 这两个订阅参数
   */
  int32 beginTime;
   ...
   * - 该字段表示后续报文为subSecurityCnt个订阅产品条目结构体,
   * 通过这样的方式可以实现同 时订阅多只产品的行情快照
   * - 每个订阅请求中最多能同时指定 4000 只产品,
   * 可以通过追加订阅的方式订阅更多数量的产品
   * - 订阅产品总数量的限制如下:
...
typedef struct _MdsMktDataRequestRsp {
   ...
   * @note    从 0.15.9.1 开始, 允许在会话过程中任意时间指定 beginTime
   * 订阅参数。不过 频繁指定起始时间,
   * 会对当前客户端的行情获取速度产生不利影响。所以应谨慎使用, 避免频繁订阅
   * @note    当订阅模式为 Append/Delete/BatchDelete 时将忽略
   *          isRequireInitialMktData、beginTime 这两个订阅参数
   */
  int32 beginTime;
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_TEST_REQUEST_REQ {0}, {0}, {0}
/* -------------------------           */
...
/* 结构体初始化值的尾部填充字段定义 */
#ifdef _MDS_ENABLE_LATENCY_STATS
#define __NULLOBJ_MDS_TEST_REQUEST_RSP_TAILER , {0, 0}, {0, 0}, {0, 0}
#else
...
#define NULLOBJ_MDS_TEST_REQUEST_RSP                                           \
  {0}, {0}, {0}, {0}, {0} __NULLOBJ_MDS_TEST_REQUEST_RSP_TAILER
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_CHANGE_PASSWORD_REQ 0, 0, {0}, {0}, {0}, {0}
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_CHANGE_PASSWORD_RSP 0, 0, {0}, {0}, 0, 0, 0, 0
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_MKT_REQ_MSG_BODY {NULLOBJ_MDS_MKT_DATA_REQUEST_REQ_BUF}
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_MKT_RSP_MSG_BODY {NULLOBJ_MDS_MKT_DATA_REQUEST_RSP}
/* -------------------------           */
