...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_MKT_DATA_SNAPSHOT_REQ 0, 0, {0}, 0
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_TRD_SESSION_STATUS_REQ 0, 0, {0}
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_REQ_HEAD 0, 0
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_RSP_HEAD 0, 0, 0, {0}, 0
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_CURSOR 0, 0, {0}, 0
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_SECURITY_CODE_ENTRY 0, 0, 0, {0}
/* -------------------------           */
...
typedef struct _MdsQryStockStaticInfoReq {
  ...
  /** 查询请求的过滤条件 */
  MdsQryStockStaticInfoFilterT qryFilter;
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_STOCK_STATIC_INFO_FILTER {0}, 0, 0, 0, {0}, 0, 0

/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_STOCK_STATIC_INFO_REQ                                  \
  {NULLOBJ_MDS_QRY_REQ_HEAD}, { NULLOBJ_MDS_QRY_STOCK_STATIC_INFO_FILTER }
...
typedef struct _MdsQryOptionStaticInfoReq {
  ...
  MdsQryReqHeadT reqHead;
  /** 查询请求的过滤条件 */
  MdsQryOptionStaticInfoFilterT qryFilter;
...
typedef struct _MdsQryOptionStaticInfoRsp {
  ...
  MdsOptionStaticInfoT qryItems[1];
} MdsQryOptionStaticInfoRspT;

/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_OPTION_STATIC_INFO_FILTER {0}, 0, 0, 0, {0}, 0, 0

/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_OPTION_STATIC_INFO_REQ                                 \
  {NULLOBJ_MDS_QRY_REQ_HEAD}, { NULLOBJ_MDS_QRY_OPTION_STATIC_INFO_FILTER }
...
typedef struct _MdsQryStockStaticInfoListReq {
  ...
  MdsQrySecurityCodeEntryT securityCodeList[1];
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_STOCK_STATIC_INFO_LIST_FILTER 0, 0, 0, {0}, 0

/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_STOCK_STATIC_INFO_LIST_REQ                             \
  {NULLOBJ_MDS_QRY_REQ_HEAD}, {NULLOBJ_MDS_QRY_STOCK_STATIC_INFO_LIST_FILTER}, \
      0, 0, {                                                                  \
    {                                                                          \
...
typedef struct _MdsQrySnapshotListReq {
  ...
  MdsQryReqHeadT reqHead;
  /** 查询请求的过滤条件 */
  MdsQrySnapshotListFilterT qryFilter;
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_SNAPSHOT_LIST_FILTER 0, 0, 0, 0, 0, {0}, 0

/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_SNAPSHOT_LIST_REQ                                      \
  {NULLOBJ_MDS_QRY_REQ_HEAD}, {NULLOBJ_MDS_QRY_SNAPSHOT_LIST_FILTER}, 0, 0, {  \
    {                                                                          \
      NULLOBJ_MDS_QRY_SECURITY_CODE_ENTRY                                      \
    }                                                                          \
  }
...
typedef struct _MdsApplUpgradeItem {
  ...
  /** 废弃的应用版本号列表 */
  char discardApplVerId[MDS_APPL_DISCARD_VERSION_MAX_COUNT][MDS_VER_ID_MAX_LEN];
  /** 废弃版本号的数目 */
  ...
  /** 主用升级源配置信息 */
  MdsApplUpgradeSourceT primarySource;

  /** 备用升级源配置信息 */
  MdsApplUpgradeSourceT secondarySource;
} MdsApplUpgradeItemT;
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_APPL_UPGRADE_SOURCE {0}, {0}, {0}, {0}, 0, 0, {0}, {0}

/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_APPL_UPGRADE_ITEM                                          \
  {0}, {0}, {0}, {{0}}, 0, 0, {0}, {0}, {NULLOBJ_MDS_APPL_UPGRADE_SOURCE}, {   \
    NULLOBJ_MDS_APPL_UPGRADE_SOURCE                                            \
  }

/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_APPL_UPGRADE_INFO                                          \
  {NULLOBJ_MDS_APPL_UPGRADE_ITEM}, {NULLOBJ_MDS_APPL_UPGRADE_ITEM}, {          \
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDS_QRY_APPL_UPGRADE_INFO_RSP {NULLOBJ_MDS_APPL_UPGRADE_INFO}
/* -------------------------           */
