 ...
 *          - 删除 MdsApi_IsBusinessError 接口, 查询接口不再返回小于 -1000
 * 的错误
 *          - 查询接口返回值变更:
 ...
 *          - 重命名 protocol_parser/errors/mds_protocol_errors.h ->
 * errors/mds_errors.h
 *          - 添加辅助实现实时行情订阅功能的接口
 ...
 *          - 新增接口 '重置线程级别的日志记录器名称
 * (MdsApi_ResetThreadLoggerName)', 以支持为当前线程设置独立的日志文件
 ...
 *          - 增加 MdsApi_IsValidTcpChannel、MdsApi_IsValidUdpChannel 等接口,
 * 用于判断通道是否已经连接且有效
 * @version 0.15.5.2    2018/01/29
 *          - 增加 MdsApi_GetLastRecvTime、MdsApi_GetLastSendTime 接口,
 * 用于获取通道最近发送/接收消息的时间
 *          - 登录失败时, 可以通过 errno/SPK_GET_ERRNO() 获取到具体失败原因
 * @version 0.15.5.3    2018/01/24
 *          - 调整接口 MdsApi_InitAll, 增加一个函数参数 (pUdpTick2AddrKey),
 * 以支持分别订阅逐笔成交和逐笔委托的行情组播
 ...
 *              - 与不带 Compressible 后缀的接口相比, 带 Compressible
 * 后缀的接口会自动检测和处理压缩过的消息,
 *                但也会因此带来微小的性能损失。如果对延迟不是极端敏感的话,
 * 可以直接使用带 Compressible 后缀的接 口, 以方便兼容不同的模式。
 * @version 0.15.5.12   2018/06/21
 *          - 增加 MdsApi_HasMoreCachedData 接口,
 ...
 *              - 添加 MdsHelper_SetTickTypeOnSubscribeByString 接口,
 * 以设置SubscribeByString默认使用的数据模式 (TickType)
 *              - 增量订阅时, 允许不指定 dataType (小于0)
 * 而继承之前订阅的数据类型
 ...
 *          - 调整L2逐笔数据的行情组播频道
 *              - 划分为两个频道, 按照频道号混合推送逐笔成交和逐笔委托,
 * 取代之前逐笔成交/逐笔委托分别推送的方式
 ...
 *          - 完善对行情订阅条件中起始时间 (beginTime) 和初始行情标志
 * (isRequireInitialMktData) 的支持
 *              - 允许在会话过程中任意时间指定 beginTime 和
 * isRequireInitialMktData 订阅参数
 *              - 频繁指定 beginTime 和 isRequireInitialMktData 订阅参数,
 * 会对当前客户 端的行情获取速度产生不利影响。应谨慎使用, 避免频繁订阅
 *          - 优化行情订阅接口,
 * 取消基础订阅接口一次最大只能订阅4000只证券代码的限制
 *              - MdsApi_SubscribeMarketData
 * @version 0.15.9.2    2019/10/08
 *          - 调整行情组播频道的划分
 *              - API及配置文件保持兼容, 但频道内容发生了变化
 *              - 划分为两个独立的沪/深快照频道和两个独立的沪/深逐笔频道,
 * 共4个组播频道
 *                  - 快照-频道1, 上海L1/L2快照
 *                  - 快照-频道2, 深圳L1/L2快照
 *                  - 逐笔-频道1, 上海逐笔成交/逐笔委托
 ...
 *              - MdsApi_ParseConfigFromFile2, 解析客户端配置文件,
 * 并可以指定是否允许配置项为空
 ...
 *          - 增加新的批量查询证券(股票/债券/基金)静态信息列表接口,
 * 以支持同时指定和查询多个证券代码
 *              - MdsApi_QueryStockStaticInfoList,
 * 批量查询证券(股票/债券/基金)静态信息列表
 *              - MdsApi_QueryStockStaticInfoList2,
 ...
 *              - MdsApi_SetRetainExtDataAble,
 * 设置是否保留(不清空)由应用层自定义使用的扩展存储空间数据 (默认为否)
 *              - MdsApi_IsRetainExtDataAble,
 * 返回是否保留(不清空)由应用层自定义使用的扩展存储空间数据
 * @version 0.15.11.16  2021/01/31
 *          - 增加辅助的会话信息访问接口
 *              - MdsApi_GetClientId, 返回通道对应的客户端类型
 *          - 增加辅助的行情订阅接口
 *              - MdsApi_SubscribeByQuery,
 * 查询证券静态信息并根据查询结果订阅行情信息
 *          - 添加辅助的订阅信息结构体操作函数
 *              - MdsHelper_AddSubscribeRequestEntry2,
 * 添加待订阅产品到订阅信息中 (没有数量限制)
 * @version 0.15.12.1   2021/04/19
 *          - 增加以下辅助接口
 *              - MdsApi_SetUdpReconnectFromNextAddrAble,
 * 设置重建连接组播通道时是否从下一个地址开始尝试 (默认为否)
 *              - MdsApi_IsUdpReconnectFromNextAddrAble,
 * 返回重建连接组播通道时是否从下一个地址开始尝试
 * @version 0.15.12.2   2021/05/13
 *          - 增加以下辅助接口
 *              - MdsApi_SetTimeOffsetMeasurable,
 * 设置登录服务器时是否自动计算和服务器之间的时间偏差 (默认为是)
 *              - MdsApi_IsTimeOffsetMeasurable,
 ...
 *              - MdsApi_QueryOptionStaticInfoList2, 批量查询期权静态信息列表
 * (字符串指针数组形式的证券代码列表)
 *
 * @since   2016/03/02
 */
...
/** 默认的UDP连接的心跳间隔 (10秒, 如果超过3倍心跳时间没有接收到任何组播消息,
 * 就可以认为组播异常) */
...
/** UDP行情订阅服务配置项名称 (逐笔成交) @deprecated 已废弃,
 * 效果等同于逐笔-频道1 */
#define MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_TICK_TRADE "udpServer.TickTrade"
/** UDP行情订阅服务配置项名称 (逐笔委托) @deprecated 已废弃,
 * 效果等同于逐笔-频道2 */
#define MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_TICK_ORDER "udpServer.TickOrder"

/** 默认的UDP行情订阅服务配置项名称 @deprecated 已废弃, 效果等同于快照-频道1 */
#define MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_SNAP1
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDSAPI_SESSION_INFO NULLOBJ_GENERAL_CLIENT_CHANNEL
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDSAPI_CHANNEL_GROUP NULLOBJ_GENERAL_CLIENT_CHANNEL_GROUP
/* -------------------------           */
...
/* 结构体初始化值定义 */
#define NULLOBJ_MDSAPI_ADDR_INFO NULLOBJ_GENERAL_CLIENT_ADDR_INFO
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDSAPI_REMOTE_CFG NULLOBJ_GENERAL_CLIENT_REMOTE_CFG
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDSAPI_ADDR_CURSOR NULLOBJ_GENERAL_CLIENT_ADDR_CURSOR
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_MDSAPI_SUBSCRIBE_INFO NULLOBJ_MDS_MKT_DATA_REQUEST_REQ_BUF
/* -------------------------           */
...
typedef struct _MdsApiClientCfg {
  ...
  union {
    ...
    MdsApiRemoteCfgT
        udpL1ChannelCfg; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };
  union {
    MdsApiRemoteCfgT udpSnap2ChannelCfg; /**< 快照行情(频道2)组播服务配置 */
    MdsApiRemoteCfgT
        udpL2ChannelCfg; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };
  union {
    MdsApiRemoteCfgT udpTick1ChannelCfg; /**< 逐笔数据(频道1)组播服务配置 */
    MdsApiRemoteCfgT
        udpTradeChannelCfg; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };
  union {
    MdsApiRemoteCfgT udpTick2ChannelCfg; /**< 逐笔数据(频道2)组播服务配置 */
    MdsApiRemoteCfgT
        udpOrderChannelCfg; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };
...
#define NULLOBJ_MDSAPI_CLIENT_CFG                                              \
  {NULLOBJ_MDSAPI_REMOTE_CFG}, {NULLOBJ_MDSAPI_REMOTE_CFG},                    \
      {{NULLOBJ_MDSAPI_REMOTE_CFG}}, {{NULLOBJ_MDSAPI_REMOTE_CFG}},            \
      {{NULLOBJ_MDSAPI_REMOTE_CFG}}, {{NULLOBJ_MDSAPI_REMOTE_CFG}}, {          \
...
typedef struct _MdsApiClientEnv {
  ...
  union {
    MdsApiSessionInfoT
        udpSnap1Channel; /**< 快照行情(频道1)组播通道的会话信息 */
    MdsApiSessionInfoT
        udpL1Channel; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };
  union {
    MdsApiSessionInfoT
        udpSnap2Channel; /**< 快照行情(频道2)组播通道的会话信息 */
    MdsApiSessionInfoT
        udpL2Channel; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };
  union {
    MdsApiSessionInfoT
        udpTick1Channel; /**< 逐笔数据(频道1)组播通道的会话信息 */
    MdsApiSessionInfoT
        udpTradeChannel; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };
  union {
    MdsApiSessionInfoT
        udpTick2Channel; /**< 逐笔数据(频道2)组播通道的会话信息 */
    MdsApiSessionInfoT
        udpOrderChannel; /**< @deprecated 已废弃, 为了保持兼容而暂时保留 */
  };

  MdsApiChannelGroupT
      udpChannelGroup; /**< 用于管理多个行情组播会话的通道组信息 */
} MdsApiClientEnvT;

/* 结构体的初始化值定义 */
#define NULLOBJ_MDSAPI_CLIENT_ENV                                              \
  {NULLOBJ_MDSAPI_SESSION_INFO}, {NULLOBJ_MDSAPI_SESSION_INFO},                \
      {{NULLOBJ_MDSAPI_SESSION_INFO}}, {{NULLOBJ_MDSAPI_SESSION_INFO}},        \
      {{NULLOBJ_MDSAPI_SESSION_INFO}}, {{NULLOBJ_MDSAPI_SESSION_INFO}}, {      \
 ...
 * @see     MdsL2OrderT
 */
typedef int32 (*F_MDSAPI_ONMSG_T)(MdsApiSessionInfoT *pSessionInfo,
 ...
 */
typedef int32 (*F_MDSAPI_ON_QRY_MSG_T)(MdsApiSessionInfoT *pQryChannel,
                                       SMsgHeadT *pMsgHead, void *pMsgItem,
 ...
 */
BOOL MdsApi_Logon(MdsApiSessionInfoT *pOutSessionInfo,
                  eMdsApiChannelTypeT channelType, const char *pUri,
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL MdsApi_LogonReplicaSet(MdsApiSessionInfoT *pOutSessionInfo,
                            eMdsApiChannelTypeT channelType,
                            const char *pUriList, const char *pUsername,
                            const char *pPassword, int32 heartBtInt,
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL MdsApi_LogonPeerNodes(MdsApiSessionInfoT *pOutSessionInfo,
                           eMdsApiChannelTypeT channelType,
                           const char *pUriList, const char *pUsername,
                           const char *pPassword, int32 heartBtInt,
                           const SSocketOptionConfigT *pSocketOptions);
 ...
 *
 * @note    频繁订阅会对当前连接的行情获取速度产生不利影响,
 * 建议每次订阅都尽量指定更多的证券 代码, 以减少订阅次数
 *
 * @param   pTcpChannel         会话信息
 * @param   pMktDataRequestReq  行情订阅请求信息
 * @param   pEntries            产品订阅列表
 * @return  TRUE 成功; FALSE 失败
 */
BOOL MdsApi_SubscribeMarketData(MdsApiSessionInfoT *pTcpChannel,
                                const MdsMktDataRequestReqT *pMktDataRequestReq,
                                const MdsMktDataRequestEntryT *pEntries);
 ...
 * @note    频繁订阅会对当前连接的行情获取速度产生不利影响,
 * 建议每次订阅都尽量指定更多的证券 代码, 以减少订阅次数
 * @note    订阅的数据类型 (dataTypes) 会以最后一次订阅为准,
 * 所以每次都应该指定为所有待订阅 的数据类型
 *
 * @param   pTcpChannel         会话信息
 ...
 *                              - 如果为空, 则使用默认的分隔符:
 *                                ',' 或 ';' 或 '|' 或 ' ' 或 '\t'
 * @param   exchangeId          证券代码所属的交易所代码 (如果证券代码没有 .SH
 * 或 .SZ 后缀的话)
 ...
 */
BOOL MdsApi_SubscribeByString(MdsApiSessionInfoT *pTcpChannel,
 ...
 * @see     MdsApi_SetThreadSubscribeBeginTime
 */
BOOL MdsApi_SubscribeByString2(MdsApiSessionInfoT *pTcpChannel,
                               const char *ppSecurityArray[],
                               int32 securityCount, eMdsExchangeIdT exchangeId,
                               eMdsMdProductTypeT mdProductType,
                               eMdsSubscribeModeT subMode, int32 dataTypes);

/**
 * 直接根据字符串形式的证券代码列表订阅行情,
 * 并通过证券代码前缀来区分和识别所属市场
 *
 * @note    代码前缀仅对 pSecurityListStr 参数指定的证券代码生效,
 * 只是为了方便区分证券代码 所属的市场,
 * 并不能直接通过代码前缀自动订阅所有匹配的股票
 * @note    为兼容之前的版本, 该接口无法指定 tickType 等订阅参数, 可以通过
 *          MdsApi_SetThreadSubscribeTickType 等接口来设置这些附加的订阅参数
 * @note    频繁订阅会对当前连接的行情获取速度产生不利影响,
 * 建议每次订阅都尽量指定更多的证券 代码, 以减少订阅次数
 ...
 *                              - "6, 300, 301" 将匹配证券代码列表中所有以 '6'
 * 或 '300' 或 '301' 起始的证券代码
 *                              - 若为NULL或空字符串, 则不会匹配任何证券代码
 *                              - 上海证券代码前缀参考:
 *                                  - "009, 01, 02, "               //国债
 *                                  - "10, 11, 12, 13, 18, 19, "    //债券
 * (企业债、可转债等)
 *                                  - "20, "                        //债券
 * (回购)
 *                                  - "5, "                         //基金
...
BOOL MdsApi_SubscribeByStringAndPrefixes(
    MdsApiSessionInfoT *pTcpChannel, const char *pSecurityListStr,
    const char *pDelim, const char *pSseCodePrefixes,
    const char *pSzseCodePrefixes, eMdsMdProductTypeT mdProductType,
    eMdsSubscribeModeT subMode, int32 dataTypes);

/**
 * 根据字符串指针数组形式的证券代码列表订阅行情,
 * 并通过证券代码前缀来区分和识别所属市场
 *
 * @note    代码前缀仅对 pSecurityListStr 参数指定的证券代码生效,
 * 只是为了方便区分证券代码 所属的市场,
...
BOOL MdsApi_SubscribeByStringAndPrefixes2(
    MdsApiSessionInfoT *pTcpChannel, const char *ppSecurityArray[],
    int32 securityCount, const char *pSseCodePrefixes,
    const char *pSzseCodePrefixes, eMdsMdProductTypeT mdProductType,
    eMdsSubscribeModeT subMode, int32 dataTypes);
 ...
 * @retval  <0                  失败 (负的错误号)
 */
int32 MdsApi_SubscribeByQuery(
    MdsApiSessionInfoT *pTcpChannel, eMdsSubscribeModeT subMode,
    int32 dataTypes, MdsApiSessionInfoT *pQryChannel,
    const MdsQryStockStaticInfoListFilterT *pQryFilter);

/**
 * 发送心跳消息
 *
 * @note    心跳消息不能发送的太过频繁, 需要控制不能超过每秒两次,
 * 否则会带来延迟惩罚。 建议使用和心跳时间相同的频率就可以 (例如: 5~30秒)
 *
 * @param   pSessionInfo        会话信息
 * @return  TRUE 成功; FALSE 失败
 */
BOOL MdsApi_SendHeartbeat(MdsApiSessionInfoT *pSessionInfo);

/**
 * 发送测试请求消息
 *
 * @note    测试请求消息的处理成本比较大, 如果发送太过频繁(超过每秒一次),
 * 会带来额外的延迟惩罚 处理。另外测试请求消息存在固有的毫秒级的发送成本,
 * 需要尽量降低测试请求的发送频率。 建议使用和心跳时间相同的频率就可以 (例如:
 * 20~60秒)
 *
 * @param   pSessionInfo        会话信息
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL MdsApi_SendTestRequest(MdsApiSessionInfoT *pSessionInfo,
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL MdsApi_Logout(MdsApiSessionInfoT *pSessionInfo, BOOL isDestory);
 ...
 * @param   pSessionInfo        会话信息
 */
void MdsApi_Destory(MdsApiSessionInfoT *pSessionInfo);
 ...
 */
int32 MdsApi_WaitOnMsg(MdsApiSessionInfoT *pTcpChannel, int32 timeoutMs,
                       F_MDSAPI_ONMSG_T fnOnMsgCallback, void *pCallbackParams);

/**
 * 阻塞接收MDS行情消息, 并调用回调函数进行消息处理 (可以处理压缩过的消息)
 *
 * - 与不带 Compressible 后缀的接口相比, 带 Compressible
 * 后缀的接口会自动检测和处理压缩过的 消息, 但也会因此带来微小的性能损失。
 * - 如果对延迟不是极端敏感的话, 可以直接使用带 Compressible 后缀的接口,
 * 以方便兼容不同的模式。
 *
 * @param   pTcpChannel         会话信息
 * @param   timeoutMs           超时时间(毫秒)
 ...
 * @exception   Others 由read()系统调用或回调函数fnOnMsgCallback返回的错误
 */
int32 MdsApi_WaitOnMsgCompressible(MdsApiSessionInfoT *pTcpChannel,
                                   int32 timeoutMs,
 ...
 */
const char *MdsApi_GetApiVersion();
 ...
 * @see         MdsL1SnapshotT
 */
int32 MdsApi_QueryMktDataSnapshot(MdsApiSessionInfoT *pQryChannel,
                                  eMdsExchangeIdT exchangeId,
                                  eMdsMdProductTypeT mdProductType,
                                  int32 instrId, MdsMktDataSnapshotT *pRspBuf);
 ...
 * @see     MdsL1SnapshotT
 */
int32 MdsApi_QuerySnapshotList(MdsApiSessionInfoT *pQryChannel,
                               const char *pSecurityListStr, const char *pDelim,
 ...
 * @see     MdsL1SnapshotT
 */
int32 MdsApi_QuerySnapshotList2(MdsApiSessionInfoT *pQryChannel,
                                const char *ppSecurityArray[],
                                int32 securityCount,
                                const MdsQrySnapshotListFilterT *pQryFilter,
                                F_MDSAPI_ON_QRY_MSG_T fnQryMsgCallback,
 ...
 */
int32 MdsApi_QuerySecurityStatus(MdsApiSessionInfoT *pQryChannel,
                                 eMdsExchangeIdT exchangeId,
                                 eMdsMdProductTypeT mdProductType,
                                 int32 instrId, MdsSecurityStatusMsgT *pRspBuf);
 ...
 */
int32 MdsApi_QueryTrdSessionStatus(MdsApiSessionInfoT *pQryChannel,
                                   eMdsExchangeIdT exchangeId,
                                   eMdsMdProductTypeT mdProductType,
...
int32 MdsApi_QueryStockStaticInfo(
    ...
    F_MDSAPI_ON_QRY_MSG_T fnQryMsgCallback, void *pCallbackParams);
...
int32 MdsApi_QueryStockStaticInfoList(
    MdsApiSessionInfoT *pQryChannel, const char *pSecurityListStr,
    const char *pDelim, const MdsQryStockStaticInfoListFilterT *pQryFilter,
    F_MDSAPI_ON_QRY_MSG_T fnQryMsgCallback, void *pCallbackParams);
...
int32 MdsApi_QueryStockStaticInfoList2(
    MdsApiSessionInfoT *pQryChannel, const char *ppSecurityArray[],
    int32 securityCount, const MdsQryStockStaticInfoListFilterT *pQryFilter,
    F_MDSAPI_ON_QRY_MSG_T fnQryMsgCallback, void *pCallbackParams);
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>MdsOptionStaticInfoT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
...
int32 MdsApi_QueryOptionStaticInfoList(
    MdsApiSessionInfoT *pQryChannel, const char *pSecurityListStr,
    const char *pDelim, const MdsQryOptionStaticInfoListFilterT *pQryFilter,
    F_MDSAPI_ON_QRY_MSG_T fnQryMsgCallback, void *pCallbackParams);
...
int32 MdsApi_QueryOptionStaticInfoList2(
    MdsApiSessionInfoT *pQryChannel, const char *ppSecurityArray[],
    int32 securityCount, const MdsQryOptionStaticInfoListFilterT *pQryFilter,
    F_MDSAPI_ON_QRY_MSG_T fnQryMsgCallback, void *pCallbackParams);
/* -------------------------           */
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL MdsApi_ConnectToUdpMcast(MdsApiSessionInfoT *pOutSessionInfo,
                              const char *pUri,
                              const SSocketOptionConfigT *pSocketOptions);
 ...
 * @exception   Others 由read()系统调用或回调函数fnOnMsgCallback返回的错误
 */
int32 MdsApi_WaitOnUdpMsg(MdsApiSessionInfoT *pUdpChannel, int32 timeoutMs,
                          F_MDSAPI_ONMSG_T fnOnMsgCallback,
                          void *pCallbackParams);
/* -------------------------           */
 ...
 * @param       pTcpAddrKey     TCP行情订阅地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @param       pQryAddrKey     行情查询服务地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @param       pUdpSnap1AddrKey
 *                              快照行情(频道1)组播地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @param       pUdpSnap2AddrKey
 *                              快照行情(频道2)组播地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @param       pUdpTick1AddrKey
 *                              逐笔数据(频道1)组播地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @param       pUdpTick2AddrKey
 *                              逐笔数据(频道2)组播地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @retval      TRUE            成功
 * @retval      FALSE           失败。此时 errno 将被设置, 可以通过
 * errno/SPK_GET_ERRNO() 获取到具体失败原因
 ...
 */
BOOL MdsApi_InitAll(MdsApiClientEnvT *pOutCliEnv, const char *pCfgFile,
                    const char *pLoggerSection, const char *pCfgSection,
                    const char *pTcpAddrKey, const char *pQryAddrKey,
                    const char *pUdpSnap1AddrKey, const char *pUdpSnap2AddrKey,
                    const char *pUdpTick1AddrKey, const char *pUdpTick2AddrKey);
 ...
 * MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_SNAP2, MDSAPI_CFG_DEFAULT_KEY_UDP_ADDR_TICK1,
 ...
 * @see         MdsApi_InitAllByCfgStruct
 */
BOOL MdsApi_InitAllByConvention(MdsApiClientEnvT *pOutCliEnv,
                                const char *pCfgFile);
 ...
 *              MdsApi_InitAllByCfgStruct 接口不会自动初始化日志记录器,
 * 需要在外面 显式的调用 MdsApi_InitLogger 来初始化API的日志记录器
 *
 * @param[out]  pOutCliEnv      输出行情客户端环境信息
 ...
 * @see         MdsApi_InitAllByConvention
 */
BOOL MdsApi_InitAllByCfgStruct(MdsApiClientEnvT *pOutCliEnv,
 ...
 * @param[out]  pCliEnv         行情客户端环境信息
 */
void MdsApi_DestoryAll(MdsApiClientEnvT *pCliEnv);
/* -------------------------           */
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL MdsApi_InitLogger(const char *pCfgFile, const char *pLoggerSection);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL MdsApi_ResetThreadLoggerName(const char *pLogSystemName);

/**
 * 初始化基于TCP协议的行情订阅通道
 * (包括完整的配置解析、连接建立、登录和行情订阅过程)
 *
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL MdsApi_InitTcpChannel(MdsApiSessionInfoT *pTcpChannel,
 ...
 *                              - @note 禁用自动订阅行情以后,
 * 需要在登录后90秒内发起行情 订阅操作, 否则服务器端将主动关闭会话
 * @retval      TRUE            成功
 * @retval      FALSE           失败。此时 errno 将被设置, 可以通过
 * errno/SPK_GET_ERRNO() 获取到具体失败原因
 *
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL MdsApi_InitTcpChannel2(MdsApiSessionInfoT *pTcpChannel,
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL MdsApi_InitQryChannel(MdsApiSessionInfoT *pQryChannel,
                           const char *pCfgFile, const char *pCfgSection,
                           const char *pAddrKey);
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL MdsApi_InitQryChannel2(MdsApiSessionInfoT *pQryChannel,
                            const MdsApiRemoteCfgT *pRemoteCfg);
 ...
 */
BOOL MdsApi_InitUdpChannel(MdsApiSessionInfoT *pUdpChannel,
                           const char *pCfgFile, const char *pCfgSection,
                           const char *pAddrKey);
 ...
 * @exception   Others          由系统调用返回的错误
 */
BOOL MdsApi_InitUdpChannel2(MdsApiSessionInfoT *pUdpChannel,
                            const MdsApiRemoteCfgT *pRemoteCfg);
 ...
 */
int32 MdsApi_ParseAddrListString(const char *pUriList,
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL MdsApi_ParseConfigFromFile(const char *pCfgFile, const char *pSection,
                                const char *pAddrKey,
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL MdsApi_ParseConfigFromFile2(const char *pCfgFile, const char *pSection,
                                 const char *pAddrKey,
                                 MdsApiRemoteCfgT *pOutRemoteCfg,
                                 MdsApiSubscribeInfoT *pOutSubscribeInfo,
                                 BOOL isRequiredCfg);
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL MdsApi_ParseAllConfig(const char *pCfgFile, MdsApiClientCfgT *pOutApiCfg);
/* -------------------------           */
 ...
 * @return          TRUE, 成功; FALSE 失败
 */
BOOL MdsApi_InitChannelGroup(MdsApiChannelGroupT *pChannelGroup);
 ...
 * @return      TRUE, 成功; FALSE 失败
 */
BOOL MdsApi_DestoryChannelGroup(MdsApiChannelGroupT *pChannelGroup);
 ...
 * @return      TRUE, 成功; FALSE 失败
 */
BOOL MdsApi_AddToChannelGroup(MdsApiChannelGroupT *pChannelGroup,
 ...
 * @return      TRUE, 成功; FALSE 失败
 */
BOOL MdsApi_DeleteFromChannelGroup(MdsApiChannelGroupT *pChannelGroup,
 ...
 */
MdsApiSessionInfoT *
MdsApi_GetFromChannelGroup(MdsApiChannelGroupT *pChannelGroup, int32 index);
 ...
 */
MdsApiSessionInfoT *
MdsApi_GetFromChannelGroupBySocket(MdsApiChannelGroupT *pChannelGroup,
                                   SPK_SOCKET socketFd);
 ...
 */
int64 MdsApi_GetChannelGroupLastRecvTime(MdsApiChannelGroupT *pChannelGroup);
 ...
 */
int64 MdsApi_GetChannelGroupLastSendTime(MdsApiChannelGroupT *pChannelGroup);
 ...
 */
int32 MdsApi_WaitOnTcpChannelGroup(MdsApiChannelGroupT *pChannelGroup,
                                   int32 timeoutMs,
                                   F_MDSAPI_ONMSG_T fnOnMsgCallback,
                                   void *pCallbackParams,
...
int32 MdsApi_WaitOnTcpChannelGroupCompressible(
    MdsApiChannelGroupT *pChannelGroup, int32 timeoutMs,
    F_MDSAPI_ONMSG_T fnOnMsgCallback, void *pCallbackParams,
    MdsApiSessionInfoT **ppFailed);
 ...
 * @exception   Others          由select/read系统调用或回调函数返回的错误
 */
int32 MdsApi_WaitOnUdpChannelGroup(MdsApiChannelGroupT *pChannelGroup,
                                   int32 timeoutMs,
                                   F_MDSAPI_ONMSG_T fnOnMsgCallback,
                                   void *pCallbackParams,
                                   MdsApiSessionInfoT **ppFailed);
 ...
 */
void MdsApi_SetThreadUsername(const char *pUsername);
 ...
 * @return  登录用户名
 */
const char *MdsApi_GetThreadUsername();
 ...
 * txt:PASSWORD
 */
void MdsApi_SetThreadPassword(const char *pPassword);

/**
 * 设置当前线程登录TCP行情通道以后是否自动执行行情订阅操作
 *
 * <p>相关说明:
 * - 对于 InitAll/InitTcpChannel 这几个初始化接口, 默认情况下,
 * 登录后会自动使用配置文件中 的配置参数执行一次行情订阅操作,
 * 可以通过该接口来设置线程级别的标志变量, 禁止登录以后自动 执行行情订阅操作。
 * - beginTime 行情订阅参数, 仅在登录后的首次行情订阅时才可以使用,
 * 禁用自动订阅行情以后, 就 可以根据需要自行控制和发起首次行情订阅了。
 * - @note 禁用自动订阅行情以后, 需要在登录后90秒内发起行情订阅操作,
 * 否则服务器端将主动关闭会话
 * </p>
 *
 ...
 */
void MdsApi_SetThreadSubscribeTickType(int32 tickType);

/**
 * 设置SubscribeByString接口默认使用的逐笔数据的数据重建标识 (tickRebuildFlag,
 * 标识是否订阅重建到的逐笔数据)
 *
 * @param   tickRebuildFlag     逐笔数据的数据重建标识 @see
 ...
 * @see     MdsApi_SubscribeByStringAndPrefixes
 */
void MdsApi_SetThreadSubscribeTickRebuildFlag(int32 tickRebuildFlag);

/**
 * 设置SubscribeByString接口默认使用的逐笔数据的过期时间类型 (tickExpireType)
 *
 * @param   tickExpireType      逐笔数据的过期时间类型 @see
 ...
 * @see     MdsApi_SubscribeByStringAndPrefixes
 */
void MdsApi_SetThreadSubscribeTickExpireType(int32 tickExpireType);
 ...
 *                              是否需要推送已订阅产品的初始的行情快照
 *                              - @see
 * MdsMktDataRequestReqT.isRequireInitialMktData
 *
 * @see     MdsApi_SubscribeByString
 * @see     MdsApi_SubscribeByStringAndPrefixes
 */
void MdsApi_SetThreadSubscribeRequireInitMd(BOOL isRequireInitialMktData);
 ...
 *                              - -1: 从头开始获取
 *                              -  0: 从最新位置开始获取实时行情
 *                              - >0: 从指定的起始时间开始获取 (HHMMSS /
 * HHMMSSsss)
 *                              - 当订阅模式为 Append/Delete/BatchDelete
 * 时将忽略该参数
 ...
 * @see     MdsApi_SubscribeByStringAndPrefixes
 */
void MdsApi_SetThreadSubscribeBeginTime(int32 beginTime);
 ...
 */
BOOL MdsApi_SetCustomizedIp(const char *pIpStr);
 ...
 * @return  客户端自定义的本地IP
 */
const char *MdsApi_GetCustomizedIp();
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL MdsApi_SetCustomizedMac(const char *pMacStr);
 ...
 * @return  客户端自定义的本地MAC
 */
const char *MdsApi_GetCustomizedMac();
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL MdsApi_SetCustomizedDriverId(const char *pDriverId);
 ...
 */
const char *MdsApi_GetCustomizedDriverId();
 ...
 */
void MdsApi_SetRetainExtDataAble(BOOL isRetainExtDataAble);
 ...
 * @return  通道对应的客户端编号
 */
int16 MdsApi_GetClientId(const MdsApiSessionInfoT *pSessionInfo);
 ...
 */
uint8 MdsApi_GetClientType(const MdsApiSessionInfoT *pSessionInfo);
 ...
 */
uint8 MdsApi_GetClientStatus(const MdsApiSessionInfoT *pSessionInfo);
 ...
 */
int64 MdsApi_GetLastRecvTime(const MdsApiSessionInfoT *pSessionInfo);
 ...
 */
int64 MdsApi_GetLastSendTime(const MdsApiSessionInfoT *pSessionInfo);
 ...
 */
int32 MdsApi_HasMoreCachedData(const MdsApiSessionInfoT *pTcpChannel);
 ...
 */
BOOL MdsApi_IsValidTcpChannel(const MdsApiSessionInfoT *pTcpChannel);
 ...
 * @return  TRUE 已连接; FALSE 未连接或连接无效
 */
BOOL MdsApi_IsValidQryChannel(const MdsApiSessionInfoT *pQryChannel);
 ...
 * @return  TRUE 已连接; FALSE 未连接或连接无效
 */
BOOL MdsApi_IsValidUdpChannel(const MdsApiSessionInfoT *pUdpChannel);
 ...
 * @return  TRUE 已连接; FALSE 未连接或连接无效
 */
BOOL MdsApi_IsValidChannelGroup(const MdsApiChannelGroupT *pChannelGroup);
 ...
 * @param   errCode             错误号
 */
void MdsApi_SetLastError(int32 errCode);
 ...
 * @return  错误号对应的错误信息
 */
const char *MdsApi_GetErrorMsg(int32 errCode);
 ...
 * @return  状态码所对应的错误信息
 */
const char *MdsApi_GetErrorMsg2(uint8 status, uint8 detailStatus);
 ...
 */
BOOL MdsApi_IsErrorOf(int32 errCode, const SErrMsgT *pErrMsg);
 ...
 * @return  TRUE 属于; FALSE 不属于
 */
BOOL MdsApi_IsErrorOf2(uint8 status, uint8 detailStatus,
 ...
 */
BOOL MdsApi_HasStockStatus(const MdsStockStaticInfoT *pStockStaticInfo,
                           uint32 status);
/* -------------------------           */
 ...
 */
void MdsHelper_SetTickTypeOnSubscribeByString(int32 tickType);
 ...
 * @param   subMode             订阅模式 @see eMdsSubscribeModeT
 */
void MdsHelper_SetSubscribeRequestMode(MdsApiSubscribeInfoT *pSubscribeInfo,
 ...
 * 将忽略该参数, 维持上一次订阅时的设置
 */
void MdsHelper_SetSubscribeRequestTickType(MdsApiSubscribeInfoT *pSubscribeInfo,
                                           int32 tickType);

/**
 * 订阅信息的结构体操作函数 - 设置逐笔数据的数据重建标识 (tickRebuildFlag,
 * 标识是否订阅重建到的逐笔数据)
 *
 * @param   pSubscribeInfo      订阅信息
 * @param   tickRebuildFlag     逐笔数据的数据重建标识 @see
...
void MdsHelper_SetSubscribeRequestTickRebuildFlag(
    MdsApiSubscribeInfoT *pSubscribeInfo, int32 tickRebuildFlag);
...
void MdsHelper_SetSubscribeRequestTickExpireType(
    MdsApiSubscribeInfoT *pSubscribeInfo, int32 tickExpireType);
...
void MdsHelper_SetSubscribeRequestRequireInitMd(
    MdsApiSubscribeInfoT *pSubscribeInfo, BOOL isRequireInitialMktData);
...
void MdsHelper_SetSubscribeRequestBeginTime(
    MdsApiSubscribeInfoT *pSubscribeInfo, int32 beginTime);
...
void MdsHelper_SetSubscribeRequestDataTypes(
    MdsApiSubscribeInfoT *pSubscribeInfo, int32 dataTypes);
 ...
 */
void MdsHelper_SetSubscribeRequestSubFlag(MdsApiSubscribeInfoT *pSubscribeInfo,
                                          eMdsExchangeIdT exchangeId,
                                          eMdsMdProductTypeT mdProductType,
 ...
 *          小于0, 失败 (负的错误号)
 */
int32 MdsHelper_AddSubscribeRequestEntry(MdsApiSubscribeInfoT *pSubscribeInfo,
                                         eMdsExchangeIdT exchangeId,
                                         eMdsMdProductTypeT mdProductType,
                                         int32 securityId);
 ...
 *          小于0, 失败 (负的错误号)
 */
int32 MdsHelper_AddSubscribeRequestEntry2(MdsApiSubscribeInfoT *pSubscribeInfo,
 ...
 */
static __inline BOOL __MdsApi_CheckApiVersion() {
