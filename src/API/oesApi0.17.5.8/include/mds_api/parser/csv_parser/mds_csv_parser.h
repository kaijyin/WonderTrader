 ...
 *          - 调整 CSV 行情数据的输出格式
 *              - 废弃 __origNetTime 字段, 保留字段位置以兼容之前格式,
 * 但字段名称修改为 __zero, 取值固定为0
 ...
 */
void *MdsCsvParser_EncodeReq(SMsgHeadT *pReqHead,
 ...
 */
MdsMktReqMsgBodyT *MdsCsvParser_DecodeReq(SMsgHeadT *pReqHead,
                                          const void *pMsgBody,
 ...
 * @return          编码后的消息体数据; NULL, 编码失败
 */
void *MdsCsvParser_EncodeRsp(SMsgHeadT *pRspHead,
 ...
 * @return          解码后的消息体数据; NULL, 解析失败
 */
MdsMktRspMsgBodyT *MdsCsvParser_DecodeRsp(SMsgHeadT *pRspHead,
                                          const void *pMsgBody,
 ...
 * @return          字段列表的长度
 */
int32 MdsCsvParser_GetRspFields(uint8 msgType, char *pBuf, int32 bufSize);
/* -------------------------           */
 ...
 *          小于0, 处理失败 (负的错误号)
 */
int32 MdsCsvParser_ParseCsvFile(const char *pCsvFile,
                                F_MDSAPI_ONMSG_T fnOnMsgCallback,
                                void *pCallbackParams);

/**
 * 解析完整的CVS格式行情数据记录 (形如: MsgId,value1,value2,...
 * 的CSV行情数据记录)
 *
 ...
 * @return          解码后的消息体数据; NULL, 解析失败
 */
MdsMktRspMsgBodyT *MdsCsvParser_DecodeCsvRecord(const char *pCsvRecord,
 ...
 * @return  序列化后的消息体数据长度
 */
int32 MdsCsvParser_EncodeStockStaticItem(const MdsStockStaticInfoT *pItem,
 ...
 * @return  序列化后的消息体数据长度
 */
int32 MdsCsvParser_EncodeOptionStaticItem(const MdsOptionStaticInfoT *pItem,
                                          char *pBuf, int32 bufSize);
 ...
 * @return  序列化后的消息体数据长度
 */
int32 MdsCsvParser_EncodeSnapshotListItem(const MdsL1SnapshotT *pItem,
                                          char *pBuf, int32 bufSize);
/* -------------------------           */
