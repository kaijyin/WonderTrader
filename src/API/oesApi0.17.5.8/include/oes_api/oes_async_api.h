 ...
 *          - 增加辅助的异步API接口
 *              - OesAsyncApi_SendReportSynchronization, 发送回报同步消息
 * (仅适用于回报通道)
 *              - OesAsyncApi_IsAllTerminated,
 * 返回异步API相关的所有线程是否都已经安全退出
 * @version 0.15.11.6   2020/07/16
 *          - 增加辅助的异步API接口
 *              - OesAsyncApi_GetAsyncQueueTotalCount,
 * 返回异步API累计已入队的消息数量
 *              - OesAsyncApi_GetAsyncQueueRemainingCount,
 * 返回队列中尚未被处理的剩余数据数量
 * @version 0.15.11.16  2021/02/23
 *          - 增加辅助的异步API接口,
 * 以支持对通信线程、回调线程等异步API线程进行初始化处理
 ...
 *          - 增加辅助的异步API接口
 *              - OesAsyncApi_IsBuiltinQueryChannelConnected,
 * 返回内置的查询通道是否已连接就绪
 ...
 *              - 删除 '期权账户结算单确认接口
 * (OesAsyncApi_SendOptSettlementConfirmReq)' 中的无效参数
 * @version 0.17.0.9    2021/04/27
 *          - 增加查询接口的包裹函数 (基于异步API内置的查询通道执行)
 *              - OesAsyncApi_SendCreditRepayReq,
 * 可以指定待归还合约编号的融资融券负债归还请求
 *              - OesAsyncApi_SendCreditCashRepayReq, 直接还款(现金还款)请求
 *              - OesAsyncApi_QueryCrdCreditAsset, 查询信用资产信息
 *              - OesAsyncApi_QueryCrdCashPosition, 查询融资融券资金头寸信息
 * (可融资头寸信息)
 *              - OesAsyncApi_QueryCrdSecurityPosition, 查询融资融券证券头寸信息
 * (可融券头寸信息)
 ...
 *              - OesAsyncApi_QueryCrdSecurityDebtStats,
 * 查询融资融券客户单证券负债统计信息
 ...
 *          - 增加查询接口的包裹函数 (基于异步API内置的查询通道执行)
 *              - OesAsyncApi_QueryCrdUnderlyingInfo,
 * 查询客户的融资融券可充抵保证金证券及融资融券标的信息
 * @version 0.17.1.1    2021/06/29
 *          - 增加查询接口的包裹函数 (基于异步API内置的查询通道执行)
 *              - OesAsyncApi_GetCrdDrawableBalance,
 * 查询融资融券业务最大可取资金
 *              - OesAsyncApi_GetCrdCollateralTransferOutMaxQty,
 * 查询融资融券担保品可转出的最大数量
 * @version 0.17.2      2021/08/02
 *          - 增加查询接口的包裹函数 (基于异步API内置的查询通道执行)
 *              - OesAsyncApi_QueryColocationPeerCashAsset,
 * 查询两地交易时对端结点的资金资产信息
 *
 * @since   2019/11/17
 */
...
#define OESAPI_CFG_DEFAULT_KEY_CPUSET_COMMUNICATION "oesapi_report"
/** 默认的异步API线程的CPU亲和性配置项名称 (异步回调线程) */
#define OESAPI_CFG_DEFAULT_KEY_CPUSET_CALLBACK "oesapi_callback"
/** 默认的异步API线程的CPU亲和性配置项名称 (I/O线程) */
#define OESAPI_CFG_DEFAULT_KEY_CPUSET_IO_THREAD "oesapi_io_thread"
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_ASYNC_CHANNEL_CFG NULLOBJ_SPK_ENDPOINT_CHANNEL_CFG
/* -------------------------           */

/**
 * OES异步API的I/O线程配置信息 (用于异步落地回报数据,
 * 可以通过配置文件开启或禁用)
 */
typedef SEndpointIoThreadCfgT OesAsyncApiIoThreadCfgT;

/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_ASYNC_IO_THREAD_CFG NULLOBJ_SPK_ENDPOINT_IO_THREAD_CFG
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_ASYNC_CONTEXT NULLOBJ_SPK_ENDPOINT_CONTEXT
/* -------------------------           */
 ...
 * - 将通过 lastOutMsgSeq 字段存储上一次会话实际已发送的出向消息序号, 即:
 * 服务器端最 后接收到的客户委托流水号(clSeqNo)
 * - 该值对应的是服务器端最后接收到并校验通过的(同一环境号下的)客户委托流水号,
 * 效果等价 于 OesApi_InitOrdChannel接口的pLastClSeqNo参数的输出值
 * - 登录成功以后, API会将该值存储在 <code>pAsyncChannel->lastOutMsgSeq</code>
 *   和 <code>pSessionInfo->lastOutMsgSeq</code> 字段中
 * - 该字段在登录成功以后就不会再更新
 * - 客户端若想借用这个字段来维护自增的"客户委托流水号(clSeqNo)"也是可行的,
 * 只是需要注
 *   意该字段在登录后会被重置为服务器端最后接收到并校验通过的"客户委托流水号(clSeqNo)"
 * </p>
 *
 * <p> @note 关于最近接收到的回报数据编号 (针对回报通道):
 * - 将通过 lastInMsgSeq 字段存储上一次会话实际接收到的入向消息序号, 即:
 * 最近接收到的 回报数据编号
 * - 该字段会在接收到回报数据并回调OnMsg成功以后实时更新,
 * 可以通过该字段获取到最近接收到 的回报数据编号
 * - 当OnConnect函数指针为空时, 会执行默认的回报订阅处理,
 * 此时会自动从断点位置继续订阅 回报数据
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_ASYNC_CHANNEL NULLOBJ_SPK_ENDPOINT_CHANNEL
/* -------------------------           */
...
typedef struct _OesAsyncApiContextParams {
  ...
  /** 是否使用忙等待模式 (TRUE:延迟更低但CPU会被100%占用;
   * FALSE:延迟和CPU使用率相对均衡) */
  ...
  /** 是否启用内置的查询通道 (TRUE:启动异步API时自动创建内置的查询通道;
   * FALSE:不创建内置的查询通道) */
...
/* 结构体初始化值定义 */
#define NULLOBJ_OESAPI_ASYNC_CONTEXT_PARAMS 0, 0, 0, 0, 0, 0, {0}
 ...
 * - 对消息体数据(pMsgItem),
 * 需要按照消息类型(pMsgHead->msgId)转换为对应的消息结构进行处理
 ...
 *
 * <p> 回调函数说明:
 * - 对于回报通道, 需要通过该回调函数完成回报订阅操作。若函数指针为空,
 * 则会使用通道配置中默认的 回报订阅参数进行订阅。若函数指针不为空,
 * 但未订阅回报, 90秒以后服务器端会强制断开连接
 ...
 */
typedef int32 (*F_OESAPI_ASYNC_ON_CONNECT_T)(OesAsyncApiChannelT *pAsyncChannel,
...
typedef int32 (*F_OESAPI_ASYNC_ON_DISCONNECT_T)(
    OesAsyncApiChannelT *pAsyncChannel, void *pCallbackParams);
...
typedef int32 (*F_OESAPI_ASYNC_ON_THREAD_START_T)(
    OesAsyncApiContextT *pAsyncContext, void *pCallbackParams);
/* -------------------------           */
 ...
 *
 * - 与 OesAsyncApi_CreateContextSimple 接口的区别为,
 * 会通过配置文件加载如下配置:
 *   - 日志配置
 *   - 异步API相关的扩展配置:
 * 异步队列的大小、是否启动独立的回调线程来执行回调处理、是否使用 忙等待模式等
 ...
 */
OesAsyncApiContextT *OesAsyncApi_CreateContext(const char *pCfgFile);
 ...
 *                              - 为空则忽略, 不初始化日志记录器
 * @param   pAsyncApiSection    异步API扩展配置参数的配置区段名称 (e.g.
 ...
 * @return  非空, 异步API的运行时环境指针; NULL, 失败
 */
OesAsyncApiContextT *OesAsyncApi_CreateContext2(const char *pCfgFile,
 ...
 * @param   pLoggerSection      日志记录器的配置区段名称 (e.g. "log")
 *                              - 为空则使用默认值
 * @param   asyncQueueSize      用于缓存回报数据的消息队列大小
 * (最大可缓存的消息数量)
 *                              - 等于0, 不创建缓存队列
 * (无需异步处理或落地回报数据)
 *                              - 小于0, 使用默认值 100000
 * @return  非空, 异步API的运行时环境指针; NULL, 失败
 */
OesAsyncApiContextT *OesAsyncApi_CreateContextSimple(const char *pLoggerCfgFile,
                                                     const char *pLoggerSection,
                                                     int32 asyncQueueSize);
 ...
 * @return  非空, 异步API的运行时环境指针; NULL, 失败
 */
OesAsyncApiContextT *OesAsyncApi_CreateContextSimple2(
    const char *pLoggerCfgFile, const char *pLoggerSection,
 ...
 */
void OesAsyncApi_ReleaseContext(OesAsyncApiContextT *pAsyncContext);
 ...
 */
BOOL OesAsyncApi_Start(OesAsyncApiContextT *pAsyncContext);
 ...
 * @param   pAsyncContext       异步API的运行时环境指针
 */
void OesAsyncApi_Stop(OesAsyncApiContextT *pAsyncContext);
 ...
 */
BOOL OesAsyncApi_IsRunning(OesAsyncApiContextT *pAsyncContext);
 ...
 */
BOOL OesAsyncApi_IsAllTerminated(OesAsyncApiContextT *pAsyncContext);
 ...
 */
int64 OesAsyncApi_GetTotalPicked(OesAsyncApiContextT *pAsyncContext);
 ...
 * @return  累计已入队的消息数量
 */
int64 OesAsyncApi_GetAsyncQueueTotalCount(OesAsyncApiContextT *pAsyncContext);
 ...
 */
int32 OesAsyncApi_GetChannelCount(OesAsyncApiContextT *pAsyncContext);
 ...
 * @return  已连接的通道数量
 */
int32 OesAsyncApi_GetConnectedChannelCount(OesAsyncApiContextT *pAsyncContext);
 ...
 */
OesAsyncApiChannelT *OesAsyncApi_AddChannel(
    OesAsyncApiContextT *pAsyncContext, eOesApiChannelTypeT channelType,
    const char *pChannelTag, const OesApiRemoteCfgT *pRemoteCfg,
    const OesApiSubscribeInfoT *pSubscribeCfg, F_OESAPI_ASYNC_ON_MSG_T fnOnMsg,
    void *pOnMsgParams, F_OESAPI_ASYNC_ON_CONNECT_T fnOnConnect,
    void *pOnConnectParams, F_OESAPI_ASYNC_ON_DISCONNECT_T fnOnDisconnect,
 ...
 * @return      非空, 异步API的连接通道信息; 空, 失败
 */
OesAsyncApiChannelT *OesAsyncApi_AddChannelFromFile(
    OesAsyncApiContextT *pAsyncContext, eOesApiChannelTypeT channelType,
    const char *pChannelTag, const char *pCfgFile, const char *pCfgSection,
    const char *pAddrKey, F_OESAPI_ASYNC_ON_MSG_T fnOnMsg, void *pOnMsgParams,
    F_OESAPI_ASYNC_ON_CONNECT_T fnOnConnect, void *pOnConnectParams,
    F_OESAPI_ASYNC_ON_DISCONNECT_T fnOnDisconnect, void *pOnDisconnectParams);
 ...
 *                              - 大于0: 返回与指定顺序号相对应的,
 * 并且与指定通道类型相 匹配的通道信息 (顺序号与通道配置的添加顺序一致)
 *                              - 小于0:
 * 返回第一个与指定通道类型相匹配的通道信息
 *                              - INT_MAX:
 * 返回最后一个与指定通道类型相匹配的通道信息
 * @return  异步API的连接通道信息
 */
OesAsyncApiChannelT *OesAsyncApi_GetChannel(OesAsyncApiContextT *pAsyncContext,
                                            int32 channelType,
                                            int32 channelIndex);
 ...
 */
OesAsyncApiChannelT *
OesAsyncApi_GetChannelByTag(OesAsyncApiContextT *pAsyncContext,
                            int32 channelType, const char *pChannelTag);
 ...
 * @return  大于等于0, 成功遍历到的通道数量; 小于0,
 * 参数错误或者回调函数的返回值小于0
 */
int32 OesAsyncApi_ForeachChannel(
    OesAsyncApiContextT *pAsyncContext, int32 channelType,
    int32 (*fnCallback)(OesAsyncApiChannelT *pAsyncChannel, void *pParams),
    void *pParams);
...
int32 OesAsyncApi_ForeachChannel2(
    OesAsyncApiContextT *pAsyncContext, int32 channelType,
    int32 (*fnCallback)(OesAsyncApiChannelT *pAsyncChannel, void *pParam1,
                        void *pParam2),
 ...
 */
BOOL OesAsyncApi_IsChannelConnected(OesAsyncApiChannelT *pAsyncChannel);
...
OesAsyncApi_GetChannelCfg(OesAsyncApiChannelT *pAsyncChannel);
...
OesAsyncApi_GetChannelSubscribeCfg(OesAsyncApiChannelT *pAsyncChannel);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_RegisterSymbioticSession(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_SendOrderReq(OesAsyncApiChannelT *pAsyncOrdChannel,
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesAsyncApi_SendOrderCancelReq(OesAsyncApiChannelT *pAsyncOrdChannel,
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesAsyncApi_SendBatchOrdersReq(OesAsyncApiChannelT *pAsyncOrdChannel,
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesAsyncApi_SendBatchOrdersReq2(OesAsyncApiChannelT *pAsyncOrdChannel,
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesAsyncApi_SendFundTransferReq(OesAsyncApiChannelT *pAsyncOrdChannel,
 ...
 * - 行为与 OesAsyncApi_SendOrderReq 接口完全一致,
 * 只是可以额外指定待归还的合约编号和归还模式
 * - 如果不需要指定待归还的合约编号和归还模式, 也直接可以使用
 * OesAsyncApi_SendOrderReq 接口 完成相同的工作
 ...
 * - 直接还券
 *
 * @note 本接口不支持直接还款, 直接还款需要使用
 * OesAsyncApi_SendCreditCashRepayReq 接口
 *
 * @param       pAsyncOrdChannel
 *                              异步API的委托连接通道
 * @param       pOrdReq         待发送的委托申报请求
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesAsyncApi_SendCreditRepayReq(OesAsyncApiChannelT *pAsyncOrdChannel,
                                     const OesOrdReqT *pOrdReq,
 ...
 * - 直接还款
 *
 * @note 直接还券、卖券还款、买券还券需要使用 OesAsyncApi_SendCreditRepayReq
 * 接口
 *
 * @param       pAsyncOrdChannel
 *                              异步API的委托连接通道
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesAsyncApi_SendCreditCashRepayReq(OesAsyncApiChannelT *pAsyncOrdChannel,
 ...
 *                              - 小于0, 从上次接收到的断点位置开始
 *                                  - @note 此处与同步API不同. 小于0时,
 * 同步API为从最 新的数据开始推送回报数据
 *                                  -
 * 如果明确需要只从最新的数据开始推送回报数据, 可以指 定一个特别大的数 (例如
 * INT_MAX)
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SendReportSynchronization(
    OesAsyncApiChannelT *pAsyncRptChannel, int8 subscribeEnvId,
    int32 subscribeRptTypes, int64 lastRptSeqNum);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SendHeartbeat(OesAsyncApiChannelT *pAsyncChannel);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SendTestReq(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * - 也可以直接使用该接口作为 OnConnect 回调函数 (与不设置 OnConnect
 * 回调函数的效果相同)
 * </p>
 *
 * @param   pAsyncChannel       异步API的连接通道信息
 * @param   pCallbackParams     外部传入的参数 (不会使用该参数, 传任意值或 NULL
 * 均可)
 * @retval  =0                  等于0, 成功
 * @retval  >0                  大于0, 处理失败, 将重建连接并继续尝试执行
 * @retval  <0                  小于0, 处理失败, 异步线程将中止运行
 */
int32 OesAsyncApi_DefaultOnConnect(OesAsyncApiChannelT *pAsyncChannel,
                                   void *pCallbackParams);
/* -------------------------           */
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_LoadCpusetCfg(OesAsyncApiContextT *pAsyncContext,
 ...
 *                              通信线程绑定的CPU列表
 * (以逗号或空格分割的CPU编号列表, CPU编号从1开始)
 *                              - e.g. "1"      绑定到 0 号核
 *                              - e.g. "1,3,5"  绑定到 0,2,4 号核
 *                              - NULL 或者 "", 不绑定CPU
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetCommunicationCpusetCfg(OesAsyncApiContextT *pAsyncContext,
 ...
 */
const char *
OesAsyncApi_GetCommunicationCpusetCfg(OesAsyncApiContextT *pAsyncContext);
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetCallbackThreadCpusetCfg(OesAsyncApiContextT *pAsyncContext,
 ...
 */
const char *
OesAsyncApi_GetCallbackThreadCpusetCfg(OesAsyncApiContextT *pAsyncContext);

/**
 * 设置异步I/O线程的CPU亲和性配置
 *
 * @param[out]  pAsyncContext   异步API的运行时环境指针
 * @param       pIoThreadCpuset 异步I/O线程绑定的CPU列表
 * (以逗号或空格分割的CPU编号列表, CPU编号从1开始)
 *                              - e.g. "1"      绑定到 0 号核
 *                              - e.g. "1,3,5"  绑定到 0,2,4 号核
 *                              - NULL 或者 "", 不绑定CPU
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetIoThreadCpusetCfg(OesAsyncApiContextT *pAsyncContext,
 ...
 */
const char *
OesAsyncApi_GetIoThreadCpusetCfg(OesAsyncApiContextT *pAsyncContext);
 ...
 *                              TRUE: 启动前预创建并校验所有的连接,
 * 如果连接失败则中止启动 FALSE: 启动前不预先创建和校验连接 (默认行为)
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetPreconnectAble(OesAsyncApiContextT *pAsyncContext,
 ...
 */
BOOL OesAsyncApi_IsPreconnectAble(OesAsyncApiContextT *pAsyncContext);
 ...
 * (默认行为)
 *                              - 1: 无需创建独立的通信线程,
 * 启动时将接管当前线程并直接 在当前线程下执行通信处理
 *                              - CHAR_MAX: 不创建通信线程,
 * 也不执行除OnConnect回调以 外的任何消息接收等异步处理,
 * 完全由当前线程控制后续的执 行 (仅适用于测试场景, 正常情况下不会使用)
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetTakeoverStartThreadFlag(OesAsyncApiContextT *pAsyncContext,
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetAsyncCallbackAble(OesAsyncApiContextT *pAsyncContext,
 ...
 */
BOOL OesAsyncApi_IsAsyncCallbackAble(OesAsyncApiContextT *pAsyncContext);
 ...
 *                              TRUE: 使用忙等待模式, 延迟更低,
 * 但CPU会被100%占用 FALSE: 不使用忙等待模式, 延迟和CPU使用率相对均衡
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetAsyncCallbackBusyPollAble(
    OesAsyncApiContextT *pAsyncContext, BOOL isBusyPollAble);
 ...
 */
int64 OesAsyncApi_GetAsyncQueueLength(OesAsyncApiContextT *pAsyncContext);
 ...
 */
int64 OesAsyncApi_GetAsyncQueueDataAreaSize(OesAsyncApiContextT *pAsyncContext);
 ...
 *                              是否启用内置的查询通道
 *                              - 如果将该参数设置为TRUE,
 * 则启动异步API时将自动创建与查 询服务的连接
 *                              - 如果不需要通过异步API查询数据的话,
 * 可以将该参数设置为 FALSE, 这样可以避免额外占用查询通道的连接数量
 *                              - 不指定的话, 默认为FALSE
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetBuiltinQueryable(OesAsyncApiContextT *pAsyncContext,
 ...
 * @return  是否启用内置的查询通道
 */
BOOL OesAsyncApi_IsBuiltinQueryable(OesAsyncApiContextT *pAsyncContext);
 ...
 */
OesApiSessionInfoT *
OesAsyncApi_GetBuiltinQueryChannelRef(OesAsyncApiChannelT *pAsyncChannel);
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_SetIoThreadCfg(OesAsyncApiContextT *pAsyncContext,
                                BOOL isEnabled, BOOL isOutputSimplify,
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesAsyncApi_LoadIoThreadCfg(OesAsyncApiContextT *pAsyncContext,
                                 const char *pCfgFile, const char *pCfgSection);
...
OesAsyncApi_GetIoThreadCfg(OesAsyncApiContextT *pAsyncContext);
...
BOOL OesAsyncApi_SetOnCommunicationThreadStart(
    OesAsyncApiContextT *pAsyncContext,
    F_OESAPI_ASYNC_ON_THREAD_START_T fnOnThreadStart, void *pCallbackParams);
 ...
 */
const char *OesAsyncApi_GetApiVersion();
 ...
 */
int32 OesAsyncApi_GetTradingDay(OesAsyncApiChannelT *pAsyncChannel);
 ...
 */
int32 OesAsyncApi_GetClientOverview(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see     OesCustItemT
 */
int32 OesAsyncApi_QueryCustInfo(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see     OesInvAcctItemT
 */
int32 OesAsyncApi_QueryInvAcct(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see     OesStockItemT
 */
int32 OesAsyncApi_QueryStock(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see     OesIssueItemT
 */
int32 OesAsyncApi_QueryIssue(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see     OesEtfItemT
 */
int32 OesAsyncApi_QueryEtf(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_QueryEtfComponent(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_QueryCashAsset(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see         OesCashAssetItemT
 */
int32 OesAsyncApi_QuerySingleCashAsset(OesAsyncApiChannelT *pAsyncChannel,
...
int32 OesAsyncApi_QueryColocationPeerCashAsset(
    OesAsyncApiChannelT *pAsyncChannel, const char *pCashAcctId,
 ...
 */
int32 OesAsyncApi_QueryCounterCash(OesAsyncApiChannelT *pAsyncChannel,
                                   const char *pCashAcctId,
 ...
 */
int32 OesAsyncApi_QueryStkHolding(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see         OesStkHoldingItemT
 */
int32 OesAsyncApi_QuerySingleStkHolding(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_QueryLotWinning(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see     OesOrdItemT
 */
int32 OesAsyncApi_QueryOrder(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see         OesOrdItemT
 */
int32 OesAsyncApi_QuerySingleOrder(OesAsyncApiChannelT *pAsyncChannel,
                                   int32 clSeqNo, OesOrdItemT *pOutOrdItem);
 ...
 * @see     OesTrdItemT
 */
int32 OesAsyncApi_QueryTrade(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_QueryMarketState(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_QueryNotifyInfo(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_QueryBrokerParamsInfo(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see     OesOptionItemT
 */
int32 OesAsyncApi_QueryOption(OesAsyncApiChannelT *pAsyncChannel,
 ...
 */
int32 OesAsyncApi_QueryOptHolding(OesAsyncApiChannelT *pAsyncChannel,
 ...
 * @see         OesOptHoldingItemT
 */
int32 OesAsyncApi_QuerySingleOptHolding(OesAsyncApiChannelT *pAsyncChannel,
                                        const char *pInvAcctId,
...
int32 OesAsyncApi_QueryOptSettlementStatement(
    OesAsyncApiChannelT *pAsyncChannel, const char *pCustId,
    char *pOutSettlInfoBuf, int32 bufSize);
/* -------------------------           */

/* ===================================================================
 * 融资融券业务特有的查询接口 (同步API查询接口的包裹函数,
 * 并基于内置的查询通道执行)
 * =================================================================== */
 ...
 * @see     OesStkHoldingItemT
 */
int32 OesAsyncApi_QueryCrdHolding(OesAsyncApiChannelT *pAsyncChannel,
                                  const OesQryStkHoldingFilterT *pQryFilter,
                                  F_OESAPI_ON_QRY_MSG_T fnQryMsgCallback,
                                  void *pCallbackParams);
 ...
 * @see         OesStkHoldingItemT
 */
int32 OesAsyncApi_QuerySingleCrdHolding(OesAsyncApiChannelT *pAsyncChannel,
                                        const char *pInvAcctId,
                                        const char *pSecurityId,
                                        OesStkHoldingItemT *pOutHoldingItem);
...
int64 OesAsyncApi_GetCrdCollateralTransferOutMaxQty(
    OesAsyncApiChannelT *pAsyncChannel, const char *pSecurityId, uint8 mktId,
