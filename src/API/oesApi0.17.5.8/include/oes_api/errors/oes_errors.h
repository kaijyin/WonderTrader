 ...
 *              - 1339, 非法的标的锁定指令类型
 * (OESERR_ILLEGAL_UNDERLYING_LOCK_TYPE)
 ...
 *              - 1404, 券商可用两融总额度不足
 * (OESERR_BROKER_CREDIT_TRADE_QUOTA)
 ...
 * (OESERR_CHIEF_OR_MAJOR_CREDIT_LIMIT)
 *              - 1425, 禁止提交限售股份为担保物
 * (OESERR_RESTRICTED_STOCKS_TO_COLLATERAL)
 *              - 1426, 禁止个人投资者提交解除限售存量股份为担保物
 * (OESERR_INDIVIDUAL_RESTRICTED_STOCKS_LIFTED_TO_COLLATERAL)
 *              - 1427, 禁止大小非开展该证券的融资融券交易
 * (OESERR_NON_TRADABLE_SHARE_CREDIT_LIMIT)
 *              - 1428, 禁止其他股东角色开展该证券的融资融券交易
 * (OESERR_OTHER_ROLES_CREDIT_LIMIT)
 ...
 *              - 1436, 非法的融资融券归还模式
 * (OESERR_ILLEGAL_CREDIT_REPAY_MODE)
 *              - 1437, 非法的担保证券划转指令类型
 * (OESERR_ILLEGAL_COLLATERAL_TRANSFER_TYPE)
 ...
 * (OESERR_REPAY_WRONG_STOCK)
 *              - 1442, 还券数量超过融券合约待归还数量
 * (OESERR_REPAY_STOCK_ORD_QTY_RANGE)
 *              - 1443, 未找到融资融券合约信息
 * (OESERR_CRD_DEBT_CONTRACT_NOT_FOUND)
 ...
 * (OESERR_BROKER_CRD_SZSE_PBU_NOT_CONFIG)
...
/** 1501, 客户单一证券持仓集中度超限 */
#define OESERR_CONCENTRATE_RATE_SECURITY_HOLDING (&__ERRS_oesApiErrors[260 + 1])
/** 1502, 客户板块持仓集中度超限 */
