 ...
 *          - 删除 OesApi_IsBusinessError 接口, 查询接口不再返回小于 -1000
 * 的错误
 *          - 查询接口返回值变更:
 *              - 单条查询接口无数据返回 SPK_NEG(ENOENT)
 *              - 批量查询接口无匹配数据时返回 0
 *              - 单条/批量 查询被服务器拒绝返回 SPK_NEG(EINVAL),
 * 具体错误信息通过日志打印
 *          - 微调oes_api日志打印格式
 * @version 0.12.3      2017/01/10
 *          - 精简API依赖的头文件, 并最小化API发布包中的头文件数量
 *          - 重命名 protocol_parser/errors/oes_protocol_errors.h ==>
 * errors/oes_errors.h
 *          - 从发布包中删除协议解析相关的头文件, 客户端如果需要的话,
 * 可以分别发布
 ...
 *              - 参数四的类型变更 OesHoldItemT ->
 * OesStkHoldingItemT(原OesStockHoldInfoT)
 *          - OesApi_QuerySingleOptHolding 参数类型变更:
 *              - 参数四的类型变更 OesHoldItemT ->
 * OesOptHoldingItemT(原OesOptionHoldInfoT)
 ...
 *              - 参数三 pMsgBody 返回数据类型变更 OesHoldItemT ->
 * OesStkHoldingItemT
 ...
 *          - 新增接口 '重置线程级别的日志记录器名称
 * (OesApi_ResetThreadLoggerName)', 以支持为当前线程设置独立的日志文件
 * @version 0.15.4      2017/08/22
 *          - 增加客户端环境号 (clEnvId),
 * 涉及到以下函数接口、数据结构、通信报文的变更:
 ...
 *                  - 回报同步请求/应答消息 (OesReportSynchronizationReqT /
 ...
 *          - 新增接口 '重置线程级别的日志记录器名称
 * (OesApi_ResetThreadLoggerName)', 以支持为当前线程设置独立的日志文件
 *          - 重命名接口函数 OesApi_SendFundTrsfReq ->
 ...
 *              - 相比原来的撤单方式, 撤单请求接口支持通过 clEnvId + clSeqNo
 * 进行撤单
 *          - 调整回报接口回调方法类型定义 F_OESAPI_ONMSG_T ->
 * F_OESAPI_ON_RPT_MSG_T
 *          - 调整查询接口回调方法类型定义 F_OESAPI_ONMSG_T ->
 * F_OESAPI_ON_QRY_MSG_T
 *          - 查询接口回调方法中增加 OesQryCursorT 类型参数, 其中携带
 * "是否是最后一条" 信息
 * @version 0.15.4.1    2017/09/25
 *          - 服务器端删除了对JSON等通信协议等支持, 改为只支持二进制协议
 * @version 0.15.4.2    2017/10/16
 *          - 回报同步请求处理改成异步应答模式, 回报通道增加消息类型
 * OESMSG_RPT_REPORT_SYNCHRONIZATION
 * @version 0.15.5      2017/11/03
 *          - 新增接口 '获取通道绑定clEnvId (OesApi_GetClEnvId)'
 * @version 0.15.5.1    2017/11/17
 *          - 增加 OesApi_IsValidOrdChannel、OesApi_IsValidRptChannel 等接口,
 * 用于判断通道是否已经连接且有效
 * @version 0.15.5.2    2018/01/29
 *          - 增加 OesApi_GetLastRecvTime、OesApi_GetLastSendTime 接口,
 * 用于获取通道最近发送/接收消息的时间
 ...
 *          - 增加 OesApi_HasMoreCachedData 接口,
 * 用于返回已经接收到但尚未被回调函数处理的缓存数据长度
 ...
 *              - OesApi_GetLastError
 *          - '成交查询应答报文(OesQryTrdRspT)' 和
 * '成交回报应答报文(OesTrdCnfmT)' 结构体中增加字段:
 ...
 *          - 调整 '出入金委托请求 (OesApi_SendFundTransferReq)' 接口,
 * 增加支持仅主柜和银行间的银证转账
 ...
 *          - 新增 ‘市场状态信息查询(OesApi_QueryMarketState)’ 接口,
 * 此接口获取到的 市场状态信息类型定义为 ‘OesMarketStateItemT’
 ...
 *          - 新增 '判断消息头中的状态码与错误信息条目是否匹配
 * (OesApi_IsErrorOf2)' 接口
 ...
 *          - 增加辅助的配置文件解析接口
 *              - OesApi_ParseConfigFromFile2, 解析客户端配置文件,
 * 并可以指定是否允许配置项为空
 ...
 *          -
 * 将会话信息中的发送时间(lastSendTime)和接收时间(lastRecvTime)升级为纳秒级时间戳
 * (STimevalT => STimespecT)
 ...
 *              - OesApi_SetRetainExtDataAble,
 * 设置是否保留(不清空)由应用层自定义使用的扩展存储空间数据 (__extData)
 *              - OesApi_IsRetainExtDataAble,
 * 返回是否保留(不清空)由应用层自定义使用的扩展存储空间数据
 *              - OesApi_SetThreadBusinessType,
 * 设置当前线程登录OES时所期望对接的业务类型 (正常情况下无需调用)
 ...
 *          - 增加以下辅助接口
 *              - OesApi_SetTimeOffsetMeasurable,
 * 设置登录服务器时是否自动计算和服务器之间的时间偏差 (默认为是)
 *              - OesApi_IsTimeOffsetMeasurable,
 * 返回登录服务器时是否自动计算和服务器之间的时间偏差
 ...
 *          - 新增 '查询期权结算单信息 (OesApi_QueryOptSettlementStatement)'
 * 接口
 *          - 新增 '期权账户结算单确认 (OesApi_SendOptSettlementConfirmReq)'
 * 接口
 ...
 *          - 新增 '返回会话是否支持指定的业务类别 (OesApi_IsBusinessSupported)'
 * 接口
 ...
 *              - 新增 当日最大可减持额度 (maxReduceQuota) 字段
 *          - '资金变动回报 (OesCashAssetReportT)' 和 '客户资金信息
 * (OesCashAssetItemT)' 结构字段变更:
 *              - 新增 未对冲实时价格保证金 (totalMarketMargin) 字段
 *              - 新增 已对冲实时价格保证金 (totalNetMargin) 字段
 * @version 0.16.2.3    2021/08/06
 *              - 删除 '异步发送期权账户结算单确认接口
 * (OesApi_SendOptSettlementConfirmReqAsynced)' 中的无效参数
 * @version 0.17    2020/04/17
 *          - '现货产品基础信息 (OesStockBaseInfoT)' 结构字段变更:
 *              - 新增 融资融券可充抵保证金证券的交易状态
 * (isCrdCollateralTradable) 字段
 ...
 *              - 新增 业务类型 (businessType) 字段
 *          - '股票持仓回报和股票持仓信息
 * (OesStkHoldingReportT/OesStkHoldingItemT)' 结构字段变更:
 *              - 新增 信用持仓标识 (isCreditHolding) 字段
 *              - 新增 可直接还券数量 (repayStockDirectAvlHld) 字段
 *              - 新增 直接还券冻结数量 (collateralRepayDirectQty) 字段
 *              - 新增 专项证券头寸可用数量
 * (specialSecurityPositionAvailableQty) 字段
 *          - '资金变动回报和客户资金信息
 * (OesCashAssetReportT/OesCashAssetItemT)' 结构字段变更:
 ...
 *              - 新增 专项资金头寸可用余额 (specialCashPositionAvailableBal)
 * 字段
 *          - '券商参数信息 (OesBrokerParamsInfoT)' 结构字段变更:
 ...
 *                  - 新增 是否支持使用 '仅归还息费' 模式归还融资融券负债的息费
 * (isRepayInterestOnlyAble) 字段
 ...
 *          - 新增 回报消息类型 '融资融券合约流水回报信息
 * (OESMSG_RPT_CREDIT_DEBT_JOURNAL)'
 ...
 *          - '融资融券合约流水变动回报和融资融券合约流水信息
 * (OesCrdDebtJournalReportT/OesCrdDebtJournalItemT)' 结构字段变更:
 *              - 新增 融券合约流水的理论发生金额 (shortSellTheoryOccurAmt) 字段
 *          - 添加独立的信用持仓查询接口:
 *              - OesApi_QueryCrdHolding, 查询信用持仓信息
 *              - OesApi_QuerySingleCrdHolding, 查询单条信用持仓信息
 *          - 新增 ’查询融资融券可充抵保证金证券及融资融券标的信息
 * OesApi_QueryCrdUnderlyingInfo‘ 接口
 * @version 0.17.1.1    2021/06/29
 *          - 新增 '查询融资融券业务最大可取资金 OesApi_GetCrdDrawableBalance'
 * 接口
 *          - 新增 '查询融资融券担保品可转出的最大数量
 * OesApi_GetCrdCollateralTransferOutMaxQty' 接口
 * @version 0.17.2      2021/08/02
 *          - 新增 '查询两地交易时对端结点的资金资产信息
 * OesApi_QueryColocationPeerCashAsset' 接口
 * @version 0.17.2.1    2021/08/05
 *          - '客户端总览信息 (OesClientOverviewT)' 结构字段变更:
 *              - 新增 客户在本结点的初始资金资产占比(百分比)
 * (initialCashAssetRatio) 字段
 *              - 新增 是否支持两地交易内部资金划拨 (isSupportInternalAllot)
 * 字段
 ...
 *          - 优化出入金金额的精度处理, 自动将出入金金额的有效精度向下舍入分,
 * 以方便前端使用 (例如: 金额 1.9999 将被自动转换为 1.9900)
 * @version 0.17.5      2021/09/30
 *          - 新增 '设置客户端默认的委托方式 OesApi_SetDefaultEntrustWay' 接口
 *          - 新增 '获取客户端默认的委托方式 OesApi_GetDefaultEntrustWay' 接口
 *          - 新增 '设置客户端会话使用的委托方式 OesApi_SetAssignedEntrustWay'
 * 接口
 *          - 新增 '获取客户端会话使用的委托方式 OesApi_GetAssignedEntrustWay'
 * 接口
 *
 * @since   2016/03/04
 */
...
/** 默认的TCP连接的心跳间隔 (30秒, 如果超过2倍心跳时间没有接收到任何网络消息,
 * 则可以认为连接异常) */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_SESSION_INFO NULLOBJ_GENERAL_CLIENT_CHANNEL
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_CHANNEL_GROUP NULLOBJ_GENERAL_CLIENT_CHANNEL_GROUP
/* -------------------------           */
...
/* 结构体初始化值定义 */
#define NULLOBJ_OESAPI_ADDR_INFO NULLOBJ_GENERAL_CLIENT_ADDR_INFO
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_REMOTE_CFG NULLOBJ_GENERAL_CLIENT_REMOTE_CFG
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_ADDR_CURSOR NULLOBJ_GENERAL_CLIENT_ADDR_CURSOR
/* -------------------------           */
...
/* 结构体的初始化值定义 */
#define NULLOBJ_OESAPI_SUBSCRIBE_INFO 0, {0}, 0
/* -------------------------           */
...
#define NULLOBJ_OESAPI_CLIENT_CFG                                              \
  {NULLOBJ_OESAPI_REMOTE_CFG}, {NULLOBJ_OESAPI_REMOTE_CFG},                    \
...
#define NULLOBJ_OESAPI_CLIENT_ENV                                              \
  {NULLOBJ_OESAPI_SESSION_INFO}, {NULLOBJ_OESAPI_SESSION_INFO}, {              \
 ...
 * @see     OesRptMsgT
 */
typedef int32 (*F_OESAPI_ON_RPT_MSG_T)(OesApiSessionInfoT *pSessionInfo,
                                       SMsgHeadT *pMsgHead, void *pMsgItem,
 ...
 * @see     OesQryRspMsgT
 */
typedef int32 (*F_OESAPI_ON_QRY_MSG_T)(OesApiSessionInfoT *pQryChannel,
                                       SMsgHeadT *pMsgHead, void *pMsgItem,
 ...
 *
 * - 关于"客户委托流水号(clSeqNo)"
 *   - 在0.11之前的版本中, "客户委托流水号" 是由客户端维护的线性递增序列,
 * 用于标识委托数据的 唯一性, 并防止重复申报
 *   - 从0.11版本开始, 不再强制要求线性递增, 只要维持在同一客户端下的唯一性即可
 *   - 客户端可以通过登录后的会话信息中的 lastOutMsgSeq
 * 字段获取到其最后发送到OES服务器的 "客户委托流水号"
 *   - 当通过撤单请求接口进行撤单时, 可以直接通过待撤订单的
 * "客户委托流水号(clSeqNo)" 进行撤单
 *   - 此外, 当需要同时建立多个委托通道报单时, 可以通过 "客户端环境号(clEnvId)"
 * 对区分不同的 委托通道, "客户委托流水号(clSeqNo)" 只要在相同的
 * "客户端环境号(clEnvId)" 下唯一即可
 *
 * - 关于"原始订单的客户订单编号(origClOrdId)"
 *   - "客户订单编号(clOrdId)",
 * 是由OES服务器端维护的、在OES内具有唯一性的内部委托编号,
 *     该编号会通过回报数据返回客户端
 *   - 当通过撤单请求接口进行撤单时, 可以通过待撤订单的
 * "客户委托流水号(clSeqNo)" 或者 "客户订单编号(clOrdId)" 进行撤单
 *
 ...
 */
int32 OesApi_SendOrderReq(OesApiSessionInfoT *pOrdChannel,
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesApi_SendOrderCancelReq(OesApiSessionInfoT *pOrdChannel,
 ...
 *
 * - 批量委托的委托请求填充规则与单条委托完全相同,
 * 回报处理规则也与单条委托完全相同:
 *   - 每笔委托请求的 "客户委托流水号(clSeqNo)" 同样必须填充,
 * 并需要维持在同一客户端下的唯一性
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesApi_SendBatchOrdersReq(OesApiSessionInfoT *pOrdChannel,
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesApi_SendBatchOrdersReq2(OesApiSessionInfoT *pOrdChannel,
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesApi_SendFundTransferReq(OesApiSessionInfoT *pOrdChannel,
                                 const OesFundTrsfReqT *pFundTrsfReq);

/**
 * 发送密码修改请求 (修改客户端登录密码)
 * 密码修改请求将通过委托通道发送到OES服务器,
 * 并将采用同步请求/应答的方式直接返回处理结果
 *
 ...
 * - 行为与 OesApi_SendOrderReq 接口完全一致,
 * 只是可以额外指定待归还的合约编号和归还模式
 * - 如果不需要指定待归还的合约编号和归还模式, 也可以直接使用
 * OesApi_SendOrderReq 接口完成 相同的工作
 * - 同其它委托接口相同, 以单向异步消息的方式发送委托申报到OES服务器,
 * OES的实时风控检查等处理 结果将通过回报数据返回
 * - 回报数据也与普通委托的回报数据完全相同
 *
 * 支持的业务范围:
 * - 卖券还款
 ...
 * - 直接还券
 *
 * @note 本接口不支持直接还款, 直接还款需要使用 OesApi_SendCreditCashRepayReq
 * 接口
 *
 * @param       pOrdChannel     委托通道的会话信息
 * @param       pOrdReq         待发送的委托申报请求
 ...
 *                              - 如需归还融券合约的息费,
 * 需指定为'仅归还息费'模式(最终能否归还取决于券商是否支持该归还模式)
 * @param       pDebtId         归还的合约编号 (可以为空)
 *                              - 若为空, 则依次归还所有融资融券合约
 *                              - 若不为空, 则优先归还指定的合约编号,
 * 剩余的资金或股份再依次归还其它融资融券合约
 * @retval      0               成功
 * @retval      <0              失败 (负的错误号)
 *
 * @exception   EINVAL          传入参数非法
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesApi_SendCreditRepayReq(OesApiSessionInfoT *pOrdChannel,
                                const OesOrdReqT *pOrdReq,
 ...
 * - 专用于直接还款业务
 * - 行为上与其它委托接口相同, 以单向异步消息的方式发送委托申报到OES服务器,
 * OES的实时风控检查 等处理结果将通过回报数据返回
 ...
 *                              - 若不为空, 则优先归还指定的合约编号,
 * 剩余的资金再依次归还其它融资融券合约
 * @param       pUserInfo       用户私有信息 (可以为空, 由客户端自定义填充,
 * 并在回报数据中原样返回)
 ...
 * @exception   Others          由send()系统调用返回的错误
 */
int32 OesApi_SendCreditCashRepayReq(OesApiSessionInfoT *pOrdChannel,
                                    int32 clSeqNo, int64 repayAmt,
                                    eOesCrdAssignableRepayModeT repayMode,
 ...
 */
int32 OesApi_WaitReportMsg(OesApiSessionInfoT *pRptChannel, int32 timeoutMs,
 ...
 */
int32 OesApi_RecvReportMsg(OesApiSessionInfoT *pRptChannel,
                           SMsgHeadT *pOutMsgHead, OesRspMsgBodyT *pOutMsgBody,
                           int32 bufSize, int32 timeoutMs);
/* -------------------------           */
 ...
 */
const char *OesApi_GetApiVersion();
 ...
 * @retval  <0                  失败 (负的错误号)
 */
int32 OesApi_GetTradingDay(OesApiSessionInfoT *pQryChannel);
 ...
 * @retval      <0              失败 (负的错误号)
 */
int32 OesApi_GetClientOverview(OesApiSessionInfoT *pQryChannel,
 ...
 * @see     OesCustItemT
 */
int32 OesApi_QueryCustInfo(OesApiSessionInfoT *pQryChannel,
 ...
 * @see     OesInvAcctItemT
 */
int32 OesApi_QueryInvAcct(OesApiSessionInfoT *pQryChannel,
 ...
 * @see     OesStockItemT
 */
int32 OesApi_QueryStock(OesApiSessionInfoT *pQryChannel,
 ...
 * @see     OesIssueItemT
 */
int32 OesApi_QueryIssue(OesApiSessionInfoT *pQryChannel,
 ...
 * @see     OesEtfItemT
 */
int32 OesApi_QueryEtf(OesApiSessionInfoT *pQryChannel,
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesEtfComponentItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 */
int32 OesApi_QueryEtfComponent(OesApiSessionInfoT *pQryChannel,
 ...
 */
int32 OesApi_QueryCashAsset(OesApiSessionInfoT *pQryChannel,
 ...
 * @see         OesCashAssetItemT
 */
int32 OesApi_QuerySingleCashAsset(OesApiSessionInfoT *pQryChannel,
...
int32 OesApi_QueryColocationPeerCashAsset(
    OesApiSessionInfoT *pQryChannel, const char *pCashAcctId,
 ...
 */
int32 OesApi_QueryCounterCash(OesApiSessionInfoT *pQryChannel,
                              const char *pCashAcctId,
 ...
 */
int32 OesApi_QueryStkHolding(OesApiSessionInfoT *pQryChannel,
 ...
 * @see         OesStkHoldingItemT
 */
int32 OesApi_QuerySingleStkHolding(OesApiSessionInfoT *pQryChannel,
 ...
 */
int32 OesApi_QueryLotWinning(OesApiSessionInfoT *pQryChannel,
 ...
 * @see     OesOrdItemT
 */
int32 OesApi_QueryOrder(OesApiSessionInfoT *pQryChannel,
 ...
 * @see         OesOrdItemT
 */
int32 OesApi_QuerySingleOrder(OesApiSessionInfoT *pQryChannel, int32 clSeqNo,
 ...
 * @see     OesTrdItemT
 */
int32 OesApi_QueryTrade(OesApiSessionInfoT *pQryChannel,
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesFundTransferSerialItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
...
int32 OesApi_QueryFundTransferSerial(
    ...
    F_OESAPI_ON_QRY_MSG_T fnQryMsgCallback, void *pCallbackParams);
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCommissionRateItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 */
int32 OesApi_QueryCommissionRate(OesApiSessionInfoT *pQryChannel,
 ...
 */
int32 OesApi_QueryMarketState(OesApiSessionInfoT *pQryChannel,
 ...
 */
int32 OesApi_QueryNotifyInfo(OesApiSessionInfoT *pQryChannel,
 ...
 */
int32 OesApi_QueryBrokerParamsInfo(OesApiSessionInfoT *pQryChannel,
 ...
 * @see     OesOptionItemT
 */
int32 OesApi_QueryOption(OesApiSessionInfoT *pQryChannel,
 ...
 */
int32 OesApi_QueryOptHolding(OesApiSessionInfoT *pQryChannel,
 ...
 * @see         OesOptHoldingItemT
 */
int32 OesApi_QuerySingleOptHolding(OesApiSessionInfoT *pQryChannel,
                                   const char *pInvAcctId,
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesOptUnderlyingHoldingItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesOptPositionLimitItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesOptPurchaseLimitItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesOptExerciseAssignItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @retval      <0              失败 (负的错误号)
 */
int32 OesApi_QueryOptSettlementStatement(OesApiSessionInfoT *pQryChannel,
                                         const char *pCustId,
                                         char *pOutSettlInfoBuf, int32 bufSize);
/* -------------------------           */
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdCreditAssetItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 */
int32 OesApi_QueryCrdCreditAsset(OesApiSessionInfoT *pQryChannel,
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdUnderlyingInfoItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdCashPositionItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdSecurityPositionItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @see     OesStkHoldingItemT
 */
int32 OesApi_QueryCrdHolding(OesApiSessionInfoT *pQryChannel,
                             const OesQryStkHoldingFilterT *pQryFilter,
                             F_OESAPI_ON_QRY_MSG_T fnQryMsgCallback,
                             void *pCallbackParams);
 ...
 * @see         OesStkHoldingItemT
 */
int32 OesApi_QuerySingleCrdHolding(OesApiSessionInfoT *pQryChannel,
                                   const char *pInvAcctId,
                                   const char *pSecurityId,
                                   OesStkHoldingItemT *pOutHoldingItem);
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdDebtContractItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdDebtJournalItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 */
int32 OesApi_QueryCrdDebtJournal(OesApiSessionInfoT *pQryChannel,
 ...
 * 查询融资融券直接还款委托信息
 *
 * @note    直接还款委托的数据类型为 <code>OesCrdCashRepayItemT</code>,
 * 与普通委托不同
 *
 * @param   pQryChannel         查询通道的会话信息
 * @param   pQryFilter          查询过滤条件
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdCashRepayItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 */
int32 OesApi_QueryCrdCashRepayOrder(OesApiSessionInfoT *pQryChannel,
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdSecurityDebtStatsItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdExcessStockItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 */
int32 OesApi_QueryCrdExcessStock(OesApiSessionInfoT *pQryChannel,
 ...
 * @param   fnQryMsgCallback    进行消息处理的回调函数
 *                              - 消息体的数据类型为
 * <code>OesCrdInterestRateItemT</code>
 * @param   pCallbackParams     回调函数的参数
 * @retval  >=0                 成功查询到的记录数
 * @retval  <0                  失败 (负的错误号)
 *
 ...
 * @retval      TRUE            成功
 * @retval      FALSE           失败。此时 errno 将被设置, 可以通过
 * errno/SPK_GET_ERRNO() 获取到具体失败原因
 *
 ...
 */
BOOL OesApi_Logon(OesApiSessionInfoT *pOutSessionInfo,
                  eOesApiChannelTypeT channelType, const char *pUri,
                  const char *pUsername, const char *pPassword, int8 clEnvId,
                  int32 heartBtInt, const SSocketOptionConfigT *pSocketOptions);
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_LogonReplicaSet(OesApiSessionInfoT *pOutSessionInfo,
                            eOesApiChannelTypeT channelType,
                            const char *pUriList, const char *pUsername,
                            const char *pPassword, int8 clEnvId,
                            int32 heartBtInt,
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_LogonPeerNodes(OesApiSessionInfoT *pOutSessionInfo,
                           eOesApiChannelTypeT channelType,
                           const char *pUriList, const char *pUsername,
                           const char *pPassword, int8 clEnvId,
                           int32 heartBtInt,
 ...
 */
BOOL OesApi_SendReportSynchronization(OesApiSessionInfoT *pRptChannel,
                                      int8 subscribeEnvId,
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesApi_SendHeartbeat(OesApiSessionInfoT *pSessionInfo);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesApi_TestOrdChannel(OesApiSessionInfoT *pOrdChannel,
                           const char *pTestReqId, int32 testReqIdSize);
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesApi_TestOrdChannel2(OesApiSessionInfoT *pOrdChannel,
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesApi_TestOrdChannelAsynced(OesApiSessionInfoT *pOrdChannel,
                                  const char *pTestReqId, int32 testReqIdSize);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesApi_TestRptChannel(OesApiSessionInfoT *pRptChannel,
                           const char *pTestReqId, int32 testReqIdSize);
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesApi_TestRptChannel2(OesApiSessionInfoT *pRptChannel,
                            OesTestRequestReqT *pTestReq);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesApi_Logout(OesApiSessionInfoT *pSessionInfo, BOOL isDestory);
 ...
 * @param   pSessionInfo        会话信息
 */
void OesApi_Destory(OesApiSessionInfoT *pSessionInfo);
/* -------------------------           */
 ...
 * @param       pOrdAddrKey     委托申报服务地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @param       pRptAddrKey     执行报告回报服务地址的配置项关键字
 * (为空或配置项未设置则不连接)
 * @param       pQryAddrKey     查询服务地址的配置项关键字
 * (为空或配置项未设置则不连接)
 ...
 * @param[out]  pOutLastClSeqNo
 * 返回服务器端最后接收到并校验通过的"客户委托流水号(clSeqNo)"
 *                              - NULL表示忽略该参数
 *                              - "客户委托流水号" 是由客户端自行维护的委托流水,
 * 用于标识 委托数据的唯一性, 并防止重复申报
 *                              - "客户委托流水号" 的取值范围没有限制,
 * 只要维持在同一客户 端的同一个环境号(clEnvId)下的唯一即可
 *                              - 建议使用递增的正整数序列作为 "客户委托流水号"
 * 的取值范围
 ...
 */
BOOL OesApi_InitAll(OesApiClientEnvT *pOutCliEnv, const char *pCfgFile,
                    const char *pLoggerSection, const char *pCfgSection,
                    const char *pOrdAddrKey, const char *pRptAddrKey,
                    const char *pQryAddrKey, int64 lastRptSeqNum,
 ...
 * @see         OesApi_InitAllByCfgStruct
 */
BOOL OesApi_InitAllByConvention(OesApiClientEnvT *pOutCliEnv,
                                const char *pCfgFile, int64 lastRptSeqNum,
                                int32 *pOutLastClSeqNo);
 ...
 *              OesApi_InitAllByCfgStruct 接口不会自动初始化日志记录器,
 * 需要在外面 显式的调用 OesApi_InitLogger 来初始化API的日志记录器
 *
 * @param[out]  pOutCliEnv      输出客户端环境信息
 ...
 * @see         OesApi_InitAllByConvention
 */
BOOL OesApi_InitAllByCfgStruct(OesApiClientEnvT *pOutCliEnv,
                               const OesApiClientCfgT *pClientCfg,
                               int64 lastRptSeqNum, int32 *pOutLastClSeqNo);
 ...
 * @return      TRUE 成功; FALSE 失败
 */
void OesApi_LogoutAll(OesApiClientEnvT *pOutCliEnv, BOOL isDestory);
 ...
 * @param[out]  pOutCliEnv      客户端环境信息
 */
void OesApi_DestoryAll(OesApiClientEnvT *pOutCliEnv);
/* -------------------------           */
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesApi_InitLogger(const char *pCfgFile, const char *pLoggerSection);
 ...
 * @return  TRUE 成功; FALSE 失败
 */
BOOL OesApi_ResetThreadLoggerName(const char *pLogSystemName);
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_InitOrdChannel(OesApiSessionInfoT *pOutOrdChannel,
                           const char *pCfgFile, const char *pCfgSection,
                           const char *pAddrKey, int32 *pOutLastClSeqNo);
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_InitOrdChannel2(OesApiSessionInfoT *pOutOrdChannel,
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_InitRptChannel(OesApiSessionInfoT *pOutRptChannel,
                           const char *pCfgFile, const char *pCfgSection,
                           const char *pAddrKey, int64 lastRptSeqNum);
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_InitRptChannel2(OesApiSessionInfoT *pOutRptChannel,
                            const OesApiRemoteCfgT *pRemoteCfg,
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_InitQryChannel(OesApiSessionInfoT *pOutQryChannel,
                           const char *pCfgFile, const char *pCfgSection,
                           const char *pAddrKey);
 ...
 * @exception   EFAULT          其他业务错误
 */
BOOL OesApi_InitQryChannel2(OesApiSessionInfoT *pOutQryChannel,
                            const OesApiRemoteCfgT *pRemoteCfg);
 ...
 */
int32 OesApi_ParseAddrListString(const char *pUriList,
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesApi_ParseConfigFromFile(const char *pCfgFile, const char *pSection,
                                const char *pAddrKey,
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesApi_ParseConfigFromFile2(const char *pCfgFile, const char *pSection,
                                 const char *pAddrKey,
                                 OesApiRemoteCfgT *pOutRemoteCfg,
                                 OesApiSubscribeInfoT *pOutSubscribeInfo,
                                 BOOL isRequiredCfg);
 ...
 * @return      TRUE 成功; FALSE 失败
 */
BOOL OesApi_ParseAllConfig(const char *pCfgFile, OesApiClientCfgT *pOutApiCfg);
/* -------------------------           */
 ...
 * @return          TRUE, 成功; FALSE 失败
 */
BOOL OesApi_InitChannelGroup(OesApiChannelGroupT *pChannelGroup);
 ...
 * @return      TRUE, 成功; FALSE 失败
 */
BOOL OesApi_DestoryChannelGroup(OesApiChannelGroupT *pChannelGroup);

/**
 * 将连接信息添加到通道组
 *
 * @note        在检测通道组下各连接的I/O事件时, 是从后向前进行扫描的,
 * 所以建议将更 活跃的连接添加到其它连接的后面
 *
 * @param       pChannelGroup   通道组信息
 * @param       pChannel        连接信息
 * @return      TRUE, 成功; FALSE 失败
 */
BOOL OesApi_AddToChannelGroup(OesApiChannelGroupT *pChannelGroup,
 ...
 * @return      TRUE, 成功; FALSE 失败
 */
BOOL OesApi_DeleteFromChannelGroup(OesApiChannelGroupT *pChannelGroup,
 ...
 *
 * @param       pChannelGroup   通道组信息
 * @param       index           下标位置 (如果小于0,
 * 则表示按照先后顺序而非下标位置进行返回)
 *                              -  0, 返回 0 号下标位置所对应的会话信息
 *                              - -1, 返回通道组下第一个有效的会话信息
 *                              - -2, 返回通道组下第二个有效的会话信息
 *                              - INT_MIN, 如果超出了通道组的有效会话数量,
 * 则返回最后一个有效的会话信息
 * @return      连接信息
 */
OesApiSessionInfoT *
OesApi_GetFromChannelGroup(OesApiChannelGroupT *pChannelGroup, int32 index);
 ...
 */
OesApiSessionInfoT *
OesApi_GetFromChannelGroupBySocket(OesApiChannelGroupT *pChannelGroup,
                                   SPK_SOCKET socketFd);
 ...
 */
int64 OesApi_GetChannelGroupLastRecvTime(OesApiChannelGroupT *pChannelGroup);
 ...
 */
int64 OesApi_GetChannelGroupLastSendTime(OesApiChannelGroupT *pChannelGroup);
 ...
 */
int32 OesApi_WaitOnChannelGroup(OesApiChannelGroupT *pChannelGroup,
                                int32 timeoutMs,
                                F_OESAPI_ON_RPT_MSG_T fnOnMsgCallback,
                                void *pCallbackParams,
 ...
 *
 * @note    推荐通过设置 OesApiRemoteCfgT.username 的方式在连接级别单独设置,
 * 以避免多个 连接之间相互干扰, 例如:
 *          - OesApiRemoteCfgT        ordChannelCfg;
 *          - strncpy(ordChannelCfg.username, "username",
 * GENERAL_CLI_MAX_NAME_LEN - 1);
 *
 * @param   pUsername           登录用户名
 */
void OesApi_SetThreadUsername(const char *pUsername);
 ...
 */
const char *OesApi_GetThreadUsername();

/**
 * 设置当前线程登录OES时使用的登录密码
 * 不设置或者参数为空的话, 登录时会尝试使用配置文件中的配置
 *
 * @note    推荐通过设置 OesApiRemoteCfgT.password 的方式在连接级别单独设置,
 * 以避免多个 连接之间相互干扰, 例如:
 *          - OesApiRemoteCfgT        ordChannelCfg;
 *          - strncpy(ordChannelCfg.password, "password",
 * GENERAL_CLI_MAX_PWD_LEN - 1);
 *
 * @param   pPassword           登录密码
 *                              - 支持通过密码前缀指定密码类型, 如 md5:PASSWORD,
 * txt:PASSWORD
 */
void OesApi_SetThreadPassword(const char *pPassword);

/**
 * 设置当前线程登录OES时使用的客户端环境号
 * 不设置或者参数为空的话, 登录时会尝试使用配置文件中的配置
 *
 * @note    推荐通过设置 OesApiRemoteCfgT.clEnvId 的方式在连接级别单独设置,
 * 以避免多个 连接之间相互干扰, 例如:
 *          - OesApiRemoteCfgT        ordChannelCfg;
 ...
 * @param   clEnvId             客户端环境号
 */
void OesApi_SetThreadEnvId(int8 clEnvId);
 ...
 *
 * @note    推荐通过设置 OesApiSubscribeInfoT.clEnvId 的方式在连接级别单独设置,
 * 以避免 多个连接之间相互干扰, 例如:
 ...
 * @param   subscribeEnvId      待订阅的客户端环境号
 */
void OesApi_SetThreadSubscribeEnvId(int8 subscribeEnvId);
 ...
 *
 * @note    只有当服务端同时支持多种业务(如现货、两融等)时,
 * 才需要通过该接口区分待对接的业务
 *          类型。实际生产环境不会采用多种业务合并部署的模式,
 * 所以正常情况下无需调用该接口。
 *
 * @note    推荐通过设置 OesApiRemoteCfgT.businessType 的方式在连接级别单独设置,
 * 以避免 多个连接之间相互干扰, 例如:
 *          - OesApiRemoteCfgT        ordChannelCfg;
 ...
 */
void OesApi_SetThreadBusinessType(int32 businessType);
 ...
 * @note    API将优先使用自动采集到的实际IP和MAC地址,
 * 只有无法采集到有效的IP/MAC地址时, 才会使用自定义的IP和MAC地址
 *
 ...
 */
BOOL OesApi_SetCustomizedIpAndMac(const char *pIpStr, const char *pMacStr);
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL OesApi_SetCustomizedIp(const char *pIpStr);
 ...
 * @return  客户端自定义的本地IP
 */
const char *OesApi_GetCustomizedIp();
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL OesApi_SetCustomizedMac(const char *pMacStr);
 ...
 * @return  客户端自定义的本地MAC
 */
const char *OesApi_GetCustomizedMac();
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL OesApi_SetCustomizedDriverId(const char *pDriverId);
 ...
 */
const char *OesApi_GetCustomizedDriverId();
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL OesApi_SetDefaultEntrustWay(char entrustWay);
 ...
 * @return  TRUE 设置成功; FALSE 设置失败 (参数格式错误)
 */
BOOL OesApi_SetAssignedEntrustWay(OesApiSessionInfoT *pSessionInfo,
                                  char entrustWay);
 ...
 * @return  客户端会话使用的委托方式
 */
char OesApi_GetAssignedEntrustWay(const OesApiSessionInfoT *pSessionInfo);

/**
 * 设置是否保留(不清空)由应用层自定义使用的扩展存储空间数据 (__extData,
 * 默认为否)
 *
 * @param   isRetainExtDataAble
 * 是否保留(不清空)由应用层自定义使用的扩展存储空间数据
 */
void OesApi_SetRetainExtDataAble(BOOL isRetainExtDataAble);
 ...
 * @return  通道对应的客户端编号
 */
int16 OesApi_GetClientId(const OesApiSessionInfoT *pSessionInfo);
 ...
 * @return  通道对应的客户端环境号
 */
int8 OesApi_GetClEnvId(const OesApiSessionInfoT *pSessionInfo);
 ...
 */
uint32 OesApi_GetBusinessType(const OesApiSessionInfoT *pSessionInfo);
 ...
 */
BOOL OesApi_IsBusinessSupported(const OesApiSessionInfoT *pSessionInfo,
 ...
 */
uint8 OesApi_GetClientType(const OesApiSessionInfoT *pSessionInfo);
 ...
 */
uint8 OesApi_GetClientStatus(const OesApiSessionInfoT *pSessionInfo);
 ...
 */
int64 OesApi_GetLastRecvTime(const OesApiSessionInfoT *pSessionInfo);
 ...
 */
int64 OesApi_GetLastSendTime(const OesApiSessionInfoT *pSessionInfo);
 ...
 */
int32 OesApi_HasMoreCachedData(const OesApiSessionInfoT *pRptChannel);
 ...
 */
BOOL OesApi_IsValidOrdChannel(const OesApiSessionInfoT *pOrdChannel);
 ...
 * @return  TRUE 已连接; FALSE 未连接或连接无效
 */
BOOL OesApi_IsValidRptChannel(const OesApiSessionInfoT *pRptChannel);
 ...
 * @return  TRUE 已连接; FALSE 未连接或连接无效
 */
BOOL OesApi_IsValidQryChannel(const OesApiSessionInfoT *pQryChannel);
 ...
 * @return  TRUE 已连接; FALSE 未连接或连接无效
 */
BOOL OesApi_IsValidChannelGroup(const OesApiChannelGroupT *pChannelGroup);
 ...
 * @param   errCode             错误号
 */
void OesApi_SetLastError(int32 errCode);
 ...
 * @return  错误号对应的错误信息
 */
const char *OesApi_GetErrorMsg(int32 errCode);
 ...
 * @return  状态码所对应的错误信息
 */
const char *OesApi_GetErrorMsg2(uint8 status, uint8 detailStatus);
 ...
 */
BOOL OesApi_IsErrorOf(int32 errCode, const SErrMsgT *pErrMsg);
 ...
 * @return  TRUE 属于; FALSE 不属于
 */
BOOL OesApi_IsErrorOf2(uint8 status, uint8 detailStatus,
 ...
 */
BOOL OesApi_HasStockStatus(const OesStockItemT *pStockItem,
 ...
 *              - 缺少 canceledQty, 将置为0
 *              - 另外,
 * 还缺少原始的委托时间(ordTime)和委托确认时间(ordCnfmTime), 将使
 *                用成交时间(trdTime)替代
 *
 ...
 * @return      提取到的委托回报信息
 */
OesOrdCnfmT *OesHelper_ExtractOrdReportFromTrd(const OesTrdCnfmT *pTrdReport,
...
/* 用于校验API版本号是否匹配的标识函数 (如果链接时报函数不存在错误,
 * 请检查API的头文件与库文件是否一致) */
 ...
 */
static __inline BOOL __OesApi_CheckApiVersion() {
  /* 如果编译时报 IsApiVersionMatched 函数不存在错误,
   * 请检查API的头文件与库文件是否一致 */
