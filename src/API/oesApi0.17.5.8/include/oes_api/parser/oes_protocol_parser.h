 ...
 */
OesReqMsgBodyT *OesParser_DecodeReq(SMsgHeadT *pReqHead, const void *pMsgBody,
 ...
 * @param           pRemoteInfo     对端身份信息, 用于打印跟踪日志
 * @param           isCopyBinary
 * 对于二进制协议，是否也同样将原始数据复制到缓存区
 * @return          编码后的消息体数据; NULL：编码失败
 */
void *OesParser_EncodeRsp(SMsgHeadT *pRspHead, const OesRspMsgBodyT *pRspBody,
                          char *pBuf, int32 bufSize, const char *pRemoteInfo,
                          BOOL isCopyBinary);
 ...
 */
void *OesParser_EncodeRptSpecial(SMsgHeadT *pRspHead,
                                 const OesRptMsgHeadT *pRptMsgHead,
                                 const OesRptMsgBodyT *pRptMsgBody, char *pBuf,
 ...
 */
OesQryReqMsgT *OesParser_DecodeQueryReq(SMsgHeadT *pReqHead,
                                        const char *pMsgBody,
 ...
 * @return  编码后的消息体数据; NULL：编码失败
 */
void *OesParser_EncodeQueryRsp(SMsgHeadT *pRspHead,
