 ...
 */
OesReqMsgBodyT *OesJsonParser_DecodeReq(SMsgHeadT *pReqHead,
                                        const void *pMsgBody,
 ...
 */
void *OesJsonParser_EncodeRsp(SMsgHeadT *pRspHead,
                              const OesRspMsgBodyT *pRspBody, char *pBuf,
 ...
 * @return          编码后的消息体数据; NULL, 编码失败
 */
char *OesJsonParser_EncodeRptItem(uint8 rptMsgType,
                                  const OesRptMsgHeadT *pRptHead,
                                  const void *pRptData, char *pBuf,
                                  int32 bufSize);

/**
 * 为执行报告回报特别定制的应答消息编码处理
 * (编码为JSON格式，用于向客户端发送应答消息)
 *
 * @param[in,out]   pRspHead        消息头
 * @param           pRspBody        原始应答数据结构体
 * @param[out]      pBuf            存储编码后数据的缓存区
 ...
 */
void *OesJsonParser_EncodeRptSpecial(SMsgHeadT *pRspHead,
