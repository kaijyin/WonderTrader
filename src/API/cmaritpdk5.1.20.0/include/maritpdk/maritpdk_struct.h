...
typedef struct {
  ...
  int StockProperty;     // 证券属性 1	融资标的, 2	融券标的, 4	担保物,
                     // 7 融资融券担保标的
...
typedef struct {
  ...
  double
      dPCJE; // 强制平仓资金 = (总负债*平仓到位比例 - 总资产)/(平仓到位比例 -1)
  double dZDPCJE; // 主动平仓资金 = (总负债*追保到位比例 - 总资产)/(追保到位比例
                  // -1)
  double dKQBZ; // 可提资产标准
  double dKQZJ; // 可取资金
  ...
  double
      dZZCJZD_BKCDR; // 科创板+创业板+科创板CDR+创业板CDR板块持仓占总资产集中度
...
typedef struct {
  ...
  double m_dYQFXYH;  // 逾期罚息已还
  char m_sQYBCFS
      [30]; // 权益补偿方式(第一位标示“配股偿还标志”，第二位标示“配债偿还标志”，第三位标示“派发权证偿还标志)
...
typedef struct {
  ...
  char m_sCLZT
      [3]; // 处理状态（0:未处理，1:全部中签扣款，2:部分中签扣款，3:全部放弃）
...
typedef struct {
  ...
  char m_sHBDM[4];  // 货币代码
  char m_sZZYMMXX
      [2]; // 证券转银行密码选项（0:都不输，1:只输资金密码，2:只输银行密码，3:两个密码都要输）
  char m_sYZZMMXX
      [2]; // 银行转证券密码选项（0:都不输，1:只输资金密码，2:只输银行密码，3:两个密码都要输）
...
struct ITPDK_CusReqInfo {
  ...
  ITPDK_CusReqInfo()
      : AccountId{0}, FundAccount{0}, SecuAccount{0}, Password{0}, OrgCode{0},
        TradeNodeID(-1), EntrustWay{0}, NodeInfo{0}, OppSecuAccount{0},
        OppSeat{0}, Token(-1), RetCode(0), ErrMsg{0} {}
};
