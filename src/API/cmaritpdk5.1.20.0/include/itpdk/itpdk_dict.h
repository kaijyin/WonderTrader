...
#define NOTIFY_ASSETVARY                                                       \
  20 // 资产变动(AccountId,FundAccount,Message)--比较频繁，不建议作为显示信息。
#define NOTIFY_QUOTEVARY                                                       \
  21 // 行情变动({Market,StockCode,varys}) vars为行情变动标志的位组合
...
// 订单交易限制
#define OT_ALO                                                                 \
  1 // 竞价限价盘 At-auction Limit Order(Pre-opening Session and Closing Auction
    // Session)
...
#define MR_FUNC_CALLBACK_FJYYW                                                 \
  103 // 非交易业务(担保物提交、担保物返还、还券划转)
...
#define MR_FUNC_CALLBACK_WD_FJYYW                                              \
  123 // 非交易业务撤单(担保物提交、担保物返还、还券划转)
...
const char table_sokhxqdjszjqk[] =
    "tSOKHXQDJSZJQK"; // 期权客户行权待交收资金缺口
const char table_sokhxqdjszqqk[] =
    "tSOKHXQDJSZQQK";                          // 期权客户行权待交收证券缺口
