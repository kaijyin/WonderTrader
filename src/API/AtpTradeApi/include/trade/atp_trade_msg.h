// code_generator_tag
// messages:545e8f3461ca9459a3a3c925e674a6b7
// template:d5e676b48a16d8942e0c6cea656a6229
// code:e7eef0b3e567570c5abb0173c23b98d7
...
struct TRADE_API ATPReqSwapPutbackOrderMsg {
  ...
  ATPShareNegotiabilityType
      share_negotiability; /// 股份性质，证券类型为定向可转债业务时必填
...
struct TRADE_API APIConstituentStockUnit {
  ATPSecurityIDType
      security_id; /// 成分股证券代码，例外：1-沪市资金代码、2-深市资金代码、3-港市资金代码、9-非沪深资金代码（全额现金申赎）
...
struct TRADE_API ATPReqTradeOrderQueryMsg {
  ...
  ATPEtfPurchaseRedemptionFlagType
      etf_purchase_redemption_flag; /// 是否返回ETF实时申赎成交：0-不返回，1-返回（ATP3.2.1开始支持）
...
struct TRADE_API ATPReqShareQueryMsg {
  ...
  ATPMarketIDType
      market_id; /// 市场代码
                 /// （默认值为0，表示查所有，一户两地账户表示查单个证券账户下所有）（ATP3.1.6版本开始支持此字段非必填）
  ATPSecurityIDType
      security_id; /// 证券代码
                   /// （默认值为空字符串，表示查所有，一户两地账户表示查单个证券账户下所有）
  ATPReportIndexType
...
struct TRADE_API APITradeOrderUnit {
  ATPBusinessTypeType business_type; /// 业务类型
  ATPSecurityIDType
      security_id; /// 证券代码（对于ETF实时申赎成交，上海市场：1-沪市资金代码、2-深市资金代码、3-港市资金代码、9-非沪深资金代码）
  ATPSecuritySymbolType security_symbol; /// 证券简称
  ATPMarketIDType market_id;             /// 市场代码
  ATPAccountIDType account_id;           /// 证券账户ID
  ...
  ATPClOrdNOType
      cl_ord_no; /// 客户订单编号（原订单的客户订单编号，若此笔记录为撤单，是为撤单的原订单编号）
  ATPOrderIDType
...
struct TRADE_API APIShareRecordETFArray {
  ATPSecurityIDType
      security_id; /// 证券代码，例外：1-沪市资金代码、2-深市资金代码、3-港市资金代码、9-非沪深资金代码（全额现金申赎）
...
struct TRADE_API ATPReqCancelBatchOrderMsg {
  ...
  ATPClientFeatureCodeType
      client_feature_code; /// 终端识别码（该字段是否必填请咨询券商）
  ATPClOrdNOType
      parent_cl_ord_no; /// 原始交易客户方（券商）订单编号，指被撤消订单的cl_ord_no（非算法母单编号）
