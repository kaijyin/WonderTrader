// code_generator_tag
// requests:8ffc1a9bd0dd3ec725af4dde9b4a63b5
// template:3329d30835d0fc5b63f16c7fd86f60cb
// code:2f77a2b08488dcffe4d5cab8f23bcee3
...
public:
   ...
   *              ENCRYPT_SCHEMA
   * “0”：表示API进行转发，API不在进行加密。“1”：表示API对消息中的密码字段进行密码
   ...
   * 1时，未配置ATP_LOGIN_ENCRYPT_PASSWORD时将采用默认内置SM2算法，未配置ATP_ENCRYPT_PASSWOR时将采用默认内置SM3算法
   * @return   	初始化结果
   * @note		只需要调用一次即可
   */
   ...
   * @type       cancel
   * @note
   * 支持以下业务类型的撤单：盘后定价交易、大宗交易、债券分销、现货交易、ETF、盘后定价（科创板、创业板）交易业务、港股通买卖、港股通公司行为、网上发行认购、网络投票、质押式回购、配售业务、分级基金分拆合并、债券转股回售、要约收购、权证行权
   */
