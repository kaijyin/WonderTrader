...
static const int64_t kAccountContractFundQuery =
    300008;                                       ///< 指定合约账户资金查询
static const int64_t kTradeCollectQuery = 300009; ///< 成交汇总查询
static const int64_t kATPReqTIBIPOQtyQuery =
    300010; ///< 科创板新股发行申购额度查询
...
static const int64_t kExtOptionFundTransfer =
    8000002; ///< 外部期权资金账户资金划拨请求
static const int64_t kExtQueryExerciseProfitLoss =
    3000004;                                           ///< 行权预估盈亏查询消息
static const int64_t kReduceHoldingTransfer = 1036001; ///< 高管额度划转业务
static const int64_t kReduceHoldingTransferCancel =
    19000006; ///< 高管额度划转撤单
...
static const int64_t kExtQueryContractSpecifications =
    3000101; ///< 融资融券合约明细查询消息
static const int64_t kExtQueryCreditAmount =
    3000102; ///< 融资融券信用额度查询消息
static const int64_t kExtQueryMarginUseable =
    3000103; ///< 融资融券保证金可用查询消息
static const int64_t kExtQueryFundLiability =
    3000107; ///< 融资融券资金负债查询消息
static const int64_t kExtQueryShareLiability =
    3000108; ///< 融资融券股份负债查询消息
static const int64_t kExtQueryContractSpecificationsManager =
    3000109; ///< 融资融券合约管理查询消息
static const int64_t kExtQueryMarginUnderlying =
    3000119; ///< 融资融券标的清单查询消息
static const int64_t kExtQueryCreditInfo =
    3000120; ///< 融资融券客户信用资料查询消息
static const int64_t kAgwMsgExtQueryFundAssetsInfo =
    3000121;                                    ///< 融资融券资产信息查询
static const int64_t kHKStocksThrough = 106301; ///< 港股通业务委托消息
static const int64_t kHKVoting = 106401;        ///< 港股通投票业务委托消息
static const int64_t kHKCorporateAction =
    106201; ///< 港股通公司行为业务委托消息
static const int64_t kHKDesignationTransfer =
    106101; ///< 港股通转托管业务委托消息
static const int64_t kHKCorporateActionCancel =
    190004; ///< 港股通公司行为撤单消息

static const int64_t kAgwMsgOptionCoveredTransUncoveredOrder =
    103501; ///< 期权普通与备兑互转订单
static const int64_t kAgwMsgExtQueryCombinExerciseMaxOrderQty =
    3000023; ///< 合并行权最大可委托数查询消息
static const int64_t kAgwMsgExtQueryCombinExerciseProfitLoss =
    3000022; ///< 合并行权预估盈亏查询消息
static const int64_t kAgwMsgExternalQueryOptionCombinationMaxOrderQty =
    3000027; ///< 期权组合策略最大可委托数查询消息
static const int64_t kAgwMsgCustPasswdModify = 8000004; ///< 客户密码修改请求
static const int64_t kAgwExternalInsTETransFundReq =
    810001; ///< 按证券账户划转资金请求消息

static const int64_t kAgwMsgExtQueryFundExReq =
    3000033;                                       ///< 增强资金查询请求消息
static const int64_t kTradeOrderExQuery = 3000035; ///< 增强成交查询
static const int64_t kAgwMsgThirdPartPassThrough =
    8000027; ///< 第三方通用转发接口
static const int64_t kBankSecuritiesFundTransfer = 8000030; ///< 银证转账
}; // namespace MsgTypeNum

template <size_t N> void SET(char (&a)[N], const std::string &str) {
  strncpy(a, str.c_str(), N);
}

template <typename T1, typename T2> void SET(T1 &t1, const T2 &t2) {
...
int32_t Connect() {
  ...
  prop.heartbeat_interval_milli =
      5000; // 发送心跳的时间间隔,单位：毫秒，默认值：5000
  ...
  // API连接
  ATPRetCodeType ec = atp_trade_api_client->Connect(
      prop, new DemoATPTradeHandler(g_login_success, g_exit));
...
int main(int argc, char *argv[]) {
  ...
  std::string tmp_bid_position_effect, tmp_offer_position_effect, tmp_leg_side1,
      tmp_leg_side2;
  ...
  try {
    ...
    cmd_parse.add<uint16_t>(
        "send_to", 0,
        "witch version send to(4=V220,2=V217,3=V212,1=V210),default 4", false,
        4);
    cmd_parse.add<int64_t>(
        "number", 0, "the send msg_type value,default 100000", false, 100000);
    cmd_parse.add<std::string>("account_id", 0, "account_id, default 000000",
                               false, "000000");
    cmd_parse.add<std::string>("dest_account_id", 0,
                               "dest_account_id, default 000000", false, "");
    cmd_parse.add<std::string>("branch_id", 0, "branch_id,default 00000", false,
                               "00000");
    cmd_parse.add<int32_t>("business_type", 0, "business_type,default 200",
                           false, 200);
    cmd_parse.add<ATPAmtType>("cash_order_qty", 0,
                              "cash_order_qty,default 100000", false, 100000);
    cmd_parse.add<ATPClOrdNOType>("cl_ord_no", 0, "cl_ord_no,default 0", false,
                                  0);
    cmd_parse.add<std::string>("client_feature_code", 0,
                               "client_feature_code ,default 0000", false,
                               "0000");
    cmd_parse.add<int32_t>("covered_or_uncovered", 0,
                           "covered_or_uncovered, default 0", false, 0);
    cmd_parse.add<std::string>("currency", 0, "currency,only CNY", false,
                               "CNY");
    cmd_parse.add<std::string>("cust_id", 0, "cust_id, default 00000000", false,
                               "00000000");
    cmd_parse.add<std::string>("designated_pbu_id", 0,
                               "designated_pbu_id,default 0000", false, "0000");
    cmd_parse.add<int32_t>("designation_trans_type", 0,
                           "designation_trans_type,default 1", false, 1);
    cmd_parse.add<std::string>("enforce_flag", 0, "enforce_flag, default 0",
                               false, "0");
    cmd_parse.add<std::string>("fund_account_id", 0,
                               "fund_account_id, default 0000000", false,
                               "0000000");
    cmd_parse.add<ATPGeneralMeetingSeqType>(
        "general_meeting_seq", 0, "general_meeting_seq,default 100001", false,
        100001);
    cmd_parse.add<ATPIndexType>("g_index", 0, "index,default 0", false, 0);
    cmd_parse.add<std::string>("remote_address_master", 0,
                               "default 127.0.0.1:32002", false,
                               "127.0.0.1:32002");
    cmd_parse.add<std::string>("remote_address_backup", 0,
                               "default 127.0.0.1:32002", false,
                               "127.0.0.1:32002");
    cmd_parse.add<ATPBoolType>("is_account_partition", 0,
                               "is_account_partition,default 1", false, 1);
    cmd_parse.add<ATPMarketIDType>("market_id", 0, "market_id, default kNull",
                                   false, ATPMarketIDConst::kNULL);
    cmd_parse.add<ATPMaxPriceLevelsType>(
        "max_price_levels", 0, "max_price_levels,default 0", false, 0);
    cmd_parse.add<ATPQtyType>("min_qty", 0, "min_qty,default 0", false, 0);
    cmd_parse.add<ATPClOrdNOType>("orig_cl_ord_no", 0,
                                  "orig_cl_ord_no,default 00001", false, 00001);
    cmd_parse.add<std::string>("order_way", 0, "order_way, default 0", false,
                               "0");
    cmd_parse.add<ATPQtyType>("order_qty", 0, "order_qty, default 10000", false,
                              10000);
    cmd_parse.add<int32_t>("order_query_condition", 0,
                           "order_query_condition,default 0", false, 0);
    cmd_parse.add<std::string>("order_type", 0, "order_type,default 0", false,
                               "0");
    cmd_parse.add<int32_t>("partition", 0, "partition,default 0", false, 0);
    cmd_parse.add<std::string>("password", 0, "password,default password1",
                               false, "password1");
    cmd_parse.add<std::string>("position_effect", 0,
                               "position_effect, default C ", false, "C");
    cmd_parse.add<ATPPriceType>("price", 0, "price,default 100000", false,
                                100000);
    cmd_parse.add<ATPReportIndexType>("query_index", 0, "query_index,default 0",
                                      false, 0);
    cmd_parse.add<int32_t>("return_seq", 0, "return_seq,default 0", false, 1);
    cmd_parse.add<ATPTotalNumType>("return_num", 0, "return_num,default 0",
                                   false, 0);
    cmd_parse.add<std::string>("remark", 0, "remark,default 00000", false,
                               "00000");
    cmd_parse.add<std::string>("security_id", 0, "security_id, default 000001",
                               false, "000001");
    cmd_parse.add<std::string>("side", 0, "side, default 0,1,2...", false, "1");
    cmd_parse.add<ATPPriceType>("stop_px", 0, "stop_px, default 10000", false,
                                10000);
    cmd_parse.add<std::string>("time_in_force", 0, "time_in_force,default 0",
                               false, "0");
    cmd_parse.add<int32_t>("transfer_way", 0, "transfer_way,default 1", false,
                           1);
    cmd_parse.add<ATPAmtType>("transfer_value", 0, "transfer_value,default 0",
                              false, 0);
    cmd_parse.add<std::string>("transferee_pbu_id", 0,
                               "transferee_pbu_id,default 0000", false, "0000");
    cmd_parse.add<std::string>("tenderer", 0, "tenderer,default 00000", false,
                               "00000");
    cmd_parse.add<std::string>("user", 0, "user,default user1", false, "user1");
    cmd_parse.add<std::string>("voting_proposal", 0,
                               "voting_proposal,default 00000", false, "00000");
    cmd_parse.add<std::string>("voting_preference", 0,
                               "voting_preference,default 1", false, "1");
    cmd_parse.add<std::string>("voting_segment", 0,
                               "voting_segment,default 00000", false, "00000");
    cmd_parse.add<ATPValidationCodeType>("validation_code", 0,
                                         "validation_code,default 0", false, 0);
    cmd_parse.add<std::string>("bid_position_effect", 0,
                               "bid_position_effect default O", false, "O");
    cmd_parse.add<std::string>("offer_position_effect", 0,
                               "offer_position_effect default O", false, "O");
    cmd_parse.add<ATPQtyType>("offer_order_qty", 0,
                              "offer_order_qty, default 10000", false, 10000);
    cmd_parse.add<ATPPriceType>("offer_price", 0, "offer_price,default 100000",
                                false, 100000);
    cmd_parse.add<std::string>("quote_req_id", 0, "quote_req_id,default 0",
                               false, "0");
    cmd_parse.add<std::string>("user_info", 0, "user_info,default api demo",
                               false, "api demo");
    cmd_parse.add<std::string>("account_sub_code", 0,
                               "account_sub_code,default 888", false, "888");
    cmd_parse.add<std::string>("leg_side1", 0, "leg_side1,default 1", false,
                               "1");
    cmd_parse.add<std::string>("leg_side2", 0, "leg_side2,default 2", false,
                               "2");
    cmd_parse.add<std::string>("strategy_id", 0, "strategy_id,default 0", false,
                               "0");
    cmd_parse.add<std::string>("secondary_order_id", 0,
                               "secondary_order_id default 0", false, "0");
    cmd_parse.add<std::string>("leg_security_id1", 0,
                               "leg_security_id1,default 0", false, "0");
    cmd_parse.add<std::string>("leg_security_id2", 0,
                               "leg_security_id2 default 0", false, "0");
    cmd_parse.add<int32_t>("price_property", 0, "price_property default 1",
                           false, 1);
    cmd_parse.add<int32_t>("trd_sub_type", 0, "trd_sub_type default 1", false,
                           1);
    cmd_parse.add<std::string>("contactor", 0, "contactor default contactor",
                               false, "contactor");
    cmd_parse.add<std::string>("contact_info", 0,
                               "contact_info default contactinfo", false,
                               "contactinfo");
    cmd_parse.add<std::string>("confirm_id", 0, "confirm_id default 123", false,
                               "123");
    cmd_parse.add<std::string>("counter_party_pbu_id", 0,
                               "counter_party_pbu_id default 123", false,
                               "123");
    cmd_parse.add<std::string>("counter_party_account_id", 0,
                               "counter_party_account_id default 123", false,
                               "123");
    cmd_parse.add<int32_t>("pricing", 0, "pricing default 1", false, 1);
    cmd_parse.add<ATPMarketIDType>("leg_market_id1", 0,
                                   "leg_market_id1 default 0", false, 0);
    cmd_parse.add<ATPCoveredOrUncoveredType>(
        "leg_covered_or_uncovered1", 0, "leg_covered_or_uncovered1 default 0",
        false, 0);
    cmd_parse.add<std::string>("dest_pbu_id", 0, "dest_pbu_id,default ", false,
                               "");
    cmd_parse.add<std::string>("disposal_pbu", 0, "disposal_pbu,default ",
                               false, " ");
    cmd_parse.add<std::string>("disposal_account_id", 0,
                               "disposal_account_id default ", false, " ");
    cmd_parse.add<ATPRepayWayType>("repay_way", 0, "repay_way,default 0", false,
                                   0);
    cmd_parse.add<std::string>("close_out_flag", 0,
                               "close_out_flag,default 0,1,2", false, "0");
    cmd_parse.add<std::string>("cash_margin_set", 0,
                               "cash_margin_set,default 0", false, "");
    cmd_parse.add<ATPDate8Type>("opening_date", 0, "opening_date,default 0",
                                false, 0);
    cmd_parse.add<std::string>("opening_serial_num", 0,
                               "opening_serial_num,default ", false, "");
    cmd_parse.add<std::string>("query_contents", 0,
                               "query_contents,default 0,1", false, "1");
    cmd_parse.add<ATPCRDUnderlyingTypeType>(
        "underlying_type", 0, "underlying_type,default 0,1,2,3", false, 0);
    cmd_parse.add<ATPAmtType>("repay_value", 0, "repay_value,default 0", false,
                              0);
    cmd_parse.add<std::string>("password_msg", 0, "password_msg,default ",
                               false, "");
    cmd_parse.add<std::string>("lot_type", 0, "lot_type default ", false, "2");
    cmd_parse.add<ATPAnnouncementNumberType>(
        "announcement_number", 0, "announcement_number default ", false, " ");
    cmd_parse.add<ATPProxyFormNumberType>(
        "proxy_form_number", 0, "proxy_form_number default ", false, " ");
    cmd_parse.add<ATPAffirmativeVotesType>(
        "affirmative_votes", 0, "affirmative_votes default ", false, " ");
    cmd_parse.add<ATPNegativeVotesType>("negative_votes", 0,
                                        "negative_votes default ", false, " ");
    cmd_parse.add<ATPAbstentionVotesType>(
        "abstention_votes", 0, "abstention_votes default ", false, " ");
    cmd_parse.add<std::string>("corporate_action_id", 0,
                               "corporate_action_id default ", false, " ");
    cmd_parse.add<std::string>("entitlement_number", 0,
                               "entitlement_number default ", false, " ");

    cmd_parse.add<std::string>("old_password", 0, "old_password default ",
                               false, " ");
    cmd_parse.add<ATPMarketIDType>("fund_out_market_id", 0,
                                   "fund_out_market_id default 0", false, 0);
    cmd_parse.add<std::string>("fund_in_account_id", 0,
                               "fund_in_account_id default 0", false, " ");
    cmd_parse.add<ATPMarketIDType>("fund_in_market_id", 0,
                                   "fund_in_market_id default ", false, 0);
    ...
    cmd_parse.add<std::string>("extra_data", 0, "extra_data default ", false,
                               " ");
    cmd_parse.add<std::string>("trade_root_num", 0, "trade_root_num default ",
                               false, " ");
    ...
    general_meeting_seq =
        cmd_parse.get<ATPGeneralMeetingSeqType>("general_meeting_seq");
    ...
    tmp_offer_position_effect =
        cmd_parse.get<std::string>("offer_position_effect");
    ...
    leg_covered_or_uncovered1 =
        cmd_parse.get<ATPCoveredOrUncoveredType>("leg_covered_or_uncovered1");
    ...
    counter_party_account_id =
        cmd_parse.get<std::string>("counter_party_account_id");
    ...
    announcement_number =
        cmd_parse.get<ATPAnnouncementNumberType>("announcement_number");
    proxy_form_number =
        cmd_parse.get<ATPProxyFormNumberType>("proxy_form_number");
    affirmative_votes =
        cmd_parse.get<ATPAffirmativeVotesType>("affirmative_votes");
    negative_votes = cmd_parse.get<ATPNegativeVotesType>("negative_votes");
    abstention_votes =
        cmd_parse.get<ATPAbstentionVotesType>("abstention_votes");
    ...
    underlying_type =
        cmd_parse.get<ATPCRDUnderlyingTypeType>("underlying_type");
  ...
  // char
  // 型值在解析命令行时会报错，故使用string类型值tmp_xxx暂时存储解析的命令行的值
  side = tmp_side[0];
  ...
  // encrypt_cfg参数填写：
  encrypt_cfg["ENCRYPT_SCHEMA"] =
      "0"; // 字符 0 表示 不对消息中的所有 password 加密
  encrypt_cfg["ATP_ENCRYPT_PASSWORD"] =
      ""; // 除登入及密码修改外其他消息的密码字段加密算法
  encrypt_cfg["ATP_LOGIN_ENCRYPT_PASSWORD"] =
      ""; // 登入及密码修改消息中密码字段的加密算法so路径
  encrypt_cfg["GM_SM2_PUBLIC_KEY_PATH"] =
      ""; // 采用国密算法时，通过该key配置 GM算法配置加密使用的公钥路径
  encrypt_cfg["RSA_PUBLIC_KEY_PATH"] =
      ""; // 如果使用rsa算法加密，通过该key配置 rsa算法配置加密使用的公钥路径

  ATPRetCodeType ec =
      ATPTradeAPI::Init(station_name, cfg_path, log_dir_path, record_all_flag,
                        encrypt_cfg, connection_retention_flag);
  if (ec != ErrorCode::kSuccess) {
  ...
  while (!g_exit) {
    ...
    if (g_login_success) {
      ...
      case MsgTypeNum::
          kAgwMsgOptionCoveredTransUncoveredOrder: // ///<
                                                   // 期权普通与备兑互转订单
      {
      ...
      case MsgTypeNum::
          kAgwMsgExtQueryCombinExerciseMaxOrderQty: // ///<
                                                    // 合并行权最大可委托数查询消息
      {
      ...
      case MsgTypeNum::
          kAgwMsgExtQueryCombinExerciseProfitLoss: // ///<
                                                   // 合并行权预估盈亏查询消息
      {
      ...
      case MsgTypeNum::
          kAgwMsgExternalQueryOptionCombinationMaxOrderQty: // ///<
                                                            // 期权组合策略最大可委托数查询消息
      {
      ...
      case MsgTypeNum::
          kAgwExternalInsTETransFundReq: // ///< 按证券账户划转资金请求消息
      {
  ...
  std::this_thread::sleep_for(
      std::chrono::seconds(10)); // 等待消息接收后关闭连接
