...
ATPClientFeatureCodeType g_client_feature_code =
    "ip:127.0.0.1;mac:2222"; // 终端识别码，由券商指定规则
...
class CustHandler : public ATPTradeHandler {
  ...
  // 客户号登入回调
  virtual void
  OnRspCustLoginResp(const ATPRspCustLoginRespOtherMsg &cust_login_resp) {
    std::cout << "OnRspCustLoginResp Recv:"
    ...
    if (cust_login_resp.permisson_error_code == 0) {
      ...
      std::cout << "CustLogin Fail, permisson_error_code :"
                << static_cast<uint32_t>(cust_login_resp.permisson_error_code)
                << std::endl;
    }
  ...
  // 客户号登出回调
  virtual void
  OnRspCustLogoutResp(const ATPRspCustLogoutRespOtherMsg &cust_logout_resp) {
    std::cout << "OnRspCustLogoutResp Recv:"
    ...
    if (cust_logout_resp.permisson_error_code == 0) {
      ...
      std::cout << "CustLogou Fail permisson_error_code :"
                << static_cast<uint32_t>(cust_logout_resp.permisson_error_code)
                << std::endl;
    }
    g_waiting_flag.store(false);
  ...
  // 订单下达内部响应
  virtual void
  OnRspOrderStatusInternalAck(const ATPRspOrderStatusAckMsg &order_status_ack) {
  ...
  // 订单下达交易所确认
  virtual void
  OnRspOrderStatusAck(const ATPRspOrderStatusAckMsg &order_status_ack) {
  ...
  // 订单下达内部拒绝
  virtual void
  OnRspBizRejection(const ATPRspBizRejectionOtherMsg &biz_rejection) {
...
ATPRetCodeType connect(ATPTradeAPI *client, ATPTradeHandler *handler) {
  ...
  prop.locations = {"10.16.131.223:32001",
                    "127.0.0.1:32495"}; // 网关主备节点的地址+端口
...
ATPRetCodeType login(ATPTradeAPI *client) {
  ...
  login_msg.login_mode =
      ATPCustLoginModeType::kFundAccountIDMode; // 登录模式，资金账号登录
...
ATPRetCodeType send(ATPTradeAPI *client) {
  ...
  p->order_qty =
      100000; // 申报数量N15(2)；股票为股、基金为份、上海债券默认为张（使用时请务必与券商确认），其他为张；1000.00股
