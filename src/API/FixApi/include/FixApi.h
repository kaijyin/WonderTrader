 ...
 * @section 介绍
 * - 接口介绍:\n
 *      东吴证券易驰统一接入最大限度的适配了顶点Fix api C接口，用户参见顶点Fix
 * api用户即可，如有需要，可直接对顶点的Fixapi组件进行替换，可更好的方便三方客户连接东吴证券核心交易系统。\n
 * - 接口文件:\n
...
*
为空或其他（为保持和顶点SDK接口一致，此接口请用默认值，目前SDK支持TLSV1以上，不支持低版本协议。）:
pszFileCert不为空,证书标准安全认证方式
*  				             pszFileCert为空, 标准认证方式
*
pszFileCA:信任证书文件（PEM格式，如果不想传入此参数，请手动安装信任证书到系统信任证书列表）
* 	 bVerify:验证服务器证书; true表示验证; false:不验证服务器证书;
建议设置为 true
* 	 nTimeOUT:认证超时时间(单位秒)
* @return 返回数据:
 ...
 */
// a5没有了
// FIX_API_EXPORT HANDLE_CONN Fix_ConnectEx(const char* sAddr, const char*
// sUser,
//     const char* sPwd, const char* sFileCert, const char* sCertPwd, const
//     char* sFileCA, const char* sProcotol, bool bVerify, int nTimeOut);
 ...
 */
FIX_API_EXPORT HANDLE_CONN Fix_Connect(const char *sAddr,
                                       const char *sUser = "",
                                       const char *sPwd = "", int nTimeOut = 0);
 ...
 *  sess -- [in] 类型为HANDLE_SESSION的会话句柄。
 *  val  -- [in]
 * 字符串类型。用于表示客户业务发生的营业代码,必须是四位的营业部代码;
 * @return 返回数据:
 * 返回值为1表示成功; 0表示失败。
 */
 ...
 * sess -- [in] 类型为HANDLE_SESSION的会话句柄。
 * val  -- [in]
 * 字符串类型。用于表示客户业务到达的目标的营业代码,必须是四位的营业部代码;
 * @return 返回数据:
 * 返回值为1表示成功; 0表示失败。
 */
...
FIX_API_EXPORT bool Fix_SetDouble(HANDLE_SESSION sess, int id, double val,
                                  int precise = 6);
// 设置 RawData 字段
FIX_API_EXPORT bool Fix_SetItemBuf(HANDLE_SESSION sess, const char *val,
                                   int len);
 ...
 * row    -- [in][option]
 * 指定返回数据结果集中的行数(行数索引值以0开始).如果不设置这个值，
 * 表示读取第一行的数据。
 * @return 返回数据:
 * 返回指定行的的字符串数据。
 */
...
FIX_API_EXPORT FIX_INT64 Fix_GetInt64(HANDLE_SESSION sess, int id,
                                      int row = -1);
...
/*
 *@brief
 *启用/禁用HANDLE_CONN断线自动重连机制，默认启用，如果第三方采用自己管理HANDLE_CONN连接,
 *必须把自动重连关闭，避免不必要的资源浪费
 */
...
*/
FIX_API_EXPORT bool Fix_SubscribeByCustomer(HANDLE_SESSION sess,
                                            FIX_INT64 &nSubs, const char *sKHH,
                                            const char *sPwd, void *pFunc,
                                            void *pData);

FIX_API_EXPORT bool Fix_SubscribeByToken(HANDLE_SESSION sess, FIX_INT64 &nSubs,
                                         const char *sKHH, const char *stoken,
                                         void *pFunc, void *pData);
/*
    MDB客户数据订阅函数
    pFunc为回调函数指针,函数定义为: bool CallBackFunc( HANDLE_CONN conn,
   HANDLE_SESS sess, int64 nSubs, void *pData ) 其中conn为连接句柄,
   nSvc表示服务号，nSubs为Fix_MDBSubscribeByCustomer还回的值，用于指明某个订阅的数据。
        pszKHH, pszPwd,分别为客户号与客户密码[密码为明文]。
    pData就是订阅时的指针(可以为NULL值)
        返回值大于0,订阅成功，否则失败.返回值可用于退订函数(数据退订函数)的输入参数
*/
FIX_API_EXPORT FIX_INT64 Fix_MDBSubscribeByCustomer(HANDLE_CONN conn,
                                                    const char *sFunc,
                                                    void *pFunc, void *pData,
                                                    const char *sKHH,
                                                    const char *sPwd);
FIX_API_EXPORT FIX_INT64 Fix_SubscribeL2IndexByCode(HANDLE_CONN conn,
                                                    void *pFunc, void *pData,
                                                    const char *sJYS,
                                                    const char *sIndexCode);
FIX_API_EXPORT FIX_INT64 Fix_SubscribeL2QuoteByZQDM(HANDLE_CONN conn,
                                                    void *pFunc, void *pData,
                                                    const char *sJYS,
                                                    const char *sZQDM);
...
/*
   ...
   nSvc表示服务号，nSubs为Fix_MDBSubscribeByCustomer还回的值，用于指明某个订阅的数据。
    ...
    pData就是订阅时的指针(可以为NULL值)
        返回值大于0,订阅成功，否则失败.返回值可用于退订函数(数据退订函数)的输入参数
*/
FIX_API_EXPORT FIX_INT64 Fix_MDBSubscribeByCustomer_RZRQ(
    HANDLE_CONN conn, const char *sFunc, void *pFunc, void *pData,
    const char *sKHH, const char *sPwd);
...
FIX_API_EXPORT bool Fix_SetSubsPacketId(HANDLE_SESSION sess,
                                        FIX_INT64 nPacketId);
...
FIX_API_EXPORT bool Fix_SetSessDataString(HANDLE_SESSION hSess,
                                          const char *sData);
...
FIX_UDT_EXPORT std::string Fix_GetStringItem(HANDLE_SESSION sess, int id,
                                             int row = -1);
 ...
 * @brief 连接消息回调。
 * @param 参数说明: 连接句柄
 * @param 参数说明: pFunc 连接句柄（回调函数原型：bool Func(HANDLE hConn, int
 * nEvent【1:掉线，0：连接成功】,void *pData)）
...
/*
    ...
    pFunc为回调函数指针,函数定义为: bool CallBackFunc( HANDLE_CONN conn,
   HANDLE_SESS sess, int64 nSubs, void *pData ) 其中conn为连接句柄,
   nSvc表示服务号，nSubs为Fix_MDBSubscribeSopQuoteByCustomer还回的值，用于指明某个订阅的数据。
        pszKHH, pszPwd,分别为客户号与客户密码。
    pData就是订阅时的指针(可以为NULL值)
    pszJYS,pszHYDM,分别表示交易所、合约代码（多个代码以分号隔开）
        返回值大于0,订阅成功，否则失败.返回值可用于退订函数(数据退订函数)的输入参数
*/
FIX_INT64 Fix_MDBSubscribeSopQuoteByCustomer(HANDLE_CONN conn,
                                             const char *sFunc, void *pFunc,
                                             void *pData, const char *sKHH,
                                             const char *sPwd, const char *sJYS,
                                             const char *sHYDM);
/*
    MDB客户数据订阅函数(个股期权)
    pFunc为回调函数指针,函数定义为: bool CallBackFunc( HANDLE_CONN conn,
   HANDLE_SESS sess, int64 nSubs, void *pData ) 其中conn为连接句柄,
   nSvc表示服务号，nSubs为Fix_MDBSubscribeByCustomer_SOP还回的值，用于指明某个订阅的数据。
        pszKHH, pszPwd,分别为客户号与客户密码。
    pData就是订阅时的指针(可以为NULL值)
        返回值大于0,订阅成功，否则失败.返回值可用于退订函数(数据退订函数)的输入参数
*/
FIX_API_EXPORT FIX_INT64
Fix_MDBSubscribeByCustomer_SOP(HANDLE_CONN conn, const char *sFunc, void *pFunc,
                               void *pData, const char *sKHH, const char *sPwd);

// 订阅行情中指定证券代码最新行情信息
// pszJYS
// 订阅交易所范围，不同交易所使用分号分割譬如SH;SZ，空串表示订阅所有交易所
// pszZQDM
// 订阅证券范围，代码区间使用逗号分割，譬如订阅600100--600300表达成600100,600300，独立代码使用分号分割，譬如订阅600033和601166表达成600033;601166
FIX_API_EXPORT FIX_INT64 Fix_SubscribeQuoteByZQDM(HANDLE_CONN conn, void *pFunc,
                                                  void *pData, const char *sJYS,
                                                  const char *sZQDM);
 ...
 *@brief 获取字段缓存
 */
FIX_API_EXPORT bool Fix_GetItemBuf(HANDLE_SESSION sess, char *out, int &outlen,
                                   int row);

// 订阅回调函数,可以用这个函数读取推送的包序号
FIX_API_EXPORT FIX_INT64 Fix_GetPublishPacketId(HANDLE_SESSION sess,
                                                int row = -1);
...
返回数据:
    pData       --
加密成功，返回加密后的密文（可显示字符），直接覆盖原来输入的数据（pData）
                -- 加密失败：传入指针为空或缓冲区长度不够，返回空字符串
*/
FIX_API_EXPORT char *Fix_UniEncode(const char *sKHH, char *pData, int &io_len,
                                   int nDataLen = 0);
 ...
 */
FIX_API_EXPORT bool Fix_GetValWithIdByIndex(HANDLE_SESSION sess, int nRow,
                                            int nIndex, int &nFid, char *out,
                                            int &nSize);

/*券商专用函数*/
FIX_API_EXPORT FIX_INT64 Fix_MDBSubscribe_Sys(HANDLE_CONN conn,
                                              const char *sFunc, void *pFunc,
                                              void *pData, int nNodeId,
                                              const char *sKHH,
                                              const char *sPwd);
...
/*
 ...
 func         -- [in] 整型；指定的服务名称。
 pFunc        -- [in] 指定的回调数据指针。
 回调函数的传参格式: bool CallBack( HANDLE_CONN conn, HANDLE_SVC svc,
 HANDLE_SESSION sess ); pszCondition -- [in]
 指定服务限制条件。比如银行发起的业务，要求指定只是做某个银行的业务。
 或只是处理的指定区域中心的的业务。
 返回数据:
 返回一个相对一个连接句柄下唯一的服务句柄。
 如果返回值为NULL,表示服务注册失败; 成功不为NULL。
 */
FIX_API_EXPORT HANDLE_SVC Fix_RegisterSvc(HANDLE_CONN conn, const char *sFunc,
                                          const char *sName, void *pFunc,
...
/*
 * @brief
 * 创建服务应答数据;用于多行的数据应答。必须和Fix_CreateNextAnswer、Fix_CreateEndMark配合使用。【暂不支持】
 * 具体可以参照Sample2的代码。
 * @param 参数说明:
 * sess        -- [in] 类型为HANDLE_SESSION的会话句柄。
 * @return 返回数据:
...
// for struct data
// req
FIX_API_EXPORT bool Fix_SetStructData(HANDLE_SESSION sess, unsigned char *pData,
                                      int nSize);
// ans
FIX_API_EXPORT bool Fix_IsStruct(HANDLE_SESSION sess, int nRow = 0);
FIX_API_EXPORT bool Fix_GetStructData(HANDLE_SESSION sess, unsigned char *pData,
                                      int &nSize, int nRow = 0);
// add the interface
// 订阅函数
FIX_API_EXPORT bool Fix_Subscribe(HANDLE_SESSION sess, FIX_INT64 &nSubs,
                                  void *pFunc, void *pData);
// 取消订阅
...
FIX_API_EXPORT char *Fix_UniEncode3rd(const char *sKHH, char *pData,
                                      int &io_len, int nDataLen = 0);
FIX_API_EXPORT bool Fix_GetBody(HANDLE_SESSION sess, int nRow, char *&out_buf,
                                int &out_len);
// 注意：该接口目前暂且只能在fix_run 或者 fix_asyncrun 后调用【暂不支持】
FIX_API_EXPORT bool Fix_GetReqBody(HANDLE_SESSION sess, char *&out_buf,
                                   int &out_len);
FIX_API_EXPORT bool Fix_SetCustomerForBindThread(HANDLE_CONN conn,
                                                 const char *pszKHH);
...
#ifdef _WIN64
FIX_API_EXPORT unsigned char *Fix_GetReqStructData(HANDLE_SESSION sess,
                                                   int &nSize);
FIX_API_EXPORT void Fix_RegGlobalStructPublishFunc(void *pFunc);
FIX_API_EXPORT unsigned char *Fix_GetStructDataEx(HANDLE_SESSION sess,
                                                  int &nSize, int nRow = 0);
#else
FIX_UDT_EXPORT unsigned char *Fix_GetReqStructData(HANDLE_SESSION sess,
                                                   int &nSize);
FIX_UDT_EXPORT void Fix_RegGlobalStructPublishFunc(void *pFunc);
FIX_UDT_EXPORT unsigned char *Fix_GetStructDataEx(HANDLE_SESSION sess,
                                                  int &nSize, int nRow = 0);
#endif
/*
 ...
 *	0:失败(预留比如手机号不存在,手机号长度不正确的方式,暂时没用)
 */
// FIX_API_EXPORT int Fix_ReqVerificationCode(HANDLE_SESSION sess,int type,const
// char* mobileNum);
...
*	 type    --
[in]登录方式,0:普通验证码登录;1:手机验证登录,2:暂时的其他登录,默认普通登录,
