 ...
 */
ENCRYPT_API void *EncryptedPasswordRSA2048(const char *buf, unsigned int len,
                                           unsigned int *out_len,
                                           void *ext_buffer,
                                           unsigned int ext_buffer_len);
 ...
 * @return 		加密后的信息
 *
 * 注意：要求在接口内申请内存保存加密后的内容作为返回值，由调用者释放内存.加密公钥已写死
 */
ENCRYPT_API void *EncryptedWithKeyRSA2048(const char *buf, unsigned int len,
                                          unsigned int *out_len,
                                          void *ext_buffer,
                                          unsigned int ext_buffer_len);
 ...
 */
ENCRYPT_API void *DecryptPasswordRSA2048(const char *buf, unsigned int len,
                                         unsigned int *out_len,
                                         void *ext_buffer,
                                         unsigned int ext_buffer_len);
}
