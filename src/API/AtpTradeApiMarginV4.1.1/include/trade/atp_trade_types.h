...
// template:3193722197a4f5b21a92934cfb63f986
// code:c20e70ef23d405015c947c6063f8ff80
...
typedef int64_t
    ATPClientSeqIDType; ///< 用户系统消息序号需要全局保持唯一，由API使用者负责维护
...
typedef uint8_t
    BusinessAbstractType; ///< 抽象业务编码：1.现货业务，2.经纪期权，3.两融业务
...
typedef char
    ATPDesignatedTransactionSecurityIDType[9]; ///< 指定交易业务委托消息证券代码
...
typedef uint8_t
    ATPImmediateEffectType;    ///< 实时划转标志,0:非实时划转,1:实时划转
...
typedef uint32_t
    ATPParticularFlagsType; ///< 特殊委托标识，按位表示特殊委托，0-正常委托，第一位表示夜市委托
...
typedef uint8_t
    ATPEtfPurchaseRedemptionFlagType; ///< 是否返回ETF实时申赎成交：0-不返回，1-返回
typedef uint64_t ATPBatchClOrdNoType;      ///< 客户自定义委托批号
typedef int8_t ATPPrTypeType;              ///< 申赎类型
typedef int32_t ATPMaxCashRatioType;       ///< ETF最大现金替代比例N6(5)
typedef int32_t ATPPremiumRatioType;       ///< 申购溢价比例N7(5)
typedef int8_t ATPSubstituteFlagType;      ///< 现金替代标志
typedef char ATPFundPropertyType;          ///< 资金属性
typedef char ATPContractSerialNumType[25]; ///< 合同序号
typedef char ATPAgwUserPasswdType[51];     ///< 网关用户密码
typedef char ATPSecurityTypeCharType[6];   ///< 证券类型（字典值请咨询券商）
typedef char ATPCustNameType[256];         ///< 客户名
typedef uint8_t ATPRepaymentType;          ///< 偿还类型
typedef uint8_t ATPLiabilityDetailType;    ///< 负债明细类型
typedef std::string ATPCommentsType;       ///< 备注信息

union ErrorCodeUnion {
public:
  ErrorCodeUnion(int param) { error_code_t = param; }
