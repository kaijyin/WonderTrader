// code_generator_tag
// requests:e0926c423290e5d030fa2fff8376026d
// template:003bbb5d74c451382456bdbedeb4a71a
// code:57d109709b972005e82f9b68da93692d
...
public:
   ...
   *              ENCRYPT_SCHEMA
   * “0”：表示API进行转发，API不再进行加密。“1”：表示API对消息中的密码字段进行密码
   *              ATP_LOGIN_ENCRYPT_PASSWORD
   * 登入及密码修改消息中密码字段的加密算法so路径 ATP_ENCRYPT_PASSWORD
   * 除登入及密码修改外其他消息的密码字段加密算法 GM_SM2_PUBLIC_KEY_PATH
   * 采用国密算法时指定的公钥key路径 RSA_PUBLIC_KEY_PATH
   * 采用RSA算法时指定的公钥key路径 注：如果ENCRYPT_SCHEMA =
   * 1时，未配置ATP_LOGIN_ENCRYPT_PASSWORD时将采用默认内置RSA算法，未配置ATP_ENCRYPT_PASSWORD时将采用默认内置SHA256算法
   * @param[in]   is_enable_latency 是否启用时延统计服务
   * @return   	初始化结果
   * @note		只需要调用一次即可
   */
  static ATPRetCodeType
  Init(const std::string &station_name = "", const std::string &cfg_path = ".",
       const std::string &log_dir_path = "", bool record_all_flag = true,
       std::unordered_map<std::string, std::string> encrypt_cfg =
           std::unordered_map<std::string, std::string>(),
       bool connection_retention_flag = false, bool is_enable_latency = false);
   ...
   *	@return		放回放大的整数
   *	@note
   *0表示不放大，1表示放大10倍，2表示放大100倍，3表示放大100倍，4表示放大10000倍
   *
   */
   ...
   *	@param[in]	multi_channel_flag 回报消息主动订阅参数 0 取消 1 订阅
   *默认0xFF
   *	@return		返回连接函数错误码
   *	@note 	 	内部保留指针在API生命周期内不能被释放
   *
   */
  ATPRetCodeType Connect(const ATPConnectProperty &property,
                         ATPTradeHandler *handler,
                         uint8_t multi_channel_flag = 0xFF);
   ...
   *
   */
  ATPRetCodeType ConnectCust(const ATPConnectCustProperty &property,
                             ATPTradeHandler *handler,
                             uint8_t multi_channel_flag = 0xFF);
   ...
   * @return      错误码描述
   */
  static std::string GetErrorInfo(ErrorCodeUnion error_code,
                                  ATPErrorCodeTypeType error_code_type);
   ...
   * @type       order
   * @note
   * 支持沪深市场主板、科创板、创业板、北交所的股票、债券、基金的竞价买卖业务
   */
   ...
   */
  ATPRetCodeType
  ReqTibAfterHourAuctionOrder(const ATPReqTibAfterHourAuctionOrderMsg *msg);
   ...
   * @type       order
   * @note
   * 支持股票的新股申购（IPO）和增发发行；支持新债的申购；支持ETF基金、LOF基金的认购
   */
   ...
   */
  ATPRetCodeType
  ReqBondDistributionOrder(const ATPReqBondDistributionOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqOptionExerciseOrder(const ATPReqOptionExerciseOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqOptionCombinationExerciseOrder(
      const ATPReqOptionCombinationExerciseOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqOptionCombinationOrder(const ATPReqOptionCombinationOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqDesignatedTransactionOrder(const ATPReqDesignatedTransactionOrderMsg *msg);
   ...
   * @type       order
   * @note
   * 买卖方向为D-申购时，申购金额（cash_order_qty）字段必填；买卖方向为E-赎回时，赎回基金份额（order_qty）字段必填
   */
   ...
   */
  ATPRetCodeType
  ReqPasswordServiceOrder(const ATPReqPasswordServiceOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqWarrantExerciseOrder(const ATPReqWarrantExerciseOrderMsg *msg);

  /**
   * @brief      分级基金实时分拆合并业务委托函数（此业务已下线）
   ...
   */
  ATPRetCodeType
  ReqStructuredFundOrder(const ATPReqStructuredFundOrderMsg *msg);
   ...
   * @type       cancel
   * @note
   * 支持以下业务类型的撤单：盘后定价交易、大宗交易、债券分销、现货交易、ETF、盘后定价（科创板、创业板）交易业务、港股通买卖、港股通公司行为、网上发行认购、网络投票、质押式回购、配售业务、债券转股回售、要约收购、权证行权<br/>按“客户自定义委托批号”处理撤单时，若找不到对应原订单、或因其他原因撤单请求被拒绝,ATP不会生成Ack响应消息。
   */
   ...
   */
  ATPRetCodeType
  ReqReduceHoldingTransferOrder(const ATPReqReduceHoldingTransferOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqReduceHoldingTransferCancelOrder(
      const ATPReqReduceHoldingTransferCancelOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExtQueryContractSpecifications(
      const ATPReqExtQueryContractSpecificationsMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryCreditAmount(const ATPReqExtQueryCreditAmountMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryMarginUseable(const ATPReqExtQueryMarginUseableMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryFundLiability(const ATPReqExtQueryFundLiabilityMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryShareLiability(const ATPReqExtQueryShareLiabilityMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExtQueryContractSpecificationsManager(
      const ATPReqExtQueryContractSpecificationsManagerMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryMarginUnderlying(const ATPReqExtQueryMarginUnderlyingMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryFundAssetsInfo(const ATPReqExtQueryFundAssetsInfoMsg *msg);
   ...
   * @type       query
   * @note
   * 相比于成交查询函数，增强成交查询函数在3.1.5版本之前支持按客户订单编号查询单条记录，并且支持撤单和废单的响应查询；在3.1.5版本及之后两者无差别
   */
   ...
   * @type       query
   * @note
   * ATP3.1.5及之前版本，按照低版本查询方式，具体如下：<br/>--单市场查询场景<br/>传入account_id_array或者market_id
   * +
   * account_id_array.account_id进行查询<br/>--多市场查询场景<br/>传入多个account_id_array进行查询
   */
   ...
   */
  ATPRetCodeType
  ReqAccountContractShareQuery(const ATPReqAccountContractShareQueryMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqAccountContractFundQuery(const ATPReqAccountContractFundQueryMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqOptionCombinationPositionQuery(const ATPReqCombinationQueryMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqOptionCombinationOrderQuery(const ATPReqCombinationQueryMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqNewMaxOrderQtyQuery(const ATPReqNewMaxOrderQtyQueryMsg *msg);
   ...
   * @type       session
   * @note
   * 账户登录模式分为三种，分别为客户号登录模式、资金账号登录模式、证券账号登录模式；客户号、资金账号（营业部字段是否必填请咨询券商）、证券账号可以根据账户登录模式三个选填一个，其他账户字段不做要求
   */
   ...
   */
  ATPRetCodeType
  ReqExtFundTransferOther(const ATPReqExtFundTransferOtherMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqAfterHoursPricingOrder(const ATPReqAfterHoursPricingOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqBlockTradeIntentionOrder(const ATPReqBlockTradeIntentionOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqBlockTradeTransactionOrder(const ATPReqBlockTradeTransactionOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqBlockTradePricingOrder(const ATPReqBlockTradePricingOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqBlockTradeCancelOrder(const ATPReqBlockTradeCancelOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtOptionFundTransferOther(const ATPReqExtOptionFundTransfeOtherMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryExerciseProfitLoss(const ATPReqExtQueryExerciseProfitLossMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqCoverMaxOrderQtyQuery(const ATPReqCoverMaxOrderQtyQueryMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqTurnToDisposalOrder(const ATPReqTurnToDisposalOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqHKStocksThroughAuctionOrder(
      const ATPReqHKStocksThroughAuctionOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqHKCorporateActionOrder(const ATPReqHKCorporateActionOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqHKDesignationTransferOrder(const ATPReqHKDesignationTransferOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqHKCorporateActionCancelOrder(
      const ATPReqHKCorporateActionCancelOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqOptionCoveredTransUncoveredOrder(
      const ATPReqOptionCoveredTransUncoveredOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExtQueryCombinExerciseMaxOrderQty(
      const ATPReqExtQueryCombinExerciseMaxOrderQtyMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExtQueryCombinExerciseProfitLoss(
      const ATPReqExtQueryCombinExerciseProfitLossMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExternalQueryOptionCombinationMaxOrderQty(
      const ATPReqExternalQueryOptionCombinationMaxOrderQtyMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExternalInsTETransFundReq(const ATPReqExternalInsTETransFundReqMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqOptionExternalInsTETransFundReq(
      const ATPReqOptionExternalInsTETransFundReqMsg *msg);
   ...
   * @type       query
   * @note
   * 相比于资金查询函数，增强资金查询函数支持币种字段填空查询所有币种的资金（ATP3.1.7开始支持）
   */
   ...
   * @type       order
   * @note
   * 批量委托类型为单代码等量拆单时：单笔上限字段（max_order_qty）为必填项，其值为等量拆单的每笔订单数量；批量委托类型为单代码递减拆单时：递减数量字段（decrease_qty）为必填项，其值为每笔的减数。单笔上限字段（max_order_qty）为必填项，其值为每笔数量的上限；批量委托类型为多代码批量报单时：多证券委托组合为必填项
   */
  ATPRetCodeType
  ReqBatchCashAuctionOrder(const ATPReqBatchCashAuctionOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqDeductionToBePaidOrder(const ATPReqDeductionToBePaidOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExtQueryLockedAccountQty(
      const ATPReqExternalQueryLockedAccountContractQtyMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqOptionAuctionForceCloseOrder(
      const ATPReqOptionAuctionForceCloseOrderMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExternalQueryOptionMarginRiskInfoMsg(
      const ATPReqExternalQueryOptionMarginRiskInfoMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExternalQueryOptionSecurityInfoMsg(
      const ATPReqExternalQueryOptionSecurityInfoMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExternalQueryOptionUnderlyingSecurityInfo(
      const ATPReqExternalQueryOptionUnderlyingSecurityInfoMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExternalQueryOptionMdSnapShotArrayInfoMsg(
      const ATPReqExternalQueryOptionMdSnapShotArrayInfoMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqHKEMaxQueryOrder(const ATPReqHKExtQueryMaxQueryOrderQtyMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExtMarginTradingPrivateAmountQuery(
      const ATPReqExtQueryMarginTradingPrivateAmoutMsg *msg);
   ...
   * @note
   */
  ATPRetCodeType ReqExtSecuritiesLendingPrivateAmountQuery(
      const ATPReqExtQuerySecuritiesLendingPrivateAmountMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqThirdPartPassThrough(const ATPReqThirdPartPassThroughMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqOptionTradeOrderQuery(const ATPReqOptionTradeOrderQueryMsg *msg);
   ...
   */
  ATPRetCodeType ReqBankSecuritiesFundTransferOther(
      const ATPReqBankSecuritiesFundTransferOtherMsg *msg);
   ...
   * @note       将资金从银行账户转入资金账户中，具体支持情况请咨询券商
   */
  ATPRetCodeType ReqCashBankSecuritiesFundTransferOther(
      const ATPReqCashBankSecuritiesFundTransferOtherMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtRealtimeFundQuery(const ATPReqExtRealtimeFundQueryMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryNewShareInfo(const ATPReqExtQueryNewShareInfoMsg *msg);
   ...
   * @business   cash,margin
   * @type       query
   * @note       支持沪深市场新股发行申购额度查询<br/>ATP3.1.7
   * 开始支持科创板新股发行申购额度查询
   */
   ...
   */
  ATPRetCodeType
  ReqExtQuerySecurityInfo(const ATPReqExtQuerySecurityInfoMsg *msg);
   ...
   * @type       order
   * @note
   * 现阶段支持沪深市场主板、科创板、创业板的股票、债券、基金的竞价买卖业务；详细可见开发指南<br/>ATP最低版本要求V3.1.9.1
   */
   ...
   */
  ATPRetCodeType
  ReqCancelAlgoParentOrder(const ATPReqCancelAlgoParentOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExternalQueryAlgoParent(const ATPReqExternalQueryAlgoParentMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExternalQueryAlgoOrder(const ATPReqExternalQueryAlgoOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqBondConsultTradeOrder(const ATPReqBondConsultTradeOrderMsg *msg);
   ...
   */
  ATPRetCodeType ReqBondConsultTradeCancelOrder(
      const ATPReqBondConsultTradeCancelOrderMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqExtQueryOrderForwardInfo(const ATPReqExtQueryOrderForwardInfoMsg *msg);
   ...
   */
  ATPRetCodeType
  ReqOperateAlgoParentOrder(const ATPReqOperateAlgoParentOrderMsg *msg);
   ...
   * @type       query
   * @note
   * 发往集中交易系统进行持仓查询，具体支持情况请咨询券商，ATP3.1.11开始支持该接口
   */
   ...
   * @type       other
   * @note
   * 集中交易系统与ATP极速交易系统持仓互转，具体支持情况请咨询券商，ATP3.1.11开始支持该接口
   */
  ATPRetCodeType ReqExtShareTransfer(const ATPReqExtShareTransferMsg *msg);

  /**
   * @brief      ETF信息查询函数
   * @param[in]  ATPReqExtQueryETFInfoMsg
   * @return     kSuccess成功；其他失败
   * @business   cash
   * @type       query
   * @note
   */
  ATPRetCodeType ReqExtQueryETFInfo(const ATPReqExtQueryETFInfoMsg *msg);

  /**
   * @brief      ETF成分股信息查询函数
   * @param[in]  ATPReqExtQueryETFComponentInfoMsg
   * @return     kSuccess成功；其他失败
   * @business   cash
   * @type       query
   * @note
   */
  ATPRetCodeType
  ReqExtQueryETFComponentInfo(const ATPReqExtQueryETFComponentInfoMsg *msg);

  /**
   * @brief      新股中签结果查询函数
   * @param[in]  ATPReqNewShareHitQueryMsg
   * @return     kSuccess成功；其他失败
   * @business   cash
   * @type       query
   * @note
   */
  ATPRetCodeType ReqNewShareHitQuery(const ATPReqNewShareHitQueryMsg *msg);

  /**
   * @brief      网关用户密码修改函数
   * @param[in]  ATPReqAgwUserPasswdModifyMsg
   * @return     kSuccess成功；其他失败
   * @business   common
   * @type       other
   * @note
   * 客户号/资金账户及密码用于身份认证，保障安全性。密码修改成功后，系统会断开被修改网关用户当前已创建的连接
   */
  ATPRetCodeType
  ReqAgwUserPasswdModify(const ATPReqAgwUserPasswdModifyMsg *msg);

  /**
   * @brief      偿还明细查询
   * @param[in]  ATPReqExtQueryRepaymentDetailMsg
   * @return     kSuccess成功；其他失败
   * @business   margin
   * @type       query
   * @note
   */
  ATPRetCodeType
  ReqExtQueryRepaymentDetail(const ATPReqExtQueryRepaymentDetailMsg *msg);
