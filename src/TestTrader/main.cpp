...
public:
  ...
  bool createTrader(const char *moduleName) {
    ...
    m_funcDelTrader =
        (FuncDeleteTrader)DLLHelper::get_symbol(hInst, "deleteTrader");
    return true;
  }
  ...
  bool entrustLmt(bool isNet) {
    ...
    for (;;) {
      ...
      if (isNet) {
        ...
        encoding_print("合约: {}.{},价格: {},数量: {},方向: {},确认(y/n)? ",
                       exchg, code, price, qty, bs == 0 ? "买" : "卖");
      } else {
        ...
        encoding_print(
            "合约: {}.{},价格: {},数量: {},方向: {},开盘: {},确认(y/n)? ",
            exchg, code, price, qty, bs == 0 ? "多" : "空",
            offset == 0 ? "开" : "平");
      }
    ...
    if (!isNet) {
      ...
      entrust->setOffsetType(
          offset == 0 ? WOT_OPEN : (bNeedToday ? WOT_CLOSETODAY : WOT_CLOSE));
    } else {
    ...
    if (!isNet)
      WTSLogger::info(
          "[{}]Entrusting,Contract: {}.{},Price: {},Volume: {},Action: {}{}",
          m_pParams->getCString("user"), exchg, code, price, qty,
          offset == 0 ? "Open" : "Close", bs == 0 ? "Long" : "Short");
    else
      WTSLogger::info(
          "[{}]Entrusting,Contract: {}.{},Price: {},Volume: {},Action: {}",
          m_pParams->getCString("user"), exchg, code, price, qty,
  ...
  bool entrustMkt() {
    ...
    for (;;) {
      ...
      encoding_print("合约: {}.{},数量: {},方向: {},开盘: {},确认(y/n)? ",
                     exchg, code, qty, bs == 0 ? "多" : "空",
                     offset == 0 ? "开" : "平");
      char c;
      std::cin >> c;
      if (c == 'y')
    ...
    WTSLogger::info("[{}]Entrusting,Contract: {}.{},Price: MarketPx,Volume: "
                    "{},Action: {}{}",
                    m_pParams->getCString("user"), exchg, code, qty,
                    offset == 0 ? "Open" : "Close", bs == 0 ? "Long" : "Short");
  ...
  bool cancel() {
    ...
    if (ordInfo == NULL) {
      WTSLogger::info(
          "Order not exists, Check your orders or query orders first");
      return false;
    }

    WTSLogger::info("[{}]Canceling [{}]...", m_pParams->getCString("user"),
                    orderid);
    WTSEntrustAction *action =
        WTSEntrustAction::create(ordInfo->getCode(), ordInfo->getExchg());
...
public:
  virtual void handleEvent(WTSTraderEvent e, int32_t ec) {
    if (e == WTE_Connect) {
      if (ec == 0) {
        ...
        m_pTraderApi->login(m_pParams->getCString("user"),
                            m_pParams->getCString("pass"), "");
      } else {
        g_exitNow = true;
  ...
  virtual void onLoginResult(bool bSucc, const char *msg,
                             uint32_t tradingdate) {
    if (bSucc) {
      WTSLogger::info("[{}] Login Succ", m_pParams->getCString("user"));
      m_bLogined = true;
    } else {
      WTSLogger::info("[{}] Login Fail: {}", m_pParams->getCString("user"),
                      msg);
      g_exitNow = true;
    }
  ...
  virtual void onRspEntrust(WTSEntrust *entrust, WTSError *err) {
    if (err) {
      WTSLogger::info("[{}] Entrust fail: {}", m_pParams->getCString("user"),
                      err->getMessage());
      StdUniqueLock lock(g_mtxOpt);
      g_condOpt.notify_all();
    }
  ...
  virtual void onRspAccount(WTSArray *ayAccounts) {
    if (ayAccounts != NULL) {
      ...
      if (accInfo) {
        WTSLogger::info("[{}]Fund data updated, balance: {:.2f}",
                        m_pParams->getCString("user"), accInfo->getBalance());
      }
    }
  ...
  virtual void onRspPosition(const WTSArray *ayPositions) {
    ...
    WTSLogger::info("[{}] Positions updated, {} item totally",
                    m_pParams->getCString("user"), cnt);
    for (uint32_t i = 0; i < cnt; i++) {
      WTSPositionItem *posItem =
          (WTSPositionItem *)((WTSArray *)ayPositions)->at(i);
      if (posItem && posItem->getTotalPosition() > 0) {
        ...
        WTSLogger::info(
            "Position of {}({}) updated, {}[{}]", posItem->getCode(),
            posItem->getDirection() == WDT_LONG ? "L" : "S",
            posItem->getTotalPosition(), posItem->getAvailPosition());
      }
    }
    StdUniqueLock lock(g_mtxOpt);
    g_condOpt.notify_all();
  ...
  virtual void onRspOrders(const WTSArray *ayOrders) {
    ...
    for (uint32_t i = 0; i < cnt; i++) {
      ...
      if (ordInfo->isAlive()) {
        ...
        WTSLogger::info("[{}] Live order, code: {}, OrderId: {}",
                        m_pParams->getCString("user"), ordInfo->getCode(),
                        ordInfo->getOrderID());
      }
    }

    WTSLogger::info("[{}] Orders updated, {} orders totally, {} orders live",
                    m_pParams->getCString("user"), cnt, m_mapOrds->size());
  ...
  virtual void onRspTrades(const WTSArray *ayTrades) {
    ...
    WTSLogger::info("[{}] Trades updates, {} trades totally",
                    m_pParams->getCString("user"), cnt);
    StdUniqueLock lock(g_mtxOpt);
    g_condOpt.notify_all();
  }
  ...
  virtual void onRspSettlementInfo(uint32_t uDate, const char *content) {
    WTSLogger::info("[{}]{} Settlement received", m_pParams->getCString("user"),
                    uDate);
    ...
    if (orderInfo->getOrderState() != WOS_Canceled) {
      if (!orderid.empty()) {
        ...
        if (m_mapOrds->find(orderid) == m_mapOrds->end()) {
          WTSLogger::info("[{}] Entrust Success,OrderID: {}",
                          m_pParams->getCString("user"), orderid);
      ...
      if (orderid.empty()) {
        WTSLogger::info("[{}]Order {} Entrust failed and canceld:{}",
                        m_pParams->getCString("user"),
                        orderInfo->getEntrustID(), orderInfo->getStateMsg());
        StdUniqueLock lock(g_mtxOpt);
        g_condOpt.notify_all();
      } else {
        WTSLogger::info("[{}] Order {} canceld:{}",
                        m_pParams->getCString("user"), orderid,
  ...
  virtual void onPushTrade(WTSTradeInfo *tradeRecord) {
    WTSLogger::info("[{}] Trade pushed,contract: {},price: {},Volume: {}",
                    m_pParams->getCString("user"), tradeRecord->getCode(),
                    tradeRecord->getPrice(), tradeRecord->getVolume());

    if (g_riskAct) {
      WTSLogger::info("[{}]{}超过最大持仓Volume,禁止open",
                      m_pParams->getCString("user"), tradeRecord->getCode());
  ...
  virtual void onTraderError(WTSError *err) {
    if (err && err->getErrorCode() == WEC_ORDERCANCEL) {
      WTSLogger::info("[{}] Canceling failed: {}",
                      m_pParams->getCString("user"), err->getMessage());
      StdUniqueLock lock(g_mtxOpt);
      g_condOpt.notify_all();
    } else if (err && err->getErrorCode() == WEC_ORDERINSERT) {
      WTSLogger::info("[{}] Entrust failed: {}", m_pParams->getCString("user"),
                      err->getMessage());
      StdUniqueLock lock(g_mtxOpt);
      g_condOpt.notify_all();
    }
...
public:
  virtual IBaseDataMgr *getBaseDataMgr() { return &g_bdMgr; }
