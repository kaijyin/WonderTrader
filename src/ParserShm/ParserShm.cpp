  ...
  _thrd_parser.reset(new StdThread([this]() {
    ...
    while (!StdFile::exists(_path.c_str())) {
      write_log(_sink, LL_WARN,
                "[ParserShm] {} not exist yet, waiting for 2 seconds", _path);
    ...
    write_log(_sink, LL_INFO, "[ParserShm] {} loaded, start to receiving",
              _path);
    ...
    while (!_stopped) {
      ...
      if (lastIdx == UINT64_MAX) // 有数据，第一次检查，则直接定位到最后一条数据
      {
        ...
        continue;
          ...
          lastIdx ==
          NODATA_FLAG) // 之前没数据的时候检查了一次，现在有数据了，从0开始读取
      {
        lastIdx = 0;
      ...
      case 0: {
        const char *fullCode =
            fmtutil::format("{}.{}", item._tick.exchg, item._tick.code);
        ...
        if (it != _set_subs.end()) {
          ...
          if (recv_cnt % _gpsize == 0)
            write_log(_sink, LL_DEBUG, "[ParserShm] {} ticks received in total",
                      recv_cnt);
        }
      } break;
      case 1: {
        const char *fullCode =
            fmtutil::format("{}.{}", item._queue.exchg, item._queue.code);
        auto it = _set_subs.find(fullCode);
        if (it != _set_subs.end()) {
          ...
          if (recv_cnt % _gpsize == 0)
            write_log(_sink, LL_DEBUG,
                      "[ParserShm] {} queues received in total", recv_cnt);
        }
      } break;
      case 2: {
        const char *fullCode =
            fmtutil::format("{}.{}", item._order.exchg, item._order.code);
        auto it = _set_subs.find(fullCode);
        if (it != _set_subs.end()) {
          ...
          if (recv_cnt % _gpsize == 0)
            write_log(_sink, LL_DEBUG,
                      "[ParserShm] {} orders received in total", recv_cnt);
        }
      } break;
      case 3: {
        const char *fullCode =
            fmtutil::format("{}.{}", item._trans.exchg, item._trans.code);
        auto it = _set_subs.find(fullCode);
        if (it != _set_subs.end()) {
          ...
          if (recv_cnt % _gpsize == 0)
            write_log(_sink, LL_DEBUG,
                      "[ParserShm] {} transactions received in total",
                      recv_cnt);
        }
      } break;
      default:
...
bool ParserShm::isConnected() { return _queue != NULL; }
