...
WtRdmDtReaderAD::WtRdmDtReaderAD() : _base_data_mgr(NULL), _hot_mgr(NULL) {}
...
void WtRdmDtReaderAD::init(WTSVariant *cfg, IRdmDtReaderSink *sink) {
  ...
  pipe_rdmreader_log(sink, LL_INFO,
                     "WtRdmDtReaderAD initialized, root data folder is {}",
                     _base_dir);
}

WTSTickSlice *WtRdmDtReaderAD::readTickSliceByCount(const char *stdCode,
                                                    uint32_t count,
                                                    uint64_t etime /* = 0 */) {
  // TODO: 以后再来实现吧
  return NULL;
...
WTSTickSlice *WtRdmDtReaderAD::readTickSliceByRange(const char *stdCode,
                                                    uint64_t stime,
                                                    uint64_t etime /* = 0 */) {
  ...
  uint32_t endTDate =
      _base_data_mgr->calcTradingDate(stdPID.c_str(), rDate, rTime, false);
  uint32_t beginTDate =
      _base_data_mgr->calcTradingDate(stdPID.c_str(), lDate, lTime, false);
  ...
  if (isEmpty) {
    ...
    LMDBHftKey lKey(cInfo._exchg, cInfo._code, beginTDate,
                    lTime * 100000 + lSecs);
    LMDBHftKey rKey(cInfo._exchg, cInfo._code, endTDate,
                    rTime * 100000 + rSecs);
    ...
    if (cnt > 0) {
      ...
      tickList._first_tick_time =
          (uint64_t)curTs.trading_date * 1000000000 +
          sInfo->offsetTime(curTs.action_time / 100000, true) +
          curTs.action_time % 100000;

      curTs = tickList._ticks.back();
      tickList._last_tick_time =
          (uint64_t)curTs.trading_date * 1000000000 +
          sInfo->offsetTime(curTs.action_time / 100000, false) +
          curTs.action_time % 100000;

      pipe_rdmreader_log(
          _sink, LL_DEBUG, "{} ticks between [{},{}] of {} loaded to cache",
          cnt, tickList._first_tick_time, tickList._last_tick_time, stdCode);
    }
  } else {
    if (bNeedOlder) {
      // 读取更早的数据
      ...
      LMDBHftKey rKey(cInfo._exchg, cInfo._code,
                      (uint32_t)(tickList._first_tick_time / 1000000000),
                      (uint32_t)(tickList._first_tick_time % 1000000000));
      LMDBHftKey lKey(cInfo._exchg, cInfo._code, beginTDate,
                      lTime * 100000 + lSecs);
      int cnt = query.get_range(
          std::string((const char *)&lKey, sizeof(lKey)),
          std::string((const char *)&rKey, sizeof(rKey)),
          [this, &tickList](const ValueArray &ayKeys,
            ...
            memcpy(&ayTicks[idx], tickList._ticks.data(),
                   sizeof(WTSTickStruct) * tickList._ticks.size());
      ...
      if (cnt > 0) {
        ...
        pipe_rdmreader_log(_sink, LL_DEBUG,
                           "{} prev ticks of {} loaded to cache", cnt, stdCode);
      }
    }
    ...
    if (bNeedNewer) {
      ...
      LMDBHftKey lKey(cInfo._exchg, cInfo._code,
                      (uint32_t)(tickList._last_tick_time / 1000000000),
                      (uint32_t)(tickList._last_tick_time % 1000000000));
      LMDBHftKey rKey(cInfo._exchg, cInfo._code, endTDate,
                      rTime * 100000 + rSecs);
      ...
      if (cnt > 0) {
        ...
        pipe_rdmreader_log(_sink, LL_DEBUG,
                           "{} newer ticks of {} loaded to cache", cnt,
                           stdCode);
      }
    }
  }
  ...
  // 全部读取完成以后，再生成切片
  {
    ...
    WTSTickStruct *pTick = std::lower_bound(
        &tickList._ticks[0], &tickList._ticks[0] + (tickList._ticks.size() - 1),
        eTick, [](const WTSTickStruct &a, const WTSTickStruct &b) {
    ...
    if (pTick->action_date > sTick.action_date ||
        (pTick->action_date == sTick.action_date &&
         pTick->action_time > sTick.action_time)) {
      cnt = eIdx + 1;
    } else {
      pTick = std::lower_bound(
          &tickList._ticks[0],
          &tickList._ticks[0] + (tickList._ticks.size() - 1), sTick,
...
WTSKlineSlice *
WtRdmDtReaderAD::readKlineSliceByCount(const char *stdCode,
                                       WTSKlinePeriod period, uint32_t count,
                                       uint64_t etime /* = 0 */) {
  // TODO: 以后再来实现吧
  return NULL;
}

WTSTickSlice *WtRdmDtReaderAD::readTickSliceByDate(const char *stdCode,
                                                   uint32_t uDate) {
  // TODO: 以后再来实现吧
  return NULL;
}
...
WTSKlineSlice *
WtRdmDtReaderAD::readKlineSliceByRange(const char *stdCode,
                                       WTSKlinePeriod period, uint64_t stime,
                                       uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  WTSCommodityInfo *commInfo =
      _base_data_mgr->getCommodity(cInfo._exchg, cInfo._product);
  ...
  if (barsList._bars.empty()) {
    pipe_rdmreader_log(_sink, LL_DEBUG, "Reading back {} bars of {}.{}...",
                       PERIOD_NAME[period], cInfo._exchg, cInfo._code);
    WtLMDBQuery query(*db);
    ...
    int cnt = query.get_range(
        std::string((const char *)&lKey, sizeof(lKey)),
        std::string((const char *)&rKey, sizeof(rKey)),
        [this, &barsList, &lKey](const ValueArray &ayKeys,
                                 const ValueArray &ayVals) {
          if (ayVals.empty())
            return;
    ...
    WtLMDBQuery query(*db);
    LMDBBarKey rKey(cInfo._exchg, cInfo._code, 0xffffffff);
    LMDBBarKey lKey(cInfo._exchg, cInfo._code,
                    (uint32_t)barsList._last_bar_time);
    int cnt = query.get_range(
        std::string((const char *)&lKey, sizeof(lKey)),
        std::string((const char *)&rKey, sizeof(rKey)),
        [this, &barsList, &lKey](const ValueArray &ayKeys,
  ...
  //
  {
    ...
    WTSBarStruct *pBar = std::lower_bound(
        &barsList._bars[0], &barsList._bars[0] + (barsList._bars.size() - 1),
        eBar, [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
          if (isDay)
    ...
    if ((isDay && pBar->date > sBar.date) ||
      ...
      pBar = std::lower_bound(
          &barsList._bars[0], &barsList._bars[0] + (barsList._bars.size() - 1),
          sBar, [isDay](const WTSBarStruct &a, const WTSBarStruct &b) {
            if (isDay)
              return a.date < b.date;
            else
              return a.time < b.time;
    ...
    WTSKlineSlice *slice =
        WTSKlineSlice::create(stdCode, period, 1, pHead, cnt);
    return slice;
  }
}

WtRdmDtReaderAD::WtLMDBPtr WtRdmDtReaderAD::get_k_db(const char *exchg,
  ...
  if (!dbPtr->open(path.c_str())) {
    pipe_rdmreader_log(_sink, LL_ERROR, "Opening {} db if {} failed: {}",
                       subdir, exchg, dbPtr->errmsg());
    return std::move(WtLMDBPtr());
  } else {
...
WtRdmDtReaderAD::WtLMDBPtr WtRdmDtReaderAD::get_t_db(const char *exchg,
                                                     const char *code) {
  ...
  boost::filesystem::create_directories(path);
  if (!dbPtr->open(path.c_str())) {
    pipe_rdmreader_log(_sink, LL_ERROR, "Opening tick db of {}.{} failed: {}",
