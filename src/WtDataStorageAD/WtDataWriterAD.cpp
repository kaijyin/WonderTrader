...
WtDataWriterAD::WtDataWriterAD()
    : _terminated(false), _log_group_size(1000), _disable_day(false),
      _disable_min1(false), _disable_min5(false), _disable_tick(false),
      _tick_cache_block(nullptr), _tick_mapsize(16 * 1024 * 1024),
...
void WtDataWriterAD::loadCache() {
  if (_tick_cache_file == NULL) {
    ...
    if (!BoostFile::exists(filename.c_str())) {
      uint64_t uSize =
          sizeof(RTTickCache) + sizeof(TickCacheItem) * CACHE_SIZE_STEP_AD;
      BoostFile bf;
    ...
    if (bNew) {
      ...
      for (uint32_t i = 0; i < _tick_cache_block->_size; i++) {
        ...
        std::string key =
            StrUtil::printf("%s.%s", item._tick.exchg, item._tick.code);
  ...
  if (_m1_cache.empty()) {
    ...
    if (!BoostFile::exists(filename.c_str())) {
      uint64_t uSize =
          sizeof(RTBarCache) + sizeof(BarCacheItem) * CACHE_SIZE_STEP_AD;
      BoostFile bf;
      bf.create_new_file(filename.c_str());
      bf.truncate_file((uint32_t)uSize);
      bf.close_file();
    ...
    _m1_cache._cache_block->_size =
        min(_m1_cache._cache_block->_size, _m1_cache._cache_block->_capacity);
  ...
  if (_m5_cache.empty()) {
    ...
    _m5_cache._cache_block->_size =
        min(_m5_cache._cache_block->_size, _m5_cache._cache_block->_capacity);
  ...
  if (_d1_cache.empty()) {
    ...
    _d1_cache._cache_block->_size =
        min(_d1_cache._cache_block->_size, _d1_cache._cache_block->_capacity);
...
void *WtDataWriterAD::resizeRTBlock(BoostMFPtr &mfPtr, uint32_t nCount) {
  ...
  try {
    ...
    pipe_writer_log(
        _sink, LL_ERROR,
        "Exception occured while expanding RT cache file of {}[{}]: {}",
...
void WtDataWriterAD::pipeToTicks(WTSContractInfo *ct, WTSTickData *curTick) {
  ...
  if (db) {
    ...
    uint32_t offTime = ct->getCommInfo()->getSessionInfo()->offsetTime(
                           actTime / 100000, true) +
                       actTime % 100000;

    LMDBHftKey key(ct->getExchg(), ct->getCode(), curTick->tradingdate(),
                   offTime);
    WtLMDBQuery query(*db);
    if (!query.put_and_commit((void *)&key, sizeof(key),
                              &curTick->getTickStruct(),
                              sizeof(WTSTickStruct))) {
      pipe_writer_log(_sink, LL_ERROR, "pipe tick of {} to db failed: {}",
                      ct->getFullCode(), db->errmsg());
    }
  }
  ...
  for (auto &item : _dumpers) {
    ...
    if (!bSucc) {
      pipe_writer_log(_sink, LL_ERROR,
                      "pipe tick data of {} via extended dumper {} failed",
                      ct->getFullCode(), id);
    }
  }
}

void WtDataWriterAD::pipeToDayBars(WTSContractInfo *ct,
                                   const WTSBarStruct &bar) {
  // 直接落地
  WtLMDBPtr db = get_k_db(ct->getExchg(), KP_DAY);
  if (db) {
    LMDBBarKey key(ct->getExchg(), ct->getCode(), bar.date);
    WtLMDBQuery query(*db);
    if (!query.put_and_commit((void *)&key, sizeof(key), (void *)&bar,
                              sizeof(WTSBarStruct))) {
      pipe_writer_log(_sink, LL_ERROR, "pipe day bar @ {} of {} to db failed",
                      bar.date, ct->getFullCode());
    } else {
      pipe_writer_log(_sink, LL_DEBUG, "day bar @ {} of {} piped to db",
                      bar.date, ct->getFullCode());
    }
  }
  ...
  for (auto &item : _dumpers) {
    ...
    bool bSucc =
        dumper->dumpHisBars(ct->getFullCode(), "d1", (WTSBarStruct *)&bar, 1);
    if (!bSucc) {
      pipe_writer_log(_sink, LL_ERROR,
                      "pipe day bar @ {} of {} via extended dumper {} failed",
                      bar.date, ct->getFullCode(), id);
    }
  }
}

void WtDataWriterAD::pipeToM1Bars(WTSContractInfo *ct,
                                  const WTSBarStruct &bar) {
  // 直接落地
  ...
  if (db) {
    ...
    if (!query.put_and_commit((void *)&key, sizeof(key), (void *)&bar,
                              sizeof(WTSBarStruct))) {
      pipe_writer_log(_sink, LL_ERROR, "pipe m1 bar @ {} of {} to db failed",
                      bar.time, ct->getFullCode());
    } else {
      pipe_writer_log(_sink, LL_DEBUG, "m1 bar @ {} of {} piped to db",
                      bar.time, ct->getFullCode());
    }
  }
  ...
  for (auto &item : _dumpers) {
    ...
    bool bSucc =
        dumper->dumpHisBars(ct->getFullCode(), "m1", (WTSBarStruct *)&bar, 1);
    if (!bSucc) {
      pipe_writer_log(_sink, LL_ERROR,
                      "pipe m1 bar @ {} of {} via extended dumper {} failed",
                      bar.time, ct->getFullCode(), id);
    }
  }
}

void WtDataWriterAD::pipeToM5Bars(WTSContractInfo *ct,
                                  const WTSBarStruct &bar) {
  // 直接落地
  ...
  if (db) {
    ...
    if (!query.put_and_commit((void *)&key, sizeof(key), (void *)&bar,
                              sizeof(bar))) {
      pipe_writer_log(_sink, LL_ERROR, "pipe m5 bar @ {} of {} to db failed",
                      bar.time, ct->getFullCode());
    } else {
      pipe_writer_log(_sink, LL_DEBUG, "m5 bar @ {} of {} piped to db",
                      bar.time, ct->getFullCode());
    }
  }
  ...
  for (auto &item : _dumpers) {
    ...
    bool bSucc =
        dumper->dumpHisBars(ct->getFullCode(), "m5", (WTSBarStruct *)&bar, 1);
    if (!bSucc) {
      pipe_writer_log(_sink, LL_ERROR,
                      "pipe m5 bar @ {} of {} via extended dumper {} failed",
                      bar.time, ct->getFullCode(), id);
    }
  }
}
...
void WtDataWriterAD::updateBarCache(WTSContractInfo *ct, WTSTickData *curTick) {
  ...
  WTSSessionInfo *sInfo =
      _bd_mgr->getSessionByCode(curTick->code(), curTick->exchg());
  ...
  if (!_disable_day && _d1_cache._cache_block) {
    ...
    if (_d1_cache._idx.find(key) == _d1_cache._idx.end()) {
      ...
      if (_d1_cache._cache_block->_size >= _d1_cache._cache_block->_capacity) {
        _d1_cache._cache_block =
            (RTBarCache *)resizeRTBlock<RTBarCache, BarCacheItem>(
                _d1_cache._file_ptr,
                _d1_cache._cache_block->_capacity + CACHE_SIZE_STEP_AD);
        pipe_writer_log(_sink, LL_INFO, "day cache resized to {} items",
                        _d1_cache._cache_block->_capacity);
      }
      bNewCode = true;
    } else {
  ...
  if (!_disable_min1 && _m1_cache._cache_block) {
    ...
    if (_m1_cache._idx.find(key) == _m1_cache._idx.end()) {
      ...
      if (_m1_cache._cache_block->_size >= _m1_cache._cache_block->_capacity) {
        _m1_cache._cache_block =
            (RTBarCache *)resizeRTBlock<RTBarCache, BarCacheItem>(
                _m1_cache._file_ptr,
                _m1_cache._cache_block->_capacity + CACHE_SIZE_STEP_AD);
        pipe_writer_log(_sink, LL_INFO, "m1 cache resized to {} items",
                        _m1_cache._cache_block->_capacity);
      }
      bNewCode = true;
    } else {
  ...
  if (!_disable_min5 && _m5_cache._cache_block) {
    ...
    if (_m5_cache._idx.find(key) == _m5_cache._idx.end()) {
      ...
      if (_m5_cache._cache_block->_size >= _m5_cache._cache_block->_capacity) {
        _m5_cache._cache_block =
            (RTBarCache *)resizeRTBlock<RTBarCache, BarCacheItem>(
                _m5_cache._file_ptr,
                _m5_cache._cache_block->_capacity + CACHE_SIZE_STEP_AD);
        pipe_writer_log(_sink, LL_INFO, "m5 cache resized to {} items",
                        _m5_cache._cache_block->_capacity);
      }
      bNewCode = true;
    } else {
...
WTSTickData *WtDataWriterAD::getCurTick(const char *code,
                                        const char *exchg /* = ""*/) {
  if (strlen(code) == 0)
    return NULL;
...
bool WtDataWriterAD::updateTickCache(WTSContractInfo *ct, WTSTickData *curTick,
                                     uint32_t procFlag) {
...
WtDataWriterAD::WtLMDBPtr WtDataWriterAD::get_k_db(const char *exchg,
  ...
  std::string path =
      StrUtil::printf("%s%s/%s/", _base_dir.c_str(), subdir.c_str(), exchg);
  boost::filesystem::create_directories(path);
  if (!dbPtr->open(path.c_str(), _kline_mapsize)) {
    if (_sink)
      pipe_writer_log(_sink, LL_ERROR, "Opening {} db at {} failed: {}", subdir,
                      path, dbPtr->errmsg());
    return std::move(WtLMDBPtr());
  }
...
WtDataWriterAD::WtLMDBPtr WtDataWriterAD::get_t_db(const char *exchg,
                                                   const char *code) {
  ...
  WtLMDBPtr dbPtr(new WtLMDB(false));
  std::string path =
      StrUtil::printf("%sticks/%s/%s", _base_dir.c_str(), exchg, code);
  boost::filesystem::create_directories(path);
  if (!dbPtr->open(path.c_str(), _tick_mapsize)) {
    if (_sink)
      pipe_writer_log(_sink, LL_ERROR, "Opening tick db at {} failed: %s", path,
                      dbPtr->errmsg());
    return std::move(WtLMDBPtr());
  }
