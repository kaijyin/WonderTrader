...
void WtDataReaderAD::init(WTSVariant *cfg, IDataReaderSink *sink,
  ...
  pipe_reader_log(sink, LL_INFO,
                  "WtDataReaderAD initialized, root data folder is {}",
                  _base_dir);
}

WTSTickSlice *WtDataReaderAD::readTickSlice(const char *stdCode, uint32_t count,
                                            uint64_t etime /* = 0 */) {
  ...
  uint32_t endTDate =
      _base_data_mgr->calcTradingDate(stdPID.c_str(), curDate, curTime, false);
  ...
  if (commInfo->isFuture()) {
    ...
    if (strlen(ruleTag) > 0)
      curCode =
          _hot_mgr->getCustomRawCode(ruleTag, cInfo.stdCommID(), endTDate);
    // else if (cInfo.isHot())
    //	curCode = _hot_mgr->getRawCode(cInfo._exchg, cInfo._product, endTDate);
    // else if (cInfo.isSecond())
    //	curCode = _hot_mgr->getSecondRawCode(cInfo._exchg, cInfo._product,
    //endTDate);
  }
  ...
  if (reload_flag == 1) {
    ...
    LMDBHftKey lKey(cInfo._exchg, cInfo._code,
                    ...
                    (uint32_t)(last_access_time % 1000000000));
    LMDBHftKey rKey(cInfo._exchg, cInfo._code, (uint32_t)(etime / 1000000000),
                    (uint32_t)(etime % 1000000000));
    int cnt = query.get_range(
    ...
    if (cnt > 0)
      pipe_reader_log(_sink, LL_DEBUG,
                      "{} ticks after {} of {} append to cache", cnt,
                      last_access_time, stdCode);
    ...
    int cnt = query.get_lowers(
        std::string((const char *)&lKey, sizeof(lKey)),
        std::string((const char *)&rKey, sizeof(rKey)), count,
        [this, &tickList](const ValueArray &ayKeys, const ValueArray &ayVals) {
    ...
    pipe_reader_log(_sink, LL_DEBUG,
                    "{} ticks of {} loaded to cache for the first time", cnt,
                    stdCode);
  }
  ...
  if (cnt_2 >= count) {
    ...
    return WTSTickSlice::create(stdCode, &tickList._ticks[ayTwo.second - count],
                                count);
  } else {
    // 如果array_two条数不够，需要再从array_one提取
    auto ayOne = tickList._ticks.array_one();
    auto diff = count - cnt_2;
    auto ret = WTSTickSlice::create(
        stdCode, &tickList._ticks[ayOne.second - diff], diff);
    if (cnt_2 > 0)
...
std::string WtDataReaderAD::read_bars_to_buffer(const char *exchg,
                                                const char *code,
  ...
  query.get_range(
      ...
      [this, &buffer](const ValueArray &ayKeys, const ValueArray &ayVals) {
        ...
        memcpy((void *)buffer.data(), ayVals.data(),
               sizeof(WTSBarStruct) * ayVals.size());
      });
  return std::move(buffer);
}

bool WtDataReaderAD::cacheBarsFromStorage(const std::string &key,
                                          const char *stdCode,
                                          WTSKlinePeriod period,
                                          uint32_t count) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  ...
  int cnt = query.get_lowers(
      std::string((const char *)&lKey, sizeof(lKey)),
      std::string((const char *)&rKey, sizeof(rKey)), count,
      [this, &barList, &lKey](const ValueArray &ayKeys,
                              const ValueArray &ayVals) {
        if (ayVals.empty())
          return;
  ...
  pipe_reader_log(_sink, LL_DEBUG, "{} {} bars of {} loaded to cache", cnt,
                  PERIOD_NAME[period], stdCode);
  return true;
}

void WtDataReaderAD::update_cache_from_lmdb(BarsList &barsList,
                                            const char *exchg, const char *code,
                                            WTSKlinePeriod period,
                                            uint32_t &lastBarTime) {
  ...
  int cnt = query.get_uppers(
      std::string((const char *)&lKey, sizeof(lKey)),
      std::string((const char *)&rKey, sizeof(rKey)), 9999,
      [this, &barsList, isDay, &lastBarTime](const ValueArray &ayKeys,
                                             const ValueArray &ayVals) {
        std::size_t cnt = ayVals.size();
  ...
  pipe_reader_log(_sink, LL_DEBUG, "{} bars of {}.{} updated to {}",
                  PERIOD_NAME[period], exchg, code,
                  isDay ? barsList._bars.back().date
                        : barsList._bars.back().time);
}

WTSBarStruct *WtDataReaderAD::get_rt_cache_bar(const char *exchg,
                                               const char *code,
                                               WTSKlinePeriod period) {
  ...
  if (wrapper != NULL) {
    if (wrapper->empty()) {
      ...
      do {
        ...
        wrapper->_cache_block->_size =
            min(wrapper->_cache_block->_size, wrapper->_cache_block->_capacity);
      ...
      if (wrapper->_last_size != wrapper->_cache_block->_size) {
        for (uint32_t i = wrapper->_last_size; i < wrapper->_cache_block->_size;
             i++) {
          const BarCacheItem &item = wrapper->_cache_block->_items[i];
          wrapper->_idx[StrUtil::printf("%s.%s", item._exchg, item._code)] = i;
        }
...
WTSKlineSlice *WtDataReaderAD::readKlineSlice(const char *stdCode,
                                              WTSKlinePeriod period,
                                              uint32_t count,
                                              uint64_t etime /* = 0 */) {
  CodeHelper::CodeInfo cInfo = CodeHelper::extractStdCode(stdCode, _hot_mgr);
  ...
  if (barsList._last_req_time < etime) {
    ...
    if (lastBarTime < etime) {
      ...
      update_cache_from_lmdb(barsList, cInfo._exchg, curCode.c_str(), period,
                             lastBarTime);
    ...
    if (lastBarTime < etime) {
      ...
      if (rtBar != NULL) {
        ...
        if (cacheBarTime > etime) {
          ...
          pipe_reader_log(
              _sink, LL_DEBUG,
              "{} bars @  {} of {} updated from cache instead of lmdb in {}",
              PERIOD_NAME[period], etime, stdCode, __FUNCTION__);
        }
      }
    }
  }
  ...
  if (cnt_2 >= count) {
    // 如果array_two条数足够，则直接返回数据块
    return WTSKlineSlice::create(stdCode, period, 1,
                                 &barsList._bars[ayTwo.second - count], count);
  } else {
    // 如果array_two条数不够，需要再从array_one提取
    auto ayOne = barsList._bars.array_one();
    auto diff = count - cnt_2;
    auto ret = WTSKlineSlice::create(
        stdCode, period, 1, &barsList._bars[ayOne.second - diff], diff);
    if (cnt_2 > 0)
      ret->appendBlock(ayTwo.first, cnt_2);
    return ret;
  }
...
void WtDataReaderAD::onMinuteEnd(uint32_t uDate, uint32_t uTime,
                                 uint32_t endTDate /* = 0 */) {
  // 这里应该触发检查
  ...
  for (auto it = _bars_cache.begin(); it != _bars_cache.end(); it++) {
    ...
    if (barsList._period != KP_DAY) {
      ...
      pipe_reader_log(_sink, LL_DEBUG,
                      "Updating {} bars of {} in section ({},{}]",
                      PERIOD_NAME[barsList._period], barsList._code,
                      lastBarTime, endBarTime);
      update_cache_from_lmdb(barsList, barsList._exchg.c_str(), cInfo._code,
                             barsList._period, lastBarTime);
      if (lastBarTime < endBarTime) {
        WTSBarStruct *rtBar =
            get_rt_cache_bar(cInfo._exchg, cInfo._code, barsList._period);
        if (rtBar->time > lastBarTime && rtBar->time <= endBarTime) {
          ...
          pipe_reader_log(
              _sink, LL_DEBUG,
              "{} bars @ {} of {} updated from cache instead of lmdb in {}",
              PERIOD_NAME[barsList._period], endBarTime, barsList._code,
              __FUNCTION__);
        }
      }
    } else if (endTDate != 0) {
...
WtDataReaderAD::WtLMDBPtr WtDataReaderAD::get_k_db(const char *exchg,
                                                   WTSKlinePeriod period) {
  ...
  if (!dbPtr->open(path.c_str())) {
    pipe_reader_log(_sink, LL_ERROR, "Opening {} db if {} failed: {}", subdir,
                    exchg, dbPtr->errmsg());
    return std::move(WtLMDBPtr());
  } else {
...
WtDataReaderAD::WtLMDBPtr WtDataReaderAD::get_t_db(const char *exchg,
                                                   const char *code) {
  ...
  boost::filesystem::create_directories(path);
  if (!dbPtr->open(path.c_str())) {
    pipe_reader_log(_sink, LL_ERROR, "Opening tick db of {}.{} failed: {}",
                    exchg, code, dbPtr->errmsg());
    return std::move(WtLMDBPtr());
  } else {
