...
void WtBtDtReaderAD::init(WTSVariant *cfg, IBtDtReaderSink *sink) {
  ...
  pipe_btreader_log(_sink, LL_INFO,
                    "WtBtDtReaderAD initialized, root data dir is {}",
                    _base_dir);
}

bool WtBtDtReaderAD::read_raw_bars(const char *exchg, const char *code,
  ...
  pipe_btreader_log(_sink, LL_DEBUG, "Reading back {} bars of {}.{}...",
  ...
  int cnt = query.get_range(std::string((const char *)&lKey, sizeof(lKey)),
                            ...
                            [this, &buffer, &lKey](const ValueArray &ayKeys,
...
bool WtBtDtReaderAD::read_raw_ticks(const char *exchg, const char *code,
  ...
  pipe_btreader_log(_sink, LL_DEBUG, "Reading back ticks on {} of {}.{}...",
                    uDate, exchg, code);
  WtLMDBQuery query(*db);
...
WtBtDtReaderAD::WtLMDBPtr WtBtDtReaderAD::get_k_db(const char *exchg,
  ...
  if (!dbPtr->open(path.c_str())) {
    pipe_btreader_log(_sink, LL_ERROR, "Opening {} db if {} failed: {}", subdir,
                      exchg, dbPtr->errmsg());
    return std::move(WtLMDBPtr());
  } else {
...
WtBtDtReaderAD::WtLMDBPtr WtBtDtReaderAD::get_t_db(const char *exchg,
                                                   const char *code) {
  ...
  if (!dbPtr->open(path.c_str())) {
    pipe_btreader_log(_sink, LL_ERROR, "Opening tick db of {}.{} failed: {}",
