...
ParserCTPMini::~ParserCTPMini() { m_pUserAPI = NULL; }
...
void ParserCTPMini::release() { disconnect(); }
...
void ParserCTPMini::OnRspError(CThostFtdcRspInfoField *pRspInfo, int nRequestID,
                               bool bIsLast) {
...
void ParserCTPMini::OnRspUserLogin(CThostFtdcRspUserLoginField *pRspUserLogin,
...
void ParserCTPMini::OnRspUserLogout(CThostFtdcUserLogoutField *pUserLogout,
                                    CThostFtdcRspInfoField *pRspInfo,
                                    int nRequestID, bool bIsLast) {
  if (m_sink) {
...
void ParserCTPMini::OnFrontDisconnected(int nReason) {
  if (m_sink) {
    write_log(m_sink, LL_ERROR,
              "[ParserCTPMini] Market data server disconnected: {}", nReason);
    m_sink->handleEvent(WPE_Close, 0);
  }
}

void ParserCTPMini::OnRspUnSubMarketData(
    CThostFtdcSpecificInstrumentField *pSpecificInstrument,
    CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {}

void ParserCTPMini::OnRtnDepthMarketData(
  ...
  WTSContractInfo *contract = m_pBaseDataMgr->getContract(
...
void ParserCTPMini::OnRspSubMarketData(
    CThostFtdcSpecificInstrumentField *pSpecificInstrument,
    CThostFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
...
void ParserCTPMini::OnHeartBeatWarning(int nTimeLapse) {
  if (m_sink)
    write_log(m_sink, LL_INFO, "[ParserCTPMini] Heartbeating, elapse: {}",
              nTimeLapse);
}
...
void ParserCTPMini::ReqUserLogin() {
  ...
  if (iResult != 0) {
    if (m_sink)
      write_log(m_sink, LL_ERROR,
                "[ParserCTPMini] Sending login request failed: {}", iResult);
  }
}
...
void ParserCTPMini::SubscribeMarketData() {
  ...
  if (m_pUserAPI && nCount > 0) {
    ...
    if (iResult != 0) {
      if (m_sink)
        write_log(m_sink, LL_ERROR,
                  "[ParserCTPMini] Sending md subscribe request failed: {}",
                  iResult);
    } else {
      if (m_sink)
        write_log(
            m_sink, LL_INFO,
            "[ParserCTPMini] Market data of {} contracts subscribed in total",
            nCount);
    }
  }
  codeFilter.clear();
...
bool ParserCTPMini::isConnected() { return m_pUserAPI != NULL; }
