...
TraderFemas::TraderFemas()
    : m_pUserAPI(NULL), m_ayPosition(NULL), m_ayOrders(NULL), m_ayTrades(NULL),
      m_ayPosDetail(NULL), m_wrapperState(WS_NOTLOGIN), m_uLastQryTime(0),
      m_iRequestID(0), m_bQuickStart(false), m_bStopped(false),
      m_bQryOnline(false), m_uLocalOrdID(1), m_bInQuery(false),
      m_strandIO(NULL), m_lastQryTime(0) {}
...
bool TraderFemas::init(WTSVariant *params) {
  ...
  const char *creatorName =
      "?CreateFtdcTraderApi@CUstpFtdcTraderApi@@SAPEAV1@PEBD@Z";
#else
  const char *creatorName =
...
uint32_t TraderFemas::genRequestID() { return m_iRequestID.fetch_add(1) + 1; }

uint32_t TraderFemas::genLocalOrdID() { return m_uLocalOrdID.fetch_add(1) + 1; }
...
int TraderFemas::doLogin() {
  ...
  if (iResult != 0) {
    write_log(m_sink, LL_ERROR,
              "[TraderFemas] Sending login request failed: {}", iResult);
  }

  return 0;
}

int TraderFemas::login(const char *user, const char *pass,
...
int TraderFemas::logout() {
  ...
  if (iResult != 0) {
    write_log(m_sink, LL_ERROR,
              "[TraderFemas] Sending logout request failed: {}", iResult);
  }
...
int TraderFemas::orderInsert(WTSEntrust *entrust) {
  ...
  if (strlen(entrust->getUserTag()) == 0) {
    /// 报单引用
    fmtutil::format_to(req.UserOrderLocalID, "{}{:012d}",
                       m_strSessionID.c_str(), genLocalOrdID());
  } else {
  ...
  if (iResult != 0) {
    write_log(m_sink, LL_ERROR, "[TraderFemas] Order inserting failed: {}",
              iResult);
  }
...
int TraderFemas::orderAction(WTSEntrustAction *action) {
  ...
  fmtutil::format_to(req.UserOrderActionLocalID, "{}{:012d}",
                     m_strSessionID.c_str(), genLocalOrdID());
  ...
  if (iResult != 0) {
    write_log(m_sink, LL_ERROR,
              "[TraderFemas] Sending cancel request failed: {}", iResult);
  }
...
void TraderFemas::OnQryFrontDisconnected(int nReason) { m_bQryOnline = false; }

void TraderFemas::OnHeartBeatWarning(int nTimeLapse) {
  write_log(m_sink, LL_DEBUG, "[TraderFemas][{}-{}] Heartbeating...",
            m_strBroker.c_str(), m_strUser.c_str());
}

void TraderFemas::OnRspQueryUserLogin(CUstpFtdcRspUserLoginField *pRspUserLogin,
...
void TraderFemas::OnRspDSUserCertification(
    CUstpFtdcDSUserCertRspDataField *pDSUserCertRspData,
    CUstpFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
  if (pRspInfo->ErrorID == 0) {
    doLogin();
  } else {
    write_log(m_sink, LL_ERROR, "[TraderFemas][{}-{}] Authentiation failed: {}",
...
void TraderFemas::OnRspUserLogin(CUstpFtdcRspUserLoginField *pRspUserLogin,
                                 CUstpFtdcRspInfoField *pRspInfo,
                                 int nRequestID, bool bIsLast) {
  if (!IsErrorRspInfo(pRspInfo)) {
    ...
    write_log(m_sink, LL_INFO, "[TraderFemas][{}-{}] Login succeed...",
              m_strBroker.c_str(), m_strUser.c_str());

    // 据说飞马不支持结算,所以查不到结算单
    write_log(
        m_sink, LL_INFO,
        "[TraderFemas][{}-{}] Querying confirming state of settlement data...",
        m_strBroker.c_str(), m_strUser.c_str());
    if (m_bQryOnline)
      onInitialized();
  } else {
    write_log(m_sink, LL_ERROR, "[TraderFemas][{}-{}] Login failed: {}",
              m_strBroker.c_str(), m_strUser.c_str(), pRspInfo->ErrorMsg);
    m_wrapperState = WS_LOGINFAILED;
...
void TraderFemas::OnRspUserLogout(CUstpFtdcRspUserLogoutField *pRspUserLogout,
                                  CUstpFtdcRspInfoField *pRspInfo,
                                  int nRequestID, bool bIsLast) {
  m_wrapperState = WS_NOTLOGIN;
  if (m_sink)
...
void TraderFemas::OnRspOrderInsert(CUstpFtdcInputOrderField *pInputOrder,
                                   CUstpFtdcRspInfoField *pRspInfo,
                                   int nRequestID, bool bIsLast) {
...
void TraderFemas::OnRspOrderAction(CUstpFtdcOrderActionField *pOrderAction,
                                   CUstpFtdcRspInfoField *pRspInfo,
                                   int nRequestID, bool bIsLast) {
  if (!IsErrorRspInfo(pRspInfo)) {
...
void TraderFemas::OnRspQryInvestorAccount(
...
void TraderFemas::OnRspQryInvestorPosition(
  ...
  if (!IsErrorRspInfo(pRspInfo) && pRspInvestorPosition) {
    ...
    WTSContractInfo *contract = m_bdMgr->getContract(
        pRspInvestorPosition->InstrumentID, pRspInvestorPosition->ExchangeID);
    if (contract) {
      WTSCommodityInfo *commInfo = contract->getCommInfo();
      WTSPositionItem *pos = WTSPositionItem::create(
          pRspInvestorPosition->InstrumentID, commInfo->getCurrency(),
          commInfo->getExchg());
      pos->setContractInfo(contract);
      pos->setDirection(wrapPosDirection(pRspInvestorPosition->Direction));
      pos->setNewPosition(pRspInvestorPosition->Position -
      ...
      if (commInfo->getCategoty() != CC_Combination) {
        int availNew =
            pRspInvestorPosition->Position - pRspInvestorPosition->YdPosition;
...
void TraderFemas::OnRspQryTrade(CUstpFtdcTradeField *pTrade,
...
void TraderFemas::OnRspQryOrder(CUstpFtdcOrderField *pOrder,
                                CUstpFtdcRspInfoField *pRspInfo, int nRequestID,
                                bool bIsLast) {
  if (bIsLast) {
    m_bInQuery = false;
...
void TraderFemas::onInitialized() {
  write_log(m_sink, LL_INFO,
            "[TraderFemas][{}-{}] Trading channel initialized...",
            m_strBroker.c_str(), m_strUser.c_str());
...
void TraderFemas::OnRspError(CUstpFtdcRspInfoField *pRspInfo, int nRequestID,
                             bool bIsLast) {
  if (m_sink)
    write_log(m_sink, LL_ERROR,
              "[TraderFemas][{}-{}] Error occured: {}, request id: {}",
              m_strBroker.c_str(), m_strUser.c_str(), pRspInfo->ErrorMsg,
              nRequestID);
}
...
int TraderFemas::wrapDirectionType(WTSDirectionType dirType,
...
WTSDirectionType
TraderFemas::wrapDirectionType(TUstpFtdcDirectionType dirType,
                               TUstpFtdcOffsetFlagType offsetType) {
...
int TraderFemas::wrapPriceType(WTSPriceType priceType,
                               bool isCFFEX /* = false */) {
...
WTSPriceType TraderFemas::wrapPriceType(TUstpFtdcPriceType priceType) {
  if (USTP_FTDC_OPT_AnyPrice == priceType ||
      USTP_FTDC_OPT_BestPrice == priceType)
    return WPT_ANYPRICE;
...
WTSTimeCondition
TraderFemas::wrapTimeCondition(TUstpFtdcTimeConditionType timeCond) {
...
WTSOrderInfo *TraderFemas::makeOrderInfo(CUstpFtdcOrderField *orderField) {
  ...
  pRet->setDirection(
      wrapDirectionType(orderField->Direction, orderField->OffsetFlag));
  ...
  if (orderField->TimeCondition == USTP_FTDC_TC_GFD) {
    ...
    if (orderField->VolumeCondition == USTP_FTDC_VC_AV ||
        orderField->VolumeCondition == USTP_FTDC_VC_MV)
...
WTSEntrust *TraderFemas::makeEntrust(CUstpFtdcInputOrderField *entrustField) {
  ...
  WTSEntrust *pRet =
      WTSEntrust::create(entrustField->InstrumentID, entrustField->Volume,
                         entrustField->LimitPrice);

  pRet->setContractInfo(ct);

  pRet->setDirection(
      wrapDirectionType(entrustField->Direction, entrustField->OffsetFlag));
  ...
  if (entrustField->TimeCondition == USTP_FTDC_TC_GFD) {
    ...
    if (entrustField->VolumeCondition == USTP_FTDC_VC_AV ||
        entrustField->VolumeCondition == USTP_FTDC_VC_MV)
      pRet->setOrderFlag(WOF_FAK);
    else
      pRet->setOrderFlag(WOF_FOK);
  }
...
WTSTradeInfo *TraderFemas::makeTradeRecord(CUstpFtdcTradeField *tradeField) {
  ...
  //	if (uTime / 10000 >= 21)
  //	{
  //		uDate = m_pMarketMgr->getPrevTDate(mInfo->getExchange(), uDate,
  //1);
  //	}
  //	else if(uTime <= 3)
  //	{
  ...
  double amount =
      mInfo->getVolScale() * tradeField->TradeVolume * pRet->getPrice();
...
void TraderFemas::OnErrRtnOrderInsert(CUstpFtdcInputOrderField *pInputOrder,
                                      CUstpFtdcRspInfoField *pRspInfo) {}

void TraderFemas::OnErrRtnOrderAction(CUstpFtdcOrderActionField *pOrderAction,
                                      CUstpFtdcRspInfoField *pRspInfo) {}

void TraderFemas::OnRtnInstrumentStatus(
...
bool TraderFemas::isConnected() { return (m_wrapperState == WS_ALLREADY); }
...
void TraderFemas::triggerQuery() {
  m_strandIO->post([this]() {
    ...
    if (curTime - m_lastQryTime < 1000) {
      ...
      m_strandIO->post([this]() { triggerQuery(); });
      return;
    }
