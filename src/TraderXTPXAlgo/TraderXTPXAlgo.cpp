...
inline bool IsErrorInfo(XTPRI *error_info) {
  if (error_info && error_info->error_id != 0) {
    std::cout << "--->>> ErrorID=" << error_info->error_id
              << ", ErrorMsg=" << error_info->error_msg << std::endl;
    return true;
  }
...
inline XTP_SIDE_TYPE wrapDirectionType(WTSDirectionType dirType,
...
inline WTSDirectionType wrapDirectionType(XTP_SIDE_TYPE side,
                                          XTP_POSITION_EFFECT_TYPE pe) {
...
WTSEntrust *TraderXTPXAlgo::makeEntrust(XTPOrderInfo *order_info) {
  ...
  WTSEntrust *pRet =
      WTSEntrust::create(code.c_str(), (uint32_t)order_info->quantity,
...
WTSEntrust *TraderXTPXAlgo::makeEntrust(XTPStrategyInfoStruct *stra_info) {
  ...
  WTSContractInfo *ct =
      _bd_mgr->getContract(stra_type.c_str(), stra_id.c_str());
  if (ct == NULL)
    return NULL;
...
WTSOrderInfo *
TraderXTPXAlgo::makeOrderInfo(XTPStrategyInfoStruct *strategyState) {
  ...
  WTSContractInfo *contract = _bd_mgr->getContract(
      std::to_string(stra_type).c_str(), std::to_string(stra_id).c_str());
  if (contract == NULL)
    return NULL;
  ...
  fmtutil::format_to(pRet->getOrderID(), "{}",
                     strategyState->m_xtp_strategy_id);
...
WTSTradeInfo *TraderXTPXAlgo::makeTradeInfo(XTPQueryTradeRsp *trade_info) {
  ...
  WTSDirectionType dType =
      wrapDirectionType(trade_info->side, trade_info->position_effect);
...
void TraderXTPXAlgo::OnDisconnected(uint64_t session_id, int reason) {
  ...
  _asyncio.post([this]() {
    write_log(_sink, LL_WARN,
              "[TraderXTPXAlgo] Connection lost, relogin in 2 seconds...");
...
void TraderXTPXAlgo::OnOrderEvent(XTPOrderInfo *order_info, XTPRI *error_info,
                                  uint64_t session_id) {
  ...
  //_api->GetAlgorithmIDByOrder(order_info->order_xtp_id,
  //order_info->order_client_id);
}

void TraderXTPXAlgo::OnTradeEvent(XTPTradeReport *trade_info,
                                  uint64_t session_id) {
  ...
  //_api->GetAlgorithmIDByOrder(trade_info->order_xtp_id,
  //trade_info->order_client_id);
}

void TraderXTPXAlgo::OnQueryTrade(XTPQueryTradeRsp *trade_info,
...
void TraderXTPXAlgo::OnQueryPosition(XTPQueryStkPositionRsp *position,
                                     XTPRI *error_info, int request_id,
                                     bool is_last, uint64_t session_id) {
  // if (is_last)
  //{
  ...
  if (!IsErrorInfo(error_info) && position) {
    ...
    if (contract) {
      ...
      std::string key =
          fmt::format("{}-{}", code.c_str(), position->position_direction);
      WTSPositionItem *pos = (WTSPositionItem *)_positions->get(key);
      if (pos == NULL) {
        pos = WTSPositionItem::create(code.c_str(), commInfo->getCurrency(),
                                      commInfo->getExchg());
      ...
      pos->setNewPosition(
          (double)(position->total_qty - position->yesterday_position));
...
void TraderXTPXAlgo::OnQueryAsset(XTPQueryAssetRsp *asset, XTPRI *error_info,
...
void TraderXTPXAlgo::OnQueryStrategy(XTPStrategyInfoStruct *strategy_info,
  ...
  if (error_info->error_id != 0) {
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Query strategy failed: {}",
              error_info->error_msg);
    return;
  }

  // std::cout << "request_id: " << request_id << ", is_last:" << is_last;
  // std::cout << ",strategy: " << strategy_info->m_xtp_strategy_id << ", client
  // id:" << strategy_info->m_client_strategy_id << ", type:" <<
  // strategy_info->m_strategy_type << ", status:" <<
  // strategy_info->m_strategy_state;//<< std::endl; std::cout <<
  // ",strategy_param: " << strategy_param << std::endl;
...
void TraderXTPXAlgo::OnStrategyStateReport(
    XTPStrategyStateReportStruct *strategy_state, uint64_t session_id) {
  std::cout << "------------------- OnStrategyStateReport -----------"
            << std::endl;

  /// 如果母单执行状态为STOPPED，此时执行算法停止
  /// 如果算法提前执行完毕，则会持续推送到算法设置的最终点以更新滑点信息等
  /// 最后推送一个最终态的母单状态回报
  if (strategy_state->m_strategy_info.m_strategy_state ==
      XTP_STRATEGY_STATE_STOPPED) {
    std::cout << "------------------ Algo Execution Stopped -----------------"
              << std::endl;

    std::cout << "m_client_strategy_id: "
              << strategy_state->m_strategy_info.m_client_strategy_id
              << std::endl;
    std::cout << "m_strategy_type: "
              << strategy_state->m_strategy_info.m_strategy_type << std::endl;
    std::cout << "m_strategy_qty: " << strategy_state->m_strategy_qty
              << std::endl;
    std::cout << "m_strategy_ordered_qty: "
              << strategy_state->m_strategy_ordered_qty << std::endl;
    std::cout << "m_strategy_execution_qty: "
              << strategy_state->m_strategy_execution_qty << std::endl;
    std::cout << "m_strategy_cancelled_qty: "
              << strategy_state->m_strategy_cancelled_qty << std::endl;
    std::cout << "m_strategy_unclosed_qty: "
              << strategy_state->m_strategy_unclosed_qty << std::endl;
    std::cout << "m_strategy_asset: " << strategy_state->m_strategy_asset
              << std::endl;
    std::cout << "m_strategy_ordered_asset: "
              << strategy_state->m_strategy_ordered_asset << std::endl;
    std::cout << "m_strategy_execution_asset: "
              << strategy_state->m_strategy_execution_asset << std::endl;
    std::cout << "m_strategy_execution_price: "
              << strategy_state->m_strategy_execution_price << std::endl;
    std::cout << "m_strategy_price_diff: "
              << strategy_state->m_strategy_price_diff << std::endl;
    std::cout << "m_strategy_asset_diff: "
              << strategy_state->m_strategy_asset_diff << std::endl;
  } else {
    std::cout << "------------------ Algo Execution Running -----------------"
              << std::endl;
...
void TraderXTPXAlgo::OnALGOUserEstablishChannel(char *user, XTPRI *error_info,
                                                uint64_t session_id) {
  std::cout << "--------------- OnALGOUserEstablishChannel -----------"
            << std::endl;
  /// 建立算法通道的异步通知
  if (error_info->error_id == 0) {
    /// 建立算法通道成功后，可以下算法母单
    ...
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Establish channel failed: {}",
              error_info->error_msg);
  }
}

void TraderXTPXAlgo::OnInsertAlgoOrder(XTPStrategyInfoStruct *strategy_info,
                                       XTPRI *error_info, uint64_t session_id) {
  std::cout << "----------------- OnInsertAlgoOrder --------------"
            << std::endl;

  /// 发送算法单后的异步通知
  if (error_info->error_id != 0) {
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Insert algo order failed: {}",
              error_info->error_msg);
  ...
  // id:" << strategy_info->m_client_strategy_id << ", type:" <<
  // strategy_info->m_strategy_type << ", status:" <<
  // (strategy_info->m_strategy_state - 0) << std::endl;
...
void TraderXTPXAlgo::OnCancelAlgoOrder(XTPStrategyInfoStruct *strategy_info,
                                       XTPRI *error_info, uint64_t session_id) {
  std::cout << "------------------- OnCancelAlgoOrder-----------" << std::endl;

  /// 算法单撤销结果通知
  if (error_info->error_id != 0) {
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Cancel algo order failed: {}",
              error_info->error_msg);
...
bool TraderXTPXAlgo::init(WTSVariant *params) {
  ...
  std::string dllpath =
      getBinDir() + DLLHelper::wrap_module(module.c_str(), "lib");
  ;
  ...
  const char *creatorName =
      "?CreateTraderApi@TraderApi@API@XTP@@SAPEAV123@EPEBDW4XTP_LOG_LEVEL@@@Z";
#else
  const char *creatorName =
      "?CreateTraderApi@TraderApi@API@XTP@@SAPAV123@EPBDW4XTP_LOG_LEVEL@@@Z";
#endif
#else
  const char *creatorName =
      "_ZN3XTP3API9TraderApi15CreateTraderApiEhPKc13XTP_LOG_LEVEL";
#endif
...
void TraderXTPXAlgo::reconnect() {
  ...
  _api = m_funcCreator(_client, ss.str().c_str(),
                       XTP_LOG_LEVEL_DEBUG); // 创建UserApi
  if (_api == NULL) {
    if (_sink)
    ...
    StdThreadPtr thrd(new StdThread([this]() {
      std::this_thread::sleep_for(std::chrono::seconds(2));
      write_log(_sink, LL_WARN, "[TraderXTPXAlgo] {} reconnecting...",
                _user.c_str());
      reconnect();
    }));
    return;
  ...
  _api->SetSoftwareKey(
      _acct_key.c_str()); // 设定用户的开发代码,在XTP申请开户时,由xtp人员提供
  _api->SetHeartBeatInterval(
      15); // 设定交易服务器超时时间,单位为秒,此为1.1.16新增接口
...
void TraderXTPXAlgo::disconnect() { release(); }

bool TraderXTPXAlgo::isConnected() { return (_state == TS_ALLREADY); }
...
bool TraderXTPXAlgo::extractEntrustID(const char *entrustid,
                                      uint32_t &orderRef) {
...
void TraderXTPXAlgo::doLogin() {
  ...
  uint64_t iResult = _api->Login(_host_oms.c_str(), _port_oms, _user.c_str(),
                                 _pass.c_str(), XTP_PROTOCOL_TCP);
  if (iResult == 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Login to OMS failed: {}",
              error_info->error_msg);
    std::string msg = error_info->error_msg;
    _state = TS_LOGINFAILED;
    _asyncio.post([this, msg] { _sink->onLoginResult(false, msg.c_str(), 0); });
  } else {
    _sessionid = iResult;
    if (!_inited) {
...

      {
        ...
        ss << _user << "_eid.sc";
        m_eidCache.init(ss.str().c_str(), _tradingday,
...

      {
        ...
        if (!StdFile::exists(path.c_str()))
          boost::filesystem::create_directories(path.c_str());
        ss << _user << "_oid.sc";
        m_oidCache.init(ss.str().c_str(), _tradingday,
                        [this](const char *message) {
                          write_log(_sink, LL_WARN, message);
                        });
      }
      ...
      write_log(_sink, LL_INFO,
                "[TraderXTPXAlgo] [{}] Login succeed, trading date: {}...",
                _user.c_str(), _tradingday);
      ...
      write_log(_sink, LL_INFO, "[TraderXTPXAlgo] [{}] Connection recovered",
                _user.c_str());
      _state = TS_ALLREADY;
    }
  }
  ...
  int login_ret = 0;
  login_ret =
      _api->LoginALGO(_host_algo.c_str(), _port_algo, _acct_algo.c_str(),
                      _password_algo.c_str(), XTP_PROTOCOL_TCP);

  if (login_ret != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Login AlgoBus failed: {}",
              error_info->error_msg);
    std::string msg = error_info->error_msg;
    _state = TS_LOGINFAILED;
    _asyncio.post([this, msg] { _sink->onLoginResult(false, msg.c_str(), 0); });
    ...
    if (!_inited) {
      ...
      write_log(
          _sink, LL_INFO,
          "[TraderXTPXAlgo] [{}] Login AlgoBus succeed, trading date: {}...",
          _user.c_str(), _tradingday);
    ...
    if (_sessionid != 0) {
      ...
      int user_ret = _api->ALGOUserEstablishChannel(_host_oms.c_str(),
                                                    _port_oms, _user.c_str(),
                                                    _pass.c_str(), _sessionid);

      if (user_ret == 0) {
        write_log(_sink, LL_INFO,
                  "[TraderXTPXAlgo] Establish channel send success: {}",
                  user_ret);
      } else {
        auto error_info = _api->GetApiLastError();
        write_log(_sink, LL_ERROR,
                  "[TraderXTPXAlgo] Establish channel send error: {}",
                  error_info->error_msg);
        std::string msg = error_info->error_msg;
        _state = TS_LOGINFAILED;
        _asyncio.post(
            [this, msg] { _sink->onLoginResult(false, msg.c_str(), 0); });
      }
    }
  }
}

int TraderXTPXAlgo::login(const char *user, const char *pass,
...
int TraderXTPXAlgo::orderInsert(WTSEntrust *entrust) {
  ...
  uint64_t iResult = _api->InsertAlgoOrder(
      _algo_type, _client, (char *)_algo_params.c_str(), _sessionid);
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Order inserting failed: {}",
              error_info->error_msg);
  }
...
int TraderXTPXAlgo::orderAction(WTSEntrustAction *action) {
  ...
  uint64_t iResult = _api->CancelAlgoOrder(
      true, strtoull(action->getOrderID(), NULL, 10), _sessionid);
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Order cancelling failed: {}",
              error_info->error_msg);
  }
...
uint32_t TraderXTPXAlgo::genRequestID() { return _reqid.fetch_add(1) + 1; }

int TraderXTPXAlgo::queryAccount() {
  int iResult = _api->QueryAsset(_sessionid, genRequestID());
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Account querying failed: {}",
              error_info->error_msg);
  }
...
int TraderXTPXAlgo::queryPositions() {
  ...
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Positions querying failed: {}",
              error_info->error_msg);
  }
...
int TraderXTPXAlgo::queryOrders() {
  int iResult =
      _api->QueryStrategy(0, 0, 0, _sessionid, genRequestID()); // 查询所有
  if (iResult != 0) {
    auto err_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPAlgo] Query orders failed: {}",
              err_info->error_msg);
  }
...
int TraderXTPXAlgo::queryTrades() {
  ...
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTPXAlgo] Trades querying failed: {}",
              error_info->error_msg);
  }
