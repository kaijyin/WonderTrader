...
WTSHotMgr::WTSHotMgr() : m_mapCustRules(NULL), m_bInitialized(false) {}
...
double WTSHotMgr::getRuleFactor(const char *ruleTag, const char *fullPid,
                                uint32_t uDate /* = 0 */) {
  if (m_mapCustRules == NULL)
    return 1.0;
  ...
  if (uDate == 0) {
    WTSSwitchItem *pItem =
        STATIC_CONVERT(dtMap->rbegin()->second, WTSSwitchItem *);
...
const char *WTSHotMgr::getPrevRawCode(const char *exchg, const char *pid,
                                      uint32_t dt) {
...
const char *WTSHotMgr::getRawCode(const char *exchg, const char *pid,
                                  uint32_t dt) {
  static thread_local char fullPid[64] = {0};
  fmtutil::format_to(fullPid, "{}.{}", exchg, pid);
...
bool WTSHotMgr::splitHotSecions(const char *exchg, const char *pid,
                                uint32_t sDt, uint32_t eDt,
...
const char *WTSHotMgr::getPrevSecondRawCode(const char *exchg, const char *pid,
                                            uint32_t dt) {
  static thread_local char fullPid[64] = {0};
  fmtutil::format_to(fullPid, "{}.{}", exchg, pid);
...
const char *WTSHotMgr::getSecondRawCode(const char *exchg, const char *pid,
                                        uint32_t dt) {
  static thread_local char fullPid[64] = {0};
  fmtutil::format_to(fullPid, "{}.{}", exchg, pid);
...
bool WTSHotMgr::splitSecondSecions(const char *exchg, const char *pid,
                                   uint32_t sDt, uint32_t eDt,
                                   HotSections &sections) {
  static thread_local char fullPid[64] = {0};
  fmtutil::format_to(fullPid, "{}.{}", exchg, pid);
    ...
    for (const std::string &pid : jExchg->memberNames()) {
      ...
      for (uint32_t i = 0; i < jProduct->size(); i++) {
        ...
        WTSSwitchItem *pItem = WTSSwitchItem::create(
            exchg.c_str(), pid.c_str(), jHotItem->getCString("from"),
            jHotItem->getCString("to"), jHotItem->getUInt32("date"));
      ...
      std::string fullCode =
          fmt::format("{}.{}", exchg.c_str(), lastCode.c_str());
...
const char *WTSHotMgr::getPrevCustomRawCode(const char *tag,
                                            const char *fullPid,
                                            uint32_t dt /* = 0 */) {
  if (m_mapCustRules == NULL)
    return "";
...
const char *WTSHotMgr::getCustomRawCode(const char *tag, const char *fullPid,
                                        uint32_t dt /* = 0 */) {
  if (m_mapCustRules == NULL)
    return "";
...
bool WTSHotMgr::isCustomHot(const char *tag, const char *fullCode,
                            uint32_t dt /* = 0 */) {
  if (m_mapCustRules == NULL)
    return false;
...
bool WTSHotMgr::splitCustomSections(const char *tag, const char *fullPid,
                                    uint32_t sDt, uint32_t eDt,
                                    HotSections &sections) {
  if (m_mapCustRules == NULL)
    return false;
  ...
  for (; cit != dtMap->end(); cit++) {
    ...
    if (curDate > eDt) {
      sections.emplace_back(
          HotSection(hotItem->from(), leftDate, eDt, prevFactor));
    } else if (leftDate < curDate) {
      // 如果开始日期小于当前切换的日期,则添加一段
      if (strlen(hotItem->from()) >
          0) // 这里from为空,主要是第一条规则,如果真的遇到这种情况,也没有太好的办法,只能不要这一段数据了,一般情况下是够的
      {
        sections.emplace_back(HotSection(hotItem->from(), leftDate,
                                         TimeUtils::getNextDate(curDate, -1),
                                         prevFactor));
      }
