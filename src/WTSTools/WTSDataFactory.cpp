...
WTSBarStruct *WTSDataFactory::updateKlineData(WTSKlineData *klineData,
                                              WTSTickData *tick,
...
WTSBarStruct *WTSDataFactory::updateMin1Data(WTSSessionInfo *sInfo,
                                             WTSKlineData *klineData,
                                             WTSBarStruct *newBasicBar,
                                             bool bAlignSec /* = false*/) {
  if (sInfo == NULL)
    return NULL;
...
WTSBarStruct *WTSDataFactory::updateMin5Data(WTSSessionInfo *sInfo,
                                             WTSKlineData *klineData,
                                             WTSBarStruct *newBasicBar,
                                             bool bAlignSec /* = false*/) {
  if (sInfo == NULL)
...
WTSBarStruct *WTSDataFactory::updateDayData(WTSSessionInfo *sInfo,
                                            WTSKlineData *klineData,
                                            WTSTickData *tick) {
...
WTSBarStruct *WTSDataFactory::updateSecData(WTSSessionInfo *sInfo,
                                            WTSKlineData *klineData,
                                            WTSTickData *tick) {
...
uint32_t WTSDataFactory::getPrevMinute(uint32_t curMinute,
                                       int period /* = 1 */) {
...
WTSKlineData *WTSDataFactory::extractKlineData(WTSKlineSlice *baseKline,
...
WTSKlineData *WTSDataFactory::extractMin1Data(WTSKlineSlice *baseKline,
                                              uint32_t times,
                                              WTSSessionInfo *sInfo,
                                              bool bIncludeOpen /* = true */,
                                              bool bAlignSec /* = false */) {
  ...
  // 检查最后一条数据
  {
    ...
    if (lastDesBar->date > lastRawBar->date ||
        lastDesBar->time > lastRawBar->time) {
      if (!bIncludeOpen)
...
WTSKlineData *WTSDataFactory::extractMin5Data(WTSKlineSlice *baseKline,
                                              uint32_t times,
                                              WTSSessionInfo *sInfo,
                                              bool bIncludeOpen /* = true */,
                                              bool bAlignSec /* = false */) {
...
WTSKlineData *WTSDataFactory::extractDayData(WTSKlineSlice *baseKline,
                                             uint32_t times,
                                             bool bIncludeOpen /* = true */) {
  // 计算时间步长
  uint32_t steplen = times;
...
WTSKlineData *WTSDataFactory::extractKlineData(WTSTickSlice *ayTicks,
                                               uint32_t seconds,
                                               WTSSessionInfo *sInfo,
  ...
  for (uint32_t i = 0; i < ayTicks->size(); i++) {
    ...
    if (bUnixTime) {
      barTime =
          (uint64_t)TimeUtils::makeTime(actDt, (long)(barTime * 1000)) / 1000;
    } else {
...
bool WTSDataFactory::mergeKlineData(WTSKlineData *klineData,
  ...
  if (!(klineData->period() == newKline->period() &&
        klineData->times() == newKline->times()))
    return false;
