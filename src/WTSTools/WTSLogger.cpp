...
inline void print_timetag(bool bWithSpace = true) {
  ...
  tm *tNow = localtime(&t);
  fmt::print("[{}.{:02d}.{:02d} {:02d}:{:02d}:{:02d}]", tNow->tm_year + 1900,
             tNow->tm_mon + 1, tNow->tm_mday, tNow->tm_hour, tNow->tm_min,
             tNow->tm_sec);
  if (bWithSpace)
    fmt::print(" ");
}
...
void WTSLogger::initLogger(const char *catName, WTSVariant *cfgLogger) {
  ...
  for (uint32_t idx = 0; idx < cfgSinks->size(); idx++) {
    ...
    if (strcmp(type, "daily_file_sink") == 0) {
      ...
      auto sink =
          std::make_shared<spdlog::sinks::daily_file_sink_mt>(filename, 0, 0);
      ...
      auto sink = std::make_shared<spdlog::sinks::basic_file_sink_mt>(
          filename, cfgSink->getBoolean("truncate"));
      sink->set_pattern(cfgSink->getCString("pattern"));
      sinks.emplace_back(sink);
      ...
      auto sink =
          std::make_shared<spdlog::sinks::ostream_sink_mt>(std::cout, true);
      sink->set_pattern(cfgSink->getCString("pattern"));
      sinks.emplace_back(sink);
    }
  }
  ...
  if (!bAsync) {
    auto logger =
        std::make_shared<spdlog::logger>(catName, sinks.begin(), sinks.end());
    ...
    auto logger = std::make_shared<spdlog::async_logger>(
        catName, sinks.begin(), sinks.end(), spdlog::thread_pool(),
...
void WTSLogger::init(const char *propFile /* = "logcfg.json" */,
                     bool isFile /* = true */,
  ...
  if (m_rootLogger == NULL) {
    throw std::runtime_error(
        "root logger can not be null, please check the config file");
  }
...
void WTSLogger::log_raw_by_cat(const char *catName, WTSLogLevel ll,
                               const char *message) {
  if (m_logLevel > ll || m_bStopped)
    return;
...
void WTSLogger::log_dyn_raw(const char *patttern, const char *catName,
                            WTSLogLevel ll, const char *message) {
  if (m_logLevel > ll || m_bStopped)
    return;
...
SpdLoggerPtr WTSLogger::getLogger(const char *logger,
                                  const char *pattern /* = "" */) {
