...
WTSBaseDataMgr::WTSBaseDataMgr()
    : m_mapExchgContract(NULL), m_mapSessions(NULL), m_mapCommodities(NULL),
      m_mapContracts(NULL) {
...
WTSCommodityInfo *WTSBaseDataMgr::getCommodity(const char *exchg,
                                               const char *pid) {
...
WTSContractInfo *WTSBaseDataMgr::getContract(const char *code,
                                             const char *exchg /* = "" */,
                                             uint32_t uDate /* = 0 */) {
  // 如果直接找到对应的市场代码,则直接
  ...
  if (strlen(exchg) == 0) {
    ...
    for (std::size_t i = 0; i < ayInst->size(); i++) {
       ...
       *	if param uDate is not zero, need to check whether contract is
       *valid
       */
      if (uDate == 0 ||
          (cInfo->getOpenDate() <= uDate && cInfo->getExpireDate() >= uDate))
        return cInfo;
    }
    return NULL;
  } else {
...
uint32_t WTSBaseDataMgr::getContractSize(const char *exchg /* = "" */,
                                         uint32_t uDate /* = 0 */) {
  uint32_t ret = 0;
  if (strlen(exchg) > 0) {
...
WTSSessionInfo *WTSBaseDataMgr::getSessionByCode(const char *code,
                                                 const char *exchg /* = "" */) {
...
bool WTSBaseDataMgr::isHoliday(const char *pid, uint32_t uDate,
                               bool isTpl /* = false */) {
...
bool WTSBaseDataMgr::loadSessions(const char *filename) {
  if (!StdFile::exists(filename)) {
    WTSLogger::error("Trading sessions configuration file {} not exists",
                     filename);
    return false;
  }
...
bool WTSBaseDataMgr::loadCommodities(const char *filename) {
  ...
  for (const std::string &exchg : root->memberNames()) {
    ...
    for (const std::string &pid : jExchg->memberNames()) {
      ...
      if (strlen(sid) == 0) {
        WTSLogger::warn("No session configured for {}.{}", exchg.c_str(),
                        pid.c_str());
        continue;
      }

      WTSCommodityInfo *pCommInfo =
          WTSCommodityInfo::create(pid.c_str(), name, exchg.c_str(), sid, hid);
  ...
  for (const std::string &exchg : root->memberNames()) {
    ...
    for (const std::string &code : jExchg->memberNames()) {
      ...
      if (jcInfo->has("product")) {
        ...
        WTSLogger::debug("Commodity {} has been automatically added",
                         key.c_str());
      }
      ...
      if (commInfo == NULL) {
        WTSLogger::warn("Commodity {}.{} not found, contract {} skipped",
                        ...
                        jcInfo->getCString("product"), code.c_str());
        continue;
      }

      WTSContractInfo *cInfo =
          WTSContractInfo::create(code.c_str(), jcInfo->getCString("name"),
                                  jcInfo->getCString("exchg"), pid.c_str());
      ...
      WTSContractList *contractList =
          (WTSContractList *)m_mapExchgContract->get(
              std::string(cInfo->getExchg()));
      if (contractList == NULL) {
        contractList = WTSContractList::create();
        m_mapExchgContract->add(std::string(cInfo->getExchg()), contractList,
                                false);
      }
...
uint64_t WTSBaseDataMgr::getBoundaryTime(const char *stdPID, uint32_t tDate,
  ...
  if (sInfo->getOffsetMins() < 0) {
    ...
    if (isStart)
      ...
      return (uint64_t)TimeUtils::getNextDate(tDate) * 10000 +
             sInfo->getCloseTime();
  } else {
...
uint32_t WTSBaseDataMgr::calcTradingDate(const char *stdPID, uint32_t uDate,
                                         uint32_t uTime,
                                         bool isSession /* = false */) {
  if (uDate == 0) {
...
uint32_t WTSBaseDataMgr::getTradingDate(const char *pid,
                                        uint32_t uOffDate /* = 0 */,
                                        uint32_t uOffMinute /* = 0 */,
                                        bool isTpl /* = false */) {
...
uint32_t WTSBaseDataMgr::getNextTDate(const char *pid, uint32_t uDate,
                                      int days /* = 1 */,
                                      bool isTpl /* = false */) {
  ...
  while (true) {
    ...
    curDate = (newT->tm_year + 1900) * 10000 + (newT->tm_mon + 1) * 100 +
              newT->tm_mday;
    if (newT->tm_wday != 0 && newT->tm_wday != 6 &&
        !isHoliday(pid, curDate, isTpl)) {
...
uint32_t WTSBaseDataMgr::getPrevTDate(const char *pid, uint32_t uDate,
                                      int days /* = 1 */,
                                      bool isTpl /* = false */) {
  uint32_t curDate = uDate;
  int left = days;
...
bool WTSBaseDataMgr::isTradingDate(const char *pid, uint32_t uDate,
                                   bool isTpl /* = false */) {
  uint32_t wd = TimeUtils::getWeekDay(uDate);
...
void WTSBaseDataMgr::setTradingDate(const char *pid, uint32_t uDate,
                                    bool isTpl /* = false */) {
  std::string tplID = pid;
  if (!isTpl)
    tplID = getTplIDByPID(pid);
