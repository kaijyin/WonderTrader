...
void handle_signal(int signum) {
  ...
  case SIGABRT_COMPAT: // SIGABRT compatible with other platforms, same as
                       // SIGABRT
  ...
  case SIGIO:    // discard signal       I/O is possible on a descriptor (see
                 // fcntl(2))
  ...
  case SIGTTIN: // stop process         background read attempted from control
                // terminal
  case SIGTTOU: // stop process         background write attempted to control
                // terminal
    sprintf(buf, "app stopped by signal %d", signum);
    g_cbSignalLog(buf);
    if (g_exitHandler)
  ...
  case SIGXCPU:   // terminate process    cpu time limit exceeded (see
                  // setrlimit(2))
  case SIGXFSZ:   // terminate process    file size limit exceeded (see
                  // setrlimit(2))
...
void install_signal_hooks(TracerLogCallback cbLog,
                          ExitHandler sigHandler = NULL) {
  g_cbSignalLog = cbLog;
