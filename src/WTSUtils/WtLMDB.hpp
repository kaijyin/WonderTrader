...
typedef std::function<void(const ValueArray &, const ValueArray &)>
    LMDBQueryCallback;
...
public:
  ...
  inline const char *errmsg() { return mdb_strerror(_errno); }
...
class WtLMDBQuery {
public:
  WtLMDBQuery(WtLMDB &db) : _txn(NULL), _commited(false), _db(db) {
    _readonly = db.is_readonly();
    _db.update_errno(
        mdb_txn_begin(_db.env(), NULL, (_readonly ? MDB_RDONLY : 0), &_txn));
...
public:
  ...
  bool put_and_commit(const std::string &key, const std::string &val) {
    return put_and_commit((void *)key.data(), key.size(), (void *)val.data(),
                          val.size());
  }
  ...
  bool put_and_commit(void *key, std::size_t klen, void *val,
                      std::size_t vlen) {
    MDB_val mKey, mData;
    mKey.mv_data = key;
    mKey.mv_size = klen;
   ...
   *	读取区间数据
   */
  int get_range(const std::string &lower_key, const std::string &upper_key,
                LMDBQueryCallback cb) {
    MDB_cursor *cursor;
    int _errno = mdb_cursor_open(_txn, _dbi, &cursor);
    _db.update_errno(_errno);
    if (_errno != MDB_SUCCESS)
    ...
    for (;
         (_errno = mdb_cursor_get(cursor, &lKey, &mData, op)) == MDB_SUCCESS;) {
      _db.update_errno(_errno);
      ...
      // 回调
      // cb(std::string((char*)lKey.mv_data, lKey.mv_size),
      // std::string((char*)mData.mv_data, mData.mv_size), false);
   ...
   *	@lower_key
   *下边界，这个必须要有，因为如果多个合约存一个库的话，不加的话可能会读到别的合约的数据
   *	@upper_key	上边界
   *	@count		目标数据条数
   *	@cb			回调函数
   */
  int get_lowers(const std::string &lower_key, const std::string &upper_key,
                 int count, LMDBQueryCallback cb) {
    MDB_cursor *cursor;
    int _errno = mdb_cursor_open(_txn, _dbi, &cursor);
    _db.update_errno(_errno);
    if (_errno != MDB_SUCCESS)
   ...
   *	@lower_key	下边界
   *	@upper_key
   *上边界，这个必须要有，因为如果多个合约存一个库的话，不加的话可能会读到别的合约的数据
   *	@count		目标数据条数
   *	@cb			回调函数
   */
  int get_uppers(const std::string &lower_key, const std::string &upper_key,
                 int count, LMDBQueryCallback cb) {
    MDB_cursor *cursor;
    int _errno = mdb_cursor_open(_txn, _dbi, &cursor);
    if (_errno != MDB_SUCCESS)
  ...
  inline int get_all(LMDBQueryCallback cb) {
    ...
    for (; _errno != MDB_NOTFOUND;) {
      ...
      ayKeys.emplace_back(
          std::string((const char *)bKey.mv_data, bKey.mv_size));
      ayVals.emplace_back(
          std::string((const char *)mData.mv_data, mData.mv_size));
    }
    cb(ayKeys, ayVals);
