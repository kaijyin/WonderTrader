...
ParserYD::~ParserYD() { m_pUserAPI = NULL; }
...
void ParserYD::notifyLogin(int errorNo, int maxOrderRef, bool isMonitor) {
  if (errorNo == 0) {
    ...
    write_log(m_sink, LL_INFO, "[ParserYD] {} login failed, error no: {}",
              m_strUserID, errorNo);
  }
}
...
void ParserYD::notifyMarketData(const YDMarketData *pDepthMarketData) {
  ...
  WTSContractInfo *contract = m_pBaseDataMgr->getContract(
      instInfo->InstrumentID, exchgInfo->ExchangeID);
  if (contract == NULL)
    return;
...
void ParserYD::release() { disconnect(); }
...
void ParserYD::DoLogin() {
  ...
  if (!m_pUserAPI->login(m_strUserID.c_str(), m_strPassword.c_str(), NULL,
                         NULL)) {
    if (m_sink)
...
void ParserYD::DoSubscribe() {
  ...
  for (auto &code : codeFilter) {
    ...
    write_log(m_sink, LL_ERROR, "[ParserYD] Subscribe md of {} {}",
              code.c_str(), bRet ? "succeed" : "failed");
  }
}
...
bool ParserYD::isConnected() { return m_pUserAPI != NULL; }
