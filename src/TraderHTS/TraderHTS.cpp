...
void ConnCallBack(const char *pKhh, const char *pConnKey, int nEvent,
                  void *pData) {
  std::cout << "Connect event khh: " << pKhh
            << "event: " << (0 == nEvent ? "恢复" : "断开") << std::endl;
}

TraderHTS::TraderHTS()
    : m_wrapperState(WS_NOTLOGIN), m_uLastQryTime(0), m_bInQuery(false),
      m_bUseEX(false), m_bASync(false), m_strandIO(NULL), m_lastQryTime(0),
      m_orderRef(1), m_lDate(0), m_mapLives(NULL), m_bdMgr(NULL),
      m_traderSink(NULL) {
...
void TraderHTS::InitializeHTS(WTSVariant *params) {
  ...
  bInited = g_callback_mgr->init(
      params); // 初始化，在所有接口使用前调用，除路径设置接口外
  if (!bInited) {
...
bool TraderHTS::init(WTSVariant *params) {
  ...
  try {
    ...
    g_callback_mgr->setHTSCallPtr(m_strUser.c_str(),
                                  this); // 将当前对象添加到全局映射表中
  } catch (...) {
    std::string msg = SECITPDK_GetLastError();
    write_log(m_traderSink, LL_ERROR, "[TraderHTS]订阅成交回报失败：{}({})",
              msg.c_str(), -1);
  }
...
void TraderHTS::reconnect() {
  ...
  //	if (m_traderSink)
  //	{
  //		write_log(m_traderSink, LL_WARN, "[TraderHTS]账号{}正在重连……",
  //m_strUser.c_str());  //m_traderSink->handleTraderLog(LL_WARN,
  //"[TraderHTS]账号%s正在重连……", m_strUser.c_str());
  //	}
  //	reconnect();
  // }));
...
void TraderHTS::htsGetCallback(const char *pTime, const char *pMsg, int nType) {
  ...
  if (NOTIFY_PUSH_ORDER == nType) {
    // 确认回报数据
    // write_log(m_traderSink, LL_INFO, "[TraderHTS] Insert order confirm
    // callback: WTH = {}, KHH = {},GDH = {},ZQDM = {}", nWTH, sKHH, sGDH,
    // sZQDM);

    OnRtnOrder(pMsg);
  } else if (NOTIFY_PUSH_MATCH == nType) {
    // 成交回报数据
    // write_log(m_traderSink, LL_INFO, "[TraderHTS] Order match confirm
    // callback: WTH = {}, KHH = {},GDH = {},ZQDM = {}", nWTH, sKHH, sGDH,
    // sZQDM);
    ...
    // 撤单回报数据
    // write_log(m_traderSink, LL_INFO, "[TraderHTS] Cancel order confirm
    // callback: WTH = {}, KHH = {},GDH = {},ZQDM = {}", nWTH, sKHH, sGDH,
    // sZQDM);
    ...
    // 撤单确认回报
    // write_log(m_traderSink, LL_DEBUG, "HTSCall cancelling report message:
    // WTH={}, KHH= {}, GDH={}, ZQDM={}, sCXBZ={}, iSBJG={} , sJGSM={}", nWTH,
    // sKHH, sGDH, sZQDM, sCXBZ, iSBJG, sJGSM);

    OnRtnOrder(pMsg);
  } else if (NOTIFY_PUSH_INVALID == nType) {
    // 废单
    write_log(m_traderSink, LL_WARN,
              "[TraderHTS] Abandoned order confirm callback: WTH = {}, KHH = "
              "{},GDH = {},ZQDM = {}",
              nWTH, sKHH, sGDH, sZQDM);
  }
}
...
void TraderHTS::OnRtnOrder(const char *pMsg) {
  ...
  bool isCancel =
      document["CXBZ"].GetString()[0] == 'W'; // 撤单标志是否是W /// O表示委托

  // 确认推送
  // std::cout << "Accept push report message: " << " code: " << code << "
  // exchg: " << "  wth: " << document["WTH"].GetInt64() << "  kfsbdbh: "  <<
  // document["KFSBDBH"].GetInt64() << "  wtjg: " <<
  // document["WTJG"].GetDouble() << "  wjsl: " << document["WTSL"].GetDouble()
  // << std::endl;
  ...
  if (contract != NULL) {
    ...
    if (!isCancel) {
      char buf[256] = {0};
      strncpy(buf, std::to_string(document["WTH"].GetInt64()).c_str(),
              sizeof(buf)); /// 这里使用KFSBDBH， 如果有问题则改为WTH
      ...
      if (ordInfo == NULL) {
        ...
        ordInfo->setOffsetType(document["JYLB"].GetInt() == 1 ? WOT_OPEN
                                                              : WOT_CLOSE);
        ...
        // ordInfo->setOrderTime(TimeUtils::makeTime(ordInfo->getOrderDate(),
        // uTime));
        ...
        strncpy(buf, std::to_string(document["KFSBDBH"].GetInt64()).c_str(),
                256);
        ordInfo->setEntrustID(buf); /// 使用开发商本地编号

        std::string usertag = m_iniHelper.readString(
            ENTRUST_SECTION, ordInfo->getEntrustID(), "");
        if (usertag.empty()) {
          ...
          if (strlen(ordInfo->getOrderID()) > 0) {
            m_iniHelper.writeString(
                ORDER_SECTION, StrUtil::trim(ordInfo->getOrderID()).c_str(),
                usertag.c_str());
            m_iniHelper.save();
          }
        }
      ...
      char buf[256] = {0};
      strncpy(buf, std::to_string(document["CXWTH"].GetInt64()).c_str(),
              sizeof(buf)); // 需要使用撤销委托号
      ordInfo = (WTSOrderInfo *)m_mapLives->grab(buf);
      if (ordInfo == NULL)
        return;
      ...
      // std::cout << "cxwth: " << document["CXWTH"].GetInt64() << " wth: " <<
      // document["WTH"].GetInt64() << " result_info: " <<
      // document["JGSM"].GetString() << std::endl;  // 使用委托号
    }
...
void TraderHTS::OnRtnTrade(const char *pMsg) {
  ...
  if (contract != NULL) {
    std::string orderid =
        std::to_string(document["WTH"].GetInt64()); // 这里使用委托号作为orderid
    ...
    trdInfo->setOffsetType(document["JYLB"].GetInt() == 1 ? WOT_OPEN
                                                          : WOT_CLOSE);
    ...
    std::string usertag = m_iniHelper.readString(
        ORDER_SECTION, StrUtil::trim(trdInfo->getRefOrder()).c_str());
    if (!usertag.empty())
...
int TraderHTS::login(const char *user, const char *pass,
...
void TraderHTS::qryGDNo() {
  m_strandIO->post([this]() {
    ...
    if (nRet < 0) {
      ...
      write_log(m_traderSink, LL_ERROR,
                "[TraderHTS]查询股东信息失败， 错误信息: {}({}) ", msg.c_str(),
                nRet);
      return;
    }

    std::cout << "SECITPDK_QueryAccInfo success. Num of results " << nRet
              << std::endl;
...
void TraderHTS::doLogin() {
  m_strandIO->post([this]() {
    ...
    // std::cout << m_strKey << "  " << m_strUser << "  " << m_strPass <<
    // std::endl;
    ...
    if (m_bUseEX) {
      ...
      nRet = SECITPDK_TradeLogin(m_strKey.c_str(), m_strUser.c_str(),
                                 m_strPass.c_str()); // 登录
    }
    ...
    if (nRet <= 0) {
      ...
      write_log(m_traderSink, LL_ERROR, "[TraderHTS]登录失败, 错误信息: {}({})",
                msg.c_str(), nRet);
    } else {
      ...
      if (lastDate != m_lDate) {
        ...
        write_log(m_traderSink, LL_INFO,
                  "[TraderHTS][%s]交易日已切换[{} -> {}]，清空本地数据缓存……",
                  m_strUser.c_str(), lastDate, m_lDate);
      }
    ...
    // cout << "SECITPDK_GetDoubleTickCount=" << SECITPDK_GetDoubleTickCount()
    // << endl; cout << "SECITPDK_GetSystemDate=" << SECITPDK_GetSystemDate() <<
    // endl; cout << "SECITPDK_GetTradeDate=" << SECITPDK_GetTradeDate() <<
    // endl; cout << "SECITPDK_GetReviseTimeAsLong=" <<
    // SECITPDK_GetReviseTimeAsLong() << endl;
...
int TraderHTS::logout() {
  ScopeGuard OnExit([]() { SECITPDK_Exit(); } // 退出，不再使用ITPDK接口时调用
  );
...
int TraderHTS::orderInsert(WTSEntrust *entrust) {
  ...
  m_strandIO->post([this, entrust]() {
    ...
    for (int i = 0; i < 100; i++) {
      ...
      if (m_bASync) {
        if (m_bUseEX) {
          nRet = SECITPDK_OrderEntrustEx_ASync(
              cusreqinfo, exchg.c_str(), entrust->getCode(), nDirection,
              entrust->getVolume(), entrust->getPrice(), DDLX_XJWT);
        } else {
          nRet = SECITPDK_OrderEntrust_ASync(
              m_strUser.c_str(), exchg.c_str(), entrust->getCode(), nDirection,
              entrust->getVolume(), entrust->getPrice(), DDLX_XJWT,
              gdh.c_str());
        }
      } else {
        if (m_bUseEX) {
          nRet = SECITPDK_OrderEntrustEx(
              cusreqinfo, exchg.c_str(), entrust->getCode(), nDirection,
              entrust->getVolume(), entrust->getPrice(), DDLX_XJWT);
        } else {
          nRet = SECITPDK_OrderEntrust(
              m_strUser.c_str(), exchg.c_str(), entrust->getCode(), nDirection,
              entrust->getVolume(), entrust->getPrice(), DDLX_XJWT,
              gdh.c_str());
        }
      ...
      if (strlen(entrust->getUserTag()) > 0) {
        ...
        m_iniHelper.writeString(ENTRUST_SECTION, entrust->getEntrustID(),
                                entrust->getUserTag());
        m_iniHelper.save();
      }
      ...
      if (nRet <= 0) {
        std::string msg = SECITPDK_GetLastError();
        write_log(m_traderSink, LL_ERROR, "[TraderHTS]委托指令发送失败: {}({})",
                  msg.c_str(), nRet);
        ...
        m_iniHelper.writeString(ORDER_SECTION,
                                StrUtil::trim(ordInfo->getOrderID()).c_str(),
                                ordInfo->getUserTag());
        m_iniHelper.save();
...
int TraderHTS::orderAction(WTSEntrustAction *action) {
  ...
  m_strandIO->post([this, action]() {
    ...
    if (m_bASync) {
      if (m_bUseEX) {
        nRet =
            SECITPDK_OrderWithdrawEx_ASync(cusreqinfo, exchg.c_str(), orderID);
      } else {
        nRet = SECITPDK_OrderWithdraw_ASync(m_strUser.c_str(), exchg.c_str(),
                                            orderID);
      }
    } else {
      if (m_bUseEX) {
        ...
        nRet =
            SECITPDK_OrderWithdraw(m_strUser.c_str(), exchg.c_str(), orderID);
      }
    };
    if (nRet <= 0) {
      string msg = SECITPDK_GetLastError(); // 下单失败，获取错误信息
      write_log(m_traderSink, LL_ERROR, "[TraderHTS]撤单指令发送失败: {}({})",
                msg.c_str(), nRet);
      ...
      write_log(m_traderSink, LL_INFO,
                "[TraderHTS] SECITPDK_OrderWithdraw 撤单成功， 委托号Wth = {}",
                nRet);
    }
...
int TraderHTS::queryAccount() {
  ...
  m_queQuery.push([this]() {
    ...
    {
      std::string msg = SECITPDK_GetLastError();
      write_log(m_traderSink, LL_ERROR, "[TraderHTS]可用资金查询失败: {}",
                msg.c_str());
    } else // 查询成功
    {
      ...
      for (auto &itr : argFund) {
        // printf("AccountId:%s;  FundAvl:%f;  TotalAsset:%f;  MarketValue:%f;
        // CurrentBalance:%f;  UnclearProfit:%f;  DiluteUnclearProfit:%f;
        // UncomeBalance:%f\n", itr.AccountId, itr.FundAvl, itr.TotalAsset,
        // itr.MarketValue, itr.CurrentBalance, itr.UnclearProfit,
        // itr.DiluteUnclearProfit, itr.UncomeBalance);
...
int TraderHTS::queryPositions() {
  ...
  m_queQuery.push([this]() {
    ...
    int64 nRet = 0;
    if (m_bUseEX) {
      nRet = (long)SECITPDK_QueryPositionsEx(cusreqinfo, SORT_TYPE_AES, nRows,
                                             0, "", "", "", 1, argHolding);
    } else {
      nRet = SECITPDK_QueryPositions(m_strUser.c_str(), SORT_TYPE_AES, nRows, 0,
                                     "", "", "", 1,
                                     argHolding); /// execflag必须大于0
    }

    if (nRet < 0) {
      std::string msg = SECITPDK_GetLastError();
      write_log(m_traderSink, LL_ERROR, "[TraderHTS]用户持仓查询失败: {}({})",
                msg.c_str(), nRet);
    } else {
      ...
      while (nRet >= nRows) {
        ...
        if (m_bUseEX) {
          nRet = (long)SECITPDK_QueryPositionsEx(
              cusreqinfo, SORT_TYPE_AES, nRows, tmp, "", "", "", 1, argHolding);
        } else {
          nRet = SECITPDK_QueryPositions(m_strUser.c_str(), SORT_TYPE_AES,
                                         nRows, tmp, "", "", "", 1,
                                         argHolding); /// execflag必须大于0
        }
        ...
        // 将单页查询结果插入到总的结果里
        if (nRet >= 0)
          argTotalHolding.insert(argTotalHolding.end(), argHolding.begin(),
                                 argHolding.end());
      ...
      for (auto &itr : argTotalHolding) {
        ...
        // std::cout << "query positions: " << " code: " << code << " exchange:
        // " << exchg << " dynamic profit: " << itr.UnclearProfit << " qty: " <<
        // (long)itr.CurrentQty << " brow index: " << (long)itr.BrowIndex <<
        // std::endl;

        WTSContractInfo *contract = m_bdMgr->getContract(code, exchg);
        if (contract) {
          WTSCommodityInfo *commInfo = contract->getCommInfo();
          WTSPositionItem *pInfo =
              WTSPositionItem::create(code, commInfo->getCurrency(), exchg);
...
int TraderHTS::queryOrders() {
  ...
  m_queQuery.push([this]() {
    ...
    const int nRows = 200;
    argOrders.reserve(
        nRows); // 需要预分配足够空间，查询结果最大返回200条，更多的话需要翻页查询

    int64 nRet = 0;
    if (m_bUseEX) {
      nRet = (long)SECITPDK_QueryOrdersEx(cusreqinfo, 0, SORT_TYPE_AES, nRows,
                                          0, "", "", 0, argOrders);
    } else {
      nRet = (long)SECITPDK_QueryOrders(m_strUser.c_str(), 0, SORT_TYPE_AES,
                                        nRows, 0, "", "", 0, argOrders);
    }
    ...
    {
      std::string msg = SECITPDK_GetLastError();
      write_log(m_traderSink, LL_ERROR, "[TraderHTS]用户订单查询失败: {}({})",
                msg.c_str(), nRet);
    } else {
      // 将结果保存到一个总的vector里，便于分页
      ...
      while (nRet >= nRows) {
        ...
        // 循环查询的时候nBrowindex+1会跳过一条
        if (m_bUseEX) {
          nRet = (long)SECITPDK_QueryOrdersEx(cusreqinfo, 0, SORT_TYPE_AES,
                                              nRows, tmp, "", "", 0, argOrders);
        } else {
          nRet = (long)SECITPDK_QueryOrders(m_strUser.c_str(), 0, SORT_TYPE_AES,
                                            nRows, tmp, "", "", 0, argOrders);
        }
        ...
        // 将单页查询结果插入到总的结果里
        if (nRet >= 0)
          argTotalOrders.insert(argTotalOrders.end(), argOrders.begin(),
                                argOrders.end());
      ...
      for (auto &itr : argTotalOrders) {
        /*printf("账户: %s, 委托号: %ld, 开发商本地编号: %ld, 申报委托号: %s,
           代码: %s, 交易所: %s, 价格: %f, 数量: %ld, 查询页码: %ld \n",
                itr.AccountId, (long)itr.OrderId, (long)itr.KFSBDBH, itr.SBWTH,
           itr.StockCode, itr.Market, itr.OrderPrice, (long)itr.OrderQty,
           (long)itr.BrowIndex);*/
        ...
        if (contract) {
          ...
          ordInfo->setOrderTime(
              TimeUtils::makeTime(ordInfo->getOrderDate(), uTime));
          ...
          ordInfo->setEntrustID(
              std::to_string(itr.KFSBDBH).c_str()); /// 这里使用开发商编号

          ordInfo->setOrderID(
              std::to_string(itr.OrderId).c_str()); /// 这里使用委托号
...
int TraderHTS::queryTrades() {
  ...
  m_queQuery.push([this]() {
    ...
    int64 nRet = 0;
    if (m_bUseEX) {
      nRet = (long)SECITPDK_QueryMatchsEx(cusreqinfo, 0, SORT_TYPE_AES, nRows,
                                          0, "", "", 0, argTrades);
    } else {
      nRet = (long)SECITPDK_QueryMatchs(m_strUser.c_str(), 0, SORT_TYPE_AES,
                                        nRows, 0, "", "", 0, argTrades);
    }

    if (nRet < 0) {
      std::string msg = SECITPDK_GetLastError();
      write_log(m_traderSink, LL_ERROR, "[TraderHTS]用户成交查询失败: {}({})",
                msg.c_str(), nRet);
    } else {
      // 将结果保存到一个总的vector里，便于分页
      ...
      while (nRet >= 200) {
        ...
        if (m_bUseEX) {
          nRet = (long)SECITPDK_QueryMatchsEx(cusreqinfo, 0, SORT_TYPE_AES,
                                              nRows, tmp, "", "", 0, argTrades);
        } else {
          nRet = (long)SECITPDK_QueryMatchs(m_strUser.c_str(), 0, SORT_TYPE_AES,
                                            nRows, tmp, "", "", 0, argTrades);
        }
        ...
        // 将单页查询结果插入到总的结果里
        if (nRet >= 0)
          argTotalTrades.insert(argTotalTrades.end(), argTrades.begin(),
                                argTrades.end());
      ...
      for (auto &itr : argTotalTrades) {
        // printf("委托号: %ld, 代码: %s 交易所: %s, 查询页码: %ld, 成交价格:
        // %f, 成交数量: %d\n", 	(long)itr.OrderId, itr.StockCode, itr.Market,
        //(long)itr.BrowIndex, itr.MatchPrice, (long)itr.MatchQty);
...
bool TraderHTS::isConnected() { return (m_wrapperState == WS_ALLREADY); }
...
bool HTSCallMgr::init(WTSVariant *params) {
  if (params) {
    ...
    bool b_inited = false;
    b_inited = SECITPDK_Init(
        HEADER_VER); // 初始化，在所有接口使用前调用，除路径设置接口外
...
void HTSCallMgr::setHTSCallPtr(std::string khh, TraderHTS *pTrader) {
  if (itpdkCallMap.find(khh) == itpdkCallMap.end()) {
    // std::cout << "Current key not existed, insert it to address map." <<
    // std::endl;
...
void HTSCallMgr::callbackMsgFunc(const char *pTime, const char *pMsg,
                                 int nType) {
  // 先进行数据解析
  ...
  rapidjson::Document document;
  if (document.Parse(pMsg).HasParseError()) {
    // cout << "HTSCallManage callbackMsgFunc Parse confirm message failed." <<
    // endl;
    return;
  }
  ...
  for (auto iter = itpdkCallMap.begin(); iter != itpdkCallMap.end(); ++iter) {
    ...
    if (strcmp(sTmp.c_str(), sKHH) == 0) {
      ...
      if (pTrader != NULL) {
        // std::cout << "HTSCallManage get report message khh: " << iter->first
...
void HTSCallMgr::callbackConnFunc(const char *pKhh, const char *pConnKey,
                                  int nEvent, int nType) {
  string sType;
