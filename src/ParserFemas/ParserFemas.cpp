...
ParserFemas::~ParserFemas() { m_pUserAPI = NULL; }
...
bool ParserFemas::init(WTSVariant *config) {
  ...
  const char *creatorName =
      "?CreateFtdcMduserApi@CUstpFtdcMduserApi@@SAPEAV1@PEBD@Z";
#else
  const char *creatorName =
...
void ParserFemas::release() { disconnect(); }
...
void ParserFemas::OnRspError(CUstpFtdcRspInfoField *pRspInfo, int nRequestID,
                             bool bIsLast) {
...
void ParserFemas::OnRspUserLogin(CUstpFtdcRspUserLoginField *pRspUserLogin,
...
void ParserFemas::OnRspUserLogout(CUstpFtdcRspUserLogoutField *pUserLogout,
                                  CUstpFtdcRspInfoField *pRspInfo,
                                  int nRequestID, bool bIsLast) {
  if (m_sink) {
...
void ParserFemas::OnFrontDisconnected(int nReason) {
  if (m_sink) {
    write_log(m_sink, LL_ERROR,
              "[ParserFemas] Market data server disconnected: {}", nReason);
    m_sink->handleEvent(WPE_Close, 0);
  }
}

void ParserFemas::OnRspUnSubMarketData(
    CUstpFtdcSpecificInstrumentField *pSpecificInstrument,
    CUstpFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {}

void ParserFemas::OnRtnDepthMarketData(
    CUstpFtdcDepthMarketDataField *pDepthMarketData) {
  if (m_pBaseDataMgr == NULL) {
    return;
  }

  uint32_t actDate = strtoul(pDepthMarketData->ActionDay, NULL, 10);
  uint32_t actTime = strToTime(pDepthMarketData->UpdateTime) * 1000 +
  ...
  WTSContractInfo *contract =
      m_pBaseDataMgr->getContract(pDepthMarketData->InstrumentID);
  if (contract == NULL)
    return;
...
void ParserFemas::OnRspSubMarketData(
    CUstpFtdcSpecificInstrumentField *pSpecificInstrument,
    CUstpFtdcRspInfoField *pRspInfo, int nRequestID, bool bIsLast) {
...
void ParserFemas::OnHeartBeatWarning(int nTimeLapse) {
  if (m_sink)
    write_log(m_sink, LL_INFO, "[ParserFemas] Heartbeating, elapse: {}",
              nTimeLapse);
}
...
void ParserFemas::ReqUserLogin() {
  ...
  if (iResult != 0) {
    if (m_sink)
      write_log(m_sink, LL_ERROR,
                "[ParserFemas] Sending login request failed: {}", iResult);
  }
}
...
void ParserFemas::SubscribeMarketData() {
  ...
  if (m_pUserAPI && nCount > 0) {
    ...
    if (iResult != 0) {
      if (m_sink)
        write_log(m_sink, LL_ERROR,
                  "[ParserFemas] Sending md subscribe request failed: {}",
                  iResult);
    } else {
      if (m_sink)
        write_log(
            m_sink, LL_INFO,
            "[ParserFemas] Market data of {} contracts subscribed in total",
            nCount);
    }
  }
  codeFilter.clear();
...
bool ParserFemas::isConnected() { return m_pUserAPI != NULL; }
