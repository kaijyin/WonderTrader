...
static inline int on_rpt_connect(OesAsyncApiChannelT *pAsyncChannel,
...
static inline int on_rpt_disconnect(OesAsyncApiChannelT *pAsyncChannel,
                                    void *pCallbackParams) {
  TraderOES *trader = (TraderOES *)pCallbackParams;
  if (trader == NULL)
    return 0;
...
static inline int on_rpt_message(OesApiSessionInfoT *pSessionInfo,
...
static inline int on_ord_connect(OesAsyncApiChannelT *pAsyncChannel,
                                 void *pCallbackParams) {
  OesAsyncApi_DefaultOnConnect(pAsyncChannel, NULL);
...
static inline int on_ord_disconnect(OesAsyncApiChannelT *pAsyncChannel,
                                    void *pCallbackParams) {
  TraderOES *trader = (TraderOES *)pCallbackParams;
  if (trader == NULL)
    return 0;
...
static inline int on_ord_message(OesApiSessionInfoT *pSessionInfo,
                                 SMsgHeadT *pMsgHead, void *pMsgItem,
                                 void *pCallbackParams) {
  TraderOES *trader = (TraderOES *)pCallbackParams;
  if (trader == NULL)
...
static inline int on_rsp_orders(OesApiSessionInfoT *pQryChannel,
                                SMsgHeadT *pMsgHead, void *pMsgItem,
...
static inline int on_rsp_account(OesApiSessionInfoT *pQryChannel,
                                 SMsgHeadT *pMsgHead, void *pMsgItem,
                                 OesQryCursorT *pQryCursor,
                                 void *pCallbackParams) {
  TraderOES *trader = (TraderOES *)pCallbackParams;
...
static inline int on_rsp_positions(OesApiSessionInfoT *pQryChannel,
                                   SMsgHeadT *pMsgHead, void *pMsgItem,
                                   OesQryCursorT *pQryCursor,
                                   void *pCallbackParams) {
  TraderOES *trader = (TraderOES *)pCallbackParams;
...
static inline int on_rsp_trades(OesApiSessionInfoT *pQryChannel,
                                SMsgHeadT *pMsgHead, void *pMsgItem,
                                OesQryCursorT *pQryCursor,
                                void *pCallbackParams) {
  TraderOES *trader = (TraderOES *)pCallbackParams;
...
TraderOES::TraderOES()
    : _context(NULL), _sink(NULL), _bd_mgr(NULL), _tradingday(0), _trades(NULL),
      _orders(NULL), _funds(NULL), _positions(NULL) {}
...
void TraderOES::reconnect() {
  ...
  OesAsyncApiChannelT *channel = OesAsyncApi_AddChannelFromFile(
      ...
      OESAPI_CFG_DEFAULT_KEY_RPT_ADDR, on_rpt_message, this, on_rpt_connect,
      this, on_rpt_disconnect, this);
  if (!channel) {
    write_log(_sink, LL_WARN,
              "[TraderOES] Loading section {} from config failed",
  ...
  _channel = OesAsyncApi_AddChannelFromFile(
      ...
      _config.c_str(), OESAPI_CFG_DEFAULT_SECTION,
      OESAPI_CFG_DEFAULT_KEY_ORD_ADDR, on_ord_message, this, on_ord_connect,
      this, on_ord_disconnect, this);
  if (!_channel) {
    write_log(_sink, LL_WARN,
              "[TraderOES] Loading section {} from config failed",
...
void TraderOES::disconnect() { release(); }

bool TraderOES::isConnected() { return (_state == TS_ALLREADY); }
...
int TraderOES::login(const char *user, const char *pass,
...
int TraderOES::orderInsert(WTSEntrust *entrust) {
  ...
  ordReq.mktId = wt_stricmp(entrust->getExchg(), "SSE") == 0
                     ? OES_MKT_SH_ASHARE
                     : OES_MKT_SZ_ASHARE;
  ordReq.ordType = OES_ORD_TYPE_LMT;
  ordReq.bsType = (entrust->getOffsetType() == WOT_OPEN) ? OES_BS_TYPE_BUY
                                                         : OES_BS_TYPE_SELL;
...
int TraderOES::orderAction(WTSEntrustAction *action) {
  ...
  cancelReq.mktId = wt_stricmp(action->getExchg(), "SSE") == 0
                        ? OES_MKT_SH_ASHARE
                        : OES_MKT_SZ_ASHARE;
  uint32_t ordref;
...
int TraderOES::queryAccount() {
  ...
  int32 ret =
      OesAsyncApi_QueryCashAsset(_channel, &qryFilter, on_rsp_account, this);
  if (ret < 0) {
...
int TraderOES::queryPositions() {
  ...
  int32 ret =
      OesAsyncApi_QueryStkHolding(_channel, &qryFilter, on_rsp_positions, this);
  if (ret < 0) {
...
WTSOrderInfo *TraderOES::makeOrderInfo(OesOrdCnfmT *pOrdItem) {
  ...
  pRet->setOffsetType(pOrdItem->bsType == OES_BS_TYPE_BUY ? WOT_OPEN
                                                          : WOT_CLOSE);
  ...
  if (pOrdItem->ordStatus >= __OES_ORD_STATUS_INVALID_MIN) {
    pRet->setError(true);
    pRet->setStateMsg(
        fmt::format("Rejected: {}", pOrdItem->ordRejReason).c_str());
  }
...
WTSTradeInfo *TraderOES::makeTradeInfo(OesTrdCnfmT *pTrdItem) {
  ...
  WTSTradeInfo *pRet =
      WTSTradeInfo::create(pTrdItem->securityId, cInfo->getExchg());
  ...
  pRet->setDirection(WDT_LONG);
  pRet->setOffsetType(pTrdItem->trdSide == OES_BS_TYPE_BUY ? WOT_OPEN
                                                           : WOT_CLOSE);
...
WTSEntrust *TraderOES::makeEntrust(OesOrdRejectT *entrustField) {
  const char *exchg =
      (OES_MKT_SH_ASHARE == entrustField->mktId) ? "SSE" : "SZSE";
  WTSContractInfo *ct = _bd_mgr->getContract(entrustField->securityId, exchg);
  if (ct == NULL)
    return NULL;

  WTSEntrust *pRet =
      WTSEntrust::create(entrustField->securityId, entrustField->ordQty,
                         entrustField->ordPrice / 10000.0, ct->getExchg());
  pRet->setContractInfo(ct);
  pRet->setDirection(WDT_LONG);
  pRet->setPriceType(WPT_LIMITPRICE);
  pRet->setOffsetType(entrustField->bsType == OES_BS_TYPE_BUY ? WOT_OPEN
                                                              : WOT_CLOSE);
  pRet->setOrderFlag(WOF_NOR);
...
void TraderOES::handle_rpt_message(SMsgHeadT *pMsgHead, void *pMsgItem) {
  ...
  case OESMSG_RPT_ORDER_INSERT: /* OES委托已生成 (已通过风控检查) @see
                                   OesOrdCnfmT */
  ...
  case OESMSG_RPT_BUSINESS_REJECT: /* OES业务拒绝 (未通过风控检查等) @see
                                      OesOrdRejectT */
    ...
    if (pRptMsg->rptBody.ordRejectRsp.origClOrdId == 0) {
      ...
      if (entrustInfo) {
        WTSError *error = WTSError::create(
            WEC_ORDERINSERT,
            fmt::format("Rejected: {}", pRptMsg->rptHead.ordRejReason).c_str());
        if (_sink)
  ...
  case OESMSG_RPT_ORDER_REPORT: /* 交易所委托回报
  ...
  case OESMSG_RPT_TRADE_REPORT: /* 交易所成交回报 @see OesTrdCnfmT */
    ...
    /* @note 成交回报中已经包含了委托回报的相关信息,
    可以通过辅助接口直接从成交回报中提取和生成委托回报数据 if
    (pRptMsg->rptBody.trdCnfm.ordStatus == OES_ORD_STATUS_FILLED) { OesOrdCnfmT
    ordReport = {NULLOBJ_OES_ORD_CNFM}; OesHelper_ExtractOrdReportFromTrd(
...
void TraderOES::handle_rsp_account(SMsgHeadT *pMsgHead, void *pMsgItem,
                                   OesQryCursorT *pQryCursor) {
...
void TraderOES::handle_rsp_orders(SMsgHeadT *pMsgHead, void *pMsgItem,
                                  OesQryCursorT *pQryCursor) {
...
void TraderOES::handle_rsp_positions(SMsgHeadT *pMsgHead, void *pMsgItem,
                                     OesQryCursorT *pQryCursor) {
  ...
  const char *exchg =
      (OES_MKT_SH_ASHARE == pStkHolding->mktId) ? "SSE" : "SZSE";
  WTSContractInfo *cInfo = _bd_mgr->getContract(pStkHolding->securityId, exchg);
  if (cInfo != NULL) {
    WTSCommodityInfo *commInfo = cInfo->getCommInfo();
    WTSPositionItem *pos = WTSPositionItem::create(
        pStkHolding->securityId, commInfo->getCurrency(), commInfo->getExchg());
    pos->setContractInfo(cInfo);
    pos->setDirection(WDT_LONG);

    double preVol = std::max(
        (double)(pStkHolding->originalHld - pStkHolding->totalSellHld), 0.0);
    double newVol = pStkHolding->originalHld + pStkHolding->totalBuyHld -
                    pStkHolding->totalSellHld - preVol;
...
void TraderOES::handle_rsp_trades(SMsgHeadT *pMsgHead, void *pMsgItem,
                                  OesQryCursorT *pQryCursor) {
