...
void initParsers(WTSVariant *cfg) {
  for (uint32_t idx = 0; idx < cfg->size(); idx++) {
    ...
    ParserAdapterPtr adapter(
        new ParserAdapter(&g_baseDataMgr, &g_dataMgr, &g_idxFactory));
...
void initialize(const std::string &filename) {
  ...
  if (cfgBF->has("rules")) {
    ...
    for (const std::string &ruleTag : tags) {
      g_hotMgr.loadCustomRules(ruleTag.c_str(),
  ...
  if (!bAlldayMode) {
    g_stateMon.initialize(config->getCString("statemonitor"), &g_baseDataMgr,
                          &g_dataMgr);
  } else {
...
int main(int argc, char *argv[]) {
  ...
  auto lParam =
      opt("-l", "--logcfg",
          "logging configure filepath, logcfgdt.yaml as default", false);
