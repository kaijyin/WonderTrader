...
inline WTSOffsetType wrapOffsetType(XTP_SIDE_TYPE side,
                                    XTP_POSITION_EFFECT_TYPE offType = 0) {
...
WTSEntrust *TraderXTP::makeEntrust(XTPOrderInfo *order_info) {
  ...
  if (!commInfo->isOption())
    pRet->setOffsetType((order_info->side == XTP_SIDE_BUY) ? WOT_OPEN
                                                           : WOT_CLOSE);
  else
    pRet->setOffsetType(
        wrapOffsetType(order_info->side, order_info->position_effect));
...
WTSTradeInfo *TraderXTP::makeTradeInfo(XTPQueryTradeRsp *trade_info) {
  ...
  if (!commInfo->isOption())
    pRet->setOffsetType((trade_info->side == XTP_SIDE_BUY) ? WOT_OPEN
                                                           : WOT_CLOSE);
  else
    pRet->setOffsetType(
        wrapOffsetType(trade_info->side, trade_info->position_effect));
...
void TraderXTP::OnDisconnected(uint64_t session_id, int reason) {
  ...
  _asyncio.post([this]() {
    write_log(_sink, LL_WARN,
              "[TraderrXTP] Connection lost, relogin in 2 seconds...");
...
void TraderXTP::OnOrderEvent(XTPOrderInfo *order_info, XTPRI *error_info,
                             uint64_t session_id) {
...
void TraderXTP::OnCancelOrderError(XTPOrderCancelInfo *cancel_info,
...
void TraderXTP::OnQueryOrder(XTPQueryOrderRsp *order_info, XTPRI *error_info,
...
void TraderXTP::OnQueryTrade(XTPQueryTradeRsp *trade_info, XTPRI *error_info,
                             int request_id, bool is_last,
                             uint64_t session_id) {
  // if (is_last)
  //{
...
void TraderXTP::OnQueryPosition(XTPQueryStkPositionRsp *position,
...
void TraderXTP::OnQueryAsset(XTPQueryAssetRsp *asset, XTPRI *error_info,
                             int request_id, bool is_last,
                             uint64_t session_id) {
  // if (is_last)
  //{
...
void TraderXTP::reconnect() {
  ...
  if (_api == NULL) {
    ...
    StdThreadPtr thrd(new StdThread([this]() {
      std::this_thread::sleep_for(std::chrono::seconds(2));
      write_log(_sink, LL_WARN, "[TraderrXTP] {} reconnecting...",
                _user.c_str());
      reconnect();
    }));
    return;
  ...
  _api->SetSoftwareKey(
      _acckey.c_str()); // 设定用户的开发代码,在XTP申请开户时,由xtp人员提供
...
void TraderXTP::disconnect() { release(); }

bool TraderXTP::isConnected() { return (_state == TS_ALLREADY); }
...
void TraderXTP::doLogin() {
  ...
  uint64_t iResult = _api->Login(_host.c_str(), _port, _user.c_str(),
                                 _pass.c_str(), XTP_PROTOCOL_TCP);
  if (iResult == 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTP] Login failed: {}",
    ...
    if (!_inited) {
      ...
      write_log(_sink, LL_INFO,
                "[TraderXTP] [{}] Login succeed, trading date: {}...",
      ...
      write_log(_sink, LL_INFO, "[TraderXTP] [{}] Connection recovered",
                _user.c_str());
      _state = TS_ALLREADY;
    }
  }
...
int TraderXTP::login(const char *user, const char *pass,
...
int TraderXTP::orderInsert(WTSEntrust *entrust) {
  ...
  req.market =
      wt_stricmp(entrust->getExchg(), "SSE") == 0 ? XTP_MKT_SH_A : XTP_MKT_SZ_A;
  ...
  if (iResult == 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTP] Order inserting failed: {}",
              error_info->error_msg);
  }
...
int TraderXTP::orderAction(WTSEntrustAction *action) {
  ...
  uint64_t iResult =
      _api->CancelOrder(strtoull(action->getOrderID(), NULL, 10), _sessionid);
  if (iResult == 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTP] Order cancelling failed: {}",
              error_info->error_msg);
  }
...
uint32_t TraderXTP::genRequestID() { return _reqid.fetch_add(1) + 1; }

int TraderXTP::queryAccount() {
  int iResult = _api->QueryAsset(_sessionid, genRequestID());
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTP] Account querying failed: {}",
              error_info->error_msg);
  }
...
int TraderXTP::queryPositions() {
  ...
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTP] Positions querying failed: {}",
              error_info->error_msg);
  }
...
int TraderXTP::queryOrders() {
  ...
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTP] Orders querying failed: {}",
              error_info->error_msg);
  }
...
int TraderXTP::queryTrades() {
  ...
  if (iResult != 0) {
    auto error_info = _api->GetApiLastError();
    write_log(_sink, LL_ERROR, "[TraderXTP] Trades querying failed: {}",
              error_info->error_msg);
  }
