...
ParserHuaX::ParserHuaX() : _api(NULL), _iRequestID(0), _uTradingDate(0) {}

ParserHuaX::~ParserHuaX() { _api = NULL; }
...
bool ParserHuaX::init(WTSVariant *config) {
  ...
  std::string path =
      StrUtil::printf("%s/%s/", _strFlowDir.c_str(), _strUser.c_str());
  ...
  const char *creatorName =
      "?CreateTstpXMdApi@CTORATstpXMdApi@TORALEV1API@@SAPEAV12@AEBD0@Z";
#else
  const char *creatorName =
      "?CreateTstpXMdApi@CTORATstpXMdApi@TORALEV1API@@SAPAV12@ABD0@Z";
#endif
#else
  // TODO linux下未修改
  const char *creatorName =
  ...
  _api = _funcCreator((TTORATstpMDSubModeType)_sub_mode.c_str()[0],
                      (TTORATstpMDSubModeType)_derive_sub_mode.c_str()[0]);
  _api->RegisterSpi(this);

  return true;
}

void ParserHuaX::release() { disconnect(); }
    ...
    if (_sink) {
      if (iResult == -1) {
        ...
        write_log(_sink, LL_ERROR,
                  "[ParserHuaX] Sending login request failed: {}", iResult);
      }
    }
  } else {
...
void ParserHuaX::DoSubscribeMD() {
  ...
  if (!codeFilter.empty()) {
    ...
    if (_api && nCount > 0) {
      int iResult =
          _api->SubscribeMarketData(subscribe, nCount, TORA_TSTP_EXD_SSE);
      if (iResult != 0) {
        if (_sink)
          write_log(
              _sink, LL_ERROR,
              "[ParserHuaX] Sending md subscribe request of SSE failed: {}",
              iResult);
      } else {
        if (_sink)
          write_log(
              _sink, LL_INFO,
              "[ParserHuaX] Market data of {} instruments of SSE subscribed",
              nCount);
      }
    }
    codeFilter.clear();
  ...
  if (!codeFilter.empty()) {
    ...
    if (_api && nCount > 0) {
      int iResult =
          _api->SubscribeMarketData(subscribe, nCount, TORA_TSTP_EXD_SZSE);
      if (iResult != 0) {
        if (_sink)
          write_log(
              _sink, LL_ERROR,
              "[ParserHuaX] Sending md subscribe request of SZSE failed: {}",
              iResult);
      } else {
        if (_sink)
          write_log(
              _sink, LL_INFO,
              "[ParserHuaX] Market data of {} instruments of SZSE subscribed",
              nCount);
      }
    }
    codeFilter.clear();
...
bool ParserHuaX::isConnected() { return _loginState == LS_LOGINED; }
...
void ParserHuaX::OnFrontDisconnected(int nReason) { _loginState = LS_NOTLOGIN; }

void ParserHuaX::OnRspUserLogin(CTORATstpRspUserLoginField *pRspUserLoginField,
  ...
  if (pRspInfoField->ErrorID == 0) {
    ...
    write_log(_sink, LL_ERROR, "[ParserHuaX] login failed: {}:{}",
              pRspInfoField->ErrorID, pRspInfoField->ErrorMsg);
    _loginState = LS_NOTLOGIN;
  }
}

void ParserHuaX::OnRspSubMarketData(
    CTORATstpSpecificSecurityField *pSpecificSecurityField,
    CTORATstpRspInfoField *pRspInfoField) {
  if (pRspInfoField->ErrorID != 0) {
    write_log(_sink, LL_ERROR, "[ParserHuaX] submarket data failed: {}:{}",
              pRspInfoField->ErrorID, pRspInfoField->ErrorMsg);
    _loginState = LS_NOTLOGIN;
  }
}
...
void ParserHuaX::OnRtnMarketData(CTORATstpMarketDataField *market_data) {
  ...
  if (ct == NULL) {
    if (_sink)
      write_log(_sink, LL_ERROR, "[ParserHuaX] Instrument {}.{} not exists...",
                exchg, code);
    return;
  }
