...
WtUftStraDemo::WtUftStraDemo(const char *id)
    : UftStrategy(id), _last_tick(NULL), _last_entry_time(UINT64_MAX),
      _channel_ready(false), _last_calc_time(0), _lots(1), _cancel_cnt(0) {}
...
const char *WtUftStraDemo::getName() { return "UftDemoStrategy"; }

const char *WtUftStraDemo::getFactName() { return FACT_NAME; }
...
void WtUftStraDemo::on_entrust(uint32_t localid, bool bSuccess,
                               const char *message) {
  if (!bSuccess) {
...
void WtUftStraDemo::on_tick(IUftStraCtx *ctx, const char *code,
                            WTSTickData *newTick) {
  ...
  uint32_t curMin = newTick->actiontime() /
                    100000; // actiontime是带毫秒的,要取得分钟,则需要除以10w
  if (curMin >
      _last_calc_time) { // 如果spread上次计算的时候小于当前分钟,则重算spread
  ...
  // 30秒内不重复计算
  uint64_t now =
      TimeUtils::makeTime(ctx->stra_get_date(),
  ...
  // 计算部分
  double pxInThry = (newTick->bidprice(0) * newTick->askqty(0) +
                     newTick->askprice(0) * newTick->bidqty(0)) /
                    (newTick->bidqty(0) + newTick->askqty(0));
  ...
  if (signal != 0) {
    ...
    if (signal > 0 && decimal::le(curPos, 0)) { // 正向信号,且当前仓位小于等于0
...

      {
        ...
        decimal::ge(
            curPos,
            0)) { // 反向信号,且当前仓位大于0,或者仓位为0但不是股票,或者仓位为0但是基础仓位有修正
      // 最新价-2跳下单
      double targetPx = price - cInfo->getPriceTick() * _offset;
...
void WtUftStraDemo::check_orders() {
  if (!_orders.empty() && _last_entry_time != UINT64_MAX) {
    uint64_t now = TimeUtils::makeTime(_ctx->stra_get_date(),
                                       _ctx->stra_get_time() * 100000 +
                                           _ctx->stra_get_secs());
    if (now - _last_entry_time >=
        _secs * 1000) // 如果超过一定时间没有成交完,则撤销
    {
      _mtx_ords.lock();
      for (auto localid : _orders) {
        ...
        _cancel_cnt++;
        _ctx->stra_log_info(
            fmt::format("Order expired, cancelcnt updated to {}", _cancel_cnt)
                .c_str());
      }
      _mtx_ords.unlock();
    }
...
void WtUftStraDemo::on_bar(IUftStraCtx *ctx, const char *code,
                           const char *period, uint32_t times,
                           WTSBarStruct *newBar) {}

void WtUftStraDemo::on_trade(IUftStraCtx *ctx, uint32_t localid,
                             const char *stdCode, bool isLong, uint32_t offset,
                             double qty, double price) {}

void WtUftStraDemo::on_position(IUftStraCtx *ctx, const char *stdCode,
  ...
  _prev = prevol;
  _ctx->stra_log_info(
      fmt::format("There are {} of {} before today", _prev, stdCode).c_str());
}

void WtUftStraDemo::on_order(IUftStraCtx *ctx, uint32_t localid,
                             const char *stdCode, bool isLong, uint32_t offset,
...
void WtUftStraDemo::on_channel_ready(IUftStraCtx *ctx) {
  ...
  if (!decimal::eq(undone, 0) && _orders.empty()) {
    // 这说明有未完成单不在监控之中,先撤掉
    _ctx->stra_log_info(
        fmt::format("{}有不在管理中的未完成单 {} 手,全部撤销", _code, undone)
            .c_str());
...
void WtUftStraDemo::on_params_updated() {
  ...
  _ctx->stra_log_info(fmtutil::format(
      "[{}] Params updated, second: {}, freq: {}, offset: {}, lots: {}",
      _id.c_str(), _secs, _freq, _offset, _lots));
}